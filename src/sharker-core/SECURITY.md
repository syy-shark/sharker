...
# Sharker Security Policy

This document names the trust boundaries Sharker treats as load-bearing,
...
Report privately by emailing **security@sharker.app** (or open a
...
- Environment details (`sharker` version from the About page, OS,
...
Sharker is a single-tenant personal desktop AI assistant. It runs as
...
- **Agent process.** The Electron main process and any code it
  loads (`@sharker/core`, `@sharker/runtime`, `@sharker/storage`,
  `@sharker/ui`, builtin tools, user skills).
- **Renderer process.** Electron's sandboxed renderer. Receives
  data only through the preload IPC bridge in
  `apps/desktop/src/preload/preload.ts`.
- **Permission engine.** `@sharker/core/permission` evaluates each tool
...
- **Trust envelope.** The set of resources Sharker inherits from the
...
The OS user account is Sharker's outer trust envelope. Inside that envelope,
...
disabled, or external profiles do not add a Sharker-managed local sandbox;
...
1. **OS user account.** Tools run with the user's privileges. The
   user is expected to run Sharker as a non-admin account on systems
   where that matters.
...
4. **`PermissionMode.ask`** as default. Mode names are UX controls,
   ...
   Sharker-managed local sandbox; the active `ExecutionBoundary` is the
   execution authority.
...
Beyond security, Sharker treats the following as user-facing
privacy commitments:
