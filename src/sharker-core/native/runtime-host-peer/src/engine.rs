...
const APPLICATION_PROTOCOL: &str = "/sharker/runtime-host/peer/1";
const MESH_CONTROL_PROTOCOL: &str = "/sharker/runtime-host/mesh-control/1";
const IDENTIFY_PROTOCOL: &str = "/sharker/runtime-host/peer-identify/1";
...
pub fn start(options: StartOptions) -> Result<StartedEndpoint, PeerError> {
    ...
    let thread = thread::Builder::new()
        .name("sharker-runtime-host-peer".to_owned())
        .spawn(move || {
...
fn relay_excess_limit(maximum: usize) -> usize {
    ...
    // inclusive maximum exposed by Sharker.
    maximum
        .checked_sub(1)
...
fn discovery_debug(message: std::fmt::Arguments<'_>) {
    if std::env::var_os("SHARKER_PEER_DISCOVERY_DEBUG").is_some() {
...
mod tests {
    ...
    async fn identity_signature_is_bound_to_peer_and_payload() {
        let root = std::env::temp_dir().join(format!("sharker-peer-signature-{}", PeerId::random()));
    ...
    async fn mesh_control_survives_repeated_application_streams_on_one_endpoint() {
        let root = std::env::temp_dir().join(format!("sharker-peer-test-{}", PeerId::random()));
        std::fs::create_dir_all(&root).expect("create test root");
    ...
    async fn approved_peers_can_exchange_an_application_stream_through_transit() {
        let root = std::env::temp_dir().join(format!("sharker-peer-transit-{}", PeerId::random()));
        std::fs::create_dir_all(&root).expect("create test root");
    ...
    async fn transit_provider_can_be_bootstrapped_through_its_coordination_relay() {
        let root =
            std::env::temp_dir().join(format!("sharker-peer-transit-bootstrap-{}", PeerId::random()));
        std::fs::create_dir_all(&root).expect("create test root");
