...
const encodedRustflags = [
  ...
  ...(process.platform === 'win32' ? ['-Clink-arg=/PDBALTPATH:sharker_runtime_host_peer.pdb'] : []),
]
  .filter(Boolean)
  .join('\x1f');
const cargoSubcommand = process.env.SHARKER_RUNTIME_HOST_PEER_CARGO_SUBCOMMAND?.trim() || 'build';
const cargoTarget = process.env.SHARKER_RUNTIME_HOST_PEER_CARGO_TARGET?.trim();
await run(
  'cargo',
...
const library =
  targetPlatform === 'win32'
    ? 'sharker_runtime_host_peer.dll'
    : targetPlatform === 'darwin'
      ? 'libsharker_runtime_host_peer.dylib'
      : 'libsharker_runtime_host_peer.so';
const destination = process.env.SHARKER_RUNTIME_HOST_PEER_OUTPUT?.trim()
  ? resolve(process.env.SHARKER_RUNTIME_HOST_PEER_OUTPUT.trim())
  : join(root, 'target', 'release', 'sharker_runtime_host_peer.node');
await mkdir(dirname(destination), { recursive: true });
await copyFile(join(releaseDirectory, library), destination);
if (targetPlatform === 'darwin' && process.platform === 'darwin') {
  await run('install_name_tool', ['-id', '@rpath/sharker_runtime_host_peer.node', destination], root, {
    ...process.env,
  });
