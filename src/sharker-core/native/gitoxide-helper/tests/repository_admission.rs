...
const HELPER: &str = env!("CARGO_BIN_EXE_sharker-gitoxide-helper");
...
fn imports_an_exact_source_head_into_a_fresh_managed_repository() {
    ...
    fixture.git([
        ...
        "-c",
        "user.email=sharker@example.invalid",
        "commit",
        "-m",
    ...
    let output = invoke_request(serde_json::json!({
        ...
        "baselineRef": "refs/sharker/baseline",
    ...
    assert_eq!(
        git_bare_output(&destination, ["rev-parse", "refs/sharker/baseline"]),
        baseline_commit
    );
    assert_eq!(
        git_bare_output(
    ...
    assert!(
        baseline_commit_text
            .contains("author Sharker Workspace Service <workspace@sharker.invalid> 946684800 +0000")
    );
    assert!(
        baseline_commit_text
            .contains("committer Sharker Workspace Service <workspace@sharker.invalid> 946684800 +0000")
    );
    assert!(baseline_commit_text.ends_with("sharker managed workspace baseline v2"));
...
fn publishes_and_exactly_retries_an_operation_candidate_without_advancing_accepted() {
    ...
    let request = serde_json::json!({
        ...
        "acceptedRef": "refs/sharker/baseline",
        "expectedBaseCommitOid": baseline,
        "expectedBaseTreeOid": baseline_tree,
        "candidateRef": "refs/sharker/candidates/1111111111111111111111111111111111111111111111111111111111111111",
...
fn rejects_an_existing_candidate_receipt_with_an_unrelated_tree_change() {
    ...
    let imported: serde_json::Value = serde_json::from_slice(&imported.stdout).unwrap();
    let candidate_ref =
        "refs/sharker/candidates/1414141414141414141414141414141414141414141414141414141414141414";
    let request = serde_json::json!({
        "protocolVersion": 1,
        "operation": "create_candidate",
        "repositoryPath": destination,
...
fn rejects_an_existing_candidate_receipt_with_a_target_mode_flip() {
    ...
    let imported: serde_json::Value = serde_json::from_slice(&imported.stdout).unwrap();
    let candidate_ref =
        "refs/sharker/candidates/1515151515151515151515151515151515151515151515151515151515151515";
    let request = serde_json::json!({
        "protocolVersion": 1,
        "operation": "create_candidate",
        "repositoryPath": destination,
...
fn rejects_an_exact_candidate_retry_when_the_result_blob_is_missing() {
    ...
    let request = serde_json::json!({
        ...
        "expectedBaseCommitOid": imported["baselineCommitOid"],
        "expectedBaseTreeOid": imported["baselineTreeOid"],
        "candidateRef": "refs/sharker/candidates/1212121212121212121212121212121212121212121212121212121212121212",
        "path": "docs/result.txt",
...
fn rejects_an_exact_candidate_retry_when_the_result_blob_is_corrupt() {
    ...
    let request = serde_json::json!({
        ...
        "expectedBaseCommitOid": imported["baselineCommitOid"],
        "expectedBaseTreeOid": imported["baselineTreeOid"],
        "candidateRef": "refs/sharker/candidates/1313131313131313131313131313131313131313131313131313131313131313",
        "path": "docs/result.txt",
...
fn rejects_a_symbolic_candidate_ref_during_exact_retry() {
    ...
    let imported: serde_json::Value = serde_json::from_slice(&imported.stdout).unwrap();
    let candidate_ref =
        "refs/sharker/candidates/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
    let request = serde_json::json!({
        "protocolVersion": 1,
        "operation": "create_candidate",
        "repositoryPath": destination,
    ...
    let alias_ref = "refs/sharker/candidate-alias";
...
fn rejects_an_annotated_tag_candidate_ref_during_exact_retry() {
    ...
    let imported: serde_json::Value = serde_json::from_slice(&imported.stdout).unwrap();
    let candidate_ref =
        "refs/sharker/candidates/cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc";
    let request = serde_json::json!({
        "protocolVersion": 1,
        "operation": "create_candidate",
        "repositoryPath": destination,
...
fn rejects_a_symbolic_accepted_ref_before_candidate_creation() {
    ...
    let imported: serde_json::Value = serde_json::from_slice(&imported.stdout).unwrap();
    let baseline = imported["baselineCommitOid"].as_str().unwrap();
    let alias_ref = "refs/sharker/accepted-alias";
    git_bare_output(&destination, ["update-ref", alias_ref, baseline]);
    git_bare_output(
        &destination,
        ["symbolic-ref", "refs/sharker/baseline", alias_ref],
    );
    ...
    let rejected = invoke_request(serde_json::json!({
        ...
        "expectedBaseCommitOid": baseline,
        "expectedBaseTreeOid": imported["baselineTreeOid"],
        "candidateRef": "refs/sharker/candidates/dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd",
        "path": "docs/result.txt",
...
fn rejects_an_annotated_tag_accepted_ref_before_candidate_creation() {
    ...
    git_bare_output(&destination, ["update-ref", "refs/sharker/baseline", &tag_oid]);
    ...
    let rejected = invoke_request(serde_json::json!({
        ...
        "expectedBaseCommitOid": baseline,
        "expectedBaseTreeOid": imported["baselineTreeOid"],
        "candidateRef": "refs/sharker/candidates/eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee",
        "path": "docs/result.txt",
...
fn publishes_an_operation_bound_no_change_receipt() {
    ...
    let baseline_tree = imported["baselineTreeOid"].as_str().unwrap();
    let candidate_ref =
        "refs/sharker/candidates/4444444444444444444444444444444444444444444444444444444444444444";
...
fn rejects_a_second_terminal_interpretation_for_the_same_operation_ref() {
    ...
    let baseline_tree = imported["baselineTreeOid"].as_str().unwrap();
    let candidate_ref =
        "refs/sharker/candidates/7777777777777777777777777777777777777777777777777777777777777777";
    ...
    let reverse_candidate_ref =
        "refs/sharker/candidates/9999999999999999999999999999999999999999999999999999999999999999";
...
fn concurrent_exact_candidate_retry_converges_to_one_commit() {
    ...
    let request = serde_json::json!({
        ...
        "expectedBaseCommitOid": imported["baselineCommitOid"],
        "expectedBaseTreeOid": imported["baselineTreeOid"],
        "candidateRef": "refs/sharker/candidates/8888888888888888888888888888888888888888888888888888888888888888",
        "path": "docs/result.txt",
...
fn concurrent_conflicting_candidate_requests_have_one_stable_winner() {
    ...
    let request = |content_base64: &str| {
        serde_json::json!({
            ...
            "expectedBaseCommitOid": imported["baselineCommitOid"],
            "expectedBaseTreeOid": imported["baselineTreeOid"],
            "candidateRef": "refs/sharker/candidates/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
            "path": "docs/result.txt",
            "contentBase64": content_base64,
            "managedTreePolicyVersion": 3,
        })
...
fn rejects_a_stale_candidate_before_publishing_its_ref() {
    ...
    let _ = git_bare_output(
        &destination,
        ["update-ref", "refs/sharker/baseline", &advanced],
    );
    ...
    let rejected = invoke_request(serde_json::json!({
        ...
        "expectedBaseCommitOid": baseline,
        "expectedBaseTreeOid": baseline_tree,
        "candidateRef": "refs/sharker/candidates/2222222222222222222222222222222222222222222222222222222222222222",
    ...
    assert!(!git_bare_succeeds(
        ...
        [
            "rev-parse",
            "refs/sharker/candidates/2222222222222222222222222222222222222222222222222222222222222222"
        ]
    ));
}
...
fn validates_the_complete_candidate_tree_before_candidate_ref_publication() {
    ...
    let rejected = invoke_request(serde_json::json!({
        ...
        "expectedBaseCommitOid": baseline,
        "expectedBaseTreeOid": baseline_tree,
        "candidateRef": "refs/sharker/candidates/3333333333333333333333333333333333333333333333333333333333333333",
        "path": ".gitattributes",
...
fn rejects_hfs_protected_candidate_paths_before_object_or_ref_publication() {
    ...
    let baseline_tree = imported["baselineTreeOid"].as_str().unwrap();
    let candidate_ref =
        "refs/sharker/candidates/6666666666666666666666666666666666666666666666666666666666666666";
...
fn rejects_a_candidate_when_the_exact_base_tree_storage_is_corrupt() {
    ...
    let candidate_ref =
        "refs/sharker/candidates/5555555555555555555555555555555555555555555555555555555555555555";
...
#[test]
fn imports_sharker_attributes_under_managed_tree_policy_v3() {
    let fixture = RepositoryFixture::sha1_with_commit();
    fs::write(
        fixture.root.join(".gitattributes"),
        b"* text=auto eol=lf\n/.claude export-ignore\n/.sharker-shots export-ignore\n/sharker-proposal-zh-review.txt export-ignore\n",
    )
    .unwrap();
...
fn rejects_a_foreign_bare_destination_without_modifying_it() {
    ...
    let output = invoke_request(serde_json::json!({
        ...
        "expectedSourceHeadCommitOid": source_head,
        "destinationRepositoryPath": destination,
        "baselineRef": "refs/sharker/accepted",
        "managedTreePolicyVersion": 3,
    }));
    ...
    assert!(!git_bare_succeeds(
        &destination,
        ["show-ref", "--verify", "refs/sharker/accepted"]
    ));
}
...
impl RepositoryFixture {
    ...
    fn sha1_with_raw_tree(entries: &[RawTreeEntry<'_>]) -> (Self, String) {
        ...
        let commit = format!(
            "tree {root_tree_oid}\nauthor Sharker Test <sharker@example.invalid> 946684800 +0000\ncommitter Sharker Test <sharker@example.invalid> 946684800 +0000\n\nraw tree fixture\n"
        );
    ...
    fn init(object_format: &str) -> Self {
        ...
        let root = std::env::temp_dir().join(format!(
            "sharker-gitoxide-helper-admission-{}-{nonce}-{}",
            std::process::id(),
