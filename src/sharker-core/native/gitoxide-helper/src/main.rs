...
const HELPER_ERROR_REASONS_V1: &[&str] = &[
    ...
    "baseline_ref_outside_sharker_namespace",
    ...
    "target_ref_outside_sharker_namespace",
...
fn import_source_head(
    ...
    if !baseline_ref.starts_with("refs/sharker/") {
        return Err("baseline_ref_outside_sharker_namespace");
    }
    ...
    let signature = gix::actor::SignatureRef {
        name: b"Sharker Workspace Service".as_bstr(),
        email: b"workspace@sharker.invalid".as_bstr(),
        time: "946684800 +0000",
    };
    let baseline_commit = destination
        .new_commit_as(
            signature,
            signature,
            "sharker managed workspace baseline v2",
            source_tree,
    ...
    destination
        .reference(
            ...
            "sharker managed workspace baseline",
        )
...
fn create_candidate(
    ...
    if !accepted_ref.starts_with("refs/sharker/") {
        return Err("target_ref_outside_sharker_namespace");
    }
    gix::refs::FullName::try_from(accepted_ref.as_str())
        .map_err(|_| "target_ref_outside_sharker_namespace")?;
    if !candidate_ref.starts_with("refs/sharker/candidates/") {
        return Err("target_ref_outside_sharker_namespace");
    }
    ...
    let commit_message =
        format!("sharker managed workspace candidate v3\nrequest-sha256 {request_digest_sha256}");
        ...
        .map_err(|_| "target_ref_unavailable")?
    {
        ...
        None => match repository.reference(
            ...
            successor_commit,
            gix::refs::transaction::PreviousValue::MustNotExist,
            "sharker managed workspace candidate",
        ) {
            Ok(_) => {}
            Err(_) => {
...
fn has_managed_candidate_signature(commit: &gix::objs::Commit) -> bool {
    let expected_name = b"Sharker Workspace Service";
    let expected_email = b"workspace@sharker.invalid";
...
fn candidate_request_digest_sha256(
    ...
    hasher.update(b"sharker.gitoxide.candidate-request.v1\0");
    for field in [
