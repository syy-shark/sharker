...
<h1 align="center">
  <img src="apps/desktop/assets/app-icons/sky.png" alt="Sharker" width="72" valign="middle" /> Apache Sharker (Incubating)
</h1>

<p align="center"><sub>Incubating at The Apache Software Foundation</sub></p>

<p align="center">
  <a href="https://github.com/apache/sharker/stargazers"><img src="https://img.shields.io/github/stars/apache/sharker?style=flat&label=%E2%98%85&color=4C8DFF" alt="GitHub stars" /></a>
  ...
  <a href="https://deepwiki.com/apache/sharker"><img src="https://img.shields.io/badge/DeepWiki-third--party%20AI%20docs-9BB8F0?style=flat" alt="DeepWiki: third-party AI-generated docs" /></a>
...
- **Shorter context is not deleted history.** Sharker can omit old tool output from the next prompt without throwing away the saved evidence.
- **One place runs the agent.** Desktop, the terminal, and Sharker evaluation all go through Runtime Host. Eval only owns the experiment and its scores.

Read [Sharker Backend Architecture](./ARCHITECTURE.md) for the design.
...
| **TUI / CLI** | Using Sharker in the current project directory or running one non-interactive Turn | `sharker`, `sharker run`; shares workspace and model connections with Desktop |
| **Eval** | Reproducible benchmark experiments across Sharker and external subjects | `sharker eval run <spec> --out <directory>` |
...
- Sharker subjects execute only through Runtime Host; external subjects use generic external subject adapters.

## Quick start

### Releases and downloads

Apache Sharker has not made an Apache release yet. Everything currently published from this repository or from a package registry was produced before or during incubation, is not an Apache Software Foundation release, and has not been reviewed or voted on by the Incubator PMC.
...
```sh
git clone https://github.com/apache/sharker.git
cd sharker
npm ci
npm run dev
```
...
Sharker does not bundle a shared model account. On first launch:
...
The repository CLI uses the same `Sharker Dev` profile as a development Desktop build. The
released `sharker` binary continues to use the `Sharker` profile; the two profiles are not copied or
...
Experiment → Cells → Attempts → Results
                    ↓
       Runtime Host executes Sharker subjects
```
...
- Resuming an interrupted turn is off by default. Set `SHARKER_RUNTIME_SAFE_BOUNDARY_RESUME=1` only if you want Desktop **Safe resume**, CLI `/resume`, and startup auto-resume — those calls hit the model and use tokens.
...
```sh
npm --workspace @sharker/runtime test
npm --workspace @sharker/eval test
...
npm --workspace @sharker/core test
```

Desktop real-window and visual verification:

```sh
npm --workspace @sharker/desktop run e2e
...
Sharker is licensed under the [Apache License 2.0](./LICENSE). See
[NOTICE](./NOTICE) for attribution information. Third-party components remain
subject to their respective licenses and notices.

Apache Sharker, Sharker, Apache, the Apache feather, and the Apache Sharker project logo are either registered trademarks or trademarks of The Apache Software Foundation.
