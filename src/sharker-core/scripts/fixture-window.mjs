...
const SETTLE_EXPR = `(async () => {
  if (!document.getElementById('sharker-contract-freeze')) {
    const style = document.createElement('style');
    style.id = 'sharker-contract-freeze';
 ...
 * an expression.
 *
 * @param {string} scenario SHARKER_E2E_FIXTURE scenario name.
 * @param {{
 *   theme?: 'light' | 'dark',
...
export async function withFixtureWindow(scenario, options, fn) {
  ...
  const userDataDir = await mkdtemp(join(tmpdir(), 'sharker-fixture-'));
