...
describe('ASF source release identity', () => {
  test('uses the incubating source distribution name for plain versions only', () => {
    assert.deepEqual(sourceCandidateIdentity('0.1.12'), {
      archiveName: 'apache-sharker-0.1.12-incubating-src.tar.gz',
      rootDirectory: 'apache-sharker-0.1.12-incubating',
      version: '0.1.12',
    });
...
describe('ASF source release verification', () => {
  ...
  test('requires a signature before reading unauthenticated candidate metadata', async () => {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'sharker-asf-auth-order-test-'));
  ...
  test('documents the checkout-only DeepSeek Harness toolchain build in the candidate', async () => {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'sharker-asf-dsh-contract-test-'));
  ...
  test('creates reproducible candidates from committed files only', async () => {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'sharker-asf-source-test-'));
    const repositoryRoot = join(temporaryRoot, 'repository');
    ...
    try {
      ...
      mkdirSync(join(repositoryRoot, '.sharker-shots'));
      writeFileSync(join(repositoryRoot, '.claude/launch.json'), '{}\n');
      writeFileSync(join(repositoryRoot, '.sharker-shots/review.png'), 'review evidence\n');
      writeFileSync(join(repositoryRoot, 'sharker-proposal-zh-review.txt'), 'working notes\n');
      mkdirSync(join(repositoryRoot, 'packages/eval/harbor/deepseek-harness-toolchain'), {
        recursive: true,
      });
      mkdirSync(join(repositoryRoot, 'packages/eval/harbor/deepseek-harness-profile'), {
      ...
      git(repositoryRoot, [
        ...
        'README.md',
        '.claude/launch.json',
        '.sharker-shots/review.png',
        'sharker-proposal-zh-review.txt',
      ...
      assert.doesNotMatch(entries, /\.claude|\.sharker-shots|sharker-proposal-zh-review/);
      assert.doesNotMatch(entries, /deepseek-harness-toolchain\/package(?:-lock)?\.json/);
  ...
  test('does not let ambient tar options control archive verification', async () => {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'sharker-asf-tar-options-test-'));
    const identity = sourceCandidateIdentity('0.1.12');
...
function writeReleaseContents(root, { includeAttributes = false } = {}) {
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ license: 'Apache-2.0', name: 'sharker', private: true, version: '0.1.12' }, null, 2)}\n`,
  );
  writeFileSync(
    join(root, 'package-lock.json'),
    `${JSON.stringify({ lockfileVersion: 3, name: 'sharker', packages: { '': { name: 'sharker', version: '0.1.12' } }, version: '0.1.12' }, null, 2)}\n`,
  );
  writeFileSync(
    join(root, 'DISCLAIMER-WIP'),
    'Apache Sharker is undergoing incubation at The Apache Software Foundation.\n',
  );
  writeFileSync(join(root, 'LICENSE'), 'Apache License, Version 2.0\n');
  writeFileSync(join(root, 'NOTICE'), 'Apache Sharker\n');
...
function createFixtureCandidate(files = {}) {
  const temporaryRoot = mkdtempSync(join(tmpdir(), 'sharker-asf-candidate-test-'));
  const identity = sourceCandidateIdentity('0.1.12');
  const sourceRoot = join(temporaryRoot, identity.rootDirectory);
  const archivePath = join(temporaryRoot, identity.archiveName);
  writeReleaseContents(sourceRoot);
