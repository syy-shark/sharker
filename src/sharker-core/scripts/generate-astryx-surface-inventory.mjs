 ...
 *
 * Both the known Astryx component set and the `@sharker/ui` re-export map are
...
function collectModuleExports(file, components, memo, seen) {
  ...
  for (const m of code.matchAll(/export\s*\*\s*from\s*['"]([^'"]+)['"]/g)) {
    ...
    if (spec.startsWith('@astryxdesign/core')) {
      ...
      if (!target) {
        throw new Error(
          `cannot resolve @sharker/ui exports: unresolved \`export * from '${spec}'\` in ${relative(root, file)}`,
        );
      }
...
const sharkerUiBarrelCache = new Map();

/**
 * Resolve the public `@sharker/ui` surface (`packages/ui/src/index.ts`) into:
 ...
 */
export function loadSharkerUiBarrel(repoRoot = root) {
  if (sharkerUiBarrelCache.has(repoRoot)) return sharkerUiBarrelCache.get(repoRoot);
  const { components } = loadAstryxComponents();
  const indexFile = join(repoRoot, 'packages/ui/src/index.ts');
  if (!existsSync(indexFile)) {
    throw new Error(`cannot resolve @sharker/ui exports: ${relative(repoRoot, indexFile)} not found`);
  }
  ...
  sharkerUiBarrelCache.set(repoRoot, result);
  return result;
}
 ...
 * - import { Badge } from '@sharker/ui' when Badge is a derived Astryx re-export
 * - <Button / <UiButton only counts if bound from those imports
 */
function collectAstryxUsage(code, astryxComponents, sharkerUiReexports) {
  const localToCanonical = new Map(); // local binding → canonical Astryx name

  // import { A, B as C } from '@astryxdesign/core...'  or from the '@sharker/ui' root barrel
  const importBlockRe =
    /import\s*\{([^}]+)\}\s*from\s*['"](@astryxdesign\/core(?:\/[^'"]+)?|@sharker\/ui)['"]/g;
  let im;
  while ((im = importBlockRe.exec(code)) !== null) {
    ...
    for (const part of spec.split(',')) {
      ...
      } else if (!isAstryxPkg && sharkerUiReexports.has(original)) {
        localToCanonical.set(local, sharkerUiReexports.get(original));
      }
    }
  }
...
export function analyzeTsx(rel, text, ctx) {
  const { astryxComponents, sharkerUiReexports, shadowNames } = ctx;
  const code = stripComments(text);
  const named = collectAstryxUsage(code, astryxComponents, sharkerUiReexports);
...
function analyze(repoRoot, rel, ctx) {
  ...
  const a = analyzeTsx(rel, text, {
    ...
    sharkerUiReexports: ctx.sharkerUiReexports,
...
export function renderAstryxSurfaceInventory(repoRoot = root) {
  ...
  const { reexports, shadowsByFile } = loadSharkerUiBarrel(repoRoot);
  const ctx = { astryxComponents: components, sharkerUiReexports: reexports, shadowsByFile };
  ...
  lines.push(
    '- **reimplementation** — a public `@sharker/ui` export shadows a shipped Astryx component (same name, not a re-export). A neutral signal for review, not proof of a semantic re-implementation.',
  );
  lines.push(
