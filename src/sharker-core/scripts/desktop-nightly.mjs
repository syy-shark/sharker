...
export function resolveRuntimeHostSetupPackage(productVersion, environment = process.env) {
  return `sharker-agent@${resolveDesktopBuildVersion(productVersion, environment)}`;
}
...
export function desktopNightlyReleaseAssetNames(version) {
  ...
  return [
    ...
    names.windowsZip,
    `Sharker-${version}-attestation.sigstore.json`,
    'dev-mac.yml',
    'dev.yml',
  ].sort();
}
...
function nightlyArtifactNames(version) {
  return {
    macZip: `Sharker-${version}-mac-arm64.zip`,
    macDmg: `Sharker-${version}-mac-arm64.dmg`,
    windowsExe: `Sharker-${version}-win-x64.exe`,
    windowsZip: `Sharker-${version}-win-x64.zip`,
  };
}
