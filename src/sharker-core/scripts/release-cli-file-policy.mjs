...
const LOCAL_PACKAGE_PREFIX = '@sharker/';
...
export function resolveReleaseWorkspacePackages(repoRoot, entryName = 'sharker-agent') {
...
// Sharker-owned convention, so they are not part of DEVELOPMENT_DIRECTORIES: a
// third-party package is free to ship a directory by that name.
const SHARKER_TEST_ONLY_DIRECTORY = 'test-only';

export function isSharkerDevelopmentArtifact(relativePath) {
  const segments = relativePath.split(/[\\/]/).filter(Boolean);
  if (segments.some((segment) => segment === 'src')) return true;
  if (segments.some((segment) => segment === SHARKER_TEST_ONLY_DIRECTORY)) return true;
  if (segments.some((segment) => DEVELOPMENT_DIRECTORIES.has(segment))) return true;
...
function releaseExportTarget(target) {
  if (typeof target === 'string') {
    return isSharkerDevelopmentArtifact(target) ? undefined : target;
  }
