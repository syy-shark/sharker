...
test('CI installs dependencies whenever the Astryx surface inventory runs', () => {
  ...
  // and parses the @sharker/ui barrel. An inventory-doc-only PR is `astryx_surface`
...
test('core CI runs the live Eval proxy lifecycle when Eval is selected', () => {
  ...
  assert.match(workflow, /SHARKER_EVAL_EGRESS_PROXY_TEST: '1'/u);
  assert.match(workflow, /docker build[\s\S]*sharker-eval-egress-proxy:12\.2\.3/u);
  assert.match(workflow, /npm --workspace @sharker\/eval run test:egress-proxy:live/u);
  assert.equal(
