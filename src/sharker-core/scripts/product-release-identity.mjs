...
export function resolveProductManifestIdentity({ rootManifest, desktopManifest, cliManifest }) {
  ...
  if (JSON.stringify(cliManifest.bin) !== JSON.stringify({ sharker: './dist/cli.js' })) {
    throw new Error('The only public CLI command must be sharker');
  }

  return {
    version,
    runtimeHostSetupPackage: `sharker-agent@${version}`,
    publicCommands: ['sharker'],
  };
}
...
export function resolveProductReleaseIdentity({
  ...
  const dmg = `Sharker-${version}-mac-arm64.dmg`;
  const macZip = `Sharker-${version}-mac-arm64.zip`;
  const exe = `Sharker-${version}-win-x64.exe`;
  const windowsZip = `Sharker-${version}-win-x64.zip`;
  const cliArchive = `Sharker-${version}-cli-mac-arm64.zip`;
  const artifacts = {
