...
const cliPackageName = 'sharker-agent';
...
export function resolveMacosArm64CliArtifactPaths(version) {
  const archiveName = `Sharker-${version}-cli-mac-arm64.zip`;
  return {
    archiveRootName: `Sharker-${version}-cli-mac-arm64`,
...
SHARKER_EVAL_SHARKER_BUNDLE_PATH=$libexec_dir
export SHARKER_EVAL_SHARKER_BUNDLE_PATH
exec "$libexec_dir/node/bin/node" "$libexec_dir/node_modules/sharker-agent/dist/cli.js" "$@"
`;
}
      ...
      await Promise.all(
        resolveWorkspaceReleaseFiles(directory, manifest).map(async (releaseFile) => {
          ...
          await cp(source, target, {
            recursive: true,
            filter: (path) => !isSharkerDevelopmentArtifact(relative(directory, path)),
          });
        }),
      );
    }),
...
export async function createSigningKeychain({ env, expectedTeamIdentifier, run, inspect }) {
  const temporaryRoot = await mkdtemp(join(tmpdir(), 'sharker-cli-signing-'));
...
export async function packageMacosArm64Cli({
  ...
  releaseSigning = env.SHARKER_CLI_RELEASE_SIGNING === '1',
  nodeArchivePath = env.SHARKER_CLI_NODE_ARCHIVE,
} = {}) {
  ...
  if (!nodeArchivePath) {
    throw new Error(`Set SHARKER_CLI_NODE_ARCHIVE to the verified ${identity.nodeArchive} path.`);
  }
  ...
  const stagingRoot = await mkdtemp(join(tmpdir(), 'sharker-cli-'));
  let complete = false;
  ...
  try {
    ...
    await Promise.all([
      ...
      writeFile(join(binDirectory, 'sharker'), macosArm64CliWrapper(), 'utf8'),
      writeFile(
        join(archiveRoot, 'README.txt'),
        [
          `Sharker CLI/TUI ${version} for Apple Silicon macOS`,
          '',
          "Add this directory's bin folder to PATH, then run:",
          '  sharker --help',
          '',
          'The archive includes its own Node.js runtime and does not require the Sharker desktop app.',
          '',
        ].join('\n'),
        'utf8',
      ),
    ...
    await Promise.all([
      ...
      chmod(join(binDirectory, 'sharker'), 0o755),
    ]);
