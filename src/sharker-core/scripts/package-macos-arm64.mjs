...
export async function packageMacosArm64({
  ...
  const dmgPath = join(releaseDirectory, `Sharker-${buildVersion}-mac-arm64.dmg`);
  const zipPath = join(releaseDirectory, `Sharker-${buildVersion}-mac-arm64.zip`);
  ...
  await run('npm', ['--workspace', '@sharker/desktop', 'run', 'package:macos-arm64']);
