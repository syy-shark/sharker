...
// The environment a Sharker fixture window launches into.
//
 ...
 *     under test;
 *   - any `SHARKER_E2E_*` the operator's shell happens to export, which silently
...
export function buildFixtureEnv(userDataDir, homeDir, options = {}) {
  ...
  // under xvfb).
  env.SHARKER_SKIP_SHELL_ENV = '1';
  env.SHARKER_E2E_USER_DATA_DIR = userDataDir;
  // Sandbox the home directory. User-scope skill discovery reads
  // `~/.sharker/skills` and `~/.agents/skills` via `os.homedir()`, and the Skills
  ...
  if (options.scenario) env.SHARKER_E2E_FIXTURE = options.scenario;
  if (options.locale) env.SHARKER_E2E_FIXTURE_LOCALE = options.locale;
  if (options.platform) env.SHARKER_E2E_FIXTURE_PLATFORM = options.platform;
  if (options.theme) env.SHARKER_E2E_FIXTURE_THEME = options.theme;
  ...
  if (options.timezone) env.SHARKER_E2E_FIXTURE_TIMEZONE = options.timezone;
  ...
  if (options.scrollMotion) env.SHARKER_E2E_FIXTURE_SCROLL_MOTION = options.scrollMotion;
  if (options.showWindow) env.SHARKER_E2E_SHOW_WINDOW = '1';
  return env;
}
