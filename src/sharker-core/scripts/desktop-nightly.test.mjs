...
test('a nightly package embeds only the Apache GitHub dev update authority', () => {
  ...
  const config = resolveDesktopBuilderConfig({
    SHARKER_DESKTOP_NIGHTLY_VERSION: version,
  });

  assert.equal(config.extraMetadata.version, version);
  assert.equal(config.extraMetadata.runtimeHostSetupPackage, `sharker-agent@${version}`);
  assert.equal(config.extraMetadata.sharkerUpdateChannel, 'nightly');
...
test('the Windows Nightly wrapper accepts dev update metadata', async () => {
  ...
  await packageWindowsX64({
    platform: 'win32',
    arch: 'x64',
    env: { SHARKER_DESKTOP_NIGHTLY_VERSION: version },
    run: async () => {},
    remove: async () => {},
    makeDirectory: async () => {},
    copy: async () => {},
...
repo: sharker
channel: dev
updaterCacheDirName: '@sharkerdesktop-updater'
`;
  ...
  for (const { platform, channel, name } of [
    {
      platform: 'darwin',
      channel: 'dev-mac',
      name: `Sharker-${version}-mac-arm64.zip`,
    },
    { platform: 'win32', channel: 'dev', name: `Sharker-${version}-win-x64.exe` },
  ]) {
    const provider = new GitHubProvider(
      { provider: 'github', owner: 'apache', repo: 'sharker', channel: 'dev' },
      {
    ...
    assert.equal(
      resolved.url.href,
      `https://github.com/apache/sharker/releases/download/v${version}/${name}`,
    );
  }
});
...
test('GitHub differential updates derive the previous blockmap from the previous versioned tag', () => {
  ...
  const currentAsset = new URL(
    `https://github.com/apache/sharker/releases/download/v${current}/Sharker-${current}-win-x64.exe`,
  );
  ...
  assert.equal(
    oldBlockmap.href,
    `https://github.com/apache/sharker/releases/download/v${previous}/Sharker-${previous}-win-x64.exe.blockmap`,
  );
...
test('formal release checks ignore the ambient Nightly packaging environment', async () => {
  ...
  await run(
    ...
    ],
    {
      ...
      env: {
        ...environment,
        SHARKER_DESKTOP_NIGHTLY_VERSION: '0.2.0-dev.42.20260829',
      },
    },
  );
});
...
test('packaging observes a valid nightly version without changing product manifests', () => {
  ...
  assert.equal(
    ...
    'sharker-agent@0.2.0-dev.42.20260829',
  );
});
