...
const { terminateChildProcessTree } = await import('@sharker/runtime/process-tree-terminator').catch(
  (cause) => {
    throw new Error(
      "electron-lifecycle needs @sharker/runtime's built dist. Run `npm --workspace @sharker/runtime run build` (or any desktop build) first.",
      { cause },
    );
  },
);
