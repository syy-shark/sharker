...
const sandboxResourcePath = join(sandboxResourceDirectory, 'sharker-windows-sandbox.exe');
...
export async function packageWindowsX64({
  ...
  const exePath = join(releaseDirectory, `Sharker-${buildVersion}-win-x64.exe`);
  const zipPath = join(releaseDirectory, `Sharker-${buildVersion}-win-x64.zip`);
  ...
  await run('npm', ['--workspace', '@sharker/desktop', 'run', 'package:windows-x64']);
