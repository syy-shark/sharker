...
export async function verifyMacosAutoupdate(
  ...
  nextDirectoryInput,
  {
    ...
    arch = process.arch,
    run = runCommand,
    makeTemporaryDirectory = () => mkdtemp(join(tmpdir(), 'sharker-macos-autoupdate-')),
  ...
  const candidateApp = join(candidateRoot, 'Sharker.app');
  const nextApp = join(nextRoot, 'Sharker.app');
  let executable = join(candidateApp, 'Contents', 'MacOS', 'Sharker');
  let feed;
  let child;
  let relaunchedPid;
  let primaryError;
  ...
  try {
    ...
    if (packagedManifest(candidateApp).sharkerUpdateTestProfile !== undefined) {
      throw new Error('The production candidate must not carry the update-test profile marker.');
    }
    if (packagedManifest(nextApp).sharkerUpdateTestProfile !== true) {
    ...
    const userData = join(candidateRoot, '.sharker-update-test-user-data');
    await Promise.all([mkdir(home), mkdir(userData)]);
    const childEnv = {
      ...process.env,
      ...isolatedUserEnv(home),
      SHARKER_SKIP_SHELL_ENV: '1',
      SHARKER_UPDATE_TEST_FEED: feed.url,
      SHARKER_UPDATE_TEST_USER_DATA_DIR: userData,
    };
    delete childEnv.SHARKER_UPDATE_MOCK_VERSION;
    delete childEnv.SHARKER_UPDATE_MOCK_STATE;
    ...
    await evaluateInRenderer(target.webSocketDebuggerUrl, 'window.sharker.app.checkForUpdates()', {
      awaitPromise: true,
      timeoutMs: 30_000,
    });
    ...
    for (;;) {
      status = await evaluateInRenderer(
        ...
        'window.sharker.app.updateStatus()',
        { awaitPromise: true },
      );
    ...
    try {
      const result = await evaluateInRenderer(
        target.webSocketDebuggerUrl,
        'window.sharker.app.installUpdate({ allowInterruptActiveTasks: true })',
