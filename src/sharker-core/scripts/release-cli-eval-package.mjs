...
const root = mkdtempSync(join(tmpdir(), 'sharker-cli-eval-validation-'));
let primaryError;
try {
  validateChecksum();
  ...
  const sharker = join(prefix, 'bin/sharker');
  const packageRoot = join(prefix, 'lib/node_modules/sharker-agent');
  if (
    !existsSync(sharker) ||
    !existsSync(join(packageRoot, 'node_modules/@sharker/eval/harbor/run_trial.py'))
  ) {
...
function validateFramework({ environment, fixture, framework, sharker, root: frameworkRoot }) {
  ...
  const pythonVariable =
    framework === 'harbor' ? 'SHARKER_RELEASE_HARBOR_PYTHON' : 'SHARKER_RELEASE_PIER_PYTHON';
  const pythonPath = process.env[pythonVariable];
  if (!pythonPath) throw new Error(`${pythonVariable} is required`);
  const pythonPathVariable = `SHARKER_RELEASE_${framework.toUpperCase()}_PYTHON_PATH`;
  const trialsVariable = `SHARKER_RELEASE_${framework.toUpperCase()}_TRIALS`;
  const tasksVariable = 'SHARKER_RELEASE_PIER_TASKS';
