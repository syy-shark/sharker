...
test('one root version defines every product artifact from one source commit', () => {
  const identity = resolveProductReleaseIdentity({
    ...
    cliManifest: { version: '1.2.3', bin: { sharker: './dist/cli.js' } },
    sha: 'a'.repeat(40),
  ...
  assert.equal(identity.runtimeHostSetupPackage, 'sharker-agent@1.2.3');
  assert.deepEqual(identity.publicCommands, ['sharker']);
  ...
  assert.equal(identity.dmg, 'Sharker-1.2.3-mac-arm64.dmg');
  assert.equal(identity.exe, 'Sharker-1.2.3-win-x64.exe');
  assert.equal(identity.cliArchive, 'Sharker-1.2.3-cli-mac-arm64.zip');
  assert.equal(Object.hasOwn(identity, 'sourceArchive'), false);
  assert.deepEqual(identity.artifacts, {
    'desktop-macos': [
      'Sharker-1.2.3-mac-arm64.dmg',
      'Sharker-1.2.3-mac-arm64.dmg.sha256',
      'Sharker-1.2.3-mac-arm64.zip',
      'Sharker-1.2.3-mac-arm64.zip.blockmap',
      'latest-mac.yml',
    ],
    'desktop-windows': [
      'Sharker-1.2.3-win-x64.exe',
      'Sharker-1.2.3-win-x64.exe.blockmap',
      'Sharker-1.2.3-win-x64.exe.sha256',
      'Sharker-1.2.3-win-x64.zip',
      'Sharker-1.2.3-win-x64.zip.sha256',
      'latest.yml',
    ],
    'cli-macos-arm64': ['Sharker-1.2.3-cli-mac-arm64.zip', 'Sharker-1.2.3-cli-mac-arm64.zip.sha256'],
  });
});

test('the standalone launcher identifies its installed Eval bundle root', async (t) => {
  const archiveRoot = await mkdtemp(join(tmpdir(), 'sharker-standalone-launcher-'));
  t.after(() => rm(archiveRoot, { recursive: true, force: true }));
  const launcher = join(archiveRoot, 'bin', 'sharker');
...
[ "$SHARKER_EVAL_SHARKER_BUNDLE_PATH" = "$expected" ]
printf verified
`,
    ),
  ]);
    ...
    assert.throws(
      () =>
        resolveProductManifestIdentity({
          ...
          cliManifest: { version, bin: { sharker: './dist/cli.js' } },
        }),
...
test('Desktop packaging derives the Runtime Host setup package from product manifests', async () => {
  ...
  assert.equal(manifestIdentity.runtimeHostSetupPackage, 'sharker-agent@1.2.3');
  ...
  assert.deepEqual(desktopBuilderConfig.extraMetadata, {
    runtimeHostSetupPackage: `sharker-agent@${checkedRootManifest.version}`,
    sharkerUpdateChannel: 'release',
  });
  assert.deepEqual(desktopBuilderConfig.publish, [
    { provider: 'github', owner: 'apache', repo: 'sharker' },
  ]);
});

test('Desktop stages release manifests before electron-builder builds the archive', async (t) => {
  const stage = await mkdtemp(join(tmpdir(), 'sharker-desktop-release-manifests-'));
  ...
  assert.ok(
    files[0].filter.includes('!node_modules/@sharker/{mcp,runtime,runtime-host}/package.json'),
  );
  assert.deepEqual(files.at(-1), { from: stage, to: 'node_modules/@sharker' });
...
test('a successful Windows upgrade invalidates stale backup authority before best-effort cleanup', async () => {
  ...
  const markerInvalidation = cleanup.indexOf('Delete "$sharkerBackupDir\\${SHARKER_BACKUP_MARKER}"');
  const backupRemoval = cleanup.indexOf('RMDir /r "$sharkerBackupDir"');
  const snapshotRemoval = cleanup.indexOf('DeleteRegKey SHELL_CONTEXT "${SHARKER_SNAPSHOT_REG_KEY}"');
...
test('platform package verifiers keep Git checks out of current artifacts', async () => {
  ...
  assert.match(
    windowsSource,
    /if \(requiresCurrentContract\) \{\s*await assertPackagedUpdateConfiguration\(resources, \{\s*channel: environment\.SHARKER_DESKTOP_NIGHTLY_VERSION \? ['"]nightly['"] : ['"]release['"],\s*\}\);\s*await assertPackagedDependencyClosure\(resources\);\s*\}\s*else await requirePath\(join\(resources, ['"]git['"]/u,
  );
...
test('the packaged-app probe rejects a mismatched Runtime Host setup package', async () => {
  const fixture = await mkdtemp(join(tmpdir(), 'sharker-packaged-manifest-'));
  try {
    ...
    await Promise.all([
      writeFile(manifestPath, JSON.stringify({ runtimeHostSetupPackage: 'sharker-agent@1.2.3' })),
      writeFile(
        join(ptyDirectory, 'index.js'),
        `module.exports = { spawn() { return {
  onData(listener) { queueMicrotask(() => listener('sharker-node-pty-ok')); },
  onExit(listener) { setImmediate(() => listener({ exitCode: 0 })); },
    ...
    await execFileAsync(process.execPath, [
      '-e',
      makePtyProbe('/bin/echo', ['sharker-node-pty-ok'], 'sharker-agent@1.2.3'),
      manifestPath,
    ]);
    await assert.rejects(
      execFileAsync(process.execPath, [
        '-e',
        makePtyProbe('/bin/echo', ['sharker-node-pty-ok'], 'sharker-agent@1.2.4'),
        manifestPath,
      ]),
...
test('product tag creation is exact and idempotent but rejects a conflicting commit', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-product-tag-'));
  ...
  try {
    ...
    await execFileAsync('git', ['config', 'user.name', 'Sharker release test'], { cwd: source });
...
test('standalone verification recognizes the current TUI status line through ANSI output', () => {
  assert.equal(
    isTuiReadyOutput(
      '\u001b[1mSharker\u001b[22m\u001b[2m · \u001b[22m\u001b[2mAuto\u001b[22m\u001b[2m · model · provider\u001b[0m',
    ),
    true,
  );
});
...
test('standalone verification assesses the downloaded quarantine state', async () => {
  ...
  await verifyQuarantinedExecution(
    '/artifact/Sharker',
    '/artifact/Sharker/libexec/node/bin/node',
    async (command) => {
...
test('CLI signing accepts one base64 PKCS12 and one isolated Developer ID identity', () => {
  ...
  assert.deepEqual(
    parseDeveloperIdApplicationIdentity(
      '  1) ABCDEF0123456789ABCDEF0123456789ABCDEF01 "Developer ID Application: Sharker Test (FABM2QUA8Q)"\n     1 valid identities found\n',
    ),
    {
      hash: 'ABCDEF0123456789ABCDEF0123456789ABCDEF01',
      name: 'Developer ID Application: Sharker Test (FABM2QUA8Q)',
...
test('the Eval workspace owns the complete runtime asset declaration', async () => {
  ...
  const evalWorkspace = workspaces.find(({ name }) => name === '@sharker/eval');
...
test('standalone packaging applies the shared CLI file policy to dependencies', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-standalone-policy-'));
  try {
...
test('standalone workspace staging keeps manifests consistent with copied files', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-standalone-workspace-'));
  const workspace = join(root, 'workspace');
  const install = join(root, 'install');
  try {
    const manifest = {
      name: '@sharker/example',
      exports: {
        '.': './dist/index.js',
...
test('one product workflow gates one draft release on every required artifact', async () => {
  ...
  assert.match(sourceAuthority, /RELEASE_REPOSITORY.*apache\/sharker/su);
  ...
  assert.doesNotMatch(verifyArtifacts, /required=\(|Sharker-\*|latest\*\.yml/u);
