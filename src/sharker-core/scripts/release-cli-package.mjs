...
const nightlyVersion = process.env.SHARKER_CLI_NIGHTLY_VERSION?.trim();
const preparedTree = process.env.SHARKER_CLI_RELEASE_PREPARED_TREE === '1';
...
const internalPackageNames = workspacePackages
  .map(({ name }) => name)
  .filter((name) => name !== 'sharker-agent');
...
const developmentGeneratedFiles = new Map([
  ['@sharker/runtime', new Set(['workers/filesystem-worker.js'])],
]);
...
function packageCli(publishable) {
  ...
  const cli = dependencyTree.dependencies?.['sharker-agent'];
  if (!cli) throw new Error('npm ls did not return the sharker-agent workspace');
...
function createDevelopmentArtifactRoot() {
  ...
  const parent = process.env.SHARKER_CLI_DEVELOPMENT_OUTPUT_ROOT?.trim();
...
function buildFromCleanDependencyTree() {
  const temporaryRoot = mkdtempSync(join(tmpdir(), 'sharker-cli-release-build-'));
  ...
  try {
    ...
    execFileSync(process.execPath, [join(cleanRoot, 'scripts/release-cli-package.mjs')], {
      ...
      env: {
        ...cleanEnvironment,
        SHARKER_CLI_RELEASE_PREPARED_TREE: '1',
        ...(preparedPeerPrebuilds
          ? { SHARKER_RUNTIME_HOST_PEER_PREBUILDS: preparedPeerPrebuilds }
          : {}),
      },
      stdio: 'inherit',
    });
...
function copyPeerPrebuildInputToCleanTree(cleanRoot) {
  const source = process.env.SHARKER_RUNTIME_HOST_PEER_PREBUILDS?.trim();
...
function checkProductionAudit() {
  const audit = spawnSync(
    'npm',
    ['audit', '--omit=dev', '--workspace', 'sharker-agent', '--json'],
    npmSpawnOptions({
      cwd: repoRoot,
      encoding: 'utf8',
...
function readCliDependencyTree() {
  return JSON.parse(
    runNpm(['ls', '--workspace', 'sharker-agent', '--omit=dev', '--all', '--long', '--json'], {
      encoding: 'utf8',
      maxBuffer: 64 * 1024 * 1024,
    }),
  );
...
function dependencyDestination(dependency) {
  ...
  if (dependency.name?.startsWith('@sharker/')) {
...
function copyRuntimeDist(source, destination, packageName = 'sharker-agent') {
  ...
  copyTreeFiles(sourceDist, join(destination, 'dist'), (relativePath) => {
    if (isSharkerDevelopmentArtifact(join('dist', relativePath))) return false;
    return (
      !developmentBuild ||
      isCurrentDevelopmentJavaScript(
        source,
...
function copyReleaseDocuments() {
  ...
  const nightlyNotice = nightlyVersion
    ? '> **Developer snapshot:** This Nightly is not an Apache release and is not intended for production use. Its version is bound to one exact `apache/sharker` commit.\n\n'
    : '';
  const nightlyNoticeZhCn = nightlyVersion
    ? '> **开发快照：**此 Nightly 不是 Apache Release，不用于生产环境；其版本只对应一个精确的 `apache/sharker` commit。\n\n'
    : '';
  writeFileSync(
...
function copyRuntimeHostPeerPrebuilds(publishable) {
  const sourceRoot = process.env.SHARKER_RUNTIME_HOST_PEER_PREBUILDS?.trim();
  ...
  if (!sourceRoot && !publishable) {
    ...
    const destination = join(destinationRoot, target, 'sharker_runtime_host_peer.node');
    buildDevelopmentPeerAddon(target, destination);
    return;
  }
  if (!sourceRoot) {
    throw new Error('SHARKER_RUNTIME_HOST_PEER_PREBUILDS must contain all release platform addons');
  }
  for (const target of targets) {
    const source = join(sourceRoot, target, 'sharker_runtime_host_peer.node');
...
function resolveDevelopmentPeerTarget() {
  const configured = process.env.SHARKER_CLI_DEVELOPMENT_PEER_TARGET?.trim();
  const target = configured || `${process.platform}-${process.arch}`;
  if (target !== 'none' && !peerPrebuildTargets.includes(target)) {
    throw new Error(
      `SHARKER_CLI_DEVELOPMENT_PEER_TARGET must be none or a supported target; found ${target}`,
    );
  }
  return target;
}
...
function buildDevelopmentPeerAddon(target, output) {
  ...
  if (target === hostTarget) {
    execFileSync(process.execPath, [buildScript], {
      cwd: repoRoot,
      env: { ...process.env, SHARKER_RUNTIME_HOST_PEER_OUTPUT: output },
      stdio: 'inherit',
    });
    return;
  }
  ...
  if (!rustTarget) {
    throw new Error(
      `Cannot build the ${target} direct-peer addon from ${hostTarget}; run Desktop on that target or provide SHARKER_RUNTIME_HOST_PEER_PREBUILDS`,
    );
  }
  ...
  execFileSync(process.execPath, [buildScript], {
    ...
    env: {
      ...process.env,
      SHARKER_RUNTIME_HOST_PEER_CARGO_SUBCOMMAND: 'zigbuild',
      SHARKER_RUNTIME_HOST_PEER_CARGO_TARGET: rustTarget,
      SHARKER_RUNTIME_HOST_PEER_OUTPUT: output,
    },
    stdio: 'inherit',
  });
}
...
function writeReleaseManifest(cli, publishable) {
  ...
  const updateCompatibility = source.sharker?.managedRuntimeHostUpdateCompatibility;
  ...
  const manifest = {
    ...
    description: nightlyVersion
      ? 'Apache Sharker (Incubating) developer snapshot; not an Apache release.'
      : 'Apache Sharker (Incubating), a local-first agent workspace for the terminal.',
    license: source.license,
    type: source.type,
    exports: {},
    bin: source.bin,
    sharker: { managedRuntimeHostUpdateCompatibility: updateCompatibility },
    engines: root.engines,
    repository: {
      type: 'git',
      url: 'git+https://github.com/apache/sharker.git',
      directory: 'packages/cli',
    },
    homepage: 'https://github.com/apache/sharker#readme',
    bugs: { url: 'https://github.com/apache/sharker/issues' },
...
function validateStaging(publishable) {
  const required = [
    ...
    'RUNTIME_HOST_PEER_DEPENDENCIES.rust.tsv',
    'RUNTIME_HOST_PEER_THIRD_PARTY_NOTICES.txt',
    'node_modules/@sharker/runtime/dist/workers/filesystem-worker.js',
    'node_modules/@sharker/runtime-host/dist/execution-candidate-main.js',
    'node_modules/@sharker/eval/dist/harbor-external-subject.js',
    'node_modules/@sharker/eval/harbor/relay_agent.py',
    'node_modules/@sharker/eval/harbor/docker-compose-egress-proxy.yaml',
  ...
  if (publishable) {
    required.push(
      ...peerPrebuildTargets.map(
        (target) => `native/runtime-host-peer/prebuilds/${target}/sharker_runtime_host_peer.node`,
      ),
    );
  } else if (privatePeerTarget !== 'none') {
    required.push(
      `native/runtime-host-peer/prebuilds/${privatePeerTarget}/sharker_runtime_host_peer.node`,
    );
  }
...
function validatePackedFiles(files, expectedDependencyManifests, publishable) {
  ...
  for (const file of files) {
    ...
    const sharkerOwned =
      ...
      (segments[0] === 'node_modules' && segments[1] === '@sharker' && segments[3] !== 'node_modules');
    if (sharkerOwned && isSharkerDevelopmentArtifact(path)) {
      throw new Error(`Development artifact escaped into the tarball: ${path}`);
    }
    if (!sharkerOwned && path.startsWith('node_modules/') && isThirdPartyDevelopmentArtifact(path)) {
...
function copyTreeFiles(source, destination, allow) {
  for (const entry of readdirSync(source, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      throw new Error(`Sharker build output contains an unexpected symlink: ${path}`);
    }
  }
}
