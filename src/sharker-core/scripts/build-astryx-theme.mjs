...
// Build the Sharker Astryx theme and strip its element-typography block (#1565 PR 3).
//
// `astryx theme build` compiles apps/desktop/src/renderer/astryx-theme/sharkerTheme.ts
// to sharker.css + sharker.js. The CSS always leads with an `@layer reset` block of
...
// Sharker: bare-element rules win wherever no product rule competes, and no layer
// order can bury them.
//
...
const themeSource = path.join('src', 'renderer', 'astryx-theme', 'sharkerTheme.ts');
const cssOut = path.join('src', 'renderer', 'astryx-theme', 'sharker.css');
// @astryxdesign/cli 0.2.0 stopped emitting sharker.variants.d.ts (`astryx theme
// build` now outputs css/js/d.ts only) — the file was tracked but never
// imported, so it is gone with the upgrade. The CLI's sharker.d.ts still opens
// with a `/// <reference>` to it, though, so post-processing strips the
// dangling line (#1980) — without it every `tsc` run that resolves sharker.d.ts
// leans on skipLibCheck to ignore the missing file.
const generatedFiles = ['sharker.css', 'sharker.js', 'sharker.d.ts'];
const canonicalCommand =
  ' * Command: astryx theme build src/renderer/astryx-theme/sharkerTheme.ts --out ' +
  'src/renderer/astryx-theme/sharker.css';
...
export function stripVariantsReference(dts) {
  return dts.replace(/^\/\/\/\s*<reference\s+path="\.\/sharker\.variants\.d\.ts"\s*\/>\s*\n/m, '');
}

function postProcessGeneratedFile(file, source) {
  const normalized = normalizeGeneratedHeader(source);
  if (file === 'sharker.css') return stripResetLayer(normalized, file);
  if (file === 'sharker.d.ts') return stripVariantsReference(normalized);
  return normalized;
}
...
export function buildAstryxTheme({ check = false, stdio = 'inherit' } = {}) {
  const temporaryDirectory = mkdtempSync(path.join(tmpdir(), 'sharker-astryx-theme-'));
  const output = path.join(temporaryDirectory, 'sharker.css');
  try {
