...
export async function verifyWindowsSandboxWorkerE2E(appDirectoryPath) {
  ...
  const appExecutable = join(appDirectory, 'Sharker.exe');
  const resourcesPath = join(appDirectory, 'resources');
  const sandboxExecutable = join(resourcesPath, 'windows-sandbox', 'sharker-windows-sandbox.exe');
  ...
  const workspace = await realpath(await mkdtemp(join(tmpdir(), 'sharker-packaged-e2e-ws-')));
  const outside = await realpath(await mkdtemp(join(homedir(), '.sharker-packaged-e2e-outside-')));
  try {
...
$imageName = [IO.Path]::GetFileName($env:SHARKER_CANCEL_SANDBOX)
...
`;
  const { stdout } = await execFileAsync(
    ...
    ['-NoProfile', '-NonInteractive', '-Command', script],
    {
      env: {
        ...process.env,
        SHARKER_CANCEL_SANDBOX: sandboxExecutable,
      },
...
async function findRecoveryRecord(requestId) {
  const ledgerRoot = join(tmpdir(), 'sharker-sandbox-acl-ledgers');
