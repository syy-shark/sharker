...
const STAGE_RUN = {
  ...
  conclusion: 'success',
  head_repository: { full_name: 'apache/sharker' },
};
...
test('registry finalization requires the exact staged bytes and dist-tag', async () => {
  const fixture = createPreparedCandidate();
  const registryDirectory = mkdtempSync(join(tmpdir(), 'sharker-cli-registry-release-'));
  ...
  await assert.rejects(
    fetchRegistryRelease({
      releaseDirectory: fixture.releaseDirectory,
      registryDirectory: mkdtempSync(join(tmpdir(), 'sharker-cli-registry-drift-')),
...
test('registry downloads stop reading as soon as the tarball exceeds its bound', async () => {
  ...
  const tarballUrl = `https://registry.npmjs.org/sharker-agent/-/${fixture.tarball}`;
  ...
  await assert.rejects(
    fetchRegistryRelease({
      releaseDirectory: fixture.releaseDirectory,
      registryDirectory: mkdtempSync(join(tmpdir(), 'sharker-cli-registry-oversized-')),
      fetchImpl,
    }),
...
test('signature audit must contain Sharker provenance for the finalized version', () => {
  const fixture = createPreparedCandidate();
  const verified = {
    invalid: [],
    missing: [],
  ...
  for (const mutate of [
    ...
    (statement) => {
      statement.predicate.runDetails.metadata.invocationId =
        'https://github.com/apache/sharker/actions/runs/999/attempts/1';
    },
    (statement) => {
...
test('signature audit tree exposes only the top-level registry package', () => {
  const fixture = createPreparedCandidate();
  const auditDirectory = mkdtempSync(join(tmpdir(), 'sharker-cli-signature-audit-'));
  ...
  assert.deepEqual(JSON.parse(readFileSync(join(auditDirectory, 'package.json'), 'utf8')), {
    name: 'sharker-cli-signature-audit',
    private: true,
    dependencies: { 'sharker-agent': fixture.version },
  });
  assert.deepEqual(
    JSON.parse(readFileSync(join(auditDirectory, 'node_modules/sharker-agent/package.json'), 'utf8')),
    { name: 'sharker-agent', version: fixture.version },
  );
});
    ...
    predicate: {
      buildDefinition: {
        ...
        resolvedDependencies: [
          {
            uri: 'git+https://github.com/apache/sharker@refs/heads/main',
      ...
      runDetails: {
        ...
        metadata: {
          invocationId: 'https://github.com/apache/sharker/actions/runs/321/attempts/1',
        },
      },
    },
  };
...
function createCandidate(version = '0.2.0', sourceVersion = version) {
  const root = mkdtempSync(join(tmpdir(), 'sharker-cli-publication-'));
  const releaseDirectory = join(root, 'packages/cli/release');
  const tarball = `sharker-agent-${version}.tgz`;
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: 'sharker-agent', version: sourceVersion })}\n`,
  );
...
function registryFetch({ fixture, bytes = fixture.bytes }) {
  ...
  return async (input, options = {}) => {
    ...
    if (url === `https://registry.npmjs.org/sharker-agent/${fixture.version}`) {
    ...
    if (url === 'https://registry.npmjs.org/sharker-agent') {
