...
export const exclusionRules = [
  ...
  },
  {
    ...
    justification:
      'Work of third parties kept under its own license, including files Sharker adapted from an upstream project and therefore does not wholly own. ASF policy treats the non-Sharker portion of a mixed-origin file as third-party work, so Sharker may not assert a whole-file ASF header over one; whether a heavily modified file should instead carry a combined header is a PPMC decision, not a mechanical sweep. Their attribution belongs to LICENSE and the NOTICE audit, not to this gate.',
    matches: (path) =>
      isOneOf(
  ...
  },
  {
    ...
    matches: isOneOf(
      'apps/desktop/src/renderer/astryx-theme/sharker.css',
      'apps/desktop/src/renderer/astryx-theme/sharker.d.ts',
      'apps/desktop/src/renderer/astryx-theme/sharker.js',
 ...
 * `opencode` hits forty-five because Sharker supports it as a provider. A net
...
const reviewedProvenance = new Map([
  ...
  [
    ...
    'Sharker-authored audit report whose subject is other projects’ licensing. The copyright lines it quotes are its findings, not a claim over the file.',
  ],
  [
  ...
  [
    ...
    'Sharker-authored generator that emits upstream copyright lines into THIRD_PARTY_NOTICES.txt. The copyright it names is its output, not its own.',
  ],
  [
    'scripts/generate-runtime-host-peer-notices.mjs',
    'Sharker-authored generator that emits upstream copyright lines into the Runtime Host peer notices. The copyright it names is its output, not its own.',
  ],
  [
    'scripts/sync-model-metadata.mjs',
    'Sharker-authored generator that writes the models.dev attribution into the header of the catalog it generates. The copyright it names is its output, not its own.',
  ],
]);
