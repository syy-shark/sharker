...
describe('ASF header classification', () => {
  ...
  test('excludes the mixed-origin files Sharker adapted from upstream projects', () => {
    for (const path of [
  ...
  test('keeps Sharker-authored files out of the third-party and fixture rules', () => {
    assert.equal(classifyPath('patches/README.md').status, 'covered');
    assert.equal(
      classifyPath('docs/eval/terminal-bench-2.1-sharker-vs-kimi-code-v11.md').status,
      'covered',
    );
  });
  ...
  test('records no exclusion for paths that export-ignore prunes', () => {
    ...
    assert.equal(classifyPath('sharker-proposal-zh-review.txt').status, 'unclassified');
  });
});
...
describe('ASF header audit over an extracted candidate', () => {
  ...
  test('does not excuse a path that export-ignore should have pruned', () => {
    const root = fixtureTree({
      ...
      'sharker-proposal-zh-review.txt': 'notes\n',
    });
    const result = auditTree({ root });
    assert.deepEqual(result.unclassified, ['sharker-proposal-zh-review.txt']);
