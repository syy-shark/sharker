...
export async function verifyPackagedWindowsApp(
  ...
  const sandboxExecutable = join(resources, 'windows-sandbox', 'sharker-windows-sandbox.exe');
  ...
  if (requiresCurrentContract) {
    await assertPackagedUpdateConfiguration(resources, {
      channel: environment.SHARKER_DESKTOP_NIGHTLY_VERSION ? 'nightly' : 'release',
    });
  ...
  if (requiresCurrentContract) {
    ...
    if (sandboxMachine !== amd64Machine) {
      throw new Error(
        `sharker-windows-sandbox.exe must be x64, found PE machine 0x${sandboxMachine.toString(16)}.`,
      );
    }
    const sandboxLaunch = {
      version: 1,
...
export async function verifyWindowsX64Release(
  ...
  const temporaryDirectory = await mkdtemp(join(tmpdir(), 'sharker-release-verify-'));
