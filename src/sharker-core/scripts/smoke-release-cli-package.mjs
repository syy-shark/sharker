...
const MODEL_ID = 'sharker-release-smoke-model';
const CONNECTION_SLUG = 'sharker-release-smoke';
const API_KEY = 'sharker-release-smoke-key';
const FILE_SENTINEL = 'SHARKER_RELEASE_FILESYSTEM_WORKER_OK';
const RESPONSE_SENTINEL = 'SHARKER_RELEASE_SMOKE_OK';
const INSTALLED_ROOT_ENV = 'SHARKER_CLI_RELEASE_INSTALLED_ROOT';
...
async function main() {
  ...
  const root = mkdtempSync(join(tmpdir(), 'sharker-cli-tarball-smoke-'));
  let primaryError;
  let cleanupError;
  try {
...
async function validateInstalledProduct(root) {
  ...
  const packageRoot =
    process.platform === 'win32'
      ? join(prefix, 'node_modules/sharker-agent')
      : join(prefix, 'lib/node_modules/sharker-agent');
  const baseEnvironment = isolatedEnvironment(join(root, 'home'));
  const sharker = process.platform === 'win32' ? join(prefix, 'sharker.cmd') : join(prefix, 'bin/sharker');
  ...
  const version = runSync(crossSpawn.sync, sharker, ['--version'], baseEnvironment, root).trim();
  if (version !== manifest.version) {
    throw new Error(`Installed CLI reports ${version}; package manifest is ${manifest.version}`);
  }
  assertOutput(runSync(crossSpawn.sync, sharker, ['--help'], baseEnvironment, root), 'Usage: sharker');
  assertOutput(
    runSync(crossSpawn.sync, sharker, ['eval', '--help'], baseEnvironment, root),
    'usage: sharker eval run',
  );
  validateInstalledRuntimeFiles(packageRoot);

  logStep('checking installed Eval spec decoding and framework preflight');
  smokeEvalPreflight({ crossSpawn: crossSpawn.sync, environment: baseEnvironment, sharker, root });
...
async function smokeRuntimeHostPeerProtocol({ packageRoot, cliEntrypoint, root }) {
  ...
  const server = await importInstalled(
    packageRoot,
    'node_modules/@sharker/runtime-host/dist/server/index.js',
  );
  const client = await importInstalled(
    packageRoot,
    'node_modules/@sharker/runtime-host/dist/client/index.js',
  );
  const mesh = await importInstalled(
    packageRoot,
    'node_modules/@sharker/runtime-host/dist/peer-mesh/index.js',
  );
  ...
  const previousNativePath = process.env.SHARKER_RUNTIME_HOST_PEER_NATIVE_PATH;
  const previousKeyPath = process.env.SHARKER_RUNTIME_HOST_PEER_KEY_PATH;
  let host;
  let connection;
  let meshAuthorityOwner;
  let meshMemberOwner;
  try {
    delete process.env.SHARKER_RUNTIME_HOST_PEER_NATIVE_PATH;
    delete process.env.SHARKER_RUNTIME_HOST_PEER_KEY_PATH;
    ...
    const nativePath = process.env.SHARKER_RUNTIME_HOST_PEER_NATIVE_PATH;
    ...
    restoreEnvironment('SHARKER_RUNTIME_HOST_PEER_NATIVE_PATH', previousNativePath);
    restoreEnvironment('SHARKER_RUNTIME_HOST_PEER_KEY_PATH', previousKeyPath);
  }
}
...
function validateInstalledRuntimeFiles(packageRoot) {
  for (const path of [
    ...
    // disclaimer next to LICENSE/NOTICE, like every other Sharker release.
    'DISCLAIMER-WIP',
    ...
    'node_modules/@sharker/eval/dist/index.js',
    'node_modules/@sharker/eval/harbor/relay_agent.py',
    'node_modules/@sharker/eval/harbor/egress-proxy/network-policy',
  ]) {
...
function smokeEvalPreflight({ crossSpawn, environment, sharker, root }) {
  const evalRoot = join(root, 'eval-preflight');
  mkdirSync(evalRoot, { recursive: true });
  const machineEnvironment = {
    ...environment,
    SHARKER_RELEASE_EVAL_PYTHON: process.execPath,
    SHARKER_RELEASE_EVAL_TASKS: evalRoot,
    SHARKER_RELEASE_EVAL_TRIALS: join(evalRoot, 'trials'),
  };
  for (const framework of ['harbor', 'pier']) {
    ...
    const result = crossSpawn(sharker, ['eval', 'run', specPath, '--out', join(evalRoot, framework)], {
      cwd: evalRoot,
    ...
    assertOutput(
      `${result.stdout ?? ''}\n${result.stderr ?? ''}`,
      `${framework} Python environment SHARKER_RELEASE_EVAL_PYTHON is unavailable or does not provide`,
    );
  }
}
  ...
  return {
    ...
    executor: {
      ...
      config: {
        ...
        pythonPathEnv: 'SHARKER_RELEASE_EVAL_PYTHON',
        trialsRootEnv: 'SHARKER_RELEASE_EVAL_TRIALS',
        ...(framework === 'pier' ? { tasksRootEnv: 'SHARKER_RELEASE_EVAL_TASKS' } : {}),
...
async function smokePty(ptySpawn, environment, cwd) {
  const result = await runPtyScenario({
    ...
    args: ['-e', 'process.stdout.write("sharker-pty-ok")'],
    cwd,
    environment,
    marker: 'sharker-pty-ok',
    timeoutMs: 30_000,
  });
...
async function smokeControlledRun({ packageRoot, cliEntrypoint, root }) {
  ...
  await withCleanup(
    async () => {
      ...
      if (result.exitCode !== 0) {
        throw new Error(`Controlled sharker run exited with ${result.exitCode}: ${result.stderr}`);
      }
      ...
      if (!provider.sawReadTool || !provider.sawFileSentinel) {
        throw new Error('Controlled sharker run did not execute the installed filesystem worker');
      }
    },
    (completed) =>
      runCleanupSteps([
...
async function configureRuntimePolicy(packageRoot, rootPath, baseUrl) {
  const authority = await importInstalled(
    packageRoot,
    'node_modules/@sharker/storage/dist/root-authority.js',
  );
  const policyModule = await importInstalled(
    packageRoot,
    'node_modules/@sharker/storage/dist/runtime-policy-stores.js',
  );
...
async function resolveInstalledDataRoots(packageRoot, environment, home) {
  const storage = await importInstalled(
    packageRoot,
    'node_modules/@sharker/storage/dist/workspace-root.js',
  );
  return storage.resolveSharkerDataRoots({ env: environment, homeDir: home, profileName: 'Sharker' });
}
