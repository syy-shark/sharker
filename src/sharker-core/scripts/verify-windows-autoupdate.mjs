...
const executableName = 'Sharker.exe';
...
export async function verifyWindowsAutoupdate(
  ...
  nextDirectoryInput,
  {
    ...
    makeTemporaryDirectory = () => mkdtemp(join(tmpdir(), 'sharker-autoupdate-')),
    run = runCommand,
  } = {},
) {
  if (platform !== 'win32') {
  ...
  const nextInstallerName = `Sharker-${nextVersion}-win-x64.exe`;
  ...
  try {
    ...
    try {
      ...
      // Nondeterministic CI state observed once: a Sharker.exe process is
    ...
    const upgradedStatusExpression = 'window.sharker.app.updateStatus()';
      ...
      smokeRenderer: async (executable, { workingDirectory }) => {
        ...
        const smokeChild = spawn(
          ...
          ],
          {
            ...
            env: { ...process.env, SHARKER_SKIP_SHELL_ENV: '1', ...smokeEnv },
