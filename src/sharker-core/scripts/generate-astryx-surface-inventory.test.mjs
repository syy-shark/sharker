...
describe('@sharker/ui barrel resolution (#3868)', () => {
  test('derives the Astryx re-export map from the real barrel', () => {
    const { reexports } = loadSharkerUiBarrel();
  ...
  test('fails closed on an unresolved relative barrel target', () => {
    ...
    assert.throws(() => loadSharkerUiBarrel(repoRoot), /unresolved/i);
  });
});

describe('analyzeTsx severity (#3868)', () => {
  const astryxComponents = loadAstryxComponents().components;
  const base = { astryxComponents, sharkerUiReexports: new Map(), shadowNames: null };
  ...
  test('counts an Astryx component imported via the @sharker/ui re-export map', () => {
    const src = `
      import { Selector } from '@sharker/ui';
      export function Row() { return <Selector value="x" />; }
    `;
    const result = analyzeTsx('apps/desktop/src/renderer/row.tsx', src, {
      ...base,
      sharkerUiReexports: new Map([['Selector', 'Selector']]),
    });
  ...
  test('does not flag a private local component that merely shares a name', () => {
    // No shadowNames for this file (it is not a public @sharker/ui export).
    const src = `
