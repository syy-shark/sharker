...
async function writeUpdateSet(directory, version, platform) {
  ...
  const artifact = isMac ? `Sharker-${version}-mac-arm64.zip` : `Sharker-${version}-win-x64.exe`;
...
test('staging creates only the exact GitHub Release assets', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-desktop-nightly-'));
  ...
  await Promise.all([
    ...
    writeFile(join(input, `Sharker-${version}-mac-arm64.dmg`), 'dmg'),
    writeFile(join(input, `Sharker-${version}-win-x64.zip`), 'windows zip'),
  ]);
  ...
  const payloadNames = [
    `Sharker-${version}-mac-arm64.dmg`,
    `Sharker-${version}-mac-arm64.zip`,
    `Sharker-${version}-mac-arm64.zip.blockmap`,
    `Sharker-${version}-win-x64.exe`,
    `Sharker-${version}-win-x64.exe.blockmap`,
    `Sharker-${version}-win-x64.zip`,
  ];
  ...
  await Promise.all([
    verifyDesktopUpdateArtifacts({
      ...
      version,
      artifactName: `Sharker-${version}-mac-arm64.zip`,
    }),
    verifyDesktopUpdateArtifacts({
      directory: release,
      metadataName: 'dev.yml',
      version,
      artifactName: `Sharker-${version}-win-x64.exe`,
    }),
  ]);
...
test('one attestation bundle is staged only as a GitHub Release asset', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-desktop-nightly-attestation-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  const output = join(root, 'output');
  const release = join(output, 'release');
  const version = '0.2.0-dev.42.20260829';
  ...
  const name = `Sharker-${version}-attestation.sigstore.json`;
