...
describe('CLI release file policy', () => {
  test('release manifests omit exports whose targets are excluded from Sharker artifacts', () => {
  ...
  test('derives the runtime workspace build order from production dependencies', () => {
    const manifests = new Map([
      ['sharker-agent', { name: 'sharker-agent', dependencies: { '@sharker/eval': '0.1.0' } }],
      ['@sharker/eval', { name: '@sharker/eval', dependencies: { '@sharker/core': '0.1.0' } }],
      ['@sharker/core', { name: '@sharker/core' }],
    ]);

    assert.deepEqual(collectWorkspaceDependencyClosure('sharker-agent', manifests), [
      '@sharker/core',
      '@sharker/eval',
      'sharker-agent',
    ]);
  ...
  test('orders selected runtime workspaces by local build-time dependencies', () => {
    const selected = [
      { name: '@sharker/runtime', manifest: { devDependencies: { '@sharker/storage': '0.1.0' } } },
      { name: '@sharker/storage', manifest: {} },
      { name: 'sharker-agent', manifest: { dependencies: { '@sharker/runtime': '0.1.0' } } },
    ];
  ...
  test('release file declarations cannot escape or overlap their workspace', () => {
    for (const releaseFiles of [
      ...
      assert.throws(
        () => workspaceReleaseFiles({ name: '@sharker/example', releaseFiles }),
        /unsafe|overlap/u,
      );
    }
  });
  ...
  test('release file declarations accept a declared directory and reject non-regular entries', () => {
    const workspace = mkdtempSync(join(tmpdir(), 'sharker-release-files-'));
    try {
  ...
  test('keeps the stricter Sharker-owned package boundary', () => {
    assert.equal(isSharkerDevelopmentArtifact('src/index.js'), true);
    assert.equal(isSharkerDevelopmentArtifact('dist/__tests__/fixture.js'), true);
    assert.equal(isSharkerDevelopmentArtifact('dist/index.js'), false);
  });

  test('development packages exclude JavaScript left by deleted sources', () => {
    const workspace = mkdtempSync(join(tmpdir(), 'sharker-development-output-'));
    try {
  ...
  test('rejects Sharker test-only modules so no test backend can ship', () => {
    for (const path of [
      ...
      assert.equal(isSharkerDevelopmentArtifact(path), true, path);
    }
    assert.equal(isSharkerDevelopmentArtifact('dist/execution-candidate-main.js'), false);
  });
  ...
  test('every workspace dist module a script references by path still exists', () => {
    ...
    for (const scriptPath of scriptPaths) {
      ...
      for (const [literal, packageName, distPath] of source.matchAll(
        /['"`]node_modules\/(@sharker\/[^/'"`]+)\/(dist\/[^'"`]+\.js)['"`]/gu,
      )) {
