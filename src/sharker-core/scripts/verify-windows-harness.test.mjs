...
it('scopes rollback registration reads and deletion to the fixture uninstaller', async () => {
  const calls = [];
  const uninstaller = 'C:\\fixture\\installed\\Uninstall Sharker.exe';
  ...
  for (const { command, args, options } of calls) {
    ...
    assert.match(script, /C:\\fixture\\installed\\Uninstall Sharker\.exe/u);
...
it('runs packaged sandbox lifecycle evidence against the exact shipped broker', async () => {
  const calls = [];
  const sandboxExecutable = 'C:\\release\\resources\\windows-sandbox\\sharker-windows-sandbox.exe';
...
it('uses the product SemVer contract throughout Windows release verification', () => {
  assert.equal(installerVersion('Sharker-1.2.3-beta.2-win-x64.exe'), '1.2.3-beta.2');
  ...
  const baseline = {
    ...
    tag: 'v1.2.3-beta.1',
    assetName: 'Sharker-1.2.3-beta.1-win-x64.exe',
    sha256: 'a'.repeat(64),
  };
...
it('reuses the packaged renderer smoke without widening rollback verification', async () => {
  ...
  const executable = join(installDirectory, 'Sharker.exe');
  const run = async (command, args, options) => {
    calls.push({ command, args, options });
...
async function makeTree(shape) {
  const root = await mkdtemp(join(tmpdir(), 'sharker-harness-test-'));
...
describe('waitForInstalledProcessesToExit', () => {
  it('returns once an enumeration reports no processes', async () => {
    const snapshots = [[{ processId: 1, name: 'Sharker.exe' }], []];
  ...
  it('never treats a failed enumeration as empty: rejects at the deadline', async () => {
    await assert.rejects(
      ...
      /Could not enumerate installed Sharker processes within 200ms: wmi stalled/,
    );
  });
});
...
describe('waitForInstalledProcessAppearance', () => {
  it('survives a failed probe and returns the later successful match', async () => {
    ...
    const probes = [
      ...
      () =>
        Promise.resolve([
          { processId: 7, name: 'Sharker.exe', path: 'C:/nowhere/installed/Sharker.exe' },
        ]),
    ];
    const matched = await waitForInstalledProcessAppearance('C:/nowhere/installed', 'Sharker.exe', {
  ...
  it('matches by executable basename, case-insensitively', async () => {
    const matched = await waitForInstalledProcessAppearance('C:/nowhere/installed', 'Sharker.exe', {
      listProcesses: async () => [
        ...
        { processId: 2, name: 'SHARKER.EXE', path: 'C:/nowhere/installed/SHARKER.EXE' },
      ],
      timeoutMs: 2_000,
      pollIntervalMs: 10,
    });
  ...
  it('reports the last probe error only when no later enumeration succeeds', async () => {
    await assert.rejects(
      () =>
        waitForInstalledProcessAppearance('C:/nowhere/installed', 'Sharker.exe', {
          listProcesses: async () => {
            throw new Error('wmi stalled');
          },
          timeoutMs: 150,
...
describe('waitForInstalledProductVersion', () => {
  it('retries a stalled PowerShell read within the outer deadline', async () => {
    ...
    const version = await waitForInstalledProductVersion('C:/installed/Sharker.exe', {
      run: async (_command, _args, options) => {
...
describe('terminateInstalledProcesses', () => {
  const process7 = { processId: 7, name: 'Sharker.exe', path: 'C:/nowhere/installed/Sharker.exe' };
    ...
    await assert.rejects(
      () =>
        terminateInstalledProcesses('C:/nowhere/installed', {
          ...
          waitForExit: async () => {
            throw new Error('Sharker.exe (7) is still running');
          },
        }),
      (error) => {
        ...
        assert.deepEqual(
          ...
          ['Sharker.exe (7) is still running', 'taskkill failed with exit code 255'],
        );
        return true;
      },
    );
...
describe('waitForUninstallRegistrationToClear', () => {
  ...
  it('tolerates probe failures but rejects with them at the deadline', async () => {
    await assert.rejects(
      ...
      /Could not read the Sharker uninstall registration within 150ms: registry stalled/,
    );
  });
});
