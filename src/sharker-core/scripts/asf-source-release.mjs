...
const archivePattern = /^apache-sharker-(\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?)-incubating-src\.tar\.gz$/;
...
const forbiddenSegments = new Set(['.agents', '.claude', '.git', '.sharker-shots', 'node_modules']);
const forbiddenRootFiles = new Set(['sharker-proposal-zh-review.txt']);
...
export function sourceCandidateIdentity(version) {
  ...
  const rootDirectory = `apache-sharker-${version}-incubating`;
  return {
...
export async function createSourceCandidate({
  ...
  const temporaryRoot = mkdtempSync(join(dirname(archivePath), '.sharker-asf-source-'));
  try {
...
export async function reproduceSourceCandidate({
  ...
  const temporaryRoot = mkdtempSync(join(tmpdir(), 'sharker-asf-reproduce-'));
  try {
...
export async function signSourceCandidate({
  ...
  const temporaryRoot = mkdtempSync(join(dirname(archivePath), '.sharker-asf-signature-'));
...
function validateArchiveContents(archivePath, identity, entries) {
  const temporaryRoot = mkdtempSync(join(tmpdir(), 'sharker-asf-contents-'));
  try {
    ...
    for (const requiredFile of requiredReleaseDocuments) {
      ...
      if (
        ...
        (!contents.includes('Apache Sharker') || !contents.includes('incubation'))
      ) {
        throw new Error('DISCLAIMER-WIP does not identify Apache Sharker as an incubating project');
      }
    }
...
function verifyDetachedSignature({ archivePath, keysPath, signaturePath }) {
  if (!existsSync(keysPath)) throw new Error(`KEYS file does not exist: ${keysPath}`);
  const temporaryHome = mkdtempSync(join(tmpdir(), 'sharker-gpg-'));
