...
const tuiReadyPattern = /Sharker\s*·\s*Auto\s*·/u;
...
async function smokeTuiInPty(archiveRoot, environment) {
  ...
  const executable = join(archiveRoot, 'bin', 'sharker');
  ...
  try {
    ...
    const spec = {
      ...
      benchmark: {
        ...
        config: { repository: 'https://github.com/apache/sharker.git' },
      },
      executor: {
        kind: 'harbor',
        config: {
          frameworkVersion: '0.20.0',
          pythonPathEnv: 'SHARKER_CLI_EVAL_SMOKE_PYTHON',
          trialsRootEnv: 'SHARKER_CLI_EVAL_SMOKE_TRIALS',
          environment: {},
          preparationEnvironment: [
            'PYTHONPATH',
            'SHARKER_CLI_EVAL_SMOKE_CACHE',
            'SHARKER_CLI_EVAL_SMOKE_MARKER',
          ],
          mounts: [],
          egressProxy: {
    ...
    const smokeEnvironment = {
      ...environment,
      SHARKER_CLI_EVAL_SMOKE_CACHE: taskCache,
      SHARKER_CLI_EVAL_SMOKE_MARKER: markerPath,
      SHARKER_CLI_EVAL_SMOKE_PYTHON: pythonIdentity.executable,
      SHARKER_CLI_EVAL_SMOKE_TRIALS: trialsRoot,
      PYTHONPATH: fixtureRoot,
    };
    const sharkerPath = join(archiveRoot, 'bin', 'sharker');
    const result = await run(sharkerPath, ['eval', 'run', specPath, '--out', outputPath], {
      cwd: smokeRoot,
    ...
    if (
      ...
      marker.relayName !== 'sharker-eval-relay'
    ) {
...
export function resolvePackagedRuntimeModelFactory(archiveRoot) {
  ...
  const requireFromCli = createRequire(cliManifestPath);
  return requireFromCli.resolve('@sharker/runtime/model-factory');
}
...
export async function verifyQuarantinedExecution(archiveRoot, nodePath, run) {
  await run('xattr', [
    ...
    '0083;00000000;GitHub;SharkerReleaseVerification',
    archiveRoot,
  ]);
...
export async function verifyMacosArm64Cli(
  archivePath,
  {
    ...
    requireReleaseSigning = process.env.SHARKER_CLI_REQUIRE_RELEASE_SIGNING === '1',
  } = {},
) {
  ...
  try {
    ...
    await Promise.all([
      ...
      assertMissing(join(archiveRoot, 'bin', 'sharker-agent')),
    ]);
    ...
    const versionResult = await run(sharkerPath, ['--version'], {
      cwd: commandWorkspace,
      env: environment,
    });
    ...
    const externalSharkerPath = join(externalBin, 'sharker');
    await mkdir(externalBin);
    await symlink(sharkerPath, externalSharkerPath);
    const externalVersionResult = await run(externalSharkerPath, ['--version'], {
      cwd: commandWorkspace,
      env: environment,
    });
    if (externalVersionResult.stdout.trim() !== version) {
      throw new Error('The sharker launcher is not relocatable through an external symlink.');
    }
    const helpResult = await run(sharkerPath, ['--help'], {
      cwd: commandWorkspace,
      env: environment,
    });
    ...
    const profileListResult = await run(sharkerPath, ['runtime-host', 'profile', 'list'], {
      cwd: commandWorkspace,
      env: environment,
    });
    ...
    const evalHelpResult = await run(sharkerPath, ['eval', '--help'], {
      cwd: commandWorkspace,
      env: environment,
    });
    if (!evalHelpResult.stdout.includes('sharker eval run <spec.json>')) {
