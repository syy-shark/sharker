...
export const WORKSPACE_PREFIX = '@sharker/';
...
export function rendererBundledRoots(manifestPath) {
  ...
  const declared = JSON.parse(readFileSync(manifestPath, 'utf8'))?.sharker
    ?.rendererBundledDependencies;
  if (!Array.isArray(declared) || declared.length === 0) {
    throw new Error(
      `${manifestPath}: sharker.rendererBundledDependencies must list the renderer roots`,
    );
  }
  return declared;
}
  ...
  if (roots.size > 0) {
    ...
    for (const [name, dependency] of Object.entries(full)) {
      ...
      if (name.startsWith(WORKSPACE_PREFIX)) {
        ...
        // (`@sharker/ui` declares @types/* and linkedom for its tests), and none
    ...
    // workspace root (`@sharker/ui`) carries no notice of its own — it is first
