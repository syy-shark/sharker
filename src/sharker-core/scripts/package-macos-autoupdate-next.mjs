...
export async function packageMacosAutoupdateNext({
  ...
  const zipName = `Sharker-${nextVersion}-mac-arm64.zip`;

  await access(join(desktopRoot, 'dist'));
  await access(join(desktopRoot, 'release', `Sharker-${manifest.version}-mac-arm64.zip`));
  ...
  const args = [
    ...
    '-c.extraMetadata.sharkerUpdateTestProfile=true',
    '-c.mac.notarize=false',
    '-c.directories.output=release-autoupdate-next',
  ];
  if (env.SHARKER_LOCAL_UPDATE_SIGNING_IDENTITY) {
    args.push(`-c.mac.identity=${env.SHARKER_LOCAL_UPDATE_SIGNING_IDENTITY}`);
