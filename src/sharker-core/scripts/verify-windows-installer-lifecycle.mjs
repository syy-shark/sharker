...
const uninstallExecutableName = 'Uninstall Sharker.exe';
...
export function installerVersion(path) {
  const match = basename(path).match(/^Sharker-(.+)-win-x64\.exe$/u);
  if (!match) {
...
export async function waitForInstalledProcessesToExit(
  ...
  for (;;) {
    ...
    if (Date.now() >= deadline) {
      if (lastProbeError) {
        throw new Error(
          `Could not enumerate installed Sharker processes within ${timeoutMs}ms: ` +
      ...
      throw new Error(
        `Installed Sharker processes did not exit within ${timeoutMs}ms: ${summary || '<unknown>'}.`,
      );
    }
...
export async function terminateInstalledProcesses(
  ...
  for (;;) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error(
          `Could not enumerate installed Sharker processes within ${enumerationTimeoutMs}ms: ` +
            `${error.message}`,
          { cause: error },
        );
      }
  ...
  try {
    ...
    throw new AggregateError(
      ...
      'Installed Sharker processes did not exit after taskkill failures.',
      { cause: exitError },
    );
  }
}
...
/**
 * Every HKCU `DisplayVersion` registered under the Sharker display name, joined
...
export async function readUninstallDisplayVersions({
  ...
  // registered DisplayName is "Sharker 0.1.11", never the bare product name. The
  // trailing space in the wildcard keeps other products starting with "Sharker"
...
$entries = Get-ChildItem 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall' |
  ...
  Where-Object { $_.DisplayName -eq 'Sharker' -or $_.DisplayName -like 'Sharker *' }
 ...
 * `_?=`, "Uninstall Sharker.exe" copies itself to %TEMP% and detaches; the copy
...
export async function waitForUninstallRegistrationToClear({
  ...
  for (;;) {
    ...
    if (Date.now() >= deadline) {
      if (lastProbeError) {
        throw new Error(
          `Could not read the Sharker uninstall registration within ${timeoutMs}ms: ` +
            `${lastProbeError.message}`,
          { cause: lastProbeError },
        );
      }
      throw new Error(
        `A Sharker uninstall registration (DisplayVersion ${JSON.stringify(versions)}) is still ` +
...
export async function verifyWindowsInstallerLifecycle(
  ...
  previousInputPath,
  {
    ...
    makeTemporaryDirectory = () => mkdtemp(join(tmpdir(), 'sharker-installer-lifecycle-')),
    run = runCommand,
