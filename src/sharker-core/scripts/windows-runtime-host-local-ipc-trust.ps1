...
public static class SharkerWindowsPipeTrustProbe
{
...
$userName = "SharkerIpc$(([Guid]::NewGuid().ToString('N')).Substring(0, 8))"
$password = "Sharker-Ipc!$([Guid]::NewGuid().ToString('N'))aA1"
...
try {
  ...
  foreach ($direction in @(
    ...
    $foreignResult = [SharkerWindowsPipeTrustProbe]::TryOpenAsUser(
      $userName,
      $env:COMPUTERNAME,
      $password,
      $pipeName,
