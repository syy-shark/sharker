...
export function waitForDevToolsPort(child, { timeoutMs = 30_000 } = {}) {
  return new Promise((resolvePromise, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new Error(
          `Packaged Sharker did not announce a DevTools port within ${timeoutMs}ms.` +
            `${buffer.trim() ? `\n${buffer.trim()}` : ''}`,
        ),
      );
    }, timeoutMs);
    ...
    const onExit = () => {
      ...
      reject(
        new Error(
          `Packaged Sharker exited before announcing a DevTools port.` +
            `${buffer.trim() ? `\n${buffer.trim()}` : ''}`,
        ),
      );
    };
...
export async function findRendererTarget(port, child, { timeoutMs = 90_000 } = {}) {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`Packaged Sharker exited before its renderer was ready.`);
    }
    try {
  ...
  throw new Error(
    `Packaged Sharker renderer did not expose CDP within ${Math.round(timeoutMs / 1000)} seconds${
 ...
 * reporting, which is how the auto-update harness drives `window.sharker.app`
...
export const RENDERER_STATE_EXPRESSION = `({
  ...
  hasBridge: Boolean(window.sharker),
  hasRoot: Boolean(document.querySelector('#root')),
  hasPreloadSkeleton: Boolean(document.querySelector('#root > .sharker-preload')),
...
function windowsWindowProbeScript(processId, nextWindowState, restoredBounds) {
  const stateTransition =
    nextWindowState === undefined
      ? ''
      : String.raw`
[void][SharkerNativeWindow]::ShowWindowAsync($handle, ${nextWindowState === 'maximized' ? 3 : 9})
$expectedZoomed = ${nextWindowState === 'maximized' ? '$true' : '$false'}
$stateDeadline = (Get-Date).AddSeconds(2)
while ([SharkerNativeWindow]::IsZoomed($handle) -ne $expectedZoomed) {
  if ((Get-Date) -ge $stateDeadline) {
    throw 'Packaged Sharker did not enter the requested native window state.'
  }
  Start-Sleep -Milliseconds 25
}`;
  const resizeRestoredWindow = restoredBounds
    ? String.raw`
if (-not [SharkerNativeWindow]::MoveWindow(
  $handle,
  ${restoredBounds.x},
  ${restoredBounds.y},
  ...
  throw 'MoveWindow failed for packaged Sharker.'
}`
    : '';
  return String.raw`
...
public static class SharkerNativeWindow {
...
if ($handle -eq [IntPtr]::Zero) {
  throw 'Packaged Sharker has no main window handle.'
}
${stateTransition}
${resizeRestoredWindow}
$rect = New-Object SharkerNativeWindow+RECT
if (-not [SharkerNativeWindow]::GetClientRect($handle, [ref]$rect)) {
  throw 'GetClientRect failed for packaged Sharker.'
}
$windowState = if ([SharkerNativeWindow]::IsZoomed($handle)) { 'maximized' } else { 'normal' }
[pscustomobject]@{
...
async function waitForWindowLayout(
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`Packaged Sharker exited during the ${expectedWindowState} transition.`);
    }
    try {
...
export async function exercisePackagedRendererMaximizeRestore(rendererTarget, child) {
  ...
  if (
    ...
    throw new Error(
      `Packaged Sharker maximize smoke did not grow the native client: ${JSON.stringify({
  ...
  if (
    ...
    throw new Error(
      `Packaged Sharker did not restore its original native client size: ${JSON.stringify({
        restored: restoredClient,
...
child.onExit(({ exitCode }) => {
  clearTimeout(timeout);
  const ok = exitCode === 0 && output.includes('sharker-node-pty-ok');
  ...
  const message = ok ? 'sharker-node-pty-ok' : 'node-pty packaged smoke failed';
...
export async function assertPackagedDependencyClosure(
  ...
  const allowed = collectPackagedAllowlist
    ...
    : collectProductionClosure('@sharker/desktop');
  const leaked = [];
...
export async function assertPackagedResources(
  ...
  const required = [
    ...
    ...(requireDirectPeerArtifact
      ? [
          join('runtime-host-peer', 'sharker_runtime_host_peer.node'),
    ...
    join('licenses', 'sharker', 'LICENSE'),
    join('licenses', 'sharker', 'NOTICE'),
    ...(requireDisclaimer ? [join('licenses', 'sharker', 'DISCLAIMER-WIP')] : []),
    ...
    ...(requireWindowsSandbox
      ? [
          join('windows-sandbox', 'sharker-windows-sandbox.exe'),
  ...
  const forbidden = [
    ...
    // sharker-cu is built from source locally and is not signed, so it may not be in
