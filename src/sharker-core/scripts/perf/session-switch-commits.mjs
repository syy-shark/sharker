...
for (const row of ROWS) {
  await client.evaluate(
    '__SHARKER_PROBE__.commits.length = 0; __SHARKER_PROBE__.dispatches.length = 0; __SHARKER_PROBE__.arm = true',
  );
  await clickSessionRow(client, row);
  await sleep(SETTLE_MS);
  await client.evaluate('__SHARKER_PROBE__.arm = false');
  const { commits } = JSON.parse(
    await client.evaluate('JSON.stringify({ commits: __SHARKER_PROBE__.commits })'),
  );
