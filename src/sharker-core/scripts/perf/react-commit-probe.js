 ...
 * app's whole life. Set `__SHARKER_PROBE__.arm = false` before profiling with the
 * sampler, or the probe's own walk lands in the profile.
 */
(() => {
  if (globalThis.__SHARKER_PROBE__) return 'already';
  const probe = {
    commits: [],
    dispatches: [],
    arm: false,
  ...
  globalThis.__SHARKER_PROBE__ = probe;
