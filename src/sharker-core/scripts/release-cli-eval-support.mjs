...
export function findReleaseTarball(releaseDirectory) {
  const tarballs = readdirSync(releaseDirectory)
    .filter((name) => /^sharker-agent-[^/]+\.tgz$/u.test(name))
...
export function isolatedEnvironment(home) {
  ...
  for (const name of Object.keys(environment)) {
    if (name.startsWith('SHARKER_EVAL_') || name.startsWith('GIT_')) delete environment[name];
  }
...
export function createTaskFixture(root, environment, taskImage) {
  ...
  run('git', ['config', 'user.name', 'Sharker Release Validation'], environment, repositoryRoot);
  run(
    'git',
    ['config', 'user.email', 'release-validation@sharker.invalid'],
    environment,
    repositoryRoot,
  );
