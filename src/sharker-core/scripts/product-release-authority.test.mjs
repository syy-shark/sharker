...
test('publication verifies live asset digests before one Stable/Latest mutation', async (t) => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-publish-authority-'));
  const recordDirectory = await mkdtemp(join(tmpdir(), 'sharker-publish-record-'));
  t.after(() => rm(directory, { recursive: true, force: true }));
  t.after(() => rm(recordDirectory, { recursive: true, force: true }));
  const version = '1.2.3';
  const macZip = `Sharker-${version}-mac-arm64.zip`;
  const exe = `Sharker-${version}-win-x64.exe`;
  ...
  const attestationBundlePath = join(recordDirectory, 'Sharker-1.2.3-attestation.sigstore.json');
  await writeFile(attestationBundlePath, attestationContents);
  const attestation = {
    name: 'Sharker-1.2.3-attestation.sigstore.json',
  ...
  const run = async (command, args) => {
    ...
    if (command === 'gh' && args.includes('repos/apache/sharker/releases/latest')) {
      latestReads += 1;
