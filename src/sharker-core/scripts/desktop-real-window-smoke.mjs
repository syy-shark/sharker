 ...
 * isolated Sharker window and records the reviewer-confirmed result as JSON +
 * Markdown under `apps/desktop/tests/real-window-smoke/`.
 */

import { execFile, spawn } from 'node:child_process';
import { terminateChildProcessTree } from '@sharker/runtime/process-tree-terminator';
...
const STARTUP_SMOKE_CHECKS = [
  {
    ...
    prompt: 'The real Sharker BrowserWindow and renderer reach a usable ready state.',
  },
];
...
function isStaleSharkerElectronProcess(entry) {
  ...
  return (
    /--user-data-dir=(?:\/private)?\/tmp\/sharker-(?:e2e-fixture|real-window-smoke|p0fix|uxp|v\d|copy|long|modal|narr|test-cap-debug)/.test(
      command,
    ) || /--user-data-dir=(?:\/private)?\/var\/folders\/.+\/sharker-real-window-smoke-/.test(command)
  );
}
...
async function cleanupStaleElectronProcesses(enabled) {
  if (!enabled) return [];
  const stale = (await listElectronRootProcesses()).filter(isStaleSharkerElectronProcess);
  if (stale.length === 0) return [];
  console.log(
    `[real-window-smoke] cleaning ${stale.length} stale Sharker Electron process(es): ${stale.map((entry) => entry.pid).join(', ')}`,
  );
...
async function launchElectron(args, diagnostics) {
  ...
  const userDataDir = join(os.tmpdir(), `sharker-real-window-smoke-${args.scenario}-${process.pid}`);
  ...
  env.SHARKER_E2E_FIXTURE_WIDTH = String(args.width);
  env.SHARKER_E2E_FIXTURE_HEIGHT = String(args.height);
  env.SHARKER_REAL_WINDOW_SMOKE = '1';
...
function renderMarkdown(report) {
  const lines = [
    '# Sharker Real Window Smoke Report',
    '',
...
async function main() {
  ...
  console.log(
    '[real-window-smoke] Build first via `npm --workspace @sharker/desktop run smoke:real-window`.',
  );
  console.log(
