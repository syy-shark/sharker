...
Sharker 只有一个执行 authority：Runtime Host。Desktop、TUI、CLI、bot 和 Eval client 都请求 Runtime Host 执行工作，不再拥有第二套 Runtime。
...
Runtime Host 拥有 Session 和 Turn identity、agent lifecycle、continuation、tools、permissions 与 events。`@sharker/eval` 只拥有 benchmark 实验语义：subjects、tasks、repetitions、cells、immutable attempts、result selection、budget 和 verifier 配置。Sharker subject 必须经过公开的 Runtime Host client/protocol 边界；外部竞品是 generic external subject。
...
continuation = Sharker subject 内部的 Runtime Host 行为
```
...
| `packages/cli` | TUI、`sharker run` 与唯一公开 `sharker eval` 路由 |
