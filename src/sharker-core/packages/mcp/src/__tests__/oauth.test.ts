...
import { MCP_CONFIG_VERSION, type McpConfigFile } from '@sharker/core/mcp';
import {
...
describe('McpClientManager OAuth E2E', () => {
  test('needs-auth → authorize → connected, with PKCE and Bearer-protected calls', async () => {
    ...
    const start = await manager.startAuthorization('remote', redirectUrl, { state: 'sharker-state' });
    ...
    assert.equal(authorizationUrl.searchParams.get('state'), 'sharker-state');
    ...
    assert.equal(start.state, 'sharker-state');
    ...
    assert.equal(location.searchParams.get('state'), 'sharker-state');
    const code = location.searchParams.get('code');
    assert.ok(code);

    const status = await manager.finishAuthorization('remote', { code, state: 'sharker-state' });
...
function createProtocolServer(reflect?: () => string): McpServer {
  const server = new McpServer(
    { name: 'sharker-oauth-fixture', version: '1.0.0' },
    { capabilities: { tools: {} } },
  );
