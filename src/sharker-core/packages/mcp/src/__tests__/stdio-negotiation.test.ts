...
import { MCP_CONFIG_VERSION, type McpConfigFile, type McpProtocolPreference } from '@sharker/core/mcp';
...
describe('McpClientManager stdio protocol negotiation', { concurrency: false }, () => {
  ...
  test('aborting during a delayed probe reaps it and never starts the actual child', async () => {
    const fixture = await fixtureConfig('modern-only', 'auto', {
      SHARKER_MCP_STDIO_FACTORY_DELAY_MS: '30000',
    });
    const manager = createManager();
...
async function fixtureConfig(
  ...
  const root = await realpath(await mkdtemp(join(tmpdir(), 'sharker-mcp-stdio-negotiation-')));
  roots.push(root);
  ...
  const config: McpConfigFile = {
    ...
    mcpServers: {
      fixture: {
        ...
        cwd: root,
        env: {
          SHARKER_MCP_STDIO_EVENT_LOG: log,
          SHARKER_MCP_STDIO_FIXTURE_VALUE: 'same-launch-input',
          ...extraEnv,
        },
