...
/** The slice of @sharker/storage's CredentialStore this adapter needs, stated
 * structurally so @sharker/mcp does not depend on @sharker/storage. */
...
export function createCredentialMcpOAuthStorage(store: McpCredentialSecretStore): McpOAuthStorage {
  return {
    ...
    // raw compare-and-set, so a writer outside this process (another Sharker
