...
It needs Docker, the pinned proxy image, and `python:3.12-slim`, so it is opt-in:
    ...
    SHARKER_EVAL_EGRESS_PROXY_TEST=1 python3 harbor/test_egress_filter_live.py
"""
...
@unittest.skipUnless(
    os.environ.get("SHARKER_EVAL_EGRESS_PROXY_TEST") == "1",
    "set SHARKER_EVAL_EGRESS_PROXY_TEST=1 to run the live mitmproxy proxy test",
)
class LiveEgressFilterTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.network = f"sharker-eval-egress-live-{run_id}-net"
        cls.proxy = f"sharker-eval-egress-live-{run_id}-proxy"
        cls.origin = f"sharker-eval-egress-live-{run_id}-origin"
        cls.workdir = Path(tempfile.mkdtemp(prefix="sharker-eval-egress-proxy-live-"))
        ...
        subprocess.run(
            [
                ...
                "127.0.0.1::8080",
                "-v",
                f"{HARBOR_DIR / 'egress_filter.py'}:/opt/sharker-eval/egress_filter.py:ro",
                PROXY_IMAGE,
            ],
            check=True,
            timeout=COMMAND_TIMEOUT_S,
    ...
    def audit_records(cls) -> list[dict[str, object]]:
        listed = subprocess.run(
            [
                ...
                "sh",
                "-c",
                "if [ -f /opt/sharker-egress-state/hits.jsonl ]; then cat /opt/sharker-egress-state/hits.jsonl; fi",
            ],
            capture_output=True,
            text=True,
            timeout=COMMAND_TIMEOUT_S,
