...
"""One-shot Harbor/Pier Agent that delegates one subject execution to @sharker/eval."""
...
RESULT_FRAME_PREFIX = "SHARKER-EVAL-RESULT-V1"
SCOPE_ERROR_PREFIX = "SHARKER-EVAL-SCOPE-ERROR-V1"
RESULT_PAYLOAD_LIMIT_BYTES = 2 * 1024
RESULT_CARRIER_LIMIT_BYTES = 64 * 1024
SUBJECT_STDOUT_PATH = "/logs/artifacts/sharker-subject.stdout.txt"
SUBJECT_STDERR_PATH = "/logs/artifacts/sharker-subject.stderr.txt"
CAPABILITY_PREFIX = "SHARKER-EVAL-CAPABILITIES-V1 "
NAMESPACE_PREFIX = "SHARKER-EVAL-POLICY-NAMESPACE-V1 "
...
PROXY_IPV4_PATH = "/opt/sharker-egress/proxy-ipv4"
PROXY_HOSTS_PREFIX = "SHARKER-EVAL-PROXY-HOST-V1 "
...
class RelayAgent(BaseAgent):
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not isinstance(teardown_timeout_ms, int) or teardown_timeout_ms <= 0:
            raise RuntimeError("Sharker Eval teardown timeout is invalid")
        self._teardown_timeout = teardown_timeout_ms / 1000

    @staticmethod
    def name() -> str:
        return "sharker-eval-relay"
    ...
    async def run(self, instruction: str, environment: Any, context: Any) -> None:
        ...
        scope_path = f"/logs/agent/.sharker-eval-{self._token}.pid"
        environment_path = f"/tmp/sharker-eval-{self._token}.env"
        try:
            cwd_prefix = f"SHARKER-EVAL-CWD-V1 {self._token} "
            ...
            if working_directory.return_code != 0 or not cwd.startswith("/") or "\x00" in cwd:
                raise RuntimeError("Sharker Eval could not resolve the task working directory")
            ...
            if not await _send(
                ...
                raise RelayTransportClosed("Sharker Eval relay transport closed before ready")
            ...
            if decision in done and execution not in done:
                decision.result()
                raise RelayTransportClosed("Sharker Eval relay received control before execution")
            ...
            if not await _send(
                ...
                raise RelayTransportClosed("Sharker Eval relay transport closed before result")
...
async def _prepare_command(
    ...
    if not isinstance(credentials, dict) or not all(
        ...
        raise RuntimeError("invalid Sharker Eval credentials")
    if not isinstance(public_environment, dict) or not all(
        isinstance(key, str) and isinstance(value, str)
        for key, value in public_environment.items()
    ):
        raise RuntimeError("invalid Sharker Eval environment")
    if set(credentials) & set(public_environment):
        raise RuntimeError("Sharker Eval environment overlaps credentials")
    capture_stdout = request.get("captureStdout", True)
    if not isinstance(capture_stdout, bool):
        raise RuntimeError("invalid Sharker Eval stdout policy")
    result_token = request.get("resultToken")
    if not isinstance(result_token, str) or re.fullmatch(r"[0-9a-f]{32}", result_token) is None:
        raise RuntimeError("invalid Sharker Eval result token")
    if "SHARKER_EVAL_RESULT_TOKEN" in credentials or "SHARKER_EVAL_RESULT_TOKEN" in public_environment:
        raise RuntimeError("Sharker Eval environment contains a reserved name")
    for label, values in (("environment", public_environment), ("credential", credentials)):
        if any(re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", key) is None for key in values):
            raise RuntimeError(f"invalid Sharker Eval {label} name")

    container_path = f"/tmp/sharker-eval-{token}.env"
    secret_path = None
    try:
        with tempfile.NamedTemporaryFile("w", encoding="utf-8", delete=False) as secret:
            ...
            for key, value in {
                **public_environment,
                **credentials,
                "SHARKER_EVAL_RESULT_TOKEN": result_token,
            }.items():
...
async def _require_constrained_subject(environment: Any) -> None:
    ...
    """
    if os.environ.get("SHARKER_EVAL_EGRESS_REQUIRED") != "1":
        return
    ...
    if probe.return_code != 0 or policy.return_code != 0:
        raise RuntimeError("Sharker Eval could not read the subject isolation evidence")
    ...
    for field in capabilities.split():
        ...
        try:
            ...
            raise RuntimeError("Sharker Eval could not read the subject capability set") from None
    if not all(field in reported for field in CAPABILITY_FIELDS):
        raise RuntimeError("Sharker Eval could not read the subject capability set")
    granted = 0
...
async def _pin_proxy_hostname(environment: Any) -> None:
    ...
    """
    host = os.environ.get("SHARKER_EVAL_EGRESS_ALLOWED_HOST") or "sharker-eval-mitmproxy"
...
"""
    ...
    if probe.return_code != 0:
        raise RuntimeError("Sharker Eval could not pin the Eval egress proxy hostname")
...
async def _persist_subject_outputs(environment: Any, result: Any) -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        if prepared.return_code != 0:
            raise RuntimeError("Sharker Eval could not prepare subject artifact output")
...
async def _settle(environment: Any, cwd: str, scope_path: str, execution: Any) -> Any:
    if execution.cancelled():
        raise RuntimeError("Sharker Eval subject execution was cancelled")
    if execution.done():
        result = execution.result()
    else:
        result = None
        for signal, timeout in (("TERM", 20), ("KILL", 10)):
            ...
            try:
                ...
                if execution.cancelled():
                    raise RuntimeError("Sharker Eval subject execution was cancelled") from None
                raise
            except TimeoutError:
                pass
        if result is None:
            raise RuntimeError("Sharker Eval subject did not settle")
...
async def _quiesce_scope(environment: Any, cwd: str, scope_path: str) -> None:
    ...
    raise RuntimeError("Sharker Eval execution scope did not quiesce")
...
async def _scope_active(environment: Any, cwd: str, scope_path: str) -> bool:
    ...
    if result.return_code == 3:
        return False
    raise RuntimeError("Sharker Eval execution scope evidence was unavailable")


def _require_message(value: dict[str, Any], token: str, kind: str) -> None:
    if value.get("token") != token or value.get("kind") != kind:
        raise RuntimeError("invalid Sharker Eval relay message")
...
async def _receive(reader: asyncio.StreamReader) -> dict[str, Any]:
    try:
        ...
        if not raw:
            raise RelayTransportClosed("Sharker Eval relay peer closed")
        value = json.loads(raw)
    except (ConnectionError, json.JSONDecodeError, ValueError) as error:
        raise RelayTransportClosed("Sharker Eval relay message was unavailable") from error
    if not isinstance(value, dict):
        raise RelayTransportClosed("Sharker Eval relay message was invalid")
    return value
