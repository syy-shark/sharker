...
AUDIT_PATH = Path(
    os.environ.get("SHARKER_EVAL_EGRESS_AUDIT", "/opt/sharker-egress-state/hits.jsonl")
)
...
def public_trajectory_repository(host: str, path_query: str) -> bool:
    return (
        ...
        and "hqeric/sharker-eval-trajectories" in path_query
    )
    ...
    try:
        ...
        flow.response = http.Response.make(
            ...
            b"Eval egress policy could not classify this request.\n",
            {
                ...
                "X-Sharker-Eval-Egress-Rule": "policy_error",
            },
        )
        try:
...
def blocked_response(rule_id: str):
    return http.Response.make(
        ...
        {
            "Content-Type": "text/plain; charset=utf-8",
            "X-Sharker-Eval-Egress-Rule": rule_id,
        },
    )
