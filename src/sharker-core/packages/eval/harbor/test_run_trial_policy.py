...
SPEC = importlib.util.spec_from_file_location("sharker_eval_run_trial", MODULE_PATH)
...
class RunTrialPolicyTest(unittest.TestCase):
    def test_forces_only_the_subject_phase_through_the_cell_proxy(self) -> None:
        ...
        with patch.dict(
            os.environ,
            {
                "SHARKER_EVAL_EGRESS_REQUIRED": "1",
                "SHARKER_EVAL_EGRESS_ALLOWED_HOST": "sharker-eval-mitmproxy",
            },
            clear=False,
        ):
            MODULE.apply_subject_egress_policy(task)
        self.assertEqual(agent.network_mode, "allowlist")
        self.assertEqual(agent.allowed_hosts, ["sharker-eval-mitmproxy"])
    ...
    def test_required_egress_fails_closed_without_the_proxy_host(self) -> None:
        ...
        with patch.dict(os.environ, {"SHARKER_EVAL_EGRESS_REQUIRED": "1"}, clear=True):
