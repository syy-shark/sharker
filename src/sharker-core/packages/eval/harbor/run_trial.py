...
async def task_cache_lock(cache_dir: Path, identity: str) -> AsyncIterator[None]:
    lock_dir = cache_dir / ".sharker-locks"
...
def ready_task(cache_dir: Path, identity: str) -> tuple[Path, Path | None]:
    namespace = cache_dir / ".sharker-tasks" / hashlib.sha256(identity.encode()).hexdigest()
...
def publish_ready_task(namespace: Path, target: Path) -> None:
    ...
    if not resolved_target.is_relative_to(resolved_namespace):
        raise RuntimeError("Harbor task cache target escaped its Sharker namespace")
...
def apply_subject_egress_policy(task: object) -> None:
    required = os.environ.get("SHARKER_EVAL_EGRESS_REQUIRED") == "1"
    allowed_host = os.environ.get("SHARKER_EVAL_EGRESS_ALLOWED_HOST")
    if not required:
        return
    if not allowed_host:
