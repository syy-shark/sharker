...
const DEEPSEEK_HARNESS_PROFILE = 'sharker-eval';

const PROVIDER_USAGE_CHECKPOINT_SCHEMA = 'sharker.external_provider_usage.v2';
...
const PROFILE_PREPARERS: Record<Profile, (setup: ProfileSetup) => Promise<string | undefined>> = {
  codex: async ({ env, home, root, proxyBaseUrl }) => {
    env.OPENAI_API_KEY = 'sharker-eval-local';
    env.CODEX_HOME = home;
    const catalog = rooted(
      root,
      '/opt/sharker-agent/node_modules/@sharker/eval/harbor/deepseek-codex-models.json',
    );
    await writeFile(
      join(home, 'config.toml'),
      [
        'model_provider = "sharker-http"',
        `model_catalog_json = ${JSON.stringify(catalog)}`,
        'model_reasoning_effort = "max"',
        '',
        '[model_providers.sharker-http]',
        'name = "Sharker Eval HTTP"',
  ...
  'claude-code': async ({ env, home, root, proxyBaseUrl }) => {
    ...
    env.ANTHROPIC_API_KEY = 'sharker-eval-local';
  ...
  reasonix: async ({ env, home, proxyBaseUrl, executableArgs }) => {
    ...
    if (provider !== 'sharker-proxy' || !model || !effort) {
    ...
    await writeFile(path, 'OPENAI_API_KEY=sharker-eval-local\n', { mode: 0o600 });
    env.OPENAI_API_KEY = 'sharker-eval-local';
  ...
  opencode: async ({ env, home, proxyBaseUrl }) => {
    env.DEEPSEEK_API_KEY = 'sharker-eval-local';
  ...
  'kimi-code': async ({ env, home, proxyBaseUrl }) => {
    ...
    env.KIMI_MODEL_API_KEY = 'sharker-eval-local';
  ...
  'deepseek-harness': async ({ env, home, root, proxyBaseUrl }) => {
    ...
    // task was left in, so no framework — this one, Sharker, or the relay — may
...
writeRelayResult(resultToken, {
  schemaVersion: 'sharker.external_subject_result.v2',
  status,
  failureReason,
  usage,
  costUsd,
...
async function prepareProfile(
  ...
  const home = rooted(root, `/tmp/sharker-eval-${selected}`);
...
function joinUpstream(baseUrl: string, requestUrl: string): string {
  ...
  const request = new URL(requestUrl, 'http://sharker-eval.local');
