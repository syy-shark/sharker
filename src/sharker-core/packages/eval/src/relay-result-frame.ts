...
export function takeRelayResultToken(): string {
  const token = process.env.SHARKER_EVAL_RESULT_TOKEN;
  delete process.env.SHARKER_EVAL_RESULT_TOKEN;
...
export function writeRelayResult(token: string, value: unknown): void {
  ...
  process.stdout.write(
    `SHARKER-EVAL-RESULT-V1 ${token} ${payload.byteLength} ${digest} ${payload.toString('base64url')}\n`,
  );
}
