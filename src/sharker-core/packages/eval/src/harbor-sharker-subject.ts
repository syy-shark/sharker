...
import { runHostedExecution } from '@sharker/runtime-host/client';
import type { HostedExecutionStartInput } from '@sharker/runtime-host/protocol';
import { captureSharkerRuntimeArtifacts, writeSharkerArtifactCollectionError } from './sharker-artifacts.js';
...
const captureArtifacts = (reason: 'settled' | 'signal') => {
  artifactCapture = artifactCapture.then(async () => {
    try {
      await captureSharkerRuntimeArtifacts({
        stateRoot: payload.rootPath,
        destinationRoot: payload.artifactRoot,
        reason,
      });
    } catch (error) {
      await writeSharkerArtifactCollectionError(payload.artifactRoot, error).catch(() => undefined);
    }
  });
...
await writeFile(
  ...
  `${JSON.stringify(sharkerEvalRuntimePolicyDocument(process.env.HTTPS_PROXY))}\n`,
  { flag: 'wx', mode: 0o600 },
);
