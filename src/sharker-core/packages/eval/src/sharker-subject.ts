...
import { isSessionToolProfile, type SessionToolProfile } from '@sharker/core/session';
import { decodeHostedExecutionProjection } from '@sharker/runtime-host/protocol';
import type { RunHostedExecutionInput } from '@sharker/runtime-host/client';
import type { JsonObject } from './experiment.js';
import {
  SHARKER_RUNTIME_ARTIFACT_PATH,
  SHARKER_SUBJECT_STDERR_PATH,
  SHARKER_SUBJECT_STDOUT_PATH,
} from './sharker-artifacts.js';
...
export function createSharkerSubjectAdapter(): SubjectAdapter {
  return {
    kind: 'sharker',
    ...
    async execute({ cell, context }) {
      ...
      const payload = Buffer.from(
        JSON.stringify({
          ...
          artifactRoot: SHARKER_RUNTIME_ARTIFACT_PATH,
      ...
      if (process.termination !== 'exited') {
        ...
        return {
          ...
          failureReason: 'Sharker subject exceeded the framework timeout',
          artifacts: sharkerArtifacts(executionId, process, settled?.costUsd),
        };
      }
      ...
      if (projection.kind === 'indeterminate') {
        return {
          ...
          failureReason: safeFailureReason(
            ...
            'Sharker execution did not settle',
          ),
          artifacts: sharkerArtifacts(executionId, process),
        };
      }
      const result = (
        ...
        status,
        failureReason,
        artifacts: sharkerArtifacts(executionId, process, projection.costUsd),
      });
      ...
      if (projection.status === 'cancelled') {
        return result('indeterminate', 'Sharker subject cancelled');
      }
      return result(
        projection.status,
        projection.status === 'completed' || projection.failureReason == null
          ? null
          : safeFailureReason(projection.failureReason, 'Sharker execution failed'),
      );
    },
  };
}
...
function subjectFailure(
  ...
  return {
    ...
    failureReason: cancelled ? 'Sharker subject cancelled' : `Sharker subject failed during ${stage}`,
    artifacts: [
      ...sharkerArtifacts(undefined, process),
      {
...
function sharkerArtifacts(
  ...
  return [
    ...
    {
      kind: 'sharker-runtime-state',
      path: SHARKER_RUNTIME_ARTIFACT_PATH,
      ...(executionId ? { executionId } : {}),
    },
    {
      kind: 'subject-stdout',
      path: SHARKER_SUBJECT_STDOUT_PATH,
      ...(process ? { bytes: Buffer.byteLength(process.stdout) } : {}),
    },
    {
      kind: 'subject-stderr',
      path: SHARKER_SUBJECT_STDERR_PATH,
    },
  ];
}

interface SharkerConfig {
...
function decodeConfig(value: JsonObject): SharkerConfig {
  const fields = [
    'nodePath',
    'shimPath',
    'runtimeHostsPath',
  ...
  if (!URL.canParse(String(config.baseUrl))) throw new Error('Sharker baseUrl is invalid');
  const hostSettlementTimeoutMs = positiveInteger(
    config.hostSettlementTimeoutMs,
    'Sharker config.hostSettlementTimeoutMs',
  );
  if (!isSessionToolProfile(config.toolProfile)) {
    throw new Error('Sharker config.toolProfile is invalid');
  }
  return { ...config, hostSettlementTimeoutMs } as unknown as SharkerConfig;
}

function exact(value: unknown, fields: readonly string[]): Record<string, unknown> {
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new Error('Sharker config must be an object');
  const record = value as Record<string, unknown>;
  if (
    Object.keys(record).length !== fields.length ||
    fields.some((field) => !Object.hasOwn(record, field))
  )
    throw new Error('Sharker config fields are invalid');
  for (const field of fields) {
    if (field === 'hostSettlementTimeoutMs') continue;
    if (typeof record[field] !== 'string' || record[field] === '')
      throw new Error(`Sharker config.${field} is invalid`);
  }
  return record;
}
