...
export const SHARKER_RUNTIME_ARTIFACT_PATH = '/logs/artifacts/sharker-runtime-host';
export const SHARKER_SUBJECT_STDOUT_PATH = '/logs/artifacts/sharker-subject.stdout.txt';
export const SHARKER_SUBJECT_STDERR_PATH = '/logs/artifacts/sharker-subject.stderr.txt';
...
export interface SharkerRuntimeArtifactManifest {
  readonly schemaVersion: 'sharker.eval.runtime_artifacts.v1';
  readonly reason: 'settled' | 'signal';
  readonly capturedAt: number;
  readonly files: readonly CapturedFile[];
}

export async function captureSharkerRuntimeArtifacts(input: {
  readonly stateRoot: string;
  readonly destinationRoot: string;
  readonly reason: SharkerRuntimeArtifactManifest['reason'];
  readonly now?: () => number;
}): Promise<SharkerRuntimeArtifactManifest> {
  ...
  try {
    ...
    const manifest: SharkerRuntimeArtifactManifest = {
      schemaVersion: 'sharker.eval.runtime_artifacts.v1',
      reason: input.reason,
...
export async function writeSharkerArtifactCollectionError(
  ...
  await writeFile(
    ...
    `${JSON.stringify({
      schemaVersion: 'sharker.eval.runtime_artifact_error.v1',
