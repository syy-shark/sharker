...
const USAGE = 'usage: sharker eval run <spec.json> --out <directory> [--cell <cell-id>]';
...
export async function runSharkerEvalCli(
  ...
  try {
    ...
    const results = await runExperiment({
      ...
      executor,
      subjects: overrides.subjects ?? [createSharkerSubjectAdapter(), createExternalSubjectAdapter()],
    ...
    (overrides.stderr ?? process.stderr.write.bind(process.stderr))(
      `sharker eval: ${error instanceof Error ? error.message : String(error)}\n${USAGE}\n`,
    );
