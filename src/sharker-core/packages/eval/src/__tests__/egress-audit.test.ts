...
test('a missing expected audit through runAttempt is infra_failed and excluded', {
  ...
  assert.deepEqual(trialConfig, [
    {
      source: '/opt/sharker-egress-state/hits.jsonl',
      destination: EGRESS_AUDIT_DESTINATION,
      service: 'sharker-eval-mitmproxy',
    },
  ]);
});
...
async function runHarborTrial(options: {
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-egress-audit-'));
...
const rewardEnv = process.env.SHARKER_TEST_REWARD;
const exceptionType = process.env.SHARKER_TEST_EXCEPTION;
const result = {
  verifier_result: {
...
await writeFile(process.env.SHARKER_TEST_TRIAL_CONFIG, JSON.stringify(config.artifacts ?? null));
const mode = process.env.SHARKER_TEST_AUDIT_MODE;
if (mode !== 'missing') {
...
`,
  ...
  const restore = setEnvironment({
    ...
    SHARKER_TEST_BUNDLE: EVAL_ROOT,
    SHARKER_TEST_AUDIT_MODE: options.auditMode,
    SHARKER_TEST_TRIAL_CONFIG: trialConfigPath,
    SHARKER_TEST_REWARD: options.reward === null ? 'none' : String(options.reward ?? 1),
    ...(options.exceptionType ? { SHARKER_TEST_EXCEPTION: options.exceptionType } : {}),
  });
  try {
...
function executorConfig(egressProxy: boolean): JsonObject {
  return {
    ...
    preparationEnvironment: [
      'SHARKER_TEST_AUDIT_MODE',
      'SHARKER_TEST_TRIAL_CONFIG',
      'SHARKER_TEST_REWARD',
      'SHARKER_TEST_EXCEPTION',
    ],
    mounts: [],
    ...(egressProxy
      ? {
