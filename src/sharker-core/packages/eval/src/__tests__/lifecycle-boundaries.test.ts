...
import { sharkerEvalRuntimePolicyDocument } from '../sharker-runtime-policy.js';
import { createSharkerSubjectAdapter } from '../sharker-subject.js';
...
test('normal verification outlives the former Eval completion deadline', {
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-verifier-deadline-'));
...
await writeFile(process.env.SHARKER_TEST_PID, String(process.pid));
...
await message();
await writeFile(process.env.SHARKER_TEST_VERIFYING, '');
while (true) {
  try { await readFile(process.env.SHARKER_TEST_RELEASE); break; }
...
`,
  ...
  const restoreEnvironment = setEnvironment({
    SHARKER_TEST_PYTHON: executable,
    SHARKER_TEST_TRIALS: root,
    SHARKER_TEST_PID: pidPath,
    SHARKER_TEST_VERIFYING: verifying,
    SHARKER_TEST_RELEASE: release,
  });
  ...
  try {
    const running = runExperiment({
      ...
      executor: createHarborExecutor(
        {
          ...executorConfig(),
          preparationEnvironment: ['SHARKER_TEST_PID', 'SHARKER_TEST_VERIFYING', 'SHARKER_TEST_RELEASE'],
        },
...
test('cancellation before verification settles remains replaceable after clean teardown', {
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-verifier-cancel-'));
  const executable = join(root, 'fake-python.mjs');
  const verifying = join(root, 'verifying');
  await writeFile(
    executable,
...
`,
  ...
  try {
    ...
    const running = runExperiment({
      ...
      store,
      executor: createHarborExecutor(
        { ...executorConfig(), preparationEnvironment: ['SHARKER_TEST_VERIFYING'] },
        join(root, 'experiment.json'),
      ),
      subjects: [
        {
...
test('used trial abort terminates the supervisor without a second wire decision', {
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-used-abort-'));
  const executable = join(root, 'fake-python.mjs');
  await writeFile(
    executable,
    `#!/usr/bin/env node
...
test('SIGKILL of the supervisor does not confirm host cleanup', { timeout: 5_000 }, async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-killed-cleanup-'));
  const executable = join(root, 'fake-python.mjs');
  await writeFile(
    executable,
    `#!/usr/bin/env node
...
test('Sharker framework termination is authoritative before stdout decoding', async () => {
  for (const stdout of ['', '{']) {
    const result = await executeSharker('framework_timeout', () => stdout);
    assert.equal(result.status, 'failed');
    assert.equal(result.failureReason, 'Sharker subject exceeded the framework timeout');
  ...
  const reported = 999;
  const retained = await executeSharker('framework_timeout', (executionId) =>
    JSON.stringify({
      executionId,
      kind: 'settled',
      status: 'cancelled',
  ...
  // run went: the runtime prices this model itself, so without this the Sharker
  // arm bills from the runtime's table and every other arm from Eval's.
  const completed = await executeSharker('exited', (executionId) =>
    JSON.stringify({
      executionId,
      kind: 'settled',
      status: 'completed',
...
test('Sharker forwards the configured Runtime Host settlement budget', async () => {
  const sharkerCell = cell('sharker', { ...sharkerConfig(), hostSettlementTimeoutMs: 120_000 });
  let settlementBudget: unknown;
  const result = await createSharkerSubjectAdapter().execute({
    cell: sharkerCell,
    context: {
      cwd: '/workspace',
      taskInput: 'solve',
  ...
  assert.throws(
    () =>
      createSharkerSubjectAdapter().validate?.(
        cell('sharker', { ...sharkerConfig(), hostSettlementTimeoutMs: 120_000.5 }),
      ),
...
// verifier while the others lose theirs. This pins both halves of the Sharker
// side: what the shim projects, and that the adapter reads the frame rather
// than re-deciding from the code it just projected.
test('the Sharker shim projects only a completed subject as a zero exit', async () => {
  const shim = new URL('../harbor-sharker-subject.js', import.meta.url);
  for (const [projection, expectedExit, expectedStatus] of [
    ...
    const root = await mkdtemp(join(tmpdir(), 'sharker-eval-shim-exit-'));
    try {
      ...
      const { exitCode, stdout } = await execFileAsync(
        ...
        [
          ...
          `data:text/javascript,${encodeURIComponent(
            `import{register}from"node:module";register("data:text/javascript,${encodeURIComponent(
              `export async function resolve(s,c,n){return s==="@sharker/runtime-host/client"?{url:${JSON.stringify(
        ...
        { env: { ...process.env, SHARKER_EVAL_RESULT_TOKEN: '0'.repeat(32) } },
      )
      ...
      const result = await createSharkerSubjectAdapter().execute({
        cell: cell('sharker', sharkerConfig()),
        context: {
          cwd: '/workspace',
          taskInput: 'solve',
          metadata: {},
      ...
      assert.ok(stdout.includes('SHARKER-EVAL-RESULT-V1'));
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  }
...
test('subject preflight settles before the attempt timer starts', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-preflight-order-'));
  const events: string[] = [];
...
test('resume replaces a terminal attempt whose subject identity is stale', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-resume-identity-'));
  const store = new FileAttemptStore(join(root, 'attempts'));
  await store.append({
...
test('eight-arm spec and wrappers freeze the working provider contracts', async () => {
  ...
  const sharker = spec.subjects.find(({ id }) => id === 'sharker')!;
  const codex = spec.subjects.find(({ id }) => id === 'codex')!;
  const claude = spec.subjects.find(({ id }) => id === 'claude-code')!;
  assert.deepEqual(sharker.credentials, ['DEEPSEEK_API_KEY']);
  assert.equal(sharker.config.connectionSlug, 'env-deepseek');
  assert.equal(sharker.config.baseUrl, 'https://api.deepseek.com');
  assert.equal(sharker.config.toolProfile, 'headless-coding-v1');
  ...
  for (const subject of spec.subjects) {
    ...
    if (subject.id === 'sharker') continue;
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-profiles-'));
  const wrapper = new URL('../harbor-external-subject.js', import.meta.url);
  const env = {
    ...process.env,
    OPENAI_API_KEY: 'test-only-key',
    ANTHROPIC_API_KEY: 'test-only-key',
    SHARKER_EVAL_RESULT_TOKEN: '0123456789abcdef0123456789abcdef',
  };
  ...
  const profileSource = join(
    root,
    'opt/sharker-agent/node_modules/@sharker/eval/harbor/deepseek-harness-profile',
  );
  ...
  try {
    for (const args of [
      ...
      [
        ...
        '/usr/bin/true',
        '--model',
        'sharker-proxy/deepseek-v4-flash',
        '--effort',
        'max',
      ],
      ...
      assert.equal(
        ...
        'sharker.external_subject_result.v2',
      );
    }
    const codexConfig = await readFile(join(root, 'tmp/sharker-eval-codex/config.toml'), 'utf8');
    ...
    assert.match(
      await readFile(join(root, 'tmp/sharker-eval-reasonix/config.toml'), 'utf8'),
      /enabled = \["bash", "read_file", "write_file", "edit_file", "glob", "grep"\]/u,
    );
    const piConfig = JSON.parse(
      await readFile(join(root, 'tmp/sharker-eval-pi/models.json'), 'utf8'),
    ) as {
      providers: {
        'sharker-proxy': {
          api: string;
    ...
    assert.equal(piConfig.providers['sharker-proxy'].api, 'openai-responses');
    assert.match(piConfig.providers['sharker-proxy'].baseUrl, /^http:\/\/127\.0\.0\.1:\d+\/v1$/u);
    assert.deepEqual(piConfig.providers['sharker-proxy'].models[0], {
    ...
    const profileRoot = join(root, 'tmp/sharker-eval-deepseek-harness/dsh/profiles');
...
function decodeResultFrame(stdout: string, token: string): Record<string, unknown> {
  ...
  assert.equal(prefix, 'SHARKER-EVAL-RESULT-V1');
...
test('eight-arm spec adds Pi with the same pinned DeepSeek execution contract', async () => {
  ...
  assert.deepEqual(
    ...
    ['sharker', 'codex', 'claude-code', 'reasonix', 'opencode', 'kimi-code', 'zcode', 'pi'],
  );
  assert.deepEqual(spec.executor.config.egressProxy, {
    composeSourceEnv: 'SHARKER_EVAL_SHARKER_BUNDLE_PATH',
    composeRelativePath: 'node_modules/@sharker/eval/harbor/docker-compose-egress-proxy.yaml',
    networkPolicyRelativePath: 'node_modules/@sharker/eval/harbor/egress-proxy/network-policy',
    proxyUrl: 'http://sharker-eval-mitmproxy:8080',
    allowedHost: 'sharker-eval-mitmproxy',
    containerCaPath: '/opt/sharker-egress/mitmproxy-ca-cert.pem',
  });
  ...
  assert.match(subjectService, /sharker-eval-egress-ca:\/opt\/sharker-egress:ro/u);
  ...
  assert.match(sidecarService, /sharker-eval-egress-ca:\/opt\/sharker-egress:ro/u);
  ...
  assert.match(networkPolicy, /\/opt\/sharker-egress\/proxy-ipv4/u);
  ...
  assert.deepEqual(
    ...
    [
      '/opt/sharker-agent',
      '/opt/sharker-node-toolchain',
      '/opt/sharker-codex-toolchain',
      '/opt/sharker-claude-code-toolchain',
      '/opt/sharker-reasonix-toolchain',
      '/opt/sharker-opencode-toolchain',
      '/opt/sharker-kimi-code-toolchain',
      '/opt/sharker-zcode-toolchain',
      '/opt/sharker-pi-toolchain',
    ],
  );
  ...
  assert.equal(
    (sharker.config as { hostSettlementTimeoutMs?: number }).hostSettlementTimeoutMs,
    120_000,
  );
  assert.equal(sharker.config.toolProfile, 'headless-coding-v1');
  assert.deepEqual(
  ...
  assert.equal(pi.config.args?.includes('/opt/sharker-pi-toolchain/bin/pi'), true);
...
test('the DeepSeek Harness arm pins its own minimal composition', async () => {
  ...
  assert.deepEqual(
    spec.executor.config.mounts.map(({ target }) => target),
    ['/opt/sharker-agent', '/opt/sharker-node-toolchain', '/opt/sharker-deepseek-harness-toolchain'],
  );
  // The single-arm spec keeps the cohort's egress enforcement rather than
  // running the harness with unaudited network access.
  assert.equal(spec.executor.config.egressProxy.proxyUrl, 'http://sharker-eval-mitmproxy:8080');
  ...
  assert.equal(args.includes('/opt/sharker-deepseek-harness-toolchain/bin/node'), true);
  assert.equal(
    args.includes(
      '/opt/sharker-deepseek-harness-toolchain/lib/dsh/node_modules/@deepseek-ai/dsh/lib/bin.js',
    ),
    true,
  );
...
test('Sharker Eval policy enables privacy independently of the tool profile', () => {
  const document = sharkerEvalRuntimePolicyDocument();
...
test('experiment specs do not declare an executor working-directory authority', async () => {
  for (const name of [
    ...
    for (const subject of spec.subjects.filter(({ kind }) => kind === 'sharker')) {
...
async function verifyPreparationCancellation(
  ...
  const root = await mkdtemp(join(tmpdir(), `sharker-eval-${stage}-cancel-`));
  const executable = join(root, 'fake-python.mjs');
...
process.on('SIGTERM', () => {
  terminations += 1;
  if (process.env.SHARKER_TEST_IGNORE_FIRST !== '1' || terminations > 1) process.exit(0);
});
await writeFile(process.env.SHARKER_TEST_PID, String(process.pid));
if (process.env.SHARKER_TEST_STAGE === 'ready') {
  const config = JSON.parse(await readFile(process.argv.at(-1), 'utf8'));
  const socket = connect(config.agent.kwargs.relay_port, config.agent.kwargs.relay_host);
  await new Promise((resolve, reject) => {
    socket.once('connect', resolve);
...
await writeFile(process.env.SHARKER_TEST_BARRIER, '');
setInterval(() => {}, 1_000);
await new Promise(() => {});
`,
  );
  ...
  const restore = setEnvironment({
    SHARKER_TEST_PYTHON: executable,
    SHARKER_TEST_TRIALS: root,
    SHARKER_TEST_BARRIER: barrier,
    SHARKER_TEST_PID: pidPath,
    SHARKER_TEST_STAGE: stage,
    SHARKER_TEST_IGNORE_FIRST: ignoreFirstTermination ? '1' : '0',
  });
  const nativeSetTimeout = globalThis.setTimeout;
  globalThis.setTimeout = ((
    callback: (...args: unknown[]) => void,
...
async function executeSharker(
  ...
  return createSharkerSubjectAdapter().execute({
    cell: cell('sharker', sharkerConfig()),
    context: {
      cwd: '/app',
      taskInput: 'solve',
...
function experiment(): ExperimentSpec {
  return {
    schemaVersion: 'sharker.eval.v1' as const,
    id: 'experiment',
...
test('pier cannot declare an egress proxy it never enforces', () => {
  const egressProxy = {
    composeSourceEnv: 'SHARKER_TEST_BUNDLE',
    composeRelativePath: 'node_modules/@sharker/eval/harbor/docker-compose-egress-proxy.yaml',
    networkPolicyRelativePath: 'node_modules/@sharker/eval/harbor/egress-proxy/network-policy',
    proxyUrl: 'http://sharker-eval-mitmproxy:8080',
    allowedHost: 'sharker-eval-mitmproxy',
  ...
  assert.throws(
    () =>
      createPierExecutor(
        { ...executorConfig(), tasksRootEnv: 'SHARKER_TEST_TASKS', egressProxy },
        'experiment.json',
      ),
...
test('Pier rejects configured mounts that collide with framework log ownership', () => {
  const root = join(tmpdir(), 'sharker-test-pier-reserved-mount');
  const restoreEnvironment = setEnvironment({
    SHARKER_TEST_MOUNT: join(root, 'mount'),
    SHARKER_TEST_PYTHON: join(root, 'python'),
    SHARKER_TEST_TASKS: join(root, 'tasks'),
    SHARKER_TEST_TRIALS: join(root, 'trials'),
  });
  try {
      ...
      assert.throws(
        () =>
          createPierExecutor(
            {
              ...executorConfig(),
              tasksRootEnv: 'SHARKER_TEST_TASKS',
              mounts: [{ sourceEnv: 'SHARKER_TEST_MOUNT', target, readOnly: true }],
            },
            'experiment.json',
          ),
        (error) =>
...
test('Pier preserves its log mounts without inheriting SHARKER_EVAL_FRAMEWORK', {
  timeout: 10_000,
}, async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-framework-env-'));
  const executable = join(root, 'fake-python.mjs');
...
import { mkdir, readFile, writeFile } from 'node:fs/promises';
const config = JSON.parse(await readFile(process.argv.at(-1), 'utf8'));
await writeFile(process.env.SHARKER_TEST_ENV, JSON.stringify({
  framework: process.env.SHARKER_EVAL_FRAMEWORK ?? null,
...
`,
  ...
  const restoreEnvironment = setEnvironment({
    SHARKER_TEST_PYTHON: executable,
    SHARKER_TEST_TRIALS: root,
    SHARKER_TEST_ENV: envDump,
    SHARKER_TEST_MOUNT: root,
    SHARKER_TEST_TASKS: root,
    SHARKER_EVAL_FRAMEWORK: 'pier',
  });
  try {
    const spec: ExperimentSpec = {
      ...
      executor: {
        ...
        config: {
          ...executorConfig(),
          tasksRootEnv: 'SHARKER_TEST_TASKS',
          mounts: [{ sourceEnv: 'SHARKER_TEST_MOUNT', target: '/input', readOnly: true }],
        },
      },
      ...
      executor: createPierExecutor(
        {
          ...executorConfig(),
          tasksRootEnv: 'SHARKER_TEST_TASKS',
          preparationEnvironment: ['SHARKER_TEST_ENV'],
          mounts: [{ sourceEnv: 'SHARKER_TEST_MOUNT', target: '/input', readOnly: true }],
        },
        join(root, 'experiment.json'),
      ),
...
function executorConfig(): JsonObject {
  return {
    ...
    pythonPathEnv: 'SHARKER_TEST_PYTHON',
    trialsRootEnv: 'SHARKER_TEST_TRIALS',
    environment: {},
    preparationEnvironment: [
      'SHARKER_TEST_BARRIER',
      'SHARKER_TEST_PID',
      'SHARKER_TEST_STAGE',
      'SHARKER_TEST_IGNORE_FIRST',
    ],
    mounts: [],
  };
}

function cell(kind: 'sharker' | 'external', config: JsonObject): ExperimentCell {
  return {
    id: `task::1::${kind}`,
...
function sharkerConfig() {
  return {
    nodePath: '/opt/node/bin/node',
    shimPath: '/opt/sharker/harbor-sharker-subject.js',
    runtimeHostsPath: '/tmp/sharker-runtime-hosts',
    baseUrl: 'https://provider.test/v1',
