...
test('provider metering checkpoint records admission before the request settles', async () => {
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-provider-checkpoint-'));
  ...
  try {
    ...
    assert.equal(checkpoint.schemaVersion, 'sharker.external_provider_usage.v2');
...
test('provider failures remain infrastructure failures until inference admission', async () => {
  for (const [
    ...
    const address = server.address();
    assert.ok(address && typeof address !== 'string');
    const root = await mkdtemp(join(tmpdir(), 'sharker-provider-admission-'));
    const child = join(root, 'child.mjs');
    await writeFile(
      child,
      "if(process.env.OPENAI_API_KEY||process.env.ANTHROPIC_API_KEY||process.env.DEEPSEEK_API_KEY!=='sharker-eval-local'||process.env.SHARKER_EVAL_RESULT_TOKEN)process.exit(9);await fetch(`${process.env.DEEPSEEK_BASE_URL}/responses`,{method:'POST',body:'{}'});console.error('stderr-sentinel-must-not-persist');console.log('stdout-sentinel-must-not-persist');console.log(JSON.stringify({type:'error'}));process.exit(1);\n",
    );
    try {
      const wrapper = new URL('../harbor-external-subject.js', import.meta.url);
...
async function executePiWithTerminalRecord(contentBytes: number): Promise<{
  ...
  const address = server.address();
  assert.ok(address && typeof address !== 'string');
  const root = await mkdtemp(join(tmpdir(), 'sharker-pi-terminal-'));
  const child = join(root, 'child.mjs');
  await writeFile(
    child,
    [
      ...
      "if(process.env.OPENAI_API_KEY!=='sharker-eval-local'||process.env.ANTHROPIC_API_KEY||process.env.DEEPSEEK_API_KEY)process.exit(9);",
      "await fetch(`${config.providers['sharker-proxy'].baseUrl}/responses`,{method:'POST',body:'{}'});",
  ...
  try {
    ...
    const { stdout } = await runWrapper(
      ...
      { OPENAI_API_KEY: 'upstream-test-key', SHARKER_EVAL_RESULT_TOKEN: RESULT_TOKEN },
    );
...
test('a request arriving while the proxy drains is refused, not counted', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-provider-late-'));
