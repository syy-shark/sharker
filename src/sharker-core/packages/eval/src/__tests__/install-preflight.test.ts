...
test('preflights the pinned Python framework and Docker before execution', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-install-preflight-'));
  ...
  const restore = setMachinePaths({
    SHARKER_TEST_PREFLIGHT_PYTHON: process.execPath,
    SHARKER_TEST_PREFLIGHT_TRIALS: trials,
    SHARKER_TEST_PREFLIGHT_MOUNT: mount,
    SHARKER_TEST_PREFLIGHT_EGRESS: egress,
    SHARKER_TEST_PREFLIGHT_SECRET: 'must-not-reach-preflight-processes',
  });
  t.after(restore);
  const calls: {
    command: string;
  ...
  await preflight(experiment('SHARKER_TEST_PREFLIGHT_SECRET', true), join(root, 'spec.json'), {
  ...
  assert.equal(calls[0]?.environment.SHARKER_TEST_PREFLIGHT_SECRET, undefined);
  assert.equal(calls[0]?.environment.SHARKER_EVAL_EGRESS_REQUIRED, '1');
  assert.equal(calls[0]?.environment.SHARKER_EVAL_EGRESS_ALLOWED_HOST, 'api.example.test');
  ...
  assert.equal(calls[0]?.environment.SHARKER_EVAL_NETWORK_POLICY_PATH, canonicalNetworkPolicyPath);
...
test('reports a missing machine mount before invoking external prerequisites', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-install-mount-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  const trials = join(root, 'trials');
  await mkdir(trials);
  const restore = setMachinePaths({
    SHARKER_TEST_PREFLIGHT_PYTHON: process.execPath,
    SHARKER_TEST_PREFLIGHT_TRIALS: trials,
    SHARKER_TEST_PREFLIGHT_MOUNT: join(root, 'missing-toolchain'),
  });
  t.after(restore);
  let commands = 0;
  ...
  await assert.rejects(
    ...
    /machine path SHARKER_TEST_PREFLIGHT_MOUNT does not exist/,
  );
...
test('rejects an unusable trials root before invoking external prerequisites', {
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-install-trials-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  const trials = join(root, 'trials');
  const mount = join(root, 'mount');
  ...
  await assert.rejects(
    ...
    /machine path SHARKER_TEST_PREFLIGHT_TRIALS is not writable and searchable/,
  );
  assert.equal(commands, 0);
});

test('rejects a dangling trials root symlink before invoking external prerequisites', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-install-trials-link-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  const trials = join(root, 'trials');
  const mount = join(root, 'mount');
  await mkdir(mount);
  ...
  await assert.rejects(
    ...
    /machine path SHARKER_TEST_PREFLIGHT_TRIALS is a dangling symbolic link/,
  );
  assert.equal(commands, 0);
});

test('identifies a mismatched Python framework environment', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-install-python-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  const trials = join(root, 'trials');
  const mount = join(root, 'mount');
  await Promise.all([mkdir(trials), mkdir(mount)]);
  ...
  await assert.rejects(
    ...
    /harbor Python environment SHARKER_TEST_PREFLIGHT_PYTHON .* harbor@0\.20\.0: installed 0\.19\.0/,
  );
});

test('rejects egress assets that escape their declared source root', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-install-egress-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  const trials = join(root, 'trials');
  const mount = join(root, 'mount');
  const egress = join(root, 'egress');
...
test('rejects egress assets that escape through a symlink', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-install-egress-link-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  const trials = join(root, 'trials');
  const mount = join(root, 'mount');
  const egress = join(root, 'egress');
...
test('propagates cancellation to an active prerequisite probe', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-install-cancel-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  const trials = join(root, 'trials');
  const mount = join(root, 'mount');
  await Promise.all([mkdir(trials), mkdir(mount)]);
  ...
  return parseExperimentSpec({
    ...
    executor: {
      ...
      config: {
        ...
        pythonPathEnv: 'SHARKER_TEST_PREFLIGHT_PYTHON',
        trialsRootEnv: 'SHARKER_TEST_PREFLIGHT_TRIALS',
        environment: { type: 'docker' },
        preparationEnvironment: credential ? [credential] : [],
        mounts: [
          {
            sourceEnv: 'SHARKER_TEST_PREFLIGHT_MOUNT',
