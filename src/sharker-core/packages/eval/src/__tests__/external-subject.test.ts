...
test('recovers lower-bound metering when external settlement is interrupted', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-metering-recovery-'));
  ...
  try {
    ...
    await writeSignedCheckpoint(
      ...
      {
        schemaVersion: 'sharker.external_provider_usage.v2',
        profile: 'codex',
        usage,
        settled: false,
        requests: 3,
...
test('a checkpoint the proxy never settled is a lower bound however complete it looks', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-metering-stale-'));
  const trialPath = join(root, 'trial');
  const usage = {
    inputTokens: 100,
    outputTokens: 20,
...
test('refuses a metering checkpoint whose counts cannot describe one run', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-metering-invalid-'));
  const trialPath = join(root, 'trial');
  try {
    await mkdir(join(trialPath, 'agent'), { recursive: true });
...
test('the pinned fingerprint is the digest of the checksum manifest', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-toolchain-identity-'));
  try {
    ...
    const identity = {
      root: '/opt/sharker-codex-toolchain',
      version: '1',
...
test('refuses a mounted toolchain that is not the pinned tree', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-toolchain-'));
  ...
  // the other's pin.
  const sourceEnv = 'SHARKER_TEST_CODEX_TOOLCHAIN';
  ...
  try {
    ...
    const cell = {
      ...externalCell({
        command: '/opt/sharker-node-toolchain/bin/node',
        args: [
          '/opt/sharker-agent/node_modules/@sharker/eval/dist/harbor-external-subject.js',
          'codex',
          'https://api.deepseek.com',
          '/',
          '/opt/sharker-codex-toolchain/bin/codex',
        ],
      }),
      executor: {
        kind: 'harbor',
...
test('a verified toolchain reaches the subject and its later attempts', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-toolchain-verified-'));
  const sourceEnv = 'SHARKER_TEST_CODEX_TOOLCHAIN';
        ...
        execute: async (input) => {
          ...
          return {
            ...
            stdout: JSON.stringify({
              schemaVersion: 'sharker.external_subject_result.v1',
              usage: null,
              costUsd: null,
              status: 'completed',
              failureReason: null,
