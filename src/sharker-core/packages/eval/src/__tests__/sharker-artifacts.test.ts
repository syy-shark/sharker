...
import {
  captureSharkerRuntimeArtifacts,
...
test('runtime artifact capture includes committed WAL rows in a standalone database', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-eval-artifacts-'));
  ...
  try {
    ...
    const manifest = await captureSharkerRuntimeArtifacts({
      stateRoot,
      destinationRoot,
      reason: 'signal',
      now: () => 42,
...
test('Sharker reports the runtime and process artifacts for settled and timed-out executions', async () => {
  for (const termination of ['exited', 'framework_timeout'] as const) {
    const result = await createSharkerSubjectAdapter().execute({
      ...
      context: {
        ...
        execute: async (input: Parameters<SubjectExecutionContext['execute']>[0]) => {
          ...
          assert.equal(payload.artifactRoot, SHARKER_RUNTIME_ARTIFACT_PATH);
          return {
            termination,
    ...
    assert.deepEqual(
      ...
      [
        { kind: 'sharker-runtime-state', path: SHARKER_RUNTIME_ARTIFACT_PATH },
        { kind: 'subject-stdout', path: SHARKER_SUBJECT_STDOUT_PATH },
        { kind: 'subject-stderr', path: SHARKER_SUBJECT_STDERR_PATH },
      ],
    );
  }
});

function sharkerCell(): ExperimentCell {
  return {
    id: 'task::1::sharker',
    ...
    subject: {
      ...
      config: {
        nodePath: '/opt/node',
        shimPath: '/opt/sharker-subject.js',
