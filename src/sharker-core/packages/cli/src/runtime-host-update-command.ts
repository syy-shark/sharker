...
import { activateRuntimeHostManagedDeployment } from '@sharker/runtime-host/client';
import {
    ...
    return await deps.withDeploymentLock(options.clientDataRoot, async () => {
      try {
        ...
        if (!serviceConfig?.managedDeploymentRoot) {
          throw new RuntimeHostServiceManagerError(
            'invalid_launch',
            'The Runtime Host service is not owned by a Sharker managed deployment',
          );
        }
...
function manualUpdateRequiredMessage(
  ...
  if (reason === 'target_not_newer') {
    return `Sharker ${targetVersion} is older than the installed Runtime Host and will not be applied automatically`;
  }
  ...
  if (reason === 'target_compatibility_unknown') {
    return `Sharker ${targetVersion} does not publish enough compatibility evidence for an automatic update`;
  }
  return `Sharker ${targetVersion} requires an explicit manual compatibility transition`;
}
