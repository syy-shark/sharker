...
import { readStableBoundedFile, syncDirectory } from '@sharker/storage/stable-storage';
import { resolveExistingStorageRoot, tryAcquireStateRootOwner } from '@sharker/storage/root-authority';
import {
...
export async function prepareRuntimeHostManagedPackageDeployment(
  ...
  const deploymentRoot = join(dataHome, 'Sharker', 'runtime-host-services', input.serviceId);
...
export async function openRuntimeHostManagedPackageDeployment(input: {
  ...
  try {
    ...
    throw new RuntimeHostManagedDeploymentError(
      'invalid_package',
      `The managed Sharker ${input.version} package is unavailable`,
      { cause: error },
    );
  }
...
export function isRuntimeHostManagedDeploymentRoot(root: string, serviceId: string): boolean {
  ...
  return (
    ...
    basename(dirname(dirname(canonical))) === 'Sharker'
  );
}
