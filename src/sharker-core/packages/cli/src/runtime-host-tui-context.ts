...
import type { ConnectionCatalogEntry, ConnectionCatalogSnapshot } from '@sharker/core/runtime-policy';
...
} from '@sharker/storage/process-lifetime-owner';
import {
...
import type { AgentGraphClientSnapshot, WorkspaceTarget } from '@sharker/runtime-host/protocol';
import {
...
export interface RuntimeHostTuiContext {
  ...
  readonly driver: ReturnType<typeof createRuntimeHostSharkerSessionDriver>;
  readonly cwd: string;
  ...
  readonly turnActivity: SharkerPiTuiTurnActivitySurface;
...
export async function createRuntimeHostTuiContext(
  ...
  try {
    ...
    const driverInput: RuntimeHostSharkerSessionDriverInput = {
      connection,
      cwd: input.cwd,
    ...
    const driver = createRuntimeHostSharkerSessionDriver(driverInput);
...
function createHostOwnedTurnActivity(): SharkerPiTuiTurnActivitySurface {
