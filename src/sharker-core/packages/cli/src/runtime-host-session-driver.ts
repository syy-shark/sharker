...
import type { CreateSessionInput } from '@sharker/core/runtime-inputs';
import { DEFAULT_SESSION_NAME } from '@sharker/core/session-name';
import {
...
import { markPersisted } from '@sharker/core/persisted-value';
import {
...
import { isSideConversationSession } from '@sharker/core/side-conversation';
import {
  createSessionCopyCleanupAuthority,
  type SessionCopyCleanupAuthority,
} from '@sharker/storage/session-copy-cleanup';
import type { ProcessLifetimeOwner } from '@sharker/storage/process-lifetime-owner';

import type { OrchestrationMode } from '@sharker/core/orchestration';
import type { PermissionMode } from '@sharker/core/permission';

import { mergeShellRunUpdate } from '@sharker/core/shell-run-result';
import { isActiveShellRunStatus } from '@sharker/core/shell-run';
import { executionBoundaryDisplayMode } from '@sharker/core/sandbox-boundary';
...
import { isRuntimeHostTerminalTurn as isTerminalTurn } from '@sharker/runtime-host/adapter';
import type { DirectRequestOperationKey, RuntimeHostConnection } from '@sharker/runtime-host/client';
import {
...
import type {
  ...
  SharkerSideConversationCloseResult,
  SharkerSideConversationOpenResult,
  SharkerSideConversationParentStatus,
  SharkerRetractedMessages,
...
export interface RuntimeHostSharkerSessionDriverInput {
...
export interface RuntimeHostSharkerSessionDriver extends SharkerSessionDriver {
...
export function createRuntimeHostSharkerSessionDriver(
  input: RuntimeHostSharkerSessionDriverInput,
): RuntimeHostSharkerSessionDriver {
  return new RuntimeHostSharkerSessionDriverImpl(input);
}

class RuntimeHostSharkerSessionDriverImpl implements RuntimeHostSharkerSessionDriver {
  readonly #connection: RuntimeHostSessionDriverConnection;
  readonly #newId: () => string;
  readonly #now: () => number;
  readonly #inspectCwdChanges: InspectCwdChanges;
  readonly #executionLocation: NonNullable<RuntimeHostSharkerSessionDriverInput['executionLocation']>;
  readonly #sessionCopyCleanup: SessionCopyCleanupAuthority | undefined;
  readonly moveSession: SharkerSessionDriver['moveSession'];
  ...
  readonly #startedTurnListeners = new Set<(turn: SharkerAttachedSessionTurn) => void>();
  ...
  constructor(input: RuntimeHostSharkerSessionDriverInput) {
  ...
  async retractQueued(): Promise<SharkerRetractedMessages> {
  ...
  async #moveSession(rawCwd: string): Promise<SharkerSessionMoveResult> {
  ...
  async switchSession(
    ...
    let relocation: SharkerSessionMoveResult | undefined;
    if (options.relocateCwd !== undefined) {
  ...
  async rewindToTurn(turnId: string): Promise<SharkerSessionRewindResult> {
  ...
  async openSideConversation(): Promise<SharkerSideConversationOpenResult> {
  ...
  ): Promise<SharkerSideConversationCloseResult> {
  ...
  async observeSideConversationParent(
    ...
    const drain = (turn: SharkerPreparedSessionTurn): void => {
      const task = (async () => {
        try {
  ...
  #publishStartedTurn(turn: SharkerPreparedSessionTurn, sessionGeneration: number): void {
  ...
  async #reattachStartedTurn(
    ...
    const turn = {
      ...
      messages: visibleTranscriptMessages(opened.messages, this.#hiddenTranscriptThroughTurnId),
      summary: projectSessionCatalogSummary(configuration.session),
    } satisfies SharkerAttachedSessionTurn;
...
function workspaceTargetForCreate(
  ...
  location: NonNullable<RuntimeHostSharkerSessionDriverInput['executionLocation']>,
): WorkspaceTarget {
...
): SharkerSideConversationParentStatus | undefined {
