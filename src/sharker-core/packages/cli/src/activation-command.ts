...
import type { CreateSessionInput, UserMessageInput } from '@sharker/core/runtime-inputs';
...
import { assertSessionBundleRootLayout } from '@sharker/storage/session-bundle-policy';
import { projectSessionCatalogSummary, readRuntimeHostSessions } from '@sharker/runtime-host/client';
...
import type { SharkerRunOutcome } from './run-command-core.js';
import { sessionEventSandboxBoundaryFailureReason } from './sandbox-boundary-failure.js';

const PROTOCOL = 'sharker.activation' as const;
...
export type SharkerActivationStatus = 'completed' | 'blocked' | 'retryable_failure' | 'fatal_failure';

export type SharkerActivationBlockedReason = 'permission_denied' | 'permission_required';
export type SharkerActivationRequiredAction = 'grant_permission' | 'retry_activation';

export interface SharkerActivationOptions {
  stateRoot: string;
...
export type ParseSharkerActivateArgsResult =
  | { kind: 'activate'; options: SharkerActivationOptions }
...
export interface SharkerActivationRequest {
  schemaVersion: 1;
  activationId: string;
...
export interface SharkerActivationContext {
  runtime: SharkerActivationRuntime;
  target: {
    connection: {
      slug: string;
      name?: string;
...
export interface SharkerActivationRuntime {
...
export interface SharkerActivationDeps {
  createContext(input: SharkerActivationContextInput): Promise<SharkerActivationContext>;
...
export interface SharkerActivationContextInput {
  ...
  readonly runOutcomeObserver?: (outcome: SharkerRunOutcome) => void | Promise<void>;
}
...
interface ActivationOutcomeLine {
  ...
  cloudSessionId: string;
  sharkerSessionId?: string;
  status: SharkerActivationStatus;
  reason?: string;
  requiredAction?: SharkerActivationRequiredAction;
  response?: unknown;
}

export function parseSharkerActivateArgs(argv: readonly string[]): ParseSharkerActivateArgsResult {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--help' || arg === '-h') return { kind: 'help', text: sharkerActivateHelpText() };
...
export function decodeActivationRequest(value: unknown): SharkerActivationRequest {
  ...
  if (
    value.sharkerSessionId !== undefined &&
    (typeof value.sharkerSessionId !== 'string' || value.sharkerSessionId.trim() === '')
  ) {
    throw new Error('activation sharkerSessionId must be a non-empty string');
  }
  if (
  ...
  return {
    ...
    ...(value.sharkerSessionId === undefined ? {} : { sharkerSessionId: value.sharkerSessionId }),
    stimulus: { type: value.stimulus.type, payload: value.stimulus.payload },
  };
}

export async function runSharkerActivationCli(
  argv: readonly string[],
  overrides: Partial<SharkerActivationDeps> = {},
): Promise<number> {
  const deps = { ...defaultSharkerActivationDeps(), ...overrides };
  const parsed = parseSharkerActivateArgs(argv);
  if (parsed.kind === 'help') {
    deps.writeStdout(`${parsed.text}\n`);
    return 0;
  }
  if (parsed.kind === 'error') {
    deps.writeStderr(`sharker activate: ${parsed.message}\n`);
    return 2;
  }

  const options = parsed.options;
  let request: SharkerActivationRequest;
  try {
    ...
    const malformedRequest: SharkerActivationRequest = {
      schemaVersion: SCHEMA_VERSION,
    ...
    deps.writeStderr(`sharker activate: ${safeErrorMessage(error)}\n`);
    return emitOutcome(
      deps.writeStdout,
      malformedRequest,
      undefined,
  ...
  write({
    ...
    ...(request.sharkerSessionId ? { sharkerSessionId: request.sharkerSessionId } : {}),
    workspaceRoot: resolve(options.workspaceRoot),
  } satisfies ActivationStartLine);
  ...
  try {
    ...
    existing = request.sharkerSessionId
      ? sessions.find((session) => session.id === request.sharkerSessionId)
      : undefined;
    if (request.sharkerSessionId && !existing) {
  ...
  let context: SharkerActivationContext | undefined;
  let session: SessionSummary | undefined = existing;
  let invocation: SharkerRunOutcome | undefined;
  ...
  const writeRuntimeEvent = (event: RuntimeEvent): void => {
    ...
    const eventLine = {
      ...
      activationId: request.activationId,
      cloudSessionId: request.cloudSessionId,
      ...(session?.id ? { sharkerSessionId: session.id } : {}),
      event: safeEvent,
...
async function finishWithoutContext(
  deps: SharkerActivationDeps,
  request: SharkerActivationRequest,
...
function emitOutcome(
  ...
  const outcome: ActivationOutcomeLine = {
    ...
    activationId: request.activationId,
    cloudSessionId: request.cloudSessionId,
    ...(sessionId ? { sharkerSessionId: sessionId } : {}),
    status,
    ...(reason ? { reason } : {}),
...
async function validateRoots(options: SharkerActivationOptions): Promise<ValidatedRoots> {
...
async function assertActivationRootLayout(
  roots: Pick<SharkerActivationOptions, 'stateRoot' | 'workspaceRoot' | 'configRoot'>,
): Promise<void> {
  await Promise.all([
...
function sharkerActivateHelpText(): string {
  return [
    'Usage: sharker activate --state-root <path> --workspace-root <path> --config-root <path>',
    '',
...
function defaultSharkerActivationDeps(): SharkerActivationDeps {
  return {
...
async function createRuntimeHostActivationContext(
  input: SharkerActivationContextInput,
): Promise<SharkerActivationContext> {
