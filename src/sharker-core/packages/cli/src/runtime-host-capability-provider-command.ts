...
import { normalizeMcpConfig } from '@sharker/storage/mcp-config-store';
import {
...
import { runRuntimeHostProcessLifecycle } from '@sharker/runtime-host/server';
import { createMcpCapabilityProvider } from './mcp-capability-provider.js';

const DEFAULT_CREDENTIAL_ENV = 'SHARKER_RUNTIME_HOST_ACCESS_CREDENTIAL';
...
export async function runRuntimeHostCapabilityProviderCli(
  ...
  const manager = new McpClientManager({
    clientName: 'sharker-capability-provider',
...
function defaultProviderClientIdentityPath(
  ...
  const identity = createHash('sha256')
    ...
    .slice(0, 24);
  const identityRoot =
    defaultClientIdentityRoot ?? join(homedir(), '.sharker', 'runtime-host-capability-providers');
