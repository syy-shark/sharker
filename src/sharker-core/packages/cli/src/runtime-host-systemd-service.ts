...
import { resolveXdgConfigHome } from '@sharker/storage/workspace-root';
import { readStableBoundedFile } from '@sharker/storage/stable-storage';
import {
...
function renderSystemdUpdateServiceWithArguments(args: readonly string[]): string {
  return [
    '[Unit]',
    'Description=Sharker Runtime Host update reconciliation',
...
export function renderSystemdUpdateTimer(serviceId: string): string {
  return [
    '[Unit]',
    'Description=Schedule Sharker Runtime Host update reconciliation',
    '',
    '[Timer]',
