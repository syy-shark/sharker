...
export function launchRuntimeHostLocalSourceRetirement(input: {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once('close', (code, signal) => {
      if (signal) {
        reject(new Error(`Sharker source retirement helper exited on ${signal}`));
      } else if (code === 0) {
        resolve('prepared');
      } else if (code === 2) {
        resolve('active_work');
        ...
        reject(new Error('The selected Sharker source package could not retire its Runtime Host'));
      }
    });
  });
}
