...
} from '@sharker/runtime/goal-turn-lifecycle';
import { type GoalTurnOutcome } from '@sharker/runtime/goal-continuation';
import type { SharkerPreparedSessionTurn } from './session-driver.js';

export interface SharkerPiTuiTurnActivity {
  activities: SessionActivityRegistry;
}

/** A Turn that another Client or the Runtime Host already started. */
export interface SharkerPiTuiTurnRequest {
  turn: SharkerPreparedSessionTurn;
}

export interface RunSharkerPiTuiTurnInput {
  turnActivity: SharkerPiTuiTurnActivity;
  request: SharkerPiTuiTurnRequest;
  shouldAbort: () => boolean;
  onStart?: () => void;
  onPrepared?: (turn: SharkerPreparedSessionTurn) => void | Promise<void>;
 ...
 */
export async function runSharkerPiTuiTurn(input: RunSharkerPiTuiTurnInput): Promise<GoalTurnOutcome> {
  const { request } = input;
