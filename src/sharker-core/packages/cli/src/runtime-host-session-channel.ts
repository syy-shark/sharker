...
export interface RuntimeHostSessionChannelOptions {
  ...
  sessionId: string;
  now: () => number;
  onTurnStarted: (turn: SharkerPreparedSessionTurn) => void;
...
export class RuntimeHostSessionChannel {
  ...
  readonly #now: () => number;
  readonly #onTurnStarted: (turn: SharkerPreparedSessionTurn) => void;
  ...
  readonly #pendingStartedTurns = new Map<string, SharkerPreparedSessionTurn>();
  ...
  #acceptCanonicalReplacement(messages: StoredMessage[]): boolean {
    ...
    if (root && !isTerminalTurn(root)) {
      ...
      if (!previousRoot || previousRoot.runId !== root.runId) {
        ...
        } satisfies SharkerPreparedSessionTurn;
