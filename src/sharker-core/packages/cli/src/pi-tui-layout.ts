...
import {
  ...
  type SharkerPiTranscriptEntry,
  type SharkerPiTranscriptMetadata,
  type SharkerPiTranscriptState,
...
export class SharkerTranscriptComponent implements Component {
  constructor(
    private readonly state: SharkerPiTranscriptState,
    private readonly metadata: () => SharkerPiTranscriptMetadata,
  ) {}

  invalidate(): void {}

  render(width: number): string[] {
    return renderSharkerPiTranscript(this.state, this.metadata(), width);
  }
  ...
  createDocumentRenderer(): (width: number) => string[] {
    ...
    const entryClones = new WeakMap<SharkerPiTranscriptEntry, SharkerPiTranscriptEntry>();
    const documentEntry = (entry: SharkerPiTranscriptEntry): SharkerPiTranscriptEntry => {
      const cached = entryClones.get(entry);
      if (cached) {
        Object.assign(cached, entry);
        return cached;
      }
      const clone = { ...entry } as SharkerPiTranscriptEntry;
...
export class SharkerStatusLineComponent implements Component {
  constructor(private readonly metadata: () => SharkerPiTranscriptMetadata) {}

  invalidate(): void {}

  render(width: number): string[] {
    return [renderSharkerPiStatusLine(this.metadata(), width)];
  }
}

export class SharkerActivityStripComponent implements Component {
  constructor(private readonly metadata: () => SharkerPiTranscriptMetadata) {}

  invalidate(): void {}

  render(width: number): string[] {
    return [renderSharkerPiActivityStrip(this.metadata(), width)];
  }
}

/** The pending-queue bar (Steering:/Queued:) rendered just above the editor. */
export class SharkerPendingQueueComponent implements Component {
  constructor(private readonly state: SharkerPiTranscriptState) {}

  invalidate(): void {}

  render(width: number): string[] {
    return renderSharkerPiPendingQueue(this.state, width);
  }
}
 ...
 */
export class SharkerPiLayoutComponent extends Container {
  ...
  constructor(
    private readonly state: SharkerPiTranscriptState,
    private readonly transcript: SharkerTranscriptComponent,
    private readonly activityStrip: SharkerActivityStripComponent,
    private readonly pendingQueue: SharkerPendingQueueComponent,
