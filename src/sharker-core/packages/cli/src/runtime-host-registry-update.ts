...
} from '@sharker/runtime-host/operator/update-package-evidence';
import type { RuntimeHostUpdateSelector } from './runtime-host-cli.js';

const PACKAGE_NAME = 'sharker-agent';
const NPM_REGISTRY = 'https://registry.npmjs.org/';
const COMPATIBILITY_FIELD = 'sharker.managedRuntimeHostUpdateCompatibility';
...
export async function resolveRuntimeHostRegistryUpdateCandidate(
  ...
  if (result.exitCode !== 0) {
    ...
    throw new RuntimeHostUpdateDiscoveryError(
      ...
      code === 'E404'
        ? `No Sharker package is published for ${target}`
        : 'The Sharker package registry is unavailable',
    );
  }
  ...
  try {
    ...
    throw new RuntimeHostUpdateDiscoveryError(
      ...
      'The npm registry returned invalid Sharker package metadata',
      { cause: error },
    );
  }
      ...
      if (bytes > REGISTRY_OUTPUT_MAX_BYTES) {
        reject(
          new RuntimeHostUpdateDiscoveryError(
            'invalid_registry_metadata',
            'The npm registry returned oversized Sharker package metadata',
          ),
        );
        return;
      }
...
function invalidMetadata(): never {
  throw new RuntimeHostUpdateDiscoveryError(
    'invalid_registry_metadata',
    'The npm registry returned incomplete Sharker package metadata',
  );
}
