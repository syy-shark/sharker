...
import {
  ...
  parseSharkerActivateArgs,
  runSharkerActivationCli,
  type SharkerActivationContext,
  type SharkerActivationDeps,
  type SharkerActivationRuntime,
} from '../activation-command.js';
import type { SharkerRunOutcome } from '../run-command-core.js';

const ROOTS = {
  stateRoot: '/tmp/sharker-state',
  workspaceRoot: '/tmp/sharker-workspace',
  configRoot: '/tmp/sharker-config',
};
...
function completedResult(): SharkerRunOutcome {
  return {
    outcomeId: 'run-1',
    status: 'completed',
...
function fakeDeps(
  options: {
    ...
    result?: SharkerRunOutcome;
    events?: SessionEvent[];
    onContext?: (input: Parameters<NonNullable<SharkerActivationDeps['createContext']>>[0]) => void;
...
): SharkerActivationDeps {
  let observer: ((result: SharkerRunOutcome) => void | Promise<void>) | undefined;
  const sessions = options.sessions ?? [];
  const runtime: SharkerActivationRuntime = {
    async createSession() {
  ...
  return {
    createContext: async (input) => {
      ...
      const context: SharkerActivationContext = {
        runtime,
        target: {
          connection: {
            slug: 'local',
...
describe('sharker activate argument and request contracts', () => {
  test('requires explicit non-overlapping roots and accepts JSON input options', () => {
    ...
    assert.equal(parseSharkerActivateArgs(['--state-root', ROOTS.stateRoot]).kind, 'error');
    assert.equal(
      parseSharkerActivateArgs([
        '--state-root',
        ROOTS.stateRoot,
  ...
  test('decodes versioned message and resumed activation requests', () => {
    assert.deepEqual(
      decodeActivationRequest(
        validRequest({
          sharkerSessionId: 'sharker-session-1',
  ...
  test('rejects malformed input before runtime startup', async () => {
    let started = false;
    const result = await runSharkerActivationCli(
      [
        '--state-root',
        ROOTS.stateRoot,
        '--workspace-root',
...
describe('sharker activate JSONL protocol', () => {
  ...
  test('retries non-permission blocked sessions instead of requesting permission', async () => {
    for (const blockedReason of ['auth', 'tool_failed'] as const) {
      ...
      const result = await runSharkerActivationCli(
        ...
        {
          ...fakeDeps({
            input: JSON.stringify(validRequest({ sharkerSessionId: 'sharker-session-1' })),
  ...
  test('resumes an existing compatible session without creating another one', async () => {
    ...
    assert.equal(JSON.parse(lines.at(-1)!).sharkerSessionId, 'sharker-session-1');
  });
...
describe('sharker activate filesystem boundaries', () => {
  test('rejects workspace/state/config overlap before bootstrap', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-activation-'));
    try {
      ...
      const reverseResult = await runSharkerActivationCli(
        [
          '--state-root',
          join(root, 'state'),
          '--workspace-root',
