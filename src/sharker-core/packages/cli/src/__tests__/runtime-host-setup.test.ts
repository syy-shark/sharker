...
import { RuntimeHostOperationError } from '@sharker/runtime-host/client';
import {
  resolveRootControlNamespace,
  resolveRootOwnershipNamespace,
  resolveStorageRoot,
  tryAcquireStateRootOwner,
} from '@sharker/storage/root-authority';
import {
...
test('on-demand setup installs one exact deployment without a service backend', async (t) => {
  const base = await realpath(await mkdtemp(join(tmpdir(), 'sharker-runtime-host-on-demand-setup-')));
  ...
  const deployment = (serviceId: string) => ({
    version: '1.2.3',
    root: join(canonicalDataHome, 'Sharker', 'runtime-host-services', serviceId),
    cliPath: '/verified/package/dist/cli.js',
    operatorPath: '/opt/sharker/operator',
  ...
  assert.equal(
    ...
    join(canonicalDataHome, 'Sharker', 'runtime-host-services', rootId),
  );
...
test('registry package identity avoids local content and recovers an interrupted removal', async (t) => {
  const base = await realpath(await mkdtemp(join(tmpdir(), 'sharker-runtime-host-registry-package-')));
  ...
  assert.equal(
    registry.root,
    join(base, 'durable-data', 'Sharker', 'runtime-host-services', serviceId),
  );
  ...
  const outsideSharker = join(base, 'outside', 'Sharker');
  await mkdir(redirectedDataHome);
  await mkdir(outsideSharker, { recursive: true });
  await writeFile(join(outsideSharker, 'sentinel'), 'outside\n');
  await symlink(outsideSharker, join(redirectedDataHome, 'Sharker'));
  ...
  assert.equal(await readFile(join(outsideSharker, 'sentinel'), 'utf8'), 'outside\n');
  assert.deepEqual(await readdir(outsideSharker), ['sentinel']);

  const redirectedServiceDataHome = join(base, 'redirected-service-data');
  const managedServices = join(redirectedServiceDataHome, 'Sharker', 'runtime-host-services');
...
test('managed operator binds its Client Data Root and routes deployment cleanup', {
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-operator-'));
  t.after(() => rm(base, { recursive: true, force: true }));
  const version = '0.2.0';
  ...
  await writeFile(
    deployment.cliPath,
    `require('node:fs').writeFileSync(process.env.SHARKER_TEST_OUTPUT, JSON.stringify(process.argv.slice(2)));\n`,
  );
  await execFile(deployment.operatorPath, ['status'], {
    env: {
      ...process.env,
      XDG_CONFIG_HOME: join(base, 'different-config'),
      SHARKER_TEST_OUTPUT: invocationPath,
    },
  });
  ...
  await execFile(
    ...
    {
      env: { ...process.env, SHARKER_TEST_OUTPUT: invocationPath },
    },
  );
  assert.deepEqual(JSON.parse(await readFile(invocationPath, 'utf8')), [
    'runtime-host',
...
async function createReleasePackage(base: string, version: string): Promise<string> {
  ...
  await mkdir(join(root, 'node_modules', '@sharker', 'runtime-host'), {
    recursive: true,
  });
  await writeFile(join(root, 'package.json'), JSON.stringify({ name: 'sharker-agent', version }));
  await writeFile(join(root, 'dist', 'cli.js'), '#!/usr/bin/env node\n');
  await writeFile(
    join(root, 'node_modules', '@sharker', 'runtime-host', 'package.json'),
    JSON.stringify({ name: '@sharker/runtime-host', version: '0.1.0' }),
  );
  return root;
}
