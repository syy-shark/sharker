...
import { parseSharkerRunArgs, runSharkerTextCliCore, type SharkerRunAdapter } from '../run-command-core.js';

const fixturePath = fileURLToPath(new URL('./run-command-fixture.js', import.meta.url));

describe('sharker run argument parsing', () => {
  test('recognizes stdin prompt mode and rejects malformed limits', () => {
    assert.deepEqual(parseSharkerRunArgs(['-']), {
      kind: 'run',
      options: { stdinPrompt: true },
    });
    assert.equal(parseSharkerRunArgs(['x', '--timeout', '0']).kind, 'error');
    assert.equal(parseSharkerRunArgs(['x', '--max-steps', '1.5']).kind, 'error');
  });

  test('accepts only the explicit non-interactive sandbox bypass flag', () => {
    assert.deepEqual(parseSharkerRunArgs(['run tools', '--yolo']), {
      kind: 'run',
      options: {
        prompt: 'run tools',
        stdinPrompt: false,
  ...
  test('parses resume and continue session selectors and rejects combining them', () => {
    assert.deepEqual(parseSharkerRunArgs(['next', '--resume', 'session-1']), {
      kind: 'run',
      options: { prompt: 'next', stdinPrompt: false, resumeId: 'session-1' },
    });
    assert.deepEqual(parseSharkerRunArgs(['next', '--continue']), {
      kind: 'run',
      options: { prompt: 'next', stdinPrompt: false, continueLatest: true },
    });
    assert.equal(parseSharkerRunArgs(['next', '--resume', 'session-1', '--continue']).kind, 'error');
  });

  test('accepts a remote Host Project and rejects client cwd semantics', () => {
    assert.deepEqual(parseSharkerRunArgs(['next', '--host', 'office', '--project', 'project-1']), {
      kind: 'run',
      options: {
        prompt: 'next',
        stdinPrompt: false,
    ...
    assert.equal(parseSharkerRunArgs(['next', '--host', 'office', '--continue']).kind, 'error');
    assert.equal(
      parseSharkerRunArgs(['next', '--host', 'office', '--cwd', '/client/path']).kind,
      'error',
    );
  });
  ...
  test('forwards --max-steps to local and Graph turn inputs', async () => {
    for (const graph of [false, true]) {
      ...
      const adapter: SharkerRunAdapter = {
        listSessions: async () => [],
      ...
      const exitCode = await runSharkerTextCliCore(
...
// signal, a real exit code — which the injected SharkerRunDeps seam cannot
// express. Everything else about `sharker run` is covered in process above.
describe('sharker run process contract', () => {
...
function runFixture(
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [fixturePath, ...args], {
      ...
      env: { ...process.env, SHARKER_RUN_FIXTURE_SCENARIO: options.scenario ?? 'echo' },
...
async function interruptFixture(
  ...
  const child = spawn(process.execPath, [fixturePath, ...args], {
    env: { ...process.env, SHARKER_RUN_FIXTURE_SCENARIO: scenario },
    stdio: ['pipe', 'pipe', 'pipe'],
  });
  child.stdin.end();
  let stdout = '';
