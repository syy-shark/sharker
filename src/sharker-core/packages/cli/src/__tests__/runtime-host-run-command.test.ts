...
} from '@sharker/runtime-host/adapter';
import { LOCAL_RUNTIME_HOST_PROFILE, type RuntimeHostConnection } from '@sharker/runtime-host/client';
import {
...
import type { RuntimeHostSharkerSessionDriver } from '../runtime-host-session-driver.js';
import type { SharkerRunContextInput, SharkerRunOutcome } from '../run-command-core.js';
import type { SharkerTranscriptReplacementReason } from '../session-driver.js';

describe('Runtime Host sharker run adapter', () => {
  ...
  test('routes both launch roots through the selected Host profile and canonical Project', async () => {
    ...
    let contextInput: SharkerRunContextInput | undefined;
  ...
  test('returns exit code 1 when an ordinary Host Turn fails', async () => {
    ...
    assert.equal(exitCode, 1);
    assert.equal(stderr.join(''), 'sharker run: Turn failed\n');
  });
  ...
  test('returns exit code 1 when the final Graph Turn fails', async () => {
    ...
    assert.equal(exitCode, 1);
    assert.equal(stderr.join(''), 'sharker run: Agent Graph final Turn failed\n');
  });

  test('waits for Host-started graph supervisor Turns before returning', async () => {
    const observed: SharkerRunOutcome[] = [];
...
function publicCommandContext(input: SharkerRunContextInput, onCreate: () => void = () => {}) {
  return {
    runtime: {
      createSession: async () => {
        onCreate();
...
function runFixture(input: {
  ...
  } as unknown as RuntimeHostSharkerSessionDriver;
  const connection = {
    hostEpoch: 'host-1',
  ...
  const createContext = (contextInput: SharkerRunContextInput) =>
  ...
  let create = () =>
    createContext({
      ...
      ...(input.observed
        ? {
            runOutcomeObserver: (result: SharkerRunOutcome) => {
