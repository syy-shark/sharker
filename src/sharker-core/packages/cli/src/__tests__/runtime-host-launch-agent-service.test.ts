...
import { RUNTIME_HOST_SERVICE_LOG_MAX_BYTES } from '@sharker/runtime-host/operator';
import {
...
const UID = 501;
const LABEL = `com.sharker.runtime-host.${SERVICE_ID}`;
...
test('renders the canonical Runtime Host command as a private persistent LaunchAgent', () => {
  const config = fixtureConfig('/tmp/node & tool', '/tmp/sharker <cli>', '/tmp/state > root');
  ...
  assert.match(plist, /<string>\/tmp\/sharker &lt;cli&gt;<\/string>/u);
...
test('renders managed update reconciliation as a periodic one-shot LaunchAgent', () => {
  const config = {
    ...fixtureConfig('/tmp/node', '/tmp/sharker', '/tmp/state'),
...
test('canonical LaunchAgent projection owns persistent enablement', async () => {
  await withFixture(async ({ homeDir, launchctl }) => {
    ...
    const supervisor = { command: [process.execPath, '/tmp/sharker-cli.js'] as const };
    const reconciliation = {
      command: ['/tmp/sharker-operator', 'reconcile-update', '--framed'] as const,
    };
...
test('installs and removes the update scheduler with a managed LaunchAgent', async () => {
  await withFixture(async ({ homeDir, cliPath, launchctl }) => {
    ...
    const logDirectory = join(homeDir, 'Library', 'Logs', 'Sharker', 'runtime-host-services');
    await Promise.all([
...
function legacyLaunchAgentPlistFixture(
  ...
  return [
    ...
    `  <string>${escape(join(homeDir, 'Library', 'Logs', 'Sharker', 'runtime-host-services', `${LABEL}.stdout.log`))}</string>`,
    '  <key>StandardErrorPath</key>',
    `  <string>${escape(join(homeDir, 'Library', 'Logs', 'Sharker', 'runtime-host-services', `${LABEL}.stderr.log`))}</string>`,
    '</dict>',
    '</plist>',
    '',
  ].join('\n');
...
async function withFixture(
  ...
  const homeDir = await mkdtemp(join(tmpdir(), 'sharker-launch-agent-test-'));
  const cliPath = join(homeDir, 'sharker-cli.js');
  try {
