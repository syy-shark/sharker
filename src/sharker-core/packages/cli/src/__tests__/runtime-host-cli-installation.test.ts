...
test('keeps one owner across release changes in the same npm global slot', async (t) => {
  ...
  await writeFile(fixture.manifestUrl, JSON.stringify({ name: 'sharker-agent', version: '2.0.0' }));
...
test('rejects development and npx packages before treating them as owners', async (t) => {
  ...
  await writeFile(
    ...
    JSON.stringify({ name: 'sharker-agent', version: '1.0.0', private: true }),
  );
  await assert.rejects(resolve(development), installationError('unsupported_installation'));

  const cacheRoot = await mkdtemp(join(tmpdir(), 'sharker-cli-npx-installation-'));
  t.after(() => rm(cacheRoot, { recursive: true, force: true }));
  const packageRoot = join(cacheRoot, '_npx', 'hash', 'node_modules', 'sharker-agent');
  const cliPath = join(packageRoot, 'dist', 'cli.js');
  const manifestUrl = pathToFileURL(join(packageRoot, 'package.json'));
  await mkdir(join(packageRoot, 'dist'), { recursive: true });
  await writeFile(cliPath, '#!/usr/bin/env node\n');
  await writeFile(manifestUrl, JSON.stringify({ name: 'sharker-agent', version: '1.0.0' }));
...
test('recognizes configured and default npx cache roots without prefix collisions', async (t) => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-cli-npx-provenance-'));
  t.after(() => rm(base, { recursive: true, force: true }));
  const configured = join(base, 'cache', '_npx', 'one', 'node_modules', 'sharker-agent');
  const defaultCache = join(base, 'home', '.npm', '_npx', 'two', 'node_modules', 'sharker-agent');
  const collision = join(base, 'cache', '_npx-other', 'sharker-agent');
  await Promise.all([
...
test('rejects non-UTF-8 package metadata instead of normalizing an owner observation', async (t) => {
  const fixture = await installationFixture(t, '1.0.0');
  const document = Buffer.from(JSON.stringify({ name: 'sharker-agent', version: '1.0.0' }));
...
async function installationFixture(t: test.TestContext, version: string) {
  const base = await realpath(await mkdtemp(join(tmpdir(), 'sharker-cli-global-installation-')));
  t.after(() => rm(base, { recursive: true, force: true }));
  const homeDir = join(base, 'home');
  const globalRoot = join(base, 'lib', 'node_modules');
  const packageRoot = join(globalRoot, 'sharker-agent');
  const cliPath = join(packageRoot, 'dist', 'cli.js');
  const manifestUrl = pathToFileURL(join(packageRoot, 'package.json'));
  await mkdir(join(packageRoot, 'dist'), { recursive: true });
  await mkdir(homeDir);
  await writeFile(cliPath, '#!/usr/bin/env node\n');
  await writeFile(manifestUrl, JSON.stringify({ name: 'sharker-agent', version }));
