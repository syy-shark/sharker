...
const ARCHIVE = tgz([{ name: 'package/package.json', body: Buffer.from('{"name":"sharker-agent"}') }]);
...
describe('managed Runtime Host update package acquisition', () => {
    ...
    await withRuntimeHostRegistryUpdateArtifact(
      ...
      async (args) => {
        if (args[0] === 'pack') {
          ...
          await writeFile(join(destination, 'sharker-agent-2.0.0.tgz'), ARCHIVE);
          return 0;
        }
        const prefix = args[args.indexOf('--prefix') + 1]!;
        const root = join(prefix, 'node_modules', 'sharker-agent');
        await Promise.all([
          mkdir(join(root, 'dist'), { recursive: true }),
          mkdir(join(root, 'node_modules', '@sharker', 'runtime-host'), { recursive: true }),
        ]);
        await Promise.all([
          writeFile(
            join(root, 'package.json'),
            JSON.stringify({ name: 'sharker-agent', version: '2.0.0' }),
          ),
          writeFile(join(root, 'dist', 'cli.js'), ''),
          writeFile(join(root, 'node_modules', '@sharker', 'runtime-host', 'package.json'), '{}'),
        ]);
        return 0;
      },
    );
  ...
  it('revalidates an archive before a coordinator can consume it', async (t) => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-update-archive-'));
    t.after(() => rm(root, { recursive: true, force: true }));
    const archive = join(root, 'sharker.tgz');
  ...
  it('binds the official archive to its extracted release evidence', async () => {
    ...
    await withRuntimeHostRegistryUpdatePackage(
      ...
      async (args) => {
        ...
        await Promise.all([
          mkdir(join(root, 'dist'), { recursive: true }),
          mkdir(join(root, 'node_modules', '@sharker', 'runtime-host'), {
            recursive: true,
          }),
        ]);
        await Promise.all([
    ...
    assert.deepEqual(calls[0]?.slice(0, 2), ['pack', 'sharker-agent@2.0.0']);
          ...
          await Promise.all([
            writeFile(
              ...
              JSON.stringify({
                name: 'sharker-agent',
                version: '2.0.0',
                sharker: { managedRuntimeHostUpdateCompatibility: 8 },
              }),
            ),
            writeFile(join(root, 'dist', 'cli.js'), ''),
            writeFile(join(root, 'node_modules', '@sharker', 'runtime-host', 'package.json'), '{}'),
  ...
  it('rejects a small verified archive whose headers claim excessive expansion', async (t) => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-update-budget-'));
    t.after(() => rm(root, { recursive: true, force: true }));
  ...
  it('accepts entries whose payload spans multiple gunzip chunks', async (t) => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-update-stream-'));
    t.after(() => rm(root, { recursive: true, force: true }));
    ...
    const big = tgz([
      ...
      { name: 'package/package.json', body: Buffer.from('{"name":"sharker-agent"}') },
  ...
  it('fails closed for tar extensions that can reinterpret a raw header size', async (t) => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-update-extended-tar-'));
    t.after(() => rm(root, { recursive: true, force: true }));
  ...
  it('requires two consecutive zero blocks to terminate a tar stream', async (t) => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-update-terminator-'));
    t.after(() => rm(root, { recursive: true, force: true }));
  ...
  it('rejects archives that are not readable gzip tarballs', async (t) => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-update-tarball-'));
    t.after(() => rm(root, { recursive: true, force: true }));
