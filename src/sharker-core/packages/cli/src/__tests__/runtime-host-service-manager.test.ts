...
describe('managed Runtime Host service', () => {
  it('parses the bounded managed service command surface', () => {
    ...
    assert.equal(
      parseRuntimeHostCommand(['service', 'peer', 'status', '--root', '/srv/sharker']).kind,
      'error',
    );
    assert.deepEqual(
      parseRuntimeHostCommand([
    ...
    assert.deepEqual(
      ...
      ]),
      {
        ...
        json: false,
        framed: true,
        clientDataRoot: '/var/lib/sharker-client',
      },
    );
  ...
  it('applies Project roots as a compare-and-set transaction and restores failed changes', async (t) => {
    const base = await realpath(await mkdtemp(join(tmpdir(), 'sharker-runtime-host-configure-')));
    ...
    const cliPath = join(base, 'sharker', 'dist', 'cli.js');
    await Promise.all([
  ...
  it('restores install config only after the candidate process is quiescent', async (t) => {
    const base = await realpath(
      await mkdtemp(join(tmpdir(), 'sharker-runtime-host-install-rollback-')),
    );
    t.after(() => rm(base, { recursive: true, force: true }));
    const stateRoot = await resolveStorageRoot({
      path: join(base, 'state'),
  ...
  it('migrates and restores an exact legacy systemd deployment inside configure', async (t) => {
    const base = await realpath(
      await mkdtemp(join(tmpdir(), 'sharker-runtime-host-legacy-configure-')),
    );
    t.after(() => rm(base, { recursive: true, force: true }));
    const clientDataRoot = join(base, 'config');
    const configPath = resolveRuntimeHostManagedServiceConfigPath(clientDataRoot);
  ...
  it('installs, reports, and cleanly uninstalls while retaining the State Root', async (t) => {
    const base = await realpath(await mkdtemp(join(tmpdir(), 'sharker-runtime-host-service-')));
    t.after(() => rm(base, { recursive: true, force: true }));
    const homeDir = join(base, 'home');
    const clientDataRoot = join(base, 'config', 'Sharker');
  ...
  it('refuses to remove a managed deployment through a redirected ancestor', async (t) => {
    const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-service-symlink-'));
    t.after(() => rm(base, { recursive: true, force: true }));
    const clientDataRoot = join(base, 'config');
    const serviceId = resolveRuntimeHostManagedServiceId(clientDataRoot);
    const deploymentRoot = join(base, 'data', 'Sharker', 'runtime-host-services', serviceId);
    const outsideRoot = join(base, 'outside', 'Sharker', 'runtime-host-services', serviceId);
    ...
    await rename(join(base, 'data', 'Sharker'), join(base, 'data', 'Sharker-original'));
    await symlink(join(base, 'outside', 'Sharker'), join(base, 'data', 'Sharker'));
  ...
  it('isolates managed services by Client Data Root without mutating on status', async (t) => {
    const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-service-profile-'));
    t.after(() => rm(base, { recursive: true, force: true }));
    const homeDir = join(base, 'home');
    const env = { XDG_CONFIG_HOME: join(base, 'xdg-config') };
    const cliPath = join(base, 'cli.js');
    const releaseRoot = join(base, 'profiles', 'Sharker');
    const developmentRoot = join(base, 'profiles', 'Sharker Dev');
    await writeFile(cliPath, '#!/usr/bin/env node\n', 'utf8');
  ...
  it('quotes systemd arguments without exposing specifier or environment expansion', () => {
    const config: RuntimeHostManagedServiceConfig = {
      schemaVersion: 1,
      rootPath: '/srv/Sharker $100%',
      projectDirectoryRoots: [{ label: 'Cash$', path: '/home/$ada/My Projects' }],
      websocket: { host: '127.0.0.1', port: 7443, path: '/runtime-host' },
      launch: {
        nodePath: '/opt/$Node 24/bin/node',
        cliPath: '/opt/Sharker/$current/cli.js',
      },
    };
    const unit = renderSystemdUnit(config, '/srv/Sharker $100%/runtime-host-service.json');
    assert.match(unit, /"\/srv\/Sharker \$\$100%%\/runtime-host-service\.json"/u);
    assert.match(unit, /"\/opt\/\$\$Node 24\/bin\/node"/u);
    assert.match(unit, /"\/opt\/Sharker\/\$\$current\/cli\.js"/u);
    ...
    const managed = { ...config, managedDeploymentRoot: '/opt/Sharker/$managed root' };
    const updateService = renderSystemdUpdateService(managed);
    const updateTimer = renderSystemdUpdateTimer('a'.repeat(64));
    assert.match(updateService, /"\/opt\/Sharker\/\$\$managed root\/operator"/u);
    ...
    const exitCode = await runManagedRuntimeHostServiceCli(
      {
        action: 'install',
        json: true,
        clientDataRoot: '/config/Sharker',
        defaultRootPath: '/config/Sharker/workspaces/default',
        nodePath: '/usr/bin/node',
        cliPath: '/opt/sharker/cli.js',
      },
      {
        manage: async () => {
  ...
  it('projects requested operator capabilities without launch configuration', async (t) => {
    ...
    assert.equal(frame.service.stateRoot, '/srv/sharker');
  ...
  it('reads service logs when an interrupted install left no config', async (t) => {
    const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-service-logs-'));
    t.after(() => rm(base, { recursive: true, force: true }));
    const backend: RuntimeHostServiceBackend = {
      ...createReadyBackend(),
  ...
  it('verifies the bound service and State Root before management mutations', async (t) => {
    const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-service-binding-'));
    t.after(() => rm(base, { recursive: true, force: true }));
    const clientDataRoot = join(base, 'config');
    const root = await resolveStorageRoot({ path: join(base, 'state'), kind: 'interactive' });
    const cliPath = join(base, 'cli.js');
  ...
  it('stops partial deployment state when start or restart fails', async (t) => {
    const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-service-start-failure-'));
    t.after(() => rm(base, { recursive: true, force: true }));
    const root = await resolveStorageRoot({ path: join(base, 'state'), kind: 'interactive' });
    const cliPath = join(base, 'cli.js');
    await writeFile(cliPath, '#!/usr/bin/env node\n', 'utf8');
  ...
  it('retires the exact managed Host only after active work is authorized', async (t) => {
    const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-retirement-'));
    t.after(() => rm(base, { recursive: true, force: true }));
    const clientDataRoot = join(base, 'config');
    const root = await resolveStorageRoot({ path: join(base, 'state'), kind: 'interactive' });
    const cliPath = join(base, 'cli.js');
  ...
  it('fails closed without stopping a successor that won the State Root', async (t) => {
    const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-retirement-generation-'));
    t.after(() => rm(base, { recursive: true, force: true }));
    const clientDataRoot = join(base, 'config');
    const root = await resolveStorageRoot({ path: join(base, 'state'), kind: 'interactive' });
    const cliPath = join(base, 'cli.js');
  ...
  it('restores the deployed service when the replacement never becomes ready', async (t) => {
    const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-service-rollback-'));
    t.after(() => rm(base, { recursive: true, force: true }));
    const clientDataRoot = join(base, 'config');
  ...
  it('distinguishes backend replacement failure from unknown target readiness', async (t) => {
    const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-update-failure-'));
    t.after(() => rm(base, { recursive: true, force: true }));
    const clientDataRoot = join(base, 'config');
    const root = await resolveStorageRoot({ path: join(base, 'state'), kind: 'interactive' });
  ...
  it('updates through the current operator and preserves exact update outcomes', async () => {
    const clientDataRoot = '/home/ada/.config/sharker';
    const serviceId = resolveRuntimeHostManagedServiceId(clientDataRoot);
    const deploymentRoot = '/home/ada/.local/share/Sharker/runtime-host-services/service';
    const expectedTarget = {
      serviceId,
      rootPath: '/home/ada/.local/share/Sharker/workspaces/default',
      rootId: 'a'.repeat(64),
    };
    const order: string[] = [];
    const service = (
  ...
  it('rejects invalid Project roots and temporary npx launch paths before deployment', async (t) => {
    const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-service-input-'));
    t.after(() => rm(base, { recursive: true, force: true }));
    const cliPath = join(base, 'cli.js');
  ...
  it('reports an unavailable systemd manager instead of not installed', async () => {
    const backend = createSystemdUserRuntimeHostService(
      resolveRuntimeHostManagedServiceId('/config/Sharker'),
      {
        serviceConfigPath: '/config/Sharker/runtime-host-service.json',
        runSystemctl: async () => ({
          exitCode: 1,
          stdout: '',
  ...
  it('treats an absent systemd supervisor as already retired', async (t) => {
    const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-systemd-retire-'));
    t.after(() => rm(base, { recursive: true, force: true }));
  ...
  it('serializes status behind an in-flight deployment', async (t) => {
    const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-service-lock-'));
    t.after(() => rm(base, { recursive: true, force: true }));
    const cliPath = join(base, 'cli.js');
    await writeFile(cliPath, '#!/usr/bin/env node\n', 'utf8');
...
function legacySystemdUnitFixture(config: RuntimeHostManagedServiceConfig): string {
  ...
  return [
    '[Unit]',
    'Description=Sharker Runtime Host',
