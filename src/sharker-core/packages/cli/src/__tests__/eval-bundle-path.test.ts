...
describe('installed Eval bundle', () => {
  test('points Eval containers at the installed package root', async (t) => {
    const packageRoot = await mkdtemp(join(tmpdir(), 'sharker-cli-eval-bundle-'));
    t.after(() => rm(packageRoot, { recursive: true, force: true }));
    await mkdir(join(packageRoot, 'node_modules/@sharker/eval'), { recursive: true });
    const environment: NodeJS.ProcessEnv = {};

    configureInstalledEvalBundle(environment, packageRoot);

    assert.equal(environment.SHARKER_EVAL_SHARKER_BUNDLE_PATH, packageRoot);
  });

  test('preserves an explicit bundle path', () => {
    const environment = { SHARKER_EVAL_SHARKER_BUNDLE_PATH: '/explicit/bundle' };

    configureInstalledEvalBundle(environment, '/installed/package');

    assert.equal(environment.SHARKER_EVAL_SHARKER_BUNDLE_PATH, '/explicit/bundle');
  });
  ...
  test('does not change source-checkout behavior without a packaged Eval runtime', () => {
    ...
    assert.equal(Object.hasOwn(environment, 'SHARKER_EVAL_SHARKER_BUNDLE_PATH'), false);
  });
});
