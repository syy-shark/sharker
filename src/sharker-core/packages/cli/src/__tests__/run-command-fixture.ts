...
// needed for.
const scenario = process.env.SHARKER_RUN_FIXTURE_SCENARIO ?? 'echo';
let observer: SharkerRunContextInput['runOutcomeObserver'];
...
const runtime: SharkerRunRuntime = {
...
async function createContext(input: SharkerRunContextInput): Promise<SharkerRunContext> {
...
runSharkerTextCliCore(process.argv.slice(2), { createContext, listSessions: async () => [] }).then(
  (code) => {
