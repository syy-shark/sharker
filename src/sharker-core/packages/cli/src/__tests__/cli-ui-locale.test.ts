...
describe('CLI TUI locale authority', () => {
  test('an explicit SHARKER_LOCALE wins over the POSIX locale', () => {
    assert.deepEqual(resolveCliUiLocale({ SHARKER_LOCALE: 'zh', LC_ALL: 'en_US.UTF-8' }), {
      ok: true,
      locale: 'zh',
    });
    assert.deepEqual(resolveCliUiLocale({ SHARKER_LOCALE: 'EN', LANG: 'zh_CN.UTF-8' }), {
      ok: true,
      locale: 'en',
    });
  });
  ...
  test('rejects an unsupported explicit preference with a configuration error', () => {
    assert.deepEqual(resolveCliUiLocale({ SHARKER_LOCALE: 'fr' }), {
      ok: false,
      message: 'Invalid SHARKER_LOCALE "fr". Expected zh, en, or auto.',
    });
  });
});
