...
import type { McpConfigFile, McpServerStatus, McpToolSnapshot } from '@sharker/core/mcp';
import type { McpClientManager } from '@sharker/mcp';
import type {
...
test('independent TUI controllers preserve concurrent additions in one workspace', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-tui-mcp-concurrent-'));
...
test('same-server credential retirement stays inside the shared config transaction', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-tui-mcp-retirement-'));
  t.after(() => rm(root, { recursive: true, force: true }));
