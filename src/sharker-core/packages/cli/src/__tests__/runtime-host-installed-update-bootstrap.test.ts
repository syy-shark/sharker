...
test('launches update coordination from a copy outside the mutable npm-global package', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-update-bootstrap-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  const packageRoot = join(root, 'global', 'node_modules', 'sharker-agent');
    ...
    runRuntimeHostInstalledUpdateBootstrap(
      ...
      },
      {
        resolveInstallation: async () => ({
          ...
          observedRelease: {
            version: '2.0.0',
            packageRoot: '/global/sharker-agent',
            cliPath: '/global/sharker-agent/dist/cli.js',
          },
        }),
        resolveCandidate: async () => ({
          kind: 'npm_registry',
