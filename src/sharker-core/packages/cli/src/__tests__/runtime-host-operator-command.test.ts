...
import type { RuntimeHostConnection } from '@sharker/runtime-host/client';
import { decodeRuntimeHostActivationFrame } from '@sharker/runtime-host/operator';
import {
...
const operatorClientDataRoot =
  process.platform === 'win32' ? 'C:\\sharker-control' : '/srv/sharker-control';
...
describe('Runtime Host operator commands', () => {
  ...
  test('parses project management and machine-readable service readiness', () => {
    assert.deepEqual(parseRuntimeHostCommand(['project', 'list', '--root', '/srv/sharker']), {
      kind: 'runtime-host-project-list',
      rootPath: '/srv/sharker',
    });
    assert.deepEqual(
      parseRuntimeHostCommand(['project', 'add', '/work/project', '--root', '/srv/sharker']),
      {
    ...
    assert.deepEqual(
      parseRuntimeHostCommand([
        ...
        '/config/Sharker/runtime-host-service.json',
        '--json',
      ]),
      {
        kind: 'runtime-host-serve',
        managedServiceConfigPath: '/config/Sharker/runtime-host-service.json',
        json: true,
      },
    );
    assert.equal(
    ...
    assert.equal(
      parseRuntimeHostCommand([
        ...
        '/config/Sharker/runtime-host-service.json',
        '--peer-native-path',
        '/opt/sharker/peer.node',
        '--peer-key',
        '/config/Sharker/peer.key',
      ]).kind,
      'error',
    );
    assert.deepEqual(
  ...
  test('emits bounded service identity and listener facts without credentials', () => {
    const event = createRuntimeHostServiceReadyEvent({
      ...
      compositionDescriptor: { id: 'sharker.interactive', revision: '2' },
    });
    ...
    assert.deepEqual(event, {
      ...
      composition: { id: 'sharker.interactive', revision: '2' },
      listeners: [
        { kind: 'local_ipc', endpoint: '/tmp/sharker.sock' },
        {
          kind: 'websocket',
          tls: true,
  ...
  test('registers a Project without preferring it unless explicitly requested', async () => {
    ...
    const output: string[] = [];
    const commands = [
      { kind: 'add' as const, rootPath: '/srv/sharker', path: 'project', prefer: false },
      { kind: 'add' as const, rootPath: '/srv/sharker', path: 'project', prefer: true },
    ];
