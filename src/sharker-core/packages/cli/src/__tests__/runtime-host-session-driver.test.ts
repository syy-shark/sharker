...
import { projectSessionCatalogSummary } from '@sharker/runtime-host/client';
import {
  createRuntimeHostSharkerSessionDriver,
  type RuntimeHostSharkerSessionDriverInput,
...
describe('Runtime Host Sharker Session driver', () => {
  ...
  test('keeps remote Session paths out of Client filesystem policy', async () => {
    const driver = createRuntimeHostSharkerSessionDriver({
    ...
    const driverWithoutProject = createRuntimeHostSharkerSessionDriver({
      connection: new FakeConnection([]).value,
      cwd: '/client/workspace',
      llmConnectionId: 'connection-1',
      llmConnectionSlug: 'openai-main',
  ...
  test('a rejecting user-command stop aborts the switch before any durable relocation commits (#3210)', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-tui-switch-stop-failure-'));
  ...
  test('relocates a moved Session through Host authority before attaching', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-tui-resume-moved-cwd-'));
    const target = join(root, 'new-worktree');
    await mkdir(target);
    try {
      const oldCwd = join(root, 'old-worktree');
  ...
  test('reattaches atomically when another client starts the successor turn', async () => {
    ...
    const started = deferred<SharkerAttachedSessionTurn>();
  ...
  test('serializes buffered successor reattach so transcript completion cannot reverse turn order', async () => {
    ...
    const switched = await driver.switchSession('session-1');
    assert.ok(switched.activeTurn);
    const started: SharkerAttachedSessionTurn[] = [];
  ...
  test('opens a hidden side copy at the latest completed Turn and removes it on close', async (t) => {
    const cleanupRoot = await mkdtemp(join(tmpdir(), 'sharker-tui-side-'));
  ...
  test('observes actionable and terminal parent status from the Host projection', async () => {
    ...
    const statuses: Array<SharkerSideConversationParentStatus | undefined> = [];
  ...
  test('starts explicit Skills through the Host command and preserves its typed feedback', async () => {
    ...
    // The failure names what could not be resolved: headless `sharker run` reports
...
class FakeConnection {
  ...
  readonly userCommandResource = {
    ...
    ref: 'sharker://runtime/background-tasks/user-command',
    mode: 'pipes' as const,
  ...
  readonly value: RuntimeHostSharkerSessionDriverInput['connection'];
  ...
  constructor(
    ...
    } satisfies RuntimeHostSharkerSessionDriverInput['connection'];
  }
