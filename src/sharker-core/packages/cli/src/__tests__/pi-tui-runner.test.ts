...
import { SHELL_RUN_UPDATE_BUFFER_MAX_ENTRIES } from '@sharker/core/shell-run-result';
import { type PermissionMode } from '@sharker/core/permission';
import { type OrchestrationMode } from '@sharker/core/orchestration';
import { type SessionEvent, type ShellRunUpdate } from '@sharker/core/events';
import { type SandboxBoundaryResponse } from '@sharker/core/sandbox-boundary';
import { type SessionSummary, type StoredMessage } from '@sharker/core/session';
import { type ThinkingLevel } from '@sharker/core/model-thinking';
import type { ConnectionCatalogSnapshot } from '@sharker/core/runtime-policy';
import { type UserQuestionResponse } from '@sharker/core/user-question';
import type { SkillInvocationResult } from '@sharker/core/skill-invocation';
import type {
...
import { type ContextDiagnostics } from '@sharker/runtime/context-diagnostics';
import type { GoalProjection, TurnResumeParkReason } from '@sharker/runtime-host/protocol';
import type {
  SharkerPreparePromptOptions,
  SharkerPreparedSessionTurn,
  SharkerAttachedSessionTurn,
  SharkerSessionMoveResult,
  SharkerSessionDriver,
  SharkerSideConversationParentStatus,
  SharkerSessionRewindResult,
  SharkerSessionSwitchOptions,
  SharkerSessionSwitchResult,
  SharkerSubmitMessageOptions,
  SharkerTranscriptReplacementReason,
  RewindTarget,
...
import type {
  SharkerOnboardingSurface,
  SharkerPiTuiTurnActivitySurface,
  ModelChoice,
...
import {
  ...
  runSharkerPiTui as runSharkerPiTuiImpl,
  type SharkerPiTuiInput,
...
type TestSharkerPiTuiInput = Omit<SharkerPiTuiInput, 'driver' | 'turnActivity'> & {
  driver: SharkerSessionDriver;
  turnActivity?: SharkerPiTuiTurnActivitySurface;
};

function runSharkerPiTui(input: TestSharkerPiTuiInput): Promise<void> {
  const { driver, turnActivity, ...rest } = input;
  return runSharkerPiTuiImpl({
    ...rest,
    driver,
...
function prepareTestPrompt(
  ...
  prompt: string,
  turnId = 'turn-1',
): Promise<SharkerPreparedSessionTurn> {
...
function fakeOnboardingSurface(opts: FakeOnboardingOpts = {}): SharkerOnboardingSurface {
  return {
...
describe('Sharker Pi TUI runner', () => {
  ...
  test('answers sequential questions inline with a choice, Escape, and type-to-jump Other', async () => {
    ...
    assertBottomPickerPlacement(
      terminal,
      'Choose an approach',
      'Sharker · Auto · claude-sonnet-4-5 · claude-subscription · /repo',
    );
  ...
  test('flows a transcript taller than the viewport into scrollback, untruncated and un-paged', async () => {
    ...
    assert.equal(
      screen[terminal.rows - 1]?.includes('Sharker · Auto · deepseek-v4-flash · deepseek · /repo'),
      true,
    );
  ...
  test('browses a long transcript without depending on terminal scrollback', async () => {
    ...
    assert.match(
      plainTerminalOutput(terminal.screenOutput()),
      /Sharker · Auto · deepseek-v4-flash · deepseek · \/repo/,
    );
  ...
  test('clears an unsent draft on Ctrl-C without closing Sharker', async () => {
    const terminal = new FakeTerminal();
    const driver = new SlashCommandDriver();
    const run = runSharkerPiTui({
      title: 'Sharker',
  ...
  test('requires a second idle Ctrl-C to exit Sharker', async () => {
    const terminal = new FakeTerminal();
    const driver = new SlashCommandDriver();
    const processExitCodes: number[] = [];
    const run = runSharkerPiTui({
  ...
  test('keeps Sharker open when Ctrl-D is pressed during a turn', async () => {
    const terminal = new FakeTerminal();
    const driver = new InterruptibleTurnDriver();
    const run = runSharkerPiTui({
      title: 'Sharker',
  ...
  test('exits on the second Ctrl-C during a control command', async () => {
    ...
    try {
      ...
      driver.releaseSetModel();
      if (terminal.stopCalls === 0) exitSharker(terminal);
      await run;
    }
  });
  ...
  test('resumes a read-only session as Read only, and never marks Auto as current', async () => {
    ...
    await waitFor(() =>
      plainTerminalOutput(terminal.screenOutput()).includes('Sharker · Read only ·'),
    );
    ...
    await waitFor(() => plainTerminalOutput(terminal.screenOutput()).includes('Sharker · Auto ·'));
  ...
  test('switches connection and model together from a cross-connection /model', async () => {
    ...
    await waitFor(() =>
      plainTerminalOutput(terminal.output()).includes('Sharker · Auto · glm-5.2 · zai · /repo'),
    );
  ...
  test('ignores a delayed title refresh after switching sessions', async () => {
    ...
    terminal.input('\r');
    await waitFor(() => terminal.titles.includes('Current title (Sharker)'));
  ...
  test('ignores a delayed title refresh after a manual rename', async () => {
    ...
    terminal.input('\r');
    await waitFor(() => terminal.titles.includes('Manual title (Sharker)'));

    driver.releaseList();
    await delay(0);
    assert.equal(terminal.titles.at(-1), 'Manual title (Sharker)');
  ...
  test('imports a foreign session from /session into a fresh handoff turn', async () => {
    const terminal = new FakeTerminal();
    // No Sharker sessions, so the only picker row is the foreign one.
    const driver = new SlashCommandDriver([]);
    const summary = {
  ...
  test('marks an inherited running Bash card detached after rewind', async () => {
    ...
    assert.equal(
      plainTerminalOutput(terminal.screenOutput()).includes('Ask Sharker to stop this task'),
      false,
    );
  ...
  test('rehydrates after buffer overflow instead of losing an evicted terminal update', async () => {
    const terminal = new FakeTerminal();
    const ref = 'sharker://runtime/background-tasks/bg-overflow';
    const branchMessages = [
      {
        type: 'tool_call',
        id: 'tool-bg',
    ...
    for (let index = 0; index < SHELL_RUN_UPDATE_BUFFER_MAX_ENTRIES; index += 1) {
      listener({
        ...
        result: {
          kind: 'shell_run',
          ref: `sharker://runtime/background-tasks/unrelated-${index}`,
          mode: 'pipes',
          status: 'running',
          cwd: '/repo',
          cmd: 'sleep 1',
  ...
  test('stops the running turn on Ctrl-C without closing Sharker', async () => {
    const terminal = new FakeTerminal();
    const driver = new InterruptibleTurnDriver();
    const run = runSharkerPiTui({
      title: 'Sharker',
  ...
  test('enables focus reporting only after raw mode, so no stray ^[[I leaks on launch', async () => {
    ...
    assert.ok(terminal.titles.includes('Sharker'));
  ...
  describe('slash commands during a running turn', () => {
    ...
    test('a Turn that attaches after a mid-turn detach does not adopt abandoned metadata', async () => {
      ...
      await delay(0);
      assert.equal(terminal.titles.includes('ABANDONED TITLE (Sharker)'), false);
      assert.equal(terminal.titles.at(-1), 'Existing chat (Sharker)');
...
function exitSharker(_terminal: FakeTerminal): void {
  const previousExitCode = process.exitCode;
  process.emit('SIGTERM');
  process.exitCode = previousExitCode;
}
...
/**
 * The parts of `SharkerSessionDriver` every fake in this file answers the same
 ...
 */
abstract class FakeSessionDriver implements SharkerSessionDriver {
  startedTurnListener: ((turn: SharkerAttachedSessionTurn) => void) | undefined;
  hostSummary: Partial<SessionSummary> = {};
  protected sessionId = 'session-1';

  abstract preparePrompt(
    prompt: string,
    options?: SharkerPreparePromptOptions,
  ): Promise<SharkerPreparedSessionTurn>;
  ...
  submitMessage(
    text: string,
    options: SharkerSubmitMessageOptions,
  ): Promise<TurnMessageSubmitResult | undefined> {
    return admitMessageAsTurn(this, text, options);
  }

  subscribeStartedTurns(listener: (turn: SharkerAttachedSessionTurn) => void): () => void {
  ...
  async switchSession(sessionId: string): Promise<SharkerSessionSwitchResult> {
  ...
  async rewindToTurn(_turnId: string): Promise<SharkerSessionRewindResult> {
...
class RejectingStopDriver extends FakeSessionDriver {
  ...
  preparePrompt(prompt: string): Promise<SharkerPreparedSessionTurn> {
...
class SandboxBoundaryPromptDriver extends FakeSessionDriver {
  ...
  async rewindToTurn(): Promise<SharkerSessionRewindResult> {
    throw new Error('rewind not supported in this fake');
  }
  startNewSession(): Promise<void> {
    return Promise.resolve();
...
class SteeringTurnDriver extends FakeSessionDriver {
  ...
  readonly turnOrchestrations: Array<SharkerPreparePromptOptions['turnOrchestration']> = [];
  ...
  async submitMessage(text: string, options: SharkerSubmitMessageOptions): Promise<undefined> {
...
class FailingOrchestrationDriver extends SteeringTurnDriver {
  override preparePrompt(): Promise<SharkerPreparedSessionTurn> {
...
class SlashCommandDriver extends FakeSessionDriver {
  ...
  readonly sessionSwitchOptions: Array<SharkerSessionSwitchOptions | undefined> = [];
  ...
  async moveSession(cwd: string): Promise<SharkerSessionMoveResult> {
    this.moves.push(cwd);
    return {
      previousCwd: '/repo',
      cwd,
  ...
  async switchSession(
    sessionId: string,
    options?: SharkerSessionSwitchOptions,
  ): Promise<SharkerSessionSwitchResult> {
  ...
  async runUserCommand(command: string) {
    ...
    return {
      ...
      result: {
        ...
        ref: `sharker://runtime/background-tasks/user-command-${this.commands.length}`,
        mode: 'pipes' as const,
...
class RunningUserCommandDriver extends SlashCommandDriver {
  ...
  readonly #transcriptListeners = new Set<
    (
      ...
      reason: SharkerTranscriptReplacementReason,
    ) => void
  >();
...
class HostSkillDriver extends SlashCommandDriver {
  ...
  override async preparePrompt(
    ...
    // `sharker run`, which reports the refusal as an ordinary failure.
...
class SideConversationDriver extends SlashCommandDriver {
  ...
  parentStatusListener:
    | ((status: SharkerSideConversationParentStatus | undefined) => void)
    | undefined;
  ...
  async observeSideConversationParent(
    ...
    listener: (status: SharkerSideConversationParentStatus | undefined) => void,
  ) {
  ...
  publishParentStatus(status: SharkerSideConversationParentStatus | undefined): void {
...
class FailingParentObserverSideConversationDriver extends SideConversationDriver {
  override async observeSideConversationParent(
    _parentSessionId: string,
    _listener: (status: SharkerSideConversationParentStatus | undefined) => void,
...
class FailingSwitchSessionDriver extends SlashCommandDriver {
  async switchSession(_sessionId: string): Promise<SharkerSessionSwitchResult> {
    throw new Error('session not found');
  }
}

class ActiveResumeDriver extends SlashCommandDriver {
  override async switchSession(sessionId: string): Promise<SharkerSessionSwitchResult> {
...
class DetachingSwitchDriver extends SlashCommandDriver {
  ...
  announceStartedTurn(turn: SharkerAttachedSessionTurn): void {
    ...
    this.startedTurnListener?.({
      ...
      messages: [
        ...
        },
        {
          ...
          content: {
            kind: 'shell_run',
            ref: 'sharker://runtime/background-tasks/bg-successor',
            mode: 'pipes',
            status: 'running',
            cwd: '/repo',
            cmd: 'build',
...
class FirstSessionPreparedDriver extends SlashCommandDriver {
  ...
  async preparePrompt(prompt: string): Promise<SharkerPreparedSessionTurn> {
    this.prepared = true;
...
class RewindDriver extends SlashCommandDriver {
  ...
  override async rewindToTurn(turnId: string): Promise<SharkerSessionRewindResult> {
...
async function runSignalExitProbe(
  ...
  const childSource = `
    import { runSharkerPiTui } from ${JSON.stringify(runnerUrl)};
    import { beginSharkerCliExit } from ${JSON.stringify(cliUrl)};
    ...
    const run = runSharkerPiTui({
      ...
      terminal,
      turnActivity,
      onProcessExit: (exitCode) => beginSharkerCliExit(exitCode),
    });
  ...
  child.stdout.on('data', (chunk: string) => {
    ...
    if (!signalSent && stdout.includes('READY')) {
      ...
      // 3s exit grace (beginSharkerCliExit) plus the pre-READY startup must not
...
async function runFatalExitProbe(
  ...
  const childSource = `
    import { runSharkerPiTui } from ${JSON.stringify(runnerUrl)};
    import { beginSharkerCliExit, formatSharkerCliFatalError } from ${JSON.stringify(cliUrl)};
    import { FakeTerminal } from ${JSON.stringify(terminalUrl)};
    ...
    try {
      const run = runSharkerPiTui({
        ...
        onProcessExit: (exitCode, error) => {
          if (error) process.stderr.write(\`${'${formatSharkerCliFatalError(error)}'}\\n\`);
          beginSharkerCliExit(exitCode);
        },
      });
      process.stdout.write('READY\\n');
      ${trigger}
    ...
    process.stdout.write('CLOSED\\n');
    if (${hangOuterCleanup}) await new Promise(() => {});
    if (fatalError) process.stderr.write(\`${'${formatSharkerCliFatalError(fatalError)}'}\\n\`);
    clearInterval(hold);
  `;
  ...
  // 3 s exit grace from beginSharkerCliExit).
  let killTimer = setTimeout(() => child.kill('SIGKILL'), 30_000);
  child.stdout.setEncoding('utf8');
