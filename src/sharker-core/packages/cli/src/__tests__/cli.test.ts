...
import { parseSharkerCliArgs, runSharkerCli } from '../cli-core.js';

describe('Sharker CLI args', () => {
  ...
  test('publishes the supported release command surface', () => {
    const help = parseSharkerCliArgs(['--help'], '0.1.0');
    assert.equal(help.kind, 'help');
    if (help.kind !== 'help') return;
    assert.match(help.text, /^  sharker              Start the TUI$/m);
    assert.doesNotMatch(help.text, /sharker-agent/);
    assert.match(help.text, /^  sharker run /m);
    assert.match(help.text, /^  sharker activate /m);
    assert.match(help.text, /^  sharker eval /m);
    assert.match(help.text, /^  sharker update --target /m);
    assert.match(
      help.text,
      /^  sharker --acp      Serve ACP v1 over stdio \(initialize only; session support in progress\)$/m,
    );
    assert.match(help.text, /^  sharker runtime-host serve /m);
    assert.doesNotMatch(help.text, /cli:dev/);
  });

  test('requires an explicit installed update target and interruption choice', () => {
    assert.deepEqual(parseSharkerCliArgs(['update', '--target', 'next'], '0.1.0'), {
      kind: 'runtime-host-installed-update',
      selector: { kind: 'channel', channel: 'next' },
      allowInterruptActiveTasks: false,
    });
    assert.deepEqual(
      parseSharkerCliArgs(['update', '--target', '1.2.3', '--allow-interrupt-active-tasks'], '0.1.0'),
      {
        kind: 'runtime-host-installed-update',
    ...
    assert.deepEqual(parseSharkerCliArgs(['update'], '0.1.0'), {
      kind: 'error',
  ...
  test('selects a Runtime Host and Project for TUI startup', () => {
    assert.deepEqual(parseSharkerCliArgs(['--host', 'office', '--project', 'project-1'], '0.1.0'), {
      kind: 'tui',
  ...
  test('parses the ACP stdio command before TUI flags', () => {
    assert.deepEqual(parseSharkerCliArgs(['--acp'], '0.1.0'), { kind: 'acp' });
    assert.deepEqual(parseSharkerCliArgs(['--acp', 'extra'], '0.1.0'), {
      kind: 'error',
  ...
  test('compiled ACP launcher rejects trailing arguments without help output', async () => {
    ...
    assert.equal(result.stderr, 'sharker --acp does not accept arguments\n');
  });

  test('uses the active launcher in development help and rejects an empty profile name', async () => {
    const help = parseSharkerCliArgs(['--help'], '0.1.0', 'npm run cli:dev --');
    assert.equal(help.kind, 'help');
    if (help.kind === 'help') {
      ...
      assert.doesNotMatch(help.text, /^  sharker runtime-host /m);
      assert.doesNotMatch(help.text, /sharker-agent/);
    }
    await assert.rejects(
      runSharkerCli(['--version'], {
        dataProfileName: '',
        cliCommand: 'invalid',
  ...
  test('does not add a discoverable global locale flag', () => {
    assert.deepEqual(parseSharkerCliArgs(['--locale', 'zh'], '0.1.0'), {
      kind: 'error',
  ...
  test('establishes the fatal exit before reporting can throw', async () => {
    ...
    const childSource = `
      import { handleSharkerCliProcessExit } from ${JSON.stringify(cliUrl)};
      try {
        handleSharkerCliProcessExit(1, new Error('fatal'), () => { throw new Error('writer failed'); });
      } catch {}
    `;
  ...
  test('compiled release and repository entries isolate profile writes', async (t) => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-cli-launch-profile-'));
    t.after(() => rm(root, { recursive: true, force: true }));
    const home = join(root, 'home');
    const applicationData = join(root, 'application-data');
    const releaseRoot = platformProfileRoot(home, applicationData, 'Sharker');
    const developmentRoot = platformProfileRoot(home, applicationData, 'Sharker Dev');
    const env = {
      ...process.env,
      HOME: home,
      USERPROFILE: home,
      ...
      SHARKER_TEST_RUNTIME_HOST_CREDENTIAL: 'opaque-token',
    };
    const profileArgs = [
      'runtime-host',
      'profile',
      ...
      'a'.repeat(64),
      '--credential-env',
      'SHARKER_TEST_RUNTIME_HOST_CREDENTIAL',
    ];
  ...
  test('compiled launchers isolate capability-provider identities', async (t) => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-cli-provider-identity-'));
    t.after(() => rm(root, { recursive: true, force: true }));
    const home = join(root, 'home');
    const applicationData = join(root, 'application-data');
    const releaseRoot = platformProfileRoot(home, applicationData, 'Sharker');
