...
import { RUNTIME_HOST_PROTOCOL_VERSION } from '@sharker/runtime-host/protocol';
import {
  createUnavailableDomainOperationHandlers,
  defineInteractiveRuntimeHostComposition,
  RuntimeHostKernel,
} from '@sharker/runtime-host/server';
import {
  HostClientCapabilityCoordinator,
  RuntimePolicyActivationGate,
  type ClientCapabilitySnapshot,
} from '@sharker/runtime-host/test-only/client-capability-host';
...
test('local TUI discovers, publishes, invokes, republishes, and closes one MCP child', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-tui-mcp-'));
  ...
  try {
    ...
    const fixturePath = fileURLToPath(
      new URL(import.meta.resolve('@sharker/mcp/test-only/stdio-server')),
    );
    await createMcpConfigStore(clientRoot).upsert('fixture', {
      command: process.execPath,
      args: [fixturePath],
      env: { SHARKER_MCP_STDIO_EVENT_LOG: eventLog },
      protocol: 'legacy',
    });
