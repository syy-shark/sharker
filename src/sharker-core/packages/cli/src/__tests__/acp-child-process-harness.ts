...
import { deriveSharkerDataRoots, resolveSharkerClientDataRoot } from '../workspace-root.js';
...
export class AcpChildProcessHarness {
  ...
  async withClient<T>(
    ...
    const app = configureClient(client({ name: 'sharker-acp-child-process-test' }));
...
export async function startAcpChildProcessHarness(
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-acp-child-process-'));
  ...
  const clientDataRoot = resolveSharkerClientDataRoot({
    env,
    homeDir: home,
    profileName: 'Sharker Dev',
  });
  const dataRoots = deriveSharkerDataRoots(clientDataRoot);
