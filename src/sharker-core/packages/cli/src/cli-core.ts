...
import { deriveSharkerDataRoots, resolveSharkerDataRoots } from './workspace-root.js';
import {
...
export interface SharkerCliLaunchOptions {
  readonly dataProfileName: string;
  readonly cliCommand: string;
  readonly capabilityProviderIdentityScope: 'legacy-home' | 'client-data-root';
}

export const RELEASE_SHARKER_CLI_LAUNCH_OPTIONS = {
  dataProfileName: 'Sharker',
  cliCommand: 'sharker',
  capabilityProviderIdentityScope: 'legacy-home',
} satisfies SharkerCliLaunchOptions;

export function parseSharkerCliArgs(
  argv: string[],
  version: string,
  cliCommand = RELEASE_SHARKER_CLI_LAUNCH_OPTIONS.cliCommand,
): SharkerCliCommand {
  ...
  if (first === '--acp') {
    return argv.length === 1
      ...
      : {
          kind: 'error',
          message: 'sharker --acp does not accept arguments',
          exitCode: 2,
          showHelp: false,
        };
  }
...
export function resolveSharkerCliExitCode(
...
export function formatSharkerCliFatalError(error: unknown): string {
  return error instanceof Error ? (error.stack ?? error.message) : String(error);
}

let processExitTimer: NodeJS.Timeout | undefined;

export function beginSharkerCliExit(commandExitCode: number): void {
  const exitCode = resolveSharkerCliExitCode(commandExitCode, process.exitCode);
...
function helpText(cliCommand: string): string {
  return [
    `Usage: ${cliCommand}`,
    '',
    'Launches the Sharker terminal UI in the current working directory.',
    '',
    'Commands:',
    ...
    '  SHARKER_RUNTIME_HOST_ACCESS_CREDENTIAL  Access credential used by runtime-host profile set',
    '',
...
export async function runSharkerCli(
  argv: string[] = process.argv.slice(2),
  options: SharkerCliLaunchOptions = RELEASE_SHARKER_CLI_LAUNCH_OPTIONS,
): Promise<number> {
  const version = await readPackageVersion();
  const command = parseSharkerCliArgs(argv, version, options.cliCommand);
  const dataRoots = resolveSharkerDataRoots({
  ...
  switch (command.kind) {
    ...
    case 'activate': {
      const { runSharkerActivationCli } = await import('./activation-command.js');
      return runSharkerActivationCli(command.args);
    }
    case 'eval': {
      const { configureInstalledEvalBundle } = await import('./eval-bundle-path.js');
      configureInstalledEvalBundle();
      const { runSharkerEvalCli } = await import('@sharker/eval');
      return runSharkerEvalCli(command.args);
    }
    case 'acp': {
      const { runSharkerAcpStdioServer } = await import('./acp/stdio-server.js');
      return runSharkerAcpStdioServer({ version });
    }
    case 'runtime-host-serve': {
      ...
      if (command.managedDeployment) {
        const { resolveRuntimeHostManagedDeployment, resolveRuntimeHostNpmDeploymentLayout } =
          await import('@sharker/runtime-host/operator');
    ...
    case 'runtime-host-service-manage': {
      ...
      const serviceDataRoots = command.clientDataRoot
        ? deriveSharkerDataRoots(command.clientDataRoot)
        : dataRoots;
    ...
    case 'tui': {
      ...
      return runRuntimeHostTui({
        ...
        cwd: process.cwd(),
        onProcessExit: handleSharkerCliProcessExit,
...
function parseTuiArgs(argv: string[]): SharkerCliCommand {
...
export function launchSharkerCli(options: SharkerCliLaunchOptions): void {
  runSharkerCli(process.argv.slice(2), options).then(
    (code) => {
      beginSharkerCliExit(code);
    },
    (error) => {
      handleSharkerCliProcessExit(1, error);
    },
  );
}
