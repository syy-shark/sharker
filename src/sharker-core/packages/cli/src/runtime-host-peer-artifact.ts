...
const PEER_NATIVE_FILE = 'sharker_runtime_host_peer.node';
...
export async function resolveRuntimeHostPeerNativePath(cliPath: string): Promise<string> {
  ...
  throw new Error(`Sharker does not include a direct-peer native artifact for ${target}`);
}
...
export async function configureRuntimeHostPeerClient(input: {
  ...
  const explicitNativePath = environment.SHARKER_RUNTIME_HOST_PEER_NATIVE_PATH?.trim();
  const explicitKeyPath = environment.SHARKER_RUNTIME_HOST_PEER_KEY_PATH?.trim();
  if (explicitNativePath || explicitKeyPath) return Boolean(explicitNativePath && explicitKeyPath);
  try {
    environment.SHARKER_RUNTIME_HOST_PEER_NATIVE_PATH = await resolveRuntimeHostPeerNativePath(
      input.cliPath,
    );
    environment.SHARKER_RUNTIME_HOST_PEER_KEY_PATH = resolveRuntimeHostClientPeerKeyPath(
