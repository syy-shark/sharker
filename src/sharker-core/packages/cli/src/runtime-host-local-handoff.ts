...
import { compareProductReleaseVersions } from '@sharker/runtime-host/operator/update-package-evidence';
import { connectExistingRuntimeHost } from '@sharker/runtime-host/client';
import {
      ...
      },
      {
        ...
        async finalizeTarget() {
          ...
          if (
            ...
            throw new RuntimeHostLocalHandoffError(
              'installed_release_mismatch',
              'The installed Sharker release changed again before local Host ownership committed',
            );
          }
        },
      },
...
export async function reconcileRuntimeHostNpmGlobalDeployment(
  ...
  if (installation.observedRelease.version !== request.target.version) {
    throw new RuntimeHostLocalHandoffError(
      'installed_release_mismatch',
      `The installed Sharker release changed from ${request.target.version} to ${installation.observedRelease.version} before local Host reconciliation`,
    );
  }
  const staged = await stageRuntimeHostNpmGlobalDeploymentTarget(
    {
...
export function resolveRuntimeHostLocalCliDeploymentRoot(
  ...
  return path.join(dataRoot, 'Sharker', 'runtime-host-deployments', 'cli', ownerKey, rootId);
}
...
async function requireCandidateEntrypoint(packageRoot: string): Promise<string> {
  ...
  try {
    ...
    throw invalidStagedPackage('The staged Sharker package has no Runtime Host candidate', cause);
  }
