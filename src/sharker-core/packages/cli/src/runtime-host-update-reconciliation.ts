      ...
      async () => {
        ...
        if (!status.service.installed || !managedDeploymentRoot) {
          throw new RuntimeHostServiceManagerError(
            'not_installed',
            'An automatic update policy requires a Sharker-managed Runtime Host service',
          );
        }
...
function writeFrame(
  ...
  if (frame.kind === 'progress') {
    if (options.framed) {
      ...
      deps.writeError(
        `Reconciling Sharker ${frame.currentVersion} toward ${frame.targetVersion} (${frame.phase})...\n`,
      );
    }
    return;
  }
...
function humanResult(frame: Extract<UpdatePolicyFrame | ReconcileUpdateFrame, { kind: 'result' }>) {
  ...
  if (result.kind === 'manual_action') {
    return `Sharker ${result.candidate.version} requires manual update action (${result.reason}).`;
  }
  ...
  if (result.kind === 'already_current') {
    return `The managed Runtime Host is current at Sharker ${result.version}.`;
  }
  if (result.kind === 'repaired') {
    return `The managed Runtime Host deployment for Sharker ${result.version} was repaired.`;
  }
  return `The managed Runtime Host was updated from Sharker ${result.previousVersion} to ${result.targetVersion}.`;
}
