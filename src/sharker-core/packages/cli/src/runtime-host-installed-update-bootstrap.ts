...
export async function runRuntimeHostInstalledUpdateBootstrap(
  ...
  if (compareProductReleaseVersions(target.version, installation.observedRelease.version) < 0) {
    throw new Error(
      `Downgrading Sharker from ${installation.observedRelease.version} to ${target.version} is not supported`,
    );
  }

  return deps.withArchive(target, async (archivePath) => {
    const temporaryRoot = await mkdtemp(join(tmpdir(), 'sharker-installed-update-coordinator-'));
    try {
      const coordinatorPackageRoot = join(temporaryRoot, 'sharker-agent');
      ...
      if (!(await stat(coordinatorCliPath)).isFile()) {
        throw new Error('The copied Sharker update coordinator has no CLI entry point');
      }
...
function launchCoordinator(input: RuntimeHostInstalledUpdateCoordinatorLaunch): Promise<number> {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once('close', (code, signal) => {
      if (signal) reject(new Error(`Sharker update coordinator exited on ${signal}`));
      else resolve(code ?? 1);
    });
  });
}
