...
import { isRuntimeHostNpmDeploymentIdentity } from '@sharker/runtime-host/operator/update-package-evidence';
import type { RuntimeHostUpdateCandidate } from './runtime-host-registry-update.js';

const PACKAGE_NAME = 'sharker-agent';
...
export async function withRuntimeHostRegistryUpdateArchive<T>(
  ...
  const temporaryRoot = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-update-'));
  try {
    let archive: string;
    try {
      ...
      if (packed !== 0) {
        throw new RuntimeHostUpdatePackageError(
          ...
          `Unable to download Sharker ${candidate.version} from the official npm registry`,
        );
      }
      ...
      throw new RuntimeHostUpdatePackageError(
        'package_download_failed',
        `Unable to prepare Sharker ${candidate.version} for an update`,
        { cause: error },
      );
    }
...
export async function withVerifiedRuntimeHostUpdateArchive<T>(
  ...
  assertCandidate(candidate);
  const temporaryRoot =
    parentTemporaryRoot ?? (await mkdtemp(join(tmpdir(), 'sharker-runtime-host-update-')));
  try {
    ...
    if (installed !== 0) {
      throw new RuntimeHostUpdatePackageError(
        'invalid_package',
        `Unable to extract the verified Sharker ${candidate.version} package`,
      );
    }
...
async function validateArchive(
  ...
  if (
    ...
    throw new RuntimeHostUpdatePackageError(
      'invalid_package',
      'The Sharker package archive is invalid',
    );
  }
  if ((await packageIntegrity(archive)) !== candidate.integrity) {
    throw new RuntimeHostUpdatePackageError(
      'package_integrity_mismatch',
      `The downloaded Sharker ${candidate.version} package does not match its registry integrity`,
    );
  }
  return archive;
}
  ...
  try {
    for await (const chunk of stream as AsyncIterable<Buffer>) {
      ...
      while (pending.length - offset >= TAR_BLOCK_BYTES) {
        ...
        if (zeroBlocks !== 0) fail('The Sharker package archive has a malformed tar terminator');
        ...
        if (entries > budget.maxEntries || extractedBytes > budget.maxExtractedBytes) {
          fail('The Sharker package archive exceeds its extraction budget');
        }
    ...
    if (error instanceof RuntimeHostUpdatePackageError) throw error;
    fail('The Sharker package archive is not a readable gzip tarball', { cause: error });
  } finally {
    stream.destroy();
  }
  if (!ended) {
    fail('The Sharker package archive is a truncated tarball');
  }
}
...
function assertSupportedTarType(header: Buffer, fail: (message: string) => never): void {
  ...
  fail('The Sharker package archive uses an unsupported extended tar header');
}
...
function tarEntrySize(header: Buffer, fail: (message: string) => never): number {
  ...
  if (!/^[0-7]+$/u.test(text)) {
    fail('The Sharker package archive has a malformed tar header');
  }
...
async function requireDownloadedArchive(downloadRoot: string): Promise<string> {
  ...
  if (entries.length !== 1 || !entries[0]?.isFile() || !entries[0].name.endsWith('.tgz')) {
    throw new RuntimeHostUpdatePackageError(
      'package_download_failed',
      'The npm registry did not return one Sharker package archive',
    );
  }
  ...
  if (
    ...
    throw new RuntimeHostUpdatePackageError(
      'invalid_package',
      'The downloaded Sharker package archive is invalid',
    );
  }
  return archive;
}
...
async function validateExtractedPackage(
  ...
  try {
    ...
    const [manifestMetadata, cli, runtimeHost] = await Promise.all([
      ...
      stat(join(packageRoot, 'node_modules', '@sharker', 'runtime-host', 'package.json')),
    ]);
    if (
      ...
      throw new RuntimeHostUpdatePackageError(
        'invalid_package',
        'The downloaded Sharker package is not a self-contained release',
      );
    }
    const manifest: unknown = JSON.parse(await readFile(manifestPath, 'utf8'));
    const compatibility =
      isRecord(manifest) && isRecord(manifest.sharker)
        ? positiveInteger(manifest.sharker.managedRuntimeHostUpdateCompatibility)
        : undefined;
    if (
      !isRecord(manifest) ||
      ...
      throw new RuntimeHostUpdatePackageError(
        'invalid_package',
        'The downloaded Sharker package does not match its registry metadata',
      );
    }
    return packageRoot;
  } catch (error) {
    ...
    throw new RuntimeHostUpdatePackageError(
      'invalid_package',
      'The downloaded Sharker package manifest is invalid',
      { cause: error },
    );
  }
}
