...
import { resolveRuntimeHostNpmDeploymentLayout } from '@sharker/runtime-host/operator';
import { syncDirectory, syncDirectoryChain, syncFile } from '@sharker/storage/stable-storage';
...
export async function prepareRuntimeHostPackageDeployment(input: {
  ...
  try {
    ...
    throw new RuntimeHostPackageDeploymentError(
      'deployment_failed',
      `Unable to install Sharker ${input.version} into the Runtime Host package store`,
      { cause: error },
    );
  }
}
...
export async function openRuntimeHostPackageDeployment(input: {
  ...
  try {
    ...
    throw new RuntimeHostPackageDeploymentError(
      'invalid_package',
      `The staged Sharker ${input.version} package is unavailable`,
      { cause: error },
    );
  }
...
async function validatePackage(path: string, version: string): Promise<string> {
  ...
  try {
    ...
    const runtimeHost = await stat(
      join(packageRoot, 'node_modules', '@sharker', 'runtime-host', 'package.json'),
    );
    if (!cli.isFile() || !runtimeHost.isFile()) throw new Error('Package payload is incomplete');
  } catch (error) {
    throw new RuntimeHostPackageDeploymentError(
      'invalid_package',
      `Sharker ${version} is not a self-contained release package`,
      { cause: error },
    );
  }
...
function assertVersion(version: string): void {
  if (!/^[0-9A-Za-z][0-9A-Za-z.+-]{0,127}$/u.test(version)) {
    throw new RuntimeHostPackageDeploymentError(
      'invalid_package',
      'The Sharker package version cannot be used as a deployment identity',
    );
  }
}
