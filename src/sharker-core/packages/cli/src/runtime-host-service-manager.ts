...
} from '@sharker/storage/process-lifetime-file-update-lock';
import {
...
async function prepareServiceConfig(
  ...
  if (!input.cliPath) {
    throw new RuntimeHostServiceManagerError(
      'invalid_launch',
      'The current Sharker CLI entry point could not be resolved',
    );
  }
  const serviceId = resolveRuntimeHostManagedServiceId(input.clientDataRoot);
  ...
  const [nodePath, cliPath] = await Promise.all([
    ...
    throw new RuntimeHostServiceManagerError(
      'invalid_launch',
      'The current Node.js or Sharker CLI installation is unavailable',
      { cause: error },
    );
  });
...
async function assertPersistentCliInstallation(
  ...
  if (await isTemporaryNpxInstallation(cliPath, { environment, homeDir })) {
    throw new RuntimeHostServiceManagerError(
      'invalid_launch',
      'A persistent Runtime Host service cannot use a temporary npx installation; install Sharker globally and retry',
    );
  }
}
