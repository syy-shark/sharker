...
export function launchRuntimeHostTargetActivator(
  ...
  return new Promise((resolve, reject) => {
    ...
    const closeError = async (): Promise<never> => {
      ...
      if (signal) throw new Error(`Sharker target activator exited on ${signal}`);
      ...
      throw new Error('The exact Sharker target could not be activated');
    };
    ...
    void closed.then(({ code, signal }) => {
      if (ready) return;
      if (signal) {
        reject(new Error(`Sharker target activator exited on ${signal}`));
      } else if (code === 3) {
