...
import {
  ...
  type ThinkingLevel,
} from '@sharker/core/model-thinking';
import { type ModelInfo, type ProviderType } from '@sharker/core/llm-connections';
...
import { projectRevisionLinkedSessionTree } from '@sharker/core/session-revisions';
import {
  slashCommandsForSurface,
  type SlashCommandIdForSurface,
} from '@sharker/core/slash-command-catalog';
...
import { formatUiMessage, type UiLocale } from '@sharker/core/ui-locale';
import {
  buildForeignSessionHandoffMessage,
  foreignSessionHandoffDisplayText,
  foreignSourceLabel,
  type ForeignSessionSummary,
} from '@sharker/core/foreign-session';
import type { ContextDiagnostics } from '@sharker/runtime/context-diagnostics';
import type { GoalTurnOutcome } from '@sharker/runtime/goal-continuation';
import type { TurnOrchestration } from '@sharker/core/runtime-inputs';
import type { SessionActivityLease } from '@sharker/runtime/goal-turn-lifecycle';
import { listApiKeyOnboardableProviders } from './onboarding-catalog.js';
import type {
  ConnectionIdentity,
  SharkerForeignSessionReader,
...
import type { InvocableSkillEntry } from '@sharker/runtime/skill-invocation';
import type {
...
import { SharkerSkillHighlightEditor } from './skill-highlight-editor.js';
import { parseGraphCommand, type ParsedGraphCommand } from '@sharker/core/graph-command';
import { parseSwarmCommand, type ParsedSwarmCommand } from '@sharker/core/swarm-command';
import {
  inspectSessionResumeAvailability,
  type SharkerAttachedSessionTurn,
  type SharkerPreparedSessionTurn,
  type SharkerSessionDriver,
  type SharkerSideConversationParentStatus,
  type SharkerSessionSwitchResult,
...
import {
  ...
  type SharkerPiTranscriptMetadata,
} from './pi-transcript.js';
import { runSharkerPiTuiTurn, type SharkerPiTuiTurnRequest } from './pi-tui-turn.js';
import { editorTheme, selectListTheme } from './tui-ansi.js';
import { SharkerAutocompleteAboveEditorComponent } from './tui-autocomplete-layout.js';
...
import {
  SharkerActivityStripComponent,
  SharkerPendingQueueComponent,
  SharkerPiLayoutComponent,
  SharkerStatusLineComponent,
  SharkerTranscriptComponent,
} from './pi-tui-layout.js';
import {
  SharkerAutocompleteProvider,
  ...
  type SharkerSlashCommand,
} from './pi-tui-pickers.js';
import { formatSharkerResumeCommand } from './cli-invocation.js';
import {
...
import type { GoalControlAction, GoalProjection } from '@sharker/runtime-host/protocol';

export interface SharkerPiTuiInput {
  ...
  onboarding?: SharkerOnboardingSurface;
   ...
   * fresh Sharker session seeded with it. Omitting it hides the feature.
   */
  foreignSessions?: SharkerForeignSessionReader;
...
export function resolveTaskbarProgress(
  ...
  environment: TaskbarProgressEnvironment = {
    ...
    override: process.env.SHARKER_TASKBAR_PROGRESS,
...
export function safeBoundaryResumeParkedCopy(reason: TurnResumeParkReason): {
  ...
  switch (reason) {
    case 'resume_feature_disabled':
      return {
        level: 'info',
        text: 'Safe-boundary resume is not enabled on this runtime (set SHARKER_RUNTIME_SAFE_BOUNDARY_RESUME=1 to enable).',
      };
...
function sessionConnectionIdentityNotice(
  ...
  identities: SharkerPiTuiInput['connectionIdentities'],
  locale: UiLocale,
): string | undefined {
...
export async function runSharkerPiTui(input: SharkerPiTuiInput): Promise<void> {
  ...
  type AttachedTurnContext =
    | { readonly kind: 'adopted'; readonly turn: SharkerPreparedSessionTurn }
    | { readonly kind: 'external'; readonly turn: SharkerAttachedSessionTurn };
  ...
  const metadata = (): SharkerPiTranscriptMetadata => ({
    title: input.title,
    cwd,
    model,
    connectionSlug,
  ...
  const transcript = new SharkerTranscriptComponent(state, metadata);
  const activityStrip = new SharkerActivityStripComponent(metadata);
  const pendingQueue = new SharkerPendingQueueComponent(state);
  const statusLine = new SharkerStatusLineComponent(metadata);
  ...
  const editor = new SharkerSkillHighlightEditor(tui, editorTheme(), {
    paddingX: 0,
    autocompleteMaxVisible: EDITOR_AUTOCOMPLETE_MAX_VISIBLE,
  });
  let refreshEditorCwd: ((cwd: string) => void) | undefined;
  const editorSurface = new SharkerAutocompleteAboveEditorComponent(editor);
  const layout = new SharkerPiLayoutComponent(
    state,
    transcript,
    activityStrip,
    pendingQueue,
  ...
  function runAgentTurn(
    request: SharkerPiTuiTurnRequest,
    authoritativeAttachedTurn?: SharkerAttachedSessionTurn,
    ...
    return runSharkerPiTuiTurn({
      ...
      onEvent: (event) => {
        ...
        applySharkerSessionEventToTranscript(state, event);
  ...
  const applySwitchResult = async ({
    ...
    messages,
    activeTurn,
  }: SharkerSessionSwitchResult): Promise<void> => {
  ...
  const showSessionList = async () => {
    ...
    // Sharker-session availability and the foreign scan are independent I/O; run
    ...
    // prefixed select value so they never collide with Sharker session ids. A scan
  ...
  // fresh Sharker session, and seed the first turn with an untrusted handoff
  ...
  type TuiSlashCommandHandler = Omit<SharkerSlashCommand, 'name' | 'aliases'>;
  ...
  const slashCommands: SharkerSlashCommand[] = slashCommandsForSurface('tui').map((spec) => ({
    name: spec.id,
  ...
  refreshEditorCwd = (nextCwd) => {
    editor.setAutocompleteProvider(
      new SharkerAutocompleteProvider(
      ...
      try {
        ...
        const recoveryHint =
          input.resumeCwd === undefined && message.startsWith('Session cwd no longer exists:')
            ? ` Retry with: ${formatSharkerResumeCommand(
                input.cliCommand ?? 'sharker',
