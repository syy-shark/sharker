...
import { failureClassFromCompleteStopReason, type SessionEvent } from '@sharker/core/events';
import { findProjectByIdentity } from '@sharker/core/project';
...
import type { InteractionPendingSnapshot, SessionCatalogItem } from '@sharker/runtime-host/protocol';
import {
  runSharkerTextCliCore,
  type SharkerRunContext,
  type SharkerRunContextInput,
  type SharkerRunDeps,
  type SharkerRunEnvironmentDeps,
  type SharkerRunOutcome,
  type SharkerRunRuntime,
...
import {
  createRuntimeHostSharkerSessionDriver,
  type RuntimeHostSharkerSessionDriver,
} from './runtime-host-session-driver.js';
import type { CreateSessionRequest, SharkerPreparedSessionTurn } from './session-driver.js';
import {
...
import { resolveSharkerClientDataRoot } from './workspace-root.js';
...
export interface RuntimeHostRunCommandDeps {
  ...
  createContext(
    ...
    input: Parameters<SharkerRunDeps['createContext']>[0],
    profile: RuntimeHostProfile,
  ): SharkerRunContext | Promise<SharkerRunContext>;
  run: typeof runSharkerTextCliCore;
}
...
export interface RuntimeHostRunContextDeps {
  createDriver(
    input: Parameters<typeof createRuntimeHostSharkerSessionDriver>[0],
...
export async function runRuntimeHostTextCli(
  ...
  options: RuntimeHostTextCliOptions = {
    cliCommand: 'sharker',
    clientDataRoot: resolveSharkerClientDataRoot(),
  },
): Promise<number> {
...
export function createRuntimeHostRunContext(
  ...
  const contextDeps = {
    createDriver: createRuntimeHostSharkerSessionDriver,
    ...overrides,
  };
...
async function prepareRuntimeHostRunInput(
  ...
  input: Parameters<SharkerRunDeps['createContext']>[0],
  cliCommand: string,
): Promise<Parameters<SharkerRunDeps['createContext']>[0]> {
...
class RuntimeHostRunRuntime implements SharkerRunRuntime {
  readonly #connection: RuntimeHostConnection;
  readonly #driver: RuntimeHostSharkerSessionDriver;
  readonly #observer: ((outcome: SharkerRunOutcome) => void | Promise<void>) | undefined;
  readonly #graphEnabled: boolean;
  readonly #sessionCwdOverride: SharkerRunContextInput['sessionCwdOverride'];
  ...
  constructor(
    connection: RuntimeHostConnection,
    driver: RuntimeHostSharkerSessionDriver,
    observer: ((outcome: SharkerRunOutcome) => void | Promise<void>) | undefined,
    graphEnabled: boolean,
    sessionCwdOverride: SharkerRunContextInput['sessionCwdOverride'],
  ...
  async *#observeTurn(
    turn: SharkerPreparedSessionTurn,
...
type TurnOutcomeObservation =
  ...
  | {
      ...
      readonly failure: NonNullable<SharkerRunOutcome['failure']>;
    }
  | {
...
class TurnOutcomeClassifier {
  ...
  outcome(incomplete: 'fail'): SharkerRunOutcome;
  outcome(incomplete: 'pending'): SharkerRunOutcome | undefined;
  outcome(incomplete: 'fail' | 'pending'): SharkerRunOutcome | undefined {
