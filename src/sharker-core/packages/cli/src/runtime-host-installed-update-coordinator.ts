...
import { type LocalHostDeploymentAuthorityOptions } from '@sharker/runtime-host/operator';
import {
...
export async function runRuntimeHostInstalledUpdateCoordinator(
  ...
  if (installation.observedRelease.version !== input.currentVersion) {
    throw new Error(
      `The installed Sharker release changed from ${input.currentVersion} to ${installation.observedRelease.version} before update coordination`,
    );
  }
  ...
  return deps.withArchive(input.target, input.archivePath, async ({ packageRoot, archivePath }) => {
    ...
    if (result.kind === 'completed') {
      process.stdout.write(`Updated Sharker to ${input.target.version}.\n`);
      return 0;
    }
...
async function finalizeInstalledPackage(
  ...
  if (
    ...
    throw new Error(
      'npm did not install the exact selected Sharker release into the same global slot',
    );
  }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once('close', (code) => {
      if (outputBytes > NPM_OUTPUT_MAX_BYTES) {
        reject(new Error('npm returned too much output while installing Sharker'));
      } else if (code !== 0) {
        reject(new Error('npm could not install the selected Sharker release'));
      } else resolve();
    });
  });
}
