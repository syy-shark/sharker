...
import { NO_REAL_CONNECTION_CODE } from '@sharker/core/connection-error-copy';
...
import { readLocalHostDeploymentRecord } from '@sharker/runtime-host/operator';
import { resolveSharkerClientDataRoot } from '@sharker/storage/workspace-root';
...
export async function connectRuntimeHostCli(
  ...
  const deps: RuntimeHostCliContextDeps = {
    ...
    executionCandidateEntrypoint: new URL(
      import.meta.resolve('@sharker/runtime-host/execution-candidate-main'),
    ),
  ...
  const clientInstanceId =
    profile.kind === 'local'
      ...
      : await deps.loadClientInstanceId(
          join(input.clientDataRoot ?? resolveSharkerClientDataRoot(), 'runtime-host-client.json'),
        );
...
async function resolveHostProfile(
  ...
  const root = input.clientDataRoot ?? resolveSharkerClientDataRoot();
