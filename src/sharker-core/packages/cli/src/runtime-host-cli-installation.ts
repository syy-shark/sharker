...
import type { RuntimeHostInstallationOwner } from '@sharker/runtime-host/operator';
import { isProductReleaseVersion } from '@sharker/runtime-host/operator/update-package-evidence';
...
export async function resolveRuntimeHostNpmGlobalInstallation(
  ...
  try {
    ...
    throw invalidInstallation('The Sharker CLI package root is unavailable', cause);
  }
  ...
  if (packageRoot !== join(globalRoot, PACKAGE_NAME)) {
    throw new RuntimeHostCliInstallationError(
      'unsupported_installation',
      'The current Sharker CLI is not installed in the active npm global prefix',
    );
  }
  return {
    owner: {
...
async function readPackageManifest(url: URL): Promise<{
  ...
  try {
    ...
    throw invalidInstallation('The Sharker CLI package manifest is unavailable', cause);
  }
  try {
    ...
    throw invalidInstallation('The Sharker CLI package manifest is invalid', cause);
  } finally {
...
async function canonicalCliPath(path: string, packageRoot: string): Promise<string> {
  try {
    ...
    throw invalidInstallation('The Sharker CLI entry point does not belong to its package', cause);
  }
}
