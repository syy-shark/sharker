...
import type { SessionEvent, ShellRunSnapshotResult, ShellRunUpdate } from '@sharker/core/events';
...
import type { CreateSessionInput, TurnOrchestration } from '@sharker/core/runtime-inputs';
...
export interface SharkerSessionMoveResult {
  previousCwd: string;
  cwd: string;
  changed: boolean;
  oldCwdDirty?: boolean;
...
export interface SharkerSessionSwitchOptions {
...
export interface SharkerSessionSwitchResult {
  summary: SessionSummary;
  messages: StoredMessage[];
  activeTurn?: SharkerPreparedSessionTurn;
  relocation?: SharkerSessionMoveResult;
}

export interface SharkerSessionRewindResult extends SharkerSessionSwitchResult {
  prompt: string;
}

export interface SharkerSideConversationOpenResult extends SharkerSessionSwitchResult {
  parentSessionId: string;
  sideSessionId: string;
}

export interface SharkerSideConversationCloseResult extends SharkerSessionSwitchResult {
  cleanup: 'removed' | 'pending';
}

export type SharkerSideConversationParentStatus =
  | 'needs_input'
  | 'needs_approval'
  | 'failed'
  | 'interrupted'
...
export interface SharkerPreparedSessionTurn {
  sessionId: string;
  turnId: string;
  runId?: string;
...
export interface SharkerAttachedSessionTurn extends SharkerPreparedSessionTurn {
  messages: StoredMessage[];
  summary: SessionSummary;
}

export interface SharkerPreparePromptOptions {
  turnId?: string;
  modelText?: string;
...
export interface SharkerSubmitMessageOptions {
  messageId: string;
...
export interface SharkerRetractedMessages {
  text: string;
 ...
 * `turn.start`'s only remaining caller is headless `sharker run`, which reports
...
export interface SharkerUserCommand {
...
export interface SharkerSessionDriver {
  ...
  runUserCommand?(command: string): Promise<SharkerUserCommand>;
  ...
  retractQueued?(): Promise<SharkerRetractedMessages>;
  ...
  moveSession?(cwd: string): Promise<SharkerSessionMoveResult>;
  switchSession(
    sessionId: string,
    options?: SharkerSessionSwitchOptions,
  ): Promise<SharkerSessionSwitchResult>;
  listRewindTargets(): Promise<RewindTarget[]>;
  rewindToTurn(turnId: string): Promise<SharkerSessionRewindResult>;
  openSideConversation?(): Promise<SharkerSideConversationOpenResult>;
...
export type SharkerTranscriptReplacementReason = 'reconcile' | 'reconnect';
