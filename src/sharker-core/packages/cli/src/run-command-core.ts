...
import { isThinkingLevel, type ThinkingLevel } from '@sharker/core/model-thinking';
import type { UserMessageInput } from '@sharker/core/runtime-inputs';
import type { ExecutionBoundaryReadModel } from '@sharker/core/sandbox-boundary';
import type { SessionSummary } from '@sharker/core/session';
import { normalizeUserSessionName } from '@sharker/core/session-name';
import type { CreateSessionRequest } from './session-driver.js';
import { selectSharkerRunSession } from './run-session-selection.js';
import { sessionEventSandboxBoundaryFailureReason } from './sandbox-boundary-failure.js';
import { resolveSharkerWorkspaceRoot } from './workspace-root.js';

export interface SharkerRunOptions {
  prompt?: string;
  stdinPrompt: boolean;
  cwd?: string;
  connection?: string;
...
export type ParseSharkerRunArgsResult =
  | { kind: 'run'; options: SharkerRunOptions }
  | { kind: 'help' }
  | { kind: 'error'; message: string };

export interface SharkerRunRuntime {
...
export interface SharkerRunContext {
  runtime: SharkerRunRuntime;
...
export interface SharkerRunOutcome {
  outcomeId: string;
...
export interface SharkerRunContextInput {
  ...
  runOutcomeObserver?: (outcome: SharkerRunOutcome) => void | Promise<void>;
  hostProfileId?: string;
  projectId?: string;
}

export interface SharkerRunDeps {
  createContext(input: SharkerRunContextInput): Promise<SharkerRunContext>;
...
export type SharkerRunAdapter = Pick<SharkerRunDeps, 'createContext' | 'listSessions'>;
export type SharkerRunEnvironmentDeps = Omit<SharkerRunDeps, keyof SharkerRunAdapter>;
...
export function parseSharkerRunArgs(argv: readonly string[]): ParseSharkerRunArgsResult {
  ...
  if (positional.length > 1) {
    return { kind: 'error', message: 'sharker run accepts at most one positional prompt' };
  }
...
export async function runSharkerTextCliCore(
  argv: readonly string[],
  adapter: SharkerRunAdapter,
  overrides: Partial<SharkerRunEnvironmentDeps> = {},
): Promise<number> {
  const deps: SharkerRunDeps = { ...defaultSharkerRunEnvironmentDeps(), ...adapter, ...overrides };
  const parsed = parseSharkerRunArgs(argv);
  if (parsed.kind === 'help') {
    deps.writeStdout(`${sharkerRunHelpText(deps.cliCommand())}\n`);
    return 0;
  }
  if (parsed.kind === 'error') {
    deps.writeStderr(`sharker run: ${parsed.message}\n\n${sharkerRunHelpText(deps.cliCommand())}\n`);
    return 2;
  }

  let prompt: string;
  let selection: Awaited<ReturnType<typeof selectSharkerRunSession>>;
  ...
  try {
    ...
    const sessions =
      parsed.options.resumeId !== undefined || parsed.options.continueLatest === true
        ...
        : [];
    selection = await selectSharkerRunSession(
      {
        sessions,
    ...
    deps.writeStderr(`sharker run: ${errorMessage(error)}\n`);
    return 2;
  }

  let outcome: SharkerRunOutcome | undefined;
  let unclassifiedBoundaryFailure = false;
  const boundaryFailureInvocationIds = new Set<string>();
  let context: SharkerRunContext;
  try {
  ...
  try {
    session =
      selection.kind === 'existing'
        ...
        : await context.runtime.createSession({
            cwd: selection.cwd,
            name: sharkerRunSessionName(prompt),
    ...
    if (selection.kind === 'existing') {
      ...
      if (parsed.options.yolo) {
        ...
        throw new Error(`cannot resume externally isolated session ${session.id} from sharker run`);
      }
    }
  } catch (error) {
    await context.close();
  ...
  try {
    ...
    for await (const event of context.runtime.sendMessage(session.id, {
      ...
      if (event.type === 'sandbox_boundary_request') {
        ...
        deps.writeStderr(
          'sharker run: sandbox boundary expansion is unavailable in non-interactive mode\n',
        );
      ...
      if (sandboxFailureReason) {
        ...
        deps.writeStderr(
          sandboxFailureReason === 'requires_bypass'
            ? 'sharker run: sandbox bypass requires an explicit --yolo\n'
            : 'sharker run: sandbox boundary expansion is unavailable in non-interactive mode\n',
        );
      }
    }
  ...
  if (timedOut) {
    deps.writeStderr(`sharker run: timed out after ${parsed.options.timeoutMs}ms\n`);
    return 1;
  }
  ...
  if (!outcome) {
    deps.writeStderr('sharker run: runtime produced no outcome\n');
    return 1;
  }
  if (outcome.status !== 'completed' || outcome.finalOutput === undefined) {
    ...
    deps.writeStderr(`sharker run: ${detail}\n`);
    return 1;
  }
...
async function resolveRunPrompt(options: SharkerRunOptions, deps: SharkerRunDeps): Promise<string> {
...
function sharkerRunHelpText(cliCommand: string): string {
  return [
...
function defaultSharkerRunEnvironmentDeps(): SharkerRunEnvironmentDeps {
  return {
    workspaceRoot: () => resolveSharkerWorkspaceRoot(),
    cliCommand: () => 'sharker',
...
function sharkerRunSessionName(prompt: string): string {
  const normalized = normalizeUserSessionName(firstLine(prompt).slice(0, 42));
  return normalized.ok ? normalized.value : 'Sharker run';
}
