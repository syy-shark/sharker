...
export function buildTaskLedgerTools(deps: { store: TaskLedgerStore }): SharkerTool[] {
  return [
...
): SharkerTool<{ tasks: Array<{ subject: string; parent_id?: string }> }, string> {
  return {
    name,
...
function buildTaskListTool(store: TaskLedgerStore): SharkerTool<
  {
    status?: (typeof TASK_STATUSES)[number];
...
function buildTaskGetTool(store: TaskLedgerStore): SharkerTool<{ id: string }, string> {
  return {
