...
export type CuDispatchOutcome =
  ...
  | {
       ...
       * belonging to the observed application — `sharker.cu/2` §1.2 makes that a
...
export type CuSemanticAction =
  ...
  | {
       ...
       * `sharker.cu/2` declares it (`{kind:"scroll", direction, pages}`) and
       * cua-driver advertises `scroll` among its element actions, so both
       * executors already speak it — this is the member that lets Sharker say it.
       */
      type: 'scroll_element';
      observationId: string;
      elementId: string;
...
/**
 * The host dispatch seam. Implemented in @sharker/computer-use by the sharker-cu
 * backend, which spawns the sharker-cu executor and speaks `sharker.cu/2` over stdio.
