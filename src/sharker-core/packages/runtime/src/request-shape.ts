 ...
 *
 * Segment order follows the stable Sharker request-prefix model used for cache
...
export function toolCatalogHash(tools: readonly SharkerTool[]): `sha256:${string}` {
  return stableHash(
    [...tools]
...
function toolShapeForDiagnostics(tool: SharkerTool): unknown {
  return {
    name: tool.name,
