...
import { projectAgentSwarmResult } from '@sharker/core/agent-swarm';
...
import { serializedByteLength } from '@sharker/core/serialized-byte-length';
import { encodeToolStepProgress, ToolOutcomeUnknownError } from '@sharker/core/events';
import type {
...
import type { ToolCallMessage, ToolResultMessage } from '@sharker/core/session';
import type {
  HostedInteractionBridge,
  HostedSandboxBoundarySettlement,
  HostedUserQuestionAnswer,
  HostedUserQuestionSettlement,
} from '@sharker/core/backend-types';
import type { PermissionMode, ToolCategory, ToolExecutionFacts } from '@sharker/core/permission';
...
} from '@sharker/core/runtime-event';
...
} from '@sharker/core/durable-tool-result-projection';
import {
...
export interface ResolvedSharkerToolCall {
  tool: SharkerTool;
  turnId: string;
  stepId?: string;
  toolCallId: string;
...
export interface SharkerTool<P = any, R = unknown> {
  ...
  permissionArgs?: (
    args: P,
    context: Pick<SharkerToolContext, 'sessionId' | 'turnId' | 'toolCallId'>,
  ) => unknown;
  /**
   ...
   */
  impl: (args: P, ctx: SharkerToolContext) => Promise<R> | R;
...
export interface SharkerToolContext {
  sessionId: string;
  runId?: string;
...
export class ToolRuntime {
  ...
  /**
   * Settle one resolved Sharker tool call. Tool/business failures resolve with a
   * provider-facing error output; durable runtime commit failures still reject.
   */
  async settleToolCall(call: ResolvedSharkerToolCall): Promise<ToolSettlement> {
  ...
  private async performToolSettlement(call: ResolvedSharkerToolCall): Promise<ToolSettlement> {
  ...
  private admitToolForStep(tool: SharkerTool, stepId: string | undefined): string | undefined {
  ...
  private reserveSubagentSlot(tool: SharkerTool): boolean {
  ...
  private releaseSubagentSlot(tool: SharkerTool): void {
  ...
  }): Pick<SharkerToolContext, 'spawnChildSession'> {
...
export function toolParameterFields(
  ...
  // whose `click_element` had a camelCase key was told `sharker_computer` takes
...
function buildTerminalFailureMessage(
  ...
  if (sandboxDenied) {
    ...
    parts.push(
      '该失败很可能来自 Sharker sandbox。请先尝试不扩大边界的替代方案；只有工具明确返回 sandbox_boundary_required 和具体 expansion 时，才能调用 request_sandbox_boundary 请求会话边界扩张，并在 expansion 里只写那一条路径。不要从命令文本猜测权限，也不要静默绕过 sandbox。',
    );
  }
  return parts.join('\n\n');
}
