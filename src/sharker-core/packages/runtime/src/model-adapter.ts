...
import type { ErrorEvent, CompleteEvent } from '@sharker/core/events';
...
import { lookupModelMetadata } from '@sharker/core/model-metadata';
...
export class ModelAdapter {
  ...
  runtimeEventReplaySupport(): ModelAdapterRuntimeEventReplaySupport {
    return {
      ...
      // assistant message (Sharker's provider-tool chronology) is still dropped,
  ...
  async startStream(input: ModelAdapterStreamInput): Promise<ModelStreamResult> {
    ...
    const sdkResult = streamText({
      ...
      maxRetries: 0,
      // Preserve the final request's Sharker-owned message projection without
  ...
  /**
   * Lower an AI SDK `streamText` result into the Sharker-owned `ModelStreamResult`.
   ...
   * are normalized to Sharker-owned contracts. No AI SDK type escapes this method.
   */
  ...
  /**
   * Translate one raw AI SDK stream chunk into zero or more Sharker-owned
   ...
   * testable through the Sharker-owned event contract.
   */
 ...
 * consumes it and emits the Sharker-owned `ModelStreamEvent`. It mirrors the AI
 * SDK chunk union just enough to read the fields Sharker cares about.
 */
          ...
          ...(responsesProviderOptions
            ...
            : openAiChatReasoningTransportState
              ? {
                  ...
                  providerOptionsOrigin: 'sharker_transport' as const,
                }
              : {}),
...
/**
 * OpenAI Responses reserves the provider name `tool_search`. Keep Sharker's
...
function compileProviderTool(
  tool: NonNullable<import('./tool-runtime.js').SharkerTool['providerTool']>,
): unknown {
  switch (tool.kind) {
...
/**
 * @deprecated alias for the Sharker-owned `NormalizedUsage` contract exported
