...
import {
  fiberStateName,
  type SharkerCompositionEntry,
  type SharkerCompositionEntryInspection,
  type SharkerCompositionApplyInput,
  type SharkerCompositionSnapshot,
  type SharkerPluginMetadata,
  type SharkerPluginPackage,
  type SharkerPluginRootId,
  SharkerPluginRuntimeError,
  SharkerPluginTransactionBuffer,
  type SharkerPluginTransaction,
...
interface LiveRoot {
  readonly id: SharkerPluginRootId;
  readonly context: Context;
  readonly entries: LiveEntry[];
}

export interface SharkerCompositionLoaderOptions {
  readonly root?: Context;
  readonly transaction?: (context: Context) => SharkerPluginTransaction | undefined;
}

export class SharkerCompositionLoader {
  readonly root: Context;
  readonly #packages = new Map<string, SharkerPluginPackage>();
  readonly #roots = new Map<SharkerPluginRootId, LiveRoot>();
  ...
  constructor(options: SharkerCompositionLoaderOptions = {}) {
    this.root = options.root ?? new Context();
    this.#transaction = options.transaction;
  }

  install(pkg: SharkerPluginPackage): Promise<void> {
  ...
  create(
    rootId: SharkerPluginRootId,
    entry: SharkerCompositionEntry,
    parentId?: string,
    position = Infinity,
  ): Promise<SharkerCompositionEntryInspection> {
    return this.apply({
  ...
  update(
    entryId: string,
    patch: Partial<Omit<SharkerCompositionEntry, 'id' | 'children'>>,
  ): Promise<SharkerCompositionEntryInspection> {
  ...
  enable(entryId: string): Promise<SharkerCompositionEntryInspection> {
    return this.update(entryId, { disabled: false });
  }

  disable(entryId: string): Promise<SharkerCompositionEntryInspection> {
    return this.update(entryId, { disabled: true });
  }

  apply(input: SharkerCompositionApplyInput): Promise<readonly SharkerCompositionEntryInspection[]> {
    return this.#mutate(async () => {
      if (
      ...
      const inspections: SharkerCompositionEntryInspection[] = [];
  ...
  inspectTree(rootId?: SharkerPluginRootId): readonly SharkerCompositionEntryInspection[] {
  ...
  inspect(entryId: string): SharkerCompositionEntryInspection {
  ...
  package(packageId: string): SharkerPluginPackage {
  ...
  snapshot(): SharkerCompositionSnapshot {
    const encode = (rootId: SharkerPluginRootId): readonly SharkerCompositionEntry[] =>
  ...
  replaceSnapshot(snapshot: SharkerCompositionSnapshot): Promise<void> {
  ...
  async #replaceSnapshot(
    ...
    if (snapshot.schemaVersion !== 1)
      throw new SharkerPluginRuntimeError('invalid_entry', 'Unsupported composition snapshot');
    ...
    const specs = new Map<SharkerPluginRootId, readonly SharkerCompositionEntry[]>([
      ['profile', snapshot.roots.profile],
      ['desktop-ui', snapshot.roots.desktopUi],
      ...Object.entries(snapshot.roots.sessions).map(
        ([id, entries]) => [`session:${id}` as SharkerPluginRootId, entries] as const,
      ),
    ]);
    const stagedRoots = new Map<SharkerPluginRootId, LiveRoot>();
    const stagedIds = new Set<string>();
    try {
      for (const [rootId, entries] of specs) {
        validatePluginRootId(rootId);
        const context = this.root.extend({ sharkerRootId: rootId });
  ...
  async #stage(
    ...
    let context = parentContext.extend({ sharkerEntryId: spec.id });
    ...
    if (!disabled && spec.packageId) {
      ...
      const metadata: SharkerPluginMetadata = Object.freeze({
        rootId,
        entryId: spec.id,
        packageId: spec.packageId,
        generation,
      });
      context = context.extend({ sharker: metadata });
      const transaction = this.#transaction?.(context) ?? new SharkerPluginTransactionBuffer(context);
      if (transaction) context = context.extend({ sharkerTransaction: transaction });
  ...
  async #commitSubtree(entry: LiveEntry): Promise<void> {
    await entry.context.sharkerTransaction?.commit();
  ...
  async #dispose(entry: LiveEntry): Promise<void> {
    ...
    try {
      await entry.context.sharkerTransaction?.rollback();
    } catch (error) {
      errors.push(error);
    }
    try {
  ...
  #root(rootId: SharkerPluginRootId): LiveRoot {
    validatePluginRootId(rootId);
    let root = this.#roots.get(rootId);
    if (!root) {
      root = { id: rootId, context: this.root.extend({ sharkerRootId: rootId }), entries: [] };
      this.#roots.set(rootId, root);
    }
    return root;
  }

  #inferRoot(parentId: string | undefined): SharkerPluginRootId {
  ...
  #assertUniqueSubtree(entry: SharkerCompositionEntry): void {
  ...
  #inspect(entry: LiveEntry): SharkerCompositionEntryInspection {
...
function entryPlugin(plugin: Plugin, inject: SharkerCompositionEntry['inject']): Plugin {
  const combined = mergeInject((plugin as Plugin.Base).inject, inject);
  return {
    name: (plugin as Plugin.Base).name ?? 'sharker-entry',
...
function mergeInject(
  ...
  right: SharkerCompositionEntry['inject'],
): Inject | undefined {
...
function freezePackage(pkg: SharkerPluginPackage): SharkerPluginPackage {
  return Object.freeze({ ...pkg, contributions: Object.freeze([...(pkg.contributions ?? [])]) });
}

function freezeEntry(entry: SharkerCompositionEntry): SharkerCompositionEntry {
  return Object.freeze({
    ...entry,
...
function serialize(entry: LiveEntry): SharkerCompositionEntry {
...
function* walk(entry: SharkerCompositionEntry): Generator<SharkerCompositionEntry> {
  yield entry;
