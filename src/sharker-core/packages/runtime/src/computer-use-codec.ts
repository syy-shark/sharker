...
import { z } from 'zod';
import { type CuAction, type CuPoint } from '@sharker/core/computer-use';
...
export function summarize(
  ...
  if (!outcome.ok) {
    ...
    // application text (`sharker.cu/2` §1.2 makes that a protocol rule). Absent
    ...
    return (
      `sharker_computer.${action.type} failed: ${outcome.error}${detail}${evidence}` +
  ...
  return (
    `sharker_computer.${action.type} ${verdict}${via} (verified=${verified})${evidence}${pointStr}${shot}` +
