...
import {
  ...
  type PolicyDecision,
  type ToolCategory,
} from '@sharker/core/permission';
import {
  SUBAGENT_PROFILES,
  type SubagentPreset,
  type SubagentProfile,
} from '@sharker/core/subagent-settings';
...
export function evaluateAgentDefinitionToolAccess(
  ...
  tool: Pick<SharkerTool, 'name' | 'categoryHint'>,
...
function resolveAgentDefinitionToolSet(
  tools: readonly SharkerTool[],
  definition: AgentRuntimeDefinition,
): { tools: SharkerTool[]; missingTools: string[] } {
...
function categoryForTool(tool: Pick<SharkerTool, 'name' | 'categoryHint'>): ToolCategory {
