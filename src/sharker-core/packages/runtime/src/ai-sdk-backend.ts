 ...
 *
 * Sharker owns the agent loop. Each ModelAdapter call performs exactly one
...
import { DEFAULT_TOOL_MODE, isToolMode, type ToolMode } from '@sharker/core/tool-mode';
import {
  resolveEffectiveOrchestration,
  type EffectiveOrchestration,
} from '@sharker/core/orchestration';
...
} from '@sharker/core/attachments';
import { stripUndefinedDeep } from '@sharker/core/tool-args-identity';
import { pricingModelKey } from '@sharker/core/usage-stats/pricing';
import type {
  LlmCallRecord,
  PricingConfig,
  ToolInvocationRecord,
} from '@sharker/core/usage-stats/types';
import type { ContextBudgetDiagnostic, PromptSegmentEstimate } from '@sharker/core/usage-stats/types';
import type {
  JSONValue,
  ModelFinishReason,
  ModelMessage,
...
import type { DurableToolResultProjection } from '@sharker/core/durable-tool-result-projection';
...
import type { ModelCallAttempt, ModelCallKind } from '@sharker/core/model-call-attempt';
import {
...
// ============================================================================
// AgentBackend interface — port contract now lives in @sharker/core/backend-types;
...
function nestableToolSnapshot(
  providerTools: readonly SharkerTool[],
  activeToolNames: readonly string[],
): ReadonlyMap<string, SharkerTool> {
...
async function validateCodeModeToolInput(tool: SharkerTool, input: unknown): Promise<unknown> {
...
class TurnScope {
  ...
  readonly activeTools = new Map<string, SharkerTool>();
  aborted = false;
  ...
  codeModeTools: ReadonlyMap<string, SharkerTool> | undefined;
            ...
            for await (const event of result.events) {
              ...
              if (event.kind === 'text-start') {
                ...
                if (event.providerOptions !== undefined) {
                  if (event.providerOptionsOrigin !== 'sharker_transport') {
                    attemptSawContinuationMetadata = true;
                  }
...
export function repairSharkerToolCall(input: {
  toolCall: RepairableAiSdkToolCall;
...
function buildInvalidSharkerTool(): SharkerTool<
