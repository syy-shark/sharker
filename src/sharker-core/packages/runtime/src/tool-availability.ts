...
import type { ToolAvailabilityDiagnostic } from '@sharker/core/usage-stats/types';
...
import type { SharkerTool, ToolGating } from './tool-runtime.js';

/** Canonical name of Sharker's provider-independent deferred-tool search connector. */
export const TOOL_SEARCH_NAME = 'tool_search';
/** Provider-safe alias used because OpenAI Responses reserves `tool_search`. */
export const TOOL_SEARCH_PROVIDER_NAME = 'sharker_tool_search';
...
export class ToolAvailabilityRuntime {
  private readonly tools: readonly SharkerTool[];
  private readonly toolsByName: ReadonlyMap<string, SharkerTool>;
  ...
  constructor(
    ...
    config: ToolAvailabilityConfig | undefined,
    private readonly invalidTool: SharkerTool,
  ) {
  ...
  prepare(
    activeTools: Map<string, SharkerTool>,
  ...
  private buildSearchConnector(
    activeTools: Map<string, SharkerTool>,
  ): SharkerTool<{ query: string; limit?: number }, ToolSearchResult> {
    return {
