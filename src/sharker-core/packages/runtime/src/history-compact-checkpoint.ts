...
import type { ExecutionLogCoverage } from '@sharker/core/execution-log-coverage';
...
export const HISTORY_COMPACT_SOURCE_POLICY_VERSION =
  'sharker.compactable_runtime_event_projection.v1' as const;
...
interface HistoryCompactCheckpointBase {
  kind: 'sharker.history_compact_checkpoint';
  checkpointId: string;
  sessionId: string;
  createdAt: number;
...
export function buildHistoryCompactCheckpoint(
  ...
  const common = {
    kind: 'sharker.history_compact_checkpoint' as const,
    checkpointId,
    sessionId: input.sessionId,
    createdAt,
    highWaterName,
...
export function renderHistoryCompactCheckpoint(checkpoint: TextHistoryCompactCheckpoint): string {
  return [
    `<sharker_history_compact_checkpoint id="${escapeAttribute(checkpoint.checkpointId)}" high_water="${escapeAttribute(checkpoint.highWaterName)}" seq="${checkpoint.highWaterSeq}" version="${checkpoint.version}">`,
...
export function validateHistoryCompactCheckpointShape(
  ...
  const through = coverage?.through as
    ...
    | undefined;
  return (
    checkpoint.kind === 'sharker.history_compact_checkpoint' &&
