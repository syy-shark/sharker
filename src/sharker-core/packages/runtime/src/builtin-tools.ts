...
import { compilePermissionProfile } from '@sharker/core/permission-profile-compiler';
import { parseAttachmentResourceRef } from '@sharker/core/attachments';
import { type SandboxBoundaryExpansion } from '@sharker/core/sandbox-boundary';
import { type StorageRef, type ToolResultContent } from '@sharker/core/events';
import { type PermissionProfile } from '@sharker/core/permission-profile';
...
import type { SharkerTool, SharkerToolContext } from './tool-runtime.js';
export type { SharkerTool, SharkerToolContext };
 ...
 * completed inside Sharker, so no matches were produced" reads as a search that
 ...
 * says Sharker cannot tell what happened to the file, and sends the model to look
...
function internalFilesystemReadFailure(tool: string, missing: string, notMeaning: string): Error {
  return new Error(
    `${tool} could not be completed inside Sharker, so ${missing}. ` +
      ...
      `work with a shell tool if you have one, and otherwise report that ${tool} is failing inside Sharker.`,
...
function internalFilesystemWriteFailure(tool: string, subject: string, extra?: string): Error {
  return new Error(
    `${tool} could not be completed inside Sharker. Sharker cannot tell whether ${subject}, ` +
...
export function buildBuiltinTools(options: BuildBuiltinToolsOptions = {}): SharkerTool[] {
  ...
  } satisfies SharkerTool;
  const tools: SharkerTool[] = [
    ...bashTools,
    ...backgroundTools,
    {
      name: 'Read',
...
function effectivePermissionProfile(
  explicitProfile: PermissionProfile | undefined,
  permissionMode: NonNullable<SharkerToolContext['permissionMode']>,
  cwd: string,
...
export function classifyRuntimeResourceRef(path: string): 'runtime' | 'file' | 'unsupported' {
  ...
  try {
    ...
    return path.trimStart().toLowerCase().startsWith('sharker:') ? 'unsupported' : 'file';
  }
  if (url.protocol !== 'sharker:') return 'file';
  if (
