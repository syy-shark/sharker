...
} from '@sharker/core/terminal-input';
...
import type { ToolExecutionFacts } from '@sharker/core/permission';
...
export function buildForegroundBashTool(options: BuildForegroundBashToolOptions): SharkerTool {
...
export function buildStopBackgroundTaskTool(backgroundTasks: BackgroundTaskStopper): SharkerTool {
  return {
    ...
    parameters: z.object({
      ref: z
        .string()
        .describe(
          'The runtime background task ref, for example sharker://runtime/background-tasks/<id>',
        ),
    }),
...
export function buildWriteStdinTool(ptyControls: PtyControlWriter): SharkerTool {
