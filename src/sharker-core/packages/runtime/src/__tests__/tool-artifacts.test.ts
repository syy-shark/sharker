...
describe('deriveToolArtifactCandidates', () => {
  test('Write derives a file-backed candidate from structured result path', () => {
    const [candidate] = deriveToolArtifactCandidates({
      ...
      result: { ok: true, path: '/workspace/sharker/docs/report.html', bytes: 15 },
    });
    ...
    expect(candidate).toEqual({
      ...
      sourcePath: '/workspace/sharker/docs/report.html',
    });
  });
  ...
  test('Edit derives a diff candidate from structured edit args', () => {
    const [candidate] = deriveToolArtifactCandidates({
      ...
      result: { ok: true, path: '/workspace/sharker/src/main.ts', replacements: 1 },
    });
  ...
  test('Bash derives only explicit stdout redirects and does not scan stdout/stderr text', () => {
    ...
    expect(candidate?.sourcePath).toBe('/workspace/sharker/reports/build.log');
    ...
    await recordToolArtifactsSafely(
      {
        ...
        result: { ok: true, path: '/workspace/sharker/secret.txt' },
      },
      async () => {
...
function writeArtifactTool(): SharkerTool {
  return {
    name: 'Write',
    ...
    impl: async (args) => {
      const path =
        typeof (args as { path?: unknown }).path === 'string'
          ...
          : 'notes.md';
      return { ok: true, path: `/workspace/sharker/${path}`, bytes: 5 };
    },
  };
}
