...
const requireLinuxSandboxSmoke = process.env.SHARKER_REQUIRE_LINUX_SANDBOX_SMOKE === '1';
const skipReason =
...
describe('Linux sandbox smoke', () => {
  ...
  test('workspace-write can write workspace files and blocks sibling paths', {
    ...
    const workspace = await mkdtemp(join(tmpdir(), 'sharker-linux-sandbox-workspace-'));
    const outside = await mkdtemp(join(tmpdir(), 'sharker-linux-sandbox-outside-'));
    ...
    const request = backend.transform({
      ...
      command: {
        ...
        args: [
          '-lc',
          `echo ok > inside.txt && echo temp-ok > /tmp/sharker-sandbox-temp.txt && ! echo nope > ${shellQuote(join(outside, 'outside.txt'))}`,
        ],
        cwd: workspace,
  ...
  test('workspace-write allows metadata writes inside the selected workspace', {
    ...
    if (!capability.available) return;
    const workspace = await mkdtemp(join(tmpdir(), 'sharker-linux-sandbox-metadata-'));
  ...
  test('missing exact-write creation fails closed when its pinned parent moves', {
    ...
    const root = await mkdtemp(join(tmpdir(), 'sharker-linux-parent-pin-'));
  ...
  test('shell-launched Node uses runtime roots and the seccomp socket filter', {
    ...
    if (!capability.available) return;
    const workspace = await mkdtemp(join(tmpdir(), 'sharker-linux-sandbox-network-'));
    const backend = new LinuxBubblewrapBackend({ capability });
    const request = backend.transform({
      platform: 'linux',
      command: {
  ...
  test('builtin Bash executes a tool from a nonstandard host PATH inside bubblewrap', {
    ...
    if (!capability.available) return;
    const workspace = await mkdtemp(join(tmpdir(), 'sharker-linux-sandbox-builtin-'));
    const toolRoot = await mkdtemp(join(homedir(), '.sharker-linux-sandbox-tool-'));
    const toolBin = join(toolRoot, 'bin');
    const toolPath = join(toolBin, 'sharker-path-probe');
    await mkdir(toolBin);
  ...
  test('builtin Bash enforces an expanded managed boundary and explicit bypass', {
    ...
    if (!capability.available) return;
    const workspace = await mkdtemp(join(tmpdir(), 'sharker-linux-bash-one-shot-workspace-'));
    const outside = await mkdtemp(join(homedir(), '.sharker-linux-bash-one-shot-outside-'));
