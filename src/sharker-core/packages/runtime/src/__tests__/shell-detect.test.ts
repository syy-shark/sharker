...
describe('buildShellSpawnPlan', () => {
  ...
  test('spawns PowerShell explicitly and preserves the command in an isolated script block', () => {
    ...
    const spawnPlan = buildShellSpawnPlan(
      ...
      command,
      { INHERITED: 'kept', __sharker_runtime_powershell_command: 'caller value' },
    );
    ...
    assert.match(script, /\[Console\]::OutputEncoding = \$__sharkerUtf8/u);
    assert.equal(spawnPlan.env?.INHERITED, 'kept');
    assert.ok(spawnPlan.env?.__SHARKER_RUNTIME_POWERSHELL_COMMAND?.startsWith(`${command}\n`));
    assert.equal(spawnPlan.env?.__sharker_runtime_powershell_command, undefined);
    assert.match(script, /SetEnvironmentVariable\('__SHARKER_RUNTIME_POWERSHELL_COMMAND', \$null\)/u);
    assert.doesNotMatch(
      script,
      /Write-Output/u,
  ...
  test('appends an exit-code wrapper so native command exit codes survive -Command', () => {
    ...
    assert.equal(
      spawnPlan.env?.__SHARKER_RUNTIME_POWERSHELL_COMMAND,
      'npm test\n' +
        '$__sharkerOk = $?\n' +
        'if (-not $__sharkerOk) { if ($LASTEXITCODE -is [int] -and $LASTEXITCODE -ne 0) { exit $LASTEXITCODE } else { exit 1 } }',
    );
    assert.equal(
      spawnPlan.args[4],
      '$__sharkerUtf8 = [System.Text.UTF8Encoding]::new($false)\n' +
        '[Console]::OutputEncoding = $__sharkerUtf8\n' +
        '$OutputEncoding = $__sharkerUtf8\n' +
        "$__sharkerCommandText = [Environment]::GetEnvironmentVariable('__SHARKER_RUNTIME_POWERSHELL_COMMAND')\n" +
        "[Environment]::SetEnvironmentVariable('__SHARKER_RUNTIME_POWERSHELL_COMMAND', $null)\n" +
        '$__sharkerCommand = [ScriptBlock]::Create($__sharkerCommandText)\n' +
        '. $__sharkerCommand',
    );
  });
...
describe('buildPtyShellSpawnPlan', () => {
  test('uses explicit argv for POSIX, cmd, and PowerShell PTYs', () => {
    ...
    assert.match(
      powershell.args[3] ?? '',
      /\$OutputEncoding = \$__sharkerUtf8\n\$__sharkerCommandText = \[Environment\]::GetEnvironmentVariable/u,
    );
    assert.equal(powershell.env?.INHERITED, 'kept');
    assert.ok(
      powershell.env?.__SHARKER_RUNTIME_POWERSHELL_COMMAND?.startsWith('Write-Output ready\n'),
    );
