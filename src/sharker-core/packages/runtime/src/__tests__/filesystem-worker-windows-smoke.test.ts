...
describe('Windows filesystem worker smoke', { skip: !enabled }, () => {
  ...
  before(async () => {
    workspace = await realpath(await mkdtemp(join(tmpdir(), 'sharker-win-worker-smoke-')));
    ...
    outside = await realpath(await mkdtemp(join(homedir(), '.sharker-win-worker-outside-')));
    // Use a realistic product-owned application directory with unrelated
    // sibling applications. The launch spec must grant only `Sharker`, never the
    // shared `Programs` parent.
    runtimeBase = await realpath(await mkdtemp(join(tmpdir(), 'sharker-win-worker-runtime-')));
    const runtimeBin = join(runtimeBase, 'Programs', 'Sharker');
