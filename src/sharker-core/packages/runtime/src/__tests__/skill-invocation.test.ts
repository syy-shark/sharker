...
import { SKILL_INVOCATION_NAME_MAX_BYTES } from '@sharker/core/skill-invocation';
...
// Tests pass an isolated homeDir so real user-level skills (~/.sharker, ~/.agents)
// on the dev machine never leak into discovery results.
async function withWorkspace(
  fn: (workspaceRoot: string, homeDir: string) => Promise<void>,
): Promise<void> {
  const workspaceRoot = await mkdtemp(join(tmpdir(), 'sharker-runtime-skill-invocation-'));
  const homeDir = await mkdtemp(join(tmpdir(), 'sharker-runtime-skill-invocation-home-'));
  try {
