...
import type { CreateSessionInput, SessionListFilter } from '@sharker/core/runtime-inputs';
import type { RuntimeEvent, RuntimeEventActions } from '@sharker/core/runtime-event';
...
describe('projectRuntimeEventsToStoredMessages', () => {
  test('exposes a session image ref as a Markdown image source to the model', () => {
    ...
    assert.match(
      ...
      /Markdown image source: "sharker:\/\/runtime\/attachments\/attachment-123"/,
    );
  });
  ...
  test('projects provider-native search through the canonical read model while replay keeps raw output', () => {
    ...
    const events = [
      ev({
        ...
        content: {
          kind: 'text',
          text: 'Sharker is current.',
          providerOptions: {
            openai: {
              itemId: 'message-1',
              annotations: [{ type: 'url_citation', url: 'https://sharker.example/' }],
            },
          },
        },
  ...
  test('replays generic provider tool results without Sharker result decoding', () => {
    const events = [
      ev({
  ...
  test('legacy archived tool-result placeholders gain a deterministic ArchiveRead ref for replay', () => {
    ...
    assert.equal(
      result?.kind === 'tool_result' && typeof result.output === 'object' && result.output !== null
        ...
        : undefined,
      `sharker://archive/artifact-tool-result/${'a'.repeat(64)}/800`,
    );
  });
      ...
      [
        ev({
          ...
          content: {
            ...
            signature: 'sig-1',
            providerOptions: { sharker: { kimiReasoningField: 'reasoning' } },
          },
          refs: { storedMessageId: 'legacy-assistant' },
        }),
        ev({
...
const ACTION_COVERAGE_SAMPLES: ActionCoverageSamples = {
  ...
  workspaceFact: {
    action: {
      kind: 'sharker.workspace.epoch_opened',
      version: 1,
      payload: {
