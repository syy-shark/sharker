...
function tool(name: string): SharkerTool {
  return {
    name,
...
async function toolNamesSeenByProvider(activeNames: ReadonlySet<string>): Promise<string[]> {
  const tools: SharkerTool[] = [tool('Read'), tool('tool_search'), tool('Rive')];
...
describe('ModelAdapter provider-step boundary', () => {
  test('uses a provider-safe alias for Sharker tool_search on OpenAI Responses', async () => {
