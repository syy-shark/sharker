...
import type { DeepResearchRun } from '@sharker/core/deep-research-run';
import { createSqliteDeepResearchStore } from '@sharker/storage/deep-research-store';
import {
...
function context(toolCallId: string): SharkerToolContext {
  return {
...
function findTool(tools: SharkerTool[], name: string): SharkerTool {
...
async function withTempRoot(fn: (root: string) => Promise<void>): Promise<void> {
  const root = await mkdtemp(join(tmpdir(), 'sharker-runtime-deep-research-'));
  try {
    await fn(root);
  } finally {
