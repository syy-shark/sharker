...
import { SharkerCompositionLoader } from '../plugin-composition-loader.js';
import {
...
test('composition tree supports nested groups and repeated package instances', async () => {
  ...
  const plugin = ((ctx: Context, config: { label: string }) => {
    activations.push(`${ctx.sharker!.entryId}:${config.label}`);
    ctx.effect(() => () => activations.push(`dispose:${ctx.sharker!.entryId}`), 'fixture');
  }) as Plugin;
  const loader = new SharkerCompositionLoader();
...
test('composition metadata wins over same-named root Services', async () => {
  ...
  root.provide('sharker', { hijacked: true });
  let seenEntryId: string | undefined;
  const loader = new SharkerCompositionLoader({ root });
  await loader.install(
    pkg('metadata-owner', (ctx: Context) => {
      seenEntryId = ctx.sharker?.entryId;
    }),
  );
  ...
  assert.deepEqual(root.get('sharker'), { hijacked: true });
  await loader.close();
});
...
test('duplicate package install is rejected without replacing live code', async () => {
  ...
  const current = (ctx: Context) => {
    live.add(ctx.sharker!.entryId);
    return () => live.delete(ctx.sharker!.entryId);
  };
...
test('disabled ancestors suppress insert, move, update, and subtree replacement activation', async () => {
  ...
  await loader.install(
    pkg('disabled-child', (ctx: Context, config: { readonly value: string }) => {
      activations.push(`${ctx.sharker!.entryId}:${config.value}`);
    }),
  );
...
test('contribution registrations are staged and owned by the entry Fiber', async () => {
  ...
  const loader = new SharkerCompositionLoader({
    root,
    transaction: (context) => new SharkerPluginTransactionBuffer(context),
  });
...
test('staging and commit failures do not retain newly created session roots', async () => {
  ...
  await loader.install(
    pkg('commit-root-failure', (ctx: Context) => {
      ctx.sharkerTransaction!.stage('failure', () => {
...
function pkg(packageId: string, host: Plugin): SharkerPluginPackage {
  return Object.freeze({ packageId, host });
}

function entry(id: string, packageId: string, config?: unknown): SharkerCompositionEntry {
