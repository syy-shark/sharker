...
`;
  ...
  it('resolves only the read-only machine source location contract', () => {
    assert.equal(
      ...
      join('/Users/ada', '.sharker', 'skill-sources'),
    );
  });
});
...
describe('shared skill preference semantics', () => {
  const inventory = [
    { ref: 'project:sharker:shared', id: 'shared' },
    { ref: 'user:sharker:shared', id: 'shared' },
  ...
  it('resolves case-only stable refs exactly while keeping bare ids normalized', () => {
    ...
    assert.deepEqual(resolveSkillPreferenceTarget(caseInventory, 'project:sharker:Shared'), {
      ok: true,
...
async function withTempRoot(fn: (root: string) => Promise<void>): Promise<void> {
  const root = await mkdtemp(join(tmpdir(), 'sharker-runtime-skill-governance-'));
  try {
    await fn(root);
  } finally {
