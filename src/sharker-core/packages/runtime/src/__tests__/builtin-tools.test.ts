...
import { z } from 'zod';
import { applySandboxBoundaryExpansion } from '@sharker/core/sandbox-boundary';
import { SHELL_RUN_ID_MAX_CHARS } from '@sharker/core/shell-run';
import {
  createDangerFullAccessPermissionProfile,
  createWorkspaceWritePermissionProfile,
  type PermissionProfile,
} from '@sharker/core/permission-profile';
...
describe('builtin apply_patch', () => {
  ...
  test('applies freeform V4A contents to real files', async (t) => {
    const cwd = await mkdtemp(join(tmpdir(), 'sharker-freeform-apply-patch-'));
  ...
  test('executor Bash executes with the same shell it declares', async () => {
    ...
    expect(
      result.output.stdout.startsWith(
        '-NoLogo -NoProfile -NonInteractive -Command $__sharkerUtf8 = [System.Text.UTF8Encoding]::new($false)\n',
      ),
    ).toBe(true);
  });
});
...
describe('builtin Bash streaming output', () => {
  ...
  test('explicit background Bash returns runtime refs and forwards its optional timeout', async () => {
    ...
    const shellRuns = {
      ...
      async runBackgroundBash(input: unknown) {
        ...
        return {
          kind: 'shell_run',
          ref: 'sharker://runtime/background-tasks/shell-run-1',
          mode: 'pty',
          status: 'running',
          cwd: '/workspace',
          cmd: 'sleep 60',
    ...
    expect((result as { ref?: string }).ref).toBe('sharker://runtime/background-tasks/shell-run-1');
  ...
  test('omits a missing subtree grant from an unrelated Linux Bash mount plan', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-linux-missing-subtree-'));
  ...
  test('does not let an unrelated stale exact-write grant block Bash', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-linux-stale-exact-'));
  ...
  test('applies the session boundary to the macOS sandbox argv without one-call permission arguments', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-bash-additional-'));
    try {
      const workspace = join(root, 'workspace');
  ...
  function windowsSandboxManager(): SandboxManager {
    ...
    return new SandboxManager([
      new WindowsBrokerSandboxBackend({
        clientPath: String.raw`C:\resources\windows-sandbox\sharker-windows-sandbox.exe`,
  ...
  test('fails Windows Bash closed when the profile requires a command sandbox the broker cannot enforce', async () => {
    ...
    const cwd = await realpath(await mkdtemp(join(tmpdir(), 'sharker-win-bash-closed-')));
    try {
      let called = false;
      const executor = fakeExecutor({
        exec: async () => {
  ...
  test('runs Windows Bash unsandboxed via the detected shell when the profile does not require a sandbox', async () => {
    ...
    const cwd = await realpath(await mkdtemp(join(tmpdir(), 'sharker-win-bash-open-')));
    try {
      let execInput: WorkspaceExecInput | undefined;
      const executor = fakeExecutor({
        exec: async (input) => {
  ...
  test('canonicalizes macOS Bash cwd and exposes the runtime executable roots', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-bash-runtime-roots-'));
    try {
      const workspace = join(root, 'workspace');
  ...
  test('Read treats runtime background task refs as whole resources', async () => {
    ...
    expect(
      (await parameters.validate({ ref: 'sharker://runtime/background-tasks/shell-run-1' })).success,
    ).toBe(true);
    ...
    await assert.rejects(
      async () => read.impl({ path: 'sharker://runtime/background-tasks/shell-run-1' }, context),
      /must be read with the ref parameter/,
    );
    const result = await read.impl({ ref: 'sharker://runtime/background-tasks/shell-run-1' }, context);
  ...
  test('Read routes opaque attachment refs through the Session-bound attachment reader', async () => {
    ...
    assert.deepEqual(await read.impl({ ref: 'sharker://runtime/attachments/attachment-1' }, context), {
      kind: 'text',
  ...
  test('Read ignores provider defaults beside a non-empty runtime ref', async () => {
    ...
    const ref = 'sharker://runtime/background-tasks/shell-run-1';
  ...
  test('preserves Bash failure contract when the executor reports non-zero exit', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'sharker-bash-executor-'));
    const bash = buildBuiltinTools({
      executor: fakeExecutor({
        exec: async () => ({
          exitCode: 4,
  ...
  test('emits stdout/stderr chunks before returning terminal result', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'sharker-bash-'));
...
describe('builtin Bash sandbox denial classification', () => {
  test('throws SandboxCommandError when a sandboxed command emits a denial message', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'sharker-bash-sandbox-denial-'));
    try {
      const executor = fakeExecutor({
        exec: async () => ({
          exitCode: 1,
...
describe('builtin read tools path containment', () => {
  test('Read rejects image content without snapshot support, regardless of extension', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-read-image-'));
  ...
  test('Read accepts contained absolute paths and rejects workspace escapes', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-read-root-'));
    const outside = await mkdtemp(join(tmpdir(), 'sharker-read-outside-'));
...
describe('builtin write tools path containment', () => {
  ...
  test('Edit rejects image results from the workspace executor', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-edit-image-'));
  ...
  test('Write accepts contained absolute paths and rejects workspace escapes', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-write-root-'));
    const outside = await mkdtemp(join(tmpdir(), 'sharker-write-outside-'));
    await mkdir(join(root, 'src'), { recursive: true });
    await symlink(outside, join(root, 'outside-link'));
  ...
  test('Edit accepts contained absolute paths and rejects workspace escapes', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-edit-root-'));
    const outside = await mkdtemp(join(tmpdir(), 'sharker-edit-outside-'));
    ...
    await runTool(
      edit,
      { path: join(root, 'inside.txt'), old_string: 'world', new_string: 'Sharker' },
      root,
    );
    expect(await readFile(join(root, 'inside.txt'), 'utf8')).toBe('hello Sharker');
  });

  test('Edit returns a file diff for a localized change in a large file', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-edit-large-diff-'));
  ...
  test('file tools stay usable when the session cwd is reached through a symlink', async () => {
    ...
    const base = await realpath(await mkdtemp(join(tmpdir(), 'sharker-symlink-cwd-')));
  ...
  test('concurrent Edits to the same file serialize — no lost update', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-edit-lock-'));
    const n = 20;
  ...
  test('concurrent Edits via different path spellings serialize on one key', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-edit-spelling-'));
    const n = 20;
    const markers = Array.from({ length: n }, (_, i) => `marker-${String(i).padStart(2, '0')}`);
    await writeFile(join(root, 'data.txt'), `${markers.join('\n')}\n`, 'utf8');
    const edit = tool('Edit');
  ...
  test('concurrent Edits through a symlinked cwd serialize on one key', async () => {
    const base = await realpath(await mkdtemp(join(tmpdir(), 'sharker-edit-symlink-')));
    const workspace = join(base, 'workspace');
  ...
  test('Write then Edit on one file resolves inside the lock — the fresh file is found', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-write-edit-'));
    const write = tool('Write');
    const edit = tool('Edit');
    ...
    await runTool(edit, { path: 'fresh.txt', old_string: 'world', new_string: 'Sharker' }, root);
    expect(await readFile(join(root, 'fresh.txt'), 'utf8')).toBe('hello Sharker\n');
  });

  test('a failing Edit releases the lock for the next op on the same file', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-edit-wedge-'));
    ...
    await runTool(edit, { path: 'data.txt', old_string: 'world', new_string: 'Sharker' }, root);
    expect(await readFile(join(root, 'data.txt'), 'utf8')).toBe('hello Sharker\n');
  });
});
...
describe('builtin FormatJson (file in place)', () => {
  ...
  test('rejects image results from the workspace executor', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-formatjson-image-'));
  ...
  test('sort_keys: true preserves __proto__ as a data property', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-formatjson-'));
...
async function linuxMissingExactWriteFixture() {
  const root = await mkdtemp(join(tmpdir(), 'sharker-linux-exact-write-'));
  const workspace = join(root, 'workspace');
  const outside = join(root, 'outside');
  await Promise.all([mkdir(workspace), mkdir(outside)]);
  const canonicalWorkspace = await realpath(workspace);
