...
test('writes broker manifests to exclusive per-process temporary files', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-windows-manifest-test-'));
...
test('transforms a Windows managed profile into a broker-client invocation', () => {
  ...
  const backend = new WindowsBrokerSandboxBackend({
    clientPath: String.raw`C:\Program Files\Sharker\sharker-windows-sandbox.exe`,
    nonce: () => 'b'.repeat(32),
    requestId: () => 'request-1',
  ...
  assert.deepEqual(result.exec.argv, [
    String.raw`C:\Program Files\Sharker\sharker-windows-sandbox.exe`,
    '--broker-local',
  ...
  const launch = {
    ...
    environment: { SHARKER_WINDOWS_SANDBOX: '1', SystemRoot: String.raw`C:\Windows` },
