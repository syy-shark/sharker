...
describe('workspace instructions prompt fragment', () => {
  it('renders global instructions before project instructions', async () => {
    await withWorkspaceAndHome(async ({ workspaceRoot, homeDir }) => {
      const sharkerDir = join(homeDir, '.sharker');
      await mkdir(sharkerDir, { recursive: true });
      await writeFile(join(sharkerDir, 'AGENTS.md'), 'GLOBAL_RULE\n', 'utf8');
  ...
  it('skips symlink escapes from allowlisted instruction filenames', async () => {
    const outsideRoot = await mkdtemp(join(tmpdir(), 'sharker-instructions-outside-'));
    await withWorkspaceAndHome(async ({ workspaceRoot, homeDir }) => {
  ...
  it('skips global symlink escapes from ~/.sharker instruction filenames', async () => {
    const outsideRoot = await mkdtemp(join(tmpdir(), 'sharker-global-instructions-outside-'));
    await withWorkspaceAndHome(async ({ workspaceRoot, homeDir }) => {
      const sharkerDir = join(homeDir, '.sharker');
      await mkdir(sharkerDir, { recursive: true });
      await writeFile(join(outsideRoot, 'AGENTS.md'), 'global outside secret', 'utf8');
      await symlink(join(outsideRoot, 'AGENTS.md'), join(sharkerDir, 'AGENTS.md'));
  ...
  it('keeps identical instructions that live in different scopes', async () => {
    ...
    await withWorkspaceAndHome(async ({ workspaceRoot, homeDir }) => {
      const sharkerDir = join(homeDir, '.sharker');
      await mkdir(sharkerDir, { recursive: true });
      await writeFile(join(sharkerDir, 'AGENTS.md'), 'SAME_TEXT\n', 'utf8');
...
async function withWorkspaceAndHome(
  ...
  const workspaceRoot = await mkdtemp(join(tmpdir(), 'sharker-workspace-instructions-'));
  const homeDir = await mkdtemp(join(tmpdir(), 'sharker-workspace-instructions-home-'));
  try {
