...
describe('active current-turn tool-result pruning', () => {
  ...
  test('request projection composes active tools with rewritten messages', async () => {
    ...
    assert.match(JSON.stringify(result.messages), /sharker\.active_archived_tool_result/);
  });
  ...
  test('oversized eligible current-turn tool result is archived and replaced', async () => {
    ...
    assert.match(secondPrompt, /sharker\.active_archived_tool_result/);
    assert.match(secondPrompt, /artifact-tool-1/);
    assert.equal(secondPrompt.includes('sharker://archive/'), true);
  ...
  test('archive failure keeps the original tool result', async () => {
    ...
    assert.doesNotMatch(JSON.stringify(rewritten.messages), /sharker\.active_archived_tool_result/);
  });
  ...
  test('tool activation still works when active pruning shares the projection pipeline', async () => {
    const runtime = new ToolAvailabilityRuntime(
      [sharkerTool('Read'), sharkerTool('RiveWorkflow')],
      { groups: [{ id: 'rive', toolNames: ['RiveWorkflow'] }] },
      sharkerTool('invalid'),
    );
    const active = new Map<string, SharkerTool>();
...
function invalidActivePlaceholder(): Record<string, unknown> {
  return {
    kind: 'sharker.active_archived_tool_result',
    rewriteVersion: 1,
    artifactId: '',
    turnId: 'turn-1',
    toolCallId: 'tool-old',
...
function sharkerTool(name: string): SharkerTool {
  return {
    name,
