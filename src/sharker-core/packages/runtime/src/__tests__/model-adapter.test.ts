...
describe('ModelAdapter stream and error normalization', () => {
  ...
  test('translates provider text, reasoning, tool calls, and errors into ModelStreamEvents', () => {
    ...
    // cross the adapter as Sharker-owned events.
    assert.deepEqual(
  ...
  test('marks OpenAI Chat field metadata as a Sharker transport hint', () => {
    const adapter = new ModelAdapter({
      connection: {
        slug: 'deepseek',
        providerType: 'deepseek',
    ...
    assert.deepEqual(adapter.translateChunk({ type: 'reasoning-delta', text: 'think' } as Chunk), [
      {
        kind: 'thinking',
        text: 'think',
        providerOptions: { sharker: { openAiChatReasoningField: 'reasoning_content' } },
  ...
  test('returns a Sharker-owned tool call event from one provider step', () => {
    const adapter = newAdapter();
    type Chunk = Parameters<typeof adapter.translateChunk>[0];
    const chunk = {
      type: 'tool-call',
  ...
  test('reduces AI SDK 7 step boundaries to Sharker-owned step-finish events', () => {
    const adapter = newAdapter();
    type Chunk = Parameters<typeof adapter.translateChunk>[0];
