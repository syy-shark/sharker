...
import type { SandboxBoundaryRequestEvent } from '@sharker/core/events';
...
import { ToolRuntime, type SharkerTool, type ToolRuntimeInput } from '../tool-runtime.js';
...
describe('ToolRuntime session sandbox boundary', () => {
  ...
  test('canonicalizes filesystem authority before creating durable pending state', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-boundary-request-'));
  ...
  test('rejects exact directory authority before creating durable pending state', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-boundary-directory-'));
    let createCalls = 0;
    const runtime = new ToolRuntime({
      turnId: 'turn-1',
      sessionId: 'session-1',
  ...
  test('counts one boundary correction per model step and keeps failure kinds independent', async () => {
    ...
    let callNumber = 0;
    const fail = async (tool: SharkerTool, stepId: string): Promise<void> => {
      callNumber += 1;
  ...
  test('a surface without boundary storage refuses with the same sentence the tool carries', async () => {
    ...
    const events: SessionEvent[] = [];
    const settlement = await runtime.settleToolCall({
      tool: buildRequestSandboxBoundaryTool() as unknown as SharkerTool,
      turnId: 'turn-1',
...
async function settle(runtime: ToolRuntime, tool: SharkerTool, toolCallId: string): Promise<void> {
  const events: SessionEvent[] = [];
  await runtime.settleToolCall({
    tool,
    turnId: 'turn-1',
