...
import type { AgentRunHeader } from '@sharker/core/agent-run';
import type { AttachmentByteReader } from '@sharker/core/attachments';
import type { BackendSendInput } from '@sharker/core/backend-types';
...
import { createManagedExecutionBoundary } from '@sharker/core/sandbox-boundary';
import type { SessionHeader } from '@sharker/core/session';
import type { StorageRef } from '@sharker/core/events';
import { encodeCanonicalRuntimeEvent } from '@sharker/core/canonical-runtime-event';
...
import type { AssistantMessage, StoredMessage, ToolResultMessage } from '@sharker/core/session';
import { z } from 'zod';
import {
  AiSdkBackend,
  INVALID_TOOL_NAME,
  ...
  normalizeAiSdkUsage,
  repairSharkerToolCall,
  type AiSdkBackendInput,
  type RunTraceEvent,
} from '../ai-sdk-backend.js';
import type { DurableSessionEventSink, SharkerTool, ToolRuntime } from '../tool-runtime.js';
...
import { decodeModelCallAttempt, type ModelCallAttempt } from '@sharker/core/model-call-attempt';
...
describe('AiSdkBackend Memory Extraction triggers', () => {
  ...
  test('exposes explicitly unsupported Memory triggers on the native OpenAI Responses lane', async () => {
      ...
      doStream: async () => {
        ...
        return {
          stream: simulateReadableStream({
            chunks: (modelCalls === 1
              ? memorySearchChunks('sharker_tool_search')
              : [
                  { type: 'stream-start', warnings: [] },
                  {
                    type: 'finish',
    ...
    assert.ok(stepZeroToolNames.includes('sharker_tool_search'));
...
describe('AiSdkBackend model history', () => {
  ...
  test('replays provider-executed CC web search with encrypted result content intact', async () => {
    ...
    assert.deepEqual(call?.input, { query: 'latest Sharker' });
  ...
  test('replays provider-executed web search before its grounded assistant text', async () => {
        ...
        runtimeContext: [
          ...
          runtimeEvent({
            ...
            content: {
              ...
              id: 'search-1',
              name: 'WebSearch',
              result: { type: 'web_search_result', query: 'latest Sharker' },
          ...
          runtimeEvent({
            ...
            content: { kind: 'text', text: 'Sharker shipped the feature.' },
          }),
        ],
      }),
    );
    ...
    assert.match(JSON.stringify(assistant), /Sharker shipped the feature/);
  });
  ...
  test('falls back to grounded text when Open Responses cannot replay a hosted tool pair', async () => {
    ...
    const prompt = compactPrompt(model) as Array<{ role: string; content: unknown }>;
    assert.match(JSON.stringify(prompt), /Sharker shipped the feature/);
  ...
  test('keeps unrelated client tool history when degrading a hosted tool pair', async () => {
    ...
    assert.equal(wire.includes('latest Sharker'), false, wire);
    ...
    assert.match(wire, /Sharker shipped the feature/);
  });
  ...
  test('archives stale RuntimeEvent tool results before replay placeholder rewrite', async () => {
    ...
    assert.match(prompt, /"kind":"sharker\.archived_tool_result"/);
  ...
  test('after-step stop preserves the current provider step usage and prevents another step', async () => {
    ...
    const stoppingTool: SharkerTool = {
      name: 'Read',
...
describe('AiSdkBackend usage telemetry', () => {
  ...
  test('a pruned tool result is readable again through the tool its placeholder names', async () => {
            ...
            : // The newest completed step is never pruned, so a second call is
              ...
              : streamCalls === 3
                ? call('tool-3', 'ArchiveRead', {
                    ...
                    ref: /sharker:\/\/archive\/[^"\\]+/.exec(JSON.stringify(prompt))?.[0] ?? 'missing',
                    operation: 'read',
                  })
                : [
                    { type: 'stream-start', warnings: [] },
    ...
    assert.match(thirdPrompt, /sharker:\/\/archive\//);
    assert.match(
  ...
  test('records active tool-result prune diagnostics in usage telemetry', async () => {
    ...
    assert.doesNotMatch(secondPrompt, /sharker\.active_archived_tool_result/);
    const thirdPrompt = JSON.stringify(prompts[2]);
...
describe('AiSdkBackend tool-call repair', () => {
  test('repairs provider tool-name case drift to the canonical Sharker tool name', () => {
    const repaired = repairSharkerToolCall({
      toolCall: {
        toolCallId: 'tool-1',
        toolName: 'bash',
        input: '{"command":"pwd"}',
...
describe('AiSdkBackend thinking persistence', () => {
  ...
  test('Alibaba Responses keeps multiple streamed reasoning items distinct through replay', async () => {
    ...
    assert.deepEqual(
      thinkingCompletes.map((event) => [
        event.text,
        (event.providerOptions?.sharkerResponses as { itemId?: unknown } | undefined)?.itemId,
      ]),
      [
  ...
  test('Alibaba Responses fails when streamed reasoning differs from the final summary', async (t) => {
    ...
    assert.equal(events.find((event) => event.type === 'complete')?.stopReason, 'error');
    assert.equal(JSON.stringify(appended).includes('sharkerResponses'), false);
  ...
  test('Alibaba Responses keeps a finalized item valid when the next item id is unsafe', async () => {
    ...
    assert.deepEqual(
      parts?.map((part) => [
        part.text,
        (part.providerOptions?.sharkerResponses as { itemId?: unknown } | undefined)?.itemId,
      ]),
      [
  ...
  test('does not synthesize assistant text when a stream ends without a trailing finish-step', async () => {
    ...
    (
      ...
      // The adapter boundary now exposes Sharker-owned `ModelStreamEvent`s, not
  ...
  test('continues a reasoning tool step from Sharker durable replay instead of SDK response shape', async () => {
...
describe('AiSdkBackend steering durability and identity', () => {
  ...
  test('executes native WebSearch inside the primary provider stream', async () => {
        ...
        stream: simulateReadableStream({
          chunks: [
            ...
            },
            {
              ...
              toolName: 'WebSearch',
              result: {
                action: { type: 'search', queries: ['latest Sharker'] },
                sources: [{ type: 'url', url: 'https://sharker.example/' }],
              },
              providerExecuted: true,
            },
            { type: 'text-start', id: 'text-1' },
            { type: 'text-delta', id: 'text-1', delta: 'Sharker is current.' },
            {
              type: 'text-end',
              id: 'text-1',
              providerMetadata: {
    ...
    assert.deepEqual(result?.type === 'tool_result' ? result.content : undefined, {
      kind: 'web_search',
      provider: 'model',
      query: 'latest Sharker',
      rows: [
        {
          title: 'sharker.example',
          url: 'https://sharker.example/',
          snippet: '',
          source: 'sharker.example',
        },
      ],
    });
    assert.equal(
        ...
        stream: simulateReadableStream({
          chunks: [
            { type: 'stream-start', warnings: [] },
            {
              ...
              toolName: 'WebSearch',
              input: JSON.stringify({ query: 'latest Sharker' }),
              providerExecuted: true,
            },
            {
              type: 'tool-result',
            ...
            { type: 'text-delta', id: 'text-cc-1', delta: 'Sharker is current.' },
...
function testTool(name: string, parameters: unknown): SharkerTool {
  return {
    name,
...
function nativeApplyPatchTool(): SharkerTool {
  return {
    name: 'apply_patch',
