...
const CRASH_CHILD_ENV = 'SHARKER_RUNTIME_CONTINUATION_CRASH_CHILD';
...
if (process.env[CRASH_CHILD_ENV] === '1') {
  ...
  describe('runtime resume phase 1 process crash harness', () => {
    test('reopens and repairs every committed continuation prefix after SIGKILL', {
      ...
      const root = await mkdtemp(join(tmpdir(), 'sharker-runtime-continuation-crash-'));
      try {
...
async function runCrashChild(): Promise<void> {
  const workspaceRoot = requiredEnv('SHARKER_RUNTIME_CONTINUATION_WORKSPACE');
  const failpoint = requiredEnv(
    'SHARKER_RUNTIME_CONTINUATION_FAILPOINT',
...
function createCrashRuntimeStore(workspaceRoot: string) {
  return createSqliteRuntimeStore(join(workspaceRoot, '.sharker', 'runtime.sqlite'));
}
...
async function crashContinuationAt(
  ...
  const child = spawn(process.execPath, [fileURLToPath(import.meta.url)], {
    ...
    env: {
      ...process.env,
      [CRASH_CHILD_ENV]: '1',
      SHARKER_RUNTIME_CONTINUATION_WORKSPACE: workspaceRoot,
      SHARKER_RUNTIME_CONTINUATION_FAILPOINT: failpoint,
    },
