...
import { DEEP_RESEARCH_SESSION_LABEL } from '@sharker/core/deep-research';
import { RUNTIME_CONTINUATION_AUTHORITY_V1 } from '@sharker/core/runtime-event-store';
import { deriveTurnRecords } from '@sharker/core/session';
...
import { buildImmutableRuntimePrefix, decodeContinuationClaim } from '@sharker/core/runtime-boundary';
import type {
...
import type { ExecutionBoundary } from '@sharker/core/sandbox-boundary';
...
import type { ContinuationClaimV1, RuntimeBoundaryDigest } from '@sharker/core/runtime-boundary';
import type {
...
import type { SessionHeader, SessionSummary, StoredMessage, TurnRecord } from '@sharker/core/session';
import type { BackendSendInput, BackendStopMode } from '@sharker/core/backend-types';
import { PlanConflictError, emptyPlanSessionState, type PlanStore } from '@sharker/core/plan';
...
import type { AgentBackend } from '@sharker/core/backend-types';
...
describe('SessionManager graph operator provisioning', () => {
  ...
  test('binds implementation operators to a durable project worktree', async () => {
    ...
    const binding = {
      ...
      branch: `sharker/subagent/${'a'.repeat(32)}`,
      ...
      worktreeChildExecutor: {
        ...
        provision: async (input) => {
          ...
          return {
            ...
            branch: `sharker/subagent/${input.leaseId.slice('subagent_worktree_'.length)}`,
          };
        },
        ensure: async () => {},
        capturePatch: async () => new Uint8Array(),
    ...
    expect(result.header.subagentWorkspace?.branch).toMatch(/^sharker\/subagent\//);
  ...
  test('recovers only the latest unpublished implementation patch idempotently', async () => {
    ...
    const binding = {
      ...
      branch: `sharker/subagent/${'d'.repeat(32)}`,
...
function testTool(name: string): SharkerTool {
  return {
    name,
