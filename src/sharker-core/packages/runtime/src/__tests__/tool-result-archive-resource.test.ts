...
describe('tool-result archive resources', () => {
  test('round-trips a first-class archive URI with integrity metadata', () => {
    ...
    assert.match(ref, /^sharker:\/\/archive\//);
    ...
    assert.equal(parseToolResultArchiveResourceRef('sharker://runtime/background-tasks/1'), null);
    assert.equal(parseToolResultArchiveResourceRef(`${ref}?unexpected=true`), null);
    assert.equal(
      parseToolResultArchiveResourceRef(`sharker://archive/%E0%A4%A/${'0'.repeat(64)}/1`),
      null,
    );
  });
