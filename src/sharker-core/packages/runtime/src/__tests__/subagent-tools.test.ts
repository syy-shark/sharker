...
import type { Task, TaskAgentOutcome, TaskLedgerStore, TaskOwner } from '@sharker/core/task-ledger';
...
import { ToolRuntime, type SharkerTool } from '../tool-runtime.js';
...
describe('subagent tools', () => {
  ...
  test('agent_spawn advertises task_id only when task binding is available', async () => {
    const advertisedProperties = async (tool: SharkerTool) => {
  ...
  test('child agent toolset enforces explore-mode read-only behavior without prompting', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'sharker-child-tools-'));
    try {
...
function testCatalogTool(name: string, categoryHint: SharkerTool['categoryHint']): SharkerTool {
  return {
    name,
    description: name,
    parameters: {},
...
function implementationCatalogTools(): SharkerTool[] {
