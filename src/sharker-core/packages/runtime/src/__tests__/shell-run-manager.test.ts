...
import { type ShellRunUpdate, type ToolResultContent } from '@sharker/core/events';
import { createSqliteShellRunStore } from '@sharker/storage/shell-run-store';
...
describe('ShellRunProcessManager', () => {
  test('rejects a model Read of a user-owned resource while preserving client inspection', async () => {
    ...
    const ref = 'sharker://runtime/background-tasks/user-command';
  ...
  test('preserves CJK PowerShell output through pipes', {
    ...
    const result = await manager.runForegroundBash(
      shellInput({
        cwd: await workspace(),
        command:
          "if (Test-Path Env:__SHARKER_RUNTIME_POWERSHELL_COMMAND) { throw 'internal command env leaked' }; New-Item -ItemType File -Path '中文文件名.txt' | Out-Null; Get-ChildItem -Name",
        shell,
      }),
    );
  ...
  test('preserves the requested cwd through a real Git Bash login PTY', {
    ...
    const cwd = await workspace();
    await writeFile(join(cwd, 'sharker-cwd-marker'), 'expected workspace', 'utf8');
    const manager = await createTestManager();
    ...
    await manager.writeStdin({
      sessionId: 'session-1',
      ref: initial.ref,
      input: "test -f sharker-cwd-marker && printf 'SHARKER_CWD_OK\\n'\r",
      abortSignal: NO_ABORT,
    });
    const observed = await waitForPtyText(manager, initial.ref, /SHARKER_CWD_OK/u, 10_000);
    assert.equal(observed.output?.mode, 'pty');
    if (observed.output?.mode !== 'pty') throw new Error('expected pty output');
    assert.match(terminalText(observed.output), /SHARKER_CWD_OK/u);
  ...
  test('uses the shared explicit PowerShell pipe plan', async () => {
    ...
    assert.equal(result.output.mode, 'pipes');
    if (result.output.mode !== 'pipes') throw new Error('expected pipes output');
    assert.match(result.output.stdout, /\$OutputEncoding = \$__sharkerUtf8/u);
    assert.match(result.output.stdout, /\$__sharkerCommand = \[ScriptBlock\]::Create/u);
  ...
  test('recovers durable starting and running records without live handles as orphaned', async () => {
    ...
    const detail = await manager.readRuntimeResource(
      'session-1',
      'sharker://runtime/background-tasks/orphan-starting',
      NO_ABORT,
    );
  ...
  test('concurrent orphan observers converge on the same durable terminal record', async () => {
    ...
    const manager = createManager(store);
    const ref = 'sharker://runtime/background-tasks/concurrent-orphan';
  ...
  test('keeps unauthorized refs non-disclosing and rejects malformed selectors before storage', async () => {
    ...
    const manager = createManager(store);
    const refs = [
      'sharker://runtime/background-tasks/unknown',
      'sharker://runtime/background-tasks/owned-by-another-session',
    ];
    ...
    await assert.rejects(
      () =>
        manager.readRuntimeResource(
          'session-1',
          'sharker://runtime/background-tasks/%2Funsafe',
          NO_ABORT,
        ),
      ...
      (error: unknown) =>
        error instanceof Error &&
        /sharker:\/\/runtime\/background-tasks\/<id>/.test(error.message) &&
  ...
  test('preserves the caller environment through a PowerShell PTY', {
    ...
    const shell = windowsPowerShellPlan();
    assert.ok(shell, 'Windows PowerShell 5.1 must exist on the Windows baseline runner');
    const marker = 'sharker-pty-caller-env';
    const previousHostOnly = process.env.SHARKER_PTY_HOST_ONLY;
    process.env.SHARKER_PTY_HOST_ONLY = 'must-not-leak';
    try {
      const manager = await createTestManager();
      const initial = await manager.runBackgroundBash(
        shellInput({
          cwd: await workspace(),
          command:
            'Write-Output "marker=$env:SHARKER_PTY_CALLER_MARKER"; Write-Output "host=$env:SHARKER_PTY_HOST_ONLY"',
          pty: true,
          shell,
          env: { SHARKER_PTY_CALLER_MARKER: marker },
        }),
      );
      const result = await waitForTerminalShellRun(manager, initial.ref);
      ...
      if (previousHostOnly === undefined) delete process.env.SHARKER_PTY_HOST_ONLY;
      else process.env.SHARKER_PTY_HOST_ONLY = previousHostOnly;
    }
  });
...
async function workspace(): Promise<string> {
  const path = await mkdtemp(join(tmpdir(), 'sharker-shell-run-'));
