...
async function withFakeLauncher<T>(run: (clientPath: string) => Promise<T> | T): Promise<T> {
  const dir = await mkdtemp(join(tmpdir(), 'sharker-windows-readiness-'));
  const clientPath = join(dir, 'sharker-windows-sandbox.exe');
...
describe('createBuiltinSandboxManager', () => {
  ...
  it('registers Windows only when the packaged native client exists', async () => {
    const resourcesPath = await mkdtemp(join(tmpdir(), 'sharker-windows-resources-'));
    try {
      await mkdir(join(resourcesPath, 'windows-sandbox'));
      await writeFile(join(resourcesPath, 'windows-sandbox', 'sharker-windows-sandbox.exe'), 'test');
...
describe('probeWindowsReadiness', () => {
  ...
  it('fails closed when the launcher file does not exist without spawning', () => {
    ...
    const missing = join(tmpdir(), 'sharker-windows-readiness-missing', 'sharker-windows-sandbox.exe');
