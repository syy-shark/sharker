...
import { createGenesisExecutionBoundary } from '@sharker/core/sandbox-boundary';
import { type LlmConnection } from '@sharker/core/llm-connections';
import { type SessionEvent } from '@sharker/core/events';
import { type SessionHeader, type StoredMessage } from '@sharker/core/session';
import type { McpToolBinding } from '@sharker/core/mcp';
import {
  createSqliteRuntimeStore,
  type SqliteRuntimeStoreFailpoint,
} from '@sharker/storage/sqlite-runtime-store';
import {
...
import { MAX_ACTIVE_SUBAGENT_TOOLS_PER_TURN, ToolRuntime, type SharkerTool } from '../tool-runtime.js';
...
describe('ToolRuntime with real SQLite boundary', () => {
  it('replays raw MCP model arguments without persisting the execution binding', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-tool-sqlite-mcp-args-'));
  ...
  it('persists a boundary-blocked Client Capability without an orphan response', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-tool-sqlite-client-capability-reject-'));
    const store = createSqliteRuntimeStore(join(root, 'runtime.sqlite'));
    try {
  ...
  it('persists a subagent admission rejection without an orphan response', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-tool-sqlite-subagent-limit-'));
    const store = createSqliteRuntimeStore(join(root, 'runtime.sqlite'));
  ...
  it('persists a preflight-rejected sibling beside an exclusive tool without an orphan response', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-tool-sqlite-exclusive-reject-'));
    const store = createSqliteRuntimeStore(join(root, 'runtime.sqlite'));
    try {
      const runtime = createTestToolRuntime({
        sessionId: 'session-1',
  ...
  it('persists one atomic prepared/outcome pair around the real implementation', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-tool-sqlite-'));
    const store = createSqliteRuntimeStore(join(root, 'runtime.sqlite'));
    try {
      let implementationCalls = 0;
      const runtime = createTestToolRuntime({
  ...
  it('does not repeat tool or projection side effects after an atomic T2 failure', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-tool-sqlite-t2-retry-'));
  ...
  it('persists the same normalized error event that the Runtime flow later observes', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-tool-sqlite-error-'));
    const store = createSqliteRuntimeStore(join(root, 'runtime.sqlite'));
    try {
      const runtime = createTestToolRuntime({
        sessionId: 'session-1',
