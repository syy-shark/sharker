...
async function makeDirs(): Promise<{ cwd: string; outside: string; cleanup: () => Promise<void> }> {
  const root = await realpath(await mkdtemp(join(tmpdir(), 'sharker-fs-authority-')));
...
function toolsFor(overrides: Parameters<typeof buildBuiltinTools>[0] = {}): SharkerTool[] {
  return buildBuiltinTools({ executor: createLocalWorkspaceExecutor(), ...overrides });
}

function toolNamed(tools: SharkerTool[], name: string): SharkerTool {
