...
import type { AgentRunHeader, AgentRunStore, EmittedAgentRunEvent } from '@sharker/core/agent-run';
import type { RuntimeEvent } from '@sharker/core/runtime-event';
import type { RuntimeEventStore } from '@sharker/core/runtime-event-store';
...
import { decodeModelCallAttempt } from '@sharker/core/model-call-attempt';
import { isSessionInlineRun } from '@sharker/core/agent-run';
import { canonicalToolArgsHash } from '@sharker/core/tool-args-identity';
import { createSqliteAgentRunStore } from '@sharker/storage/agent-run-store';
import { createWorkspaceRuntimeStore } from '@sharker/storage/runtime-event-persistence';
...
test('Side Conversation snapshots preserve ordinary archived tool results', () => {
  const message: Extract<StoredMessage, { readonly type: 'tool_result' }> = {
    ...
    content: {
      kind: 'json',
      value: {
        kind: 'sharker.archived_tool_result',
        rewriteVersion: 1,
...
test('conversation copy rejects a retained AgentRun without RuntimeEvent facts', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-conversation-missing-runtime-copy-'));
  try {
...
test('conversation copy rewrites a complete tool recovery bundle atomically', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-conversation-recovery-copy-'));
  const runStore = createSqliteAgentRunStore(root);
  ...
  try {
    ...
    const sourceEvents: RuntimeEvent[] = [
      ...
      runtimeEvent({
        ...
        actions: {
          toolRecovery: {
            kind: 'sharker.tool.reconcile_result',
            version: 1,
            payload: {
      ...
      runtimeEvent({
        ...
        actions: {
          toolRecovery: {
            kind: 'sharker.tool.recovery_decision',
            version: 1,
            payload: {
    ...
    const reconcile = targetEvents.find(
      (event) => event.actions?.toolRecovery?.kind === 'sharker.tool.reconcile_result',
    )?.actions?.toolRecovery;
    const decisionEvent = targetEvents.find(
      (event) => event.actions?.toolRecovery?.kind === 'sharker.tool.recovery_decision',
    );
    ...
    assert.equal(decision?.kind, 'sharker.tool.recovery_decision');
    if (decision?.kind !== 'sharker.tool.recovery_decision') {
...
test('conversation copy rewrites the parent operation id of a nested Code Mode call', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-conversation-copy-parent-op-'));
  const runStore = createSqliteAgentRunStore(root);
  const runtimeEventStore = createSqliteRuntimeStore(join(root, 'runtime.sqlite'));
  try {
    await runStore.ready?.();
...
test('conversation copy validates operational events before persisting target ledgers', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-conversation-copy-preflight-'));
  try {
    const runStore = createSqliteAgentRunStore(root);
    const runtimeEventStore = createWorkspaceRuntimeStore(root);
    await runStore.createRun(
...
test('conversation copy rewrites the nested identity of a model call attempt', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-conversation-model-call-copy-'));
  try {
    const runStore = createSqliteAgentRunStore(root);
    const runtimeEventStore = createWorkspaceRuntimeStore(root);
    await runStore.createRun(
...
test('conversation copy repairs a model call attempt stranded by a pre-fix copy', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-conversation-model-call-legacy-'));
  try {
    const runStore = createSqliteAgentRunStore(root);
    const runtimeEventStore = createWorkspaceRuntimeStore(root);
    await runStore.createRun(
...
test('conversation copy clones one terminal Runtime ledger with new owned identities', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-conversation-runtime-copy-'));
  try {
    const runStore = createSqliteAgentRunStore(root);
    const runtimeEventStore = createWorkspaceRuntimeStore(root);
    ...
    const sourceAttachmentText = [
      '![chart](sharker://runtime/attachments/artifact-source)',
      'sharker://runtime/attachments/artifact-source?session=other',
    ].join('\n');
    const targetAttachmentText = [
      '![chart](sharker://runtime/attachments/artifact-target)',
      'sharker://runtime/attachments/artifact-source?session=other',
    ].join('\n');
    const sourceEvents: RuntimeEvent[] = [
      runtimeEvent({
...
test('conversation copy rebuilds an inline checkpoint without legacy child events in its prefix', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-conversation-checkpoint-copy-'));
  try {
    const runStore = createSqliteAgentRunStore(root);
    const runtimeEventStore = createWorkspaceRuntimeStore(root);
...
test('conversation copy rebuilds a resumed child checkpoint over its child run chain', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-conversation-child-checkpoint-copy-'));
  try {
    const runStore = createSqliteAgentRunStore(root);
    const runtimeEventStore = createWorkspaceRuntimeStore(root);
    const rootRun = agentRunHeader({
