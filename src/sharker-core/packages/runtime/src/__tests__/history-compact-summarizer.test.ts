 ...
 *
 * Run: `npm --workspace @sharker/runtime run test`
 */
import { MockLanguageModelV4 } from 'ai/test';
import { describe, test } from 'node:test';
import assert from 'node:assert/strict';
import { expect } from '../test-helpers.js';
import type { RuntimeEvent, RuntimeEventContent } from '@sharker/core/runtime-event';
  ...
  test('produces schema-valid tool-result messages (toolName + wrapped output) and does not fall back', async () => {
    ...
    const events: RuntimeEvent[] = [
      ...
      ev({
        role: 'tool',
        author: 'tool',
        content: { kind: 'function_response', id: 'fc1', name: 'read', result: { name: 'sharker' } },
      }),
    ...
    expect(toolPart.output).toEqual({ type: 'json', value: { name: 'sharker' } });
  });
