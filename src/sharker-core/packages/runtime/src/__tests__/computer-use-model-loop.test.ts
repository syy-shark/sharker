...
describe('AiSdkBackend Computer Use model loop', () => {
  test('the safe function-tool contract is identical across target provider connections', async () => {
    for (const providerType of [
      ...
      assert.match(serialized, /sharker_computer/);
    }
  });
  ...
  test('a model discovers, observes, mutates semantically, reads the fresh frame, and completes', async () => {
    ...
    assert.deepEqual(
      ...
      ['sharker_computer', 'sharker_computer', 'sharker_computer'],
    );
    assert.equal(telemetry.length, 3);
    assert.equal(
      telemetry.every((record) => record.toolName === 'sharker_computer'),
      true,
    );
    ...
    assert.equal(
      (modelTools[0] as Array<{ name?: string }>).some((tool) => tool.name === 'sharker_computer'),
      true,
    );
  });
