...
describe('macOS filesystem worker smoke', { skip: process.platform !== 'darwin' }, () => {
  ...
  before(async () => {
    workspace = await realpath(await mkdtemp(join(tmpdir(), 'sharker-worker-smoke-workspace-')));
    outside = await realpath(await mkdtemp(join(homedir(), '.sharker-worker-smoke-outside-')));
