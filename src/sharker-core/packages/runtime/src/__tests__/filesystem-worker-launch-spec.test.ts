...
test('Windows packaged worker grants only its product-owned application directory', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-windows-launch-spec-'));
  try {
    const applicationDirectory = join(root, 'Programs', 'Sharker');
    await mkdir(applicationDirectory, { recursive: true });
    const executable = join(applicationDirectory, 'Sharker.exe');
