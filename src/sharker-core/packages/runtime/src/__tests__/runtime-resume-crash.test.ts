...
const CRASH_CHILD_ENV = 'SHARKER_RUNTIME_RESUME_CRASH_CHILD';
...
if (process.env[CRASH_CHILD_ENV] === '1') {
  ...
  describe('runtime resume phase 0 process crash harness', () => {
    test('reopens every fully committed P0-P11 ledger prefix after SIGKILL', {
      ...
      const root = await mkdtemp(join(tmpdir(), 'sharker-runtime-resume-crash-'));
      try {
...
async function runCrashChild(): Promise<void> {
  const workspaceRoot = requiredEnv('SHARKER_RUNTIME_RESUME_WORKSPACE');
  const sessionId = requiredEnv('SHARKER_RUNTIME_RESUME_SESSION');
  const runId = requiredEnv('SHARKER_RUNTIME_RESUME_RUN');
  const markerPath = requiredEnv('SHARKER_RUNTIME_RESUME_FINALLY_MARKER');
  const events = JSON.parse(
    Buffer.from(requiredEnv('SHARKER_RUNTIME_RESUME_EVENTS'), 'base64').toString('utf8'),
  ) as RuntimeEvent[];
...
async function crashWriterAfterCommit(input: {
  ...
  const child = spawn(process.execPath, [fileURLToPath(import.meta.url)], {
    ...
    env: {
      ...process.env,
      [CRASH_CHILD_ENV]: '1',
      SHARKER_RUNTIME_RESUME_WORKSPACE: input.workspaceRoot,
      SHARKER_RUNTIME_RESUME_SESSION: input.sessionId,
      SHARKER_RUNTIME_RESUME_RUN: input.runId,
      SHARKER_RUNTIME_RESUME_FINALLY_MARKER: input.markerPath,
      SHARKER_RUNTIME_RESUME_EVENTS: Buffer.from(JSON.stringify(input.events), 'utf8').toString(
        'base64',
      ),
    },
