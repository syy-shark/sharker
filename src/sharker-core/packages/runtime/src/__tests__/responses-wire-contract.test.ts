...
import { z } from 'zod';
import { modelMetadataIdsForProvider } from '@sharker/core/model-metadata';
import { PROVIDER_REGISTRY } from '@sharker/core/llm-connections';
import { thinkingVariantsForModel } from '@sharker/core/model-thinking';
...
describe('responses wire contract', () => {
  test('does not route Sharker tool_search history through OpenAI native tool_search validation', async () => {
    ...
    const result = await adapter.startStream({
      ...
      tools: {
        tool_search: {
          description: 'Search Sharker deferred tools',
          inputSchema: z.object({ query: z.string() }),
        },
      },
      activeTools: ['tool_search'],
      system: 'Call tool_search; keep existing sharker_tool_search text unchanged.',
      onStreamActivity: () => {},
    ...
    assert.equal(
      ...
      'Call sharker_tool_search; keep existing sharker_tool_search text unchanged.',
    );
  });
  ...
  test('keeps provider-executed tool history free of dangling outputs', async () => {
    ...
    const messages = openAiCodexCompactionMessages([
      ...
      event(
        ...
        'model',
        'agent',
        { kind: 'text', text: 'Sharker shipped.' },
          ...
          content: [
            {
              ...
              toolCallId: 'call-read',
              toolName: 'Read',
              output: { type: 'text', value: '{"name":"sharker"}' },
            },
          ],
        },
      ],
