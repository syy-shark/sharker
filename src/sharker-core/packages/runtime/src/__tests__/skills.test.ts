...
import type { SharkerToolContext } from '../tool-runtime.js';
...
${'x'.repeat(MAX_SKILL_TOOL_BODY_CHARS + 1)}`);
    ...
    assert.deepEqual(result.manifest.metadata, { author: 'sharker' });
    assert.deepEqual(
...
# Malformed`,
  ...
  it('reports deterministic duplicate id and display-name diagnostics', async () => {
    await withWorkspace(async (workspaceRoot) => {
      const projectRoot = await mkdtemp(join(tmpdir(), 'sharker-runtime-skill-duplicates-'));
      try {
...
Open local targets carefully.`,
      ...
      await mkdir(join(workspaceRoot, '.sharker'), { recursive: true });
      await writeFile(join(workspaceRoot, '.sharker', 'skills-state.json'), '{not json', 'utf8');
  ...
  it('writeSkillRuntimeState does not write through a symlinked workspace metadata directory', async () => {
    await withWorkspace(async (workspaceRoot) => {
      const outside = await mkdtemp(join(tmpdir(), 'sharker-skill-state-outside-'));
      try {
        await writeSkill(
          workspaceRoot,
          'browser-helper',
...
Open local targets carefully.`,
        ...
        await symlink(outside, join(workspaceRoot, '.sharker'));
  ...
  it('reports blocked discovery roots instead of collapsing them into an empty catalog', async () => {
    await withWorkspace(async (workspaceRoot) => {
      const outside = await mkdtemp(join(tmpdir(), 'sharker-skill-discovery-outside-'));
  ...
  it('diagnoses unusable symlinked skill directories without hiding valid skills', async () => {
    await withWorkspace(async (workspaceRoot) => {
      const outside = await mkdtemp(join(tmpdir(), 'sharker-skill-entry-outside-'));
      const skillsDir = join(workspaceRoot, 'skills');
...
# Valid`,
  ...
  it('readSkillRuntimeState does not read through a symlinked state file', async () => {
    await withWorkspace(async (workspaceRoot) => {
      const outside = await mkdtemp(join(tmpdir(), 'sharker-skill-state-file-outside-'));
      try {
        await writeSkill(
          workspaceRoot,
          'browser-helper',
...
Open local targets carefully.`,
        ...
        await symlink(externalState, join(workspaceRoot, '.sharker', 'skills-state.json'));
...
Use host tools.`,
      ...
      const result = await tool.impl({ name: 'gated-helper' }, {} as unknown as SharkerToolContext);
...
Use host tools.`,
      ...
      } as unknown as SharkerToolContext);
      assert.equal(hidden.ok, false);
...
Plain work.`,
  ...
  it('scanSkills discovers skills from multiple directories and dedupes by id (first-found wins)', async () => {
    await withWorkspace(async (workspaceRoot) => {
      const projectRoot = await mkdtemp(join(tmpdir(), 'sharker-runtime-skills-proj-'));
      try {
...
Body.`,
  ...
  it('resolveSkillDiscoveryPaths returns the five standard paths with containment roots in precedence order', () => {
    ...
    assert.deepEqual(entries, [
      { dir: '/repo/.sharker/skills', containmentRoot: '/repo', scope: 'project', source: 'sharker' },
  ...
  it('keeps a scope-aware inventory while retaining only the highest-precedence duplicate', async () => {
    await withWorkspace(async (workspaceRoot) => {
      ...
      await writeSkillInDirectory(
        join(projectRoot, '.sharker', 'skills'),
        'writer',
        'Project Writer',
      ...
      assert.equal(scan.skills[0].source, 'sharker');
      assert.equal(scan.skills[0].ref, 'project:sharker:writer');
  ...
  it('uses one canonical inventory for prompt, Skill, and SkillSearch without exposing shadowed duplicates', async () => {
    await withWorkspace(async (workspaceRoot) => {
      ...
      const context = {
        ...
        turnId: 'turn-1',
        cwd: projectRoot,
      } as SharkerToolContext;

      const loaded = await skillTool.impl({ name: 'writer' }, context);
      assert.equal(loaded.ok, true);
      if (!loaded.ok) return;
      assert.equal(loaded.skill.ref, 'project:sharker:writer');
  ...
  it('re-evaluates zero-match schema v2 bare preferences after multi-scope discovery changes', async () => {
    await withWorkspace(async (workspaceRoot) => {
      ...
      await mkdir(join(workspaceRoot, '.sharker'), { recursive: true });
      await writeFile(
        join(workspaceRoot, '.sharker', 'skills-state.json'),
        JSON.stringify({
          schemaVersion: 2,
...
async function withWorkspace(fn: (workspaceRoot: string) => Promise<void>): Promise<void> {
  const workspaceRoot = await mkdtemp(join(tmpdir(), 'sharker-runtime-skills-'));
  try {
