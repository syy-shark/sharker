...
import { z } from 'zod';
import type { SessionEvent, ToolResultContent } from '@sharker/core/events';
...
function makeTool(name: string, impl: string[]): SharkerTool {
  return {
    name,
    description: name,
...
function makeFailingTool(name: string, impl: string[], message = 'boom'): SharkerTool {
  return {
    name,
    description: name,
    parameters: z.object({}).passthrough(),
...
// failures.
function makeTerminalTool(name: string, impl: string[], exitCode: number): SharkerTool {
  return {
    name,
    description: name,
    parameters: z.object({}).passthrough(),
...
function makeShellRunTool(name: string, impl: string[], status: ObservedShellRunStatus): SharkerTool {
  return {
    name,
    description: name,
    parameters: z.object({}).passthrough(),
...
function makeComputerFailureTool(impl: string[], failureClass?: 'ambiguous_target'): SharkerTool {
  return {
    ...
    impl: (args) => {
      ...
      return {
        text: 'sharker_computer failed: stale_frame',
        error: 'stale_frame',
...
describe('loop-gate for repeated identical FAILING tool calls', () => {
  ...
  test('returned shell_run terminal states are observations, not tool failures', async () => {
    for (const status of observedShellRunStatuses) {
      ...
      const args = { ref: 'sharker://runtime/background-tasks/shell-run-1' };
