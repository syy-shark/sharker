...
describe('createRequestCustomizationFetch', () => {
  test('applies generic headers and extra fields to a JSON POST request', async () => {
    ...
    const fetch = createRequestCustomizationFetch(
      ...
      {
        headers: { 'HTTP-Referer': 'https://sharker.example', 'X-Title': 'Sharker' },
    ...
    assert.ok(request);
    assert.equal(request.headers.get('http-referer'), 'https://sharker.example');
    assert.equal(request.headers.get('x-title'), 'Sharker');
