...
import { type AgentGraphScheduleUpdate } from '@sharker/core/agent-graph-schedule';
...
import { createSessionStore, isSessionNotFoundError } from '@sharker/storage/session-store';
import { createSqliteSessionMetadataStore } from '@sharker/storage/sqlite-session-metadata-store';
...
describe('host-managed agent graph coordinator', () => {
  test('authorizes only selected committed results from an earlier epoch of the same root', async () => {
    ...
    try {
      ...
      const view = tools.find((tool) => tool.name === VIEW_AGENT_GRAPH_TOOL_NAME) as SharkerTool<
        Record<string, never>,
        {
      ...
      >;
      const update = tools.find((tool) => tool.name === UPDATE_AGENT_GRAPH_TOOL_NAME) as SharkerTool<
  ...
  test('boots an empty graph from agent work and recovers it without duplicate topology', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-graph-coordinator-'));
  ...
  test('advances only a finished and quiescent graph to a deterministic next epoch', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-graph-epoch-cutover-'));
  ...
  test('persists work-keyed reconciliation failures across coordinator recovery', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-graph-failure-recovery-'));
    const sessionStore = createSessionStore(root);
    const runStore = createSqliteAgentRunStore(root);
    const runtimeEventStore = createWorkspaceRuntimeStore(root);
    const controlStore = createSqliteSessionMetadataStore(
...
function localReadTools(): SharkerTool[] {
