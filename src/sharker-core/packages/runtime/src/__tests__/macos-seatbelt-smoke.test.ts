...
async function makeWorkspace(): Promise<string> {
  return realpath(await mkdtemp(join(tmpdir(), 'sharker-seatbelt-workspace-')));
}
...
describe('macOS Seatbelt smoke', { skip: !canRunSeatbelt }, () => {
  ...
  it('allows ancestor directory reads without exposing ancestor file contents', async () => {
    const ancestorRoot = await realpath(await mkdtemp(join(tmpdir(), 'sharker-seatbelt-ancestors-')));
  ...
  it('allows temp writes when workspace and temp roots use symlinked paths', async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'sharker-seatbelt-temp-workspace-'));
    const slashTmpFile = join('/tmp', `sharker-seatbelt-slash-tmp-${process.pid}-${Date.now()}`);
    cleanup.push(workspaceRoot, slashTmpFile);

    const child = runSeatbeltCommand(
      workspaceRoot,
      `created=$(/usr/bin/mktemp -d "$TMPDIR/sharker-seatbelt.XXXXXX") && /usr/bin/touch ${JSON.stringify(slashTmpFile)} && /bin/rm -rf "$created" ${JSON.stringify(slashTmpFile)}`,
      createWorkspaceWritePermissionProfile(),
      true,
    );
  ...
  it('denies writes outside the workspace root', async () => {
    const workspaceRoot = await makeWorkspace();
    const outsideRoot = await realpath(await mkdtemp(join(tmpdir(), 'sharker-seatbelt-outside-')));
  ...
  it('allows only the exact outside path in the expanded session boundary', async () => {
    const workspaceRoot = await makeWorkspace();
    const outsideRoot = await realpath(await mkdtemp(join(tmpdir(), 'sharker-seatbelt-additional-')));
    cleanup.push(workspaceRoot, outsideRoot);
