...
import { ToolRuntime, formatDeferredNotLoadedText, type SharkerTool } from '../tool-runtime.js';
...
function tool(name: string, implCalls: string[]): SharkerTool {
  return {
    name,
    description: name,
    parameters: z.object({}),
...
function run(h: Harness, t: SharkerTool, args: unknown = {}) {
  return h.runtime
    .settleToolCall({
      tool: t,
      turnId: 'turn-1',
...
describe('tool-availability execute-boundary guard', () => {
  ...
  test('keeps WriteStdin args exact across canonical ledgers and projects telemetry', async () => {
    ...
    const args = {
      ref: 'sharker://runtime/background-tasks/pty-1',
