...
import { applySandboxBoundaryExpansion, type ExecutionBoundary } from '@sharker/core/sandbox-boundary';
...
describe('Linux filesystem worker smoke', { skip }, () => {
  ...
  before(async () => {
    workspace = await realpath(await mkdtemp(join(tmpdir(), 'sharker-linux-worker-workspace-')));
    outside = await realpath(await mkdtemp(join(homedir(), '.sharker-linux-worker-outside-')));
