...
import { TERMINAL_RUNTIME_EVENT_STATUSES, type RuntimeEvent } from '@sharker/core/runtime-event';
import {
 ...
 * against fixtures and keeps `@sharker/storage` out of `@sharker/runtime`.
 *
...
function projectEventSteps(events: readonly RuntimeEvent[]): TraceStep[] {
  ...
  for (const event of events) {
    ...
    if (recovery?.kind === 'sharker.tool.recovery_decision') {
