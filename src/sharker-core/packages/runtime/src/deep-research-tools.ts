...
} from '@sharker/core/deep-research-run';
...
export function buildDeepResearchTools(deps: BuildDeepResearchToolsDeps): SharkerTool[] {
  return [
    buildStartTool(deps),
...
export function isDeepResearchToolAllowed(tool: Pick<SharkerTool, 'name'>): boolean {
  return DEEP_RESEARCH_ALLOWED_TOOL_NAMES.has(tool.name);
}

function buildStartTool(deps: BuildDeepResearchToolsDeps): SharkerTool<
  {
    objective: string;
...
function buildReadArtifactTool(deps: BuildDeepResearchToolsDeps): SharkerTool<
  {
    artifact_id: string;
    offset_chars?: number;
    max_chars?: number;
...
function buildSaveArtifactTool(deps: BuildDeepResearchToolsDeps): SharkerTool<
  {
...
function buildUpdateChecklistTool(deps: BuildDeepResearchToolsDeps): SharkerTool<
  {
    item_id: string;
...
function buildRecordStepTool(deps: BuildDeepResearchToolsDeps): SharkerTool<
  {
...
function buildCheckpointTool(deps: BuildDeepResearchToolsDeps): SharkerTool<
  {
    round: number;
...
): SharkerTool<Record<string, never>, string> {
  return {
...
function buildCompleteTool(deps: BuildDeepResearchToolsDeps): SharkerTool<
  {
...
function mutationContext(ctx: SharkerToolContext): {
  runId?: string;
  turnId: string;
  toolCallId: string;
} {
...
function stableArtifactId(ctx: SharkerToolContext): string {
