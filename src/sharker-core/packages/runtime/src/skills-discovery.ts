...
export type SkillDiscoverySource = 'sharker' | 'agents' | 'legacy' | 'custom';
...
export type SkillSourceResolver = (
  context: Pick<SharkerToolContext, 'sessionId' | 'cwd'>,
) => SkillSource;
...
export function resolveSkillDiscoveryPaths(
  ...
  const entries: SkillDiscoveryEntry[] = [
    { dir: join(cwd, '.sharker', 'skills'), containmentRoot: cwd, scope: 'project', source: 'sharker' },
    {
    ...
    { dir: join(home, '.sharker', 'skills'), containmentRoot: home, scope: 'user', source: 'sharker' },
    {
