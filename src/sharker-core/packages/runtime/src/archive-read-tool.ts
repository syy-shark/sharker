...
export function buildArchiveReadTool(reader: ToolResultArchiveResourceReader): SharkerTool {
  const parameters = z.preprocess(
    ...
    z
      .object({
        ref: z
          .string()
          .describe('A sharker://archive/... ref returned in an archived tool-result placeholder'),
        operation: z
  ...
  return {
    ...
    activityKind: 'read',
    description:
      'Inspect or page through a tool-result archive returned as a sharker://archive/... ref. Start with inspect. For agent_swarm archives, query one itemId at a time. Results are strictly bounded so reading an archive cannot immediately trigger another archive.',
