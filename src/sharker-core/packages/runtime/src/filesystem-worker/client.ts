...
import { canReadPath, canWritePath, type PermissionProfile } from '@sharker/core/permission-profile';

import { compilePermissionProfile } from '@sharker/core/permission-profile-compiler';

import { type ExecutionBoundary, type SandboxBoundaryExpansion } from '@sharker/core/sandbox-boundary';
...
export class FilesystemWorkerClient {
  ...
  async execute(input: FilesystemWorkerExecuteInput): Promise<FilesystemWorkerResult> {
    ...
    //   while this call waited — typically a cooperative Sharker delete that ran
