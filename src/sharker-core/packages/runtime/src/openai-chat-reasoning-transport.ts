...
const EMPTY_REASONING_MARKER = '\0SHARKER_OPENAI_CHAT_EMPTY_REASONING\0';
...
const SHARKER_PROVIDER_OPTIONS_NAMESPACE = 'sharker';

export function openAiChatReasoningFieldProviderOptions(
  field: OpenAiChatReasoningField,
): Record<string, Record<string, string>> {
  return { [SHARKER_PROVIDER_OPTIONS_NAMESPACE]: { openAiChatReasoningField: field } };
}

export function openAiChatReasoningFieldFromProviderOptions(
  providerOptions: Record<string, unknown> | undefined,
): OpenAiChatReasoningField | undefined {
  const sharker = providerOptions?.[SHARKER_PROVIDER_OPTIONS_NAMESPACE];
  if (!isRecord(sharker)) return undefined;
  const field = sharker.openAiChatReasoningField ?? sharker.kimiReasoningField;
