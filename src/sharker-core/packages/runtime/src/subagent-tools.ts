...
import { z } from 'zod';
import { TASK_ID_MAX_CHARS, isSafeTaskId, type TaskLedgerStore } from '@sharker/core/task-ledger';
import { decodeCanonicalToolResultContent } from '@sharker/core/tool-result-record-schema';
import { isSafeSubagentPresetId } from '@sharker/core/subagent-settings';
...
export function buildChildAgentTools(tools: readonly SharkerTool[]): SharkerTool[] {
  const seen = new Set<string>();
  const out: SharkerTool[] = [];
...
export function buildSubagentListTool(): SharkerTool<
...
export function buildSubagentOutputTool(): SharkerTool<
  {
...
export function buildSubagentProjectionTools(): SharkerTool[] {
