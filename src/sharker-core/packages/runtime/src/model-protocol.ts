/**
 * Sharker-owned provider-boundary model protocol types (#1381 slice 1).
 *
 * This module is the single seam where the provider-boundary message/value
 * contract is *owned* by Sharker. Runtime consumers (history projection,
 ...
 * contracts below are adapted from the AI SDK and modified by Sharker. The source
 ...
 *
 * Sharker modified the adapted material: the deprecated `file-*` and `image-*`
 ...
 * Sharker-authored and have no upstream counterpart.
 *
 * The dependency boundary is Sharker-owned: the generated declaration of this
 * module imports nothing from `ai` or `@ai-sdk/*`. Lowering Sharker messages to
 * AI SDK types, and normalizing AI SDK responses back to Sharker types, happens
...
import type { CacheMissInputSource } from '@sharker/core/usage-stats/types';
...
/**
 * Sharker-owned provider-facing tool contract. The schema stays opaque because
...
export type ModelToolDefinition =
  ...
  | {
      kind: 'provider';
      providerTool: NonNullable<import('./tool-runtime.js').SharkerTool['providerTool']>;
    };
 ...
 * Consumers build and read this Sharker-owned union; `ModelAdapter` lowers it to
 ...
 * Owned by Sharker; only the `ModelAdapter` normalization helper reads it.
 */
...
/**
 * Sharker-owned, provider-agnostic token-usage contract. `ModelAdapter`
...
/**
 * Normalized provider finish reason as a Sharker-owned string. The raw AI SDK
...
/**
 * Provider request metadata reduced to the Sharker-owned message projection.
...
/**
 * Sharker-owned discriminated stream event. `ModelAdapter` translates each raw
...
export type ModelStreamEvent =
  ...
  | {
      ...
      /** Sharker-authored replay hint; absent provider metadata stays fail-closed. */
      providerOptionsOrigin?: 'sharker_transport';
    }
