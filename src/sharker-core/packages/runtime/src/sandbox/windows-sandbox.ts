...
import { isCanonicalWindowsPath } from '@sharker/core/windows-path';
...
export function createWindowsBrokerManifestWriter(
  ...
  const directory = mkdtempSync(join(temporaryRoot, 'sharker-windows-sandbox-'));
  return (manifest) => {
