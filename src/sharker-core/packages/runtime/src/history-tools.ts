...
} from '@sharker/core/thread-search';
import { z } from 'zod';
 ...
 */
export function buildHistoryTools(deps: HistoryToolDeps): readonly SharkerTool[] {
  return [buildSearchHistoryTool(deps), buildReadHistoryTool(deps)];
}

export function buildSearchHistoryTool(deps: HistoryToolDeps): SharkerTool {
  return {
...
export function buildReadHistoryTool(deps: HistoryToolDeps): SharkerTool {
  return {
