...
import type { StorageRef, ToolResultContent } from '@sharker/core/events';
...
import { decodePersistedToolResultContent } from '@sharker/core/tool-result-record-schema';
import { isEmittedAgentRunEventType, isSessionInlineRun } from '@sharker/core/agent-run';
import {
  decodeModelCallAttempt,
  MODEL_CALL_ATTEMPT_EVENT_TYPE,
} from '@sharker/core/model-call-attempt';
import { TOOL_RECOVERY_DECISION_FACT_KIND } from '@sharker/core/tool-recovery-fact';
import {
...
function rewriteAttachmentResourceRefs(
  ...
  return text.replace(/sharker:\/\/runtime\/attachments\/[^\s)\]}>`'",;:!]+/g, (candidate) => {
    const parsed = parseAttachmentResourceRef(candidate);
    const artifactId = parsed ? artifactIds.get(parsed.artifactId) : undefined;
    return artifactId ? `sharker://runtime/attachments/${artifactId}` : candidate;
  });
}
