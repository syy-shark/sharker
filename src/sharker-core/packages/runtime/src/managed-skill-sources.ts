...
export function resolveManagedSkillSourcesRoot(homeDir = homedir()): string {
  const override = process.env.SHARKER_SKILL_SOURCES_ROOT;
  if (override && isAbsolute(override) && process.env.SHARKER_E2E_FIXTURE) {
    return override;
  }
  return join(homeDir, '.sharker', 'skill-sources');
}
