...
export type SkillInventoryResolver = (
  context: Pick<SharkerToolContext, 'sessionId' | 'turnId' | 'cwd'>,
...
export class SkillShadowSelectionTracker {
  ...
  record(context: Pick<SharkerToolContext, 'sessionId' | 'turnId'>, refs: readonly string[]): void {
  ...
  observe(
    context: Pick<SharkerToolContext, 'sessionId' | 'turnId'>,
    ref: string,
  ): {
    rank?: number;
...
): SharkerTool<{ name: string }, LoadSkillInstructionsResult> {
...
export function buildSkillSearchAgentTool(
  ...
  host?: HostCapabilities | HostCapabilitiesResolver,
  options: SkillToolOptions = {},
): SharkerTool<{ query: string; limit?: number }, SkillSearchResult> {
