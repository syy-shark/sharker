 ...
 *
 * `SessionStore` comes from `@sharker/storage`; its public interface owns
...
import { messageContentsEqual, normalizeMessageContent } from '@sharker/core/events';
import type {
  SessionHeader,
  SessionHeaderPatch,
...
import { DEFAULT_TOOL_MODE } from '@sharker/core/tool-mode';
import type {
...
import {
  ...
  type PlanStore,
} from '@sharker/core/plan';
import { DEFAULT_SESSION_NAME } from '@sharker/core/session-name';
import { DEEP_RESEARCH_SESSION_LABEL, isDeepResearchSession } from '@sharker/core/deep-research';
import {
...
import { decodeAgentGraphIntentClaim } from '@sharker/core/agent-graph-control';
import { executionBoundaryContains } from '@sharker/core/sandbox-boundary';
import { failureClassFromCompleteStopReason } from '@sharker/core/events';
import { isActiveShellRunStatus } from '@sharker/core/shell-run';
import { isSessionInlineRun } from '@sharker/core/agent-run';
import { isTerminalRuntimeEvent } from '@sharker/core/runtime-event';
import type {
  AgentGraphIntentClaim,
  AgentGraphIntentClaimStore,
} from '@sharker/core/agent-graph-control';
import type {
  AgentGraphOperatorProvisionRequest,
  AgentGraphOperatorProvisionResult,
  AgentGraphProvisionedEdge,
} from '@sharker/core/agent-graph-topology';
import type { AgentGraphScheduleUpdateSource } from '@sharker/core/agent-graph-schedule';
import type {
  AgentRunEvent,
  AgentRunHeader,
  AgentRunStore,
  RootExecutionDescriptor,
} from '@sharker/core/agent-run';
import type { ArtifactRecord } from '@sharker/core/artifacts';
import type { ContinuationClaimV1 } from '@sharker/core/runtime-boundary';
import type {
  RuntimeEventStore,
  RuntimeContinuationAuthorityStore,
} from '@sharker/core/runtime-event-store';
import type { RuntimeEvent, ToolBoundaryProtocol } from '@sharker/core/runtime-event';
import type { RunCompositionSnapshot } from '@sharker/core/run-composition';
import type {
  SubagentWorkspaceBinding,
  SubagentWorktreeExecutor,
} from '@sharker/core/subagent-workspace';
import type { SubagentPreset } from '@sharker/core/subagent-settings';
import type { ResolvedSubagentPreset } from './configured-subagent-catalog.js';
import { AGENT_GRAPH_OPERATOR_PROVISION_SCHEMA_VERSION } from '@sharker/core/agent-graph-topology';
...
import type { AgentBackend, BackendStopMode } from '@sharker/core/backend-types';
...
import type { ModelCallAttempt } from '@sharker/core/model-call-attempt';
import type {
...
import type { ModelCallCommit } from '@sharker/core/agent-run';
...
interface SessionManagerBaseDeps {
  ...
  newId: () => string;
  now: () => number;
  childTools?: readonly SharkerTool[];
...
export class SessionManager {
  ...
  private async childToolsForSession(sessionId: string): Promise<readonly SharkerTool[]> {
...
function continuationRepairEventId(
  ...
  return `continuation-repair-${kind}-${createHash('sha256')
    .update(`sharker.continuation-repair.${kind}.v1\0${claimId}`)
    .digest('hex')
    .slice(0, 32)}`;
}
