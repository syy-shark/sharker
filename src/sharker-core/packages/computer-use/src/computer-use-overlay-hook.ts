...
import type { CuAction, CuPoint } from '@sharker/core/computer-use';
import type {
  CuOverlayHook,
 ...
 *
 * Sharker has one of those two reasons available and not the other. The launch
...
export function createComputerUseOverlayHook(controller: OverlayCursorSink): CuOverlayHook {
  return {
    ...
    onActionEnd(action, result, context) {
      ...
      // path, which is the only path Sharker dispatches on by default — ended in
