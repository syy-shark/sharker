...
// The `sharker.cu/2` wire contract, host side. Mirrors `sharker-cu`'s
...
export const SHARKER_CU_PROTOCOL_VERSION = 'sharker.cu/2';

/** JSON-RPC error codes (§1.1). These describe the request, never the world. */
export const SHARKER_CU_RPC_ERROR = {
  parse: -32700,
...
/**
 * §2/§6.3: the only value Sharker ships. It is declared once because the handshake
 * that sends it and the dispatch reader that verifies the executor honoured it
 * must never be able to disagree.
 */
export const SHARKER_CU_ALLOW_GLOBAL_POINTER = false;

export interface SharkerCuRpcErrorBody {
  code: number;
  message: string;
  data?: Record<string, unknown>;
}

export interface SharkerCuRpcResponse {
  jsonrpc: '2.0';
  id: number;
  result?: Record<string, unknown>;
  error?: SharkerCuRpcErrorBody;
}

/** A `result` envelope (§1.1): the tagged union that carries the world. */
export type SharkerCuEnvelope =
  ...
  | ({ ok: false; error: SharkerCuDomainError } & Record<string, unknown>);

export interface SharkerCuDomainError {
  code: string;
  message: string;
...
// ---------------------------------------------------------------------------
// §7.1 domain code → Sharker error code. Mechanical, no inference, no message
...
/**
 * The Sharker codes §7.1 can produce.
 *
 ...
 */
export type SharkerCuMappedErrorCode = (typeof DOMAIN_ERROR_CODES)[keyof typeof DOMAIN_ERROR_CODES];

/** `undefined` means this host does not know the code — treat as version skew. */
export function mapSharkerCuDomainError(code: string): SharkerCuMappedErrorCode | undefined {
...
export type SharkerCuDomainErrorCode = keyof typeof DOMAIN_ERROR_CODES;
export const SHARKER_CU_DOMAIN_ERROR_CODES = Object.keys(
  DOMAIN_ERROR_CODES,
) as readonly SharkerCuDomainErrorCode[];

// ---------------------------------------------------------------------------
// §6.3/§6.5 declared dispatch fields.
// ---------------------------------------------------------------------------
export const SHARKER_CU_DISPATCH_OUTCOMES = ['ok', 'refused', 'failed', 'unknown'] as const;
export type SharkerCuDispatchOutcome = (typeof SHARKER_CU_DISPATCH_OUTCOMES)[number];

export const SHARKER_CU_DISPATCH_PATHS = [
  'ax_action',
  'ax_attribute',
  'ax_select',
  'cg_event_pid',
...
export type SharkerCuDispatchPath = (typeof SHARKER_CU_DISPATCH_PATHS)[number];
 ...
 */
export const SHARKER_CU_ELEMENT_ACTIONS = [
  'press',
  'confirm',
  'open',
  'show_menu',
...
export const SHARKER_CU_VERIFICATION_METHODS = [
  'none',
  'action_result',
  'value_readback',
  'selection_readback',
...
export type SharkerCuVerificationMethod = (typeof SHARKER_CU_VERIFICATION_METHODS)[number];

/** §6.3: the pairing is fixed. `none` is dispatched nothing, so it pairs with any tier. */
const PATHS_BY_TIER: Record<ComputerUseDispatchTier, readonly SharkerCuDispatchPath[]> = {
...
const GLOBAL_POINTER_PATHS: readonly SharkerCuDispatchPath[] = ['cg_event_global'];

export interface SharkerCuVerification {
  method: SharkerCuVerificationMethod;
  observedChange: boolean;
}

export interface SharkerCuSettle {
  waitedMs: number;
  quiesced: boolean;
  reason: string;
}
...
// closed set of modifiers; Sharker's callers hold xdotool-flavoured strings like
// `cmd+a`. The host owns the translation because Sharker's runtime owns every
...
// ---------------------------------------------------------------------------
export const SHARKER_CU_KEY_MODIFIERS = ['command', 'shift', 'option', 'control', 'fn'] as const;
export type SharkerCuKeyModifier = (typeof SHARKER_CU_KEY_MODIFIERS)[number];
 ...
 */
export const SHARKER_CU_NAMED_KEYS = [
  'Return',
  'Tab',
  'Space',
  'Escape',
...
export type SharkerCuNamedKey = (typeof SHARKER_CU_NAMED_KEYS)[number];
...
// `parseSharkerCuKeyChord('constructor')` returned a chord whose key was a
...
const MODIFIER_ALIASES: Record<string, SharkerCuKeyModifier> = Object.assign(Object.create(null), {
  cmd: 'command',
  command: 'command',
  meta: 'command',
  super: 'command',
...
const NAMED_KEY_ALIASES: Record<string, SharkerCuNamedKey> = Object.assign(Object.create(null), {
  ...Object.fromEntries(SHARKER_CU_NAMED_KEYS.map((key) => [key.toLowerCase(), key])),
  enter: 'Return',
  esc: 'Escape',
  spc: 'Space',
  pgup: 'PageUp',
...
export interface SharkerCuKeyChord {
  key: string;
  modifiers: SharkerCuKeyModifier[];
}
 ...
 */
export function parseSharkerCuKeyChord(input: string): SharkerCuKeyChord | undefined {
  ...
  const modifiers: SharkerCuKeyModifier[] = [];
...
export interface SharkerCuDispatchResult {
  toolCallId: string;
  outcome: SharkerCuDispatchOutcome;
  tier: ComputerUseDispatchTier;
  path: SharkerCuDispatchPath;
  effect: ComputerUseEffect;
  verification: SharkerCuVerification;
  settle?: SharkerCuSettle;
  snapshot?: SharkerCuSnapshot;
  /** §6.5: the code is held to §7.1's closed set, unlike a refusal's own. */
  postObservationError?: { code: SharkerCuDomainErrorCode; message: string };
}

// ---------------------------------------------------------------------------
// §5 observation.
// ---------------------------------------------------------------------------
export interface SharkerCuElement {
  token: string;
...
export interface SharkerCuImage {
  path: string;
  format: 'png' | 'jpeg';
  widthPx: number;
  heightPx: number;
...
export interface SharkerCuDisplay {
  displayId: string;
 ...
 */
export interface SharkerCuSnapshotTarget {
  pid: number;
  windowId: number;
  appId: string;
  appName?: string;
 ...
 */
export interface SharkerCuWindow {
  pid: number;
  windowId: number;
  appId: string;
  layer: number;
...
export interface SharkerCuMenu {
  elements: SharkerCuElement[];
  truncated: { elements: boolean; depth: boolean };
}

export type SharkerCuDifferencePresentation = 'no-change' | 'difference' | 'full';

export interface SharkerCuDifferenceChange {
...
export interface SharkerCuDifference {
  baseSnapshotId: string;
  presentation: SharkerCuDifferencePresentation;
  changes: SharkerCuDifferenceChange[];
  removedStableIdRanges: Array<{ start: number; end: number }>;
}

export interface SharkerCuSnapshot {
  snapshotId: string;
  capturedAt: number;
  target: SharkerCuSnapshotTarget;
  windowDigest: string;
  focusedElementToken: string | null;
  selectedText: { text: string; truncated: boolean } | null;
  image: SharkerCuImage | null;
  displays: SharkerCuDisplay[];
  obscuringRects: ComputerUseRect[];
  elements: SharkerCuElement[];
  truncated: { elements: boolean; depth: boolean };
  difference?: SharkerCuDifference;
  /**
...
export class SharkerCuProtocolViolation extends Error {
  constructor(
    readonly method: string,
    readonly reason: string,
  ) {
    super(`sharker-cu protocol violation in ${method}: ${reason}`);
    this.name = 'SharkerCuProtocolViolation';
  }
}
...
function requireRecord(method: string, value: unknown, what: string): Record<string, unknown> {
  if (!isRecord(value)) throw new SharkerCuProtocolViolation(method, `${what} is not an object`);
  return value;
}

function requireString(method: string, value: unknown, what: string): string {
  if (typeof value !== 'string' || value.length === 0) {
    throw new SharkerCuProtocolViolation(method, `${what} is not a non-empty string`);
  }
  return value;
}
...
function requireNumber(method: string, value: unknown, what: string): number {
  if (typeof value !== 'number' || !Number.isFinite(value)) {
    throw new SharkerCuProtocolViolation(method, `${what} is not a finite number`);
  }
  return value;
}
...
function requireNonNegativeInteger(method: string, value: unknown, what: string): number {
  ...
  if (!Number.isInteger(number) || number < 0) {
    throw new SharkerCuProtocolViolation(method, `${what} is not a non-negative integer`);
  }
  return number;
}
...
function requireBoolean(method: string, value: unknown, what: string): boolean {
  if (typeof value !== 'boolean') {
    throw new SharkerCuProtocolViolation(method, `${what} is not a boolean`);
  }
  return value;
}
...
function requireMember<T extends string>(
  ...
  if (typeof value !== 'string' || !(members as readonly string[]).includes(value)) {
    throw new SharkerCuProtocolViolation(method, `${what} is outside its closed set`);
  }
  return value as T;
}
...
export function requireDigest(method: string, value: unknown, what: string): string {
  if (typeof value !== 'string' || !HASH_PATTERN.test(value)) {
    throw new SharkerCuProtocolViolation(method, `${what} is not a "sha256:" lowercase-hex digest`);
  }
  return value;
}
...
function requireArray(method: string, value: unknown, what: string): unknown[] {
  if (!Array.isArray(value)) throw new SharkerCuProtocolViolation(method, `${what} is not an array`);
  return value;
}
...
function requireNullableString(method: string, value: unknown, what: string): string | null {
  ...
  if (typeof value !== 'string' || value.length === 0) {
    throw new SharkerCuProtocolViolation(method, `${what} is neither null nor a non-empty string`);
  }
  return value;
}
...
function requireNullableBoolean(method: string, value: unknown, what: string): boolean | null {
  if (value === null || value === undefined) return null;
  if (typeof value !== 'boolean') {
    throw new SharkerCuProtocolViolation(method, `${what} is neither null nor a boolean`);
  }
  return value;
}
...
function optionalText(method: string, value: unknown, what: string): string | undefined {
  ...
  if (typeof value !== 'string') {
    throw new SharkerCuProtocolViolation(method, `${what} is not a string`);
  }
  return value;
}
...
export function readEnvelope(method: string, result: unknown): SharkerCuEnvelope {
  const record = requireRecord(method, result, 'result');
  if (record.ok === true) return record as { ok: true } & Record<string, unknown>;
  if (record.ok !== false) throw new SharkerCuProtocolViolation(method, 'result.ok is not a boolean');
...
export function readElement(method: string, value: unknown): SharkerCuElement {
  ...
  return {
    ...
    actions: actions.map((action, index) =>
      requireMember(method, action, SHARKER_CU_ELEMENT_ACTIONS, `element.actions[${index}]`),
    ),
...
function readImage(method: string, value: unknown): SharkerCuImage | null {
  if (value === null || value === undefined) return null;
  return readImageField(method, value);
}

/** Every image is a file reference; there is no inline branch to fall back to (§8). */
export function readImageField(method: string, value: unknown): SharkerCuImage {
...
export function readWindow(method: string, value: unknown): SharkerCuWindow {
...
export function readApp(method: string, value: unknown): SharkerCuApp {
 ...
 */
export function readLaunchedApp(method: string, value: unknown): SharkerCuLaunchedApp {
...
export function readSnapshot(method: string, value: unknown): SharkerCuSnapshot {
  ...
  const stableElements = parsedElements.filter(
    (element): element is SharkerCuElement & { stableId: number } => element.stableId !== undefined,
  );
  if (stableElements.length !== 0 && stableElements.length !== parsedElements.length) {
    throw new SharkerCuProtocolViolation(
      method,
  ...
  if (stableIds.size !== stableElements.length) {
    throw new SharkerCuProtocolViolation(method, 'snapshot.elements contains duplicate stable ids');
  }
...
function readDifference(method: string, value: unknown): SharkerCuDifference {
...
function readMenu(method: string, value: unknown): SharkerCuMenu {
 ...
 * executor moved the system cursor, which is the one invariant Sharker does not
...
export function readDispatchResult(
  ...
  const path = requireMember(method, envelope.path, SHARKER_CU_DISPATCH_PATHS, 'path');
  const outcome = requireMember(method, envelope.outcome, SHARKER_CU_DISPATCH_OUTCOMES, 'outcome');
  ...
  if (outcome !== 'ok' && effect === 'confirmed') {
    ...
    throw new SharkerCuProtocolViolation(method, `outcome '${outcome}' reported effect 'confirmed'`);
  }
  if (path !== 'none' && !PATHS_BY_TIER[tier].includes(path)) {
    throw new SharkerCuProtocolViolation(method, `tier '${tier}' does not permit path '${path}'`);
  }
  ...
  return {
    ...
    verification: {
      method: requireMember(
        method,
        verification.method,
        SHARKER_CU_VERIFICATION_METHODS,
        'verification.method',
      ),
