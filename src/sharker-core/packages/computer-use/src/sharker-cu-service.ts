...
// Supervises one `sharker-cu` executor child and speaks `sharker.cu/2` to it over
// line-delimited JSON-RPC 2.0 on stdio (`sharker-cu`'s docs/HOST_PROTOCOL.md §1).
//
...
// sharker-cu's teardown against cua-driver.
...
import {
  ...
  type SharkerCuRpcErrorBody,
  type SharkerCuRpcResponse,
  readEnvelope,
...
export type SharkerCuServiceState =
  | 'idle'
  | 'starting'
  | 'ready'
  | 'backing_off'
...
export interface SharkerCuServiceSnapshot {
  state: SharkerCuServiceState;
  generation: number;
  restartAttempts: number;
  executor?: SharkerCuExecutorInfo;
}

export interface SharkerCuReleaseEvent {
  generation: number;
...
export class SharkerCuLifecycleError extends Error {
  constructor(
    ...
    super(`${code}: ${message}`);
    this.name = 'SharkerCuLifecycleError';
  }
}

export function isSharkerCuLifecycleError(
  error: unknown,
  code?: HostLifecycleErrorCode,
): error is SharkerCuLifecycleError {
  return error instanceof SharkerCuLifecycleError && (code === undefined || error.code === code);
}

/** A JSON-RPC `error` (§1.1): the request was unusable, not a fact about the world. */
export class SharkerCuRpcError extends Error {
  constructor(
    readonly method: string,
    readonly body: SharkerCuRpcErrorBody,
  ) {
    super(`sharker-cu ${method}: ${body.message} (${body.code})`);
    this.name = 'SharkerCuRpcError';
  }
}

export interface SharkerCuExecutorInfo {
  name: string;
  version: string;
  commit?: string;
}

export interface SharkerCuCapabilities {
 ...
 */
export interface SharkerCuLimits {
...
export interface SharkerCuHandshake {
  executor: SharkerCuExecutorInfo;
  pid: number;
  capabilities: SharkerCuCapabilities;
  limits: SharkerCuLimits;
}

export interface SharkerCuServiceOptions {
  /** Absolute path to the `sharker-cu` executable; spawned as a DIRECT child (§11). */
  binaryPath: string;
  ...
  onRelease?: (event: SharkerCuReleaseEvent) => void;
}

interface PendingRequest {
  sessionId?: string;
  stage: HostRequestStage;
  resolve: (response: SharkerCuRpcResponse) => void;
...
export class SharkerCuService {
  ...
  negotiated(): SharkerCuHandshake | undefined {
  ...
  private assertActive(): void {
    if (this.disposed) {
      throw new SharkerCuLifecycleError(
        'service_unavailable',
        'sharker-cu service disposed',
        this.generation,
      );
    }
  }
  ...
  async ensureStarted(signal?: AbortSignal): Promise<SharkerCuHandshake> {
    this.assertActive();
  ...
  private async startWithBudget(): Promise<void> {
    ...
    while (this.restartAttempts < maxAttempts) {
      ...
      try {
        ...
        if (
          isSharkerCuLifecycleError(error, 'service_mismatch') ||
          (isSharkerCuLifecycleError(error) && error.fatal) ||
    ...
    throw new SharkerCuLifecycleError(
      ...
      `sharker-cu restart budget exhausted: ${
  ...
  private async start(): Promise<void> {
    ...
    // fails: sharker-cu is unsigned, hand-built and not distributed, so ENOENT,
  ...
  private async hello(): Promise<SharkerCuHandshake> {
    const timeoutMs = this.opts.handshakeTimeoutMs ?? DEFAULT_HANDSHAKE_TIMEOUT_MS;
    const protocol = this.opts.protocolVersion ?? SHARKER_CU_PROTOCOL_VERSION;
    const response = await this.request(
      'host.hello',
      {
        protocol,
        host: { name: 'sharker', version: this.opts.hostVersion },
        hostPid: process.pid,
        imageDir: this.opts.imageDir,
        allowGlobalPointer: SHARKER_CU_ALLOW_GLOBAL_POINTER,
      },
      { timeoutMs },
    );
    if (response.error) {
      ...
      throw new SharkerCuLifecycleError(
        response.error.code === SHARKER_CU_RPC_ERROR.protocolVersionMismatch
          ? 'service_mismatch'
          : 'service_unavailable',
  ...
  private async verifyExecutable(): Promise<string> {
    try {
      ...
      this.state = 'unavailable';
      if (isSharkerCuLifecycleError(error)) throw error;
      ...
      throw new SharkerCuLifecycleError(
        'service_unavailable',
        `sharker-cu executable is not usable at ${this.opts.binaryPath}: ${
  ...
  private onStdout(child: ChildProcessWithoutNullStreams, chunk: string): void {
    ...
    const rest = decodeJsonLines(this.buffer, chunk, {
      ...
      onMessage: (value) => {
        ...
        const message = value as unknown as SharkerCuRpcResponse;
   ...
   * was "sharker-cu exited after request delivery" for a child that never execed.
   */
  private terminationReason(reason: SharkerCuReleaseEvent['reason'], delivered: boolean): string {
    if (this.childError) {
      return `sharker-cu could not be executed: ${this.childError.message}`;
    }
    switch (reason) {
      case 'request_timeout':
        return 'sharker-cu did not answer within the host deadline and was terminated';
      case 'protocol_violation':
        return 'sharker-cu sent stdout that is not this protocol and was terminated';
      case 'session_cleared':
        return 'sharker-cu was terminated while clearing a session with a request in flight';
      case 'disposed':
        return 'the host shut sharker-cu down';
      case 'restart_exhausted':
        return 'sharker-cu could not be started within the restart budget';
      default:
        return delivered
          ? 'sharker-cu exited after request delivery'
          : 'sharker-cu exited before request delivery';
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      timer = setTimeout(() => {
        ...
        entry.reject(
          new SharkerCuLifecycleError(
            'service_unavailable',
            `sharker-cu ${method} timed out before delivery`,
            this.generation,
            entry.stage,
          ),
        );
  ...
  /**
   * One request/response round trip. JSON-RPC `error` becomes SharkerCuRpcError;
   ...
   */
  async call(method: string, params: unknown, signal?: AbortSignal): Promise<SharkerCuEnvelope> {
    this.assertActive();
    await this.ensureStarted(signal);
    this.assertActive();
    const response = await this.request(method, params, { ...(signal ? { signal } : {}) });
    if (response.error) throw new SharkerCuRpcError(method, response.error);
  ...
  private kill(reason: SharkerCuReleaseEvent['reason']): void {
    const child = this.child;
    if (!child) return;
    child.kill('SIGKILL');
  ...
  dispose(): void {
    ...
    // already had this exact guard and sharker-cu's copy dropped it.
    const starting = this.starting;
    this.disposed = true;
    this.state = 'disposed';
    const child = this.child;
 ...
 *
 * These throw `SharkerCuProtocolViolation` rather than a plain `Error` so the one
 ...
 * same thing.
 */
function violation(what: string, reason: string): SharkerCuProtocolViolation {
  return new SharkerCuProtocolViolation('host.hello', `${what} ${reason}`);
}
...
function readExecutorInfo(value: unknown): SharkerCuExecutorInfo {
...
function readCapabilities(value: unknown): SharkerCuCapabilities {
  const record = (value ?? {}) as Record<string, unknown>;
...
function readLimits(value: unknown): SharkerCuLimits {
