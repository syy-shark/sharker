...
// The `sharker.cu/2` CuDispatchBackend. Speaks the host protocol (`sharker-cu`'s
// docs/HOST_PROTOCOL.md) to `sharker-cu`, the native macOS executor; section
...
//
// What the protocol declares and Sharker's own types cannot yet carry: per-element
...
import {
  ...
  type SharkerCuDispatchResult,
  type SharkerCuDomainError,
  type SharkerCuDomainErrorCode,
  type SharkerCuElement,
  type SharkerCuEnvelope,
  type SharkerCuImage,
  type SharkerCuMappedErrorCode,
  type SharkerCuSnapshot,
  type SharkerCuWindow,
} from './sharker-cu-protocol.js';
import {
  isSharkerCuLifecycleError,
  SharkerCuRpcError,
  SharkerCuService,
  type SharkerCuReleaseEvent,
  type SharkerCuServiceSnapshot,
} from './sharker-cu-service.js';

/**
 * `CuAction.scrollAmount` has no declared unit at the tool boundary ("Amount for
 * scroll", 0..100) while `sharker.cu/2` declares pages. The conversion is fixed
 ...
 * window array it sees at launch time always answers with an empty one. sharker.cu
 ...
 * name it cannot use.
 */
const ELEMENT_ACTION_NAMES = SHARKER_CU_ELEMENT_ACTIONS.filter((name) => name !== 'scroll_to_visible');

export interface SharkerCuBackendOptions {
  /** Absolute path to the `sharker-cu` executable. */
  binaryPath: string;
  /**
   ...
   */
  onTrace?: (event: SharkerCuTraceEvent) => void;
  onSessionInvalidated?: (input: {
    sessionId: string;
    reason: SharkerCuReleaseEvent['reason'];
    outcomeUnknown: boolean;
  }) => void;
}

export type SharkerCuTraceEvent =
  | {
      type: 'observe';
      toolCallId?: string;
      snapshotId: string;
  ...
  | {
       ...
       *
       * That text used to go to the model. `failed: service_mismatch — sharker-cu
...
export type SharkerCuHostErrorKind =
  | 'rpc_rejected'
  | 'protocol_violation'
  | 'unknown_refusal'
  | 'executor_gone'
 ...
 */
const HOST_ERROR_SENTENCE: Record<SharkerCuHostErrorKind, string> = {
  rpc_rejected:
...
/**
 * What `sharker.cu/2` carries that Sharker's shared Computer Use contract does not
 * carry yet.
 *
 ...
 * `SharkerCuObservation` IS a `CuObservation`, so nothing downstream changes and
 ...
 * `@sharker/runtime`. Keeping them here in the meantime is the difference between
 * an executor that quietly discards protocol answers and one that carries them
 * as far as the contract allows.
 */
export type SharkerCuObservedElement = CuObservedElement & {
  subrole?: string;
  placeholder?: string;
  actions?: string[];
  /**
...
export type SharkerCuObservation = Omit<CuObservation, 'elements'> & {
  elements: SharkerCuObservedElement[];
 ...
 * reads it. `sharker.cu/2` §1.2 makes it a rule for the executor's own sentences
 ...
 */
type SharkerCuFailureOutcome = Extract<CuRunResult['outcome'], { ok: false }> & {
  messageIsAppTextFree?: boolean;
};

/**
 * The semantic actions this executor can carry out, which is a superset of the
 * ones Sharker's tool surface can currently express.
 *
 ...
 */
export type SharkerCuSemanticAction =
...
export interface SharkerCuLaunchedApp {
  pid: number;
  bundleId: string;
  name?: string;
 ...
 */
export type SharkerCuBackend = Omit<
  CuDispatchBackend,
...
> & {
  ...
  runSemantic(
    action: SharkerCuSemanticAction,
    signal: AbortSignal,
    context: CuRunContext,
  ): Promise<CuRunResult>;
  launchApp(
    ...
    signal: AbortSignal,
    context: CuRunContext,
  ): Promise<SharkerCuLaunchedApp>;
  executorState: () => SharkerCuServiceSnapshot;
 ...
 * call sites. `sharker.cu/2` §1.2 makes it a protocol rule for the executor's own
 ...
 * test noticing.
 */
function addressable(snapshot: SharkerCuSnapshot): readonly SharkerCuElement[] {
 ...
 *
 * Total by construction: `SharkerCuMappedErrorCode` is the set §7.1 can produce,
 * so a new row in that table without a sentence here does not build.
 */
const DOMAIN_REFUSAL_SENTENCE: Record<SharkerCuMappedErrorCode, string> = {
  stale_frame:
 ...
 */
function postObservationSentence(method: string, code?: SharkerCuDomainErrorCode): string {
  if (code === undefined) {
  ...
  return `${method} reached the screen, but the window could not be read afterwards. ${
    DOMAIN_REFUSAL_SENTENCE[mapSharkerCuDomainError(code) ?? 'outcome_unknown']
  }`;
}

function nextMoveFor(
  mapped: SharkerCuMappedErrorCode,
  error: SharkerCuDomainError,
  refusal?: SharkerCuDispatchResult,
...
function failure(error: ComputerUseErrorCode, message: string): CaptureFailure {
  const outcome: SharkerCuFailureOutcome = { ok: false, error, message, messageIsAppTextFree: true };
  return { outcome };
}

/** The host cleared the session while this operation was still queued. */
class SharkerCuSessionCleared extends Error {
  constructor() {
    super('session was cleared before request delivery');
    this.name = 'SharkerCuSessionCleared';
  }
}
 ...
 */
class SharkerCuDomainRefusal extends Error {
  constructor(
    readonly method: string,
    readonly domain: SharkerCuDomainError,
  ) {
    super(`sharker-cu ${method} refused: ${domain.code}`);
    this.name = 'SharkerCuDomainRefusal';
  }
}
 ...
 * Sharker code directly because no executor code was involved, and it travels the
 * same path as a domain refusal so neither escapes as an unmapped exception.
 */
class SharkerCuHostRefusal extends Error {
  constructor(
    readonly code: ComputerUseErrorCode,
    message: string,
  ) {
    super(message);
    this.name = 'SharkerCuHostRefusal';
  }
}

export function createSharkerCuBackend(opts: SharkerCuBackendOptions): SharkerCuBackend {
  const imageDir = opts.imageDir ?? join(tmpdir(), `sharker-cu-images-${process.pid}-${randomUUID()}`);
  ...
  let sessionClearReleaseEvents: SharkerCuReleaseEvent[] | undefined;
  let disposed = false;

  function trace(event: SharkerCuTraceEvent): void {
    try {
      opts.onTrace?.(event);
    } catch {
  ...
  function applyServiceRelease(events: readonly SharkerCuReleaseEvent[]): void {
  ...
  const service = new SharkerCuService({
  ...
  async function withOperationQueue<T>(
    ...
    if (disposed) throw new Error('sharker-cu backend disposed');
    // §9 gives the executor per-target lanes; the host queue stays upstream of
    // them so one Sharker turn never has two dispatches in flight at once.
    ...
    try {
      ...
      if (
        ...
        throw new SharkerCuSessionCleared();
      }
   ...
   * driving a real screen was told `service_mismatch — sharker-cu
  ...
  /** Translate an executor or transport failure into a Sharker outcome. */
  function backendFailure(method: string, error: unknown): CaptureFailure | undefined {
    if (error instanceof SharkerCuSessionCleared) {
      return hostErrorFailure(method, 'aborted', 'cancelled', error.message);
    }
    if (error instanceof SharkerCuDomainRefusal) {
    ...
    if (error instanceof SharkerCuHostRefusal) return failure(error.code, error.message);
    if (isSharkerCuLifecycleError(error)) {
      ...
      const kind: SharkerCuHostErrorKind =
        error.code === 'aborted'
          ? 'cancelled'
    ...
    if (error instanceof SharkerCuProtocolViolation) {
    ...
    if (error instanceof SharkerCuRpcError) {
      if (error.body.code === SHARKER_CU_RPC_ERROR.sessionUnknown) {
        return failure(
          'stale_frame',
      ...
      if (error.body.code === SHARKER_CU_RPC_ERROR.shuttingDown) {
  ...
  function domainFailure(
    ...
    const mapped = mapSharkerCuDomainError(error.code);
    if (!mapped) {
      // §7.1 is a closed table. An unknown code is version skew, and guessing a
      // Sharker code for it is exactly the archaeology this protocol removes.
      service.reportProtocolViolation();
    ...
    const wouldRequirePath =
      detail && typeof detail.wouldRequirePath === 'string' ? detail.wouldRequirePath : undefined;
    const outcome: SharkerCuFailureOutcome = {
      ok: false,
      error: mapped,
  ...
  async function ensureSession(sessionId: string, signal: AbortSignal): Promise<void> {
    ...
    const envelope = await service.call(
      'session.begin',
      // Observation is window-scoped in Sharker, so the session's ScreenCaptureKit
    ...
    if (!envelope.ok) {
      throw new SharkerCuDomainRefusal('session.begin', envelope.error);
    }
  ...
  function limits() {
    ...
    if (!negotiated) {
      ...
      throw new SharkerCuHostRefusal(
        'service_unavailable',
  ...
  function storeSnapshot(snapshot: SharkerCuSnapshot, context: CuRunContext): void {
    dropExpired();
  ...
  async function readFrame(image: SharkerCuImage): Promise<CuScreenshot | CaptureFailure> {
    let bytes: Buffer;
    try {
  ...
  function toDisplays(snapshot: SharkerCuSnapshot): ComputerUseDisplayIdentity[] | undefined {
  ...
  async function toObservation(
    ...
    const observation: SharkerCuObservation = {
      ...
      // titleless full-screen surface that is the Dock and not a cover. sharker-cu
      // answers it directly.
      //
  ...
  async function listWindows(sessionId: string, signal: AbortSignal): Promise<SharkerCuWindow[]> {
    const envelope = await service.call('window.list', { session: sessionId }, signal);
    if (!envelope.ok) throw new SharkerCuDomainRefusal('window.list', envelope.error);
    const windows = envelope.windows;
    if (!Array.isArray(windows)) {
      throw new SharkerCuProtocolViolation('window.list', 'windows is not an array');
    }
  ...
  async function resolveTarget(
    ...
    if (input.windowId === undefined) {
      ...
      if (!winner) {
        throw new SharkerCuHostRefusal('target_missing', 'no visible window is available to observe');
      }
  ...
  async function observe(
    ...
    try {
      ...
      if (!envelope.ok) throw new SharkerCuDomainRefusal('observe', envelope.error);
      ...
      // backend's does.
      const mapped =
        error instanceof SharkerCuDomainRefusal
  ...
  function dispatchOutcome(method: string, result: SharkerCuDispatchResult): CuDispatchOutcome {
  ...
  async function completeDispatch(
    ...
    const result = readDispatchResult(method, envelope, SHARKER_CU_ALLOW_GLOBAL_POINTER);
    trace({
      type: 'dispatch',
      ...(context.toolCallId ? { toolCallId: context.toolCallId } : {}),
      method,
    ...
    if (!result.snapshot) {
      ...
      const unknown: SharkerCuFailureOutcome = {
        ok: false,
  ...
  function elementAction(
    ...
    switch (action.type) {
      ...
      default:
        // Reached only by an action Sharker can express and this backend cannot
   ...
   */
  function attemptFor(action: SharkerCuSemanticAction): DispatchAttempt {
  ...
  async function dispatchElement(
    action: Exclude<SharkerCuSemanticAction, { type: 'press_key' }>,
  ...
  function forgetUnusableSnapshot(error: SharkerCuDomainError, snapshot: StoredSnapshot): void {
    const unusable =
      error.code === 'outcome_unknown' ||
  ...
  function refusedDispatch(
    method: string,
    envelope: Extract<SharkerCuEnvelope, { ok: false }>,
    snapshot: StoredSnapshot,
    context: CuRunContext,
    attempt?: DispatchAttempt,
  ): CuRunResult {
    const refusal = readDispatchResult(method, envelope, SHARKER_CU_ALLOW_GLOBAL_POINTER);
  ...
  /**
   * §6.4: the host parses, before it sends. Sharker's callers hold xdotool-flavoured
  ...
  function keyAction(raw: string): { wire: Record<string, unknown> } | { refusal: CaptureFailure } {
    const chord = parseSharkerCuKeyChord(raw);
    if (!chord) {
      return {
        refusal: failure(
          'unsupported_action',
  ...
  return {
    ...
    async listApps(signal) {
      return withOperationQueue(signal, async (): Promise<CuAppSummary[]> => {
        try {
          ...
          if (!envelope.ok) throw new SharkerCuDomainRefusal('apps.list', envelope.error);
          const apps = envelope.apps;
          if (!Array.isArray(apps)) {
            throw new SharkerCuProtocolViolation('apps.list', 'apps is not an array');
          }
      ...
      return withOperationQueue(
        ...
        async () => {
          try {
            ...
            if (!envelope.ok) throw new SharkerCuDomainRefusal('apps.launch', envelope.error);
      ...
      try {
        return await withOperationQueue(
          ...
          async (): Promise<CuRunResult> => {
            ...
            if (!wire) {
              // `cursor_position`, `hold_key` and `zoom` have no sharker.cu/2
              ...
              // sharker.cu/2" reads as a version problem it might route around.
              return failure(
                'unsupported_action',
    ...
    clearSession(sessionId) {
      const releases: SharkerCuReleaseEvent[] = [];
      ...
      // produced and removes any executor-drawn cursor. Sharker has already been
