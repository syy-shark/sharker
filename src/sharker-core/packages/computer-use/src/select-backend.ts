...
import { buildComputerUseTools, type ComputerUseToolSet } from '@sharker/runtime/computer-use-tools';
import { type CuOverlayHook, type CuDispatchBackend } from '@sharker/runtime/computer-use-types';
import { createSharkerCuBackend } from './sharker-cu-backend.js';
import type { SharkerCuBackendOptions } from './sharker-cu-backend.js';
import type { SharkerCuServiceSnapshot } from './sharker-cu-service.js';
 ...
 */
export const CU_BACKEND_IDS = ['sharker-cu'] as const;
export type CuBackendId = (typeof CU_BACKEND_IDS)[number];

export const DEFAULT_CU_BACKEND_ID: CuBackendId = 'sharker-cu';

type DisposableBackend = CuDispatchBackend & {
  clearSession?: (sessionId: string) => void;
  dispose?: () => void;
  /** sharker-cu supervises one child, not a role pair, so it reports its own shape. */
...
export interface SharkerCuSelection {
  ...
  createBackend?: (options: SharkerCuBackendOptions) => DisposableBackend;
}

export type ComputerUseBackendSelection = SharkerCuSelection;

export function selectComputerUseBackend(deps?: SharkerCuSelection): SelectedComputerUseBackend {
  ...
  try {
    ...
    const backend = (deps.createBackend ?? createSharkerCuBackend)({
      binaryPath,
