...
// Unit test for the sharker-cu CuDispatchBackend. Drives the module against a MOCK
// executor (a small CommonJS node script written to a temp dir) that speaks
// `sharker.cu/2` — the real `sharker-cu` binary is never spawned, and does not exist
...
import type { CuaBoundAction } from '@sharker/runtime/cua-frame-state';

import type { CuObservation, CuRunContext } from '@sharker/runtime/computer-use-types';
import {
  createSharkerCuBackend,
  type SharkerCuBackendOptions,
  type SharkerCuObservation,
} from '../sharker-cu-backend.js';
import { parseSharkerCuKeyChord } from '../sharker-cu-protocol.js';
...
// A CommonJS mock sharker-cu executor. No backticks / ${} inside → embedded via
...
if (process.argv[2] !== 'host') {
  process.stderr.write('mock sharker-cu: expected argv[2] === "host", got ' + JSON.stringify(process.argv[2]) + '\n');
  process.exit(64);
}
const LOG = process.env.SHARKERCU_MOCK_LOG || '';
const PROTOCOL = process.env.SHARKERCU_MOCK_PROTOCOL || 'sharker.cu/2';
const DISPATCH_ERROR = process.env.SHARKERCU_MOCK_DISPATCH_ERROR || '';
const TIER = process.env.SHARKERCU_MOCK_TIER || 'ax';
const PATH_NAME = process.env.SHARKERCU_MOCK_PATH || 'ax_action';
const BAD_IMAGE_SHA = process.env.SHARKERCU_MOCK_BAD_IMAGE_SHA === '1';
// A frame past the model-context cap: readable window, unreturnable picture.
const BIG_IMAGE = process.env.SHARKERCU_MOCK_BIG_IMAGE === '1';
const BARE_IMAGE_SHA = process.env.SHARKERCU_MOCK_BARE_IMAGE_SHA === '1';
const UNVERIFIED_EFFECT = process.env.SHARKERCU_MOCK_UNVERIFIED_EFFECT === '1';
const NO_POST_SNAPSHOT = process.env.SHARKERCU_MOCK_NO_POST_SNAPSHOT === '1';
// The action closed the thing it acted on, so the post-action observation could
// not be taken and never will be.
const POST_WINDOW_GONE = process.env.SHARKERCU_MOCK_POST_WINDOW_GONE === '1';
// A refusal that carries only 'error', the way the sharker.cu/1 executor emitted it.
const BARE_REFUSAL = process.env.SHARKERCU_MOCK_BARE_REFUSAL === '1';
const REFUSAL_PATH = process.env.SHARKERCU_MOCK_REFUSAL_PATH || 'none';
...
const NO_WOULD_REQUIRE = process.env.SHARKERCU_MOCK_NO_WOULD_REQUIRE === '1';
const REFUSAL_OUTCOME = process.env.SHARKERCU_MOCK_REFUSAL_OUTCOME || 'refused';
const OK_OUTCOME = process.env.SHARKERCU_MOCK_OK_OUTCOME || 'ok';
const SESSION_ERROR = process.env.SHARKERCU_MOCK_SESSION_ERROR || '';
const WINDOW_LIST_ERROR = process.env.SHARKERCU_MOCK_WINDOW_LIST_ERROR || '';
const MALFORMED = process.env.SHARKERCU_MOCK_MALFORMED || '';
const LAUNCH_ERROR = process.env.SHARKERCU_MOCK_LAUNCH_ERROR || '';
const HANG_OBSERVE = process.env.SHARKERCU_MOCK_HANG_OBSERVE === '1';
const TRUNCATED = process.env.SHARKERCU_MOCK_TRUNCATED === '1';
let DIFFERENCE_PRESENTATION = '';
const LAUNCH_TOOK_FOREGROUND = process.env.SHARKERCU_MOCK_LAUNCH_FOREGROUND === '1';
const WINDOW_ORIGIN_Y = Number(process.env.SHARKERCU_MOCK_WINDOW_ORIGIN_Y || '25');
...
function handle(msg) {
  ...
  switch (msg.method) {
    case 'host.hello':
      ...
      ok(id, {
        protocol: PROTOCOL,
        executor: { name: 'sharker-cu-mock', version: '0.0.1', commit: 'testing' },
        pid: process.pid,
        capabilities: {
          captureStream: false,
...
// Same rationale as sharker-cu-service.test.ts: the mock appends to its log from
...
function makeBackend(
  opts: {
    ...
    physicalInputRecentlyActive?: SharkerCuBackendOptions['physicalInputRecentlyActive'];
    allowCompatibilityInputDispatch?: boolean;
    onTrace?: SharkerCuBackendOptions['onTrace'];
  } = {},
): { backend: ReturnType<typeof createSharkerCuBackend>; logPath: string; imageDir: string } {
  ...
  process.env.SHARKERCU_MOCK_LOG = logPath;
  process.env.SHARKERCU_MOCK_PROTOCOL = opts.protocol ?? 'sharker.cu/2';
  process.env.SHARKERCU_MOCK_DISPATCH_ERROR = opts.dispatchError ?? '';
  process.env.SHARKERCU_MOCK_TIER = opts.tier ?? 'ax';
  process.env.SHARKERCU_MOCK_PATH = opts.path ?? 'ax_action';
  process.env.SHARKERCU_MOCK_BAD_IMAGE_SHA = opts.badImageSha ? '1' : '';
  process.env.SHARKERCU_MOCK_BIG_IMAGE = opts.bigImage ? '1' : '';
  process.env.SHARKERCU_MOCK_BARE_IMAGE_SHA = opts.bareImageSha ? '1' : '';
  process.env.SHARKERCU_MOCK_NO_POST_SNAPSHOT = opts.noPostSnapshot ? '1' : '';
  process.env.SHARKERCU_MOCK_UNVERIFIED_EFFECT = opts.unverifiedEffect ? '1' : '';
  process.env.SHARKERCU_MOCK_POST_WINDOW_GONE = opts.postWindowGone ? '1' : '';
  process.env.SHARKERCU_MOCK_BARE_REFUSAL = opts.bareRefusal ? '1' : '';
  process.env.SHARKERCU_MOCK_REFUSAL_PATH = opts.refusalPath ?? 'none';
  process.env.SHARKERCU_MOCK_NO_WOULD_REQUIRE = opts.noWouldRequirePath ? '1' : '';
  process.env.SHARKERCU_MOCK_REFUSAL_OUTCOME = opts.refusalOutcome ?? 'refused';
  process.env.SHARKERCU_MOCK_OK_OUTCOME = opts.okOutcome ?? 'ok';
  process.env.SHARKERCU_MOCK_SESSION_ERROR = opts.sessionError ?? '';
  process.env.SHARKERCU_MOCK_WINDOW_LIST_ERROR = opts.windowListError ?? '';
  process.env.SHARKERCU_MOCK_MALFORMED = opts.malformed ?? '';
  process.env.SHARKERCU_MOCK_LAUNCH_ERROR = opts.launchError ?? '';
  process.env.SHARKERCU_MOCK_HANG_OBSERVE = opts.hangObserve ? '1' : '';
  process.env.SHARKERCU_MOCK_TRUNCATED = opts.truncated ? '1' : '';
  process.env.SHARKERCU_MOCK_LAUNCH_FOREGROUND = opts.launchTookForeground ? '1' : '';
  process.env.SHARKERCU_MOCK_WINDOW_ORIGIN_Y = String(opts.windowOriginY ?? 25);
  const backend = createSharkerCuBackend({
    binaryPath: mockPath,
    imageDir,
...
async function observeFixture(
  backend: ReturnType<typeof createSharkerCuBackend>,
...
before(async () => {
  workDir = await mkdtemp(join(tmpdir(), 'sharker-cu-test-'));
  mockPath = join(workDir, 'sharker-cu-mock.cjs');
...
describe('sharker-cu backend', () => {
  it('opens with host.hello and refuses to move the system cursor', async () => {
    ...
    assert.equal(hello?.protocol, 'sharker.cu/2');
  ...
  it('fails loudly on a protocol version mismatch and does not retry', async () => {
    const { backend, logPath } = makeBackend({ protocol: 'sharker.cu/99' });
  ...
  it('rejects a tier/path pair outside the declared table instead of coercing it', async () => {
    ...
    assert.doesNotMatch(message, /tier|path|sharker\.cu/);
  ...
  it('keeps executor diagnostics out of what the model reads', async () => {
    // `sharker-cu dispatch.element: Invalid params (-32602)` was reaching the
    ...
    assert.doesNotMatch(
      message,
      /dispatch\.element|sharker-cu|sharker\.cu|wormhole_collapsed|-?32\d\d\d/,
    );
  ...
  it('accepts a refusal that carries the four declared fields and keeps the executor', async () => {
    ...
    assert.equal(result.outcome.evidence?.effect, 'unverifiable');
    // §1.1: a refusal is an outcome, not a protocol violation. The sharker.cu/1
...
describe('sharker-cu backend selection', () => {
  it('is reached by being named, and refuses without a pinned digest', () => {
    ...
    const selected = selectComputerUseBackend({
      backendId: 'sharker-cu',
      binaryPath: '/tmp/does-not-matter',
      expectedBinarySha256: 'deadbeef',
      createBackend,
    });
    assert.equal(selected.backendId, 'sharker-cu');
    assert.equal(made, 1);
  ...
  it('is what a caller that names nothing gets', () => {
    ...
    if (process.platform !== 'darwin') return;
    assert.equal(DEFAULT_CU_BACKEND_ID, 'sharker-cu');
    let made = 0;
    const selected = selectComputerUseBackend({
      binaryPath: '/tmp/does-not-matter',
      expectedBinarySha256: 'deadbeef',
...
describe('sharker-cu key chord parsing', () => {
  it('cannot be spelled with an Object.prototype member', () => {
    ...
    // `__proto__` with an object, so `parseSharkerCuKeyChord('constructor')`
    ...
    for (const spelling of ['constructor', '__proto__', 'constructor+a', 'cmd+constructor']) {
      assert.equal(
        parseSharkerCuKeyChord(spelling),
        undefined,
