...
// Unit test for SharkerCuService, the supervisor between the sharker-cu backend and
// the executor child. Drives it against a MOCK executor (a small CommonJS node
// script written to a temp dir) that speaks `sharker.cu/2` — the real `sharker-cu`
...
import { SharkerCuProtocolViolation } from '../sharker-cu-protocol.js';
import {
  isSharkerCuLifecycleError,
  SharkerCuService,
  type SharkerCuLimits,
  type SharkerCuServiceOptions,
} from '../sharker-cu-service.js';

const SANE_LIMITS: SharkerCuLimits = {
...
if (process.argv[2] !== 'host') {
  process.stderr.write('mock sharker-cu: expected argv[2] === "host"\n');
  process.exit(64);
}
const LOG = process.env.SHARKERCU_SVC_LOG || '';
const LIMITS = process.env.SHARKERCU_SVC_LIMITS || '{}';
...
const AFTER_HELLO = process.env.SHARKERCU_SVC_AFTER_HELLO || '';
// Never answer these methods, so the host's deadline and $/cancel are what act.
const SILENT = (process.env.SHARKERCU_SVC_SILENT || '').split(',').filter(Boolean);
// Answer $/cancel with a real refusal, which is what §7.2 asks an executor to do.
const ANSWER_CANCEL = process.env.SHARKERCU_SVC_ANSWER_CANCEL === '1';
// Print this many bytes of a single response, to push the stdout budget.
const RESPONSE_PAD = Number(process.env.SHARKERCU_SVC_RESPONSE_PAD || '0');
function logRec(rec) {
...
function handle(msg) {
  ...
  if (msg.method === 'host.hello') {
    send({ jsonrpc: '2.0', id: msg.id, result: { ok: true, protocol: 'sharker.cu/2',
      executor: { name: 'sharker-cu-mock', version: '0.0.1' }, pid: process.pid,
...
let workDir = '';
let mockPath = '';
const services: SharkerCuService[] = [];
...
function makeService(
  opts: {
    limits?: Partial<SharkerCuLimits>;
    afterHello?: string;
    silent?: string[];
  ...
  const options: SharkerCuServiceOptions = {
    ...
    childEnv: {
      ...process.env,
      SHARKERCU_SVC_LOG: logPath,
      SHARKERCU_SVC_LIMITS: JSON.stringify({ ...SANE_LIMITS, ...(opts.limits ?? {}) }),
      SHARKERCU_SVC_AFTER_HELLO: opts.afterHello ?? '',
      SHARKERCU_SVC_SILENT: (opts.silent ?? []).join(','),
      SHARKERCU_SVC_ANSWER_CANCEL: opts.answerCancel ? '1' : '',
      SHARKERCU_SVC_RESPONSE_PAD: String(opts.responsePad ?? 0),
    },
  };
  const service = new SharkerCuService(options);
...
before(async () => {
  workDir = await mkdtemp(join(tmpdir(), 'sharker-cu-service-test-'));
...
describe('sharker-cu supervisor: what the child may put on stdout', () => {
  it('survives a JSON value that is not a response, and does not answer for it', async () => {
    ...
    try {
      ...
      assert.equal(handshake.executor.name, 'sharker-cu-mock');
...
describe('sharker-cu supervisor: the handshake is a closed contract (§2)', () => {
  const rejected: Array<[string, Partial<SharkerCuLimits>, RegExp]> = [
      ...
      await assert.rejects(
        ...
        (error: unknown) => {
          assert.ok(
            ...
            `expected SharkerCuProtocolViolation, got ${(error as Error)?.constructor?.name}`,
          );
...
describe('sharker-cu supervisor: cancelling a delivered request (§7.2/§7.3)', () => {
  it('settles an aborted delivered request in the cancel grace, not the deadline', async () => {
    ...
    await assert.rejects(call, (error: unknown) => {
      assert.ok(isSharkerCuLifecycleError(error), 'a cancelled dispatch is a lifecycle error');
      return true;
    });
...
describe('sharker-cu supervisor: saying what actually failed', () => {
  it('reports an exec failure as one, with the errno', async () => {
    // sharker-cu is unsigned, hand-built and not distributed, so exec failure is
    ...
    // reported as "sharker-cu exited after request delivery" — for a child that
  ...
  it('reports an absent binary as unavailable rather than a version skew', async () => {
    ...
    await assert.rejects(
      () => service.ensureStarted(),
      (error: unknown) => {
        assert.ok(isSharkerCuLifecycleError(error, 'service_unavailable'));
  ...
  it('keeps saying why after the restart budget is spent', async () => {
    ...
    // loop-local: "sharker-cu restart budget exhausted: undefined", for the rest of
...
describe('sharker-cu supervisor: shutdown', () => {
  it('leaves no image directory behind when disposed during startup', async () => {
    ...
    // has this guard and sharker-cu's copy dropped it.
