...
// trycua/cua-driver (MCP) and sharker-cu (sharker.cu/1). Both frame one JSON value per
...
// cua-driver kills the child to cancel a delivered request, sharker-cu sends
// `$/cancel` and waits for the executor's own answer (sharker.cu/1 §7.2), and the
...
export interface JsonLineDecoderHandlers {
  ...
  /**
   * A line that is not JSON. sharker.cu/1 §1 makes this a protocol violation the
