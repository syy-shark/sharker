...
export const SESSION_SNAPSHOT_WORKSPACE_POLICY_V1: SessionSnapshotWorkspacePolicy = Object.freeze({
  ...
  classify(entry: SessionSnapshotWorkspaceEntry): SessionSnapshotWorkspacePolicyDecision {
    ...
    if (
      ...
      lowerSegments.includes('.sharker-cache') ||
      lowerSegments.includes('.turbo')
    ) {
      return { kind: 'exclude', category: 'cache' };
    }
    if (lowerSegments.includes('.sharker-runtime') || lowerSegments.includes('.sharker-activation')) {
 ...
 * mutations for this Sharker Session, drain already-admitted state, Artifact and
...
export interface SessionSnapshotQuiescenceAuthority {
  runQuiescent<T>(
    input: {
      readonly sharkerSessionId: string;
 ...
 * Sharker Session, and bind the grant to the exact policy version, normalized
    ...
    return this.#cleanup.ownCreation(
      {
        ...
        kind: 'revision',
        sourceSessionId: normalized.sharkerSessionId,
...
class FileQuiescentSessionSnapshotCoordinator implements QuiescentSessionSnapshotCoordinator {
  ...
  async prepare(input: PrepareQuiescentSessionSnapshotInput): Promise<PreparedSessionBundleHandle> {
    const sharkerSessionId = requireSharkerSessionId(input.sharkerSessionId);
    ...
    try {
      ...
      const result = await this.#quiescence.runQuiescent(
        { sharkerSessionId, cancellation: cancellation.value },
        async () => {
          if (operationStarted) {
...
function requireSharkerSessionId(value: unknown): string {
  if (typeof value !== 'string' || !isSafeSessionId(value)) {
    throw new SessionSnapshotError('invalid_input', 'Sharker Session identity is invalid', {
      details: { phase: 'admission' },
    });
  }
  return value;
...
function normalizeSnapshotStagingLease(
  ...
  return Object.freeze({
    ...
    sharkerSessionId: requireSharkerSessionId(value.sharkerSessionId),
  });
}
