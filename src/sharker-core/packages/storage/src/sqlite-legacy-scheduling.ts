...
} from '@sharker/core/scheduled-task';
...
export function planLegacyScheduledTasks(
  ...
  if (tasks.length > SCHEDULED_TASK_CATALOG_MAX_ITEMS) {
    throw new Error(
      `Released scheduling catalog has ${tasks.length} records, exceeding the supported ${SCHEDULED_TASK_CATALOG_MAX_ITEMS}; reopen this workspace with the previous Sharker release and reduce the catalog before upgrading`,
    );
  }
  return tasks;
}
...
function assertLegacyAutomationEmpty(database: DatabaseSync): void {
  ...
  if (definition) {
    throw new Error(
      `Released Automation ${String(definition.automation_id)} cannot be migrated without losing its configuration; reopen this workspace with the previous Sharker release and remove or export Automation before upgrading`,
    );
  }
