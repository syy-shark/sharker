 ...
 *      capability — that contract exists for Sharker's own workspace; foreign
 ...
 *      reader at ~/.ssh.
 *   3. BOUNDED. Byte caps from @sharker/core/foreign-session apply to every
...
export function isClaudeCodeImportEnabled(
  ...
  return env.SHARKER_IMPORT_CLAUDE_CODE !== '0';
}

export function isCodexImportEnabled(
  env: Record<string, string | undefined> = process.env,
): boolean {
  return env.SHARKER_IMPORT_CODEX !== '0';
}
