...
const MANAGED_DEPENDENCY_IDENTITY_DOMAIN = 'sharker.managed_dependency_environment.v1\0';
const MANAGED_DEPENDENCY_TREE_DOMAIN = 'sharker.managed_dependency_environment.tree.v1\0';
...
const MANAGED_DEPENDENCY_PRODUCER_POLICY_DOMAIN =
  'sharker.managed_dependency_environment.producer_policy.v1\0';
const MANAGED_DEPENDENCY_PRODUCER_POLICY_V1 = Object.freeze({
  ...
  filesystem: 'sharker_owned_staging_only' as const,
...
public static class SharkerWindowsStreamQuery
{
...
foreach ($path in $paths) {
  if ([SharkerWindowsStreamQuery]::HasAlternateDataStream([string]$path)) {
...
export interface ManagedDependencyEnvironmentProducerCapabilityV1 {
  ...
  readonly filesystem: 'sharker_owned_staging_only';
...
async function openOrPublishEnvironment(input: {
  ...
  const scratchRoot = join(projectRoot, '.sharker-runtime');
