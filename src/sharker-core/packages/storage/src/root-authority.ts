...
export const STORAGE_ROOT_MARKER_FILE = '.sharker-storage-root.json';
...
export function resolveRootControlNamespace(): string {
  try {
    ...
    if (process.platform === 'darwin') {
      return join(accountHome, 'Library', 'Caches', 'Sharker', 'runtime-hosts');
    }
    if (process.platform === 'win32') {
      return join(accountHome, 'AppData', 'Local', 'Sharker', 'runtime-hosts');
    }
    return join(accountHome, '.cache', 'sharker', 'runtime-hosts');
  } catch (error) {
...
export function resolveRootOwnershipNamespace(): string {
  try {
    ...
    if (process.platform === 'darwin') {
      return join(accountHome, 'Library', 'Application Support', 'Sharker', 'state-root-owners');
    }
    if (process.platform === 'win32') {
      return join(accountHome, 'AppData', 'Local', 'Sharker', 'state-root-owners');
    }
    return join(accountHome, '.local', 'share', 'Sharker', 'state-root-owners');
  } catch (error) {
    throw normalizeAuthorityFailure(
      error,
      'control_io_failed',
