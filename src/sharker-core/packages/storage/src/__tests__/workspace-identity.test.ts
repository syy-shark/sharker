...
test('a new workspace marker contains only its schema version and UUID', async () => {
  const workspace = await mkdtemp(join(tmpdir(), 'sharker-workspace-marker-shape-'));
  try {
...
test('creating a workspace identity keeps a Git worktree clean', async () => {
  const workspace = await mkdtemp(join(tmpdir(), 'sharker-workspace-git-clean-'));
  try {
...
test('an existing workspace marker becomes locally ignored after Git initialization', async () => {
  const workspace = await mkdtemp(join(tmpdir(), 'sharker-workspace-git-existing-'));
  try {
...
test('a subdirectory workspace stays clean inside a linked Git worktree', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-workspace-git-linked-'));
  try {
...
test('repeated Git workspace resolution adds one local exclude rule', async () => {
  const workspace = await mkdtemp(join(tmpdir(), 'sharker-workspace-git-idempotent-'));
  try {
    await execFileAsync('git', ['init', '--quiet'], { cwd: workspace });
...
test('concurrent Git workspace resolution returns one clean identity', async () => {
  const workspace = await mkdtemp(join(tmpdir(), 'sharker-workspace-git-concurrent-'));
  try {
    await execFileAsync('git', ['init', '--quiet'], { cwd: workspace });
...
test('a full Git exclude does not grow when resolving workspace identity', async () => {
  const workspace = await mkdtemp(join(tmpdir(), 'sharker-workspace-git-exclude-full-'));
  try {
    await execFileAsync('git', ['init', '--quiet'], { cwd: workspace });
    const { stdout } = await execFileAsync(
      'git',
...
test('a malformed enclosing Git repository prevents publishing a new marker', async () => {
  const workspace = await mkdtemp(join(tmpdir(), 'sharker-workspace-git-malformed-'));
  try {
    await writeFile(join(workspace, '.git'), 'gitdir: /missing/sharker-git-dir\n', 'utf8');
...
test('a non-Git workspace resolves when the Git executable is unavailable', async () => {
  const workspace = await mkdtemp(join(tmpdir(), 'sharker-workspace-no-git-required-'));
  try {
...
test('a Git workspace does not publish a marker when the Git executable is unavailable', async () => {
  const workspace = await mkdtemp(join(tmpdir(), 'sharker-workspace-git-unavailable-'));
  try {
    await execFileAsync('git', ['init', '--quiet'], { cwd: workspace });
...
test('an unmarked read-only workspace fails without leaving marker state', {
  ...
  const workspace = await mkdtemp(join(tmpdir(), 'sharker-workspace-read-only-'));
  try {
...
test('a tar archive round-trip preserves workspace identity', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-workspace-identity-'));
  try {
...
test('importing an existing marker into a Git worktree keeps it clean', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-workspace-import-git-'));
  try {
    const source = join(base, 'source');
    const imported = join(base, 'imported');
    await mkdir(source);
