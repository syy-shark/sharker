...
test('rejects a second authority for the same storage root in another process', async (t) => {
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-owner-process-'));
  const child = spawn(process.execPath, [ownerChildEntrypoint], {
    env: { ...process.env, SHARKER_DEPENDENCY_OWNER_ROOT: storageRoot },
...
for (const failpoint of [
  ...
  test(`converges after process exit at ${failpoint}`, async (t) => {
    const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-crash-'));
    t.after(() => rm(storageRoot, { recursive: true, force: true }));
    await assert.rejects(
      execFileAsync(process.execPath, [childEntrypoint], {
        env: {
          ...process.env,
          SHARKER_DEPENDENCY_CRASH_ROOT: storageRoot,
          SHARKER_DEPENDENCY_CRASH_POINT: failpoint,
        },
        windowsHide: true,
      }),
