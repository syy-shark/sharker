...
test('State Root composition binding is durable, idempotent, and exclusive', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-state-root-composition-'));
  try {
    ...
    assert.ok(owner);
    try {
      await bindStateRootComposition(owner.lease, 'sharker.interactive');
      ...
      await assert.rejects(
        () => bindStateRootComposition(owner.lease, 'sharker.batch-test'),
        (error: unknown) =>
...
test('State Root composition binding rejects a corrupt existing authority record', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-state-root-composition-corrupt-'));
  try {
    const capability = trackControlDirectory(
      await resolveStorageRoot({ path: root, kind: 'interactive' }),
    );
    ...
    try {
      await writeFile(
        ...
        '{"schemaVersion":1,"compositionId":"sharker.interactive","extra":true}\n',
      );
      await assert.rejects(
        () => bindStateRootComposition(owner.lease, 'sharker.interactive'),
        (error: unknown) =>
