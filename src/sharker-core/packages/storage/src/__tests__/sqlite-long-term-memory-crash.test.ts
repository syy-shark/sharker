...
import type { MemoryItemWrite } from '@sharker/core/long-term-memory';
import {
  SqliteMemoryItemStore,
  type SqliteMemoryItemStoreFailpoint,
} from '../sqlite-long-term-memory-store.js';

const childMode = process.env.SHARKER_LONG_TERM_MEMORY_CRASH_CHILD;
...
if (childMode) {
  ...
  test('retains one committed Item and its receipt when killed between COMMIT and return', {
    ...
    const root = await mkdtemp(join(tmpdir(), 'sharker-long-term-memory-crash-'));
    const databasePath = join(root, 'memory.sqlite');
    const child = spawn(process.execPath, [fileURLToPath(import.meta.url)], {
      env: {
        ...process.env,
        SHARKER_LONG_TERM_MEMORY_CRASH_CHILD: 'after_commit',
        SHARKER_LONG_TERM_MEMORY_CRASH_DB: databasePath,
      },
...
async function runCrashChild(mode: string): Promise<void> {
  ...
  const databasePath = requiredEnv('SHARKER_LONG_TERM_MEMORY_CRASH_DB');
