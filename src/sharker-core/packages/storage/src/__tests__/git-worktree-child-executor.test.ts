...
describe('Git worktree child executor', () => {
  ...
  test('reuses the durable branch/path binding across executor restart and child edits', async () => {
    ...
    await git(first.worktreePath, 'switch', '-c', 'sharker/issue-3-a-contract');
    ...
    await git(
      ...
      'config',
      '--local',
      `branch.${first.branch}.sharker-worktree-lease`,
...
async function temporaryRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'sharker-worktree-child-'));
  cleanup.push(root);
  return root;
}
