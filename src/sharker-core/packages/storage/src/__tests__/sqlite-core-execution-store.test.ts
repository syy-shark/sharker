...
import type { InteractionCanonicalOutcome, InteractionRequest } from '@sharker/core/interaction';
...
async function withRoot(run: (root: string) => Promise<void>): Promise<void> {
  const root = await mkdtemp(join(tmpdir(), 'sharker-sqlite-execution-'));
  try {
    await run(root);
  } finally {
...
function runComposition(seed: string): NonNullable<AgentRunHeader['runComposition']> {
  return {
    schemaVersion: 1,
    composerId: 'sharker.interactive',
    composerRevision: '1',
    sourceRevisions: [
