...
describe('SQLite SessionStore', () => {
  test('requires the reserved WorkHub Coordination identity and role together', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-workhub-coordination-identity-role-'));
  ...
  test('keeps the WorkHub Coordination Session durable but outside ordinary catalogs and route candidates', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-workhub-coordination-session-'));
    const store = createSessionStore(root);
    try {
  ...
  test('quarantines the reserved Coordination identity when its role is missing', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-workhub-coordination-missing-role-'));
    const store = createSessionStore(root);
    try {
  ...
  test('ordinary Sessions have no external origin and provenance metadata is immutable', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-external-origin-'));
    const store = createSessionStore(root);
    try {
  ...
  test('folds retired Session and transcript values only on persisted reads', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-persisted-decode-'));
    const store = createSessionStore(root);
  ...
  test('looks up complete published import counts with bounded newest Session ids', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-external-origin-lookup-'));
    const store = createSessionStore(root);
  ...
  test('bounds external import lookup source and recent-id requests', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-external-origin-bounds-'));
    const store = createSessionStore(root);
    try {
      await assert.rejects(
  ...
  test('fails closed when persisted external origin metadata is malformed', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-external-origin-invalid-'));
    const store = createSessionStore(root);
    let sessionId = '';
    try {
  ...
  test('persists session metadata and messages in one SQLite authority', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-sqlite-'));
    const store = createSessionStore(root);
    try {
      const session = await store.create(makeInput());
  ...
  test('keeps staging imports outside the catalog pagination domain', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-staging-catalog-'));
    const store = createSessionStore(root);
    try {
  ...
  test('a generated title fills an absence and never overwrites a rename', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-generated-title-'));
    const store = createSessionStore(root);
    try {
  ...
  test('a Session freezes its route on the first user message, a subagent at birth', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-route-freeze-'));
    const store = createSessionStore(root);
    try {
  ...
  test('appending the first user message locks the session before any read', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-lock-heal-'));
    const store = createSessionStore(root);
    try {
      const session = await store.create(makeInput());
      await store.appendMessage(session.id, {
  ...
  test('commits message and catalog projection atomically', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-atomic-'));
    const store = createSessionStore(root);
    try {
      const session = await store.create(makeInput());
  ...
  test('pages the durable transcript by sequence, bytes, and a fixed watermark', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-transcript-pages-'));
    const store = createSessionStore(root);
    try {
      const session = await store.create(makeInput());
  ...
  test('pages both new chunked messages and legacy inline v22 records', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-transcript-chunks-'));
    const message = {
      type: 'user' as const,
      id: 'message-large',
      turnId: 'turn-large',
  ...
  test('rejects corrupt chunked messages on paged and ordinary reads', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-transcript-corruption-'));
    const store = createSessionStore(root);
    let sessionId = '';
    try {
      const session = await store.create(makeInput());
  ...
  test('bounds transcript identity reconciliation before message materialization', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-transcript-reconciliation-'));
    const store = createSessionStore(root);
    try {
      const session = await store.create(makeInput());
  ...
  test('notifies transcript observers only after successful durable appends', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-transcript-observer-'));
    const store = createSessionStore(root);
    try {
      const session = await store.create(makeInput());
  ...
  test('pages turn contributions at a fixed transcript watermark', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-turn-contributions-'));
    const store = createSessionStore(root);
    try {
      const session = await store.create(makeInput());
      await store.appendMessages(session.id, [
  ...
  test('bounds turn contribution source scanning independently of turn count', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-turn-source-bound-'));
    const store = createSessionStore(root);
    try {
      const session = await store.create(makeInput());
  ...
  test('samples a bounded prompt landmark index across the durable transcript', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-turn-landmarks-'));
    const store = createSessionStore(root);
    try {
      const session = await store.create(makeInput());
      await store.appendMessages(
  ...
  test('keeps every prompt landmark when long turns fit within the landmark limit', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-turn-landmarks-long-turns-'));
    const store = createSessionStore(root);
    try {
      const session = await store.create(makeInput());
      await store.appendMessages(
  ...
  test('keeps legacy prompts when newer turns have indexed admissions', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-turn-landmarks-mixed-'));
    const store = createSessionStore(root);
    try {
      const session = await store.create(makeInput());
      await store.appendMessages(
  ...
  test('clears unread when the current read marker is already the latest visible message', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-read-marker-'));
    const store = createSessionStore(root);
    try {
      const session = await store.create(makeInput());
      await store.appendMessage(session.id, {
  ...
  test('reads back a legacy fake-backend session instead of migrating or rejecting it', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-legacy-fake-'));
    const store = createSessionStore(root);
    let sessionId: string;
    try {
  ...
  test('persists an immutable Connection identity when supplied by Host admission', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-connection-identity-'));
    const store = createSessionStore(root);
    try {
  ...
  test('deletes metadata and messages through the same transaction boundary', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-session-delete-'));
    const store = createSessionStore(root);
    try {
      const session = await store.create(makeInput());
      await store.appendMessage(session.id, {
