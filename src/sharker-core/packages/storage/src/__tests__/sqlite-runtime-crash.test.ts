...
const childMode = process.env.SHARKER_SQLITE_CRASH_CHILD;
...
async function withKilledChild(
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-sqlite-crash-'));
  const dbPath = join(root, 'runtime.sqlite');
  const markerPath = join(root, 'effect.marker');
  const child = spawn(process.execPath, [fileURLToPath(import.meta.url)], {
    env: {
      ...process.env,
      SHARKER_SQLITE_CRASH_CHILD: mode,
      SHARKER_SQLITE_CRASH_DB: dbPath,
      SHARKER_SQLITE_CRASH_MARKER: markerPath,
    },
...
async function runCrashChild(mode: string): Promise<void> {
  const dbPath = requiredEnv('SHARKER_SQLITE_CRASH_DB');
  const markerPath = requiredEnv('SHARKER_SQLITE_CRASH_MARKER');
