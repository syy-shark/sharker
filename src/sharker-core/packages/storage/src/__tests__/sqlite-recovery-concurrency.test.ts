...
import type { WorkspaceBaselineAuthorityInput } from '@sharker/core/workspace-version-authority';
import { canonicalToolArgsHash } from '@sharker/core/tool-args-identity';
import { scanToolLedger } from '@sharker/core/tool-ledger-scanner';
import {
...
const RUN_RACE_AMPLIFICATION = process.env.SHARKER_STORAGE_STRESS === '1';
...
describe('SQLite recovery authority multi-process races', () => {
  ...
  it('allows concurrent operational owners to initialize the same fresh WAL database', async () => {
    ...
    // storage stress route (SHARKER_STORAGE_STRESS=1) alongside the other
    // multi-process probes, and out of every ordinary run.
    const rounds = RUN_RACE_AMPLIFICATION ? 12 : 1;
    for (let round = 0; round < rounds; round += 1) {
      const root = await mkdtemp(join(tmpdir(), 'sharker-operational-fresh-open-race-'));
  ...
  it('serializes concurrent operational runtime migration', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-operational-migration-race-'));
    const dbPath = join(root, 'runtime.sqlite');
    const startPath = join(root, 'start');
    try {
  ...
  it('captures a fast worker initialization failure after releasing the barrier', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-recovery-race-invalid-db-'));
    try {
...
async function withPreparedDatabase(
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-recovery-race-'));
  const dbPath = join(root, 'runtime.sqlite');
  const startPath = join(root, 'start');
  const store = createSqliteRuntimeStore(dbPath);
  try {
...
function startWorker(
  ...
  const child = spawn(
    ...
    [fileURLToPath(new URL('./fixtures/sqlite-recovery-concurrency-child.js', import.meta.url))],
    {
      env: {
        ...process.env,
        SHARKER_SQLITE_RECOVERY_CONCURRENCY_MODE: mode,
        SHARKER_SQLITE_RECOVERY_CONCURRENCY_DB: dbPath,
        SHARKER_SQLITE_RECOVERY_CONCURRENCY_START: startPath,
        ...(stopPath ? { SHARKER_SQLITE_RECOVERY_CONCURRENCY_STOP: stopPath } : {}),
      },
