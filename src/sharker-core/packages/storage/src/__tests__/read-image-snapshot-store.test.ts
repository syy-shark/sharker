...
import { ReadImageSnapshotStoreError, type ContextOffloadLimits } from '@sharker/core/context-offload';
import {
...
async function withInteractiveOwner(run: (owner: InteractiveRootOwner) => Promise<void>) {
  const root = await mkdtemp(join(tmpdir(), 'sharker-read-image-context-store-'));
