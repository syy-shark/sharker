...
test('storage writer composition rejects reuse until close completes', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-storage-composition-'));
  try {
...
test('opening a second storage writer composition creates a usable lifecycle', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-storage-composition-failure-'));
  try {
    const capability = trackControlDirectory(
      await resolveStorageRoot({ path: root, kind: 'interactive' }),
    );
...
test('context-offload authority is optional and participates in composition close', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-storage-context-composition-'));
  try {
    const capability = trackControlDirectory(
      await resolveStorageRoot({ path: root, kind: 'interactive' }),
    );
...
test('an unavailable context-offload authority does not fail the storage composition', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-storage-context-unavailable-'));
  try {
...
test('a failed close keeps the lease unavailable', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-storage-composition-close-failure-'));
  try {
    const capability = trackControlDirectory(
      await resolveStorageRoot({ path: root, kind: 'interactive' }),
    );
...
test('a failed runtime-policy hook rolls back before reopening', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-storage-composition-hook-failure-'));
  try {
    const capability = trackControlDirectory(
      await resolveStorageRoot({ path: root, kind: 'interactive' }),
    );
