...
test('atomically reapplies current owner schema without republishing its registry', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-current-convergence-'));
...
test('retires completed released migration metadata during schema convergence', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-cutover-retirement-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
...
test('preserves an interrupted released cutover journal and fails closed', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-cutover-interrupted-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
...
test('preserves a cutover journal with an unfamiliar column shape and fails closed', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-cutover-shape-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    acquireOperationalStateDatabase(root).close();
    const legacy = new DatabaseSync(databasePath);
...
test('preserves a cutover journal with an altered constraint and fails closed', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-cutover-constraint-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    acquireOperationalStateDatabase(root).close();
    const legacy = new DatabaseSync(databasePath);
...
test('preserves a cutover journal carrying an extra trigger and fails closed', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-cutover-trigger-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    acquireOperationalStateDatabase(root).close();
    const legacy = new DatabaseSync(databasePath);
...
test('preserves a cutover journal naming an unknown store and fails closed', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-cutover-unknown-store-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    acquireOperationalStateDatabase(root).close();
    const legacy = new DatabaseSync(databasePath);
...
test('preserves a completed row carrying an unfamiliar validation key and fails closed', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-cutover-extra-key-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    acquireOperationalStateDatabase(root).close();
    const legacy = new DatabaseSync(databasePath);
...
test('preserves a completed row missing a released validation key and fails closed', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-cutover-missing-key-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    acquireOperationalStateDatabase(root).close();
    const legacy = new DatabaseSync(databasePath);
...
test('preserves a malformed released import source and fails closed', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-import-malformed-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    acquireOperationalStateDatabase(root).close();
    const legacy = new DatabaseSync(databasePath);
...
test('preserves a session import source with a missing session and fails closed', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-import-missing-session-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    acquireOperationalStateDatabase(root).close();
    const legacy = new DatabaseSync(databasePath);
...
test('rolls back every scope when migration publication fails', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-rollback-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
...
test('migrates released Reminder state after Automation is retired', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-v016-'));
  try {
    const databasePath = join(root, 'runtime.sqlite');
    await copyV016Database(databasePath);
    const legacy = new DatabaseSync(databasePath);
...
test('finishes a released cleanup backfill interrupted after adding its column', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-interrupted-cleanup-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    await copyV016Database(databasePath);
    const legacy = new DatabaseSync(databasePath);
...
test('leaves released Automation unchanged when its configuration cannot be preserved', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-v016-automation-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    await copyV016Database(databasePath);
...
test('rejects legacy Automation tables without their registry authority', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-v016-missing-automation-scope-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    await copyV016Database(databasePath);
    const legacy = new DatabaseSync(databasePath);
...
test('rejects a released Workflow registry whose reminder table is missing', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-v016-missing-reminders-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    await copyV016Database(databasePath);
    const legacy = new DatabaseSync(databasePath);
...
test('rejects a released Reminder table after its Workflow authority removed it', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-stale-reminders-'));
  try {
    const lease = acquireOperationalStateDatabase(root);
...
test('cleans the known removed Automation v2 scope', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-automation-v2-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    await copyV016Database(databasePath);
    const legacy = new DatabaseSync(databasePath);
...
test('leaves an oversized released scheduling catalog unchanged', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-oversized-catalog-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    await copyV016Database(databasePath);
    const legacy = new DatabaseSync(databasePath);
...
test('does not classify a SQLite write failure as a migration blocker', {
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-readonly-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    await copyV016Database(databasePath);
    const legacy = new DatabaseSync(databasePath);
...
test('rejects a newer scope before migrating an older scope', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-mixed-version-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    const lease = acquireOperationalStateDatabase(root);
    lease.close();
...
test('rejects a newer runtime schema without changing the database', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-newer-runtime-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    const lease = acquireOperationalStateDatabase(root);
    lease.close();
...
test('rejects newer session metadata before migrating older runtime state', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-newer-metadata-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    const lease = acquireOperationalStateDatabase(root);
    lease.close();
...
test('rejects an unknown operational schema without changing the database', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-unknown-scope-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    const lease = acquireOperationalStateDatabase(root);
    lease.close();
    ...
    assert.throws(
      () => acquireOperationalStateDatabase(root),
      /Operational schema future_scope is unknown to this Sharker build/,
    );
...
test('rejects an invalid registered schema version before migrating', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-operational-invalid-version-'));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    const lease = acquireOperationalStateDatabase(root);
    lease.close();
...
async function assertReleasedReminderRejected(
  ...
  const root = await mkdtemp(join(tmpdir(), `sharker-operational-v016-${name}-`));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    await copyV016Database(databasePath);
    const database = new DatabaseSync(databasePath);
...
async function assertCurrentDatabaseRejected(
  ...
  const root = await mkdtemp(join(tmpdir(), `sharker-operational-${name}-`));
  const databasePath = join(root, 'runtime.sqlite');
  try {
    acquireOperationalStateDatabase(root).close();
    const database = new DatabaseSync(databasePath);
