...
async function tempHome(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'sharker-foreign-'));
}
...
describe('foreign session store — enable flags', () => {
  it('defaults on, disabled by exactly "0"', () => {
    ...
    assert.equal(isClaudeCodeImportEnabled({ SHARKER_IMPORT_CLAUDE_CODE: '0' }), false);
    assert.equal(isCodexImportEnabled({ SHARKER_IMPORT_CODEX: '1' }), true);
    assert.equal(isCodexImportEnabled({ SHARKER_IMPORT_CODEX: '0' }), false);
  });
  ...
  it('reports only sources that are enabled AND present on disk', async () => {
    ...
    const disabled = createForeignSessionStore({
      homeDir: home,
      env: { SHARKER_IMPORT_CLAUDE_CODE: '0' },
    });
