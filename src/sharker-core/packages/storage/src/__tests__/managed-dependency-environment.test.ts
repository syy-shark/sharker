...
test('rejects a producer that does not declare the exact hermetic capability', async (t) => {
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-capability-'));
...
test('rejects a second authority for the same storage root in one process', async (t) => {
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-single-owner-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
  const producer = {
...
test('rejects a published environment whose dependency content was modified', async (t) => {
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-tamper-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
  const producer = {
    capability: FIXTURE_PRODUCER_CAPABILITY,
    packageManagerName: 'npm' as const,
...
test('keeps the receipt in a constrained authority outside the producer-owned artifact domain', async (t) => {
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-receipt-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
  const producer = {
    capability: FIXTURE_PRODUCER_CAPABILITY,
    packageManagerName: 'npm' as const,
...
test('rejects a coordinated artifact and co-located receipt rewrite', async (t) => {
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-coordinated-tamper-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
  const producer = {
    capability: FIXTURE_PRODUCER_CAPABILITY,
    packageManagerName: 'npm' as const,
...
test('rejects an environment id that is not the digest of the requested identity', async (t) => {
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-identity-forgery-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
  const producer = {
    capability: FIXTURE_PRODUCER_CAPABILITY,
    packageManagerName: 'npm' as const,
...
test('rejects an NTFS alternate stream created inside a dependency artifact', {
  ...
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-ads-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
  const producer = {
    capability: FIXTURE_PRODUCER_CAPABILITY,
    packageManagerName: 'npm' as const,
...
test('rejects an NTFS alternate stream attached to the published dependency root', {
  ...
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-root-ads-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
  const producer = {
    capability: FIXTURE_PRODUCER_CAPABILITY,
    packageManagerName: 'npm' as const,
...
test('rejects an NTFS alternate stream attached to a published nested directory', {
  ...
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-目录-ads-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
  const producer = {
    capability: FIXTURE_PRODUCER_CAPABILITY,
    packageManagerName: 'npm' as const,
...
test('accepts a POSIX package bin symlink whose target remains inside the dependency root', {
  ...
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-bin-link-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
  const producer = {
    capability: FIXTURE_PRODUCER_CAPABILITY,
    packageManagerName: 'npm' as const,
...
test('publishes authority-owned file inodes instead of producer-owned inodes', async (t) => {
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-inode-handoff-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
...
test('seals the complete authority-owned tree before publishing its receipt', async (t) => {
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-durable-tree-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
...
test('publishes Windows read-only dependency files without changing their final mode', {
  ...
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-readonly-file-'));
...
test('isolates published POSIX content from a producer-retained writable handle', {
  ...
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-retained-handle-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
...
test('publishes one Sharker-owned artifact for concurrent equivalent acquisitions', async (t) => {
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-environment-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
    ...
    producer: {
      ...
      async provision(input) {
        ...
        await writeFile(
          ...
          'export const source = "sharker-owned";\n',
          'utf8',
        );
      },
    },
...
test('close drains an acquisition through lease installation before deciding its outcome', async (t) => {
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-close-drain-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
...
test('collects the least-recently-used unleased environment under the cache quota', async (t) => {
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-gc-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
  let provisionCalls = 0;
  const producer = {
    capability: FIXTURE_PRODUCER_CAPABILITY,
...
test('does not collect a published environment while its acquisition is still pending', async (t) => {
  const storageRoot = await mkdtemp(join(tmpdir(), 'sharker-dependency-pending-gc-'));
  t.after(() => rm(storageRoot, { recursive: true, force: true }));
