 ...
 *
 * This list is the package's SQLite boundary, stated out loud. `@sharker/storage`
 ...
 * `sharker --help` included — load SQLite.
 *
...
/** Collects every `@sharker/storage` import specifier in the repository's sources, once. */
...
async function runStorageImportScan(): Promise<StorageImportScan> {
  ...
  const specifierPattern =
    /(?:\bfrom\s*|\bimport\s*\(?\s*|\brequire\s*\(\s*)['"]@sharker\/storage(\/[^'"]*)?['"]/gu;
...
test('no source file imports the retired bare specifier', { timeout: 60_000 }, async () => {
  ...
  assert.deepEqual(
    bareImporters,
    [],
    'bare `@sharker/storage` imports resolve to the removed `.` entrypoint and fail at runtime',
  );
});
