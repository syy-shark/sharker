...
const roots: string[] = [];
const identityBytes = Buffer.from('{"schemaVersion":1,"sharkerSessionId":"sharker-α"}', 'utf8');
...
test('a process crash leaves owned staging that can be reclaimed without touching decoys', {
  ...
  const stagingPrefix = '.crash-hydrated.sharker-session-bundle-staging-';
  const child = spawn(
    process.execPath,
    [childPath, archivePath, artifact.archiveDigest, JSON.stringify(limits), destinationRoot],
    { stdio: ['ignore', 'pipe', 'pipe'] },
  ...
  try {
    ...
    const unrelatedRoot = await mkdtemp(join(tmpdir(), 'sharker-session-bundle-unrelated-'));
...
test('removes an ownership record when its initialization write fails', async () => {
  ...
  assert.deepEqual(
    (await readdir(fixture.root)).filter((name) =>
      name.startsWith('.owner-write-failure-hydrated.sharker-session-bundle-staging-'),
    ),
    [],
  );
});
...
test('rejects corrupted payloads and transport mismatches without publishing hydration', async () => {
  ...
  await assert.rejects(lstat(destinationRoot), { code: 'ENOENT' });
  assert.equal(
    (await readdir(fixture.root)).some((name) => name.includes('.sharker-session-bundle-staging-')),
    false,
  );
...
test('pack enforces caller quotas and cleans unpublished output', async () => {
  ...
  assert.equal(
    (await readdir(fixture.root)).some((name) => name.includes('.sharker-session-bundle-pack-')),
    false,
  );
});
...
async function createFixture(): Promise<{
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-session-bundle-codec-'));
  roots.push(root);
