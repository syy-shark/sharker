...
const root = process.env.SHARKER_CONTEXT_OFFLOAD_CRASH_ROOT;
...
const store = new SqliteContextOffloadStore(join(root, CONTEXT_OFFLOAD_DATABASE_NAME), {
  ...
  failpoint(point) {
    const requested = process.env.SHARKER_CONTEXT_OFFLOAD_CRASH_POINT ?? 'after_managed_file_publish';
...
await store.put({
  ...
  owner: {
    ...
    ownerId: process.env.SHARKER_CONTEXT_OFFLOAD_OWNER_ID ?? 'read-call-1',
  },
  bytes: new TextEncoder().encode(
    process.env.SHARKER_CONTEXT_OFFLOAD_VALUE ?? 'crash-safe-managed-value',
  ),
  mediaType: 'image/png',
});
