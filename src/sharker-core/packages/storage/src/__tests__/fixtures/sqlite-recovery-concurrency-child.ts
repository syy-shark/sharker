...
import { type ToolRecoveryFactEnvelope } from '@sharker/core/tool-recovery-fact';
import { type WorkspaceBaselineAuthorityInput } from '@sharker/core/workspace-version-authority';
import { createRuntimeBoundaryCursor, runtimePrefixSegment } from '@sharker/core/runtime-boundary';
...
const mode = requiredEnv('SHARKER_SQLITE_RECOVERY_CONCURRENCY_MODE');
const dbPath = requiredEnv('SHARKER_SQLITE_RECOVERY_CONCURRENCY_DB');
const startPath = requiredEnv('SHARKER_SQLITE_RECOVERY_CONCURRENCY_START');
const stopPath = process.env.SHARKER_SQLITE_RECOVERY_CONCURRENCY_STOP;
...
try {
  ...
  if (mode === 'open_only' || mode === 'operational_open_only') {
    if (!stopPath) throw new Error('Missing SHARKER_SQLITE_RECOVERY_CONCURRENCY_STOP');
...
function completedBundle() {
  return {
    ...
    decisionRuntimeEvent: recoveryFact(
      'completed-decision',
      'sharker.tool.recovery_decision',
      {
...
function recoveryFact(
  id: string,
  kind: 'sharker.tool.reconcile_result' | 'sharker.tool.recovery_decision',
