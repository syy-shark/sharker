...
describe('Work Board store', () => {
  ...
  test('migrates a real workflow schema 8 database through event-only version 10', async () => {
    const base = await mkdtemp(join(tmpdir(), 'sharker-work-board-schema8-'));
  ...
  test('backs up and restores board items with the operational state', async () => {
    const base = await mkdtemp(join(tmpdir(), 'sharker-work-board-backup-'));
    const stateRoot = join(base, 'state');
...
async function withTempRoot(run: (root: string) => Promise<void>): Promise<void> {
  const root = await mkdtemp(join(tmpdir(), 'sharker-work-board-'));
  try {
    await run(root);
  } finally {
