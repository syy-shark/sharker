...
test('recovers a durable managed-file publication intent after process exit', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-context-offload-publication-crash-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  await assert.rejects(
    execFileAsync(process.execPath, [managedPublicationCrashChild], {
      env: { ...process.env, SHARKER_CONTEXT_OFFLOAD_CRASH_ROOT: root, NODE_NO_WARNINGS: '1' },
      windowsHide: true,
    }),
...
test('recovers deterministic managed-file staging after process exit', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-context-offload-staging-crash-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  await assert.rejects(
    execFileAsync(process.execPath, [managedPublicationCrashChild], {
      env: {
        ...process.env,
        SHARKER_CONTEXT_OFFLOAD_CRASH_ROOT: root,
        SHARKER_CONTEXT_OFFLOAD_CRASH_POINT: 'after_managed_file_staging',
        NODE_NO_WARNINGS: '1',
      },
      windowsHide: true,
    }),
...
test('reports continuation while pending managed-file deletions remain', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-context-offload-pending-files-'));
  t.after(() => rm(root, { recursive: true, force: true }));
    ...
    await assert.rejects(
      execFileAsync(process.execPath, [managedPublicationCrashChild], {
        env: {
          ...
          SHARKER_CONTEXT_OFFLOAD_CRASH_ROOT: root,
          SHARKER_CONTEXT_OFFLOAD_CRASH_POINT: 'after_managed_file_staging',
          SHARKER_CONTEXT_OFFLOAD_OWNER_ID: `read-${index}`,
          SHARKER_CONTEXT_OFFLOAD_VALUE: value,
          NODE_NO_WARNINGS: '1',
        },
        windowsHide: true,
      }),
...
test('rejects a managed-value directory that resolves outside the Storage Root', async (t) => {
  const fixture = await createFixture(t);
  const outside = await mkdtemp(join(tmpdir(), 'sharker-context-offload-outside-'));
...
test('rejects a database schema newer than this authority understands', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-context-offload-newer-'));
  const path = join(root, CONTEXT_OFFLOAD_DATABASE_NAME);
  t.after(() => rm(root, { recursive: true, force: true }));
...
async function createFixture(
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-context-offload-'));
  const path = join(root, CONTEXT_OFFLOAD_DATABASE_NAME);
  let nextId = 1;
