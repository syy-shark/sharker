...
} from '@sharker/core/long-term-memory';
import {
  ...
  test('normalizes keys and searches global plus the selected workspace', async () => {
    await withStore(async ({ store }) => {
      ...
      const workspace = await createItem(
        ...
        'create-workspace',
        write({
          content: 'Sharker uses RuntimeEvent as evidence.',
          kind: 'knowledge',
          scopeType: 'workspace',
          scopeKey: 'workspace-sharker',
          keys: [
      ...
      assert.deepEqual(await itemIds(store, ['runtime'], 'prefix', 'workspace-sharker'), [workspace]);
      assert.deepEqual(await itemIds(store, ['memory_'], 'prefix', 'workspace-sharker'), [workspace]);
...
async function withTempRoot(run: (root: string) => Promise<void>): Promise<void> {
  const root = await mkdtemp(join(tmpdir(), 'sharker-long-term-memory-'));
  try {
