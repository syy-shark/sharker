...
} from '@sharker/core/workspace-version-authority';
import {
  decodeRuntimeEvent,
...
import { type ToolRecoveryDecisionFact } from '@sharker/core/tool-recovery-fact';
import { canonicalToolArgsHash, stableJsonStringify } from '@sharker/core/tool-args-identity';
import { encodeCanonicalRuntimeEvent } from '@sharker/core/canonical-runtime-event';
import { decodePersistedAgentRunHeader, type AgentRunHeader } from '@sharker/core/agent-run';
...
} from '@sharker/core/runtime-boundary';
import {
  assertToolRecoveryEventBundle,
  interpretScannedToolRecovery,
} from '@sharker/core/tool-recovery-bundle';
import {
...
export type { ToolRecoveryMode } from '@sharker/core/runtime-event';
    ...
    RuntimeWorkspaceVersionAuthorityStore
{
  ...
  async commitToolRecoveryBundle(input: RuntimeRecoveryBundleCommit): Promise<void> {
    ...
    this.transaction(() => {
      ...
      if (!decision || decision.kind !== 'sharker.tool.recovery_decision') {
  ...
  private assertExactRecoveryBundleAlreadyCommitted(
    ...
    const completed =
      decision?.kind === 'sharker.tool.recovery_decision' &&
