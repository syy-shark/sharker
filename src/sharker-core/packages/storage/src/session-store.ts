...
import { DEFAULT_SESSION_NAME, normalizeUserSessionName } from '@sharker/core/session-name';
import {
...
import { isCollaborationMode } from '@sharker/core/collaboration';
import { isOrchestrationMode } from '@sharker/core/orchestration';
import { decodePersistedPermissionMode, isPermissionMode } from '@sharker/core/permission';
import type { PersistedValue } from '@sharker/core/persisted-value';
import { isSubagentWorkspaceBinding } from '@sharker/core/subagent-workspace';
import { WORKSPACE_AUTHORITY_SESSION_ID } from '@sharker/core/workspace-version-authority';
import type {
...
export interface SessionAuthorityStore extends SessionStore, MessageAdmissionStore {
  ...
  ready(): Promise<void>;
  /** Atomically create a Session from already-converted Sharker raw messages. */
...
class SqliteSessionStore implements SessionAuthorityStore {
  ...
  async readHeaderRecordSnapshot(sessionId: string): Promise<SessionHeaderSnapshot> {
    await this.ensureReady();
    // `sharker --resume <legacy-id>` reads the header before any list; the
