...
import { cleanSessionRecapText } from '@sharker/runtime/session-recap';
import { fallbackSessionTitle, sessionTitleSource } from './session-title.js';
import { type RuntimeReadModelSessionView } from '@sharker/runtime/runtime-read-model';
import {
...
function recapArtifactIds(
  ...
  const digest = createHash('sha256')
    .update('sharker-session-recap-v1\0')
    .update(sessionId)
    .update('\0')
    .update(effectId)
    .digest('hex');
