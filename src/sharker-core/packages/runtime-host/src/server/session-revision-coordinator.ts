...
import { SIDE_CONVERSATION_SESSION_LABEL } from '@sharker/core/side-conversation';
...
} from '@sharker/runtime/conversation-copy';
import { isArchivedToolResultPlaceholder } from '@sharker/runtime/context-budget';
...
} from '@sharker/storage/task-ledger-authority';
import type {
  OperationOutcome,
...
export interface HostSessionRevisionCoordinatorOptions {
  ...
  readonly graph: Pick<
    import('@sharker/runtime/stream-graph-coordinator').AgentGraphCoordinator,
...
function collectArchivedToolResultPlaceholders(
  ...
  const add = (value: unknown): boolean => {
    if (!isRecord(value) || value.kind !== 'sharker.archived_tool_result') return true;
