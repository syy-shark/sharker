...
import { buildWebFetchTool } from '@sharker/runtime/web-fetch-tool';
import { createLocalWebFetchExecutor } from '@sharker/runtime/local-web-fetch';
import {
...
import type { RuntimePolicyOperationCoordinator } from '@sharker/storage/runtime-policy-stores';
...
export function createHostWebFetchTool(input: HostWebFetchServiceInput): SharkerTool {
  return createHostWebFetchToolFromService(createHostWebFetchService(input));
}

export function createHostWebFetchToolFromService(service: HostWebFetchService): SharkerTool {
