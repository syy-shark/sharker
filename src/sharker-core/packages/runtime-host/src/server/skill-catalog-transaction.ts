...
import { MANAGED_SKILL_BASELINE_RELATIVE_PATH } from '@sharker/runtime/skills';

const TRANSACTION_DIRECTORY = join('.sharker', 'skill-transactions');
...
const MANAGED_FROZEN_PREFIX = '.sharker-frozen-';
...
async function prepareRoot(rootInput: string, create: boolean): Promise<PreparedRoot | undefined> {
  ...
  const metadata = join(root, '.sharker');
