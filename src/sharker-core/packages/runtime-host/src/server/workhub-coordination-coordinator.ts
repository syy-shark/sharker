...
import { normalizeMessageContent } from '@sharker/core/events';
...
import type { SessionAuthorityStore, SessionHeaderSnapshot } from '@sharker/storage/session-store';
import type {
  OperationOutcome,
...
const CREATE_FINGERPRINT = `sha256:${createHash('sha256')
  .update('sharker:workhub-coordination-session:v1', 'utf8')
  .digest('hex')}`;
const COORDINATION_CWD_DIRECTORY = 'workhub-coordination';
const COORDINATION_TOOL_PROFILE = 'workhub-coordination-v1' as const;
const SYNTHETIC_COORDINATION_MODEL_ID = 'sharker-workhub-coordination';
