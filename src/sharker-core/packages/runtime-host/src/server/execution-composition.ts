...
import { MAX_READ_IMAGE_BYTES } from '@sharker/core/attachments';
import type { ContextOffloadLimits } from '@sharker/core/context-offload';
import { messageContentDigest, normalizeMessageContent } from '@sharker/core/events';
import {
  describeChatConfigurationReason,
  NO_REAL_CONNECTION_CODE,
} from '@sharker/core/connection-error-copy';
...
import { emptyPlanSessionState } from '@sharker/core/plan';
...
import { filterModelVisibleTaskLedgerTasks } from '@sharker/core/task-ledger';
import { AgentGraphCoordinator } from '@sharker/runtime/stream-graph-coordinator';
import { AgentGraphSupervisorWakeCoordinator } from '@sharker/runtime/agent-graph-supervisor-wake';
import {
  BackendRegistry,
  SessionManager,
  type BackendFactory,
} from '@sharker/runtime/session-manager';
import { buildToolsForAgentDefinition } from '@sharker/runtime/agent-catalog';
import { buildHistoryTools } from '@sharker/runtime/history-tools';
import { createLocalContinuationSafetyInspector } from '@sharker/runtime/continuation-safety';
import { createConfiguredSubagentCatalog } from '@sharker/runtime/configured-subagent-catalog';
import { buildHostCapabilitiesFromBinding } from '@sharker/runtime/skills';
import {
  createBuiltinSandboxManager,
  isBuiltinFilesystemWorkerSandboxAvailable,
} from '@sharker/runtime/sandbox';
import {
  createFilesystemWorkerLaunchSpecProvider,
  FilesystemWorkerClient,
} from '@sharker/runtime/filesystem-worker';
import { isOAuthEnrollmentProviderEnabled } from '@sharker/runtime/oauth-provider-contracts';
import {
  loadHistoryCompactCheckpointsFromRunLedger,
  loadLatestHistoryCompactCheckpointFromRunLedger,
} from '@sharker/runtime/history-compact-ledger';
import { prepareSkillInvocationMessageFromInventory } from '@sharker/runtime/skill-invocation';
import { RuntimeReadModel } from '@sharker/runtime/runtime-read-model';
import {
  renderAgentSwarmSupervisorWake,
  shouldWakeAgentSwarmSupervisor,
} from '@sharker/runtime/agent-swarm-status-tool';
import { SessionActivityRegistry } from '@sharker/runtime/goal-turn-lifecycle';
import { ShellRunProcessManager } from '@sharker/runtime/shell-run-manager';
import {
  resolveShellPlan,
  resolveTurnShellPlan,
  validateShellPreference,
} from '@sharker/runtime/shell-detect';
import { type SharkerTool } from '@sharker/runtime/tool-runtime';
import { type RuntimeHostedRootAuthority } from '@sharker/runtime/message-authority';
...
} from '@sharker/storage/artifact-stores';
import {
  isSessionNotFoundError,
  SessionMetadataConflictError,
} from '@sharker/storage/execution-stores';
import { createExternalSessionAdapterRegistry } from '@sharker/storage/external-sessions';
import { createGitWorktreeChildExecutor } from '@sharker/storage/git-worktree-child-executor';
import { runWithStorageRootLease } from '@sharker/storage/root-authority';
import { createInteractiveContextOffloadReader } from '@sharker/storage/context-offload-store';
import { openStorageWriterComposition } from '@sharker/storage/storage-writer-composition';
import type { RuntimePolicyStoresWriter } from '@sharker/storage/runtime-policy-stores';
import { resolveWorkspaceIdentity } from '@sharker/storage/workspace-identity';
...
export async function createExecutionRuntimeHostComposition(
  ...
  try {
    ...
    let scheduledTaskTool: SharkerTool | undefined;
    ...
    const resolveInteractiveToolSurface = async (input: {
      ...
      readonly boundTools?: readonly SharkerTool[];
      readonly childTools?: readonly SharkerTool[];
      readonly parentAgentTools?: readonly SharkerTool[];
    }) => {
    ...
    manager = new SessionManager({
      ...
      newId: randomUUID,
      now: Date.now,
      safeBoundaryResumeEnabled: process.env.SHARKER_RUNTIME_SAFE_BOUNDARY_RESUME === '1',
...
function subagentWritebackArtifactId(sessionId: string, turnId: string): string {
  const digest = createHash('sha256')
    .update('sharker-subagent-writeback-v1\0')
    .update(sessionId)
    .update('\0')
    .update(turnId)
    .digest('hex')
