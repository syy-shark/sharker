...
import type { SessionEvent, ShellRunUpdate } from '@sharker/core/events';
import { projectToolArgsPreview } from '@sharker/core/tool-quiet-preview';
import {
...
function projectSessionSnapshot(
  ...
  const projectEntry = <T extends { readonly content: import('@sharker/core/events').MessageContent }>(
    entry: T,
  ): T => ({
    ...entry,
 ...
 * computed and bounded in `@sharker/core`; the extra byte check here is the
