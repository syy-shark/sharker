...
import { manageChildProcessLifecycle } from '@sharker/runtime/child-process-lifecycle';
import {
...
const SHARKER_REF_PATTERN = /^refs\/sharker\/[A-Za-z0-9][A-Za-z0-9._/-]{0,255}$/;
  ...
  const { artifact, sourceRepositoryPath } = await runGitoxideOperationWithinDeadlineInternal({
    ...
    operation: async () => {
      ...
      if (
        ...
        !SHARKER_REF_PATTERN.test(input.baselineRef)
      ) {
        throw new GitoxideHelperInvocationError(
          'gitoxide_helper_invocation_invalid',
  ...
  const { artifact, repositoryPath } = await runGitoxideOperationWithinDeadlineInternal({
    ...
    operation: async () => {
      ...
      if (
        ...
        !SHARKER_REF_PATTERN.test(input.acceptedRef) ||
        !/^refs\/sharker\/candidates\/[0-9a-f]{64}$/.test(input.candidateRef) ||
...
function candidateRequestDigestSha256(input: {
  ...
  const hash = createHash('sha256').update('sharker.gitoxide.candidate-request.v1\0', 'utf8');
  for (const field of [
...
function isSourceImportObservation(
  ...
  return (
    ...
    SHARKER_REF_PATTERN.test(value.baselineRef) &&
