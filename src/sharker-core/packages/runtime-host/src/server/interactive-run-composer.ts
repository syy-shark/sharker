...
} from '@sharker/core/side-conversation';
import { type RunCompositionSourceRevision } from '@sharker/core/run-composition';
import {
  buildDeepResearchSystemPromptFragment,
  isDeepResearchSession,
} from '@sharker/core/deep-research';
import { activePlanExecution, type PlanSessionState, type PlanStore } from '@sharker/core/plan';
...
import type { RuntimePolicySnapshot } from '@sharker/core/runtime-policy';
import type { SessionToolProfile } from '@sharker/core/session';
import { type TaskLedgerStore } from '@sharker/core/task-ledger';
import { assembleMainSessionSystemPrompt } from '@sharker/runtime/system-prompt/main-session-prompt';
import { buildAskUserQuestionTool } from '@sharker/runtime/ask-user-question-tool';
import { buildBuiltinTools, type BuildBuiltinToolsOptions } from '@sharker/runtime/builtin-tools';
import {
  buildCancelPlanTool,
  buildSubmitPlanTool,
  buildUpdatePlanTool,
} from '@sharker/runtime/plan-tools';
import { buildParentAgentTools } from '@sharker/runtime/subagent-tools';
import { buildPersonalizationPromptFragment } from '@sharker/runtime/system-prompt/personalization-prompt';
import { buildRequestSandboxBoundaryTool } from '@sharker/runtime/sandbox-boundary-tool';
import {
...
} from '@sharker/runtime/skills';
import { buildTaskLedgerTools } from '@sharker/runtime/task-ledger-tools';
import { buildWorkspaceInstructionsPromptFragment } from '@sharker/runtime/system-prompt/workspace-instructions';
import { isDeepResearchToolAllowed } from '@sharker/runtime/deep-research-tools';
import { listRunnableBuiltinAgentDefinitions } from '@sharker/runtime/agent-catalog';
import { renderPlanModePrompt, selectCollaborationTools } from '@sharker/runtime/plan-mode';
import { routeWebFetchTools } from '@sharker/runtime/web-fetch-tool';
import { routeWebSearchTools } from '@sharker/runtime/native-web-search-tool';
import { type SharkerTool } from '@sharker/runtime/tool-runtime';
import { type ToolGroup } from '@sharker/runtime/tool-availability';
import { resolveTurnShellPlan, type TurnShellPlan } from '@sharker/runtime/shell-detect';
import type {
...
const INTERACTIVE_RUN_COMPOSER_ID = 'sharker.interactive';
...
export interface InteractiveRunComposerInput {
  ...
  readonly scheduledTaskTool?: SharkerTool;
  readonly goalTools?: readonly SharkerTool[];
  ...
  readonly deepResearch?: {
    readonly tools: readonly SharkerTool[];
  };
}
...
export interface InteractiveRunComposerFactoryInput
  ...
  readonly resolveRootTools?: (sessionId: string) => Promise<readonly SharkerTool[]>;
  readonly childTools?: readonly SharkerTool[];
  readonly worktreePatchWriteBackAvailable?: boolean;
  readonly planStore?: PlanStore;
  readonly deepResearchTools?: readonly SharkerTool[];
...
export function routeInteractiveRunToolSurface(input: InteractiveRunToolSurfaceInput): {
  ...
  const route = (tools: readonly SharkerTool[]): SharkerTool[] => {
...
function assertUniqueToolNames(tools: readonly SharkerTool[]): void {
...
function buildDefaultHostTools(
  ...
  hostTools: readonly SharkerTool[] = [],
  scheduledTaskTool?: SharkerTool,
  goalTools: readonly SharkerTool[] = [],
  parentAgentTools: readonly SharkerTool[] = [],
  plan?: InteractiveRunComposerInput['plan'],
  deepResearchTools: readonly SharkerTool[] = [],
): SharkerTool[] {
...
function requireDeepResearchTools(tools: readonly SharkerTool[] | undefined): readonly SharkerTool[] {
