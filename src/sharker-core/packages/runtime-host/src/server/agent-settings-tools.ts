...
} from '@sharker/core/runtime-policy';
...
type AgentSettingsUpdateResult =
  | {
      readonly kind: 'sharker_settings_update';
      readonly ok: true;
...
export function buildHostAgentSettingsTools(
...
): readonly SharkerTool[] {
  const getTool: SharkerTool<Record<string, never>, AgentSettingsSnapshot> = {
    name: 'SharkerSettingsGet',
    displayName: 'Read Sharker settings',
    description:
      'Read the safe, non-secret Runtime settings that Sharker may help configure. ' +
  ...
  const updateTool: SharkerTool<AgentSettingsPatch, AgentSettingsUpdateResult> = {
    name: 'SharkerSettingsUpdate',
    displayName: 'Update Sharker settings',
    description:
      'Update the safe, non-secret Runtime settings owned by the Runtime Host. ' +
      'Use only when the user explicitly asks to change Sharker itself. Every effective change requires confirmation.',
    ...
    impl: async (patch, context) => {
      ...
      if (!context.askUserQuestion) {
        return {
          kind: 'sharker_settings_update',
          ok: false,
          applied: false,
          reason: 'confirmation_unavailable',
          message: 'Sharker settings were not changed because confirmation is unavailable.',
          settings: projectSettings(initial.policy),
        };
      }
      const answer = await context.askUserQuestion([
        {
          question: `Apply these Sharker setting changes?\n${changes.map((change) => `• ${change}`).join('\n')}`,
          options: [
            { label: 'Apply changes', description: 'Persist the listed Sharker settings now.' },
      ...
      if (answer.answers[0]?.answer !== 'Apply changes') {
        ...
        return {
          ...
          applied: false,
          reason: 'cancelled',
          message: 'Sharker settings were not changed.',
      ...
      for (let attempt = 0; attempt < 3; attempt += 1) {
        ...
        return {
          ...
          applied: true,
          changes,
          message: `Updated ${changes.length} Sharker setting${changes.length === 1 ? '' : 's'}.`,
          settings: projectSettings(updated.policy),
        };
      }
      throw new Error('Runtime Policy kept changing while Sharker settings were updated');
    },
  };
...
function unchanged(policy: RuntimePolicy): AgentSettingsUpdateResult {
  return {
    ...
    ok: true,
    applied: false,
    message: 'The requested Sharker settings already have those values.',
