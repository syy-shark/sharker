...
const ACCEPTED_REPOSITORY_REF = 'refs/sharker/accepted' as const;
...
> {
  ...
  const candidateRef = `refs/sharker/candidates/${createHash('sha256').update(input.operationId).digest('hex')}`;
