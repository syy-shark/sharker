...
import { authorizeConnectionModel } from '@sharker/core/llm-connections';
import { isModelExplicitlyUnsupportedForChat } from '@sharker/core/model-catalog';
import { thinkingVariantsForConnection } from '@sharker/core/model-thinking';
import {
...
import type { CreateStableSessionRequest } from '@sharker/storage/session-store';
...
export class HostSessionCatalogCoordinator {
  ...
  async #resolveModel(
    ...
    if (readiness.kind === 'provider_retired') {
      throw new SessionOperationFailure(
        'invalid_request',
        'Session model connection uses a sign-in that was removed from Sharker',
      );
    }
    if (
      selected.connectionId !== undefined &&
