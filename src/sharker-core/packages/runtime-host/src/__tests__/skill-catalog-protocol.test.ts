...
describe('Runtime Host Skill catalog protocol', () => {
  ...
  test('decodes bounded Session and new-Session invocable queries and pages', () => {
    ...
    const result = {
      ...
      revision: REVISION,
      items: [
        { ref: 'project:sharker:review', id: 'review', name: 'Review', description: 'Review code' },
      ],
      nextCursor: null,
    };
    assert.deepEqual(
    ...
    const pages = [
      page('governance', [
        ...
        governanceItem({
          ...
          ref: 'diagnostic:project:sharker:0',
          id: 'source-0',
          name: '',
          description: '',
          declaredTools: [],
  ...
  test('encodes an empty bundled catalog through the Host output codec', async () => {
    const base = await mkdtemp(join(tmpdir(), 'sharker-skill-catalog-protocol-'));
  ...
  test('allows bounded human ids in governance while rejecting control-character identities', () => {
    const humanIdPage = page('governance', [
      governanceItem({ id: 'valid skill', ref: 'project:sharker:valid skill' }),
    ]);
    assert.deepEqual(
    ...
    assertInvalidResponse(
      'skill.catalog.query',
      page('governance', [governanceItem({ ref: 'project:sharker:bad\u0000skill' })]),
    );
  ...
  test('decodes all mutation variants and requires preview hashes only for force update', () => {
    ...
    const mutations: readonly SkillCatalogMutation[] = [
      ...
      { kind: 'set_enabled', ref: 'user:sharker:research-brief', enabled: false },
      { kind: 'set_pinned', ref: 'user:sharker:research-brief', pinned: true },
    ];
    ...
    for (const mutation of [
      ...
      { kind: 'set_pinned', ref: 'user:sharker:research-brief', pinned: true, retry: true },
    ]) {
      assertInvalidRequest('skill.catalog.mutate', {
        context: CONTEXT,
        expectedRevision: REVISION,
