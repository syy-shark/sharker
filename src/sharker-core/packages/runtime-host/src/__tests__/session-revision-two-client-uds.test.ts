...
import { WORKHUB_COORDINATION_SESSION_ID } from '@sharker/core/session';
import { agentGraphIdForRootSession } from '@sharker/runtime/stream-graph-coordinator';
import { FAKE_ASK_USER_QUESTION_PROMPT } from '@sharker/runtime/test-only/fake-backend';
...
test('two Clients share exact retryable Session branch and revision authority', {
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-session-revision-'));
