...
import { openInteractiveDeepResearchStoreForWrite } from '@sharker/storage/deep-research-authority';
...
test('two Clients and a restarted production Host share one Deep Research projection', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-deep-research-uds-'));
