...
import { CONTEXT_BUDGET_EXHAUSTED_DETAILS, TOOL_OUTPUT_DELTA_MAX_CHARS } from '@sharker/core/events';
import { CONNECTION_CATALOG_MAX_ENABLED_MODEL_IDS } from '@sharker/core/runtime-policy';
import {
...
describe('Runtime Host bootstrap protocol', () => {
  ...
  test('accepts bounded explicit Skill identities on turn.start', () => {
    ...
    assert.deepEqual(start(['writer', 'project:sharker:reviewer']), {
      requestId: 'skill-start',
      operation: 'turn.start',
      input: {
        sessionId: 'session-1',
        turnId: 'turn-skill-1',
        content: { text: '' },
        skillIds: ['writer', 'project:sharker:reviewer'],
      },
    });
    assert.doesNotThrow(() =>
