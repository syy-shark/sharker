...
import { RuntimeMessageAuthorityInvariantError } from '@sharker/runtime/message-authority';
import type {
...
import { rootTurnAdmissionRecordFits } from '@sharker/storage/execution-stores';
...
test('consumes an active-target admission before the terminal transition can make it idle', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-workhub-active-consume-'));
...
test('idle recovery starts one real preassigned WorkHub root and restores the remainder', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-workhub-idle-recovery-'));
  const store = createSessionStore(root);
  t.after(async () => {
    await store.close?.();
    await rm(root, { recursive: true, force: true });
...
test('idle recovery keeps promoted steering ahead of distinct real WorkHub roots', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-workhub-promoted-recovery-'));
  const store = createSessionStore(root);
  t.after(async () => {
    await store.close?.();
    await rm(root, { recursive: true, force: true });
...
function largeSkillInvocation() {
  ...
  return {
    ...
    receipts: loaded.map((skill, index) => ({
      ...
      success: true as const,
      ref: `project:sharker:${index}:${'r'.repeat(280)}`,
      id: skill.id,
      name: skill.name,
      scope: 'project' as const,
      source: 'sharker' as const,
