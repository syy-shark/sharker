...
import { REQUEST_BODY_OVERLAY_MAX_BYTES } from '@sharker/core/runtime-policy';
import { FAKE_ASK_USER_QUESTION_PROMPT, FakeBackend } from '@sharker/runtime/test-only/fake-backend';
import { type SharkerToolContext } from '@sharker/runtime/tool-runtime';
...
test('model settings tool confirms and atomically updates canonical Runtime Policy', async () => {
  await withCoordinator(async ({ coordinator, stores }) => {
    const tool = coordinator.modelTools.find(({ name }) => name === 'SharkerSettingsUpdate');
    assert.ok(tool);
    if (!tool) return;
    const questions: string[] = [];
    const toolContext: SharkerToolContext = {
      sessionId: 'session-1',
      turnId: 'turn-1',
      cwd: '/workspace',
      toolCallId: 'call-1',
...
test('production composition shares one gate across mutation and backend activation', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-policy-composition-'));
...
test('production mutation releases the gate before active-turn backend disposal completes', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-policy-active-turn-refresh-'));
  const root = join(base, 'interactive');
  const capability = await resolveStorageRoot({ path: root, kind: 'interactive' });
  const owner = await tryAcquireInteractiveRootOwner(capability);
  assert.ok(owner);
...
test('production policy mutation drains and poisons activation when cached backend disposal fails', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-policy-disposal-failure-'));
  const root = join(base, 'interactive');
  const capability = await resolveStorageRoot({ path: root, kind: 'interactive' });
  const owner = await tryAcquireInteractiveRootOwner(capability);
  assert.ok(owner);
...
async function withCoordinator(
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-policy-host-'));
  const root = join(base, 'interactive');
