...
import type { GoalAuthorityRecord } from '@sharker/core/goal';
...
import { openInteractiveGoalAuthorityForWrite } from '@sharker/storage/goal-authority';
...
test('one Host Goal is shared across clients with CAS control and crash-clear residency', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-goal-'));
...
test('session retirement forgets a terminal Goal without recreating deleted authority', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-goal-retirement-'));
  const capability = await resolveStorageRoot({
    path: join(base, 'root'),
    kind: 'interactive',
  });
...
test('restart settles the durable current Goal execution through Hosted Execution authority', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-goal-recovery-'));
  const capability = await resolveStorageRoot({
    path: join(base, 'root'),
    kind: 'interactive',
  });
...
test('restart replaces a stale current execution with the current durable Goal intent', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-goal-stale-execution-'));
  const capability = await resolveStorageRoot({
    path: join(base, 'root'),
    kind: 'interactive',
  });
...
test('goal.arm creates one Goal per Session and refuses a second while it is unfinished', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-goal-arm-'));
  const capability = await resolveStorageRoot({
    path: join(base, 'root'),
    kind: 'interactive',
  });
...
test('a Goal armed but never carried by a Turn does not start itself after a restart', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-goal-armed-restart-'));
  const capability = await resolveStorageRoot({
    path: join(base, 'root'),
    kind: 'interactive',
  });
...
test('resuming an armed Goal drives it, and a restart puts that drive back', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-goal-armed-resume-'));
  const capability = await resolveStorageRoot({
    path: join(base, 'root'),
    kind: 'interactive',
  });
...
test('an arm admitted before the drain creates no Goal after it', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-goal-arm-drain-'));
  const capability = await resolveStorageRoot({
    path: join(base, 'root'),
    kind: 'interactive',
  });
