...
import { resolveRootControlNamespace, resolveStorageRoot } from '@sharker/storage/root-authority';
...
test('one Local IPC owner and one authenticated WebSocket Client control the same Session', {
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-authenticated-websocket-'));
...
test('an authenticated WebSocket Client reconnects after service restart to canonical state', {
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-authenticated-websocket-restart-'));
  const root = join(base, 'root');
  const capability = await resolveStorageRoot({ path: root, kind: 'interactive' });
...
test('access credentials persist only as hashes and stay revoked after reload', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-access-authority-'));
  try {
...
test('capability-provider credentials retain a Host-verified Client owner identity', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-access-authority-provider-owner-'));
  try {
    const authority = await openRuntimeHostAccessAuthority(directory);
...
test('an unbound WebSocket credential cannot claim an existing bound Client identity', {
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-bound-client-websocket-'));
  const root = join(base, 'root');
  const capability = await resolveStorageRoot({ path: root, kind: 'interactive' });
  const host = await startExecutionRuntimeHostService({
    rootPath: root,
...
test('writes the capability-owner schema only after the association commits', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-access-authority-owner-schema-'));
...
test('credential rotation preserves authority and cannot outlive its active source', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-access-authority-rotation-'));
  const authority = await openRuntimeHostAccessAuthority(directory);
  try {
...
test('a Client-bound pairing candidate can be claimed by exactly one Client identity', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-access-authority-client-claim-'));
...
test('principal revocation is atomic with pairing finalization', async () => {
  ...
  for (const order of ['finalize-first', 'revoke-first'] as const) {
    const directory = await mkdtemp(join(tmpdir(), `sharker-access-principal-revoke-${order}-`));
    const authority = await openRuntimeHostAccessAuthority(directory);
    try {
      const principal = {
...
test('a revoked Client-bound credential remains readable after restart', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-access-authority-bound-revoke-'));
  const authority = await openRuntimeHostAccessAuthority(directory);
  const candidate = await authority.prepare({
    principalKind: 'remote_owner',
...
test('guarded credential revocation requires its active credential', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-access-authority-guarded-revoke-'));
  const authority = await openRuntimeHostAccessAuthority(directory);
  try {
...
test('keeps published credential state authoritative when directory sync is uncertain', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-access-authority-unknown-commit-'));
...
test('expired pairing candidates are denied and removed from durable access state', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-access-authority-expired-pairing-'));
  const credential = 'sharker_rh_expired_pairing';
  const path = join(directory, 'runtime-host-access.json');
  try {
    await writeFile(
      path,
...
test('closed access authority cannot expire credentials owned by its successor', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-access-authority-close-'));
  const path = join(directory, 'runtime-host-access.json');
  ...
  try {
    ...
    await writeFile(
      ...
      `${JSON.stringify({
        schemaVersion: 1,
        credentials: [storedCredential('successor-credential', 'sharker_rh_successor', 'active')],
      })}\n`,
      { mode: 0o600 },
    );
...
test('keeps a formerly accepted local-only grant inert when opening an existing access file', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-access-authority-legacy-'));
  const credential = 'sharker_rh_existing';
  try {
    await writeFile(
      join(directory, 'runtime-host-access.json'),
      `${JSON.stringify({
...
test('releases a retired operation grant when opening an existing access file', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-access-authority-retired-grant-'));
  const credential = 'sharker_rh_existing_usage_client';
  try {
    await writeFile(
      join(directory, 'runtime-host-access.json'),
      `${JSON.stringify({
...
test('migrates the released transcript query grant when opening an existing access file', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-access-authority-transcript-legacy-'));
  const credential = 'sharker_rh_existing_transcript_client';
  try {
    await writeFile(
      join(directory, 'runtime-host-access.json'),
      `${JSON.stringify({
...
test('adds bounded turn landmarks to an existing turn-query grant', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-access-authority-turn-landmarks-'));
  const credential = 'sharker_rh_existing_turn_client';
  try {
    await writeFile(
      join(directory, 'runtime-host-access.json'),
      `${JSON.stringify({
...
test('a rejected required WebSocket listener releases Local IPC and root ownership', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-websocket-startup-rollback-'));
  const root = join(base, 'root');
  const capability = await resolveStorageRoot({ path: root, kind: 'interactive' });
  try {
    await assert.rejects(
