...
describe('Runtime Host profiles', () => {
    ...
    await catalog.save(
      {
        ...
        transport: {
          kind: 'ssh',
          destination: 'operator@example.com',
          activation: { kind: 'ssh_operator', operatorPath: '/opt/sharker/bin/operator' },
        },
        rootId: ROOT_B,
      },
      'activated-token',
    ...
    await connectRemoteRuntimeHostProfile(
      {
        profile: {
          ...
          transport: {
            kind: 'ssh',
            destination: 'operator@example.com',
            activation: { kind: 'ssh_operator', operatorPath: '/opt/sharker/operator' },
          },
          rootId: ROOT_A,
        },
        credential: 'opaque-token',
      ...
      },
      {
        activateSshOperator: async (input) => {
          ...
          assert.equal(input.operatorPath, '/opt/sharker/operator');
  ...
  test('reports an incompatible remote Host with redacted compatibility details before readiness', async () => {
    ...
    const handshake = incompatibleHandshake({
      ...
      compositionId: 'sharker.different-composition',
    ...
    await assert.rejects(
      ...
      (error: unknown) => {
        ...
        assert.match(error.message, /sharker\.interactive/u);
        assert.match(error.message, /sharker\.different-composition/u);
  ...
  test('includes composition fields only when the Client and Host composition ids differ', async () => {
    ...
    assert.match(different.message, /Client composition id sharker\.interactive/u);
    assert.match(different.message, /Host composition id sharker\.different-composition/u);
  ...
  test('sanitizes Host composition identity only in the human-readable message', () => {
    const compositionId = 'sharker.different\u001b[31m\u202eline\u2066isolate';
    ...
    assert.match(error.message, /Host composition id sharker\.different�\[31m�line�isolate/u);
    assert.match(
      error.message,
...
async function profilePath(): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-profiles-'));
