...
import { TOOL_BOUNDARY_PROTOCOL_V1 } from '@sharker/core/runtime-event';
...
import type { ConnectionCatalogEntry } from '@sharker/core/runtime-policy';
...
import { BackendRegistry, SessionManager } from '@sharker/runtime/session-manager';
import { buildTaskLedgerTools } from '@sharker/runtime/task-ledger-tools';
import {
  buildRecoveredTerminalRuntimeEvent,
  classifyTerminalRuntimeLedger,
  commitTerminalRunWithRuntimeFact,
} from '@sharker/runtime/terminal-run-commit';
import {
  FAKE_ASK_USER_QUESTION_PROMPT,
  FAKE_WAIT_FOR_STEERING_PROMPT,
  FakeBackend,
} from '@sharker/runtime/test-only/fake-backend';
...
import { OPERATIONAL_STATE_DATABASE_NAME } from '@sharker/storage/operational-state-store';
import { openInteractiveRuntimePolicyStoresForWrite } from '@sharker/storage/runtime-policy-stores';
import {
...
export class ExecutionFixture {
  ...
  private spawnHost(
    ...
    if (safeBoundaryResumeEnabled) env.SHARKER_RUNTIME_SAFE_BOUNDARY_RESUME = '1';
    else delete env.SHARKER_RUNTIME_SAFE_BOUNDARY_RESUME;
    const child = fork(
...
export async function withExecutionRoot(
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-execution-'));
