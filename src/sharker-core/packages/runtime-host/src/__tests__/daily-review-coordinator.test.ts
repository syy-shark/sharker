...
import { localDayBoundsAt, type DailyReviewArchive } from '@sharker/core/daily-review';
import { openInteractiveDailyReviewAuthorityForWrite } from '@sharker/storage/daily-review-authority';
import { acquireOperationalStateDatabase } from '@sharker/storage/operational-state-store';
...
async function withCoordinator(
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-daily-review-coordinator-'));
