...
import { TOOL_SEARCH_NAME, ToolAvailabilityRuntime } from '@sharker/runtime/tool-availability';
...
test('unknown Client Capability loads, invokes, and rebinds after UDS reconnect', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-client-capability-'));
  ...
  try {
    ...
    const activeTools = new Map<string, SharkerTool>();
...
function invalidTool(): SharkerTool {
  return {
    name: 'invalid',
