...
async function fixture(t: test.TestContext): Promise<Fixture> {
  const rootPath = await mkdtemp(join(tmpdir(), 'sharker-managed-root-'));
  const authorityRoot = await mkdtemp(join(tmpdir(), 'sharker-managed-authority-'));
...
test('strictly decodes every level of the canonical deployment contract', () => {
  const config = createConfig('/srv/sharker/state', 'a'.repeat(64));
    ...
    tryAcquireRuntimeHostLaunchOwner(
      input.capability,
      {
        ...
        processLaunch: {
          ...
          entrypointPath: '/opt/sharker/runtime-host/versions/unclaimed/candidate.js',
        },
      },
      input.authority,
    ),
...
test('concurrent claims cannot adopt an unsynced authority directory', async (t) => {
  const input = await fixture(t);
  const authorityBase = await mkdtemp(join(tmpdir(), 'sharker-managed-durability-'));
