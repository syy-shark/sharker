...
for (const failpoint of [
  ...
  test(`managed recovery cleans ${failpoint} without reopening staged artifacts`, async () => {
    ...
    const gcDirectory = join(root, '.sharker', 'skill-transactions', entries[0]);
...
function frozenSkillPath(root: string): string {
  ...
  const frozen = readdirSync(skills).filter((entry) => entry.startsWith('.sharker-frozen-'));
...
test('recovery rejects modified staged artifacts and leaves live files untouched', async () => {
  ...
  const transaction = await onlyTransaction(root);
  await writeFile(join(transaction, 'next', '.sharker', 'baseline', 'SKILL.md'), 'tampered');
...
test('transactions write only skills and .sharker/skill-transactions in the Data Root', async () => {
  const root = await tempRoot();
  ...
  assert.deepEqual((await readdir(root)).sort(), ['.sharker', 'skills', 'unrelated.json']);
  assert.deepEqual(await readdir(join(root, '.sharker')), ['skill-transactions']);
  assert.deepEqual(await transactionEntries(root), []);
...
async function tempRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'sharker-skill-transaction-'));
  roots.add(root);
  return root;
}
...
async function writeManagedDirectory(
  ...
  await mkdir(join(directory, '.sharker', 'baseline'), { recursive: true });
  await writeFile(join(directory, '.sharker', 'baseline', 'SKILL.md'), artifacts.baseline);
}
...
async function readManagedDirectory(
  ...
  const [skill, lock, baseline] = await Promise.all([
    ...
    readFile(join(directory, '.sharker', 'baseline', 'SKILL.md'), 'utf8'),
  ]);
...
async function transactionEntries(root: string): Promise<string[]> {
  try {
    return await readdir(join(root, '.sharker', 'skill-transactions'));
  } catch (error) {
...
async function frozenEntries(root: string): Promise<string[]> {
  try {
    return (await readdir(join(root, 'skills')))
      .filter((entry) => entry.startsWith('.sharker-frozen-'))
      .sort();
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') return [];
    throw error;
...
async function onlyTransaction(root: string): Promise<string> {
  const entries = await transactionEntries(root);
  assert.equal(entries.length, 1);
  return join(root, '.sharker', 'skill-transactions', entries[0]);
}
