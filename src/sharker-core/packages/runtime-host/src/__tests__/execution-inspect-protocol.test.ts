...
import type { AgentRunInspectDocument, SessionInspectDocument } from '@sharker/core/execution-inspect';
import {
  decodeClientFrame,
  decodeHostFrame,
...
function agentRunDocument(sessionId = 'session-1', agentRunId = 'run-1'): AgentRunInspectDocument {
  return {
    schemaVersion: 'sharker.agent_run_inspect.v1',
    kind: 'agent_run',
    agentRun: {
      sessionId,
      agentRunId,
...
function sessionDocument(sessionId: string): SessionInspectDocument {
  return {
    schemaVersion: 'sharker.session_inspect.v1',
    kind: 'session',
    session: {
      sessionId,
      name: 'Session',
