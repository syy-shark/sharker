...
test('preserves a valid Candidate invocation failure across the detached stderr boundary', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-candidate-diagnostic-'));
 ...
 * runtime. Walk the built module graph across the bundled `@sharker/*` packages
...
function resolveModule(specifier: string, parent: string): string | undefined {
  try {
    ...
    if (specifier.startsWith('@sharker/')) {
