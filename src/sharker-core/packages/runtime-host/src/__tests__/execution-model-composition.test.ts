...
} from '@sharker/core/sandbox-boundary';
import { PROVIDER_DEFAULTS } from '@sharker/core/llm-connections';
import { createWorkspaceWritePermissionProfile } from '@sharker/core/permission-profile';
import { decodeRunCompositionSnapshot } from '@sharker/core/run-composition';
import { decodeCanonicalToolResultContent } from '@sharker/core/tool-result-record-schema';
import { type ModelCallAttempt, type ModelCallKind } from '@sharker/core/model-call-attempt';
import { type RuntimeEvent } from '@sharker/core/runtime-event';
import { createDefaultRuntimePolicy } from '@sharker/core/runtime-policy';
import type { PlanSessionState, PlanStore } from '@sharker/core/plan';
import type { TaskLedgerStore } from '@sharker/core/task-ledger';
import {
  serializeOAuthSubscriptionTokens,
  type OAuthSubscriptionTokens,
} from '@sharker/runtime/subscription-credentials';
import { type BackendFactoryContext } from '@sharker/runtime/session-manager';
import { type AiSdkBackendInput, type RunTraceEvent } from '@sharker/runtime/ai-sdk-backend';
import { type FilesystemWorkerExecuteInput } from '@sharker/runtime/filesystem-worker';
import { type SharkerTool, type SharkerToolContext } from '@sharker/runtime/tool-runtime';
import {
  type ProxiedFetchProxy,
  type ProxiedFetchTransport,
} from '@sharker/runtime/network/scoped-fetch-transport';
import { type ScannedSkill } from '@sharker/runtime/skills';
import { agentGraphIdForRootSession } from '@sharker/runtime/stream-graph-coordinator';
import { resolveTurnShellPlan, ShellPreferenceError } from '@sharker/runtime/shell-detect';
import { buildParentAgentTools } from '@sharker/runtime/subagent-tools';
import { SESSION_RECAP_INSTRUCTION } from '@sharker/runtime/session-recap';
import { createToolResultArchiveCapability } from '@sharker/runtime/tool-result-archive-capability';
import { loadHistoryCompactCheckpointsFromRunLedger } from '@sharker/runtime/history-compact-ledger';
import { stableHash, toolCatalogHash } from '@sharker/runtime/request-shape';
import { toolAvailabilityHash } from '@sharker/runtime/tool-availability';
import { createSqliteRuntimeStore } from '@sharker/storage/sqlite-runtime-store';
import { createAgentGraphControlStore } from '@sharker/storage/agent-graph-control-store';
import { openInteractiveArtifactStoreForWrite } from '@sharker/storage/artifact-stores';
import { openInteractiveExecutionStoresForWrite } from '@sharker/storage/execution-stores';
import {
  openInteractiveRuntimePolicyStoresForWrite,
  type RuntimePolicyStoresWriter,
} from '@sharker/storage/runtime-policy-stores';
import { resolveStorageRoot, tryAcquireInteractiveRootOwner } from '@sharker/storage/root-authority';
import { openInteractiveTaskLedgerStoreForWrite } from '@sharker/storage/task-ledger-authority';
import {
  openInteractiveUsageStoresForWrite,
  type InteractiveUsageStoresWriter,
} from '@sharker/storage/usage-stores';
...
test('production Host executes Bash against the current live sandbox boundary', {
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-managed-bash-'));
  ...
  try {
    if (process.platform === 'darwin') {
      outsideRoot = await mkdtemp(join(homedir(), '.sharker-host-sandbox-boundary-'));
      sandboxPaths = {
...
test('backend creation admits an enabled model a live list omits', async () => {
  // A live list is the strongest observation Sharker has and still cannot refuse
...
test('Host reopens one projected image from its ArtifactStore authority', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-projection-image-'));
...
test('backend abort cannot cancel the authority-owned OAuth refresh used by its successor', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-oauth-backend-'));
  const capability = await resolveStorageRoot({
    path: join(base, 'interactive'),
    kind: 'interactive',
  });
      ...
      backendCreationFixture({
        ...
        resolveExecutionConnection: async () =>
          readyExecutionConnection(provider.baseUrl, {
            requestHeaders: { 'X-Sharker-Test': 'tui-shared-setting' },
...
test('hosted execution freezes the headless coding provider wire contract', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-hosted-profile-wire-'));
  const root = join(base, 'interactive');
  const provider = await startProvider();
  const capability = await resolveStorageRoot({ path: root, kind: 'interactive' });
  const owner = await tryAcquireInteractiveRootOwner(capability);
...
test('production Host executes a canonical ai-sdk Session against a real provider wire', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-real-model-'));
  const root = join(base, 'interactive');
...
test('production Host executes and durably supervises an Agent Graph over a real provider wire', {
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-agent-graph-'));
  const root = join(base, 'interactive');
  const project = join(base, 'project');
  const provider = await startProvider();
  ...
  try {
    ...
    assert.equal(rootRun?.runComposition?.composerId, 'sharker.interactive');
...
test('production Host executes a durable runnable child with an exact tool ceiling', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-child-agent-'));
  const root = join(base, 'interactive');
  const project = join(base, 'project');
  const provider = await startProvider();
...
test('production Host publishes and retires an implementation child patch', async () => {
  ...
  try {
    ...
    await git(
      project,
      '-c',
      'user.name=Sharker Test',
      '-c',
      'user.email=test@sharker.invalid',
      'commit',
      '-m',
      'fixture',
    );
...
test('Host auxiliary calls preserve resolved DeepSeek reasoning settings', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-deepseek-auxiliary-'));
  const provider = await startProvider();
  const capability = await resolveStorageRoot({
    path: join(base, 'interactive'),
    kind: 'interactive',
...
test('Host auxiliary models meter provider usage and abort physical requests', {
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-goal-evaluator-'));
  const provider = await startProvider();
  const capability = await resolveStorageRoot({
    path: join(base, 'interactive'),
    kind: 'interactive',
...
test('one turn shares one canonical Skill inventory across prompt and lazy tools', async () => {
  ...
  assert.match(firstPrompt ?? '', /^You are Sharker,/);
  ...
  const searchTool = composition.tools.find((tool) => tool.name === 'SkillSearch') as
    | SharkerTool<{ query: string }, { matches: Array<{ ref: string }> }>
    | undefined;
  assert.ok(skillTool);
...
test('backend composition survives a moved saved Git Bash executable while Bash fails closed', async () => {
  ...
  const connection = readyExecutionConnection()
    .connection as unknown as import('@sharker/core/llm-connections').RuntimeExecutionConnection;
  ...
  const bash = composer.tools.find((tool) => tool.name === 'Bash') as
    | SharkerTool<{ command: string }, unknown>
    | undefined;
...
test('a bound tool ceiling excludes dynamic Client Capability tools', () => {
  const boundTool: SharkerTool = {
    name: 'bounded_tool',
    description: 'The only tool admitted for this activation.',
    parameters: {},
    impl: async () => 'bounded',
  };
  const capabilityTool: SharkerTool = {
  ...
  const scheduledTaskTool: SharkerTool = {
    name: 'ScheduledTask',
...
function requireRuntimeResourceRef(body: Record<string, unknown>): string {
  const ref = JSON.stringify(body).match(/sharker:\/\/runtime\/background-tasks\/[A-Za-z0-9_-]+/)?.[0];
...
async function handleProviderRequest(
  ...
  requests.push({
    ...
    customHeader: request.headers['x-sharker-test'] as string | undefined,
    body,
  });
