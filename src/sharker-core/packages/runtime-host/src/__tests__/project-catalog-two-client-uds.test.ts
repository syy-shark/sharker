...
import { openInteractiveProjectCatalogForWrite } from '@sharker/storage/project-catalog-authority';
...
test('two UDS clients converge on one Host-owned Project Catalog', {
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-project-catalog-two-client-'));
