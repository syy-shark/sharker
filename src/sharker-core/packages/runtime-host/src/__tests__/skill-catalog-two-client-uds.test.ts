...
test('two local IPC clients converge on one lease-bound Skill catalog authority', {
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-skill-catalog-two-client-'));
  const dataRoot = join(base, 'data-root');
  const projectRoot = join(base, 'project');
  const isolatedHome = join(base, 'home');
  const skillDirectory = join(projectRoot, '.sharker', 'skills', 'private-project-skill');
  ...
  try {
    ...
    assert.ok(
      desktopPage.items.some(
        (item) => item.kind === 'skill' && item.ref === 'project:sharker:private-project-skill',
      ),
    );
    ...
    const blockedDelete = await desktop.request(
      'skill.catalog.mutate',
      {
        ...
        mutation: { kind: 'delete', ref: 'project:sharker:private-project-skill' },
      },
      REQUEST_TIMEOUT_MS,
    );
