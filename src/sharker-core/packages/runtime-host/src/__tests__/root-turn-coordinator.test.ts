...
import { FakeBackend, FAKE_ASK_USER_QUESTION_PROMPT } from '@sharker/runtime/test-only/fake-backend';
import {
  IMPLEMENTATION_AGENT_DEFINITION,
  LOCAL_READ_AGENT_PROFILE,
} from '@sharker/runtime/agent-catalog';
import { mcpProxyToolName } from '@sharker/runtime/mcp-tools';
import {
...
} from '@sharker/runtime/message-authority';
import {
  RuntimeInteractionAdmissionRejectedError,
  type RuntimeInteractionAuthority,
  type RuntimeInteractionRunClosureReason,
} from '@sharker/runtime/interaction-authority';
import { type PreparedSkillInvocationMessage } from '@sharker/runtime/skill-invocation';
import type {
  AgentBackend,
  BackendCompactHistoryInput,
  BackendSendInput,
} from '@sharker/core/backend-types';
import { messageContentDigest, type SessionEvent } from '@sharker/core/events';
import {
...
test('hosted linked child roots share admission, message, terminal, and stop authority', {
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-linked-root-authority-'));
...
async function createFailureFixture(options: {
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-root-turn-message-failure-'));
  const capability = await resolveStorageRoot({
    path: join(base, 'root'),
    kind: 'interactive',
  });
