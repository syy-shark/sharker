...
import { FakeBackend } from '@sharker/runtime/test-only/fake-backend';
import { GOAL_SET_TOOL_NAME } from '@sharker/runtime/goal-tools';
import { goalCheckpoint } from '@sharker/runtime/goal-state';
...
} from '@sharker/storage/goal-authority';
import {
  resolveStorageRoot,
...
async function createFixture(options: { recoverAdmissions?: boolean } = {}): Promise<Fixture> {
  const base = await mkdtemp(join(tmpdir(), 'sharker-goal-root-'));
