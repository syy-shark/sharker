...
test('credential metadata inspection does not create missing State Roots', async (t) => {
  const parent = await mkdtemp(join(tmpdir(), 'sharker-access-metadata-'));
...
test('credential metadata exposes only usable public access state', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-access-metadata-'));
  ...
  const activeSecret = 'sharker_rh_active_secret';
  const pendingSecret = 'sharker_rh_pending_secret';
  const expiredSecret = 'sharker_rh_expired_secret';
  const revokedSecret = 'sharker_rh_revoked_secret';
  const credential = (
    credentialId: string,
    secret: string,
  ...
  const guest = {
    ...credential('guest', 'sharker_rh_guest_secret', 'active'),
...
test('releases a retired execution.inspect.resolve grant from an existing access file', async (t) => {
  const dir = await mkdtemp(join(tmpdir(), 'sharker-access-retired-'));
