...
async function authority(t: test.TestContext): Promise<LocalHostDeploymentAuthorityOptions> {
  const authorityRoot = await mkdtemp(join(tmpdir(), 'sharker-local-owner-'));
...
test('resolves one durable account-local namespace outside cache and State Root paths', () => {
  assert.equal(
    ...
    '/home/ada/.local/share/Sharker/runtime-host-ownership',
  );
  assert.equal(
    resolveLocalHostDeploymentAuthorityRoot({
      platform: 'darwin',
    ...
    '/Users/ada/Library/Application Support/Sharker/runtime-host-ownership',
  );
  assert.equal(
    resolveLocalHostDeploymentAuthorityRoot({
      platform: 'win32',
    ...
    'C:\\Users\\Ada\\AppData\\Local\\Sharker\\runtime-host-ownership',
  );
});

test('different process data environments still compete for one account authority', async (t) => {
  const parent = await mkdtemp(join(tmpdir(), 'sharker-local-owner-env-'));
...
test('creates a missing authority hierarchy before durably publishing the first record', async (t) => {
  const parent = await mkdtemp(join(tmpdir(), 'sharker-local-owner-parent-'));
  t.after(() => rm(parent, { recursive: true, force: true }));
  const options = { authorityRoot: join(parent, 'account', 'Sharker', 'runtime-host-ownership') };
...
test('does not sync above the OS-managed account-home durability boundary', async (t) => {
  ...
  const parent = await mkdtemp(join(tmpdir(), 'sharker-local-owner-home-boundary-'));
  const homeDir = join(parent, 'home');
...
test('retries a directory-entry durability barrier after mkdir', async (t) => {
  if (process.platform === 'win32') return;
  const parent = await mkdtemp(join(tmpdir(), 'sharker-local-owner-mkdir-sync-'));
  t.after(() => rm(parent, { recursive: true, force: true }));
  let remainingFailures = 2;
  const options: LocalHostDeploymentAuthorityOptions = {
    authorityRoot: join(parent, 'account', 'Sharker', 'runtime-host-ownership'),
