...
} from '@sharker/core/usage-stats/pricing';
import type { PricingConfig } from '@sharker/core/usage-stats/types';
import { BUILTIN_PRICING } from '@sharker/runtime/telemetry';
...
test('usage authority drain rejects a new pricing mutation with typed lifecycle failure', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-usage-pricing-drain-'));
...
describe('production Usage/Pricing UDS', () => {
  test('two clients share usage projection and one revision-CAS pricing authority', {
    ...
    const base = await mkdtemp(join(tmpdir(), 'sharker-usage-pricing-two-client-'));
...
async function withUsageAuthority(
  ...
  const base = await mkdtemp(join(tmpdir(), `sharker-usage-pricing-${name}-`));
  const capability = await resolveStorageRoot({
    path: join(base, 'interactive-root'),
    kind: 'interactive',
  });
