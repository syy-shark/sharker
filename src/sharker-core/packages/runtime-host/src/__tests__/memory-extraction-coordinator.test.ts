...
import type { RuntimePolicyReader } from '@sharker/storage/runtime-policy-stores';
import {
  buildHistoryCompactCheckpoint,
  type HistoryCompactCheckpoint,
} from '@sharker/runtime/history-compact-checkpoint';

import { type MemoryExtractionSourceSnapshot } from '@sharker/runtime/memory-extraction';
  ...
  test('extracts incidental memory through the post-terminal memory_extract path', async () => {
    await withMemoryWriter(async (writer) => {
      ...
      const stored = await writer.searchByKeys({
        ...
        match: 'exact',
        workspaceKey: '/workspace/sharker',
      });
      assert.deepEqual(
  ...
  test('crosses Runs with a Session Cursor, preserves provider configuration, appends changes, and replays exactly', async () => {
    await withMemoryWriter(async (writer) => {
      ...
      assert.deepEqual(
        ...
        [
          ['The user prefers detailed English.', 'workspace', '/workspace/sharker'],
...
function header(): SessionHeader {
  return {
    id: 'session-1',
    workspaceRoot: '/workspace/sharker',
    cwd: '/workspace/sharker',
    createdAt: 1,
    name: 'Memory test',
    titleIsManual: false,
    isFlagged: false,
...
async function withMemoryWriter(
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-memory-extraction-'));
