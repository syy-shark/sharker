...
import { DEEP_RESEARCH_SESSION_LABEL, DEEP_RESEARCH_SESSION_NAME } from '@sharker/core/deep-research';
import { type RelayModelProfile } from '@sharker/core/model-thinking';
import {
...
import { type ProjectCatalog, ProjectUnavailableError } from '@sharker/storage/project-catalog';
import { SessionNotFoundError } from '@sharker/storage/session-store';
import type { ResolveExecutionConnectionResult } from '@sharker/storage/runtime-policy-stores';
import {
...
test('creation refuses a retired provider on the default target', async () => {
  ...
  assert.deepEqual(outcome, {
    ...
    error: {
      code: 'invalid_request',
      message: 'Session model connection uses a sign-in that was removed from Sharker',
    },
  });
  assert.equal(createAttempts, 0);
});
...
test('creation persists a canonical cwd while fingerprints retain exact target intent', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-session-create-cwd-'));
...
test('creation resolves a Project alias and records Host-owned usage', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-session-create-project-'));
...
test('Host-path relocation canonicalizes once and commits through Runtime authority', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-session-relocate-cwd-'));
  const target = join(root, 'target');
  const link = join(root, 'link');
  await mkdir(target);
  await symlink(target, link, 'dir');
