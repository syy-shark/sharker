...
test('revision is stable, project-bound, excludes state timestamps, and tracks external content', async () => {
  ...
  await createSkill(join(fixture.project, '.sharker', 'skills'), 'alpha', skillBody('Alpha', 'one'));
  ...
  await mkdir(join(fixture.root, '.sharker'), { recursive: true });
  await writeFile(
    join(fixture.root, '.sharker', 'skills-state.json'),
    stateFile('project:sharker:alpha', true, false, '2026-01-01T00:00:00.000Z'),
  );
  const withState = await start(repository, fixture.project, 'governance');
  await writeFile(
    join(fixture.root, '.sharker', 'skills-state.json'),
    stateFile('project:sharker:alpha', true, false, '2027-02-02T00:00:00.000Z'),
  );
  ...
  await writeFile(
    join(fixture.project, '.sharker', 'skills', 'alpha', 'SKILL.md'),
  ...
  const otherProject = await tempDirectory('sharker-skill-other-project-');
  await createSkill(
    join(otherProject, '.sharker', 'skills'),
    'alpha',
    skillBody('Alpha', 'externally changed'),
  );
...
test('continuation pages are pinned to a freshly scanned revision', async () => {
  const fixture = await createFixture();
  const root = join(fixture.project, '.sharker', 'skills');
...
test('continuation rejects a cursor at the exact end of the current view', async () => {
  const fixture = await createFixture();
  await createSkill(
    join(fixture.project, '.sharker', 'skills'),
    'only-skill',
...
test('governance context status contains structural facts without synthetic budget ranks', async () => {
  ...
  assert.equal(governanceItem(page, 'project:sharker:active').contextStatus, 'unknown');
...
test('removed bundled sources lose provenance trust without disabling the local copy', async () => {
  ...
  const content = skillBody('Retired Bundled Skill', 'installed by an older Sharker release');
...
test('external project sources are read-only while Data Root preferences use durable schema v2', async () => {
  ...
  const state = JSON.parse(
    await readFile(join(fixture.root, '.sharker', 'skills-state.json'), 'utf8'),
...
test('noncanonical external ids remain wire-safe governance entries', async () => {
  ...
  const repository = fixture.repository();
  const page = await start(repository, fixture.project, 'governance');
  assert.equal(governanceItem(page, 'project:sharker:valid skill').id, 'valid skill');
  ...
  await assert.rejects(readFile(join(fixture.root, '.sharker', 'skills-state.json')), {
    code: 'ENOENT',
  });
  ...
  const disabled = await repository.mutate({
    ...
    mutation: {
      kind: 'set_enabled',
      ref: 'project:sharker:valid skill',
      enabled: false,
    },
  });
...
test('repository preserves filesystem-safe ids and codec preserves the 256-byte display-id boundary', async () => {
  ...
  const repository = fixture.repository();
  const page = await start(repository, fixture.project, 'governance');
  const filesystemRef = `project:sharker:${filesystemId}`;
...
test('Host previews, updates, and recovers the canonical Desktop managed-install layout', async () => {
  ...
  await mkdir(join(skillDirectory, '.sharker', 'baseline'), { recursive: true });
  ...
  await writeFile(join(skillDirectory, '.sharker', 'baseline', 'SKILL.md'), versionOne);
  ...
  assert.equal(
    await readFile(join(skillDirectory, '.sharker', 'baseline', 'SKILL.md'), 'utf8'),
    versionTwo,
  );
});
...
test('managed update blocks a symlink redirected outside its discovery root after scanning', async () => {
  ...
  await mkdir(join(containedSkill, '.sharker', 'baseline'), { recursive: true });
  await writeFile(
  ...
  await writeFile(join(containedSkill, '.sharker', 'baseline', 'SKILL.md'), installedContent);
  ...
  const outsideSkill = await createSkill(
    await tempDirectory('sharker-skill-symlink-race-outside-'),
    sourceId,
    outsideContent,
  );
  await mkdir(join(outsideSkill, '.sharker', 'baseline'), { recursive: true });
  await writeFile(
  ...
  await writeFile(join(outsideSkill, '.sharker', 'baseline', 'SKILL.md'), outsideContent);
...
test('revision covers exact managed lock and baseline bytes and rejects post-snapshot edits', async () => {
  ...
  const skillDirectory = join(fixture.root, 'skills', sourceId);
  const baselinePath = join(skillDirectory, '.sharker', 'baseline', 'SKILL.md');
...
async function createFixture(): Promise<Fixture> {
  const base = await tempDirectory('sharker-skill-repository-');
  const root = join(base, 'data');
  const project = join(base, 'project');
  const home = join(base, 'home');
  const sources = join(home, '.sharker', 'skill-sources');
  await Promise.all([
