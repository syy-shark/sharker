...
test('imports only the exact repository identity bound to the admission capability', async (t) => {
  ...
  assert.equal(
    gitBare(destinationRepositoryPath, ['rev-parse', 'refs/sharker/accepted']),
...
test('publishes an operation-bound candidate without advancing accepted authority', async (t) => {
  ...
  assert.equal(
    ...
    `refs/sharker/candidates/${createHash('sha256').update('operation-1').digest('hex')}`,
  );
  assert.equal(
    gitBare(destinationRepositoryPath, ['rev-parse', 'refs/sharker/accepted']),
    imported.baselineCommitOid,
  ...
  assert.equal(candidateProof.acceptedRef, 'refs/sharker/accepted');
...
async function createRepository(t: TestContext, objectFormat: 'sha1' | 'sha256') {
  const repositoryPath = await realpath(await mkdtemp(join(tmpdir(), 'sharker-gitoxide-admission-')));
