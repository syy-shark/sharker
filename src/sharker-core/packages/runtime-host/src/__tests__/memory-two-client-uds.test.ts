...
} from '@sharker/storage/memory-bundle-store';
import {
...
test('two UDS clients share one recoverable Memory authority across Host death', {
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-memory-'));
  ...
  try {
    ...
    try {
      ...
      const rawContent = [
        ...
        '',
        '## UDS preference',
        '<!-- sharker-memory: id=uds status=active scope=workspace -->',
        'x'.repeat(40_000),
...
async function seedPartialMemoryTransaction(
  ...
  try {
    ...
    const targetMemory = Buffer.from('# Sharker Memory\n\n## Seed\nSeed content.\n');
    const targetPending = Buffer.from('# Sharker Pending Memory\n');
