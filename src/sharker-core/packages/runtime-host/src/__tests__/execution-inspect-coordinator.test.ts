...
import type { AgentRunHeader, EmittedAgentRunEvent } from '@sharker/core/agent-run';
import { MODEL_CALL_ATTEMPT_SCHEMA_VERSION } from '@sharker/core/model-call-attempt';
...
async function withCoordinator(
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-host-inspect-'));
