...
import type { StoredInteractionRequest } from '@sharker/storage/interaction-store';
import { acquireOperationalStateDatabase } from '@sharker/storage/operational-state-store';
import {
  createSessionEventMapMemory,
  mapSessionEventToRuntimeEvent,
} from '@sharker/runtime/session-event-runtime-mapper';
...
async function withStores(
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-canonical-session-'));
