...
import type { ShellRunRecord } from '@sharker/core/shell-run';
import { FAKE_ASK_USER_QUESTION_PROMPT, FakeBackend } from '@sharker/runtime/test-only/fake-backend';
import { LOCAL_READ_AGENT_DEFINITION } from '@sharker/runtime/agent-catalog';
import { SessionManager } from '@sharker/runtime/session-manager';
import { fingerprintAgentGraphRunnableIntent } from '@sharker/runtime/stream-graph-admission';
import type { AgentGraphRunnableIntent } from '@sharker/runtime/stream-graph-readiness';
...
} from '@sharker/storage/long-term-memory-store';
import {
  resolveStorageRoot,
  tryAcquireInteractiveRootOwner,
  type InteractiveRootOwner,
} from '@sharker/storage/root-authority';
import { openInteractiveUsageStoresForWrite } from '@sharker/storage/usage-stores';
import { openInteractiveShellRunStoreForWrite } from '@sharker/storage/shell-run-authority';
 ...
 * filename is `OPERATIONAL_STATE_DATABASE_NAME` in `@sharker/storage`, which the
...
async function withCompositionRoot(
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-execution-composition-'));
