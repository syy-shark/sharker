...
import type { ProxiedFetchProxy } from '@sharker/runtime/network/scoped-fetch-transport';
import type {
...
test('Host WebSearch consumes one canonical credential/proxy snapshot and closes transport', async () => {
        ...
        fetch: async (_input, init) => {
          ...
          return Response.json({
            results: [
              {
                title: 'Sharker',
                url: 'https://sharker.example/current',
  ...
  const result = await tool.impl({ query: ' latest Sharker ', limit: 1 }, context());
...
function context(): SharkerToolContext {
  return {
    sessionId: 'session-1',
    turnId: 'turn-1',
    cwd: '/tmp',
