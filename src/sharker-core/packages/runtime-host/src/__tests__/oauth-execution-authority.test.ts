...
import { type ProxiedFetchTransport } from '@sharker/runtime/network/scoped-fetch-transport';
import {
...
async function withCopilotCredential(
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-oauth-execution-'));
...
async function withSeededOAuthCredential(
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-host-oauth-seeded-'));
  const capability = await resolveStorageRoot({
    path: join(base, 'interactive'),
    kind: 'interactive',
  });
