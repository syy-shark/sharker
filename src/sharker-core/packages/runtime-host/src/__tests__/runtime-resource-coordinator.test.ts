...
import { SHELL_RUN_SOURCE_TOOL_CALL_ID_MAX_BYTES } from '@sharker/core/shell-run';
import {
  type ShellRunSnapshotResult,
  type ShellRunStateResult,
  type ShellRunUpdate,
} from '@sharker/core/events';
import type { ShellRunBashInput, ShellRunWriteInput } from '@sharker/runtime/shell-run-contract';
import { ShellPreferenceError } from '@sharker/runtime/shell-detect';
...
const RUNTIME_REF = 'sharker://runtime/background-tasks/shell-1';
...
function shellRef(index: number): string {
  return `sharker://runtime/background-tasks/shell-${index}`;
}
