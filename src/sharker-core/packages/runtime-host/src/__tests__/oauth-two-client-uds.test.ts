...
import { parseOAuthSubscriptionTokens } from '@sharker/runtime/subscription-credentials';
...
test('two OAuth creates bind distinct entities and present only on their initiating Clients', {
  ...
  const base = await mkdtemp(join(tmpdir(), 'sharker-oauth-two-client-'));
...
test('OAuth enrollment honors the Codex opt-out flag over the real endpoint', {
  ...
  const cases = [['openai-codex', { SHARKER_CODEX_SUBSCRIPTION_EXPERIMENTAL: '0' }]] as const;
...
async function assertProviderDisabledOverUds(
  ...
  const base = await mkdtemp(join(tmpdir(), `sharker-oauth-disabled-${provider}-`));
  const root = join(base, 'root');
  let host: RuntimeHostKernel | undefined;
