  ...
  test('retires a bound child worktree only after the Session tombstone commits', async () => {
    await withHarness(async (harness) => {
      const binding = {
        ...
        worktreePath: '/tmp/sharker-subagent-worktree',
...
async function withHarness(
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-session-retirement-'));
...
interface RetirementHarness {
  ...
  retireWorktree:
    | ((binding: import('@sharker/core/subagent-workspace').SubagentWorkspaceBinding) => Promise<void>)
    | undefined;
}
