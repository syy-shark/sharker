...
import type { SandboxBoundaryRequest } from '@sharker/core/sandbox-boundary';
import type { SandboxBoundaryRequestEvent, UserQuestionRequestEvent } from '@sharker/core/events';
import {
...
} from '@sharker/storage/interaction-store';
import {
  resolveStorageRoot,
...
async function withStore(run: (context: StoreContext) => Promise<void>): Promise<void> {
  const base = await mkdtemp(join(tmpdir(), 'sharker-interaction-coordinator-'));
