...
import type { MessageContent } from '@sharker/core/events';
...
import type { ScheduledTask } from '@sharker/core/scheduled-task';
...
type TaskLedgerTool<Input> = SharkerTool<Input, string> & {
  parameters: { parse(value: unknown): Input };
};

async function withOwnedTaskLedgerToolPort<T>(
  fixture: ExecutionFixture,
  run: (coordinator: HostTaskLedgerCoordinator, tools: SharkerTool[]) => Promise<T>,
): Promise<T> {
  const owner = await tryAcquireInteractiveRootOwner(fixture.capability);
  assert.ok(owner);
...
function taskLedgerToolContext(
  fixture: ExecutionFixture,
  identity: Pick<SharkerToolContext, 'runId' | 'turnId' | 'toolCallId'>,
): SharkerToolContext {
  return {
    sessionId: fixture.sessionId,
    cwd: fixture.root,
