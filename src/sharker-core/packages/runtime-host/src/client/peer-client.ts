...
export function createRuntimeHostPeerClientFromEnvironment(
  ...
  const nativePath = environment.SHARKER_RUNTIME_HOST_PEER_NATIVE_PATH;
  const keyPath = environment.SHARKER_RUNTIME_HOST_PEER_KEY_PATH;
  if (!nativePath || !keyPath) {
    throw new RuntimeHostPeerError(
      'peer_native_unavailable',
      'Experimental direct peer requires SHARKER_RUNTIME_HOST_PEER_NATIVE_PATH and SHARKER_RUNTIME_HOST_PEER_KEY_PATH',
    );
  }
