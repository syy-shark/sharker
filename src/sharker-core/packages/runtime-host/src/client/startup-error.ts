  ...
  switch (reason) {
    case 'stored_data_incompatible':
      return new RuntimeHostStartupError(
        reason,
        'Sharker cannot read part of this workspace’s stored data. The workspace was left in place. Update Sharker or report diagnostic code STORED_DATA_INCOMPATIBLE.',
      );
    case 'operational_state_migration_blocked':
      return new RuntimeHostStartupError(
        reason,
        'Sharker could not safely upgrade this workspace and left it unchanged. Reopen it with the previous Sharker release to export or remove incompatible data, then try again. Diagnostic code: OPERATIONAL_STATE_MIGRATION_BLOCKED.',
      );
    ...
    case 'startup_timeout':
      return new Error(
        `No Runtime Host became ready before the startup deadline elapsed${electionDiagnosticSuffix(diagnostic)}. Retry; if this workspace needs longer to open (large workspaces can after an upgrade), set SHARKER_RUNTIME_HOST_ELECTION_DEADLINE_MS to allow more time.`,
      );
    case 'host_unresponsive':
      return new Error(
        `A Runtime Host was found but did not become ready before the startup deadline elapsed${electionDiagnosticSuffix(diagnostic)}. It may still be opening this workspace (large workspaces can need longer right after an upgrade); retrying once it settles usually succeeds, or set SHARKER_RUNTIME_HOST_ELECTION_DEADLINE_MS to allow more time.`,
      );
  }
}
