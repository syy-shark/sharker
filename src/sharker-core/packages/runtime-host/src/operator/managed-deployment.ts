...
import {
  ...
  syncDirectory,
  syncDirectoryChain,
} from '@sharker/storage/stable-storage';
import { z } from 'zod';
...
export function resolveRuntimeHostManagedDeploymentAuthorityRoot(
  ...
  const segments =
    platform === 'darwin'
      ? ['Library', 'Application Support', 'Sharker', 'runtime-host-deployments']
      : platform === 'win32'
        ? ['AppData', 'Local', 'Sharker', 'runtime-host-deployments']
        : ['.local', 'share', 'Sharker', 'runtime-host-deployments'];
