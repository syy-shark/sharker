...
import { withProcessLifetimeFileUpdateLock } from '@sharker/storage/process-lifetime-file-update-lock';
import { z } from 'zod';
import {
...
function resolveLocalHostDeploymentAuthorityLocation(
  ...
  const pathSegments =
    platform === 'darwin'
      ? ['Library', 'Application Support', 'Sharker', 'runtime-host-ownership']
      : platform === 'win32'
        ? ['AppData', 'Local', 'Sharker', 'runtime-host-ownership']
        : ['.local', 'share', 'Sharker', 'runtime-host-ownership'];
  return {
