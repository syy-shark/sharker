 ...
 * coding agent (Claude Code, Codex). Sharker can list them and, on request,
 * distill one into a handoff digest so the user continues work in a fresh
 * Sharker session without re-explaining context.
 *
 ...
 * through before it may reach a Sharker surface or an LLM prompt:
 *
 ...
 *
 * IO lives in @sharker/storage (foreign-session-store.ts); this module is pure.
 */
...
export interface ForeignSessionSummary {
  ...
  /** Source-native id (Claude uuid / Codex thread id). Opaque to Sharker. */
  id: string;
 ...
 * into Sharker context. Old tool output is stale evidence anyway; the handoff
 ...
 * foreign-session scanner below and the Codex Session adapter in `@sharker/storage`.
 ...
 * Sharker-controlled enums/timestamps.
 */
