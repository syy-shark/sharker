 ...
 * Sharker's existing raw Session representation.
 *
 * There is intentionally no intermediate external-message model here. Each
 * adapter owns its source format and emits canonical Sharker StoredMessages.
 */
export interface ExternalSharkerSession {
...
export interface ExternalSessionAdapter {
  ...
  readSession(sessionId: string): Promise<ExternalSharkerSession>;
}
