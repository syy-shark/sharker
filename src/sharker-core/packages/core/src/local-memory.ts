...
export function defaultLocalMemoryMarkdown(sha256: Sha256Digest, now = Date.now()): string {
  ...
  return [
    ...
    '',
    '## 示例：我的偏好',
    `<!-- sharker-memory: id=${exampleId} origin=manual createdAt=${timestamp} -->`,
    exampleContent,
    '',
  ].join('\n');
}
...
export function appendManualLocalMemoryEntryDraft(
  ...
  const entry = [`## ${title}`, `<!-- sharker-memory: ${meta} -->`, content].join('\n');
  const draft =
...
export function setLocalMemoryEntryStatusDraft(
  ...
  const metaLine = `<!-- sharker-memory: ${serializeMetaComment(meta)} -->`;
...
function parseMetaComment(line: string): Record<string, string> | null {
  const match = /^<!--\s*sharker-memory:\s*(.*?)\s*-->$/.exec(line.trim());
