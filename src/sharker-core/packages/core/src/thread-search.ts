 ...
 *   - Contract: `@sharker/core/search` (PR-SEARCH-0 + PR-SEARCH-1.5 `SearchResultTarget`).
 ...
 *   - Snippets are redacted via `@sharker/core/redaction.redactSecrets()`.
 ...
 *   - Result limits come from `@sharker/core/search.normalizeSearchLimit`
 ...
 *     (thread navigation does NOT use `sharker://session`).
 *
 ...
 *   - No `sharker://session` URI construction.
 */
...
export async function runThreadSearch(
  ...
  // L3: query / limit normalization via @sharker/core helpers — single
  ...
  sessionScan: for (const session of sessions) {
    ...
    for (let messageIndex = 0; messageIndex < messages.length; messageIndex += 1) {
      ...
      results.push({
        ...
        // `url` field for thread results (sharker://session is deferred).
        target: {
          kind: 'thread',
          sessionId: session.id,
          ...(turnId ? { turnId } : {}),
