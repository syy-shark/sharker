...
describe('PermissionProfile factories', () => {
  ...
  test('workspace-write profile allows tmp writes when tmp context is provided', () => {
    ...
    expect(canWritePath(profile, '/private/tmp/sharker/out.txt', WORKSPACE_CONTEXT)).toBe(true);
    expect(canWritePath(profile, '/tmp/sharker-out.txt', WORKSPACE_CONTEXT)).toBe(true);
    expect(canWritePath(profile, '/tmp2/sharker-out.txt', WORKSPACE_CONTEXT)).toBe(false);
  });
  ...
  test('danger-full-access profile is managed unrestricted access with network enabled', () => {
    ...
    expect(canWritePath(profile, '/var/log/sharker.log')).toBe(true);
  });
});
...
describe('PermissionProfile matcher rules', () => {
  ...
  test('special entries resolve through matcher context', () => {
    ...
    expect(canWritePath(profile, '/private/tmp/sharker/result.txt', WORKSPACE_CONTEXT)).toBe(true);
    expect(canWritePath(profile, '/private/tmp2/sharker/result.txt', WORKSPACE_CONTEXT)).toBe(false);
  });
});
