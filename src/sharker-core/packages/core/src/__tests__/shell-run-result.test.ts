...
describe('mergeShellRunState', () => {
  ...
  it('rejects conflicting state at one revision and a different ref', () => {
    ...
    const other = shellRun({ ref: 'sharker://runtime/background-tasks/other', revision: 3 });
...
describe('ShellRun view updates', () => {
  ...
  it('bounds hydration updates while retaining the latest revision of recently active refs', () => {
    ...
    const first = shellRunUpdate({
      result: shellRun({ ref: 'sharker://runtime/background-tasks/run-1', revision: 1 }),
    });
    const second = shellRunUpdate({
      result: shellRun({ ref: 'sharker://runtime/background-tasks/run-2', revision: 1 }),
    });
    const refreshedFirst = shellRunUpdate({
      result: shellRun({ ref: 'sharker://runtime/background-tasks/run-1', revision: 2 }),
    });
    const third = shellRunUpdate({
      result: shellRun({ ref: 'sharker://runtime/background-tasks/run-3', revision: 1 }),
    });
    ...
    assert.deepEqual(
      ...
      [
        ['sharker://runtime/background-tasks/run-1', 2],
        ['sharker://runtime/background-tasks/run-3', 1],
      ],
    );
...
function shellRun(
  ...
  return {
    kind: 'shell_run',
    ref: 'sharker://runtime/background-tasks/run-1',
    mode: 'pipes',
    status: 'running',
    cwd: '/repo',
    cmd: 'sleep 1',
