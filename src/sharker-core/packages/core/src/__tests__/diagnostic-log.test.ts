...
test('collapses the home path belonging to the process that captured the logs', () => {
  assert.equal(
    collapseHomePath('/home/remote-user/.sharker/logs', '/home/remote-user', 'linux'),
    '~/.sharker/logs',
  );
  assert.equal(
    collapseHomePath('C:\\Users\\Remote\\.sharker', 'c:\\users\\remote', 'win32'),
    '~\\.sharker',
  );
  assert.equal(
    collapseHomePath(
      'file:///C:/Users/Remote%20User%231/.sharker/logs',
