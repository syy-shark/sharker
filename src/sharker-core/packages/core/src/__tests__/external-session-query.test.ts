...
function summary(over: Partial<ExternalSessionSummary> = {}): ExternalSessionSummary {
  return { id: 'sess-1', name: 'Fix the parser', cwd: '/Users/z/sharker-agent', ...over };
}
...
describe('externalSessionMatchesQuery', () => {
  ...
  test('a term matches the project path', () => {
    ...
    assert.equal(externalSessionMatchesQuery(summary(), { text: 'sharker-agent' }), true);
  ...
  test('cwd and text both apply, not either', () => {
    const query = { cwd: '/Users/z/sharker-agent', text: 'parser' };
  ...
  test('a term matching one field is enough', () => {
    assert.equal(
      externalSessionMatchesQuery(summary({ name: 'untitled' }), { text: 'sharker-agent' }),
      true,
    );
