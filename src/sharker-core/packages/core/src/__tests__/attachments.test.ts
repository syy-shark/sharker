...
describe('attachment resource refs', () => {
  test('round-trips one canonical Session Artifact without embedding Session authority', () => {
    ...
    assert.equal(value, 'sharker://runtime/attachments/attachment-123');
  ...
  test('rejects file paths and non-canonical resource spellings', () => {
    ...
    assert.equal(parseAttachmentResourceRef('sharker://runtime/attachments/a?session=other'), null);
    assert.equal(parseAttachmentResourceRef('sharker://runtime/attachments/a/b'), null);
  });
});
