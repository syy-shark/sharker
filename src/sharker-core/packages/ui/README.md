...
Shared UI layer for the Sharker desktop app. Astryx is the authority for generic components; Sharker keeps product-specific composition and state. The package is consumed by `apps/desktop`'s renderer, while the preload bridge imports types only.

The published Astryx API is a fixed dependency boundary. New consumers adapt to its taxonomy instead of recreating retired Sharker APIs or styling its internal DOM; existing renderer overrides are acknowledged transitional states documented in `apps/desktop/src/renderer/README.md`.
...
| `src/primitives/` | Sharker-specific compositions that do not duplicate an Astryx component. | product-specific |
...
```ts
import { Badge, Button, ChatView, Composer, PageHeader, useToast } from '@sharker/ui';
```

Sub-path exports (declared in `package.json` `exports`): `@sharker/ui/artifact-preview-registry`, `@sharker/ui/assistant-stream`, `@sharker/ui/icons`, and `@sharker/ui/sharker-uri`. (`@sharker/ui/icons` re-exports Lucide symbols; model-provider brand logos live in the renderer's `settings/provider-*`, not here — bot-provider logos are in `@sharker/ui`'s `bot-brand-logo`.)
...
- **Generic component need** → use the closest published Astryx taxonomy and redesign the Sharker consumer when the old shape does not fit.
