...
import { SETTINGS_SECTIONS, type SettingsSection } from '@sharker/core/settings';
 ...
 */
export function parseSharkerUri(href: string): SharkerUriDest | null {
  if (typeof href !== 'string') return null;
  if (href.length === 0 || href.length > RAW_HREF_MAX_LENGTH) return null;
  if (!href.startsWith('sharker:')) return null;
  ...
  if (url.protocol !== 'sharker:') return null;
 ...
 */
export function isSharkerUriCandidate(href: string): boolean {
  return typeof href === 'string' && /^sharker:/i.test(href);
}
