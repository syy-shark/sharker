 ...
 * module so the `@sharker/ui` public API surface stays identical.
 *
...
import { SharkerWordmark } from './sharker-wordmark.js';
...
export function EmptyChatHero(props: {
  ...
  // #1433: the visual used to be two chat bubbles plus a Sharker/user avatar
  ...
  return (
    <section className="sharker-hero sharker-hero-empty-chat" aria-label={copy.ariaLabel}>
      {/* 160px puts the mark's cap height at ~42px against the 25px
          ...
          neither led. */}
      <div className="sharker-hero-visual">
        <SharkerWordmark width={160} />
      </div>
      <header>
        <h1>
...
export function DeepResearchEmptyHero(props: { onPromptSuggestion?(prompt: string): void }) {
  ...
  return (
    <section className="sharker-hero sharker-hero-empty-chat sharker-hero-deep-research" aria-label={copy.ariaLabel}>
      <header>
        <span className="sharker-hero-eyebrow">
      ...
      <ol className="sharker-deep-research-workflow" aria-label={copy.workflowAriaLabel}>
        {copy.workflow.map((step) => (
          <li key={step.title}>
            <span className="sharker-deep-research-workflow-title">{step.title}</span>
            <span className="sharker-deep-research-workflow-body">{step.body}</span>
          </li>
        ))}
      </ol>
      <section className="sharker-deep-research-report" aria-label={copy.reportAriaLabel}>
        <h2>{copy.reportTitle}</h2>
        <ul>
          {copy.report.map((section) => (
            <li key={section.title}>
              <span className="sharker-deep-research-report-title">{section.title}</span>
              <span className="sharker-deep-research-report-body">{section.body}</span>
            </li>
          ))}
        </ul>
      </section>
      <section className="sharker-deep-research-scope" aria-label={copy.scopeAriaLabel}>
        <h2>{copy.scopeTitle}</h2>
        <ul>
          {copy.scope.map((option) => (
            <li key={option.label}>
              <span className="sharker-deep-research-scope-label">{option.label}</span>
              <span className="sharker-deep-research-scope-body">{option.body}</span>
            </li>
          ))}
        </ul>
      </section>
      <section className="sharker-deep-research-evidence" aria-label={copy.evidenceAriaLabel}>
        <h2>{copy.evidenceTitle}</h2>
        <ul>
          {copy.evidence.map((item) => (
            <li key={item.title}>
              <span className="sharker-deep-research-evidence-title">{item.title}</span>
              <span className="sharker-deep-research-evidence-body">{item.body}</span>
            </li>
          ))}
        </ul>
      </section>
      <section className="sharker-deep-research-progress" aria-label={copy.progressAriaLabel}>
        <h2>{copy.progressTitle}</h2>
        <ul>
          {copy.progress.map((item) => (
            <li key={item.title}>
              <span className="sharker-deep-research-progress-title">{item.title}</span>
              <span className="sharker-deep-research-progress-body">{item.body}</span>
            </li>
          ))}
        </ul>
      </section>
      {props.onPromptSuggestion && (
        <ul className="sharker-prompt-suggestions" aria-label={copy.startersAriaLabel}>
