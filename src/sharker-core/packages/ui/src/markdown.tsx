 ...
 * `SharkerUriContext` (which the renderer installs once at the App root)
 ...
 * of `components.tsx`. `SharkerUriContext` was already a public export
 * (the renderer's main.tsx provides the dispatcher), so `index.ts`
 * re-exports the new module to keep the `@sharker/ui` surface identical.
  ...
  return (
    <Suspense
      ...
      fallback={streaming ? null : (
        <div className="sharker-markdown sharker-markdown-pending" style={{ whiteSpace: 'pre-wrap' }}>
          {safeText}
        </div>
      )}
    >
 ...
 * typed `SharkerUriDest` and routes to whatever real navigation surface
 ...
 * lazy body reads it via `useContext(SharkerUriContext)` imported back
 * from here.
 */
export const SharkerUriContext = createContext<((dest: import('./sharker-uri.js').SharkerUriDest) => void) | undefined>(undefined);
