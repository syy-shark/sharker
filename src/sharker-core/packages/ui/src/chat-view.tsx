...
import type { DeepResearchClientProgress } from '@sharker/core/deep-research-run';
...
import { isDeepResearchSession } from '@sharker/core/deep-research';
...
export function TranscriptHistoryNotice({
  ...
  return (
    <HStack
      className="sharker-transcript-history-controls"
      gap={2}
      hAlign="center"
      vAlign="center"
      wrap="wrap"
...
export function ChatView(props: {
   ...
   *  avoid bringing the full provider SVG library into @sharker/ui. */
   ...
   * stay outside `@sharker/ui`, but they read the SAME turn objects the transcript
   ...
   * sessions list — @sharker/ui never queries the storage layer directly.
   */
  branchBanner?: {
   ...
   * `attachments.readBytes`; non-desktop hosts may omit it. Keeps @sharker/ui
  ...
  if (!props.activeSession) {
    ...
    return (
      <section
        className="sharker-main agents-chat-panel agents-chat-view-root"
        role="region"
      ...
      >
        ...
        {/* No status strip on the empty-session screen: it has no session, so
            ...
            owns. */}
        <ChatMessageList
          className="sharker-chat-message-list sharker-chatContent"
  ...
  const emptyContent = props.messageLoading
    ? (
        <div className="sharker-chat-message-loading">
  ...
  return (
    ...
    >
      ...
      <div className="sharker-chat-shell">
          ...
          {showEmptyState ? null : (
            <>
              {/* A transient is already the first visible conversation row.
                  Do not prepend the empty-chat Sharker hero while that optimistic
              ...
              {turns.map((turn) => {
                return (
                  <div
                    key={turn.turnId}
                    className="sharker-transcript-turn"
              ...
              {/* #642 fallback: streaming began before the optimistic user turn
                  ...
                  answer in a bare `.sharker-turn` so it isn't dropped. Mutually
              ...
              {streamingActive && !tailTurnId && (
                <section className="sharker-turn" data-live-streaming="true">
                  <LocalizedChatMessage
        ...
        {selectionQuote && (props.onQuoteSelection || props.onAskAboutSelection) ? (
          selectionActionsLayer.render(
            <div
              className="sharker-quote-actions"
...
export function DeepResearchProgressPanel({
  ...
  return (
    <section
      className="sharker-deep-research-run-panel"
      aria-label={copy.ariaLabel}
      data-status={run.status}
    >
      <div className="sharker-deep-research-run-summary">
        <div>
        ...
        <div className="sharker-deep-research-run-actions">
          <span className="sharker-deep-research-run-count">
          ...
          {run.status === 'completed' && run.implementationPrompt && onContinue && (
            <Button
              ...
              variant="secondary"
              size="sm"
              className="sharker-deep-research-handoff-button"
      ...
      <div className="sharker-deep-research-run-grid">
        <div>
        ...
        <div>
          ...
          {run.blockers.length > 0 ? (
            <ul className="sharker-deep-research-run-blockers">
