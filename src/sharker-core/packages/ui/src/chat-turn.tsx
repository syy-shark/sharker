...
import { formatAbsoluteTimestamp } from '@sharker/core/relative-time';
...
function AttachmentImage(props: { attachment: AttachmentRef }) {
  ...
  if (!src) {
    return (
      <Thumbnail
        className="sharker-user-attachment-thumbnail"
...
const UserMessageBody = memo(function UserMessageBody(props: {
  ...
  const userMetadata = (
    <ChatMessageMetadata
      className="sharker-message-meta"
      timestamp={
        props.ts !== undefined ? (
          ...
          (<Timestamp className="sharker-message-time-inline" value={props.ts} format="auto" isLive />)
        ) : undefined
      }
      footer={
        <>
  ...
  return (
    <>
      {nonImageAttachments.length > 0 ? (
        <HStack gap={1} wrap="wrap" maxWidth="100%" className="sharker-user-attachment-tokens">
      ...
      {props.quotes && props.quotes.length > 0 ? (
        <div className="sharker-user-quotes">
      ...
      {imageAttachments.length > 0 ? (
        <HStack gap={1} wrap="wrap" maxWidth="100%" className="sharker-user-attachments">
      ...
      <ChatMessageBubble
        className="sharker-chat-message-bubble sharker-chat-message-bubble-user"
  ...
  return (
    <div data-transient-message-id={message.id}>
      <LocalizedChatMessage
        ...
        sender="user"
        className="sharker-chat-message sharker-user-message"
      >
        <UserMessageBody
...
export const TurnView = memo(function TurnView(props: {
   ...
   * own `enabled` flag + tooltip; @sharker/ui doesn't compute these
   ...
   * @sharker/ui just renders the badge UI.
   */
  ...
  return (
    <section
      className="sharker-turn"
      data-sharker-contract="markdown-flow"
    ...
    >
      ...
      {turn.notes.map((note) => (
        <ChatSystemMessage
          key={note.id}
          className="sharker-chat-system-message"
        ...
        if (segment.kind === 'user') {
          ...
          return (
            <LocalizedChatMessage
              ...
              accessibleLabel={copy.userAriaLabel}
              sender="user"
              className="sharker-chat-message sharker-user-message sharker-steering-message"
            >
              <UserMessageBody
                messageId={message.id}
                text={message.text}
        ...
        return (
          <Fragment key={assistantKey}>
            <LocalizedChatMessage
              ...
              className="sharker-chat-message sharker-assistant-answer"
            >
            <div className="sharker-assistant-answer-content">
              ...
              {ownsTurnChrome && turn.status === 'failed' && props.failedReasonLabel && (
                <Banner
                  ...
                  container="section"
                  className="sharker-turn-failed-banner"
            ...
            {ownsTurnChrome &&
              (props.liveStreaming ? (
                /* #642: reserved-height footer placeholder while streaming — same
                   ...
                   clickable regenerate/branch on a still-streaming answer. */
                <div aria-hidden="true" className="sharker-live-turn-footer-placeholder" />
              ) : (
                props.footerActions &&
 ...
 * map; @sharker/ui renders the badge UI. PR109e-e.
 */
...
/** Must match the `.sharker-turn-working-phrase` transition duration in styles.css. */
...
export function TurnRunningStatus(props: {
  ...
  return (
    <div
      className="sharker-turn-processing"
      role="status"
    ...
    >
      ...
      <span className="sharker-turn-indicator-text" aria-hidden="true">
        <span className="sharker-turn-working-phrase" data-fading={phraseFading || undefined}>
          {props.activityLabel ?? phrases[phraseIndex] ?? copy.processing}
        </span>
        {elapsedMs !== undefined && (
          <>
            <span className="sharker-turn-status-separator">·</span>
            <span className="sharker-turn-elapsed">{formatTurnDuration(elapsedMs)}</span>
          </>
        )}
      </span>
    </div>
...
export function ModelProviderRetryIndicator(props: { retry: LiveProviderRetry }) {
  ...
  return (
    <Banner
      ...
      container="section"
      role="status"
      className="sharker-turn-provider-retry"
      {...(scheduledA11y
        ? {
    ...
    <ChatMessageBubble
      ...
      className={
        props.phase === 'historical'
          ? 'sharker-chat-message-bubble sharker-chat-message-bubble-assistant'
          : 'sharker-chat-message-bubble sharker-chat-message-bubble-assistant sharker-bubble-streaming'
      }
    >
      <Markdown
        text={props.text}
      ...
      {truncated && (
        <Tooltip content={copy.outputTruncatedTitle}>
          ...
          <Badge
            ...
            className="sharker-turn-truncation-badge"
            role="status"
            aria-live="polite"
...
function ProcessingBlock(props: {
  ...
  const { entries } = props;
  return (
    <div className="sharker-processing-sequence">
...
function DeepThinking(props: { text: string; live: boolean; settledText?: string; truncated?: boolean }) {
  ...
  return (
    <ChatReasoning
      className="sharker-deep-thinking"
      label={label}
