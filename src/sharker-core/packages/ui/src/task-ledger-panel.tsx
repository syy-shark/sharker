...
import type { Task, TaskStatus } from '@sharker/core/task-ledger';
import {
  ICON_SIZE,
  AlertCircle,
  Ban,
...
export function TaskLedgerPanel(props: TaskLedgerPanelProps) {
  ...
  return (
    <section className="sharker-task-ledger-panel" aria-label={copy.ariaLabel}>
      {props.error ? (
        <Banner
          status="error"
          role="alert"
          className="sharker-task-ledger-message"
          title={props.error}
          endContent={props.onRetry ? (
            <IconButton
              variant="ghost"
              size="sm"
              className="sharker-task-ledger-retry"
        ...
        <Spinner size="sm" shade="subtle" label={copy.loading} className="sharker-task-ledger-message" />
      ) : (
        <>
          {model.activeCount > 0 ? (
            <div className="sharker-task-ledger-tree" role="tree" aria-label={copy.activeAriaLabel}>
              <TaskLedgerTree tasks={model.activeTree} copy={copy} />
            </div>
          ) : (
            <EmptyState
              isCompact
              className="sharker-task-ledger-empty"
          ...
          {model.recentTerminalCount > 0 && (
            <Collapsible
              className="sharker-task-ledger-terminal"
              defaultIsOpen={false}
              trigger={(
                <span className="sharker-task-ledger-terminal-trigger">
                  <span>{copy.recent}</span>
                  <span>{model.recentTerminalCount}</span>
                </span>
              )}
            >
              <div className="sharker-task-ledger-tree" role="tree" aria-label={copy.recentAriaLabel}>
    ...
    return siblings.map((task, index) => (
      ...
      >
        {childrenByParent.has(task.id) ? (
          <div role="group" className="sharker-task-ledger-group">
...
function TaskLedgerRow({ task, copy, level, position, setSize, children }: {
  ...
  return (
    <div
      className="sharker-task-ledger-row"
      role="treeitem"
      aria-level={level}
    ...
    >
      ...
      <code className="sharker-task-ledger-key">{task.key}</code>
      <span className="sharker-task-ledger-subject" title={task.subject}>{task.subject}</span>
      <span className="sharker-task-ledger-meta">
        <span>{copy.status[task.status]}</span>
        {owner && <span title={owner}>{owner}</span>}
      </span>
      {detail && <span className="sharker-task-ledger-detail" title={detail}>{detail}</span>}
      {children}
    </div>
  );
}
