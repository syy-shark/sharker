...
import { DAILY_REVIEW_RANGES, DAILY_REVIEW_SECTION_KEYS } from '@sharker/core/daily-review';
import { uiLocaleToIntlLocale } from '@sharker/core/ui-locale';
import {
  Banner,
  Button,
  Divider,
...
export function DailyReviewPanel(props: {
  ...
  if (route.kind === 'report') {
    return (
      <ModulePage title={props.hubHeader?.title ?? copy.page.title}>
        <div key="report" className="sharker-module-page-panel">
          <DailyReviewReport
  ...
  return (
          ...
          <Toolbar
            ...
            endContent={(
              <>
                ...
                />
                <Text type="label" weight="semibold" className="sharker-daily-review-range-label">{rangeLabel}</Text>
                <Button
                  variant="ghost"
                  size="sm"
                  isIconOnly
    ...
    >
      <div key="activity" className="sharker-module-page-panel" data-loading={loading ? 'true' : undefined}>
      {error ? (
        <Banner
          status="warning"
      ...
      {!displayedSummary && loading ? (
        ...
        <div
          ...
          className="sharker-daily-review-content"
          aria-busy={loading}
          data-refreshing={loading ? 'true' : undefined}
        >
          <div className="sharker-daily-review-metrics" role="group" aria-label={copy.overview.ariaLabel(displayedRangeLabel)}>
          ...
          <VStack gap={2}>
            ...
            {displayedSummary.sessions.length > 0 ? (
              <List density="balanced" hasDividers className="sharker-module-page-rows">
...
function DailyReviewMetric(props: { label: string; value: string }) {
  return (
    <VStack className="sharker-daily-review-metric" gap={0}>
...
function DailyReviewReport(props: {
  ...
  return (
    <VStack className="sharker-daily-review-report" gap={5}>
        ...
        <VStack gap={6}>
          {sections.map((section, index) => (
            <VStack key={section.key} gap={2}>
              ...
              <div className="sharker-daily-review-report-prose">
