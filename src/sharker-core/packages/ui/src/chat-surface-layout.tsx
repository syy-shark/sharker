...
export type ChatSurfaceLayoutProps = Omit<ComponentProps<typeof ChatLayout>, 'autoScroll'> & {
   ...
   * Sharker's single authority, which is what a `ChatView` transcript needs: it
...
/**
 * Sharker's product seam for the Astryx chat page shell.
 *
...
export function ChatSurfaceLayout({
  ...
  const layout = (
    <ChatLayout
      ...
      // moment its scroll layer is off. Sharker's reads Sharker's pin instead.
      scrollButton={hostOwned ? <TranscriptScrollButton /> : props.scrollButton}
      density={density}
      className={cn('sharker-chat-layout', className)}
