 ...
 * dismissal. Sharker only adapts the product search boundary and renders result
 * content.
 */
  ...
  return (
    <AstryxLocaleProvider overrides={astryxOverrides}>
      <AstryxCommandPalette
        ...
        width={560}
        maxHeight="64vh"
        data-sharker-contract="search-modal"
        input={(
          <CommandPaletteInput
        ...
        renderItem={(item) => {
          ...
          return (
            <div className="sharker-search-modal-result">
              <div className="sharker-search-modal-result-title">
                {result.title}
              </div>
              {result.summary && (
                <div className="sharker-search-modal-result-meta">
                  {result.summary}
                </div>
              )}
              {result.snippet && (
                <div className="sharker-search-modal-result-snippet">
...
function renderSearchSnippet(snippet: string, query: string): ReactNode {
  ...
  while (matchIndex !== -1) {
    ...
    parts.push(
      <mark
        key={`${matchIndex}-${end}`}
        className="sharker-search-modal-snippet-hit"
      >
