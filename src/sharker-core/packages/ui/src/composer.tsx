 ...
 * grammar (`SKILL_INVOCATION_TOKEN_SOURCE` in `@sharker/core`), the same one
 ...
 * (`project:.sharker/skills:writer` would parse as `project`), and ids are unique
...
export const Composer = forwardRef<
  ComposerHandle,
  {
     ...
     *  injected by the desktop app to keep the provider SVG library out of @sharker/ui. */
    ...
    activeThinkingLevels?: readonly import('@sharker/core/model-thinking').ThinkingLevel[];
    activeThinkingLevel?: import('@sharker/core/model-thinking').ThinkingLevel;
    onThinkingLevelChange?(level: import('@sharker/core/model-thinking').ThinkingLevel | undefined): void | Promise<void>;
    newChatThinkingLevels?: readonly import('@sharker/core/model-thinking').ThinkingLevel[];
    newChatThinkingLevel?: import('@sharker/core/model-thinking').ThinkingLevel;
    onNewChatThinkingLevelChange?(level: import('@sharker/core/model-thinking').ThinkingLevel | undefined): void | Promise<void>;
    /**
...
>(function Composer(props, ref) {
  ...
  const triggers = useMemo<ChatComposerTrigger[]>(() => {
    ...
    if (props.onSearchMentionFiles) {
      list.push({
        ...
        renderItem: (item) => (
          <>
            <FileText size={ICON_SIZE.control} aria-hidden="true" className="sharker-composer-mention-icon" />
            <span className="sharker-composer-mention-text">
              <span className="sharker-composer-mention-name">{inlineReferenceFileBasename(item.id)}</span>
              <span className="sharker-composer-mention-secondary">{item.id}</span>
            </span>
          </>
        ),
    ...
    if (
      ...
      list.push({
        ...
        renderItem: (item) => {
              ...
              <>
                {Icon ? (
                  <Icon
                    ...
                    aria-hidden="true"
                    className="sharker-composer-mention-icon"
                  />
                ) : null}
                <span className="sharker-composer-mention-text">
                  <span className="sharker-composer-mention-name">
                    {command.name}
                    <span className="sharker-composer-command-token">/{command.id}</span>
                  </span>
                  <span className="sharker-composer-mention-secondary">
                    {command.description}
                  </span>
                </span>
              </>
          ...
          return (
            <>
              <Sparkles size={ICON_SIZE.control} aria-hidden="true" className="sharker-composer-mention-icon" />
              <span className="sharker-composer-mention-text">
                <span className="sharker-composer-mention-name">{skill.name}</span>
                <span className="sharker-composer-mention-secondary">{skill.description}</span>
              </span>
            </>
          );
        },
        ...
        // survives.
        //
        // No colour: Sharker blue is the single product accent, and a staged
   ...
   * way out.
   *
   * Which mode a mark is comes from its icon, never from a hue. Sharker blue is
   ...
   *
   * That rule lives in the host, with the rest of `.sharker-composer-*` — this
  ...
  return (
    <>
      {!props.hidden && noModelConnection && (
        <div className="sharker-composer-no-model-hint" role="status">
          <span>{props.noModelHint ?? copy.noModelHint}</span>
          {props.onOpenModelSettings && (
            <UiButton
              variant="ghost"
              size="sm"
              className="sharker-composer-no-model-hint-action"
      ...
      {!props.hidden && props.revisionNotice && (
        <div className="sharker-composer-revision-notice" role="status" data-revision-notice="true">
          <Pencil size={ICON_SIZE.meta} aria-hidden="true" />
          <span className="sharker-composer-revision-notice-text">
            {props.revisionNotice.title}
            {props.revisionNotice.detail ? <span className="sharker-composer-revision-notice-detail">{props.revisionNotice.detail}</span> : null}
          </span>
          <UiButton
            variant="ghost"
            size="sm"
            className="sharker-composer-revision-notice-cancel"
      ...
      <form
        ref={formRef}
        className="sharker-composer composer"
        hidden={props.hidden}
        data-drag-active={dragActive ? 'true' : undefined}
        data-sharker-file-drop-target={canAcceptDroppedFiles() ? 'true' : undefined}
      ...
      >
        <AstryxChatComposer
          className="sharker-composer-astryx"
          data-sharker-contract="composer-inner"
          ...
          drawer={drawerTokenCount > 0 ? (
            <ChatComposerDrawer
              className="sharker-composer-drawer"
              ...
              onPointerMove={(event) => {
                ...
                toggle.style.setProperty('--sharker-drawer-tooltip-x', `${x}px`);
              }}
              ...
              onPointerLeave={(event) => {
                event.currentTarget
                  ...
                  ?.style.removeProperty('--sharker-drawer-tooltip-x');
              }}
            >
              <div className="sharker-composer-context-drawer" role="group" aria-label={copy.stagedContext}>
                ...
                {props.pendingAttachments?.map((attachment, index) => {
                  ...
                  return (
                    ...
                    >
                      <Token
                        size="sm"
                        className="sharker-composer-attachment-token"
          ...
          input={(
            <div
              className="sharker-composer-input"
            ...
            >
              <ChatComposerInput
                ...
                data-sharker-contract="composer-input"
                className="sharker-composer-editor"
                value={text}
              ...
              />
              {dragActive && (
                <span className="sharker-visually-hidden" role="status" aria-live="polite">
                  {copy.dropToImport}
                </span>
              )}
            </div>
          ...
          footerActions={(
            <div className="sharker-composer-left-controls">
              {/* Resting order: ＋ leftmost, then permission icon. */}
              {showPlusMenu ? (
                <span className="sharker-composer-plus-menu">
                  <DropdownMenu
                    placement="above"
                    hasChevron={false}
                    className="sharker-composer-quiet-menu sharker-composer-plus-panel"
                    button={{
                      ...
                      <DropdownMenuItem
                        ...
                        className={
                          props.mentionSkillsLoading === true
                            ? 'sharker-composer-skills-loading'
                            : undefined
                        }
              ...
              {/* Model + thinking sit left after permission (adjacent pair), not
                  ...
                  starts or ends. */}
              <div className="sharker-model-selection-controls">
                {props.activeSession ? (
                  <ChatModelSwitcher
              ...
              {!props.activeSession && props.workspacePicker ? (
                <div className="sharker-composer-workspace">
              ...
              {activeModeMarks.map((mark) => (
                <IconButton
                  ...
                  type="button"
                  size="sm"
                  className="sharker-composer-mode-button"
                  data-mode={mark.id}
                  label={mark.label}
