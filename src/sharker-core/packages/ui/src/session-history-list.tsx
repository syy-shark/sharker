...
export function SessionHistoryList() {
  ...
  function handleListKeyDown(event: KeyboardEvent<HTMLDivElement>) {
    ...
    const row = active.closest<HTMLElement>(
      '[data-sharker-contract="session-row"], [data-session-id]',
    );
    const sessionId =
  ...
  return (
    <div className="sharker-session-list" onKeyDown={handleListKeyDown}>
      <SessionListGroups
        groups={
          rail.groups
            ? rail.groups.map((g) => ({
  ...
  return (
    <>
      ...
      {props.groups.map((group) => {
        ...
        if (!group.label) {
          return (
            <div key={group.key} className="sharker-session-group">
              {items}
            </div>
          );
        }
        return (
          <SideNavSection key={group.key} title={group.label} className="sharker-session-group">
            {items}
          </SideNavSection>
        );
      })}
...
function ProjectNavRow(props: {
  ...
  return (
    <div data-project-id={props.groupKey} className="sharker-project-row">
      <SideNavItem
        key="navigation"
        label={props.label}
        icon={FolderOpen}
...
const SessionNavRow = memo(function SessionNavRow(props: {
  ...
  return (
    <div
      className="sharker-session-row"
      data-sharker-contract="session-row"
      ...
      <SideNavItem
        ...
        icon={
          signal ? (
            ...
            <span className="sharker-session-row-signal-empty" aria-hidden="true" />
          )
        }
        onClick={(event) => {
        ...
        endContent={
          ...
          <span className="sharker-session-row-end">
            {props.meta ? (
              <span className="sharker-session-row-host-badge" title={props.meta}>
                <Badge variant="neutral" label={props.meta} />
              </span>
            ) : null}
            <span className="sharker-session-row-time">
              {props.session.lastMessageAt ? (
                <RelativeTime
                  ts={props.session.lastMessageAt}
                  variant="sidebar"
                  className="sharker-session-row-time-label"
                  suppressTitle
                />
              ) : null}
            </span>
            {rowDescription ? (
              <span className="sharker-visually-hidden">{rowDescription}</span>
            ) : null}
          </span>
        }
      />
...
function ProjectItemMeta(props: {
  ...
  const copy = getConversationCopy(useUiLocale()).sessions;
  return (
    <span className="sharker-session-row-end sharker-project-item-end">
      ...
      {props.reserveAction ? (
        <span className="sharker-session-row-trailing" aria-hidden="true" />
      ) : null}
    </span>
  );
}
...
function ProjectItemActions(props: {
  ...
  return (
    <span className="sharker-session-row-action" data-position={props.position} ref={trailingRef}>
      <MoreMenu
        size="sm"
...
function SessionItemActions(props: {
  ...
  return (
    <span
      className="sharker-session-row-action"
