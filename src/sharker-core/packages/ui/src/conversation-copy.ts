...
import type { DeepResearchReportSectionKey } from '@sharker/core/deep-research-run';
import type { ProviderRetryReason } from '@sharker/core/events';
import type { PermissionMode } from '@sharker/core/permission';
import type { SessionBlockedReason, SessionStatus } from '@sharker/core/session';
...
} from '@sharker/core/deep-research';
...
const CONVERSATION_COPY = {
  zh: {
    ...
    composer: {
      ...
      streaming: 'Sharker 正在回答…', processing: 'Sharker 正在处理…', continuing: 'Sharker 继续中…',
    ...
    chat: {
      ...
      memory: '记忆', memoryAriaLabel: '本地记忆已启用', memoryTitle: '本地记忆文件夹已加入 agent 系统提示。点击进入设置 · 记忆管理。', deepResearch: '深度研究', deepResearchAriaLabel: '深度研究，只读探索', deepResearchTitle: '深度研究任务使用只读探索边界：先阅读和分析，默认不改文件。',
      deepResearchProgress: {
        ariaLabel: '深度研究实时进度',
        title: '研究进度',
  ...
  en: {
    ...
    deepResearchEmpty: {
      ...
      startersAriaLabel: 'Deep Research starters', starters: [
        { label: 'Research a reference project', prompt: 'Read this project without changing files. Map its structure, core modules, startup path, data flow, and tests; then list reusable design ideas, risks, and an implementation order for Sharker.' },
        { label: 'Read a reference project end to end', prompt: 'Perform a deep, read-only study of this project. Map modules and trace core features, runtime, storage, permissions, UI, tests, and docs. Express each idea as borrow / diverge / risk / gate and recommend an implementation order for Sharker.' },
        { label: 'Compare a feature implementation', prompt: 'Compare this feature in the reference project and Sharker without changing files. Identify key files, runtime boundaries, UI entry points, persistence, tests, and the smallest mergeable improvement.' },
    ...
    composer: {
      ...
      streaming: 'Sharker is responding…', processing: 'Sharker is working…', continuing: 'Sharker is continuing…',
    ...
    permissions: {
      mode: {
        ...
        ask: { label: 'Auto', hint: "Runs inside Sharker's protection; asks before going further." },
    ...
    chat: {
      ...
      memory: 'Memory', memoryAriaLabel: 'Local memory enabled', memoryTitle: 'The local memory folder is included in the agent system prompt. Click to manage it in Settings · Memory.', deepResearch: 'Deep Research', deepResearchAriaLabel: 'Deep Research, read-only exploration', deepResearchTitle: 'Deep Research uses a read-only boundary: inspect and analyze first, without changing files by default.',
      deepResearchProgress: {
