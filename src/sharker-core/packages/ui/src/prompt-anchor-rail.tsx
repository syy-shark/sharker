...
export function keepActivePromptRailTickVisible(rail: HTMLElement): void {
  const tick = rail.querySelector<HTMLElement>('.sharker-prompt-rail-tick[data-active="true"]');
  if (!tick) return;
...
export const PromptAnchorRail = memo(function PromptAnchorRail({ turns, scrollRef, onNavigateFallback, onNavigateStart }: PromptAnchorRailProps): React.ReactElement | null {
  ...
  return (
    <div
      className="sharker-prompt-rail-anchor"
      style={
        safeArea
          ? ({
              '--sharker-prompt-rail-scrollport': `${safeArea.scrollport}px`,
              '--sharker-prompt-rail-dock': `${safeArea.dock}px`,
            } as CSSProperties)
          : undefined
      }
    >
      <nav
        className="sharker-prompt-rail"
        ...
        {railTurns.map((turn, index) => {
          ...
          return (
            <HoverCard
              ...
              content={
                <span className="sharker-prompt-rail-preview">
                  <span className="sharker-prompt-rail-preview-prompt">{preview}</span>
                  {replyPreview ? (
                    <span className="sharker-prompt-rail-preview-reply">{replyPreview}</span>
                  ) : null}
                </span>
              }
            >
              <Button
                ...
                className="sharker-prompt-rail-tick"
                ...
                style={
                  {
                    '--sharker-prompt-rail-index': index,
                    '--sharker-prompt-rail-scale': scale,
                  } as CSSProperties
                }
              >
                <span className="sharker-prompt-rail-tick-bar" />
              </Button>
            </HoverCard>
          );
        })}
