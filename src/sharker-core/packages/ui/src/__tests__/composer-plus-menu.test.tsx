...
async function plusMenu(props: Parameters<typeof Composer>[0]): Promise<string> {
  ...
  assert.ok(markup.includes('sharker-composer-plus-menu'), 'the composer rendered no ＋ menu');
  return markup;
}
...
test('a populated skill catalog renders the row enabled with no caveat', async () => {
  ...
  assert.equal(menu.includes('sharker-composer-skills-loading'), false);
});
...
test('a loading catalog holds the row still and marks the held state', async () => {
  ...
  assert.equal(
    skillsRow(menu).includes('sharker-composer-skills-loading'),
    true,
...
test('Ask mode checks the Ask row and paints a footer mark', async () => {
  ...
  assert.equal(count(markup, 'sharker-composer-mode-button'), 1);
