...
import { SharkerUriContext, Markdown } from '../markdown.js';
...
it('renders Markdown emphasis and LaTeX without exposing their source delimiters', () => {
  ...
  assert.match(markup, /class="sharker-math sharker-math-inline"/);
...
it('keeps URL, email, and Markdown markers atomic inside math', () => {
  ...
  assert.equal((markup.match(/class="sharker-math sharker-math-inline"/g) ?? []).length, 3);
...
it('falls back to ordinary Markdown for an empty formula', () => {
  ...
  assert.doesNotMatch(markup, /class="sharker-math/);
  assert.doesNotMatch(markup, /\\\(|\\\)/);
  assert.match(markup, /Empty \( \) end/);
});

it('keeps literal math transport syntax as prose', () => {
  const literalToken = '\uE000SHARKER_MATH:0:78\uE001';
  const markup = renderToStaticMarkup(createElement(MarkdownBody, {
...
it('does not let an unmatched math delimiter hide a later formula', () => {
  for (const text of [
    ...
    assert.match(markup, /class="sharker-math/);
    assert.match(markup, /class="katex/);
  }
});
...
it('keeps scanning after a malformed math transport prefix', () => {
  const markup = renderToStaticMarkup(createElement(MarkdownBody, {
    text: `bad \uE000SHARKER_MATH:bad then \\(x\\)`,
  }));

  assert.match(markup, /SHARKER_MATH:bad/);
  assert.match(markup, /class="sharker-math sharker-math-inline"/);
});
...
it('renders display math while leaving ordinary currency alone', () => {
  ...
  assert.match(markup, /class="sharker-math sharker-math-display"/);
  assert.match(markup, /class="katex-display"/);
  assert.doesNotMatch(markup, /class="sharker-math sharker-math-inline"/);
});
...
it('does not treat shell variables, currency, or inline code as dollar-delimited math', () => {
  ...
  assert.equal((markup.match(/class="sharker-math sharker-math-inline"/g) ?? []).length, 1);
  assert.match(markup, /class="katex"/);
});
...
it('keeps the copy control in a toolbar above a one-line code scroll viewport', () => {
  ...
  assert.match(markup, /data-sharker-code-layout="single-line"/);
...
it('does not force the single-line scrollbar layout on multiline code', () => {
  ...
  assert.match(markup, /data-sharker-code-layout="multi-line"/);
...
it('preserves allowlisted Sharker navigation links through sanitization', () => {
      ...
      LocaleProvider,
      {
        ...
        children: createElement(
          ...
          createElement(MarkdownBody, {
            text: '[Models](sharker://settings/models)',
          }),
        ),
      },
    ),
  ...
  assert.match(markup, /data-sharker-uri-kind="settings"/);
...
it('does not treat navigation and communication schemes as image resources', () => {
  for (const src of [
    'sharker://tool/run',
    'SHARKER://auth/login',
    'sharker://settings/models',
    'sharker://compose?text=hello',
    'sharker://runtime/attachments/attachment-123?session=other',
    'sharker://runtime/attachments/not-an-artifact',
...
it('shows an attachment placeholder when no session reader is installed', () => {
  const markup = renderToStaticMarkup(createElement(MarkdownBody, {
    text: '![preview](sharker://runtime/attachments/attachment-123)',
  }));

  assert.match(markup, />\[preview\]</);
  assert.doesNotMatch(markup, /sharker:\/\/runtime\/attachments/);
  assert.doesNotMatch(markup, /<img\b/);
});
...
it('defers Mermaid fences beyond the per-Markdown automatic diagram budget', () => {
  ...
  assert.equal(markup.match(/data-sharker-mermaid-state="loading"/g)?.length, 3);
  assert.equal(markup.match(/data-sharker-mermaid-state="deferred"/g)?.length, 1);
  assert.match(markup, /Render diagram/);
  assert.doesNotMatch(markup, /sharkermermaiddeferred/);
});
...
it('enforces per-diagram and total automatic Mermaid source budgets', () => {
  ...
  const source = [nearHalfTotal, nearHalfTotal, 'x'.repeat(250)]
    ...
    .join('\n\n');
  assert.equal(
    applyMermaidRenderBudget(source).match(/```sharkermermaiddeferred/g)?.length,
    1,
  );
});
...
it('does not render Mermaid while the assistant turn is streaming', () => {
  ...
  assert.doesNotMatch(markup, /data-sharker-contract="mermaid"/);
});
