...
const project: ProjectRecord = {
  id: 'project-1',
  name: 'Sharker',
  locations: [{ path: '/workspace/sharker', isWorktree: false }],
  available: true,
  preferredPath: '/workspace/sharker',
};
...
function assertNoNestedButtons(markup: string): void {
  ...
  // `.sharker-session-row-action`, so the nesting survives parsing and is caught.
...
test('renders session navigation and row actions as sibling controls', () => {
  ...
  assert.match(markup, /class="sharker-session-row-action"/);
...
test('renders a scan-friendly compact timestamp in the session rail', () => {
  ...
  try {
    ...
    assert.equal(document.querySelector('.sharker-session-row-time-label')?.textContent, '46min');
  } finally {
...
test('renders collapsible project navigation and row actions as sibling controls', () => {
  ...
  const { document } = parseHTML(markup);
  const projectRow = document.querySelector('.sharker-project-row');
  const action = document.querySelector<HTMLButtonElement>(
    'button[aria-label="Sharker project actions"]',
  );
  ...
  const metadata = projectRow.querySelector('.sharker-project-item-end');
