...
test('keeps settled math stable while the live Markdown tail grows and flushes', async () => {
  ...
  const math = container.querySelector('.sharker-math');
  assert.ok(math);
  ...
  assert.equal(container.querySelector('.sharker-math'), math);
  ...
  assert.ok(container.querySelector('.sharker-math .katex'));
  assert.equal(container.querySelector('a'), null);
...
test('never exposes math transport syntax as a formula crosses the display cursor', async () => {
  ...
  for (let tick = 1; tick <= 8 && container.querySelector('.sharker-math') === null; tick++) {
    const frame = frames.shift();
    assert.ok(frame);
    await act(() => frame(tick * 100));
    assert.doesNotMatch(container.textContent ?? '', /SHARKER_MATH|\uE000|\uE001/);
  }
...
test('keeps split fenced-code openers literal through final flush', async () => {
  for (const { prefix, target } of [
    ...
    assert.equal(container.querySelector('.sharker-math'), null);
    assert.doesNotMatch(container.textContent ?? '', /SHARKER_MATH|\uE000|\uE001/);
    await act(() => root.unmount());
  }
});
