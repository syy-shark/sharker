...
test('places the aborted turn outcome after its timeline content', async () => {
  ...
  const answer = container.querySelector('.sharker-chat-message-bubble-assistant');
  const assistantMessage = container.querySelector('.sharker-assistant-answer');
  const outcome = container.querySelector('.astryx-chat-system-message[role="status"]');
...
test('keeps the assistant answer element as a turn settles around it', async () => {
  ...
  const streaming = container.querySelector('.sharker-assistant-answer');
  const streamingBubble = container.querySelector('.sharker-chat-message-bubble-assistant');
  ...
  const settled = container.querySelector('.sharker-assistant-answer');
  const settledBubble = container.querySelector('.sharker-chat-message-bubble-assistant');
...
test('gives each answer in a steered turn its own stable element', async () => {
  ...
  const answers = [...container.querySelectorAll('.sharker-assistant-answer')];
  ...
  const settledAnswers = [...container.querySelectorAll('.sharker-assistant-answer')];
...
test('keeps Astryx auto formatting live for user-message timestamps', async (context) => {
  ...
  const timestamp = container.querySelector('.sharker-message-time-inline time');
...
test('announces settlement when a persisted answer is promoted to a completed live one', async () => {
  ...
  assert.equal(
    container.querySelector('.sharker-bubble-streaming') === null,
    false,
