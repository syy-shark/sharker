 ...
 * highlighting, and copy feedback. Sharker's Markdown layer keeps only the
 ...
 *
 * Astryx owns stream pacing and incremental parsing. Sharker keeps only the
...
import { SharkerUriContext } from './markdown.js';
...
const DEFERRED_MERMAID_LANGUAGE = 'sharkermermaiddeferred';
...
export function MarkdownBody(props: {
  ...
  return (
    <div
      data-sharker-contract="markdown"
    ...
    >
      <AstryxMarkdown
        ...
        // `components.code` bare — no spacing, no width, no alignment. Sharker
        ...
        // that rule is scoped to `.sharker-turn` precisely because the other
...
function MarkdownCode(props: {
  ...
  return (
    <div
      className={`sharker-markdown-code sharker-markdown-code-${props.density}`}
      data-sharker-code-layout={isSingleLine ? 'single-line' : 'multi-line'}>
      <CodeBlock
        code={props.code}
...
function MarkdownImage(props: { src: string; alt: string }) {
  ...
  if (attachment) {
    ...
    return (
      <img
        className="sharker-markdown-attachment-image"
        src={attachmentSrc}
        alt={props.alt}
      />
    );
  ...
  // Remote images can be badges or sentence-level icons, so preserve Sharker's
...
/**
 * Route internal Markdown navigation through Sharker's typed allowlist. Invalid
 * internal destinations never fall through to the operating system, and
 * external links are limited to the three schemes Sharker deliberately exposes.
 */
function MarkdownLink(props: { href: string; children: ReactNode }) {
  const { href, children } = props;
  const dispatch = useContext(SharkerUriContext);
  const copy = getSharedUiCopy(useUiLocale()).markdown;

  if (isSharkerUriCandidate(href)) {
    const dest = parseSharkerUri(href);
    if (dest && dispatch) {
      return (
        <AstryxLink
          type="inherit"
          hasUnderline
          data-sharker-uri-kind={dest.kind}
