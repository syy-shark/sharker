...
import { isInFlightToolStatus } from '@sharker/core/tool-result-status';
...
function LoadToolResultPreview(props: {
  ...
  return (
    <div className={previewVariants({ part: 'load-tool' })} data-kind="load_tool">
      <span className="sharker-load-tool-icon" aria-hidden="true">
        <Icon size={ICON_SIZE.chrome} />
      </span>
      <div className="sharker-load-tool-summary">
        <p className={previewVariants({ part: 'load-tool-title' })}>{desc.title}</p>
        <p className="sharker-load-tool-description">{desc.description}</p>
        <p className={previewVariants({ part: 'load-tool-count' })}>
          <span>{desc.label}</span>
          <span className="sharker-load-tool-separator" aria-hidden="true" />
          <span>{desc.countLabel}</span>
        </p>
      </div>
      {(desc.groupId || desc.toolIds.length > 0) && (
        <details className="sharker-load-tool-technical">
...
export function ToolCallDetail({
  ...
  return (
    <div className="sharker-tool-call-detail">
      {showSandboxBanner && (
      ...
      {hasSharedPanelContent && (
        <div data-slot="tool-output" className="sharker-tool-output-stack">
          {(() => {
  ...
  return (
    <>
      {segments.map((segment) => segment.kind === 'tools' ? (
        <ChatToolCalls
          key={segment.key}
          className="sharker-tool-activity-card"
          calls={segment.calls}
        />
      ) : (
        <LinkedAgentList
      ...
      {props.rows.map((row) => {
        ...
        return (
          <ListItem
            ...
            label={(
              <span className="sharker-subagent-session-label">
                <Text type="label" maxLines={1}>
                  {row.name}
                </Text>
                {row.target ? (
                  <Text type="body" color="secondary" maxLines={1} className="sharker-subagent-session-summary">
                    {row.target}
                  </Text>
                ) : null}
                {row.failureClass ? (
            ...
            endContent={(
              <span className="sharker-subagent-session-end">
...
function RequiresBypassBanner(props: {
  ...
  return (
    <Banner
      status="warning"
      className="sharker-requires-bypass-banner"
      ...
      <pre ref={preRef} className={TOOL_OUTPUT_BODY_CLASS} data-live={props.live ? 'true' : undefined}>
        {props.chunks.map((chunk) => (
          <span
            key={chunk.seq}
            className={cn(
              'sharker-tool-output-chunk',
              chunk.stream === 'stderr' && 'sharker-tool-output-chunk-stderr',
              chunk.redacted && 'sharker-tool-output-chunk-redacted',
            )}
          ...
          >
            {chunk.text}
            {chunk.redacted && (
              <span className="sharker-tool-output-redacted">
                {' '}{copy.redacted}
              </span>
            )}
          </span>
...
function SandboxBlockedBanner(props: {
  ...
  return (
    <Banner
      status="warning"
      className="sharker-sandbox-blocked-banner"
      icon={<ShieldAlert size={ICON_SIZE.chrome} aria-hidden="true" />}
      title={bannerCopy.title}
      description={(
        <span className="sharker-sandbox-blocked-description">
          <span>{bannerCopy.description}</span>
          {errorText && (
            <code className="sharker-sandbox-blocked-error">
      ...
      endContent={errorText ? (
        <UiButton
          variant="ghost"
          size="sm"
          className="sharker-sandbox-blocked-copy"
