...
import { isShellOutput, type ShellOutput } from '@sharker/core/shell-run';
import { normalizeSearchUrl } from '@sharker/core/search';
import { ptyHumanTerminalText } from '@sharker/core/pty-output-view';
import { readWriteStdinInputPreview } from '@sharker/core/tool-activity-args';
import { type ToolResultContent } from '@sharker/core/events';
  ...
  return (
    <div
      ...
      className={cn(
        ...
        props.attention === 'error' && 'sharker-tool-output-destructive-border',
        props.attention === 'warning' && 'sharker-tool-output-warning-border',
      )}
    >
      {command && (
        <div className="sharker-tool-output-command-row">
          <code className={TOOL_OUTPUT_COMMAND_CLASS}>{command}</code>
          <div className="sharker-tool-output-command-actions">
            {props.actions}
            <UiButton
              variant="ghost"
              size="sm"
              className="sharker-tool-output-command-copy"
...
function PtyControlPreview(props: {
  ...
  if (operation?.kind !== 'pty_control') {
    return <p className={cn(TOOL_OUTPUT_NOTE_CLASS, 'sharker-tool-output-destructive')}>{copy.ptyFailed}</p>;
  }
  const parts: string[] = [];
  if (operation.input) {
  ...
  return (
    <p className={cn(
      ...
      'sharker-tool-output-wrap',
      operation.failed && 'sharker-tool-output-destructive',
    )}>
 ...
 * this. The `.sharker-tool-diff-*` classes it renders through are the ones
  ...
  return (
    ...
    >
      ...
      {props.failureMessage && (
        <p className={cn(TOOL_OUTPUT_NOTE_CLASS, props.sandboxBlocked ? 'sharker-tool-output-warning' : 'sharker-tool-output-destructive')}>
          {redactSecrets(props.failureMessage)}
        </p>
      )}
      {cancelled && (
        <p className={cn(TOOL_OUTPUT_NOTE_CLASS, 'sharker-tool-output-destructive')}>
      ...
      {props.sandboxBlocked && !cancelled && !timedOut && (
        <p className={cn(TOOL_OUTPUT_NOTE_CLASS, 'sharker-tool-output-warning')}>
  ...
  return (
    ...
    >
      ...
      {result.failureMessage && (
        <p className={cn(TOOL_OUTPUT_NOTE_CLASS, sandboxBlocked ? 'sharker-tool-output-warning' : 'sharker-tool-output-destructive')}>
      ...
      className={cn(
        ...
        props.attention && (
          props.sandboxBlocked
            ? 'sharker-tool-output-warning-border'
            : 'sharker-tool-output-destructive-border'
        ),
      )}
    >
      <header className="sharker-pty-shell-header">
        <span className="sharker-pty-shell-title">
          Shell
        </span>
      </header>
      <div className="sharker-pty-shell-body">
      ...
      <footer className="sharker-pty-shell-footer">
        <ShellRunStatus
...
function ShellRunStatus(props: {
  ...
  switch (props.status) {
    case 'starting':
    case 'running':
      return <><Loader2 size={ICON_SIZE.control} aria-hidden="true" className="sharker-spin" />{copy.running}</>;
    case 'completed':
...
function PtyTerminalSurface(props: { text: string }) {
  ...
  return (
    <pre
      ref={ref}
      className={cn(TOOL_OUTPUT_BODY_CLASS, 'sharker-pty-terminal-body')}
      data-stream="pty"
...
function shellRunStatusLabel(status: string, locale: import('@sharker/core/ui-locale').UiLocale): string {
  const copy = getToolActivityCopy(locale).result;
...
function WebSearchPreview(props: {
  ...
  if (rows.length === 0) {
    return (
      <div className="sharker-web-result" data-kind="web_search">
        <header className="sharker-web-result-header">
  ...
  return (
    <div className="sharker-web-result" data-kind="web_search">
      ...
      <ul className="sharker-web-result-list">
        {rows.map((row, idx) => (
          <li key={`${row.url}-${idx}`}>
...
function WebSearchErrorPreview(props: {
  ...
  return (
    <div className="sharker-web-result sharker-web-result-error" data-kind="web_search_error">
      <header className="sharker-web-result-header">
      ...
      <p className="sharker-web-result-error-message">{redactSecrets(props.message)}</p>
      <p className="sharker-web-result-repair">{repairCopy}</p>
    </div>
  );
}
