...
import { parseUnifiedDiffRows } from '@sharker/core/unified-diff';
  ...
  return (
    <pre className={`${previewVariants({ part: 'diff-body' })}${props.className ? ` ${props.className}` : ''}`}>
      {rows.map((row, index) => (
        ...
        >
          {numbered && (
            <span className="sharker-tool-diff-gutter" style={{ minWidth: `${digits}ch` }}>
              {row.lineNumber ?? ''}
            </span>
          )}
          <span className="sharker-tool-diff-marker">{row.marker}</span>
          <span className="sharker-tool-diff-code">
