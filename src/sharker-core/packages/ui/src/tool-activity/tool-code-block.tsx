...
export function ToolCodeBlock(props: {
  ...
  const codeBlock = (
    <CodeBlock
      ...
      // which is the same 0.875rem the product's own `--sharker-text-code` well
...
export function ToolDetailReveal(props: { children: ReactNode }) {
  return (
    <div className="sharker-chat-tool-detail">
      <div className="sharker-chat-tool-detail-inner">{props.children}</div>
    </div>
  );
}
