...
import type { CapabilityAuditReport } from '@sharker/core/capability-audit';
import { deriveCapabilityAuditReport } from '@sharker/core/capability-audit';
import {
  Button as UiButton,
  EmptyState,
  IconButton,
...
export function SkillsModuleMain(props: {
  ...
  function catalogInstallButton(key: string, name: string, installing: boolean, installed: boolean, onInstall?: () => void) {
    const installIcon = installing
      ...
      : installed
        ? (
          <span className="sharker-skill-install-complete-icon">
  ...
  const marketPanel = (
    <div className="sharker-module-page-panel">
      ...
      {allManagedSources.length === 0 ? (
        ...
        <List density="balanced" hasDividers className="sharker-module-page-rows" aria-label={copy.market.ariaLabel}>
            ...
            return (
              <ListItem
                ...
                description={[
                  ...
                  .join(' · ')}
                startContent={(
                  <span className="sharker-module-market-icon" aria-hidden="true">
  ...
  const builtinPanel = (
    <div className="sharker-module-page-panel">
      ...
      <List
        ...
        density="balanced"
        hasDividers
        className="sharker-module-page-rows"
  ...
  const installedPanel = (
    <div className="sharker-module-page-panel" ref={rowsContainerRef} {...rovingRows}>
      ...
      {skills.length === 0 ? (
        ...
        (<List density="balanced" hasDividers className="sharker-module-page-rows" aria-label={copy.installed.listAriaLabel}>
          {filteredSkills.map((skill) => {
            ...
            return (
              <ListItem
                key={skillRef}
                label={(
                  <span className="sharker-skill-row-label">
                    {skill.name}
                    {(skillNameCounts.get(skill.name) ?? 0) > 1 && (
                      <code className="sharker-skill-row-slug">{skill.id}</code>
                    )}
                  </span>
                )}
  ...
  return (
    <section className="sharker-main detailPane sharker-module-main agents-chat-panel" data-page-shell="layout" data-module="skills" aria-label={props.hubHeader?.title ?? copy.page.title}>
      <ModulePage
        ...
        inspectorAutoSaveId="sharker-skill-inspector"
