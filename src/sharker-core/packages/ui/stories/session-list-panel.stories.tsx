...
import type { SessionBlockedReason, SessionStatus, SessionSummary } from '@sharker/core/session';
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      const targetRow = Array.from(
        ref.current?.querySelectorAll<HTMLElement>('[data-sharker-contract="session-row"]') ?? [],
...
export const ProjectGroups: Story = {
  render: () => {
    const sharker = makeProject({
      id: 'project-sharker',
      name: 'sharker-agent',
      preferredPath: '/workspace/sharker-agent',
      locations: [
        { path: '/workspace/sharker-agent', isWorktree: false },
        { path: '/workspace/sharker-agent/.worktree/sidebar', isWorktree: true },
      ],
    });
