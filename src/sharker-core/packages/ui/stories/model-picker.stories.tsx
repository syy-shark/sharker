...
export const ExistingConversation: Story = {
  ...
  play: async ({ canvasElement, globals }) => {
    ...
    const announcement = canvasElement.querySelector<HTMLElement>('.sharker-model-switch-announcement');
    await expect(announcement).toHaveAttribute('role', 'status');
    await expect(trigger).not.toHaveAttribute('aria-description');
    await expect(announcement).toBeEmptyDOMElement();
    await expect(document.body.querySelector('.sharker-model-switch-notice')).not.toBeInTheDocument();
    ...
    const notice = document.body.querySelector('.sharker-model-switch-notice');
    ...
    await expect(announcement).toHaveTextContent(warning);
    await expect(document.body.querySelector('.sharker-model-switch-notice')).toBeInTheDocument();
  },
};
...
export const ThinkingLevelSeparate: Story = {
  render: function ThinkingLevelSeparateRender() {
    ...
    return (
      <div className="sharker-model-selection-controls" style={{ width: 'max-content' }}>
        <NewChatModelPicker
          label={selectedLabel(value)}
          choices={CHOICES}
          currentValue={value}
