...
const terminalResult = {
  kind: 'terminal',
  cwd: '/Users/yuhan/workspace/oss/sharker-agent',
  cmd: 'npm run -w @sharker/desktop build-storybook',
  status: 'completed',
  exitCode: 0,
...
const terminalFailureResult = {
  kind: 'terminal',
  cwd: '/Users/yuhan/workspace/oss/sharker-agent',
  cmd: 'npm run -w @sharker/eval test',
  status: 'failed',
  exitCode: 1,
  output: pipeOutput(
    'running eval tests\n',
...
const shellRunResult = {
  ...
  ref: 'shell-7f21',
  cwd: '/Users/yuhan/workspace/oss/sharker-agent',
  cmd: 'npm run -w @sharker/ui test -- --watch',
  status: 'running',
  startedAt: NOW - 1_400,
  updatedAt: NOW,
  revision: 3,
...
const webSearchResult = {
  kind: 'web_search',
  provider: 'tavily',
  query: 'Sharker Storybook ToolActivity review states',
  rows: [
    {
...
export const statusOverviewItems = [
  ...
  toolItem({
    ...
    status: 'running',
    args: { cmd: 'npm run -w @sharker/desktop build-storybook' },
  ...
  toolItem({
    ...
    status: 'errored',
    args: { cmd: 'npm run -w @sharker/eval test' },
...
const writeOverwriteDiffResult = {
  kind: 'file_diff',
  paths: ['.sharker/config.json'],
  diff: [
    '--- a/.sharker/config.json',
    '+++ b/.sharker/config.json',
    '@@ -1,5 +1,5 @@',
    ' {',
    '   "theme": "dark",',
    '-  "model": "glm-5",',
