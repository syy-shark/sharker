...
// and catalogued Sharker's parallel motion vocabulary as if it were the contract.
const meta = {
  ...
  render: () => (
    <div style={{ alignItems: 'end', display: 'grid', gap: 24, gridTemplateColumns: 'repeat(2, minmax(96px, 1fr))' }}>
      {/* This story is the keyframe's only consumer since the product's
          `.sharker-shimmer` recipe died (#1980), so the keyframe lives here
          rather than in sharker-tokens.css. */}
      <style>{'@keyframes sharker-shimmer { 0% { background-position: -200% 0; } 100% { background-position: 200% 0; } }'}</style>
      ...
      <div style={{ alignItems: 'center', display: 'grid', gap: 8, justifyItems: 'center' }}>
        <span
          aria-hidden="true"
          style={{
            animation: 'sharker-shimmer 1.8s var(--ease-linear) infinite',
            background:
