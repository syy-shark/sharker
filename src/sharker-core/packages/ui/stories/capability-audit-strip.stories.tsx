 ...
 * page (skills-panel.tsx) — `.sharker-module-page-panel`, a padded grid block
 * in the module page's single scrolling content column. The previous frame
 * here exercised a `.sharker-module-main` grid row assignment and its
 * `:has(> .sharker-capability-audit-strip)` third-row rule; both were deleted
 ...
 * page mounts the same component inside `.sharker-module-page-body` — a
...
function ModulePage(props: { children: React.ReactNode }) {
  return (
    <section
      className="sharker-main detailPane sharker-module-main agents-chat-panel"
      data-page-shell="layout"
      aria-label="Capability audit module content"
    >
      <div className="sharker-module-page-panel">{props.children}</div>
    </section>
  );
}
