...
- Added the installable Sharker CLI package and its protected staged npm release pipeline, including cross-platform artifact and Eval validation (#3169, #3173, #3185, #3188, #3192, #3197, #3200, #3201).
...
- Disabled TUI taskbar-progress keepalives by default on native Windows and
  ...
  taskbar unresponsive. `SHARKER_TASKBAR_PROGRESS=1` restores the prior behavior,
  while `SHARKER_TASKBAR_PROGRESS=0` disables it explicitly.
...
- Gave projects a first-class Settings page: manage every project Sharker knows,
...
- Added bounded long-term memory extraction (#2117) and a runtime `WebFetch`
  tool (#2362).
- Moved the CLI onto Runtime Host: `sharker run` (#2337) and TUI sessions
...
- Runtime and headless: compacted and sanitized agent tool contracts (#2349),
  ...
  liveness (#2392), measured the wire the Sharker runtime actually dials (#2286),
...
- Let the Sharker benchmark arm run where its competitors run (#2177), bound
...
- Instructions now load from `~/.sharker/AGENTS.md` (or `CLAUDE.md`/`GEMINI.md`)
...
- Bounded Headless teardown and marked the output replay (#2151), reaped the
  ...
  reward survive a missing agent self-report (#2112), made Sharker's settlement
...
- Rebuilt Computer Use around `sharker-cu` as the executor that actually runs:
...
- Removed `cua-driver` now that `sharker-cu` is the Computer Use executor.
...
Opt out with `SHARKER_CONTEXT_ACTIVE_TOOL_RESULT_PRUNE=off` (both sides).
Tune the threshold with `SHARKER_CONTEXT_ACTIVE_TOOL_RESULT_MAX_ESTIMATED_TOKENS`
and the start step with `SHARKER_CONTEXT_ACTIVE_TOOL_RESULT_MIN_STEP_NUMBER`.
