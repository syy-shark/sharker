...
$workRoot = Join-Path $tempRoot "sharker-phase4-adversarial-$PID"
$ledgerRoot = Join-Path ([IO.Path]::GetTempPath()) 'sharker-sandbox-acl-ledgers'
$registrySubkey = "Software\Sharker\SandboxPhase4\$PID"
$registryPath = "HKCU:\$registrySubkey"
$registryValueName = 'HostSecret'
$pipeShortName = "sharker-phase4-host-$PID"
$pipeName = "\\.\pipe\$pipeShortName"
$hostSecretName = 'SHARKER_PHASE4_HOST_SECRET'
$listener = $null
$pipe = $null
$quarantinedSid = $null
$quarantineRoot = $null
...
function Write-LaunchRequest {
  ...
  $request = [ordered]@{
    ...
    network = 'restricted'
    environment = @{ SHARKER_PHASE4_ALLOWED = 'allowed' }
    timeoutMs = 120000
  }
