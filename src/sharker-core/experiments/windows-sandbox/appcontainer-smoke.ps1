...
$launcher = if ($LauncherPath) {
  ...
  Join-Path $PSScriptRoot 'launcher\target\debug\sharker-windows-sandbox.exe'
}
...
$requestPath = Join-Path $tempRoot "sharker-windows-appcontainer-$PID.json"
$secretPath = Join-Path $tempRoot "sharker-windows-appcontainer-secret-$PID.txt"
$allowedReadPath = Join-Path $tempRoot "sharker-windows-appcontainer-allowed-read-$PID.txt"
$allowedWriteRoot = Join-Path $tempRoot "sharker-windows-appcontainer-allowed-write-$PID"
$allowedWritePath = Join-Path $allowedWriteRoot 'write.txt'
$staleRoot = Join-Path $tempRoot "sharker-windows-appcontainer-stale-$PID"
$ledgerRoot = Join-Path ([IO.Path]::GetTempPath()) 'sharker-sandbox-acl-ledgers'
...
try {
  ...
  if ($exitCode -ne 0 -or
      ...
      $rendered -notmatch '"desktop":"sharker-sandbox-desktop\.') {
