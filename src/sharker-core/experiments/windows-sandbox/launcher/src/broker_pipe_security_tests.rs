...
mod tests {
    ...
    #[test]
    fn accepts_only_private_sharker_pipe_namespace() {
        assert_eq!(
            validate_pipe_name(r"\\.\pipe\sharker-sandbox-session_42"),
            Ok(())
        );
        assert_eq!(
        ...
        assert_eq!(
            validate_pipe_name(r"\\server\pipe\sharker-sandbox-session"),
            Err(PipeSecurityError::InvalidPipeName)
        );
        assert_eq!(
            validate_pipe_name(r"\\.\pipe\sharker-sandbox-..\escape"),
            Err(PipeSecurityError::InvalidPipeName)
        );
    }
