...
#[cfg(not(windows))]
compile_error!("sharker-windows-sandbox is Windows-only");
...
fn main() -> ExitCode {
    match run() {
        ...
        Err(error) => {
            eprintln!("sharker-windows-sandbox: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn run() -> Result<u8, String> {
    ...
    let first = args.next().ok_or_else(|| {
        "usage: sharker-windows-sandbox [--atomic|--broker-validate] <request.json>".to_owned()
    })?;
    if first == "--self-probe" {
...
/// (`sharker-sandbox-desktop.<pid>.<nonce>`). Attesting against the prefix rather
...
const PRIVATE_DESKTOP_PREFIX: &str = "sharker-sandbox-desktop.";
