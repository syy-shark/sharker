...
pub(crate) fn acl_mutex_name(user_sid: &str) -> String {
    format!(r"Global\Sharker.WindowsSandbox.AclLedger.v2.{user_sid}")
}

/// Serializes the readiness probe's AppContainer profile lifecycle. The probe
/// profile (`sharker.readiness.<hash>`) is a per-user, machine-wide registration
...
pub(crate) fn readiness_mutex_name(user_sid: &str) -> String {
    format!(r"Global\Sharker.WindowsSandbox.ReadinessProfile.v1.{user_sid}")
}
...
pub fn with_acl_grants<T>(
    ...
    let ledger_root = std::env::temp_dir().join("sharker-sandbox-acl-ledgers");
...
pub(crate) fn ledger_lease_name(user_sid: &str, request_id: &str) -> String {
    format!(
        r"Global\Sharker.WindowsSandbox.AclLease.{user_sid}.{:x}",
...
fn quarantine_ledger(path: &Path, reason: &str) -> Result<(), String> {
    ...
    eprintln!(
        "sharker-windows-sandbox: {reason}; quarantined {} to {}",
        path.display(),
        quarantined.display()
    );
    Ok(())
