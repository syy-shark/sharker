...
impl AppContainerProfile {
    unsafe fn create(request_id: &str) -> Result<Self, String> {
        let name = appcontainer_profile_name(request_id);
        let display_name = wide("Sharker Windows Sandbox");
        let description = wide("Per-launch AppContainer profile for Sharker sandbox execution");
        let mut sid = null_mut();
        let result = unsafe {
    ...
    unsafe fn create_readiness() -> Result<Self, String> {
        ...
        let display_name = wide("Sharker Windows Sandbox Readiness");
        let description =
            wide("Throwaway AppContainer profile for the Sharker sandbox readiness probe");
        let mut sid = null_mut();
        let result = unsafe {
            CreateAppContainerProfile(
                name.as_ptr(),
...
pub(crate) fn appcontainer_profile_name(request_id: &str) -> Vec<u16> {
    ...
    wide(&format!("sharker.sandbox.{}", &digest[..32]))
}

/// The readiness probe's AppContainer profile name. It lives under a distinct
/// `sharker.readiness.` prefix, structurally disjoint from the production
/// `sharker.sandbox.` namespace [`appcontainer_profile_name`] derives, so even if
...
pub(crate) fn appcontainer_readiness_profile_name() -> Vec<u16> {
    ...
    wide(&format!("sharker.readiness.{}", &digest[..32]))
}
...
pub(crate) unsafe fn create_confined_desktop(
    ...
    let name = wide(&format!(
        "sharker-sandbox-desktop.{}.{nonce:032x}",
        std::process::id()
    ));
