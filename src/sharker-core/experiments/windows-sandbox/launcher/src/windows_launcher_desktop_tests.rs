...
fn private_placement_requires_the_launcher_owned_desktop_prefix() {
    assert!(desktop_is_private_placement(
        "sharker-sandbox-desktop.4321.1699999999999999999"
    ));
    // Desktop names are case-insensitive on Windows.
    assert!(desktop_is_private_placement("SHARKER-SANDBOX-DESKTOP.1.2"));
    ...
    assert!(!desktop_is_private_placement("sharker-sandbox-desktop."));
}
...
fn ten_confined_desktops_can_be_held_live() {
    ...
    let name: Vec<u16> = "sharker-desktop-heap-test"
        .encode_utf16()
