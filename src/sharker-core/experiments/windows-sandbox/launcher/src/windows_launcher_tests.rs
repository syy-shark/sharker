...
mod tests {
    ...
    fn appcontainer_profile_identity_is_unique_and_bounded_per_request() {
        ...
        assert!(rendered.starts_with("sharker.sandbox."));
        assert_eq!(rendered.len(), "sharker.sandbox.".len() + 32);
    }
    ...
    fn readiness_profile_name_is_fixed_and_self_reconciling() {
        ...
        assert!(rendered.starts_with("sharker.readiness."));
        assert_eq!(rendered.len(), "sharker.readiness.".len() + 32);
    }

    #[test]
    fn readiness_namespace_is_disjoint_from_production() {
        // Structural separation, not just a different hash input: the readiness
        // profile lives under `sharker.readiness.` while every production profile
        // lives under `sharker.sandbox.`, so the two lifecycles can never resolve to
        ...
        let production_str =
            String::from_utf16(&production_reserved[..production_reserved.len() - 1])
                .expect("name");
        assert!(readiness_str.starts_with("sharker.readiness."));
        assert!(production_str.starts_with("sharker.sandbox."));
    }
    ...
    fn readiness_lease_name_is_scoped_and_distinct() {
        ...
        assert!(name.starts_with(r"Global\Sharker.WindowsSandbox.ReadinessProfile."));
