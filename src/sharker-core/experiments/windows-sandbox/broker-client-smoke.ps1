...
$launcher = Join-Path $PSScriptRoot 'launcher\target\debug\sharker-windows-sandbox.exe'
if (-not (Test-Path -LiteralPath $launcher)) {
  throw "Missing launcher binary: $launcher"
}

$sid = [Security.Principal.WindowsIdentity]::GetCurrent().User.Value
$pipeSuffix = "sharker-sandbox-client-ci-$PID"
$pipeName = "\\.\pipe\$pipeSuffix"
$manifestPath = Join-Path $env:RUNNER_TEMP "sharker-windows-broker-client-$PID.json"
$launch = @{
  version = 1
...
$digestInput = Join-Path $env:RUNNER_TEMP "sharker-windows-client-digest-$PID.json"
