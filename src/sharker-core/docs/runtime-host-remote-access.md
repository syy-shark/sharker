...
Sharker Desktop, TUI, and CLI can connect to a Runtime Host through TLS, SSH, or explicitly enabled plaintext WebSocket. The CLI and TUI also support the experimental direct-peer transport described below.
...
```sh
npx --yes --package sharker-agent@latest sharker runtime-host setup \
  --principal my-desktop \
  --preset desktop-client \
  --root "$HOME/.sharker/runtime-host" \
...
instead of accumulating credentials. The command installs its exact Sharker package into a managed
...
Run `npx --yes --package sharker-agent@latest sharker runtime-host service uninstall` on the Host to remove the service and
managed package. The State Root and Project data are retained.

## Manual Host setup

Build Sharker on the remote machine, choose a persistent State Root, and register each Project remote Clients may use:

```sh
npm run build
npm --workspace sharker-agent exec -- sharker runtime-host project add /srv/projects/example --root /srv/sharker
npm --workspace sharker-agent exec -- sharker runtime-host project list --root /srv/sharker
```
...
```sh
npm --workspace sharker-agent exec -- sharker runtime-host serve \
  --root /srv/sharker \
...
When any `--project-root <label>=<absolute-path>` option is present, only those roots are available to remote directory browsing. The option is repeatable up to eight times. Sharker resolves every root at startup and keeps browsing and registration contained within the selected root.
...
```sh
npm --workspace sharker-agent exec -- sharker runtime-host access issue \
  --root /srv/sharker \
  --principal my-desktop \
  --preset desktop-client
```
...
```sh
sharker runtime-host service install \
  --root /srv/sharker \
  --project-root projects=/srv/projects
sharker runtime-host service status --json
```

The install command persists the current exact Node and Sharker CLI paths. Re-running it updates the
same OS-managed service, and an omitted WebSocket port preserves the existing port. Before
uninstalling the npm package, remove the service with `sharker runtime-host service uninstall`. Service
...
global Sharker installation, not `npx`. A replacement is committed only after the new Runtime Host is
...
```sh
sharker runtime-host service peer enable \
  --expected-service-id '<serviceId>' \
  --expected-root-path '<rootPath>' \
  --expected-root-id '<rootId>'

sharker runtime-host service peer descriptor \
  --expected-service-id '<serviceId>' \
  --expected-root-path '<rootPath>' \
  --expected-root-id '<rootId>'
```
...
credential created by setup through `SHARKER_RUNTIME_HOST_ACCESS_CREDENTIAL`. Disable and re-enable
...
are advertised to Mesh peers, and Sharker still requires the application stream to upgrade to a direct
...
npm --workspace sharker-agent exec -- sharker runtime-host serve \
  ...
  --websocket-port 7443 \
  --tls-certificate /etc/sharker/tls.crt \
  --tls-private-key /etc/sharker/tls.key \
  --json
```
...
Sharker runs the system `ssh` executable without a shell and forwards a temporary Client-loopback port to the Host loopback listener. Normal OpenSSH aliases, keys, agents, and host verification apply. Host entries that configure additional port forwarding are rejected. Sharker never edits SSH config or cleans up shared OpenSSH state when a Profile is removed.
...
The Client Profile must separately persist the plaintext acknowledgement. Sharker never downgrades TLS or SSH to plaintext. Copy the service command's JSON `rootId`; Clients pin it to the expected State Root.
...
page. Sharker keeps the existing Local Host and State Root, moves that Host under the OS service
...
```sh
export SHARKER_RUNTIME_HOST_ACCESS_CREDENTIAL='<credential>'

# Direct TLS
sharker runtime-host profile set \
...
unset SHARKER_RUNTIME_HOST_ACCESS_CREDENTIAL
```

Then select a Host Project explicitly:

```sh
sharker --host office --project '<projectId>'
sharker run --host office --project '<projectId>' "Summarize this project"
```
...
- Revoke a credential on the Host with `sharker runtime-host access revoke --root /srv/sharker --credential <credentialId>`.
