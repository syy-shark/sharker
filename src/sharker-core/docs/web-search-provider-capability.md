...
Sharker originally exposed only a Tavily-backed `WebSearch` tool. That made live
...
pair is unsupported, Sharker omits the native tool from that turn. Tavily remains
...
| CLI / TUI / `sharker run` | the same persisted settings | May add provider-native `WebSearch` |
| Runtime Host | runtime-policy web-search settings | May add provider-native `WebSearch` |
| Headless Harbor | explicit `SHARKER_WEB_SEARCH_ENABLED=true` | May add provider-native `WebSearch` |
...
Messages is not enough to infer hosted-search support; Sharker uses explicit model
...
DeepSeek live verification on 2026-08-04 completed one real Sharker `AiSdkBackend`
...
annotations in that response, so Sharker preserves citations when supplied but
does not synthesize or invent them.

The same key was also verified through DeepSeek's Anthropic-compatible endpoint:
one real Sharker turn sent `web_search_20250305`, received one provider-executed
...
- `sharker run` through a persisted DeepSeek Responses connection;
- `sharker run` through a persisted DeepSeek Anthropic-compatible connection;
...
using the CC wire requires an explicit Anthropic-compatible connection. Sharker
...
| Provider or access path | Official hosted search surface | Model boundary | Sharker state |
| --- | --- | --- | --- |
| DeepSeek | Responses `web_search`, server-executed | `deepseek-v4-flash` and `deepseek-v4-pro` | Integrated through `openai-responses` |
| OpenAI API | Responses `web_search` tool | Sharker currently enables the native path for GPT-5 families, whose runtime wire is already Responses | Integrated through `openai-responses` |
...
| xAI API / OAuth | Responses Agent Tools `web_search` | Sharker currently enables the verified Grok 4.5 Responses route | Integrated through `openai-responses` |
| Alibaba Model Studio | Responses `web_search` | Qwen 3.5 Plus/Flash provider support is recorded | Provider supports it; Sharker Responses adapter pending |
...
| Google Gemini | Gemini API grounding with Google Search | Supported Gemini 2.0+ model families vary by release | Provider supports it; Sharker adapter pending |
| Z.AI / GLM Coding Plan | Chat tool and standalone Web Search API | GLM model/tool availability varies by endpoint | Provider supports it; Sharker adapter pending |
| Mistral | Agents/Conversations `websearch` connector | Agent-capable models | Provider supports it; Sharker adapter pending |
| Groq | Compound systems with built-in web search | `groq/compound` and `groq/compound-mini` | Provider supports it; Sharker adapter pending |
| OpenRouter | Web plugin and `:online` variants | Depends on routed model/plugin support | Provider supports it; Sharker adapter pending |
