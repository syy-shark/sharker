...
Most users configure only a few providers, but Sharker currently loads metadata for every provider and hundreds of models on startup. This data should remain behind the main-process authority boundary, with the renderer receiving only the lightweight projection needed for the current UI.
...
3. `@sharker/ui` `modelMenuGroups` → `PROVIDER_DEFAULTS`
...
大多数用户只配置少数几个 provider，但 Sharker 当前会在启动时加载全部 provider 和数百个模型的元数据。完整目录应留在 main process 的权威边界内，renderer 只接收当前界面所需的轻量投影。
...
1. `thinkingVariantsForModel` → `model-thinking.ts` → `model-metadata.ts`
2. `buildChatModelChoices` → `model-catalog-choices.ts` → `model-catalog.ts`
3. `@sharker/ui` 的 `modelMenuGroups` → `PROVIDER_DEFAULTS`
4. `provider-display.tsx` → `PROVIDER_DEFAULTS`
5. `OnboardingHero` → `RECOMMENDED_PROVIDER_TYPES`
