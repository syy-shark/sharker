...
# Sharker CLI npm release operations

[简体中文](./cli-npm-release.zh-CN.md)

This runbook is the operational authority for publishing the `sharker-agent` npm installation channels. The root `package.json` remains the sole Sharker product-version authority, and `packages/cli/package.json` must match it. Every public npm version must come from an exact tarball validated by the shared package workflow.
...
retaining Sharker's stronger protected-Environment, staged-publishing, 2FA, and Finalize controls.
...
In the `sharker-agent` package settings, configure one GitHub Actions trusted publisher:
...
`sharker-agent` tarball, and publishes it under `nightly`. Only after the exact version and dist-tag are
...
records the exact Runtime Host setup specifier, for example `sharker-agent@0.2.0-dev.42.20260829`; it
...
pages. Developers may install the moving channel explicitly with `sharker-agent@nightly`; product
...
4. Confirm the target version is absent from both public and staged package state:
   ...
   ```sh
   version=0.2.0
   npm view "sharker-agent@$version" version --registry https://registry.npmjs.org/
   npm stage list sharker-agent --registry https://registry.npmjs.org/
   ```
...
node scripts/product-release-authority.mjs verify-draft \
  "v$version" "$source_commit" apache/sharker
```
...
```sh
version=0.1.0
npm view sharker-agent dist-tags --json --registry https://registry.npmjs.org/
```
...
5. Approve the Environment. Confirm the workflow matches every live Draft digest to the exact
   ...
   `Sharker-<version>-attestation.sigstore.json`, publishes the convenience Release, and makes a stable
...
```sh
version=0.2.0
npm view "sharker-agent@$version" version dist.tarball dist.integrity --json
npm view sharker-agent dist-tags --json
```
...
```sh
known_good=0.2.0
npm dist-tag add "sharker-agent@$known_good" latest
```
...
bad_version=0.2.1
recovery_tag=latest
npm deprecate "sharker-agent@$bad_version" "Known issue; install sharker-agent@$recovery_tag."
```
