...
# Sharker Computer Use Foundation Contract
...
上述文件位于独立逆向实验仓库。本文只记录由 Sharker 测试锁定的合同，
不把外部路径声明为本仓库内链接。
...
4. Execution ownership
   - sharker-cu 是唯一 native executor；window/page discovery、semantic preparation、input dispatch 和 effect readback 均留在该边界内。
...
7. Approval and privacy
   ...
   - Sharker 采用分级短 lease：metadata read、screenshot read、pointer mutation、keyboard mutation、semantic mutation 分离；目标、action class、observation 或 session generation 变化时重新授权。
...
Sharker 自己的 Electron renderer 也是 Computer Use 的目标。它不能依赖截图或运行时注入
...
Sharker 在两个 native executor 上做过真机实测：cua-driver（trycua，Rust，MCP）和一个
自有的 Swift executor（协议 `sharker.cu/2`）。下面每一条都由真机实测得出，写在这里是
...
结果是**模型永远只看到错误码**，看不到那句可操作的话。`sharker.cu/2` 把这些收成闭
...
模型还能按坐标点。Sharker 的坐标路径默认关闭，藏掉的元素就是**够不到**的元素。跨 10
