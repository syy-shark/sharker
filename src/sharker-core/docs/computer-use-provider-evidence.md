...
The fixture helper imports Electron only. It does not import Sharker Runtime,
...
- `real-runtime`: a live provider model used the production Sharker runtime;
...
using the `SHARKER_CU_AX_MODEL_LAB_ROOT` fixture checkout described in
...
- tool exposure: direct E2E, with only the production `sharker_computer` tool;
...
`sharker-agent/sharker-cu#2`, the window-transition follow-up in #3, and the direct
