...
# Sharker full product completion and test plan
...
> Owner: @xuan
> Date: 2026-05-22
> Scope: one-month completion plan for Sharker desktop product capability, quality gates, and release testing.
...
Goal: Sharker stops behaving like a generic chat list and starts behaving like a workbench with explicit state.
...
- `npm --workspace @sharker/desktop run screenshots` captures all fixture
  scenarios across light/dark, 1280/990 width, normal/reduced motion.
- `npm --workspace @sharker/desktop run screenshots:diff:stable` is the
