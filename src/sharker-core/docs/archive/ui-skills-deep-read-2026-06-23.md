...
ask `f424c6b5`. Synthesis below — converged rules + sharker delta + 1 final
ship list.
...
| Compositor-only motion (`transform`/`opacity` only, ≤ 200ms) | baseline-ui, fixing-motion-perf, Impeccable /animate | Sharker may transition `background-color` on hover (paint) — acceptable but verify nothing animates layout |
| Reduced-motion guards | All 5 | ✅ done round 11 |
| Single accent, saturation < 80%, OKLCH neutrals tinted toward brand | Taste-Skill, Impeccable /colorize | sharker uses raw `%foreground` overlays, no brand tint |
...
| Sidebar primary nav 5-7 items, distinct active state | UIDB, Taste-Skill | sharker has more than 7 in "AI 与集成" group — could collapse; active state is light-fill, no accent bar |
| One CSS variable for all motion timings (no scattered ms values) | 12-principles "timing-consistent" | Sharker has scattered 150ms values, no `--motion-fast` token |
...
| Inter/system as default font flagged as "AI tell" | Taste-Skill, redesign, soft | sharker is system-ui default — flagged |
| Lucide-as-default flagged | Taste-Skill | sharker is lucide-react — flagged, Better Icons proposed |
...
| Page header height 56-72px | UIDB | sharker uses padding stack, not a true bar — partial |

## Anti-patterns triggered by sharker today
...
- **Taste-Skill banned: border-b on every row.** ❌ — wait, sharker DOES use `border-bottom` hairlines between rows. WAWQAQ asked for this explicitly in PR-GROUPED-CARD-0 ("内容紧密、用分隔线划开"), so this disagreement stays unresolved between Taste-Skill and WAWQAQ. Defer to WAWQAQ.

## 10 highest-value action items for sharker
