...
# Sharker 设计系统契约 · design-system.md
...
>
> 这是 Sharker 桌面端的设计系统**契约**，不是手册、不是规范文档、不是 marketing
...
- 不是一份 marketing-style "Sharker 设计哲学" — 那是另一篇文档的职责。
- 不是 reference implementation re-engineering doc 的复述，详细对照见
  `notes/sharker-vs-reference implementation-gap-v2.md`。
...
| `apps/desktop/src/renderer/sharker-tokens.css` token 定义 | ✅（本文件 §1 是其规范） |
| `apps/desktop/src/renderer/styles.css` 组件 recipes | ✅ |
| Settings IA + Coming Soon copy | ✅（§5 文案契约） |
| Markdown 渲染 / hljs 调色板 | ✅ |
| 后端 IPC / runtime / @sharker/core 的 token 命名 | ❌（后端 contract 见 @kenji notes/）|
...
1. **token 不二选一**：颜色/间距/字号/阴影/动效曲线/z-index 全部来自
   `sharker-tokens.css`，不在组件里硬编码 hex / px / 数字曲线。
...
> 单一权威：`apps/desktop/src/renderer/sharker-tokens.css`。本节按"类别 → 名称 →
...
**Heading em scaling**：`.sharker-bubble-assistant h1-h4` 用 em off body 15（h1=1.47em, h2=1.27em, h3=1.07em, h4=0.93em），track root 自动。Surface-specific hero 大字（onboarding/settings title 16–32px）也用 em。
...
- 文章标题、章节标题用 600 weight + `text-wrap: balance`（见
  `.sharker-bubble-assistant h1..h4`）。
...
> 这是有意的反规范：Sharker 整体 0px 直角 + 关键交互组件 6/8/12px + 大面板上限 12px。
...
Sharker 不提供用户可配置的界面密度。新增布局应按具体 surface 的信息量定间距，不要新增全局 density switch。
...
图标由 `@sharker/ui/icons` 出口（通用 UI 图标底层是 `lucide-react`；bot/channel 品牌 SVG
...
- **chrome 是唯一被 token 化的档。** nav 图标（`.sharker-nav-icon`）和 `packages/ui/src/ui.tsx` 的 `buttonVariants`（`[&_svg]:size-[var(--icon-size,1rem)]`）都消费 `--icon-size`，所以两者不会再各走各的（历史缺陷：nav 18px 而按钮 1rem）。改这一个 token 同时移动两者。
- **密集 / hero 是 call-site 刻意值，不收敛。** 它们是有意的视觉层级，不是漂移；不要用全局 `svg.lucide { width }` 之类的规则一锅端，那会把 11–14px 的小图标静默放大、把 hero 压小。
- **chrome 图标统一 80% ink（`--foreground-secondary`），颜色由组件 variants 声明，不写在 unlayered CSS 里。** 按钮内图标继承按钮文字色即可；nav 行因 darwin glass 把行文字提到 100%（`theme-glass.css` 的 `html[data-os="darwin"] .sharker-nav-row`），图标由 `navRowVariants` 的 `[&_.sharker-nav-icon]` utility 钉在 80%，active 行提亮到 100%。不要在 unlayered CSS（如 `.sharker-nav-icon`）里写 `color`——它会静默压过 `@layer utilities` 里的全部状态意图（历史缺陷：nav 图标被钉在 `--muted-foreground`（50% ink），active/tone 提亮失效，且与 titlebar 图标的 80% ink 不一致）。
...
### 3.1 Button (`.sharker-button`)

- **位置**：CSS recipe `apps/desktop/src/renderer/styles.css` 内（注：基础 token
  recipe 在 `sharker-tokens.css` 末尾 `@layer components` 也定义了一份兜底）。
...
- **token**：4 个 variant 颜色绑定 info / success / warning / error；icon 来自
  `@sharker/ui/icons`（底层 `lucide-react`），`size={16}` 固定。
...
### 3.3 Modal（useModalA11y hook + `.sharker-modal-backdrop` / `.sharker-modal`）

- **位置**：hook `components.tsx:75`；CSS recipe `sharker-tokens.css:1225` (.sharker-modal-backdrop)。
...
> 这是 Sharker 安全 UX 的核心，所有改动须先经 @kenji review。
...
- **token**：`data-tone="destructive"` 时 primary button 走 destructive
  variant；hint 区使用 `--info` tinted 背景；命令预览走 `.sharker-code` (mono)。
- **ARIA**：`role="alertdialog"`、`aria-modal="true"`、`aria-labelledby="permissionTitle"`；
  reason 区有 `data-reason` 与 `.sharker-reason-text` tone 联动。
...
- **token**：背景 `--background`；focus-within border 与 box-shadow 见
  `.sharker-composer-inner:focus-within`；max-height 240px hardcoded（设计取舍）。
...
- **键盘契约**：
  ...
  - IME composing 期间所有快捷键 short-circuit（`event.nativeEvent.isComposing`
    或 `event.key === 'Process'`）
- **placeholder**：`"给 Sharker 发消息…"` 固定 — 不要根据 mode 改 placeholder（用户已经看
  得到 mode 切换器）。
- **5 态**：
  ...
  - streaming：Toolbar swap → "Sharker 正在思考… [Esc] 或点 Stop 中断" + pulsing
...
- **5 态**：
  ...
  - streaming：`data-streaming="true"` → 顶部加 pulse dot + 副文本 "Sharker 正
...
- **职责**：把"user → tools → assistant"渲染为一个 visual unit (`<section
  class="sharker-turn">`)。turn 的 meta（model · duration · cost）落在 footer
...
- **token**：状态点 dot 走 `.sharker-tool-status-dot[data-status="…"]`；
  running 时附 box-shadow 脉动 `sharker-tool-pulse`；errored 时 border / 背景皆
  转 destructive 浅色。
...
- **5 态**：
  ...
  - disabled：禁用某行（如 provider 报告该 model 不可用）→ opacity 0.5 + 不可
    focus；当前未触发
  - loading：整个 ProvidersPanel 自身处理 `loading` 态，使用 `.sharker-skeleton-card` 占位
  - error：fetchModels 失败 → toast.error + 留在 fallback 模式

### 3.11 MessageCopyButton（`components.tsx:893`）

- **两种变体**：
  - **floating（无 label）**：默认。`.sharker-message-copy` absolute top-right，
...
- **不变量**：
  ...
  - block code 用 `.sharker-code-block` wrapper：language pill header + copy 按钮
...
### 3.13 Tabs（`packages/ui/src/primitives/tabs.tsx` + `sharker-tab`）

Tab spec 是“一个 primitive + 两个 variant + neutral state token”，不是每 surface 手写。

- **primitive**：`packages/ui/src/primitives/tabs.tsx`（Base UI Tabs）。`TabsTab` 吐 `sharker-tab` class；`TabsList` 接受 `variant="underline" | "pill"` 并设 `data-variant`。`ui.tsx` 的 `TabsRoot/TabsList/TabsTrigger/TabsPanel` re-export 这一套，不走第二套手写。
- **variant**：
...
- **不要**：手写 `[data-state=active]::after` under-bar、手写 `[data-active="true"]` brand 背景、per-surface 藏 Base UI indicator。active/hover 一律走 `.sharker-tab`。
...
- **contract**：`tab-spec-499-contract.test.ts` 锁每个 surface 用 `sharker-tab` / `SettingsSegmented` + 无手写 active/hover CSS；`state-token-governance-499-contract.test.ts` 禁 selected/active 用 brand token（tab 不再是例外，无需 allowlist）。
...
| Cursor pulse (`sharker-cursor`) | 1100ms (loop) | `ease-in-out` |
| Skeleton shimmer (`sharker-shimmer`) | 1500–1600ms (loop) | `ease-in-out` |
...
4. **prefers-reduced-motion: reduce** 触发 `sharker-tokens.css:1380` 全局兜底：
   ```css
   *, *::before, *::after {
   ...
   - 这是**最大锤**：组件不需要再单独写 reduced-motion 分支，**除非**默认
     ...
     `.sharker-bubble-streaming::after` 已经处理）。
...
源：`@sharker/core` 的 `generalizedErrorMessage()`（xuan `01b533f`）。6 种 errorClass：
...
|---|---|---|---|---|
| Chat 主区（`.sharker-chat`） | ✅ 全局 focus ring | ✅ streaming caret + composer dot | ✅ tool error banner + chat header alert | ✅ EmptyChatHero / OnboardingHero |
...
```
npm --workspace @sharker/desktop test
  → pretest:
      ├── npm --workspace @sharker/core run build
      ├── npm --workspace @sharker/ui run build
...
✅ 使用 `sharker-tokens.css` 中 token；新 token 必须先登记到 §1
...
✅ `.hljs-*` 选择器来自 `sharker-tokens.css`，oklch(from var(--accent) …) 系列
...
> **PR108 选定**（@kenji 2026-05-22）：作为 Sharker 从 chat app 跨入 workbench 的
...
```ts
// preload window.sharker.artifacts
artifacts: {
...
新增 `ArtifactPane` 组件，位于 chat shell 的右侧分栏。`<aside class="sharker-artifact-pane">`：

- 默认隐藏；当 session 内至少 1 个 live artifact 时显示
- 用户可手动 collapse/expand（持久化到 localStorage `sharker-artifact-pane-v1` 类似
  PR54 模式）
- 宽度：默认 360px，可拖拽 280-560 范围，复用 `sharker-resize-handle` 模式
- 内部三个区域：
...
| kind | preview 组件 | 限制 |
|---|---|---|
| `file` | `<pre class="sharker-artifact-file">` + 复用 rehype-highlight | text 文件；非文本走 `image` / `pdf` 分支 |
...
4. **外链 v1 = 完全禁用**（@kenji 2026-05-22 review #6 — corrects earlier
   ...
   **不会**自动冒泡到主 renderer 的 `setWindowOpenHandler`；它们只是静默失
   ...
   - **HTML transform**：main 读取 HTML 时把 `<a href>` 改写为
     `<button data-sharker-link="..." />` + 注入 inline script 经 `postMessage`
...
- **smoke path**（smoke.md 新增 Path 11）：
  - 入口：用 `SHARKER_VISUAL_SMOKE_FIXTURE=artifact-pane` 启动
...
- **smoke path**（smoke.md Path 13）：
  - 入口：用 `SHARKER_VISUAL_SMOKE_FIXTURE=artifact-errors` 启动
...
| Sub-PR | 范围 | 责任 |
|---|---|---|
| **PR108a** | `@sharker/core` 加 `artifacts.ts` + types；`@sharker/storage` 加 artifact 存储；fixture seed 数据 | @xuan |
...
Composer streaming 状态目前没有 `aria-live`。屏幕阅读器用户感知不到 "Sharker
正在思考…"。需要决定：
...
**Status enum** (`@sharker/core` `SessionStatus`，PR109a 落地):
...
| `running` | runtime 流式响应或工具/网关请求执行中 | 脉动 accent `Loader2` icon + "Sharker 正在思考…" preview (PR85) | active / review / aborted / blocked |
...
`SessionBlockedReason` (`@sharker/core`, PR109a):
...
**TurnRecord 字段** (`@sharker/core`, PR109c):
...
- `SessionChangedReason` 新增 `turn-status-change`（独立于 session-level
  `status-change`）
- `@sharker/ui` `TurnViewModel` surface 同名字段；
...
Sharker 当前第一步实现为 core contract + existing module surface：skills 仍来自
文件系统扫描结果（`window.sharker.skills.list()`），scheduled tasks 使用统一 catalog。
`@sharker/core/capability-audit` 把这两类现有快照派生成一个可测试的
...
来自 @kenji 自己的 `notes/sharker-reference implementation-reference implementation-design-gap-v2.md` +
`notes/sharker-redesign-v1-contract-smoke-gate.md`。这些是 next-wave 的工作
...
- 新增**必须**同 PR 修改 `sharker-tokens.css` + 本文档 §1 对应小节。
...
引用 `SHARKER_VISUAL_SMOKE_FIXTURE=<scenario>` 中的某一个 scenario（@xuan
...
| lucide-react | ^0.577.0 | 通用 UI icon（packages/ui/src/icons.tsx 是唯一接口；call-site 用 `@sharker/ui/icons`） |
...
| 主题 | 路径 |
|---|---|
| Token 定义 | `apps/desktop/src/renderer/sharker-tokens.css` |
