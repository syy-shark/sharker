...
- `notes/reference implementation-memory-reverse-2026-05-25.md` (kenji msg `66fd3eab`) — reference implementation facts + Sharker quasi-memory inventory.
...
1. **Sleep cycle / 4-layer consolidation** (`22-embeddings.md`) — Sharker has no sleep stage. No code path automatically aggregates, summarizes, or merges memory entries.
...
5. **Activity-derived memory** — Activity recorder feeds Sharker memory only via `activity_observation` candidate source, never as a durable source. Activity audit (separate lane) MUST run before any activity→memory pipeline.
6. **Unauthenticated local route** (reference implementation's `05-express-api.md` exposes memory over local HTTP) — Sharker memory MUST NOT have a local HTTP endpoint; if Local Gateway lane lands, it MUST go through capability scope allowlist + audit log, never default-on.
...
These existing Sharker surfaces contain durable user-correlated data BUT MUST NOT be treated as `MemorySource` and MUST NOT auto-promote:
