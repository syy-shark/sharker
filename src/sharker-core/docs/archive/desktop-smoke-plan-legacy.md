...
# Sharker desktop smoke test plan
...
Either start clean (`rm -rf ~/Library/Application\ Support/sharker` on
...
launched build (`npm --workspace @sharker/desktop run dev` or a packaged
build).
...
```bash
npm --workspace @sharker/desktop run smoke:real-window
```

The script builds `@sharker/core`, `@sharker/ui`, and desktop, launches a real
...
```bash
SHARKER_VISUAL_SMOKE_FIXTURE=all npm --workspace @sharker/desktop run dev
```

Single-scenario launches are also supported:

```bash
SHARKER_VISUAL_SMOKE_FIXTURE=first-run npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=provider-workspace npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=fallback-source npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=fetched-empty npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=connection-error npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=turn-narrative npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=streaming-sidebar npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=permission-destructive npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=artifact-pane npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=artifact-errors npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=stale-sessions npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=settings-data npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=settings-personalization npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=settings-network npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=settings-bots npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=settings-about npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=settings-theme npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=settings-daily-review npm --workspace @sharker/desktop run dev
SHARKER_VISUAL_SMOKE_FIXTURE=workstation-statuses npm --workspace @sharker/desktop run dev
```
...
npm --workspace @sharker/desktop run screenshots:single artifact-pane

# All scenarios × all variants (full regression baseline)
npm --workspace @sharker/desktop run screenshots
```

Output: `apps/desktop/tests/screenshots/<scenario>/<variant>.png`.

Implementation: the script spawns `electron .` once per (scenario,
variant) with `SHARKER_VISUAL_SMOKE_FIXTURE=<scenario>` +
`SHARKER_VISUAL_SMOKE_AUTO_CAPTURE=<variant>` (+ optional
`SHARKER_VISUAL_SMOKE_REDUCED_MOTION=1`). The renderer waits 2 RAFs + 400ms
idle after fixture settle, then calls `window.sharker.visualSmoke.capture()`.
...
```bash
npm --workspace @sharker/desktop run screenshots
npm --workspace @sharker/desktop run screenshots:diff:stable
```
...
```bash
npm --workspace @sharker/desktop run screenshots:baseline:stable
```
...
```bash
npm --workspace @sharker/desktop run screenshots:diff      # all scenarios
npm --workspace @sharker/desktop run screenshots:baseline  # full promotion
```

### Reduced-motion variant (PR-IR-04)

Combine `SHARKER_VISUAL_SMOKE_REDUCED_MOTION=1` with any of the above to
...
```bash
SHARKER_VISUAL_SMOKE_FIXTURE=artifact-pane \
  SHARKER_VISUAL_SMOKE_REDUCED_MOTION=1 \
  npm --workspace @sharker/desktop run dev
```

Implementation: main process passes the flag through `VisualSmokeState`;
renderer applies `data-sharker-reduced-motion="true"` to `<html>`; CSS in
...
`SHARKER_VISUAL_SMOKE_FIXTURE` is set.
...
**Pass signal.**
- The chat surface renders **OnboardingHero** (the "Welcome to Sharker"
...
2. Verify the composer toolbar swaps in **"Sharker 正在思考…"** with the
...
5. Delete the currently-active session mid-stream. Options, easiest
   ...
   - **IPC-level (preferred for automated test runs)**: from DevTools
     console, fire `window.sharker.sessions.remove(activeSessionId)`. The
     `sessions:changed { reason: 'deleted', sessionId }` broadcast is
     the contract under test, not the right-click affordance.
   - **GUI**: from a *second* Sharker window pointed at the same workspace
...
- The user message, the tool activity panel, and the assistant
  ...
  class="sharker-turn">`), not three free-floating items.
...
- Session A's row shows a small pulsing accent-tinted dot next to
  the session name.
- The row preview text shows *"Sharker 正在思考…"* (overrides the
...
**Precondition.** Sharker running with at least one verified connection
...
- "导出当前对话为 Markdown" lands a structured markdown doc on the
  clipboard with `# {sessionName}` + `## 你` / `## Sharker` sections;
...
**Precondition.** Sharker running in fixture mode (`SHARKER_VISUAL_SMOKE_FIXTURE=all`)
...
must still flow through `window.sharker`.
...
**Pass signal.**
- `window.sharker.settings`, `window.sharker.app.openPath`,
  `window.sharker.connections`, `window.sharker.sessions`, and
  `window.sharker.visualSmoke` all respond through preload IPC.
- No external page opens inside the Sharker BrowserWindow; allowed http(s) /
...
**Steps.**
1. Launch Sharker with the fixture above. The artifact session is activated
...
9. Click the collapse toggle in the pane header. Pane should shrink to a
   ...
   collapsed (persisted via localStorage `sharker-artifact-pane-collapsed-v1`).
...
**Steps.**
1. Launch Sharker against the workspace.
...
**Steps.**
1. Launch Sharker with the fixture above. The "Artifact Pane 验收" session
...
**Steps.**
1. Launch Sharker with the fixture above. The "正在生成报告" session is
   active.
...
- Running session shows no spinning indicator (the `Loader2` icon
  ...
  active or `SHARKER_VISUAL_SMOKE_REDUCED_MOTION=1` is set).
...
```bash
SHARKER_VISUAL_SMOKE_FIXTURE=turn-control-history SHARKER_VISUAL_SMOKE_THEME=light \
  SHARKER_VISUAL_SMOKE_AUTO_CAPTURE=primary-light-1280 npm run dev
```
...
- **S4 — branch banner positive vs negative.**
  ...
  - In `turn-control-branch-orphan`, NO banner appears in the chat
    ...
    DOM check: `.sharker-session-branch-banner` must not exist.
- **S5 — visual-smoke collapses smooth scroll.** Lineage badge clicks
  ...
  because the fixture sets `data-sharker-visual-smoke="true"` on `<html>`.
...
### S4 — C2: `sharker://` internal markdown URI router

**Contract invariant.**
- `parseSharkerUri(href)` is **strict** (lowercase `sharker:` only) and
  closed-world: only `sharker://settings/<SettingsSection>` and
  `sharker://compose?text=...` resolve to typed `SharkerUriDest`. Any
  other shape returns `null` and surfaces as an inline broken-link
  span with a typed `data-reason`.
- `isSharkerUriCandidate(href)` is case-insensitive (`/^sharker:/i`) — it
  exists so case-variants (`Sharker://settings/account`) route to the
...
**Targeted tests** (`apps/desktop/src/main/__tests__/sharker-uri.test.ts`,
41 cases total — 29 from PR-UI-RENDER-2 + 12 from C2 fixup):
- Lowercase `sharker://settings/account` → typed destination.
- `Sharker://settings/account` (case-variant) → `parseSharkerUri` returns
  `null` but `isSharkerUriCandidate` returns `true`.
- `SHARKER://settings/account` → same as above.
...
- `sharker-info://...` (similar prefix) → not a candidate.

**Source-gate grep.**
- `MarkdownLink` renderer dispatches via `isSharkerUriCandidate(href)` →
  `parseSharkerUri(href)` → internal button OR broken span; external
...
**Source-gate grep.**
- `.sharker-tool`, `.sharker-tool-diff`, `.sharker-tool-terminal` — radius
  10 px (was 8 px).
- `.sharker-artifact-pane-collapse` `:focus-visible` selector present
...
- Renderer path: `applyThemePalette(palette)` writes
  `<html data-sharker-theme="<palette>">`; `'default'` removes the
  attribute. CSS variable overrides live in `sharker-tokens.css` under
  static `[data-sharker-theme="..."]` selectors.
...
- Renderer: `applyThemePalette` consumed only from normalized
  settings; no raw `localStorage.getItem('sharker-theme-palette-v1')`
  read path today.
...
- If a pre-React palette read is added later (FOUC prevention),
  ...
  written directly to `data-sharker-theme`.
...
- `localStorage('sharker-toast-position-v1')` is a pre-React boot
...
- `localStorage.getItem('sharker-toast-position-v1')` → only flows
  through `isToastPosition()`.
- `localStorage.setItem('sharker-toast-position-v1', …)` writes only
  settings-load normalized value or post-onUpdate-success
  normalized return.
- Active code must contain NO `querySelector('.sharker-toast-viewport')`
...
**Status**: contract-only. No Sharker Computer Use implementation
...
architectural prior art that informs these boundaries. Sharker's
...
- The CU overlay (the highlight ring / target box / cursor halo
  that shows where Sharker is about to click) exists ONLY during an
...
- Overlay teardown MUST fire on every action-terminating event:
  ...
  closes Sharker / permission revoked mid-action.
...
- Reduced-motion: overlay animations respect
  `prefers-reduced-motion` (same `data-sharker-reduced-motion` channel
...
- Overlay never becomes a `<button>` / `<a>` / tab-order target /
  ...
  target app (or, if Sharker window is foreground, on Sharker).
...
- Renderer never imports a coordinate-click IPC and never calls
  `window.sharker.computerUse.click({ x, y })` from user input
...
**Contract invariant.**
- Any text Sharker extracts from a screen — OCR output, AX tree dump,
  selected-text query result, clipboard sample, window title, app
  bundle id, focused element label, URL bar contents — MUST run
  through the runtime's `redactSecrets` (the `@sharker/runtime`
...
**Source-gate grep.**
- Every CU action handler in `@sharker/runtime` that returns
...
**Contract invariant.**
- User-initiated abort (Esc, "Cancel" button in chat, close Sharker,
...
- Renderer abort affordance (Esc in chat, Cancel button) wires
  through to `window.sharker.computerUse.abort(actionId)`. No
...
- Sharker's eventual CU implementation does NOT have to use a separate
...
- The Sharker workspace's existing IPC allowlist patterns
...
- The `redactSecrets` helper currently lives in `@sharker/ui`. A CU
  threat model implementation will need a runtime-side import
  path (already exists at `@sharker/runtime` re-export); the renderer
  defense-in-depth in `@sharker/ui` stays unchanged.
