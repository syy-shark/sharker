...
目标不是再列一组 wish list，而是把 Sharker 从 "能聊天的桌面 app" 推向
reference implementation / reference implementation 那类成熟工作台时必须补齐的系统 contract。

审计参考：

- Sharker 当前代码：`packages/core/src/session.ts`,
...
Sharker 当前的 tool result 都落在 transcript 里：
...
Sharker 已有 `ToolProgressEvent`，但当前 runtime tools 多数只在 tool 完成后返回完整 result。
...
Sharker 有 turn grouping，但没有 turn-level controls：
...
Sharker only lists installed skills from `workspaceRoot/skills`. There is no first-class source, no source-scoped permissions, no automations.
...
This is a strong model for Sharker because it keeps external capability visible and revocable.
...
Sharker has personalization prompt, but no memory object:
...
Why first: this changes Sharker from "chat transcript" to "work product workspace".
...
From reference implementation/reference implementation, do not copy these patterns into Sharker:
...
Sharker should copy structure and state models, not unsafe shortcuts.
