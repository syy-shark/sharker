---
title: Sharker Runtime 主线教学手册
document: SHARKER_RUNTIME_MAINLINE_TEACHING_MANUAL
version: 1.0
status: architecture teaching manual
date: 2026-06-24
audience: "第一次理解 Sharker runtime 的工程师、后续 runtime 架构维护者"
scope: "用设计动机和心智模型解释当前 Sharker runtime 为什么分层、每层解决什么问题、少一层会怎样；源码入口只作为文末简要索引。"
primary_spine:
  - SessionManager
  - RuntimeKernel
  - AgentRun
...
# Sharker Runtime 主线教学手册

Sharker runtime 的核心设计，不是把一条用户消息直接送给模型，而是把“一次对话回合”拆成可展示、可运行、可追溯、可恢复的几类事实。

如果只用一句话概括：

> Sharker 用 `SessionManager -> RuntimeKernel -> AgentRun -> RuntimeRunner -> AiSdkFlow -> AiSdkBackend` 这条主线，把“用户看到的一轮对话”和“系统内部一次可记录的模型/工具运行”分开管理。
...
Sharker runtime 解决的不是“怎么调用一次 LLM API”这么窄的问题。如果只是调用模型，代码可以很简单：拿用户输入，拼历史，调用 provider，流式返回文本。但 Sharker 要处理的是 agent 产品里的完整运行问题。
...
这些场景有共同的核心：都需要模型/工具循环。它们也有不同的外层责任：桌面关心 UI stream，headless 关心 workspace 和 verifier，autonomous 关心 attempt/retry，Harbor 关心 container 和 benchmark reward。Sharker runtime 的分层，就是为了让“共同核心”可以复用，同时让“外层责任”不要污染模型循环。
...
读 Sharker runtime 前，最好先建立三个词：turn、run、event ledger。这三个词容易被混在一起，但它们不是同一件事。
...
所以 Sharker 需要保留 session/turn 这套外部接口。如果没有 turn 这个心智模型，runtime 会很容易变成内部 run ledger 的展示工具，UI 和用户操作会被迫理解太多底层概念。
...
所以 Sharker 逐渐把 runtime 事实写成 `RuntimeEvent`。这些事件以后可以被投影回 UI，也可以用于模型 replay，也可以帮助恢复、检查和调度。如果只有聊天记录字符串，很多语义会被压扁：工具调用变成一段文本，thinking 是否可 replay 不清楚，terminal 状态和 verifier 状态容易混在一起。

这就是为什么 runtime-event ledger 很重要。

## 一条消息进入 Sharker 后发生了什么
...
当前 Sharker 已经有 `AiSdkBackend` 能产生 renderer-facing `SessionEvent`。这些事件服务 UI 和旧 session projection 很久了，不能一下子全部丢掉。但新 runtime 需要 canonical `RuntimeEvent`。
...
Sharker runtime 当前最容易让新人困惑的地方，是为什么同时有 `SessionStore`、`AgentRunStore`、`RuntimeEventStore`。直觉上，一个系统不是应该只有一份聊天记录吗？问题在于，它们回答的问题不同。
...
如果没有 `RuntimeEventStore`，Sharker 只能继续依赖 legacy projection。那会让 provider-native replay、read model、checkpoint high-water mark 都缺少可靠基础。
...
当前 Sharker 是迁移态，所以会双写、会 fallback、会比较 projection。这不是设计混乱，而是为了在不打断现有产品路径的情况下，把语义权威逐步迁到 runtime events。

## Runtime-event Replay 为什么不是简单重放日志

`RuntimeEventStore` 很重要，但不能被误解成“把 runtime-events.jsonl 从头到尾喂给模型就行”。Sharker 的 replay 是有 gate 的。
...
Python `SharkerAgent` 是 Harbor installed agent。它在 Harbor 环境里拿到任务指令，必要时启动 host-side Node cell，并把模型可见的工具执行通过 HTTP executor 转回 Harbor task container。

真正的 Sharker runtime session 在 Node cell 里创建。cell 创建 stores、backend registry、`SessionManager`、session，然后 drain `sendMessage()`，最后写 `runtime-events.jsonl` 和 `sharker-cell-output.json`。
...
当前 Sharker 还没有把所有 checkpoint / warm retry 问题都解决。但现在的分层已经在为这些问题铺路。
...
第八个误解：Harbor 是 Sharker runtime 的一种 backend。不是。Harbor 是 benchmark/container/tool-execution 外壳；Sharker runtime session 在 Node cell 里。
...
Sharker runtime 当前的设计可以从一个简单问题开始理解：一条消息进入 Sharker 后，为什么不直接丢给模型？

答案是：因为 Sharker 需要的不只是模型回复，还需要 session 兼容展示、run 生命周期、invocation 语义、runtime-event replay、工具/权限执行、headless/task/Harbor 外壳，以及未来可恢复和可调度的事实边界。
...
- `packages/headless/harbor/sharker_agent.py`
