...
1. **Chip expansion + CSS-label migration** — 7+ hand-rolled tone→alpha tables collapse into
   primitives/chip.tsx. Order: primitive-free CSS labels first (sharker-skill-library-status-label,
   sharker-scheduled-task-run-status, sharker-daily-review-archive-status), then settingsAuthActionPill
...
2. **Item adoption for list rows** — primitives/item.tsx already encodes 4%/8% fills; add a
   ...
   sharker-skill-library-row, daily-review archive/session rows, settingsOsPermissionRow.
...
3. **PageHeader primitive** — module h2 shells (sharker-module-main-header, sharker-scheduled-task-hero) +
...
5. **SectionHeader + ActionRow (+ EmptyState inline variant)** — three section-header dialects
   (sharker-skill-section-row / sharker-daily-review-section-title with ::before accent bar /
   settingsPermissionSectionHeader); action rows: settingsActionRow already shared, fold
   sharker-module-main-actions / sharker-scheduled-task-top-actions / sharker-daily-review-actions.
...
- [~] 2 Item rows — round 2 shipped on feat/item-row-convergence: Item gained
      ...
      sharker-skill-library-row (bespoke grid + parent-li hover, skills.test.ts
...
- [x] 3 PageHeader — SHIPPED on feat/page-header-convergence: new
      ...
      Migrated all 6 call sites: skills-panel sharker-module-main-header,
      scheduled-task-panel sharker-scheduled-task-hero (contentClassName=sharker-scheduled-task-heading),
      ...
      unchanged). check-a11y: added `Sharker` to the brand allow-list (About
