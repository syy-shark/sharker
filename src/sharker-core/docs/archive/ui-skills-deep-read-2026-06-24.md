...
## Cross-reference: what's already covered in Sharker source

The 11+ external + 13 local skills converge on a tight set of rules.
Sharker coverage (audit done 2026-06-24):

| Rule | Sharker source ref | Status |
|------|----------------|--------|
| transform + opacity only animations | All keyframes (sharker-list-row-enter, sharker-tool-card-enter, sharker-message-row-enter, sharker-onboarding-child-enter, sharker-hero-enter, sharker-composer-stream-bounce, sharker-composer-permission-pulse) | ✅ |
| Reduced-motion guards | ~12 `@media (prefers-reduced-motion: reduce)` blocks | ✅ |
| Custom cubic-bezier (`--ease-sharker` = `cubic-bezier(0.16, 1, 0.3, 1)`) | All easing references | ✅ |
...
| `text-wrap: pretty` on body, `balance` on headings | `.sharker-bubble-assistant p` (pretty), h1-h4 (balance) | ✅ (shipped this round) |
| Body max-width 65-75ch | `.sharker-bubble-assistant { max-width: 72ch }` + escape valves on pre/table/code | ✅ (shipped this round) |
| h1/h2 negative tracking | `letter-spacing: -0.012em` on h1/h2 | ✅ (shipped this round) — conservative for CJK mix |
| `tabular-nums` on numeric text | `.sharker-bubble-assistant code`, `.sharker-artifact-row-meta`, `.sharker-list-row-meta`, `.sharker-tool-duration` | ✅ (shipped this round) |
| Nested-bezel avatar | `.sharker-message-avatar` 24×24 full-circle + 3-layer box-shadow | ✅ (shipped this round) |
| Stagger entrance on hero | `.sharker-onboarding > *:nth-child(n)` with 0/60/120/180/220ms cascade | ✅ (shipped this round) |
| Perpetual micro-interaction on pending state | `.sharker-composer-permission-dot` 1.4s pulse + `.sharker-composer-streaming-dot` 1.05s bounce | ✅ (shipped this round) |
| `<details>` chevron rotation + body fade-in | `.sharker-turn-thinking` summary::before chevron + body @starting-style opacity ramp | ✅ (shipped this round) |
...
| Code blocks: language pill + copy button + 10px radius | `.sharker-code-block` matches | ✅ |
...
- **Heavy display typography (`clamp(4rem, 10vw, 15rem)`)**: industrial-brutalist style, doesn't match Sharker's calm desktop aesthetic. Skip.
- **Inline typography images at type-height**: stitch pattern, not applicable to a chat tool.
- **Footer with legal links**: Sharker is a desktop app, no public footer. Skip.
- **GSAP scroll-trigger paradigms**: Sharker doesn't have marketing scroll surfaces. Skip.
