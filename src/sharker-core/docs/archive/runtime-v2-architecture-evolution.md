...
This document proposes a long-term evolution path for Sharker's agent runtime. It
...
The goal is not to replace the Vercel AI SDK. The goal is to make Sharker own its
runtime semantics while continuing to use the AI SDK as the main model/tool
stepping engine.

## Executive Summary

Sharker already has a working local coding agent runtime:
...
This proposal is based on the current Sharker implementation and on the runtime
...
Sharker has different product constraints, so the evolution should be adapted:
...
- Sharker does not have its own explicit `Flow.Run`; the model/tool loop is
...
engine, but wraps it in Sharker-owned invocation and event semantics.
...
The AI SDK remains the main engine, but Sharker gains explicit flow input/output
semantics.
...
- How should branch/agent transfer be represented before Sharker has a full
  multi-agent tree?
