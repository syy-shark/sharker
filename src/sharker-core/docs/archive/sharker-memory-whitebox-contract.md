...
# Sharker memory — white-box contract anchor (2026-05-29)
...
## <title or slug>
<!-- sharker-memory: id=<id> origin=manual createdAt=<unix-ms> -->
<content lines>

## <next title>
<!-- sharker-memory: id=<id2> origin=manual createdAt=<unix-ms> -->
...
```
...
```html
<!-- sharker-memory: id=<id> origin=<origin> createdAt=<ms> updatedAt=<ms> status=<status> tags=<csv> decayTtlMs=<ms> -->
```
...
If we adopt:
- `MemoryProvider` interface in `@sharker/core/memory`
...
aggregation. Sharker has workspaces. V0.3 could expose a
