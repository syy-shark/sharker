...
Sharker already had the pieces of a local desktop coding agent: sessions, model
...
provider / AI SDK details
  -> ModelAdapter
  -> Sharker runtime events and usage records
```
...
```sh
npm --workspace @sharker/core run typecheck
npm --workspace @sharker/storage run test
npm --workspace @sharker/runtime run typecheck
npm --workspace @sharker/runtime run test
npm --workspace @sharker/desktop run build:main
git diff --check
```
