---
title: Sharker heavy-task / Terminal-Bench 主线 System Design
document: SHARKER_HEAVY_TASK_MAINLINE_SYSTEM_DESIGN_V1
version: 1.0
status: design draft with rationale
date: 2026-06-24
audience: "准备实现、评审或维护 Sharker heavy-task / Terminal-Bench 路径的新工程师"
derived_from:
  - "Sharker heavy-task / Terminal-Bench 主线教学手册 v1"
  - "Sharker heavy-task teaching outline"
...
# Sharker heavy-task / Terminal-Bench 主线 System Design v1

这份文档不是教学手册的压缩版，而是从教学手册反推出来的一版系统设计文档。

教学手册回答的是：

> 新人应该怎样一步一步理解 issue #102 后的 Sharker heavy-task 主线？

系统设计文档回答的是：

> 如果我们要把 heavy-task 作为 Sharker 长期 benchmark task-run 能力，哪些边界、状态、事件、fallback、trace 解释和 rollout 纪律必须固定下来？

本文的中心设计判断是：

> Sharker heavy-task 是一个薄的、公开的、持久的 benchmark 工程师循环。它把 inventory、todo、runnable artifact、public check、semantic self-check 和 compact evidence 变成可持久、可投影、可导出的路标，但不改变 official verifier/scorer 的外部权威。

换句话说，这不是一个 benchmark hint 系统，也不是一个 live proof-chain 框架。

它是 Sharker 现有 headless task-run 主线上的一层 discipline：
...
本文面向 Sharker 新人开发者。读者不需要先记住 issue #102 的所有 PR，但需要知道 Sharker 里大概有这些概念：
...
这个答案不够。Sharker/Terminal-Bench 的 trace 显示，更真实的问题包括：
...
heavy-task 不是一条新 runtime。它挂在 Sharker 现有 headless task-run 主线上。
...
> heavy-task 只能增强 task-run discipline，不能重写 Sharker model/tool stepping engine，也不能绕开 official verifier/scorer。
...
模型 loop 结束后，Sharker 进入 official evaluation path。
...
当前 sequence 仍然是正确底座，因为它给 Sharker 补上了缺失的状态基座：
...
```text
Sharker should improve phase transition and retry anchors.
...
```text
When finalization.eligible is true, should Sharker run a bounded final turn?
```
...
sharker-p1d-sqlite-gcov-agent-owned-selfcheck-v4pro-20260623b / sqlite-with-gcov__d4sAyT9

MIPS thin 2h/160 self-check trace:
sharker-mips-glm52-thin-selfcheck-2h-160steps-20260624a / make-mips-interpreter__nEaN77x
