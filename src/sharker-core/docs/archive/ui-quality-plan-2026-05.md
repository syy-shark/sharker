...
# Sharker UI Quality Plan (one-month delivery contract)
...
@kenji's `notes/sharker-one-month-completion-test-plan.md` (cross-cutting
...
- Renderer-only IPC helpers (`window.sharker.*`)
...
| 4 | **Fixture scenario** | At least one `SHARKER_VISUAL_SMOKE_FIXTURE=...` scenario seeds the surface with realistic data | `apps/desktop/src/main/visual-smoke-fixture.ts` |
...
| 10 | **Motion contract** | Animations respect `prefers-reduced-motion`; durations from token catalog | `packages/ui/src/sharker-tokens.css` + component CSS |
...
- Backend logic (lives in @sharker/runtime / @sharker/storage workspaces)
- Network calls (mocked in pure helpers when needed; live calls live in
  @sharker/runtime tests)
...
- Every component MUST use stable spacing that matches its local
  information density; Sharker does not expose a global UI density toggle.
- Every component MUST render correctly under light + dark theme via
  `.dark` class.
- Tokens MUST come from `sharker-tokens.css`; **no hardcoded color** in
...
- Errors go through `generalizedErrorMessage()` from `@sharker/core` (PR58,
  PR74).
...
- Renderer NEVER inspects credential storage directly. Provider secret presence
  goes through `window.sharker.connections.hasSecret(slug)`; subscription OAuth
...
- Backend `redactSecrets` from `@sharker/core` is **authoritative**.
...
Rows are **all UI surfaces** in Sharker. Columns are §1 gates 1–12.
...
boots Sharker under each `SHARKER_VISUAL_SMOKE_FIXTURE=...` scenario, applies
...
`npm --workspace @sharker/desktop run screenshots:diff:stable` exits 1 on
...
- Viewport misconfiguration: `SHARKER_VISUAL_SMOKE_WIDTH/HEIGHT` not
...
**What.** Add `SHARKER_VISUAL_SMOKE_REDUCED_MOTION=1` env var that forces
...
| Theme tokens (`sharker-tokens.css`) | @yuejing | — |
| @sharker/ui package exports | @yuejing | @xuan (build chain) |
