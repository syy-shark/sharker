...
This note captures the reference's Settings module so future Sharker PRs can
...
reference than in Sharker. Concrete numbers from a second RE pass:
...
**Sharker delta found 2026-06-23**:
...
> `npm --workspace @sharker/desktop run screenshots:chat-chrome:check`.
...
- Deeplink scheme: external triggers can deep-link to a specific tab:
  `qoder-work://settings/preferences`. Sharker's equivalent could be
  `sharker://settings/<tab>`.
...
Sharker today has 17 nav items in Settings (some of them — `网络`,
...
### Proposed consolidated Sharker Settings nav (target: ~9 items)
...
- Update SETTINGS_NAV + section render switch + all deeplinks (`sharker://settings/...`)
...
- Add `.sharker-settings-row`, `.sharker-settings-page` primitives in styles.css
