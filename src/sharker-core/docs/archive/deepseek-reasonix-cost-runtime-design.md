---
title: Sharker DeepSeek-Reasonix 成本运行线 System Design
document: DEEPSEEK_REASONIX_COST_RUNTIME_SYSTEM_DESIGN_V2
version: 2.0
status: design draft with rationale
date: 2026-06-24
audience: "准备实现或评审 Sharker cost-runtime 方向的新工程师"
derived_from:
  - "Sharker Runtime 主线教学手册 v2"
  - "Sharker DeepSeek-Reasonix 成本运行线初学者教纲 v3"
---
<!--
...
# Sharker DeepSeek-Reasonix 成本运行线 System Design v2
...
系统设计文档要回答的是：“如果我们要把这条线作为 Sharker 的长期 runtime 设计，哪些边界、状态、数据结构、fallback 和取舍必须被固定下来？”

本文的中心设计判断是：

> DeepSeek-Reasonix 成本运行线不应该新建一条 agent flow。它应该沿着 Sharker 已有的 `SessionManager -> RuntimeKernel -> AgentRun -> RuntimeRunner -> AiSdkFlow -> AiSdkBackend -> ModelAdapter/ToolRuntime` 主线，把 provider-visible request 的成本纪律安装到现有边界上。
...
这个答案只对了一半。DeepSeek 这类 provider 的成本不能只看 total input。Sharker 已经观察过反直觉案例：history budget 让 total input 下降，但 cache miss 和估算成本反而上升。
...
因为很多设计不是为了本轮能跑，而是为了长期能解释、能回退、能迁移、能做 checkpoint、能比较 paid matrix。一个只追求“这轮少传点 token”的实现，短期看更直接；但 Sharker 要做的是 agent runtime，不是一次性 prompt 工具。它必须在成本、正确性、证据、可恢复、工具权限、UI 投影之间同时站住。
...
- output tokens
- reasoning tokens
- miss counter 是 provider 明确给出的，还是 Sharker 推导出来的
- durable prefix 是否变化
...
这些解释不等价于 provider 内部 cache key。Sharker 不知道 DeepSeek 的真实 cache key。Sharker 做的是本地 request-shape 诊断。
...
DeepSeek 成本线涉及 provider request、history replay、tools、usage、telemetry，看起来像可以新建一个 `CostRuntime`。但 Sharker 已经有完整模型/工具循环和 runtime ledger 迁移路径。
...
`AiSdkFlow` 是迁移桥，不是新 engine。它存在的原因是 Sharker 正在从 legacy `SessionEvent` 迁向 canonical `RuntimeEvent`。如果把成本 engine 放在这里，它就会同时承担迁移 adapter 和 provider runtime 两种责任。
...
- 能把 provider 明确给出的 miss 和 Sharker 推导出的 miss 区分开。
...
- `derived` 表示 Sharker 用 input/cache-hit/cache-write 推导。
- 缺失时不得假装精确。
...
注意：这里的分解是 Sharker 的诊断模型，不是 DeepSeek 的内部实现。
...
为什么要引入 projection 这个概念？因为“删历史”把两个动作混在了一起：一个是原始事实保存，另一个是本轮 provider request 视图。Sharker 需要优化第二个，保留第一个。
...
为什么要把 full registry 和 active tools 分开？因为 Sharker 需要本地保留完整执行能力，同时不想每轮都把全部 schema 广告给 provider。schema visibility 是 request shape；dispatch capability 是本地执行能力。二者不是同一件事。
...
Sharker 选择 conservative group activation，是为了在减少 initial schema 成本和保持行为稳定之间取平衡。
...
> Sharker 把成本优化从“压 prompt”升级成“管理 provider-visible request 的事实边界”。
...
最深一层收益是 correctness。Sharker 不因为省 token 就丢掉事实：
...
- hash 只能说明 Sharker 自己送出的 durable prefix 是否变化。
...
Sharker DeepSeek-Reasonix 成本运行线的系统设计，不是“把 prompt 压短”，也不是“为 DeepSeek 新建一个 flow”。
...
守住这些边界，Sharker 才能在不牺牲 correctness 和可恢复性的前提下，继续把 DeepSeek 这类 cache-sensitive provider 的成本问题做深。
...
- DeepSeek-Reasonix cost-runtime Rive reading artifacts from workflow
  `sharker.deepseek-reasonix-cost-mainline`
- DeepSeek-Reasonix beginner teaching guide and fine outline from workflow
  `sharker.deepseek-reasonix-deep-read`
