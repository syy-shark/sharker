...
visual north star for Sharker. Built from the extracted bundle at
...
rule (memory `project_reference_layout_lane`) requires every artifact in Sharker's
...
macOS default is `light-glass` (transparent + native vibrancy). Sharker does **not**
...
Sharker currently has neither `data-font-size` nor `data-font-style` runtime
...
Below is the light-parchment family — the warm-canvas theme that sharker's
...
Sharker's tokens (`sharker-tokens.css`) use `oklch()` — semantically equivalent. The
...
the gap with sharker — sharker's cards have 8-10px radii, 0.04 foreground shadows,
and use `var(--border)` (more like `--color-border-secondary`). Visually sharker's
...
stale extracted CSS reintroduced the gradient into sharker's
`.sharker-shell-2col` and `--agents-layout-bg` tokens, which WAWQAQ
flagged as wrong:

> 谁让你他妈的用渐变的啊？参考实现就没有啊

PR-CHAT-CHROME-FIX-0 (`ed292897`) reverted both sharker tokens to flat
...
Sharker uses `oklch(from var(--foreground) l c h / 0.04)` style alpha shadows
mostly — semantically the same lift, just expressed via oklch. The numeric gap:
sharker's lift cards use 0.10 in the second shadow layer (per `da1a40f1` removal).
...
**Already ported into sharker:**
- `agents-fade-in` → kept as ambient utility
- `agents-slide-up` → `sharker-message-row-enter` (rounds 24/30) and `sharker-hero-enter` (round 29/30)
- `workbench-card-scale-in` → `sharker-tool-card-enter` (round 23/30)
- `chat-input-glow-pulse` → `sharker-composer-glow-pulse`
...
- `industry-guide-marquee` — non-applicable (no Sharker counterpart)
...
className tokens — useful for naming Sharker equivalents.
...
Reference uses `[data-*]` heavily for state. Subset relevant to Sharker:
...
Sharker mostly uses `[data-active="true"]` on list rows. There's no `[data-agents-view]` route attribute — module switching uses local React state, not a documented DOM contract. Adopting `[data-agents-view]` would make CSS targeting easier.

---

## 9. Specific gap analysis: Sharker vs reference

| Surface              | Reference recipe                                            | Sharker current                                                | Gap to close in Phase 2                              |
|----------------------|-------------------------------------------------------------|-------------------------------------------------------------|------------------------------------------------------|
...
- Convert from Base UI Dialog overlay to an inline `.sharker-module-main` view.
- Header: `.sharker-page-heading` (30px display) + lede paragraph + narrow variant
...
- Add `.sharker-card-base` mixin via custom property: a single rule that defines
  the recipe (1px border-tertiary, 6px radius, 4% lift). Compose into
  `.sharker-skill-library`, `.sharker-scheduled-task-shell`, future settings page.
- Add `.sharker-page-heading` typography class (30px / 1.2 line / weight 600).
- Optionally add `[data-sharker-view=plan|skills|settings|...]` on a wrapper to
...
half CSS and half native window-level treatment. Sharker has no quickpick-equivalent.
...
Sharker mostly uses regular className prefixes (`.sharker-*`). Adopting
...
- **Settings page surfaces:** `settings_tab_view` / `settings_tab` /
  ...
  Settings has *tabs* (not the single rail Sharker has), with
...
- **Sidebar:** `sidebar_chat_group_set` / `sidebar_chat_group_delete` /
  ...
  chats, IM channels, cron). Sharker's sidebar is flat.

Sharker has none of the group-nesting in its sidebar. The reference layout
keeps an entire IM-hub and a workspace-group structure that Sharker would
...
Sharker's chat-markdown-renderer to wrap each delta in a span; skipping
...
Sharker's `.settingsHeader h1` is currently 30/600 after Phase 2B — already
matches. The `-narrow` responsive variant doesn't exist in Sharker; could
...
- b) Apply page-card recipe to chat composer card outer chrome
     (`.composer .sharker-composer-inner` → 6px radius + hairline border +
...
sharker doesn't have). In `color-mix` browsers it gets 90% of that token
...
infinite. Sharker has the same glow ported as `sharker-composer-glow-pulse`
(2 layers via `::before` and `::after`); the wing-direction
alternation is the magic that makes the composer feel "alive" rather
than "blinking". Verifying sharker's port — it uses the SAME pattern
...
Sharker doesn't have a parchment-specific composer palette — the
...
Sharker has no equivalent banner mascots. They are a meaningful visual
...
illustration assets the sharker team would need to commission.
...
Sharker could adopt: any third-party brand motion (e.g., the loader.gif
...
(e.g., assistant Markdown body). Sharker mostly defaults to selectable;
the reference flips the default to "non-selectable" globally
(probably for the chrome) and opts in with `.allow-text-selection`.
This is the reverse policy from Sharker but probably worth aligning on
...
page card" pattern — exactly what sharker should adopt when it does the
...
**Sharker uses `var(--accent)` everywhere**, including Settings. Adopting
...
Settings modal palette. Sharker's Settings nav has its own
...
Sharker uses Base UI primitives too, but our CSS file has hand-rolled
...
`user-select: none` — the opposite of Sharker, which inherits browser
default (text everywhere). This explains why reference chat looks
"more app-like" while Sharker can accidentally enter text selection
mode on chrome clicks.

**Phase 3 candidate** (atlas §13.7): apply `user-select: none` to
`.sharker-session-panel`, `.sharker-nav-row`, `.sharker-list-row`,
`.sharker-module-main-header` headings (chrome surfaces only), and
...
- **Reference uses `cmd+k` global** to focus the chat input (`focus-input`
  / `agent-input-history` in JS chunk strings); Sharker has `cmd+k` but
...
- **Reference has `[data-canvas-dialog]`** that overrides the standard
  ...
  preview / artifact dialog variant. Sharker doesn't have a canvas-style
...
typography via a specific variable font with bespoke axes. Sharker uses
...
creates a "content fades into chrome" effect. Sharker has no equivalent;
...
subsequent). Sharker uses Geist Variable from `@fontsource-variable/geist`
...
Sharker's `data-os` is set on `<html>` after Phase 2C (atlas §12.1).
...
13. `[data-glass-effects]` opt-out toggle — small Settings JSX + CSS
    pattern gating
14. Login surface (Sharker has no auth; future surface if we add it)
