...
> 目标：在 sharker-agent 中实现"专家团 / 数字同事"功能。
> 本文基于对 **WorkBuddy**（腾讯 CodeBuddy 换皮）与 **QoderWork**（阿里 Qoder）两款已安装应用的逆向，
> 加上对 sharker-agent 自身架构的映射，给出可落地的设计。
> 逆向原始 spec：`scratchpad/RE-workbuddy-experts.md`、`scratchpad/RE-qoderwork-agents.md`、`scratchpad/sharker-architecture-map.md`。
...
## 2. sharker-agent 现有可复用底座（三块几乎现成）
...
**关键判断：**
1. sharker 的 `agent-catalog.ts`（每个 `AgentDefinition` 带 `id/name/description/systemPrompt/tools/permissionMode`）
   与 WorkBuddy 的"专家 = 带人格的 subagent"**结构同构**。→ 单专家直接落在这里。
2. sharker 的 subagent runtime（`spawnChildAgent` + `agent_spawn` 工具，父子 AgentRun）
   与 WorkBuddy team 的"lead 通过 Agent 工具派生 member"**结构同构**。→ 专家团直接落在这里。
3. sharker 的技能市场（远程/内置目录 + `contentSha256` + install/lock/update）
...
- 目录：内置 `bundled-expert-catalog.generated.ts`（复用 memory 里已逆向的 13 skills 那套种子流程）；
  managed `~/.sharker/experts/<id>/EXPERT.md`；后续加 `sourceType:'remote'` 拉远程 manifest（COS 那套）。
...
plugin 市场，数字同事 = 雇佣专家的持久化助理外壳"。这套模型和 sharker-agent 的
