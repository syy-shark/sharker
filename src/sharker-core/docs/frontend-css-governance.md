...
Sharker's frontend styling combines Astryx, `@sharker/ui` product compositions, and renderer surface CSS. Cascade order is an explicit contract rather than an implementation detail.
...
- Historical recipes at the end of `sharker-tokens.css` and `reference-shell.css` are transitional exceptions. Do not add new surface rules to them.

### Selector naming

- Shared renderer and `@sharker/ui` selectors use the kebab-case `.sharker-*` dialect.
...
Astryx reset and component layers come first; Sharker base tokens and product `components` come later. Keep layer ownership at the closest existing seam instead of adding a higher-priority compatibility layer.

## 4. `!important`

- `!important` is allowed by default only for accessibility helpers such as `.sharker-visually-hidden`, and for reduced-motion or e2e-fixture overrides.
...
- Shared custom properties belong in `apps/desktop/src/renderer/sharker-tokens.css`.
...
2. Keep generic component chrome in Astryx and product composition in `@sharker/ui` or the matching renderer surface.
