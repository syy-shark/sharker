...
Sharker Desktop、TUI 和 CLI 可以通过 TLS、SSH 或明确启用的明文 WebSocket 连接 Runtime Host。CLI 和 TUI 还支持下文所述的实验性 direct peer transport。
...
`--principal` 应使用稳定标识；重复执行会替换该 Client 的 credential，不会不断累积 credential。命令会把当前精确版本的 Sharker 安装到托管目录，启动仅监听 loopback 的服务，验证新 credential，然后只显示一次连接信息。TUI 或 CLI 使用 `terminal-client`。

在 Host 上运行 `npx --yes --package sharker-agent@latest sharker runtime-host service uninstall` 会删除 service 与托管 package，但保留 State Root 和 Project 数据。

## 手动设置 Host

在远程机器构建 Sharker，选择持久的 State Root，并注册允许 remote Client 使用的 Project：
...
只要提供了 `--project-root <label>=<absolute-path>`，远程目录浏览就只会显示这些根目录。该参数最多可重复八次。Sharker 会在启动时解析每个根目录，并确保浏览和注册始终限制在当前选择的根目录内。
...
安装命令会持久保存当前精确的 Node 与 Sharker CLI 路径。重复执行会更新同一个 OS-managed
service；未指定 WebSocket port 时会保留现有端口。卸载 npm 包前，应先执行
`sharker runtime-host service uninstall`。卸载 service 会保留 State Root 与 Project 数据。Linux
要求启用 systemd user lingering，macOS 要求存在活跃的 GUI 登录会话；条件不满足时安装会给出
可操作的错误，不会声称服务能够持久运行。Service 必须从持久的全局 Sharker 安装中安装，不能使用
...
`SHARKER_RUNTIME_HOST_ACCESS_CREDENTIAL` 提供 setup 创建的 credential。Disable 后重新 enable 会保留
...
连接，也可能拒绝或中止 reservation。只有已接受的 reservation 才会向 Mesh Peer 发布，Sharker 仍要求
...
Sharker 不经过 shell，直接运行系统 `ssh`，把 Client 的临时 loopback port 转发到 Host 的 loopback listener。正常的 OpenSSH alias、key、agent 与 host verification 仍然生效；配置了额外 port forwarding 的 Host 条目会被拒绝。Sharker 不会修改 SSH config，也不会在删除 Profile 时清理共享的 OpenSSH 状态。
...
Client Profile 还必须单独持久化明文风险确认。Sharker 不会把 TLS 或 SSH 自动降级为明文。复制 service 命令输出的 JSON `rootId`；Client 会用它固定预期的 State Root。
...
如需从另一台 Desktop 访问当前电脑，可在同一设置页开启**远程访问**。Sharker 会保留现有
...
sharker run --host office --project '<projectId>' "总结这个项目"
```
...
- Runtime Host protocol operation 不能升级、重启或终止 service process；Desktop 管理使用独立认证的
  ...
  supervisor 会将其重新拉起。
- 在 Host 上使用 `sharker runtime-host access revoke --root /srv/sharker --credential <credentialId>` 撤销 credential。
