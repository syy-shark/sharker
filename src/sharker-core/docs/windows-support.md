...
5. Launch Sharker, configure a model under **Settings → Models**, and install `ripgrep` with
   `winget install BurntSushi.ripgrep.MSVC` if Runtime's `Grep` tool is needed. Restart Sharker after
   changing `PATH`.
...
recovery, if ever needed: close Sharker, delete the
...
To uninstall, use **Settings → Apps → Installed apps → Sharker → Uninstall**. Back up any important
workspace data first; the preview does not yet claim business-data migration guarantees.

## 安装 Windows x64 预览版

只使用 Sharker GitHub Release 附带的 Windows 资产。NSIS 安装包名为
`Sharker-<version>-win-x64.exe`；ZIP 主要用于便携检查和问题排查。
...
4. 运行安装包。预览版尚无 Authenticode 签名，SmartScreen 会提示未知发布者；只有完成第 3 步后，
   才应选择 **更多信息 → 仍要运行**。
5. 启动 Sharker，在 **设置 → 模型**中配置模型。需要 Runtime `Grep` 工具时，执行
   `winget install BurntSushi.ripgrep.MSVC`，并在 `PATH` 更新后重启 Sharker。
...
接管该备份并完成升级——这次重跑就是受支持的恢复方式，门禁对其有断言。如需手工恢复：关闭 Sharker、删除
...
卸载入口为 **设置 → 应用 → 已安装的应用 → Sharker → 卸载**。预览版尚未承诺业务数据迁移，
请先备份重要 workspace 数据。
...
so `syncDirectory()` is a no-op. Sharker therefore does not currently promise POSIX-equivalent
