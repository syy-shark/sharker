...
The primary model path is the provider-neutral `sharker_computer` function tool
...
Sharker keeps the same separation:

- `sharker_computer` is the primary model-facing path;
...
- provider adapters use the same `sharker_computer` contract rather than a
...
getAIModel -> streaming tool_use -> AiSdkBackend -> ToolRuntime
  -> sharker_computer -> list_apps -> observe -> set_value -> final response
```
...
The core `sharker_computer` parser remains strict and provider neutral.
