...
Sharker keeps skill bodies out of the always-on system prompt. The prompt contains
...
copy has a scope-aware `ref` (for example `project:sharker:writer` or
...
Runtime preferences use `.sharker/skills-state.json` schema v2:
...
Removing an entry from `BUNDLED_SKILL_CATALOG` stops Sharker from distributing or
