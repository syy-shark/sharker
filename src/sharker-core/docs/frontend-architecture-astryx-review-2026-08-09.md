...
```
Astryx primitives/theme  →  @sharker/ui compositions  →  desktop host (shell, settings, workbar)
```
...
└── appFrame (app-shell.tsx ~3.1k lines)
    ...
    ├── Astryx AppShell
    │   ├── SideNav → SessionListPanel (@sharker/ui)
    │   └── content
    │       ├── Module routes → ModulePage (@sharker/ui → Astryx Layout)
    │       ├── ChatSurfaceLayout (@sharker/ui → Astryx ChatLayout + conversationKey patch)
...
| Chat page shell | `@sharker/ui` `ChatSurfaceLayout` | scroll / dock / follow |
| Transcript product | `@sharker/ui` chat-turn, tool-activity | turns, tools, heroes |
| Settings | desktop `settings/*` kit | modal IA + rows |
| Modules | `ModulePage` + desktop MCP | dense list + inspector |
| Substrate | Astryx + `sharker-tokens.css` + generated theme | primitives / tokens |
...
**Anchors:** `settings-surface.tsx` (section + localStorage + `sharker:jumpToSettingsSection` + parent intents); `ProvidersPanel` nested catalog/setup/detail; five openers on the host.
...
**Anchors:** `packages/ui/src/tool-activity.tsx` + `tool-activity/**`; `previewVariants` / `ToolOutputSurface` in `primitives/chat.tsx`; ~254 `.sharker-*` rules in `packages/ui/src/styles.css`; renderer `chat-message.css` still reaches into tool cards.
...
**Direction:** Freeze new preview cards; pure `toolName → PreviewKind` map; generic mono/error → Astryx; structured multi-part results only as product kinds; ban new renderer selectors on `.sharker-tool-*` (move overrides into package/theme).

### A7 — CSS dialect sprawl (high)

**Anchors:** three eras on one node — e.g. module mains `sharker-main detailPane sharker-module-main agents-chat-panel` (`module-pages.tsx`); settings camelCase under `styles/settings/**`; package vs renderer dual ownership.
...
**Anchors:** Skills/Plan/Daily in `@sharker/ui` `module-pages.tsx`; MCP in desktop `mcp-page.tsx` with its own skeleton dialect.
...
|------|----------------|-------------|
| `styles/chat-header.css` | `min-height: 40px` | `.sharker-session-context__inner` band |
...
| Recipe | Example |
|--------|---------|
| `sharker-lazy-fallback` + Spinner | overlays, workbar suspense, modules |
| `WorkbarPanelLoading` | `session-workbar.tsx` |
| `SettingsSkeletonStack` | settings pages |
| `sharker-module-list-skeleton` | `mcp-page.tsx` |
