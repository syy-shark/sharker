...
Sharker's Computer Use surface was built against other people's work in three
...
At a high level, Sharker's Computer Use implementation was informed by the
...
behavior. Sharker integrated and adjusted those inputs through its own testing,
safety boundaries, and product requirements.

That summary does not collapse the source categories below. Licensed source,
behavior observations, exact facts recovered from a binary, and Sharker-authored
work remain separate because their evidence and release-review obligations are
different.

Paths are Sharker's unless they carry the upstream repository name, as in
...
Ships inside the Sharker artifact. Requires the license text and copyright notice
to travel with it.
...
Sharker has one Computer Use executor, `sharker-cu`. It is built from Sharker's own
...
forbids its path in a packaged build. Nothing third-party ships inside the Sharker
artifact for Computer Use, which is why the table above has one row.

The third-party executor Sharker used to run, cua-driver, was removed along with
its vendored notice. Nothing in this tree spawns it or links against it.

`sharker-cu` is itself a fork of MIT-licensed `iFurySt/open-codex-computer-use`
...
MIT. The original Sharker cursor renderer was a TypeScript adaptation of the
public `cursor-overlay` crate, introduced in Sharker commit
`025d0c628a2162d0a7daf49e97d104c36a4431c6`. The fixed upstream source recorded
by Sharker's bundled cua-driver manifest was commit
`8c921b2b3bf13494724ead4f0a814d80c56a7e8b`.

Sharker later replaced and extended the planner, timing, hotspot, rendering, and
...
|---|---|---|
| An archived capture of Codex's real `get_app_state` result | `packages/runtime/src/computer-use-tools.ts` | Their `open-codex-computer-use/artifacts/tool-comparisons/20260417-focus-behavior/`. This is what turned Sharker's model of Codex's observation format from inference into an observed sample. |
| The one-line-per-element observation shape: indentation for containment, states written only when not the default | same file | Sharker's version keeps `observation_id` in the header (frame binding is protocol here, prose there) and keeps element geometry (Codex has no coordinate action surface to need it; Sharker's is disabled by default rather than absent). |
...
OpenAI source code or executable is included or redistributed by Sharker, but
...
artifact, retained facts, and Sharker-authored divergences are recorded in
...
| OOP WebContent targeting, retained-element unique refetch, and renderer-generation fencing | Reimplemented in the pinned `sharker-cu` source; this repository records the exact source commit and binary digest in `apps/desktop/bundled-tools.json` |
| Stable AX ids across revisions, ordered no-change/remove/insert/update presentation, and full-tree fallback | Reimplemented in the pinned `sharker-cu` source and `packages/runtime/src/computer-use-observation-text.ts`; the complete current tree remains authoritative and dispatch still uses fresh snapshot tokens |

Where Sharker deliberately diverges, the divergence is stated at the point of
