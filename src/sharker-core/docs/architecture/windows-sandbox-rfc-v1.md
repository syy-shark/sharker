...
- Status: implementation baseline selected; first preview slice ([#2961](https://github.com/sharker-agent/sharker-agent/pull/2961)) merged 2026-08-17; product integration continuing under release validation (preview scope in §6.5)
- Tracking: Windows Phase 4 in [issue #2142](https://github.com/sharker-agent/sharker-agent/issues/2142)
- Updated: 2026-08-18
- Owners: `@sharker/runtime` sandbox boundary and Runtime Host execution composition
...
W0 selected a Sharker-owned Rust implementation rather than importing another product's setup and
...
| Source | Reviewed evidence | What Sharker takes from it | What Sharker does not assume |
| --- | --- | --- | --- |
...
| Gemini CLI `1ac337739586` | `WindowsSandboxManager.ts`, `GeminiSandbox.cs`, sandbox docs | Environment scrubbing, restricted-token launch, suspended Job assignment, explicit documentation of persistent low-integrity labels | Its network throttle or best-effort ACL behavior is sufficient for Sharker |
...
| OpenCode `cc4b45612974` | official Windows documentation recommends WSL | WSL is a viable explicit external environment | WSL provides Sharker's native Windows backend |

Codex, Gemini, and Chromium materially informed the layered broker, Job, ACL recovery, and
fail-closed contracts. Sharker did not copy their product protocols. Executable evidence overruled the
...
Sharker packages a small native Rust broker/client. Runtime Host compiles `PermissionProfile` into a
...
- Sharker's sandbox setup records, ACL ownership ledger, executable, and broker protocol.

The Windows kernel, signed Sharker binaries, Runtime Host, and the parent user session are trusted. The
boundary does not defend against an administrator, kernel compromise, or an already-compromised
same-user process outside Sharker. Sandboxed code is treated as malicious after its first instruction.
...
- Sharker-owned ACL changes are attributed to a unique per-launch principal, record their actual
...
- Future domain allowlists must use a Sharker-owned proxy; they must not compile DNS answers into a
...
- The readiness probe's throwaway profile lifecycle is isolated and fail-closed. _(Implemented: the
  probe profile lives under a dedicated `sharker.readiness.` namespace that is structurally disjoint
  from the production `sharker.sandbox.` namespace, and its reserved `requestId` is rejected by launch
...
[#2961](https://github.com/sharker-agent/sharker-agent/pull/2961), merged 2026-08-17 — enforces a subset
...
[#2142](https://github.com/sharker-agent/sharker-agent/issues/2142).
...
The first implementation needs no elevated setup. Windows creates a request-derived Sharker
...
| --- | --- | --- |
| Dedicated sandbox identities + restricted token + Job + private desktop + ACL ledger + WFP/firewall | Codex demonstrates this agent-oriented shape, including setup and adversarial tests | Reference for future stronger tiers; runner evidence showed the Sharker candidate could not reliably initialize real children |
| AppContainer + atomic Job + one-shot broker + ACL ledger | Microsoft and Chromium document the primitives; Sharker Windows 2025 CI proves the composed boundary | Selected for the native backend |
...
| Low integrity ACL + Job | Gemini implements this lightweight path | Rejected for Sharker's strong tier: persistent labels, best-effort ACL failures, and network throttling do not meet fail-closed policy |
...
- [x] build a Sharker-owned Rust launcher with reproducible MSVC CI;
