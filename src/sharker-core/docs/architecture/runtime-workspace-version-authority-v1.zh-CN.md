...
> 哪一个 Git commit/tree 已被 Sharker 正式接受为某个 workspace epoch 的初始版本？
...
| 项目 | 决策 |
|---|---|
| 协议 owner | `@sharker/core` 的 strict fact contract 与 pure scanner |
...
```text
sessionId     = sharker_workspace_authority
...
```ts
{
  kind: 'sharker.workspace.baseline_accepted',
  version: 1,
  payload: {
...
event，因此 caller 没有机会夹带另一条 semantic lane。该 seam 所在模块不从 `@sharker/storage` 导出；
...
- `.sharker-workspace.json` 在 managed worktree 的 identity/exclude 策略；
