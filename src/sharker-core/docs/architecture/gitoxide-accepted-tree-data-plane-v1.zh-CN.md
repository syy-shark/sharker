...
本切片只拥有 stage，不拥有 promote；任何 candidate 路径都不得推进 `refs/sharker/accepted` 或签发新的
accepted capability。
...
- import authority 在任何 destination side effect 前验证 helper 的 operation feature attestation；导入成功
  后只在固定 `refs/sharker/accepted` namespace 捕获 helper capability、managed repository path、accepted
...
accepted ref 在本切片中只读，并与 `refs/sharker/candidates/<operation-hash>` namespace 不重叠。第二次 accepted
