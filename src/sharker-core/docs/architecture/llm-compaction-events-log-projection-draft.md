...
title: "Chapter 3: Compaction Is a Projection—How Sharker Lets the LLM Forget Without Losing History"
language: en
...
# Chapter 3: Compaction Is a Projection—How Sharker Lets the LLM Forget Without Losing History

> This chapter answers one question: when the complete Agent history no longer fits in the model context, how can Sharker reduce what the LLM sees without damaging the fact space needed for replay, audit, and future projections? The answer is not “replace the log with a summary.” It is: **define compaction as a lossy projection of the Runtime Events Log. The log preserves facts, a checkpoint preserves a continuation view with an explicit coverage boundary, and each provider request consumes only the projection appropriate at that moment.**
...
Imagine that a user has worked with Sharker for two hours:
...
6. They asked Sharker to continue with the next failure.
...
Sharker's problem is therefore not:
...
Sharker's core relationship can be written as:
...
Sharker therefore persists more than a string. It persists a `HistoryCompactCheckpoint`:
...
The prefix constraint gives Sharker three properties:
...
If any check fails, the checkpoint cannot replace the current source prefix. Sharker reports `coverage_miss` or `source_hash_mismatch`; it does not accept a projection merely because it resembles the current history.
...
```text
<sharker_history_compact_checkpoint ...>
  summary: ...
  coverage: ...
  limitations: ...
</sharker_history_compact_checkpoint>
...
The model may temporarily see fewer old details, but the source log remains intact. Sharker can compact again or build a new projection under another policy later.
...
Sharker has one LLM compaction mechanism and one adjacent current-request rewrite:
...
First, storage does not immediately shrink when the prompt becomes shorter. Sharker keeps the source log and targets inference context rather than fact storage for savings.
...
Fourth, V2 checkpoints do not currently record a complete summarizer model identity, prompt version, or request-shape hash. They are sufficient to replay an accepted projection safely, but not to promise deterministic regeneration. If Sharker later needs compactor-version comparisons, offline regressions, or explanation of summary drift, those identities should enter an explicitly versioned projection manifest.
...
Sharker's LLM compaction is not a destructive rewrite of a conversation table. It is a projection chain rooted in the canonical Runtime Events Log:
