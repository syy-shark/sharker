...
`sharker-gitoxide-helper` 每次启动只执行以下协议：
...
| 项目 | v1 合同 |
| --- | --- |
| operation owner | 单次 `sharker-gitoxide-helper` 子进程 |
...
Cargo 编译 `sha256` feature 只用于识别并给出稳定拒绝，不代表 Sharker 已支持 SHA-256 repository。
