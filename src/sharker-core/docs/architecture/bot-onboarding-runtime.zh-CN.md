...
# Sharker IM 扫码接入 runtime architecture

状态：V1 implemented and verified（2026-07-18）

## 1. 目标与边界

Sharker 的 IM 接入同时支持两条路径：

- 快捷接入：用户扫描平台二维码，平台完成应用或机器人注册，Sharker 自动取得 credential、启用 channel 并启动 runtime bridge。
...
- `@sharker/core/bot-onboarding`：provider、brand、state 和 renderer-safe snapshot contract；不包含 I/O。
...
- `@sharker/runtime/bots`：credential 生效后的长连接、消息映射、allowlist 和发送能力。
...
`FeishuBotBridge` 使用 official Channel API 的 WebSocket transport。handshake timeout 为 15 秒。消息先由 SDK 标准化，再映射为 Sharker `BotIncomingMessage`；raw payload 不跨 runtime boundary。
