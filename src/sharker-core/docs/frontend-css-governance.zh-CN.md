...
本仓库的前端样式体系由 Astryx、`@sharker/ui` 产品组合样式和 renderer surface CSS 组成。级联顺序必须被明确约束，不能随意改动。
...
- `sharker-tokens.css` 尾部的历史 recipe 和 `reference-shell.css` 是待收敛的 transitional exceptions；不要继续向这两个例外增加 surface 规则。

### Selector 命名

- renderer 与 `@sharker/ui` 的共享 selector 使用 kebab-case `.sharker-*` 方言。
...
Astryx reset 和组件层在前，Sharker base token 与产品 `components` 在后。应在最近的现有职责缝隙解决覆盖，不再增加更高优先级的兼容层。

## 4. `!important` 使用规则

- 默认只允许两类场景使用 `!important`：
  - 无障碍辅助规则，例如 `.sharker-visually-hidden`
...
- 自定义 CSS 变量统一放在：
  - `apps/desktop/src/renderer/sharker-tokens.css`
- 只有组件局部变量允许例外，但必须带：
  - `/* local: ... */`
- 禁止新增以下硬编码值：
  - 颜色
...
2. 通用组件外观留给 Astryx，产品组合样式放在 `@sharker/ui` 或对应 renderer surface。
3. 清理 dead selector。
