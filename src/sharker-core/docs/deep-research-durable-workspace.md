...
[issue #566](https://github.com/sharker-agent/sharker-agent/issues/566).
...
repository. The Sharker implementation is built independently on Sharker's existing
...
3. root-session gating for Sharker-owned write tools; and
4. the explicit transition from a read-only research session to a new normal
   implementation task.

## Paper-to-Sharker mapping

The paper describes a persistent workspace shared by a Context Builder and a
Report Writer. Sharker adopts the systems boundary, not the reference
implementation:

| FS-Researcher idea | Sharker reproduction | Deliberate difference |
| --- | --- | --- |
| File system as external memory | JSONL event ledger plus Artifact Store | Sharker reuses app-owned storage rather than exposing arbitrary project paths |
...
narrow exception for writes into Sharker-owned state only. They do not expose a
...
- [ ] Tool writes are limited to Sharker-owned ledger and artifact state.
...
npm --workspace @sharker/storage test
