...
What Sharker's Computer Use mirror copies from Codex, where each fact came from,
and the four places Sharker deliberately does something else. It keeps confirmed
...
## Where Sharker does something else, and why

**Two controls, not three.** Codex's `close` dismisses one tile and `hide`
dismisses the stack. Sharker mirrors one window at a time, so those are the same
...
working on in Picture-in-Picture mode"; Sharker's appears whenever a run starts, so
...
windows* — Codex registers several hosts, including `avatar-overlay`. Sharker has
