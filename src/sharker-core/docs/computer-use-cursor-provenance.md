...
This document records the exact source boundary for Sharker's agent-cursor overlay
and the independent replacement completed for Apache Sharker issue #3293. The
...
https://github.com/apache/sharker/issues/3293#issuecomment-5371901326

The derivation used four Sharker product requirements:
...
rasterized from this geometry with Sharker's existing blue gradient on a
...
rather than a cosmetic preference. Sharker's earlier raw path-length addition and
backwards-arrival penalty remain around this core, as issue #3293 requires.

The independently derived scorer continues to evaluate Sharker's existing
single-segment cubic candidate family. It does not import any candidate
generator from the historical artifact.

As a new Sharker integration choice for this replacement, the nine-candidate
budget is allocated by selection rather than a Cartesian product. This
reconciles the independent count of nine with Sharker's retained five-way
...
Before issue #3293, Sharker retained exact inputs recovered from this artifact:
...
The first Sharker cursor renderer was a TypeScript adaptation of `trycua/cua`'s
MIT-licensed `cursor-overlay`, introduced in Sharker commit
`025d0c628a2162d0a7daf49e97d104c36a4431c6`. The fixed upstream commit recorded
by Sharker was `8c921b2b3bf13494724ead4f0a814d80c56a7e8b`.
...
## Retained Sharker-authored or Sharker-adjusted behavior

Issue #3293 deliberately preserves the surrounding Sharker work:
...
- Sharker's brand palette, click pulse, shadow treatment, and host integration.

The result remains a mixed-lineage Sharker implementation: an MIT-derived
renderer foundation, Sharker-authored product behavior, and a current cursor value
