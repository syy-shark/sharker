...
Written 2026-07-27 for sharker.
...
`/Applications`, choose `Sharker.app`, then come back and tick a checkbox.
Six steps, and the file picker is where most people give up.

sharker today does the honest-but-minimal thing: `permissions-actions.ts`
...
- [`riko2chen/AskForPermission`](https://github.com/riko2chen/AskForPermission)
  — MIT, native Swift/AppKit. Not directly usable (sharker is Electron) but
  its window-location approach is the correct one.
- Alma (closed-source; behaviour observed in the local `~/alma-re`
  study archive). Electron, so its shape maps onto sharker almost directly.
...
- **The drag image should look like the destination row**, not like the
  ...
  row (icon + "Sharker"). What you drag looks like what it becomes, which
...
- **No detection that the user closed System Settings or gave up.** The
  ...
  it is dismissed by hand. sharker should treat "Settings went away" and a
