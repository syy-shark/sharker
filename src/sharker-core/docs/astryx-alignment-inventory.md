...
Sharker product surfaces. Severity: **blocker** (raw control when an Astryx twin
exists / broken hierarchy) · **reimplementation** (a public `@sharker/ui` export
...
| API · Use the System | Public `@sharker/ui` export whose name shadows a shipped Astryx component (defined locally, not a re-export) | Re-export the Astryx component, or confirm the local one is intentional |
