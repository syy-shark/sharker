...
} from '@sharker/runtime-host/profile-kind';
...
import type { TurnOrchestration, SessionListFilter, RegenerateTurnInput } from '@sharker/core/runtime-inputs';
...
} from '@sharker/core/web-search';
import {
  isSessionTrace,
} from '@sharker/core/session-trace';
...
import type { GoalState } from '@sharker/runtime/goal-state';
...
import {
  ...
  type TestProxyInput,
} from '@sharker/core/settings/network-settings';
...
import type { AttachmentRef, InlineReference, QuoteRef } from '@sharker/core/events';
import type { OnboardingMilestoneId } from '@sharker/core/onboarding';
import {
...
function scopedRuntimeHost(scope: DesktopTargetScope): SharkerBridge['runtimeHost'] {
  return {
...
const runtimeHost: SharkerBridge['runtimeHost'] = {
  query(operation, input) {
...
const sharkerBridge = {
  ...
  newTasks: {
    ...
    async listInvocableSkills(
      ...
      context?: {
        ...
        permissionMode?: import('@sharker/core/settings').ChatDefaultPermissionMode;
      },
    ) {
      return ipcRenderer.invoke(
  ...
  sessions: {
    ...
    async send(sessionId, command) {
      ...
      const result = (await ipcRenderer.invoke(
        ...
        session.sessionId,
        encoded,
      )) as Awaited<ReturnType<SharkerBridge['sessions']['send']>>;
      return result.ok
    ...
    async submitMessage(sessionId, placement, command) {
      ...
      )) as Awaited<ReturnType<SharkerBridge['sessions']['submitMessage']>>;
      return result.ok
        ? { ...result, attachments: projectDesktopAttachmentRefs(session.scope, result.attachments) }
        : result;
    },
  ...
  memory: {
    ...
    listVault(host?: DesktopRuntimeHostRef) {
      return invokeSelectedRuntimeHost(host, 'memory:listVault');
    },
    readVaultFile(path: string, host?: DesktopRuntimeHostRef) {
      return invokeSelectedRuntimeHost(host, 'memory:readVaultFile', path);
    },
    writeVaultFile(path: string, content: string, host?: DesktopRuntimeHostRef) {
      return invokeSelectedRuntimeHost(host, 'memory:writeVaultFile', path, content);
    },
    deleteVaultFile(path: string, host?: DesktopRuntimeHostRef) {
      return invokeSelectedRuntimeHost(host, 'memory:deleteVaultFile', path);
    },
  },
  attachments: {
    pickFiles(): Promise<
...
} satisfies SharkerBridge;

// E2E-only async controls. Real users never get these: the preload mirrors the
// main process's isolated-E2E gate (startup-context.ts) — SHARKER_E2E alone is
...
if (process.env.SHARKER_E2E === '1' && process.env.SHARKER_E2E_USER_DATA_DIR) {
  ...
  sharkerBridge.newTasks.listInvocableSkills = wrapLatched(
    sharkerBridge.newTasks.listInvocableSkills.bind(sharkerBridge.newTasks),
    'newTasks.listInvocableSkills',
  );
  sharkerBridge.sessions.list = wrapLatched(
    sharkerBridge.sessions.list.bind(sharkerBridge.sessions),
    'sessions.list',
  );
  const subscribeSessionEvents = sharkerBridge.sessions.subscribeEvents.bind(sharkerBridge.sessions);
  sharkerBridge.sessions.subscribeEvents = (
    sessionId,
    handler,
    onSeeded,
    onObservationSeed,
  ...
  const listInvocableSkills = sharkerBridge.skills.listInvocable.bind(sharkerBridge.skills);
  sharkerBridge.skills.listInvocable = async (...args) => {
    try {
  ...
  contextBridge.exposeInMainWorld('sharkerE2eLatch', {
...
contextBridge.exposeInMainWorld('sharker', sharkerBridge);
