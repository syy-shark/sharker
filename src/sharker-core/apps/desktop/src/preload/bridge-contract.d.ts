...
import type { ConnectionEvent } from '@sharker/core/connections';
import type {
...
} from '@sharker/core/llm-connections';
import type {
  AppIcon,
  AppIconChoice,
  AppIconTarget,
...
import type { BotProvider } from '@sharker/core/bot-chat-settings';
import type { BotOnboardingSnapshot, BotOnboardingStartInput } from '@sharker/core/bot-onboarding';
import type { HealthSnapshot } from '@sharker/core/health';
import type { ExecutionBoundaryReadModel, SandboxBoundaryResponse } from '@sharker/core/sandbox-boundary';
import type {
...
import type { RuntimeHostProfileKind } from '@sharker/runtime-host/profile-kind';
import type { PermissionMode } from '@sharker/core/permission';
import type { CollaborationMode } from '@sharker/core/collaboration';
...
} from '@sharker/core/runtime-inputs';
import type { PlanSessionState } from '@sharker/core/plan';
import type { SearchErrorReason, SearchRequest, SearchResult } from '@sharker/core/search';
import type { SessionChangedEvent, SessionSummary, TurnRecord } from '@sharker/core/session';
import type { ThinkingLevel } from '@sharker/core/model-thinking';
import type { E2eFixtureState } from '@sharker/core/e2e-fixture';
import type {
  GitReviewReadResult,
  GitReviewSource,
} from '@sharker/core/git-review';
import type {
...
} from '@sharker/core/artifacts';
import type { CapabilitySnapshotCollection, PermissionSnapshot } from '@sharker/core/capabilities';
import type { LocalMemoryState } from '@sharker/core/local-memory';
import type {
  AuthorizationUrlPayload,
  SubscriptionActionResult,
} from '@sharker/core/oauth-subscription';
import type { CreateScheduledTaskInput, ScheduledTask, UpdateScheduledTaskInput } from '@sharker/core/scheduled-task';
import type { ProjectRecord } from '@sharker/core/project';
import type {
  DailyReviewArchive,
  ...
  DailyReviewRange,
  DailyReviewSummary,
} from '@sharker/core/daily-review';
import type { WebSearchProvider, WebSearchResponse } from '@sharker/core/web-search';
import type { BrowserState, BrowserViewRect } from '@sharker/core/browser';
import type { Task, TaskLedgerChangedEvent } from '@sharker/core/task-ledger';
import type { DeepResearchChangedEvent, DeepResearchClientProgress } from '@sharker/core/deep-research-run';
import type {
  DesktopTranscriptBatch,
  DesktopTranscriptHandle,
} from './transcript-contract.js';
import type { PetPackManifestV1 } from '@sharker/core/pet';
import type { WorkBoardItem, WorkBoardListQuery, WorkBoardPage } from '@sharker/core/work-board';
import type { WorkBoardMutationOptions } from '@sharker/storage/work-board-store';
import type {
  OperationInput,
  OperationOutcome,
  OperationOutput,
...
import type { AgentGraphEpochDirectory } from '@sharker/runtime-host/client';
import type {
  RuntimeHostServiceManagementFrame,
  RuntimeHostSetupPhase,
} from '@sharker/runtime-host/operator';
import type {
  RendererRuntimeHostCommandOperation,
  RendererRuntimeHostQueryOperation,
} from './runtime-host-renderer-operations.js';
import type { SessionTrace } from '@sharker/core/session-trace';
import type { UsageSummaryV2 } from '@sharker/core/usage-stats/types';
import type { UsageProvenance } from '@sharker/core/usage-ledger-merge';
import type { ContextDiagnosticsResult } from '@sharker/runtime-host/protocol';
import type { TestProxyInput } from '@sharker/core/settings/network-settings';
...
import type { Result } from '@sharker/core/result';
import type { CreateSessionRequestInput } from '@sharker/core/runtime-inputs';
import type {
  McpConfigAddResult,
  ...
  McpServerStatus,
  McpTestResult,
} from '@sharker/core/mcp';
import type {
  AgentGraphClientSnapshot,
  AgentGraphClientSnapshotOptions,
  AgentGraphOperatorInspection,
} from '@sharker/runtime/stream-graph-read-model';
import type { BotStatus, WechatBridgeQrCodeResult } from '@sharker/runtime/bots';
import type { ShellRunPtyDataEvent, ShellRunPtySnapshot } from '@sharker/runtime/shell-run-contract';
import type { BundledSkillCatalogEntry, ManagedSkillSourceEntry, ManagedSkillUpdatePreview, SkillEntry } from '@sharker/ui';
import type { ConfigCategory } from '@sharker/storage/config-transfer';
import type { OnboardingMilestone, OnboardingMilestoneId, OnboardingState } from '@sharker/core/onboarding';
import type {
...
export interface OnboardingSnapshot {
  ...
  connections: import('@sharker/core/llm-connections').IdentifiedLlmConnection[];
  defaultSlug: string | null;
  chatModelChoices: import('@sharker/core/chat-model-choice').ChatModelChoice[];
  sessionSendOutcomes: Record<string, import('@sharker/core/session-send-projection').SessionSendProjection>;
}
...
export interface DesktopRuntimeHostManagementProgress {
  ...
  readonly phase:
    | 'preparing_cli'
    | import('@sharker/runtime-host/operator').RuntimeHostServiceUpdatePhase;
}
...
export type DesktopRuntimeHostPeerMeshAction =
  import('@sharker/runtime-host/operator').RuntimeHostPeerMeshManagementAction;

export type DesktopRuntimeHostPeerMeshResult =
  | import('@sharker/runtime-host/protocol').PeerMeshQueryResult
  | import('@sharker/runtime-host/protocol').PeerMeshInvitationResult;
...
export type DesktopRuntimeHostUpdateSchedulingState =
  | 'unsupported'
  | import('@sharker/runtime-host/operator').RuntimeHostUpdateSchedulerState;
...
export interface SharkerBridge {
  ...
  runtimeHostManagement: {
    ...
    setUpdatePolicy(
      profileId: string,
      policy: import('@sharker/runtime-host/operator').RuntimeHostManagedUpdatePolicy,
  ...
  newTasks: {
    ...
    ): Promise<import('@sharker/runtime/skill-invocation').InvocableSkillEntry[]>;
    getReadiness(
      target: DesktopNewTaskTarget,
      input?: DesktopTaskSubmissionReadinessRequest,
    ): Promise<import('@sharker/core/task-submission-readiness').TaskSubmissionReadinessSnapshot>;
    searchFiles(
      target: DesktopNewTaskTarget,
      query: string,
  ...
  sessions: {
    ...
    send(
      ...
      command: {
        ...
        retainedAttachments?: import('@sharker/core/events').AttachmentRef[];
        turnOrchestration?: TurnOrchestration;
        quotes?: import('@sharker/core/events').QuoteRef[];
        workspaceFileReferences?: Array<
          Pick<import('@sharker/core/events').InlineReference, 'value' | 'start'>
        >;
      },
    ): Promise<
      | {
          ...
          steered?: never;
          messageId?: never;
          attachments: import('@sharker/core/events').AttachmentRef[];
          inlineReferences: import('@sharker/core/events').InlineReference[];
          skillInvocation: import('@sharker/runtime/skill-invocation').SkillInvocationResult;
        }
      | {
          ok: true;
          /**
    ...
    ): Promise<import('@sharker/runtime-host/protocol').TurnMessageQueryResult>;
    queryMessageExecutions(
      sessionId: string,
      messageIds: readonly string[],
    ): Promise<import('@sharker/runtime-host/protocol').TurnMessageExecutionQueryResult>;
  ...
  goal: {
    ...
    get(sessionId: string): Promise<import('@sharker/runtime/goal-state').GoalState | null>;
    /**
  ...
  connections: {
    ...
    getRequestHeaders(slug: string, host?: DesktopRuntimeHostRef): Promise<import('@sharker/core/llm-connections').SavedRequestHeaders>;
    setRequestHeaders(
      slug: string,
      headers: readonly import('@sharker/core/llm-connections').RequestHeaderUpdate[],
      host?: DesktopRuntimeHostRef,
    ): Promise<import('@sharker/core/llm-connections').SavedRequestHeaders>;
  ...
  memory: {
    ...
    listVault(host?: DesktopRuntimeHostRef): Promise<
      | { ok: true; value: import('@sharker/core/local-memory-vault').MemoryVaultListing }
      | { ok: false; message: string }
    >;
    readVaultFile(path: string, host?: DesktopRuntimeHostRef): Promise<
      | { ok: true; value: { path: string; content: string; updatedAt: number } }
      | { ok: false; message: string }
    >;
    writeVaultFile(path: string, content: string, host?: DesktopRuntimeHostRef): Promise<
      | { ok: true; value: { path: string; updatedAt: number } }
      | { ok: false; message: string }
    >;
    deleteVaultFile(path: string, host?: DesktopRuntimeHostRef): Promise<
