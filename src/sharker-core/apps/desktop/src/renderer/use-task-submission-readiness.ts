...
import type { TaskSubmissionReadinessSnapshot } from '@sharker/core/task-submission-readiness';
import type {
  ...
  const checkNow = useCallback(async () => {
    ...
    try {
      const next = sessionId
        ? await window.sharker.taskReadiness.getSnapshot(request, sessionId)
        : newTaskTarget
          ? await window.sharker.newTasks.getReadiness(newTaskTarget, request)
          : undefined;
