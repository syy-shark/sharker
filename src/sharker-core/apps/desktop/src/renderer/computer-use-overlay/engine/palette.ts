...
/**
 * Sharker's brand cursor palette, derived from the app's primary token
 * `--action` = oklch(0.62 0.19 264) (a blue/indigo). Gradient tip→tail around it
 * plus a soft brand bloom, so the agent cursor reads as "Sharker" rather than a
 ...
 */
export function sharkerBrandPalette(): Palette {
  return {
    name: 'sharker_brand',
