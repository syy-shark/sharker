...
// Sharker's agent-cursor renderer.
//
// This file has mixed provenance:
// - the first Sharker renderer was a TypeScript adaptation of the MIT-licensed
...
//   blend were independently derived for Sharker and publicly recorded before the
...
//   authored or adjusted in Sharker.
//
...
import { sharkerBrandPalette, type Palette, rgba } from './palette.js';
...
// Sharker's independently derived 30-field motion configuration.
...
/**
 * Sharker's independently derived release gate. Five-nines progress leaves at
...
// The independently derived core score is dimensionless. Sharker's pre-existing
 ...
 * derived motion fields; it remains Sharker's local clamp for stopping very long
...
/**
 * Sharker's raw path-length and backwards-arrival additions around the independent
...
/**
 * Builds Sharker's existing single-segment cubic candidate family and returns the
 * cheapest under the independent core score plus Sharker's retained additions.
 *
...
export class CursorEngine {
  ...
  private palette: Palette = sharkerBrandPalette();
  ...
  /**
   * Sharker applies one rotation term bounded by scootRotationMax. The independent
  ...
  setSession(_sessionId: string): void {
    this.palette = sharkerBrandPalette();
  }
  ...
  moveTo(x: number, y: number, _endHeading?: number, clickOnArrive = false): void {
    ...
    // Sharker fades the first appearance in with the independently derived dart's
  ...
  paint(ctx: CanvasRenderingContext2D, originX: number, originY: number): void {
    ...
    // The native cursor multiplies its scale by 0.7 while pressed. Sharker's 0.9
  ...
  private paintAgentCursor(ctx: CanvasRenderingContext2D, pressedAmount: number): void {
    ...
    // colour literal in its drawing code. The rounded join Sharker had was
