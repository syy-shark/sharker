...
export type SettingsProjectsCopy = {
  runtimeHost: {
    ...
    >;
    updatePhase: Record<
      'preparing_cli' | import('@sharker/runtime-host/operator').RuntimeHostServiceUpdatePhase,
      string
    >;
  ...
  zh: {
    runtimeHost: {
      ...
      setupPhase: {
        ...
        installing_package: '正在安装 Sharker…',
      ...
      directPeerAutomaticRelayDiscoveryHelp:
        '协调节点使用 Circuit Relay v2 协议，仅帮助建立端到端直连，不承载应用流量。Sharker 会通过公共 IPFS 网络尽力发现可用节点；手动设置的节点优先。',
      ...
      updatePolicy: '更新策略',
      updatePolicyDescription: '选择这个 Host 跟随的 Sharker 版本',
      ...
      updatePolicyNotNewer: (version: string) => `Sharker ${version} 不高于当前版本`,
      updatePolicyManualAction: (version: string) => `Sharker ${version} 需要手动更新`,
      ...
      uninstallConfirmBody: '这会停止并移除 Sharker 管理的服务与程序，但保留 State Root、项目和任务数据。当前 Desktop Profile 不会被删除。',
    ...
    clearDefault: '取消默认',
    remove: '从 Sharker 移除',
    removeConfirmTitle: '从 Sharker 移除这个项目？',
    // The one thing a user actually fears here, stated first and plainly.
    removeConfirmBody: '仅从 Sharker 的项目列表移除，磁盘上的文件不受影响。该项目下已有的任务会移到"未归属"分组，不会被删除。',
    removeConfirm: '移除',
    removeCancel: '取消',
    actionFailed: '操作失败',
    unavailable: '目录不可用',
  ...
  en: {
    runtimeHost: {
      ...
      setupPhase: {
        ...
        installing_package: 'Installing Sharker…',
      ...
      directPeerAutomaticRelayDiscoveryHelp:
        'Coordination peers use Circuit Relay v2 only to establish an end-to-end direct connection; they never carry application traffic. Sharker discovers candidates through the public IPFS network on a best-effort basis, while manually configured peers remain preferred.',
      ...
      updatePolicyDescription: 'Choose which Sharker release this Host follows',
      ...
      updatePolicyNotNewer: (version: string) => `Sharker ${version} is not newer than this Host`,
      updatePolicyManualAction: (version: string) => `Sharker ${version} needs a manual update`,
      updatePolicyManualReason: {
      ...
      uninstallConfirmBody: 'This stops and removes the Sharker-managed service and program, while preserving the State Root, projects, and task data. The Desktop profile is not removed.',
    ...
    remove: 'Remove from Sharker',
    removeConfirmTitle: 'Remove this project from Sharker?',
    removeConfirmBody:
      'This only removes it from Sharker’s project list; the files on disk are untouched. Tasks under this project move to “Ungrouped” and are not deleted.',
