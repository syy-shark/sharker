...
import { generalizedErrorMessage, generalizedErrorMessageChinese } from '@sharker/core/redaction';

import { type UiCatalog, type UiLocale } from '@sharker/core/ui-locale';

import { type PermissionMode } from '@sharker/core/permission';

import { type ChatDefaultPermissionMode, type SettingsSection } from '@sharker/core/settings';
...
import { type GoalStatus } from '@sharker/core/goal';
...
type ShellCopy = {
  ...
  chrome: {
    ...
    enterWorkbarFullscreen: string;
    exitWorkbarFullscreen: string;
  ...
  zh: {
    ...
    sessionSettingsActions: {
      ...
      permissionDescriptions: {
        ...
        ask: '自动：在 Sharker 的保护层内执行；需要超出当前权限范围时会先来问你。',
        bypass: '本地工具直接访问你的文件和网络，不经 Sharker 的保护层。',
      },
      bypassConfirmTitle: '切换到完全权限？',
      bypassConfirmDescription:
        '本地工具将直接读写你的文件并访问网络，不经 Sharker 的保护层。仅用于你完全信任、或已在外部隔离环境中运行的任务。',
    ...
    goalDialog: {
      ...
      conditionLabel: '达成条件',
      conditionDescription: '用一句话说明什么算做完；Sharker 每轮都据此判断。',
    ...
    commandPalette: {
      ...
      permissionModes: {
        ...
        ask: { label: '权限 · 自动', hint: '在 Sharker 的保护层内运行；需要超出当前权限范围时再询问' },
        bypass: {
          label: '权限 · 完全权限',
          hint: '不经 Sharker 的保护层，直接访问你的文件和网络',
        },
      },
    ...
    chrome: {
      ...
      enterWorkbarFullscreen: '全屏工作栏',
      exitWorkbarFullscreen: '退出全屏',
  ...
  en: {
    ...
    projectActions: {
      ...
      openPathFailures: {
        ...
        'not-allowed': 'The path is outside the folders that Sharker can open.',
    ...
    sessionSettingsActions: {
      ...
      permissionDescriptions: {
        ...
        ask: "Auto: runs inside Sharker's protection layer and asks before anything goes beyond the current permissions.",
        bypass: "Local tools reach your files and your network directly, outside Sharker's protection layer.",
      },
      bypassConfirmTitle: 'Switch to full access?',
      bypassConfirmDescription:
        "Local tools will read and write your files and reach the network directly, outside Sharker's protection layer. Use only for tasks you fully trust, or ones already isolated by their environment.",
    ...
    goalDialog: {
      title: 'Set a goal',
      description: 'Sharker continues on its own after each turn until the goal is met, judged impossible, or a budget below is reached. You can stop it any time from above the composer.',
      conditionLabel: 'Completion condition',
      conditionDescription: 'One sentence for what counts as done; Sharker checks it after every turn.',
    ...
    errorBoundary: {
      ...
      title: 'The Sharker renderer crashed',
    ...
    commandPalette: {
      ...
      permissionModes: {
        ...
        ask: {
          ...
          hint: "Run inside Sharker's protection layer; ask before going beyond the current permissions",
        },
        bypass: {
          label: 'Permissions · Full access',
          hint: "Reach your files and your network directly, outside Sharker's protection layer",
        },
      },
    ...
    chrome: {
      ...
      enterWorkbarFullscreen: 'Full screen workbar',
      exitWorkbarFullscreen: 'Exit full screen',
