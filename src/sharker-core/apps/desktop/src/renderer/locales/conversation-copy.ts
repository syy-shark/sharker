...
import type { ChatConfigurationReason } from '@sharker/core/connection-readiness';
import type { SessionSendProjection } from '@sharker/core/session-send-projection';

import type { ModelCallKind } from '@sharker/core/model-call-attempt';
  ...
  zh: {
    ...
    model: {
      ...
      configurationReason: {
        ...
        provider_retired: '当前任务绑定的连接，其登录方式已从 Sharker 移除，无法用于发送。请到 设置 · 模型 改用其他连接后新建任务。',
      },
    },
    ...
    health: {
      blocked: {
        ...
        provider_retired: { label: '登录方式已停用', tooltip: (name) => `任务绑定的连接 "${name}" 使用的登录方式已从 Sharker 移除，发送会失败。请到 设置 · 模型 改用其他连接。` },
  ...
  en: {
    ...
    model: {
      ...
      configurationReason: {
        ...
        provider_retired: 'The sign-in this task\u2019s connection uses was removed from Sharker, so it cannot send. Switch to another connection in Settings · Models, then start a new task.',
      },
    },
    ...
    health: {
      blocked: {
        ...
        provider_retired: { label: 'Sign-in retired', tooltip: (name) => `The sign-in that connection "${name}" uses was removed from Sharker, so sending fails. Switch to another connection in Settings · Models.` },
