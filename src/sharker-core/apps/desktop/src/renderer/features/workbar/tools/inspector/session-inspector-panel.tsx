...
import { uiLocaleToIntlLocale, type UiLocale } from '@sharker/core/ui-locale';
import { traceTurnIdentityKey } from '@sharker/core/session-trace';
import { useToast, useUiLocale } from '@sharker/ui';
import { ICON_SIZE, Activity, AlertTriangle, Copy } from '@sharker/ui/icons';
import {
...
export function SessionInspectorPanel(props: { sessionId: string; active: boolean }) {
  ...
  return (
    <Section
      variant="transparent"
      padding={4}
      className="sharker-inspector-panel"
      data-sharker-contract="session-inspector"
    ...
    >
      ...
      <VStack gap={6} height="100%">
        ...
        <div
          role="status"
          aria-live="polite"
          className="sharker-inspector-status"
        ...
        {(!model.empty || snapshot.nextCursor) && (
          <div className="sharker-inspector-raw" data-sharker-contract="session-inspector-raw">
            <VStack gap={2}>
              <div className="sharker-inspector-section-head">
                <Heading level={3} className="sharker-inspector-section-title">
              ...
              {model.coverage && (
                <p
                  className="sharker-inspector-coverage-note"
                  data-sharker-contract="session-inspector-coverage"
                >
              ...
              <ol className="sharker-inspector-turns">
...
function InspectorOverview(props: {
  ...
  return (
    <VStack gap={6} data-sharker-contract="session-inspector-overview">
      {props.showTotals && (
        <VStack gap={2} data-sharker-contract="session-inspector-stats">
...
function InspectorOverviewStat(props: { label: string; value: ReactNode }) {
  return (
    <div className="sharker-inspector-section-head">
      ...
      <span className="sharker-inspector-section-readout">{props.value}</span>
    </div>
  );
}
...
function FactRow(props: { label: ReactNode; value: ReactNode; swatch?: ReactNode }) {
  return (
    <div className="sharker-inspector-grid-row">
      <dt>
        {props.swatch}
        {props.label}
      </dt>
      <dd className="sharker-inspector-grid-value">{props.value}</dd>
    </div>
  );
}
...
function InspectorContextSection(props: {
  ...
  return (
    <VStack gap={2} data-sharker-contract="session-inspector-context">
      <div className="sharker-inspector-section-head" data-level={level}>
        <Heading level={3} className="sharker-inspector-section-title">
          {copy.overview.context}
        </Heading>
        <span className="sharker-inspector-section-readout">
          {formatCompactNumber(context.usedTokens)} / {formatCompactNumber(context.windowTokens)} ·{' '}
          {formatPercent(context.ratio)}
        </span>
      </div>

      <div className="sharker-inspector-context-track" data-level={level} aria-hidden="true">
        {context.segments.map((segment) => (
          <span
            key={segment.kind}
            className="sharker-inspector-context-band"
      ...
      <dl className="sharker-inspector-grid">
        {context.segments.map((segment) => (
          <FactRow
            key={segment.kind}
            ...
            swatch={
              <span
                className="sharker-inspector-context-swatch"
                data-segment={segment.kind}
                data-level={level}
                aria-hidden="true"
              />
...
export function InspectorCompositionSection(props: {
  ...
  return (
    <VStack gap={2} data-sharker-contract="session-inspector-composition">
      <div className="sharker-inspector-section-head">
        <Heading level={3} className="sharker-inspector-section-title">
          {labels.title}
        </Heading>
      </div>
      <p className="sharker-inspector-section-note">{labels.basis}</p>
      ...
      {state.status === 'unrecorded' ? (
        ...
        <p className="sharker-inspector-section-note">{labels.unrecorded}</p>
      ) : (
        <>
          <div className="sharker-inspector-composition-track" aria-hidden="true">
            {state.composition.parts.map((part) => (
              <span
                key={part.kind}
                className="sharker-inspector-composition-band"
          ...
          <dl className="sharker-inspector-grid">
            {state.composition.parts.map((part) => (
              <FactRow
                ...
                swatch={
                  <span
                    className="sharker-inspector-composition-swatch"
                    data-segment={part.kind}
                    aria-hidden="true"
                  />
                }
          ...
          {(state.composition.tools.length > 0 || state.composition.unlabelledTools) && (
            <>
              <Heading level={4} className="sharker-inspector-section-subtitle">
                {labels.tools}
              </Heading>
              <dl className="sharker-inspector-grid">
                {state.composition.tools.map((tool) => (
                  <FactRow
                    key={tool.name}
                    label={<span className="sharker-inspector-composition-name">{tool.name}</span>}
...
function TurnRow(props: {
  ...
  return (
    <li
      className="sharker-inspector-turn"
      data-sharker-contract="session-inspector-turn"
      data-failed={turn.failed || undefined}
    >
      <div className="sharker-inspector-turn-head">
        <Text type="label" className="sharker-inspector-turn-label">
        ...
        {turn.failed && (
          <span
            className="sharker-inspector-turn-failure"
            data-sharker-contract="session-inspector-turn-failed"
          >
        ...
        {/* Two tiers inside the meta, not one: the cost is what a reader
            ...
            as a single texture. */}
        <span className="sharker-inspector-turn-meta">
          {formatDuration(turn.durationMs)} ·{' '}
          <span className="sharker-inspector-turn-cost">
            {formatCost(turn.costUsd, copy.costUnavailable)}
          </span>
        </span>
      </div>
      <ol className="sharker-inspector-steps">
        {turn.steps.map((step) => (
          <StepRow
            key={step.id}
            step={step}
...
function StepRow(props: {
  ...
  return (
    <li
      className="sharker-inspector-step"
      data-sharker-contract="session-inspector-step"
      data-failed={step.failed || undefined}
    >
      <span className="sharker-inspector-step-text">
        <span className="sharker-inspector-step-label">{label}</span>
        {qualifier && <span className="sharker-inspector-step-detail">{qualifier}</span>}
        {pricingKey && (
          <span className="sharker-inspector-pricing-key">
        ...
        {step.recovered && (
          <span className="sharker-inspector-step-recovered">{copy.recoveredAs(step.recovered)}</span>
        )}
      </span>
      {meta.length > 0 && <span className="sharker-inspector-step-meta">{meta.join(' · ')}</span>}
    </li>
  );
}
