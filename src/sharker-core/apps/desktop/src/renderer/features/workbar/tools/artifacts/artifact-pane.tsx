...
import type { ArtifactDescriptor, ArtifactKind } from '@sharker/core/artifacts';
import type { UiLocale } from '@sharker/core/ui-locale';
import { formatRelativeTimestamp } from '@sharker/core/relative-time';
import { generalizedErrorMessage, generalizedErrorMessageChinese, redactSecrets } from '@sharker/core/redaction';
import {
  Badge,
  Banner,
  Button,
...
export function ArtifactPane(props: {
  ...
  function focusComposer() {
    ...
    requestAnimationFrame(() => {
      const composer = document.querySelector<HTMLElement>('.sharker-composer [contenteditable="true"]');
      composer?.focus();
    });
  }
  ...
  return (
    <div className="sharker-artifact-pane" role="region" aria-label={copy.pane.panelAria} onKeyDown={handlePaneKeyDown}>
      {activeListError && (
        <Banner
          status="error"
          className="sharker-artifact-list-error"
      ...
      {view.kind === 'list' ? (
        activeRecords.length > 0 ? (
          <ul
            ref={listRef}
            className="sharker-artifact-list"
            role="listbox"
            aria-label={copy.pane.listAria}
            aria-activedescendant={selectedId ? `sharker-artifact-row-${selectedId}` : undefined}
            tabIndex={0}
            onKeyDown={handleListKeyDown}
          >
            {activeRecords.map((record) => (
              <li key={record.id} className="sharker-artifact-list-item">
                <Button
                  id={`sharker-artifact-row-${record.id}`}
                  variant="ghost"
                  className="sharker-artifact-row"
                  role="option"
                  ...
                  label={record.name}
                  icon={(
                    <span className="sharker-artifact-row-icon" aria-hidden="true">
                      <KindIcon kind={record.kind} />
                    </span>
                  )}
                  endContent={(
                    <span className="sharker-artifact-row-meta">
                      <span className="sharker-artifact-row-size">{formatBytes(record.sizeBytes)}</span>
                      <span className="sharker-artifact-row-time">
                        {formatRelativeTimestamp(record.createdAt, Date.now(), locale)}
                      </span>
                      {record.status === 'deleted' && (
                        <Badge variant="error" className="sharker-artifact-row-badge" label={copy.pane.deletedBadge} />
                      )}
                    </span>
                  )}
                />
          ...
          <AstryxEmptyState
            className="sharker-artifact-list-empty"
            icon={<FileText size={ICON_SIZE.empty} aria-hidden="true" />}
            {...emptyStateCopy}
          />
        )
      ) : previewRecord ? (
        <div className="sharker-artifact-preview-screen">
          <header className="sharker-artifact-preview-header">
            <Button
              variant="ghost"
              size="sm"
              isIconOnly
            ...
            />
            <div className="sharker-artifact-preview-heading">
            ...
            <MoreMenu
              className="sharker-artifact-preview-more"
              size="sm"
          ...
          <div
            ref={previewRef}
            className="sharker-artifact-preview"
            role="region"
...
/* PR-FORMAT-BYTES-DEDUP-0 (round 21/30): the local `formatBytes`
   was a less-robust variant of the one in `@sharker/ui`
   components.tsx. Removed; we now import the shared helper. */

/* PR-FORMAT-RELATIVE-DEDUP-0 (round 22/30): the local
   `formatRelative` was a less-feature variant of @sharker/core's
