...
import { ICON_SIZE, ChevronLeft, ChevronRight, Home, RotateCw, X } from '@sharker/ui/icons';
import {
  isBrowserStartSurfaceUrl,
  normalizeBrowserAddressInput,
  type BrowserState,
} from '@sharker/core/browser';
...
function isLocalHtmlAddressInput(raw: string): boolean {
  const trimmed = raw.trim();
  if (/^file:/i.test(trimmed)) return /\.(html?)(?:[?#]|$)/i.test(trimmed);
  return /\.(html?)$/i.test(trimmed) && !/^[a-z][a-z0-9+.-]*:/i.test(trimmed);
}
...
export function BrowserPanel(props: { sessionId: string; hidden: boolean }) {
  ...
  const openAddress = useCallback((raw: string) => {
    if (isLocalHtmlAddressInput(raw)) {
  ...
  return (
    ...
    >
      <Toolbar
        ...
        startContent={(
          <>
            ...
            <div className="sharker-browser-address-field">
              <TextInput
                type="text"
      ...
      />
      <div className="sharker-browser-strip" ref={stripRef}>
        {onStartSurface ? (
          <BrowserStartPage
            placeholder={copy.addressPlaceholder}
