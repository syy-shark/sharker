...
import type { ChatModelChoice } from '@sharker/ui';
...
export function useWorkbarController(
  ...
  const seenLiveBrowserSessionIdsRef = useRef<ReadonlySet<string>>(new Set());
  ...
  useEffect(
    () =>
      browser.subscribeLive((payload) => {
        const next = new Set(payload.sessionIds);
        const prev = seenLiveBrowserSessionIdsRef.current;
        seenLiveBrowserSessionIdsRef.current = next;
        setLiveBrowserSessionIds(payload.sessionIds);
        const sessionId = activeSessionIdRef.current;
        if (sessionId && next.has(sessionId) && !prev.has(sessionId)) {
          openTool('browser');
        }
      }),
    [browser, openTool],
