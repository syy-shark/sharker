...
import type { SessionViewMode } from '@sharker/ui';
 ...
 * `--sharker-sidenav-width` so the titlebar's session breadcrumb starts where the
...
export function createSessionRailLayoutStore() {
  ...
  return {
    ...
    setCollapsed(next: boolean): void {
      ...
      safeLocalStorageSet('sharker-chat-list-collapsed-v1', next ? 'true' : 'false');
    },
    ...
    setWidth(next: number): void {
      ...
      widthPersistHandle = setTimeout(() => {
        safeLocalStorageSet('sharker-chat-list-width-v1', String(width));
