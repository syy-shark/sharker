...
import { Check, Eye, EyeOff, FolderOpen } from '@sharker/ui/icons';
import type {
...
export function RemoteProjectDirectoryDialog(props: {
  ...
  async function load(target: DirectoryLoad): Promise<void> {
    ...
    try {
      if (target.kind === 'initial') {
        const nextRoots = await window.sharker.projects.getDirectoryRoots(target.host);
        const nextRoot = nextRoots[0];
        if (!nextRoot) throw new Error('Runtime Host did not publish a project directory');
        const next = await window.sharker.projects.listDirectory({
          rootId: nextRoot.id,
          segments: [],
        }, target.host);
  ...
  async function register(): Promise<void> {
    ...
    setError(undefined);
    try {
      const project = await window.sharker.projects.registerDirectory({
        rootId: root.id,
        segments,
      }, host);
      if (request.current !== sequence) return;
