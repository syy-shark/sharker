...
import type { PlanExecutionStep, PlanProposal, PlanSessionState } from '@sharker/core/plan';
import type { SessionEvent } from '@sharker/core/events';
import type { SessionSummary } from '@sharker/core/session';
import { Banner } from '@astryxdesign/core/Banner';
import { Collapsible } from '@astryxdesign/core/Collapsible';
import { Badge, type BadgeVariant, Button as UiButton, useToast, useUiLocale } from '@sharker/ui';
...
export function usePlanModeState(session: SessionSummary | undefined): PlanModeState {
  ...
  const refresh = useCallback(async () => {
    if (!session) return;
    setState(await window.sharker.sessions.getPlanState(session.id));
  }, [session?.id]);
  ...
  useEffect(() => {
    ...
    refreshOrReport();
    const unsubscribeEvents = window.sharker.sessions.subscribeEvents(session.id, (event: SessionEvent) => {
      if (
    ...
    const unsubscribePlanChanges = window.sharker.sessions.subscribePlanChanges(
      session.id,
      refreshOrReport,
    );
    return () => {
  ...
  const requestRevision = useCallback(async (proposalId: string): Promise<void> => {
    if (!session) return;
    await run(async () => {
      await window.sharker.sessions.requestPlanRevision(session.id, proposalId);
    });
  }, [run, session?.id]);
  ...
  const approve = useCallback(async (proposal: PlanProposal): Promise<void> => {
    ...
    await run(async () => {
      await window.sharker.sessions.approvePlan(session.id, {
  ...
  const resume = useCallback(async (executionId: string): Promise<void> => {
    ...
    await run(async () => {
      await window.sharker.sessions.resumePlan(session.id, executionId, input.turnId);
  ...
  const abandon = useCallback(async (executionId: string, title: string): Promise<void> => {
    ...
    if (!confirmed) return;
    await run(async () => {
      await window.sharker.sessions.abandonPlanExecution(session.id, executionId);
    });
