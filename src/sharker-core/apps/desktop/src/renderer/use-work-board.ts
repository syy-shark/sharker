...
export function useWorkBoard(query?: WorkBoardListQuery): UseWorkBoardResult {
  ...
  const load = useCallback(
    (preserveItems: boolean) => {
      ...
      void listWindow(window.sharker.workBoard.list, query, loadedItemCount).then(
        (result) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.sharker.workBoard.subscribeChanges(
  ...
  const loadMoreAt = useCallback((cursor: string) => {
    ...
    void window.sharker.workBoard.list({ ...query, cursor }).then(
      (result) => {
        if (revision !== revisionRef.current) return;
        if (!result.ok) {
          setSnapshot((current) => ({
  ...
  return {
    ...
    retryContinuation,
    loadMore,
    create: (input) => mutate(() => window.sharker.workBoard.create(input)),
    update: (id, patch, options) =>
      mutate(() => window.sharker.workBoard.update(id, patch, options)),
    archive: (id, options) => mutate(() => window.sharker.workBoard.archive(id, options)),
    unarchive: (id, options) => mutate(() => window.sharker.workBoard.unarchive(id, options)),
    remove: async (id, options) => {
      await mutate(() => window.sharker.workBoard.remove(id, options));
    },
  };
}
