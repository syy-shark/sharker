  ...
  async function refreshConnections() {
    ...
    try {
      ...
      if (target.kind === 'session' && target.sessionId) {
        next = await window.sharker.connections.getSnapshot(target.sessionId);
      } else if (target.kind === 'new-task' && target.host) {
        next = await window.sharker.newTasks.getConnections(target.host);
      } else if (target.kind === 'default') {
        next = (
          await runOnDefaultRuntimeHost((host) =>
            window.sharker.connections.getSnapshot(undefined, host),
          )
        ).value;
      } else return;
