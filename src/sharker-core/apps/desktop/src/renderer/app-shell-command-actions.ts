...
import type { SessionStartMode } from '@sharker/core/deep-research';
  ...
  return buildCommandList({
    ...
    onTestConnection: async (slug) => {
      ...
      try {
        const { value: result, diagnosticTarget } = await runOnDefaultRuntimeHost((host) =>
          window.sharker.connections.test(slug, undefined, host),
        );
    ...
    onSetDefaultConnection: async (slug) => {
      ...
      try {
        await runOnDefaultRuntimeHost((host) =>
          window.sharker.connections.setDefault(slug, host),
        );
        await refreshConnections();
        const conn = connections.find((c) => c.slug === slug);
    ...
    onSaveActiveConversationToFile: async () => {
      ...
      const sanitizedSession = sessionName
        ...
        .replace(/["<>:|?*]/g, "")
        .slice(0, 80);
      const defaultName = `sharker-${sanitizedSession}-${yyyy}-${mm}-${dd}.md`;
      try {
        const result = await window.sharker.sessions.saveConversationToFile({
          markdown,
          defaultName,
        });
        if (result.ok) {
    ...
    onOpenLocalMemoryFile: async () => {
      ...
      try {
        const { value: result, diagnosticTarget } = await runOnDefaultRuntimeHost((host) =>
          window.sharker.memory.openFile(host),
        );
        if (!result.ok) {
    ...
    onCopyDiagnostics: async () => {
      ...
      try {
        await window.sharker.diagnostics.copyReport({
          surface: "manual",
          ...(target ? { target } : {}),
        });
    ...
    onTestNetworkProxy: async () => {
      ...
      try {
        ...
        const { value: result, diagnosticTarget } = await runOnDefaultRuntimeHost((host) =>
          window.sharker.settings.testNetworkProxy(undefined, host),
        );
