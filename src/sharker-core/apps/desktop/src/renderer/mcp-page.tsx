...
import { MCP_CONFIG_VERSION, isMcpStdioConfig } from '@sharker/core/mcp';
import {
  Banner,
  Button,
  Collapsible,
...
export function McpPage(props: { hubHeader?: ModuleHubHeader }) {
  ...
  async function reload() {
    ...
    try {
      const { value: [nextConfig, nextStatuses] } = await runOnDefaultRuntimeHost((host) =>
        Promise.all([
          window.sharker.mcp.getConfig(host),
          window.sharker.mcp.listStatuses(host),
        ]),
      );
  ...
  useEffect(() => {
    void reload();
    return window.sharker.mcp.subscribeChanges((next) => {
  ...
  async function installCatalogEntry(entry: McpCatalogEntry) {
    ...
    try {
      ...
      const next = await runOnDefaultRuntimeHost((host) =>
        window.sharker.mcp.install(entry.id, structuredClone(entry.config), host),
      );
  ...
  async function cancelCatalogInstall(entry: McpCatalogEntry) {
    ...
    try {
      const next = await runOnDefaultRuntimeHost((host) =>
        window.sharker.mcp.cancelInstall(entry.id, host),
      );
      if (!mounted.current) return;
      setConfig(next.value);
  ...
  async function importJson(event: React.FormEvent) {
    ...
    try {
      const next = await runOnDefaultRuntimeHost((host) =>
        window.sharker.mcp.importConfig(editor.source, host),
      );
      if (!mounted.current) return;
  ...
  async function toggle(serverId: string, server: McpServerConfig, enabled: boolean) {
    ...
    try {
      const next = await runOnDefaultRuntimeHost((host) =>
        window.sharker.mcp.upsert(serverId, { ...server, enabled }, host),
      );
  ...
  async function testServer(serverId: string) {
    ...
    try {
      const { value: result, diagnosticTarget } = await runOnDefaultRuntimeHost((host) =>
        window.sharker.mcp.test(serverId, host),
      );
      if (!mounted.current) return;
  ...
  async function remove(serverId: string) {
    ...
    try {
      const next = await runOnDefaultRuntimeHost((host) =>
        window.sharker.mcp.remove(serverId, host),
      );
      if (!mounted.current) return;
      setConfig(next.value);
  ...
  const searchSummary = normalizedQuery ? (
    <div className="sharker-module-search-summary" role="status" aria-live="polite">
  ...
  const marketPanel = (
    <div className="sharker-module-page-panel">
      {searchSummary}
      {marketEntries.length === 0 ? (
        ...
        <List density="balanced" hasDividers className="sharker-module-page-rows" aria-label={copy.page.market}>
          {marketEntries.map((entry) => {
            const installed = Boolean(config.mcpServers[entry.id]);
            return (
              <ListItem
                key={entry.id}
                data-sharker-contract="mcp-market-row"
                label={entry.name}
                description={[
                  entry.description,
                ...
                startContent={(
                  <span
                    className="sharker-module-market-icon"
                    data-brand={entry.id}
  ...
  const installedPanel = (
    <div className="sharker-module-page-panel" ref={rowsContainerRef} {...rovingRows}>
      {/* Selecting a row moves no focus, so nothing else would tell a screen
          reader that the details opened. This says so, politely. */}
      <p className="sharker-visually-hidden" role="status" aria-live="polite">
      ...
      {busy === 'load' ? (
        ...
        <div className="sharker-module-list-skeleton" role="status" aria-busy="true" aria-label={copy.page.loading}>
          {[0, 1, 2].map((index) => (
            <div key={index} className="sharker-module-list-skeleton-row" aria-hidden="true">
              <Skeleton width={28} height={28} radius={2} index={index} />
              <div className="sharker-module-list-skeleton-lines">
        ...
        /* Selectable, otherwise inert rows: the switch, test, edit and
           ...
           item. */
        <List density="balanced" hasDividers className="sharker-module-page-rows" aria-label={copy.page.installed}>
          {installedEntries.map(([serverId, server]) => {
            ...
            return (
              <ListItem
                ...
                label={serverId}
                description={(
                  <span className="sharker-module-row-description" data-sharker-contract="mcp-server-description">
  ...
  return (
    <section className="sharker-main detailPane sharker-module-main agents-chat-panel" data-page-shell="layout" data-module="mcp" data-sharker-contract="module-main" aria-label={props.hubHeader?.title ?? 'MCP'}>
      <ModulePage
        ...
        inspectorAutoSaveId="sharker-mcp-inspector"
        ...
        actions={
          <div
            className="sharker-module-main-actions"
            data-sharker-contract="module-actions"
            role="group"
        ...
        toolbar={(
          <div className="sharker-module-page-bar">
...
function McpInstallButton(props: {
  ...
  return (
    <IconButton
      className="sharker-mcp-install-button"
      size="sm"
      variant="ghost"
      ...
      icon={props.phase ? (
        <span className="sharker-mcp-install-icon" data-phase={props.phase}>
          <Loader2 size={ICON_SIZE.chrome} className="sharker-mcp-install-spinner" aria-hidden="true" />
          <X size={ICON_SIZE.chrome} className="sharker-mcp-install-cancel" aria-hidden="true" />
        </span>
...
function McpServerInspector(props: {
  ...
  return (
    <VStack className="sharker-mcp-inspector" gap={4}>
      <VStack gap={2}>
        ...
        <Text type="body" color="secondary" className="sharker-mcp-inspector-endpoint">
          <code>{endpoint}</code>
        </Text>
      </VStack>
      ...
      {status?.tools.length ? (
        <>
          ...
          <VStack gap={2}>
            ...
            <div className="sharker-mcp-tool-list">{status.tools.map((tool) => <code key={tool.name}>{tool.name}</code>)}</div>
          </VStack>
        </>
      ) : null}
      {status?.stderrTail?.length ? (
        <pre className="sharker-mcp-stderr">{status.stderrTail.join('\n')}</pre>
      ) : null}
    </VStack>
  );
}
...
function McpEditorDialog(props: {
  ...
  return (
    <Dialog
      ...
      className="sharker-mcp-editor-dialog"
    ...
    >
      <Layout
        ...
        {!editing && (
          <RadioList
            className="sharker-mcp-editor-choice"
          ...
          >
            <RadioListItem
              ...
              startContent={<Terminal size={ICON_SIZE.control} className="sharker-mcp-choice-icon" aria-hidden="true" />}
            />
            <RadioListItem
              value="json"
              label={props.copy.editor.pasteJson}
              startContent={<FileCode size={ICON_SIZE.control} className="sharker-mcp-choice-icon" aria-hidden="true" />}
            />
          </RadioList>
        )}
        {props.state.mode === 'json' ? (
          <form className="sharker-mcp-json-form" onSubmit={props.onImport}>
            <div className="sharker-mcp-json-field">
            ...
            <div className="sharker-mcp-editor-footer"><Button variant="ghost" onClick={() => props.onOpenChange(false)} label={props.copy.editor.cancel} /><Button type="submit" variant="primary" isLoading={props.saving} label={props.copy.editor.importConnect} /></div>
          </form>
        ) : (
          <form className="sharker-mcp-manual-form" onSubmit={props.onSave}>
            <RadioList
              className="sharker-mcp-editor-choice"
            ...
            >
              ...
              <RadioListItem
                ...
                startContent={<Globe size={ICON_SIZE.control} className="sharker-mcp-choice-icon" aria-hidden="true" />}
              />
            </RadioList>
            <div className="sharker-mcp-form-fields">
              <div className="sharker-mcp-primary-fields">
              ...
              {props.state.draft.kind === 'stdio' ? (
                <>
                  ...
                  >
                    <div className="sharker-mcp-advanced-fields">
                      <Selector
            ...
            <div className="sharker-mcp-editor-footer"><Button variant="ghost" onClick={() => props.onOpenChange(false)} label={props.copy.editor.cancel} /><Button type="submit" variant="primary" isLoading={props.saving} label={props.copy.editor.saveConnect} /></div>
          </form>
        )}
          </LayoutContent>
        }
