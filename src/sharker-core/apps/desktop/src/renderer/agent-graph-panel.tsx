...
import type { AgentGraphEpochSummary } from '@sharker/runtime-host/protocol';
import { IconButton, Selector, type SelectorOptionType } from '@sharker/ui';
import { ICON_SIZE, ChevronDown, X } from '@sharker/ui/icons';
...
export function AgentGraphPanel(props: {
  ...
  useEffect(() => {
    ...
    const scheduler = createAgentGraphRefreshScheduler(async (fence) => {
      ...
      try {
        ...
        if (!cachedDirectory) {
          directory = await window.sharker.graphs.listEpochs(props.rootSessionId);
        } else {
          const currentPage = await window.sharker.graphs.listCurrentEpochs(props.rootSessionId);
          directory = sameEpochPage(cachedDirectory, currentPage)
            ? cachedDirectory
            : await window.sharker.graphs.listEpochs(props.rootSessionId);
        }
        ...
        const next = await window.sharker.graphs.getSnapshot(props.rootSessionId, { graphId });
    ...
    const unsubscribe = window.sharker.graphs.subscribe(props.rootSessionId, () =>
  ...
  const stopGraph = async (expectedGraphId: string): Promise<void> => {
    ...
    try {
      await window.sharker.graphs.stop(rootSessionId, expectedGraphId);
    } catch {
  ...
  return (
    <section
      className="sharker-agent-graph-panel"
      aria-label={copy.title}
      data-collapsed={collapsed ? 'true' : 'false'}
    >
      <header className="sharker-agent-graph-heading">
        <div className="sharker-agent-graph-heading-copy">
          <strong>{copy.title}</strong>
          {epochs.length > 1 && snapshot ? (
            <Selector
              className="sharker-agent-graph-epoch-selector"
              size="sm"
              label={copy.epoch}
              isLabelHidden
          ...
          {epochsTruncated ? (
            <span className="sharker-agent-graph-epoch-capped">{copy.cappedEpochs(epochs.length)}</span>
          ) : null}
          {snapshot ? (
            <span className="sharker-agent-graph-progress">
        ...
        <div className="sharker-agent-graph-heading-actions">
          {stopAvailable ? (
            <Button
              variant="secondary"
              size="sm"
          ...
          {dismissAvailable && snapshot ? (
            <IconButton
              variant="ghost"
              size="sm"
              className="sharker-agent-graph-dismiss"
              label={copy.dismiss}
          ...
          <IconButton
            variant="ghost"
            size="sm"
            className="sharker-agent-graph-collapse-toggle"
      ...
      {!collapsed ? (
        <div className="sharker-agent-graph-content" id={contentId}>
          {stopError ? (
            <Banner status="error" role="alert" title={copy.stopFailed} />
          ) : null}

          {loading && !snapshot ? (
            <Spinner size="sm" shade="subtle" label={copy.loading} className="sharker-agent-graph-empty" />
          ) : null}
          {error ? (
            <Banner
              status="error"
          ...
          {snapshot ? (
            <>
              <div className="sharker-agent-graph-section-label">{copy.operators}</div>
              {snapshot.operators.length === 0 ? (
                <EmptyState
                  isCompact
                  className="sharker-agent-graph-empty"
                  title={copy.noOperators}
                />
              ) : (
                <ul className="sharker-agent-graph-operators">
                  {snapshot.operators.map((operator) => {
                    ...
                    return (
                      <li key={operator.operatorId} data-status={operator.status}>
                        <span className="sharker-agent-graph-status-dot" aria-hidden="true" />
                        <span className="sharker-agent-graph-operator-copy">
                          <strong>{operator.agentId}</strong>
                          <span>{work?.instructionPreview ?? operator.operatorId}</span>
                          {wait ? <span className="sharker-agent-graph-wait">{wait}</span> : null}
                        </span>
                        <span className="sharker-agent-graph-operator-status">
              ...
              {snapshot.omitted.operators > 0 ? (
                <div className="sharker-agent-graph-omitted">
                  {copy.hiddenOperators(snapshot.omitted.operators)}
                </div>
              ) : null}
              {snapshot.finish ? (
                <div className="sharker-agent-graph-results">
