...
const LIVE_DEPS: UseOnboardingSnapshotDeps = {
  getSnapshot: () => window.sharker.onboarding.getSnapshot(),
  subscribeInvalidations(onInvalidate) {
    const unsubscribeSessions = window.sharker.sessions.subscribeChanges(() => onInvalidate());
    const unsubscribeConnections = window.sharker.connections.subscribeEvents(() => onInvalidate());
    return () => {
      unsubscribeSessions();
