...
import { useUiLocale } from '@sharker/ui';
import type {
  CreateWorkBoardItemInput,
  WorkBoardItem,
  WorkBoardListQuery,
  WorkBoardScope,
} from '@sharker/core/work-board';
import { ListTodo } from '@sharker/ui/icons';
...
function WorkBoardRow(props: {
  ...
  return (
    <li className="sharker-work-board-row" data-archived={item.archived || undefined}>
      <div className="sharker-work-board-row-main">
        {actions.kind === 'active' && actions.renaming ? (
          <TextInput
            className="sharker-work-board-rename-input"
            size="sm"
            label={copy.rename}
            isLabelHidden
            hasAutoFocus
          ...
          <span className="sharker-work-board-title">{item.title}</span>
        )}
        {item.archived && <span className="sharker-work-board-archived-tag">{copy.archived}</span>}
      </div>
      <div className="sharker-work-board-row-actions">
...
export function WorkBoardPanel(props: {
  ...
  return (
    <section
      className="sharker-work-board-panel"
      aria-label={filter === 'project' ? copy.project : copy.inbox}
    >
      {actionError && (
        <Banner status="error" role="alert" className="sharker-work-board-message" title={actionError} />
      )}
      <div className="sharker-work-board-filters">
        <Button
          size="sm"
      ...
      <div className="sharker-work-board-create">
        <TextInput
          className="sharker-work-board-create-input"
          size="sm"
      ...
      {board.error && board.items.length === 0 ? (
        <Banner
          status="error"
          role="alert"
          className="sharker-work-board-message"
        ...
        <>
          ...
          {board.loading && board.items.length === 0 ? (
            <Spinner size="sm" shade="subtle" label={copy.loading} className="sharker-work-board-message" />
          ) : (
            <>
              {activeItems.length === 0 && archivedItems.length === 0 ? (
                <EmptyState
                  isCompact
                  className="sharker-work-board-empty"
                  icon={<ListTodo size={24} aria-hidden="true" />}
                  title={copy.empty}
                />
              ) : (
                <ul className="sharker-work-board-list">
