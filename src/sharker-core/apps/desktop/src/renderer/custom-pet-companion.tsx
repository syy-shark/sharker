...
} from '@sharker/core/pet';
import { useMountedRef } from '@sharker/ui';
import {
...
function readPetMotionPreference(): boolean {
  ...
  return shouldReducePetMotion({
    reducedMotionAttribute: root.dataset.sharkerReducedMotion === 'true',
    e2eFixtureAttribute: root.dataset.sharkerE2eFixture === 'true',
    systemPreference:
...
function usePetMotionPreference(): boolean {
  ...
  useEffect(() => {
    ...
    observer.observe(root, {
      attributes: true,
      attributeFilter: ['data-sharker-reduced-motion', 'data-sharker-e2e-fixture'],
    });
...
function PetSpriteCanvas(props: {
  ...
  return (
    <canvas
      ref={canvasRef}
      className="sharker-custom-pet-companion__canvas"
...
export function CustomPetCompanion(props: {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const next = await loadSelectedPetCompanion(window.sharker.pets);
  ...
  useEffect(() => {
    void refresh();
    const unsubscribe = window.sharker.pets.subscribeChanges((event) => {
  ...
  return (
    <div className="sharker-custom-pet-companion" aria-hidden="true">
      <PetSpriteCanvas
        companion={companion}
