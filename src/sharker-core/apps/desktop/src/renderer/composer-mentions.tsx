 ...
 * `window.sharker` state (app-shell-composer-attachment-owner-contract). Derives
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      ...
      const request = sessionId
        ? window.sharker.skills.listInvocable(sessionId)
        : newTaskTarget
          ? window.sharker.newTasks.listInvocableSkills(newTaskTarget, context)
          : Promise.resolve([]);
      void request.then(
        (next) => {
    ...
    refresh();
    const unsubscribeSessions = window.sharker.sessions.subscribeChanges((event) => {
      if (
        sessionId &&
    ...
    const unsubscribeContext = sessionId
      ? window.sharker.mcp.subscribeChanges(() => refresh())
      : window.sharker.newTasks.subscribeChanges(() => refresh());
    return () => {
      cancelled = true;
      requestVersion += 1;
      unsubscribeSessions();
    ...
    async (query: string): Promise<ReadonlyArray<{ relativePath: string }>> => {
      try {
        const result = sessionId
          ? await window.sharker.workspace.searchFiles(query, { sessionId })
          : newTaskTarget
            ? await window.sharker.newTasks.searchFiles(newTaskTarget, query)
