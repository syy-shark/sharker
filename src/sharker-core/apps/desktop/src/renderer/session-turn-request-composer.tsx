...
import { Button, Text, TextArea, useToast, useUiLocale } from '@sharker/ui';
import type { SessionTurnAccessRequest } from '@sharker/runtime-host/protocol';
...
export function SessionTurnRequestComposer(props: {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const result = await window.sharker.sessionCollaboration.getTurnRequests(props.sessionId);
        if (!disposed) {
  ...
  async function submit(): Promise<void> {
    ...
    setWorking(true);
    try {
      const request = await window.sharker.sessionCollaboration.requestTurn(
        props.sessionId,
        attempt,
      );
      ...
      try {
        const current = await window.sharker.sessionCollaboration.getTurnRequests(props.sessionId);
  ...
  async function dismiss(requestId: string): Promise<void> {
    try {
      await window.sharker.sessionCollaboration.acknowledgeTurnRequest(
        props.sessionId,
        requestId,
      );
      setRequests((current) =>
