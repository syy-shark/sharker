...
import type { SessionChangedEvent, SessionSummary, StoredMessage } from '@sharker/core/session';
import type { SessionEvent } from '@sharker/core/events';
import type { SessionEventStreamSnapshot } from '@sharker/core/session-event-health';
import type { ThemePalette, ThemePreference } from '@sharker/core/settings';
import type { UiLocale } from '@sharker/core/ui-locale';
import { generalizedErrorMessageChinese } from '@sharker/core/redaction';
import { sessionExpectsEventStream } from '@sharker/core/session-event-health';
import { type ShellRunUpdate } from '@sharker/core/events';
import type { LiveTurnProjection, NavSelection } from '@sharker/ui';
...
export function useAppShellPersistenceEffects(options: {
  ...
  useEffect(() => {
    safeLocalStorageSet('sharker-nav-selection-v1', JSON.stringify(options.navigationState));
...
export function useAppShellBootstrapSubscriptions(options: {
  ...
  useEffect(() => {
    ...
    const unsubscribeConnections = window.sharker.connections.subscribeEvents(handleConnectionSubscriptionEvent);
    const unsubscribeRuntimeHostChanges =
      window.sharker.runtimeHostProfiles.subscribeChanges(handleRuntimeHostChange);
    const refreshRuntimeHostSettingsMirrors = () => {
      void options.refreshShellSettings();
      void options.refreshConnections();
    };
    const unsubscribeSettingsExternal = window.sharker.settings.subscribeExternalChanged(
      refreshRuntimeHostSettingsMirrors,
    );
    const unsubscribeClientSettings = window.sharker.settings.subscribeClientChanged(
      () => void options.refreshShellSettings(),
    );
    const unsubscribeSessionChanges = window.sharker.sessions.subscribeChanges(handleSessionChange);
    const unsubscribeWindowCommand = window.sharker.appWindow.subscribeCommand(handleWindowCommand);
    markRendererMounted();
    return () => {
...
export function useActiveSessionEvents(options: {
  ...
  useLayoutEffect(() => {
    ...
    const openTranscript = (signal: AbortSignal) =>
      window.sharker.transcripts.open(
        activeId,
        (batch) => {
          if (disposed) return;
          try {
    ...
    const subscribeSessionEvents = () => {
      ...
      const unsubscribe = window.sharker.sessions.subscribeEvents(
        activeId,
        (event) => {
...
export function useShellRunUpdates(options: {
  ...
  const applyUpdates = useEffectEvent(
    (sessionId: string, updates: Awaited<ReturnType<typeof window.sharker.shellRuns.list>>) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.sharker.shellRuns.subscribeUpdates((update) => {
      if (disposed) return;
    ...
    const unsubscribeResync = window.sharker.shellRuns.subscribeResync((event) => {
