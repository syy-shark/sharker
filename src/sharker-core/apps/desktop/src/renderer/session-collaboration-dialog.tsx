...
function ShareSessionDialog(props: Props) {
  ...
  async function readProjection() {
    if (props.requiresRemoteAccess) {
      const remoteAccess = await window.sharker.localRuntimeHostRemoteAccess.getSnapshot();
      if (remoteAccess.state !== 'on') return { kind: 'remote_access_off' } as const;
    }
    const [nextAccess, nextRequests] = await Promise.all([
      window.sharker.sessionCollaboration.getAccess(props.sessionId),
      window.sharker.sessionCollaboration.getTurnRequests(props.sessionId),
    ]);
    return {
      kind: 'available',
      access: nextAccess,
  ...
  async function createInvitation(allowInsecure = false): Promise<void> {
    ...
    try {
      ...
      if (props.requiresRemoteAccess) {
        const access = await window.sharker.localRuntimeHostRemoteAccess.getSnapshot();
        if (access.state !== 'on') {
          props.onEnableRemoteAccess();
          return;
        }
      }
      const created = await window.sharker.sessionCollaboration.prepareInvitation(
        props.sessionId,
        preset,
        allowInsecure,
      );
  ...
  async function revokePrincipal(principalId: string): Promise<void> {
    setWorking(true);
    try {
      await window.sharker.sessionCollaboration.revokePrincipal(props.sessionId, principalId);
      await refresh();
    } catch (error) {
      toast.error(copy.shareTitle, errorMessage(error));
    } finally {
  ...
  async function revokeGrant(grant: SessionCollaborationGrant): Promise<void> {
    setWorking(true);
    try {
      await window.sharker.sessionCollaboration.revokeGrant(
        props.sessionId,
        grant.grantId,
      );
      await refresh();
  ...
  async function decideTurnRequest(
    ...
    setWorking(true);
    try {
      await window.sharker.sessionCollaboration.decideTurnRequest(
        props.sessionId,
        request.requestId,
        decision,
      );
