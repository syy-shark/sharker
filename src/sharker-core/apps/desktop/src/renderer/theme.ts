...
// frame, same rationale as `sharker-theme-v1`.
const UI_FONT_SIZE_STORAGE_KEY = 'sharker-ui-font-size-v1';
const TERMINAL_FONT_SIZE_STORAGE_KEY = 'sharker-terminal-font-size-v1';
// The renderer type scale is generated from TYPE_SCALE_BASE_PX (the same
// constant sharkerTheme.ts feeds into expandTypeScale) and every --font-size-*
 ...
 *
 * Also persists the preference to `sharker-theme-v1` in localStorage so the
...
export function applyTheme(pref: ThemePreference): () => void {
  ...
  safeLocalStorageSet('sharker-theme-v1', pref);
  ...
  void window.sharker.appWindow.setThemeSource(pref).catch(() => {});
...
export function setTitlebarModalDimmed(dimmed: boolean): void {
  ...
  void window.sharker?.appWindow?.setTitlebarControlsVisible?.(!dimmed).catch(() => {});
  syncTitleBarOverlay(document.documentElement);
}

/**
 * PR-UI-2 (@yuejing 2026-05-22): apply a base46 palette by writing
 * `data-sharker-theme="<palette>"` on `<html>`. CSS variable overrides
 * live in `sharker-tokens.css`. `default` removes the attribute so the
 * original Sharker palette renders.
 *
...
export function applyThemePalette(palette: ThemePalette): void {
  const root = document.documentElement;
  if (palette === 'default') {
    root.removeAttribute('data-sharker-theme');
  } else {
    root.setAttribute('data-sharker-theme', palette);
  }
  safeLocalStorageSet('sharker-theme-palette-v1', palette);
...
function dimmedTitlebarColor(backgroundHex: string, isDark: boolean): string {
  // The app's theme pins the scrim to black@50%/80% (astryx-theme/sharker.css);
