...
import type { SettingsSection, ThemePreference } from '@sharker/core/settings';
...
export function createAppShellE2eFixtureActions(options: {
  ...
  async function applyE2eFixture() {
    const state = await window.sharker.e2eFixture.getState();
    if (!state) return;
    if (state.now) {
    ...
    document.documentElement.setAttribute('data-sharker-e2e-fixture', 'true');
    ...
    if (state.scrollMotion) {
      document.documentElement.setAttribute('data-sharker-scroll-motion', state.scrollMotion);
    }
    ...
    // `state` is null without `SHARKER_E2E_FIXTURE`.
    if (state.theme) {
    ...
    // null without SHARKER_E2E_FIXTURE).
    if (state.reducedMotion) {
      document.documentElement.setAttribute('data-sharker-reduced-motion', 'true');
    }
    ...
    // (e2eFixture.getState returns null without SHARKER_E2E_FIXTURE).
