...
import type { TransientUserMessageProjection } from '@sharker/ui';
import { MESSAGE_QUEUE_MAX_ENTRIES } from '@sharker/runtime-host/protocol';
  ...
  async function retireCancelledTransientMessages(sessionId: string): Promise<void> {
    ...
    try {
      ...
      for (let from = 0; from < messageIds.length; from += MESSAGE_QUEUE_MAX_ENTRIES) {
        const result = await window.sharker.sessions.queryCancelledMessages(
          sessionId,
