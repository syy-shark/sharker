...
import { isThemePalette } from '@sharker/core/settings';
...
export function applyCachedThemeBeforeMount(): void {
  const cachedThemePreference = safeLocalStorageGet('sharker-theme-v1');
  ...
  const cachedPalette = safeLocalStorageGet('sharker-theme-palette-v1');
  if (cachedPalette && isThemePalette(cachedPalette) && cachedPalette !== 'default') {
    document.documentElement.setAttribute('data-sharker-theme', cachedPalette);
  } else {
    document.documentElement.removeAttribute('data-sharker-theme');
  }
