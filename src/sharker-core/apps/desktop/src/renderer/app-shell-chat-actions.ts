...
import type { InlineReference, QuoteRef } from '@sharker/core/events';
...
import type { SkillInvocationResult } from '@sharker/runtime/skill-invocation';
...
export function createAppShellChatActions(deps: {
  ...
  function showTransientUserMessage(
    ...
    messageId: string,
    text: string,
    attachments: readonly import('@sharker/core/events').AttachmentRef[] = [],
    options: {
  ...
  async function submitAndProject(input: {
    ...
    placement: 'current_turn' | 'next_turn';
    command: Omit<
      Parameters<typeof window.sharker.sessions.submitMessage>[2],
      'messageId'
    >;
    displayText?: string;
    quotes?: readonly QuoteRef[];
    ...
    const result = await window.sharker.sessions.submitMessage(sessionId, placement, {
      ...input.command,
      messageId,
    });
  ...
  async function send(
    ...
    const discardUnsentSession = async () => {
      ...
      try {
        await window.sharker.sessions.remove(sessionId);
    ...
    try {
      ...
      if (!initialSessionId) {
        ...
        const session = await window.sharker.newTasks.create(initialNewTaskTarget, {
        ...
        if (newChatAskMode) {
          await window.sharker.sessions.setPermissionMode(session.id, 'explore');
          clearNewChatAskMode();
        }
        ...
        // the new-chat surface, so the empty-session Sharker hero cannot paint
  ...
  async function respondToSandboxBoundary(response: SandboxBoundaryResponse) {
    ...
    if (!sessionId) return;
    try {
      await window.sharker.sessions.respondToSandboxBoundary(sessionId, response);
  ...
  async function respondToUserQuestion(response: UserQuestionResponse) {
    ...
    if (!sessionId) return;
    try {
      await window.sharker.sessions.respondToUserQuestion(sessionId, response);
      onInteractionChanged?.(sessionId);
