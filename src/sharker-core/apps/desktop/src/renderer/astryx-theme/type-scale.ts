...
// The renderer type-scale base, in px. `sharkerTheme.ts` (THE type-scale
...
// module rather than in `sharkerTheme.ts` so the pre-paint bootstrap path
// (`cached-theme-bootstrap.ts` → `theme.ts`) does not pull the full theme
// (icon registry included) into the first frame.
//
// `DEFAULT_UI_FONT_SIZE` in `@sharker/core/settings` must equal this value —
