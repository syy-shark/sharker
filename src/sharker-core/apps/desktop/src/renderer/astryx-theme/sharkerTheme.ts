...
// Sharker's Astryx theme (#1565 PR 3). An `extends` of the neutral default theme
...
// Sharker's product density genuinely requires: the type scale (see below). The
...
// it at the source.
//
// Regenerate the sharker.css / sharker.js artifacts with:
...
export const sharkerTheme = defineTheme({
  name: 'sharker',
  ...
  // sharker.js ships no icons and every semantic icon in Astryx components
  // silently falls back.
  icons: neutralIconRegistry,
  // THE type-scale authority for the whole renderer.
  //
  // Sharker is denser than Astryx's neutral default (`{base: 14, ratio: 1.2}`).
  // That density used to be expressed as `html { font-size: 13px }` in
  // sharker-tokens.css, which is not a type scale at all but an implicit ×0.8125
  ...
  // base × ratio^n), and the four product tiers in sharker-tokens.css alias its
  ...
  // never as low as the 9.75px Sharker had. 1.125 keeps 11 and 20 on the ladder
  // while moving
  ...
  // leads with Figtree, which Sharker does not bundle, so every Astryx surface
  // (Markdown prose included) declared a family that silently fell back —
  // and its stack carries no CJK face, while Sharker is CJK-first. Declaring the
  ...
  // are gone, and sharker-tokens.css reads these directly (#1875).
  typography: {
  ...
  // palette.
  //
  // Astryx ships these as static light-dark() pairs, which cannot follow Sharker's
  ...
  // opposite reason: Astryx's scale covers everything Sharker needs from type,
  ...
  // and ~0.010 against Sharker's 0.045). The line states the boundary; the wash
