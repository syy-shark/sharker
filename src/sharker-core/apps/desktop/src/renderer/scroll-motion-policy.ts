 ...
 *   1. `data-sharker-reduced-motion="true"` on the document root — set by
 *      the PR-IR-04 reduced variant of the e2e-fixture fixture.
 *   2. `data-sharker-e2e-fixture="true"` on the document root — set by
...
export interface ScrollMotionPolicyInputs {
  /** `document.documentElement.dataset.sharkerReducedMotion === 'true'` */
  reducedMotionAttr: boolean;
  /** `document.documentElement.dataset.sharkerE2eFixture === 'true'` */
  e2eFixtureAttr: boolean;
  /** `window.matchMedia('(prefers-reduced-motion: reduce)').matches` */
  prefersReducedMotion: boolean;
  /**
   * `document.documentElement.dataset.sharkerScrollMotion`, set by a fixture that
...
export function readScrollMotionBehavior(): ScrollMotionBehavior {
  ...
  const prefersReducedMotion =
    ...
    window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const requested = root.dataset.sharkerScrollMotion;
  return resolveScrollMotionBehavior({
    reducedMotionAttr: root.dataset.sharkerReducedMotion === 'true',
    e2eFixtureAttr: root.dataset.sharkerE2eFixture === 'true',
