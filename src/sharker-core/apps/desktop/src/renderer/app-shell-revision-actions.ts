...
import type { ComposerHandle } from '@sharker/ui';
...
export function createAppShellRevisionActions(deps: {
  ...
  async function abandonRevisionCopy(
    ...
    try {
      ...
      await window.sharker.sessions.abandonSessionCopy(draft.sourceSessionId, draft.copyId);
  ...
  async function prepareRevisionSend(text: string): Promise<boolean> {
    ...
    try {
      const newSession = await window.sharker.sessions.reviseBeforeTurn(sourceSessionId, {
