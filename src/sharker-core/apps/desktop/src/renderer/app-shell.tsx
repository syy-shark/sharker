...
import type { ChatDefaultPermissionMode } from '@sharker/core/settings';
import type { SlashCommandIdForSurface } from '@sharker/core/slash-command-catalog';
import type { UiLocale, UiLocalePreference } from '@sharker/core/ui-locale';
import { collapseSessionRevisions } from '@sharker/core/session-revisions';
import { isLinkedSubagentSession } from '@sharker/core/session';
import { resolveUiLocale } from '@sharker/core/ui-locale';
import { slashCommandsForSurface } from '@sharker/core/slash-command-catalog';
import { hasSettledInitialOnboarding } from '@sharker/core/onboarding-milestone';
import {
  ChatSurfaceLayout,
  type ComposerHandle,
...
import { MessageCircleQuestion, Minimize2 } from '@sharker/ui/icons';
...
export function AppShell({ initialOnboardingSnapshot = null }: AppShellProps = {}) {
  ...
  const errorToastAction = useMemo<ToastErrorAction>(
    () => ({
      ...
      onClick: (input) => window.sharker.diagnostics.copyReport({
        surface: 'toast',
        title: input.title,
...
function AppShellContent({
  ...
  const {
    ...
    toastApi,
    service: window.sharker.attachments,
  });
  const {
    pendingQuotes,
    addQuote,
  ...
  useEffect(() => {
    ...
    return startWorkHubCoordinationLifecycle({
      resolve: () => window.sharker.workHub.resolveCoordinationSession(),
      subscribeHostChanges: (handler) =>
        window.sharker.runtimeHostProfiles.subscribeChanges(handler),
      subscribeAvailabilityChanges: (handler) =>
        window.sharker.connections.subscribeEvents((event) => {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const enabled = (await window.sharker.settings.getClient()).workHub.enabled;
        if (disposed) return;
    ...
    void refresh();
    const unsubscribe = window.sharker.settings.subscribeClientChanged(() => void refresh());
    return () => {
      disposed = true;
      unsubscribe();
    };
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void window.sharker.diagnostics
      ...
      .then((interrupted) => {
        ...
        toastApi.toast({
          ...
          action: {
            ...
            onClick: () =>
              window.sharker.diagnostics.copyPreviousMainProcessInterruption(),
          },
        });
      })
      .catch((error) =>
  ...
  useEffect(() => {
    ...
    const unsubscribeUpdateStatus = window.sharker.app.subscribeUpdateStatus((next) => {
      receivedPush = true;
      if (!cancelled) setAppUpdateStatus(next);
    });
    void window.sharker.app
      .updateStatus()
      .then((next) => {
  ...
  const openUpdateDownload = useCallback(() => {
    if (updateReminder?.state === 'downloaded') {
      ...
      void requestDownloadedAppUpdate({
        installUpdate: (input) => window.sharker.app.installUpdate(input),
  ...
  const orchestrationModeIntent = useSessionSettingIntent<OrchestrationMode>({
    ...
    write: async (sessionId, mode) => {
      await window.sharker.sessions.setOrchestrationMode(sessionId, mode);
      return true;
    },
    refreshCatalog: refreshSessions,
    onWriteError: (sessionId, error) => {
  ...
  async function commitPlanMode(sessionId: string, active: boolean): Promise<boolean> {
    const planState = await window.sharker.sessions.getPlanState(sessionId);
    ...
    if (!active && latestProposal?.status === 'pending_approval') {
      ...
      await window.sharker.sessions.abandonPlanProposal(sessionId, latestProposal.proposalId);
    } else {
      await window.sharker.sessions.setCollaborationMode(
        sessionId,
        active ? 'plan' : 'agent',
      );
    }
  ...
  // e2e-fixture always writes `data-sharker-e2e-fixture="true"` but
  // `data-sharker-reduced-motion="true"` is only set on the reduced
  ...
  // `auto`:
  //   1. `data-sharker-reduced-motion="true"` — PR-IR-04 reduced variant
  //   2. `data-sharker-e2e-fixture="true"` — PR-IR-02 any capture
   ...
   *  invocation. Same human-in-the-loop rule as sharker://compose — we never
  ...
  useEffect(
    () =>
      window.sharker.sessions.subscribeActiveInteractions(({ sessionId, interactions }) => {
  ...
  const desktopSlashCommands = useMemo<readonly ComposerSlashCommandOption[]>(
    () => {
      ...
      const availableCommands = slashCommandsForSurface('desktop').filter(
        ({ id }) => Boolean(activeId) && !(streaming && id === 'compact'),
      );
  ...
  const workHubController = useMemo(
    () => createWorkHubController({
      coordination: createDesktopWorkHubCoordinationPort({
        ...
        transcripts: window.sharker.transcripts,
        record: (input) =>
          window.sharker.workHub.record(workHubCoordinationSessionId!, input),
        candidates: () =>
          window.sharker.workHub.candidates(workHubCoordinationSessionId!),
        act: (input) =>
          window.sharker.workHub.act(workHubCoordinationSessionId!, input),
      }),
      sessions: createDesktopWorkHubSessionPort({
        sessions: scopeWorkHubSessionsToCoordinationHost(
          window.sharker.sessions,
          {
  ...
  async function sendWithAttachments(
    ...
    if (slashCommand?.kind === 'compact') {
      ...
      try {
        const result = await window.sharker.sessions.compact(sessionId);
  ...
  async function updateQueuedEntry(
    ...
    await runQueueEntryAction((sessionId) =>
      window.sharker.sessions.updateQueueEntry(sessionId, entryId, expectedQueueRevision, text)
    );
  }
  ...
  async function deleteQueuedEntry(entryId: string): Promise<void> {
    ...
    const sessionId = await runQueueEntryAction((sessionId) =>
      window.sharker.sessions.retractQueueEntry(sessionId, entryId).then(() => undefined)
    );
  ...
  async function promoteQueuedEntry(entryId: string): Promise<void> {
    await runQueueEntryAction((sessionId) =>
      window.sharker.sessions.promoteQueueEntry(sessionId, entryId).then(() => undefined)
    );
  }

  async function reorderQueuedEntries(entryIds: readonly string[]): Promise<void> {
    await runQueueEntryAction((sessionId) =>
      window.sharker.sessions.reorderQueueEntries(sessionId, entryIds).then(() => undefined)
    );
  }
  ...
  const {
    ...
    notifyRunEnded: ({ kind, sessionId, body }) => {
      ...
      void window.sharker.notifications.runEnded({ kind, title, body }).catch(() => {});
    },
  });
  ...
  useEffect(() => {
    ...
    void window.sharker.sessions.listTurnLandmarks(sessionId).then(
      (snapshot) => {
   ...
   * router. Receives a typed `SharkerUriDest` from the link override in
   ...
   *     keeps an injected `sharker://compose?text=ransfer my keys...`
   *     from sending without a human in the loop.
   *
   * No other cases exist today by design — the parser only emits
   * these two discriminants. If a new variant is added in `SharkerUriDest`,
   ...
   */
  function dispatchSharkerUri(dest: SharkerUriDest) {
    switch (dest.kind) {
      case 'settings':
    ...
    <div
      ...
      style={
        sessionListCollapsed
          ? undefined
          : ({
              '--sharker-sidenav-width': `${sessionListWidth}px`,
            } as CSSProperties)
      }
    >
      <LiveTurnReconciler
      ...
      <header
        className="sharker-window-titlebar"
      ...
      <AstryxAppShell
        ...
        /* Astryx's default: nav column takes --color-background-body, content takes
           ...
           sharkerTheme.ts, so the shell follows a palette switch. Declared rather
      ...
      >
        <AppShellDetailPanel agentsView={agentsView}>
          {/* PR-UI-RENDER-2: install the internal-URI dispatcher
              ...
              About settings) doesn't auto-route sharker:// links,
              which is correct: those surfaces shouldn't be a
              navigation entry point. */}
          <SharkerUriContext.Provider value={dispatchSharkerUri}>
          <div className="sharker-detail-with-artifacts">
            <div className="mainColumn" data-home-surface={homeSurfaceActive ? 'true' : undefined}>
              ...
              <ChatSurfaceLayout
                ...
                scrollOwner="host"
                data-sharker-onboarding={showOnboardingHero ? 'true' : undefined}
                scrollToBottomLabel={
                ...
                composer={
                  ...
                  // hint reads "Sharker 正在处理…"; in a mid-turn lull it reads the calm
                  // "Sharker 继续中…". Both are mutually exclusive with activeStreamingLive.
              ...
              >
                ...
                onReadAttachmentBytes={window.sharker.attachments.readBytes}
                ...
                onSkip={async () => {
                  try {
                    await runOnDefaultRuntimeHost((host) =>
                      window.sharker.onboarding.setMilestone(
                        'initial_onboarding',
                        'skipped',
                        host,
                      ),
