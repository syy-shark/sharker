...
import type { TurnFooterActionMeta } from '@sharker/ui';
  ...
  async function handleTurnFooterAction(turnId: string, actionId: TurnFooterActionMeta['id']): Promise<void> {
    ...
    try {
      if (actionId === 'regenerate') {
        await window.sharker.sessions.regenerateTurn(sessionId, {
          sourceTurnId: turnId,
        });
        ...
        const newSession = await window.sharker.sessions.branchFromTurn(sessionId, {
