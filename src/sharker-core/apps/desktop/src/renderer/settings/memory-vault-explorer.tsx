 ...
 * 读写走 `window.sharker.memory.*Vault*`，与助手的 SharkerMemoryVault* 工具同一套路径。
 */
import { useEffect, useMemo, useState } from 'react';
import { Button, EmptyState, RelativeTime, TextArea, useToast, useUiLocale } from '@sharker/ui';
import { MarkdownBody } from '@sharker/ui';
import { ChevronDown, ChevronRight, FileText, FolderOpen, ICON_SIZE } from '@sharker/ui/icons';
import type { MemoryVaultDirNode, MemoryVaultNode } from '@sharker/core/local-memory-vault';
import { todayEpisodicPath } from '@sharker/core/local-memory-vault';
...
export function MemoryVaultExplorer() {
  ...
  async function refreshTree(selectPath?: string) {
    const result = await window.sharker.memory.listVault();
    if (!result.ok) {
  ...
  async function openFile(path: string) {
    const result = await window.sharker.memory.readVaultFile(path);
    if (!result.ok) {
      toast.error(copy.loadFailed, result.message);
      return;
    }
  ...
  async function saveFile() {
    ...
    setBusy(true);
    try {
      const result = await window.sharker.memory.writeVaultFile(selectedPath, content);
      if (!result.ok) {
  ...
  async function createTodayFile() {
    ...
    setBusy(true);
    try {
      const existing = await window.sharker.memory.readVaultFile(path);
      if (existing.ok) {
        setOpenFolders((current) => new Set([...current, 'episodic']));
        await openFile(path);
        return;
      }
      const result = await window.sharker.memory.writeVaultFile(path, `# ${path.slice('episodic/'.length).replace(/\.md$/, '')}\n\n`);
      if (!result.ok) {
        toast.error(copy.saveFailed, result.message);
        return;
      }
  ...
  async function removeSelected() {
    ...
    setBusy(true);
    try {
      const result = await window.sharker.memory.deleteVaultFile(selectedPath);
      if (!result.ok) {
