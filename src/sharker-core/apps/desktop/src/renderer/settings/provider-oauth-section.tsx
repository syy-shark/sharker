...
function SubscriptionLoginPanel(props: {
  ...
  const flow = useOAuthLoginFlow({
    bridge: runtimeHostOAuthLoginBridge(
      isXai ? window.sharker.xaiOAuth : window.sharker.openAiCodex,
      host,
    ),
...
function GitHubCopilotLoginPanel(props: { onLoginSuccess(): void | Promise<void> }) {
  ...
  const flow = useOAuthLoginFlow({
    bridge: {
      getAccountState: () => window.sharker.githubCopilotSubscription.getAccountState(host),
      logout: () => window.sharker.githubCopilotSubscription.logout(host),
    } as OAuthLoginFlowBridge,
    display: { name: 'GitHub Copilot', shortName: 'GitHub Copilot' },
    onLoginSuccess: props.onLoginSuccess,
    direct: {
      login: () => window.sharker.githubCopilotSubscription.connectExistingLogin(host),
      refreshTokens: () => window.sharker.githubCopilotSubscription.refreshTokens(host),
    },
  });
...
async function getSubscriptionSnapshot(
  ...
  if (serviceId === 'github-copilot') {
    return window.sharker.githubCopilotSubscription.getAccountState(host);
  }
  if (serviceId === 'xai') {
    return window.sharker.xaiOAuth.getAccountState(host);
  }
  return (await window.sharker.openAiCodex.getAccountState(host)) as SubscriptionSnapshot;
}
