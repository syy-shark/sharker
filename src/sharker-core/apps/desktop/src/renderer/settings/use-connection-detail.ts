...
import { PROVIDER_DEFAULTS, connectionEnabledModelIds } from '@sharker/core/llm-connections';
import { buildConnectionModelCatalogEntries } from '@sharker/core/model-catalog';
...
export function oauthLoginServiceFor(
  ...
  switch (providerType) {
    case 'openai-codex':
      return {
        bridge: runtimeHostOAuthLoginBridge(window.sharker.openAiCodex, host, connectionId),
        display: { name: 'OpenAI Codex', shortName: 'Codex' },
        showsDeviceCode: true,
      };
    case 'xai-oauth':
      return {
        bridge: runtimeHostOAuthLoginBridge(window.sharker.xaiOAuth, host, connectionId),
...
export function useConnectionDetail(props: ConnectionDetailProps) {
  ...
  async function save(field: 'key' | 'endpoint' | 'name'): Promise<boolean> {
    ...
    try {
      ...
      // glm-4.7 / 4.6 / 4.5), which looks like Sharker doesn't support newer
   ...
   * serves but Sharker has never heard of has no other way in (#1584).
   *
