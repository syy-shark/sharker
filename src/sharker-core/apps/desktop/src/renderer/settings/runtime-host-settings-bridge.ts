...
export function runtimeHostConnectionsBridge(
  ...
  return {
    getSnapshot: () => window.sharker.connections.getSnapshot(undefined, host),
    setDefault: (slug) => window.sharker.connections.setDefault(slug, host),
    setDefaultModel: (input) => window.sharker.connections.setDefaultModel(input, host),
    create: (input) => window.sharker.connections.create(input, host),
    update: (slug, patch) => window.sharker.connections.update(slug, patch, host),
    delete: (slug) => window.sharker.connections.delete(slug, host),
    test: (slug, options) => window.sharker.connections.test(slug, options, host),
    fetchModels: (slug) => window.sharker.connections.fetchModels(slug, host),
    hasSecret: (slug) => window.sharker.connections.hasSecret(slug, host),
    getRequestHeaders: (slug) => window.sharker.connections.getRequestHeaders(slug, host),
    setRequestHeaders: (slug, headers) =>
      window.sharker.connections.setRequestHeaders(slug, headers, host),
    subscribeEvents: (handler) =>
      window.sharker.connections.subscribeEvents(handler, host),
  };
}

export function runtimeHostOAuthLoginBridge(
  bridge: typeof window.sharker.openAiCodex | typeof window.sharker.xaiOAuth,
  host: DesktopRuntimeHostRef,
  connectionId?: string,
