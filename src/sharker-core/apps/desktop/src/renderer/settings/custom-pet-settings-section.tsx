...
import { Badge, Button, useMountedRef, useToast, useUiLocale } from '@sharker/ui';
...
export function CustomPetSettingsSection() {
  ...
  const refreshLibrary = useCallback(async (options?: {
    ...
    try {
      const [pets, selectedPetId] = await Promise.all([
        window.sharker.pets.list(),
        window.sharker.pets.getSelection(),
      ]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.sharker.pets.subscribeChanges(() => {
  ...
  async function importPet() {
    ...
    setMutation('import');
    try {
      const result = await window.sharker.pets.importLocalDirectory();
  ...
  async function selectPet(petId: string | null) {
    ...
    try {
      const result = await window.sharker.pets.select(petId);
      if (!mountedRef.current) return;
      if (!result.ok) {
  ...
  async function requestRemovePet(petId: string) {
    ...
    try {
      const result = await window.sharker.pets.remove(petId);
      if (!mountedRef.current) return;
      if (!result.ok) {
