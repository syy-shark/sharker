...
import { Banner, Button, useUiLocale } from '@sharker/ui';
...
export function RuntimeHostSshTerminalDialog() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.sharker.runtimeHostSshTerminal.subscribe((event) => {
      if (hydrating) {
        pending.push(event);
        return;
      }
    ...
    void window.sharker.runtimeHostSshTerminal.getSnapshot().then(
      (snapshot) => {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      if (sessionId) {
        void window.sharker.runtimeHostSshTerminal
  ...
  function onOpenChange(open: boolean) {
    if (!open) {
      const sessionId = sessionIdRef.current;
      if (sessionId) {
        void window.sharker.runtimeHostSshTerminal.cancel(sessionId).catch(() => undefined);
      }
    }
    if (!open) {
      sessionIdRef.current = undefined;
