...
import type { WebSearchCredentialStatus } from '@sharker/core/web-search';
import { normalizeSearchUrl } from '@sharker/core/search';
import { webSearchCredentialStatusFromResponse } from '@sharker/core/web-search';
import { Button, Selector, StatusDot, TextInput, RelativeTime, Switch, redactSecrets, useMountedRef, useToast, useUiLocale } from '@sharker/ui';
...
import { dotForStatus, type StatusSemantic } from '@sharker/ui';
...
export function WebSearchSettingsPage(props: {
  ...
  async function updateWebSearch(
    patch: NonNullable<Parameters<typeof window.sharker.settings.update>[0]['webSearch']>,
  ...
  async function runTest() {
    ...
    try {
      const result = await window.sharker.webSearch.test({
  ...
  async function runLiveQuery() {
    ...
    try {
      const result = await window.sharker.webSearch.query({
        provider: webSearch.defaultProvider,
        query: trimmed,
        limit: 5,
      }, host);
