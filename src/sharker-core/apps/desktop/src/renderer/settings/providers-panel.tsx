...
import { ICON_SIZE, ChevronRight, Cpu } from '@sharker/ui/icons';
import {
  type LlmConnection,
  type ProviderType,
} from '@sharker/core/llm-connections';
import { dotForStatus, useMountedRef, useUiLocale } from '@sharker/ui';
...
export function ProvidersPanel({ bridge, initialPage = 'connections', initialConnectionSlug, initialCreateProviderType, onInitialCatalogConsumed, onInitialCreateProviderConsumed }: {
  ...
  useSettingsRouteFocus({
    ...
    resolveTarget: (current) => {
      ...
      if (current === 'catalog') return find('[data-sharker-contract="provider-catalog"] input');
      if (current === 'setup') {
        return find('[data-sharker-contract="provider-setup"] input')
          ?? find('[data-sharker-contract="provider-setup"]');
      }
      ...
      if (current === 'detail') return find('[data-sharker-contract="connection-detail"]');
  ...
  if (loading) {
    return (
      <VStack className="providersPanel" gap={6} data-sharker-contract="providers-panel" aria-busy="true" aria-label={copy.loadingAria}>
        <VStack gap={1.5}>
  ...
  return (
    <VStack className="providersPanel" gap={2} data-sharker-contract="providers-panel">
      {level === 'detail' && selected ? (
        ...
        (<VStack gap={5} tabIndex={-1} role="region" aria-labelledby={detailTitleId} className="settingsRouteLevel" data-sharker-contract="connection-detail">
          <SettingsRouteHeader
            onBack={goToList}
        ...
        <>
          <HStack gap={2} vAlign="center" hAlign="between">
            ...
            <Button
              ...
              onClick={openCatalog}
              data-sharker-contract="add-connection"
            />
          </HStack>
          {loadError ? (
            <Banner
