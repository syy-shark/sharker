...
import { isRelayProviderType, PROVIDER_DEFAULTS } from '@sharker/core/llm-connections';
import { hasModelMetadata } from '@sharker/core/model-metadata';
import {
...
export function ConnectionDetail(props: ConnectionDetailProps) {
  ...
  // Mirrors `isRealConnection` in @sharker/core/connection-readiness.ts.
...
function ConnectionDetailInner(props: ConnectionDetailProps) {
  ...
  // A model gets capability switches when Sharker cannot describe it otherwise.
      ...
      <DetailSection title={copy.modelManagement} description={copy.modelManagementHelp}>
        ...
        <AddModelDialog
          ...
          /* The catalog, not just the selection: `models` is usually a proper
             ...
             hand-typed context window that overrides the one Sharker already
             knows. */
...
function GitHubCopilotReloginNoticeForCurrentGeneration(props: {
  ...
  async function connect() {
    ...
    try {
      const result = await window.sharker.githubCopilotSubscription.connectExistingLogin(host);
