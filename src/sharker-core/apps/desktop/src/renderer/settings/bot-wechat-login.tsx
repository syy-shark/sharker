...
import type { WechatBridgeQrCodeResult } from '@sharker/runtime/bots';
import { Button, EmptyState, FormLayout, Spinner, TextInput, useUiLocale, Banner } from '@sharker/ui';
import { ICON_SIZE, MessageSquare } from '@sharker/ui/icons';
...
export function WechatQrLoginModal(props: {
  ...
  useEffect(() => {
    ...
    loadingQrRef.current = true;
    setLoading(true);
    void window.sharker.settings.bots.wechatQrCode()
      .then((next) => {
        if (!active) return;
        setResult(next);
