...
export function useMemoryDocumentController(props: MemoryDocumentControllerProps) {
  ...
  async function reload(): Promise<boolean> {
    ...
    try {
      const next = await window.sharker.memory.getState(undefined, host);
  ...
  async function setEnabled(enabled: boolean) {
    try {
      await runMemoryWriteAction('enable', async (isCurrent) => {
        const next = await window.sharker.memory.setEnabled(enabled, host);
  ...
  async function setAgentReadEnabled(agentReadEnabled: boolean) {
    try {
      await runMemoryWriteAction('agent-read', async (isCurrent) => {
        const next = await window.sharker.memory.setAgentReadEnabled(agentReadEnabled, host);
        await props.onReloadSettings();
        if (!isCurrent()) return;
        setState(next);
        setDraft(next.content);
  ...
  async function save() {
    try {
      await runMemoryWriteAction('save', async (isCurrent) => {
        const next = await window.sharker.memory.save(draft, host);
        if (!isCurrent()) return;
  ...
  async function reset() {
    try {
      await runMemoryWriteAction('reset', async (isCurrent) => {
        const next = await window.sharker.memory.reset(host);
        if (!isCurrent()) return;
        setState(next);
        setDraft(next.content);
        setLastSaveSummary(null);
    ...
    await runMemoryAction('backup:latest:restore', async () => {
      try {
        await runMemoryWriteAction('restore', async (isCurrent) => {
          ...
          if (!ok) return;
          if (!isCurrent()) return;
          const result = await window.sharker.memory.restoreLatestBackup(host);
          if (!isCurrent()) return;
    ...
    await runMemoryAction(`backup:${backup.kind}:restore`, async () => {
      try {
        await runMemoryWriteAction('restore', async (isCurrent) => {
          ...
          if (!ok) return;
          if (!isCurrent()) return;
          const result = await window.sharker.memory.restoreBackup(backup.kind, host);
          if (!isCurrent()) return;
          setState(result.state);
          setDraft(result.state.content);
          setLastSaveSummary(null);
  ...
  async function openFile() {
    await runMemoryAction('memory:file:open', async (isCurrent) => {
      try {
        const result = await window.sharker.memory.openFile(host);
        if (!isCurrent()) return;
        if (!result.ok) {
          reportHostError(
  ...
  async function openLatestBackup() {
    await runMemoryAction('backup:latest:open', async (isCurrent) => {
      try {
        const result = await window.sharker.memory.openLatestBackup(host);
        if (!isCurrent()) return;
        if (!result.ok) {
          reportHostError(
  ...
  async function openBackupCandidate(backup: NonNullable<LocalMemoryState['latestBackup']>) {
    await runMemoryAction(`backup:${backup.kind}:open`, async (isCurrent) => {
      try {
        const result = await window.sharker.memory.openBackup(backup.kind, host);
        if (!isCurrent()) return;
        if (!result.ok) {
          reportHostError(
  ...
  async function openFolder() {
    await runMemoryAction('memory:folder:open', async (isCurrent) => {
      try {
        const result = await window.sharker.app.openPath('memory', undefined, host);
        if (!isCurrent()) return;
        if (!result.ok) {
          reportHostError(
  ...
  async function addManualMemoryEntry() {
    ...
    try {
      await runMemoryWriteAction('save', async (isCurrent) => {
        ...
        const next = await window.sharker.memory.save(result.draft, currentHost);
        if (!isCurrent()) return;
        setState(next);
        setDraft(next.content);
        if (next.status === 'safe_mode') {
  ...
  async function updateMemoryEntryStatus(
    ...
    try {
      await runMemoryWriteAction('entry-status', async (isCurrent) => {
        const next = await window.sharker.memory.save(result.draft, host);
        if (!isCurrent()) return;
        setState(next);
        setDraft(next.content);
        if (next.status === 'safe_mode') {
