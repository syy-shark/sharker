...
export function useAppShellProjectContext(options: {
  ...
  const refreshDefaultProjectState = async (
    ...
    const { snapshot, info } = await window.sharker.projects.getDefaultContext(host);
    if (
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.sharker.projects.subscribeChanges(refresh);
    refresh();
    return () => {
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      ...
      return window.sharker.projects.getLocalSnapshot().then(
        (snapshot) => {
    ...
    const unsubscribe = window.sharker.projects.subscribeLocalChanges(() => void refresh());
    void refresh();
    return () => {
      cancelled = true;
      unsubscribe();
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      const generation = ++refreshGeneration;
      return window.sharker.projects.getSnapshot(sessionId).then(
        (snapshot) => {
          if (cancelled || generation !== refreshGeneration) return;
    ...
    const unsubscribe = window.sharker.projects.subscribeChanges(() => void refresh(), sessionId);
    void refresh();
    return () => {
      cancelled = true;
      unsubscribe();
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void window.sharker.app.sessionProjectInfo(sessionId).then(
      (info) => {
