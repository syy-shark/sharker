...
import { sharkerTheme } from './astryx-theme/sharker';
...
export function App({
  ...
  // `.sharker-preload` skeleton before React paints. A layout effect is too early
  ...
  // `window.sharker` is undefined outside Electron (storybook), so guard it.
  useEffect(() => {
    let secondFrame = 0;
    const firstFrame = requestAnimationFrame(() => {
      secondFrame = requestAnimationFrame(() => {
        void window.sharker?.appWindow?.notifyRendererReady?.();
      });
    });
