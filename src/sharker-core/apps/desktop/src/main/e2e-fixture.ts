...
import type { E2eFixtureScenario, E2eFixtureState } from '@sharker/core/e2e-fixture';
import { MODEL_CALL_ATTEMPT_EVENT_TYPE } from '@sharker/core/model-call-attempt';
...
export function resolveE2eFixture(
  ...
  if (isPackaged) throw new Error('SHARKER_E2E_FIXTURE is only available in dev/test builds.');
  if (!E2E_FIXTURE_SCENARIOS.has(rawScenario as E2eFixtureScenario)) {
    throw new Error(`Unknown SHARKER_E2E_FIXTURE scenario: ${rawScenario}`);
  }
