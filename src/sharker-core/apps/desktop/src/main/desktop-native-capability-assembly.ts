...
import { createComputerUseOverlayHook } from '@sharker/computer-use';
...
export function assembleDesktopNativeCapabilities(
  ...
  const computerUseHost = createComputerUseHost({
    ...
    ...(deps.isComputerUseRealModelE2e
      ? {
          onTrace: (event) => {
            const tracePath = process.env.SHARKER_CU_REAL_MODEL_TRACE;
            if (!tracePath) return;
  ...
  const computerUseTools = applyComputerUseRealModelPolicy(
    computerUse.tools,
    deps.isComputerUseRealModelE2e
      ? parseComputerUseRealModelPolicy(process.env.SHARKER_CU_REAL_MODEL_POLICY)
      : undefined,
  );
