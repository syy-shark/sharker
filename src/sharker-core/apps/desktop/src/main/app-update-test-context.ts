...
export function resolveUpdateFeedOverride(
  ...
  try {
    ...
    throw new TypeError(`SHARKER_UPDATE_TEST_FEED is not a URL: ${JSON.stringify(raw)}`);
  }
  if (
    url.protocol !== 'http:' ||
    ...
    throw new TypeError(
      'SHARKER_UPDATE_TEST_FEED must be http://127.0.0.1:<port>[/path] ' +
 ...
 * test-only successor carries `sharkerUpdateTestProfile: true` in package.json and
...
export function resolveUpdateTestUserDataDirectory({
  ...
  if (explicitDirectory) {
    if (!resolveUpdateFeedOverride(feedUrl)) {
      throw new TypeError('SHARKER_UPDATE_TEST_USER_DATA_DIR requires SHARKER_UPDATE_TEST_FEED');
    }
    if (!isAbsolute(explicitDirectory)) {
      throw new TypeError('SHARKER_UPDATE_TEST_USER_DATA_DIR must be an absolute path');
    }
  ...
  if (
    ...
    !('sharkerUpdateTestProfile' in manifest) ||
    manifest.sharkerUpdateTestProfile !== true
  ) {
    return undefined;
  }
  const bundle = dirname(dirname(dirname(executablePath)));
  if (basename(bundle) !== 'Sharker.app') {
    throw new TypeError(`Update-test executable is not inside Sharker.app: ${executablePath}`);
  }
  return join(dirname(bundle), '.sharker-update-test-user-data');
}
