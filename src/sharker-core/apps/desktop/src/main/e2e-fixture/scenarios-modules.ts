  ...
  const config = {
    ...
    mcpServers: {
      filesystem: {
        enabled: false,
        command: 'npx',
        args: ['-y', '@modelcontextprotocol/server-filesystem', '/workspace/sharker'],
      },
      'linear-remote': {
        enabled: false,
 ...
 * ~/.sharker/skill-sources; the dev-gated SHARKER_SKILL_SOURCES_ROOT override
...
export async function seedSkillsMarketFixture(workspaceRoot: string): Promise<void> {
  const sourcesRoot = join(workspaceRoot, '.sharker', 'skill-sources');
  process.env.SHARKER_SKILL_SOURCES_ROOT = sourcesRoot;
