...
/** True when `target` is `root` or a path contained by `root` (after the caller realpaths both). */
export function isInsideOrSamePath(root: string, target: string): boolean {
