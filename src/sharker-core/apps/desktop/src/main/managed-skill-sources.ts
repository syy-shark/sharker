...
import { isPathInside, isSafeSkillId } from '@sharker/runtime/path-containment';
...
export { listManagedSkillSources, readManagedSkillSource, resolveManagedSkillSourcesRoot, toManagedSkillSourceEntry } from '@sharker/runtime/skills';
export type { ManagedSkillSourceRecord } from '@sharker/runtime/skills';
...
async function writeContainedBufferFile(
  ...
  const tempPath = join(rootDir, `.sharker-source-write.${process.pid}.${Date.now()}.tmp`);
  try {
