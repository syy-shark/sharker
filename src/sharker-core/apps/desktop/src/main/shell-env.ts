...
// variables remain owned by the process that launched Sharker.
//
 ...
 * - `SHARKER_SKIP_SHELL_ENV=1` is set (escape hatch for CI / debugging)
...
export async function resolveShellEnv(timeoutMs = DEFAULT_TIMEOUT_MS): Promise<void> {
  ...
  if (process.env.SHARKER_SKIP_SHELL_ENV === '1') return;
...
export function buildCaptureCommand(
  ...
  // let shell startup files inject Sharker's test, debug, and renderer controls
  ...
  // unsupported (not a Sharker audience) and falls through here — the previous
