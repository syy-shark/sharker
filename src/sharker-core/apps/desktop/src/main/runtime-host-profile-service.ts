...
import { runtimeHostAccessCredentialFingerprint } from "@sharker/runtime-host/operator";
import type { CredentialStore } from "@sharker/storage/credential-store";
import { withFileUpdateLock } from "@sharker/storage/file-update-lock";
import type {
...
export function createDesktopRuntimeHostProfileService(input: {
  ...
  const assertPreferencesWritable = (): void => {
    ...
    if (input.startup.preferencesReadFailure) {
      throw new Error(
        "Saved Runtime Host settings could not be read; restart Sharker before changing them",
