...
import type { SharkerCuBackendOptions } from '@sharker/computer-use';
import type { SharkerCuServiceSnapshot } from '@sharker/computer-use';
import {
  selectComputerUseBackend,
  type SelectedComputerUseBackend,
} from '@sharker/computer-use';
import type { CuOverlayHook } from '@sharker/runtime/computer-use-types';
...
export function createComputerUseHost(input: {
  ...
  const binaryPath = input.binaryPath ?? (input.isPackaged
    ? join(input.resourcesPath, 'bin', 'sharker-cu')
    : resolve(
        dirname(fileURLToPath(import.meta.url)),
        '..',
        '..',
  ...
  try {
    ...
    const expectedBinarySha256 = manifest.sharkerCu?.binarySha256;
    if (input.isPackaged && manifest.sharkerCu?.distributionReady !== true) {
 ...
 * sharker-cu supervises a single child, so the reported state is the state.
 */
export function computerUseServiceHealth(
  backendId: SelectedComputerUseBackend['backendId'],
  state: SharkerCuServiceSnapshot | undefined,
): {
  ...
  if (backendId === 'none' || !state) {
    return {
      state: 'not_available',
      reason: '未找到通过完整性检查且可分发的 sharker-cu executor。',
    };
  }
  switch (state.state) {
    case 'disposed':
      return { state: 'not_available', reason: 'sharker-cu executor 已停止。' };
    case 'unavailable':
      return { state: 'not_available', reason: 'sharker-cu executor 启动失败或已退出。' };
    case 'starting':
    case 'backing_off':
      return { state: 'degraded', reason: 'sharker-cu executor 正在启动或恢复。' };
    case 'ready':
      return { state: 'healthy', reason: 'sharker-cu executor 已就绪。' };
    default:
      return { state: 'not_run', reason: 'sharker-cu 已可用，将在首次调用时启动。' };
  }
}
