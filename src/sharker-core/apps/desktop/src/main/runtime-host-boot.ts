...
import { type SessionChangedEvent, type SessionChangedReason } from '@sharker/core/session';
import { isBotDeliveryProvider } from '@sharker/core/bot-chat-settings';
import { resolveSystemUiLocale } from '@sharker/core/ui-locale';
import {
  PROVIDER_DEFAULTS,
  providerAuthRequiresSecret,
} from "@sharker/core/llm-connections";
import { BotRegistry, type BotIncomingMessage } from '@sharker/runtime/bots';
import {
  SCHEDULED_TASK_NATIVE_EFFECT_SERVICE_ID,
  SCHEDULED_TASK_NATIVE_EFFECT_SERVICE_VERSION,
} from '@sharker/runtime/scheduled-task-tools';
import { buildMcpTools } from '@sharker/runtime/mcp-tools';
import {
...
import { openRuntimeHostPeerMeshOwner } from '@sharker/runtime-host/peer-mesh';
import type { WorkspaceTarget } from "@sharker/runtime-host/protocol";
import { runtimeHostProfileUsesHostWorkspace } from "@sharker/runtime-host/profile-kind";
import { createCredentialMcpOAuthStorage, McpClientManager } from "@sharker/mcp";
import { createWorkBoardStore } from "@sharker/storage/work-board-store";
import { createFileCredentialStore } from "@sharker/storage/credential-store";
import { createMcpConfigStore } from "@sharker/storage/mcp-config-store";
import { createSettingsStore } from "@sharker/storage/settings-store";
import { resolveStorageRoot } from "@sharker/storage/root-authority";
...
import { createSessionCopyCleanupAuthority } from "@sharker/storage/session-copy-cleanup";
import {
...
import { buildMemoryVaultTools } from "./memory-vault-tools.js";
...
const startHidden =
  ...
  process.env.SHARKER_E2E_SHOW_WINDOW !== "1";
...
const clientSettingsEffects = createClientSettingsEffects({
  ...
  applyBotSettings: (settings) => botRegistry.applySettings(settings),
...
let localMemoryDocumentClient: DesktopRuntimeHostClient | undefined;
const memoryVaultTools = buildMemoryVaultTools({
  workspaceRoot: () => workspaceRoot,
  replaceMemoryDocument: async (content) => {
    if (!localMemoryDocumentClient) throw new Error("Memory is unavailable");
    await replaceRuntimeHostMemoryDocument(localMemoryDocumentClient, content);
  },
});
...
const updateMockState =
  process.env.SHARKER_UPDATE_MOCK_STATE === "available" ||
  process.env.SHARKER_UPDATE_MOCK_STATE === "downloading" ||
  process.env.SHARKER_UPDATE_MOCK_STATE === "downloaded"
    ? process.env.SHARKER_UPDATE_MOCK_STATE
    : undefined;
const updateTestFeed = process.env.SHARKER_UPDATE_TEST_FEED;
...
const updateService = createAppUpdateService({
  ...
  mockLatestVersion: process.env.SHARKER_UPDATE_MOCK_VERSION,
...
runtimeHostManager = await startRuntimeHostDesktopManager(
  {
    ...
    candidateEntrypoint: new URL(
      import.meta.resolve(
        isE2e
          ? "@sharker/runtime-host/test-only/execution-candidate-e2e-main"
          : "@sharker/runtime-host/execution-candidate-main",
      ),
    ),
    ipcMain,
    workspaceRoot,
    ...
    nativeCapabilities: {
      ...
      additionalGroups: () => {
        ...
        return [
          ...
          {
            offerId: "desktop_memory_vault",
            label: "Memory vault",
            description:
              "Create, read, update, and delete markdown files in the local memory folder.",
            tools: memoryVaultTools,
          },
          {
...
function registerHostClientIpc(
  ...
  if (!usesHostWorkspace) localMemoryDocumentClient = client;
...
function resolveDesktopE2eFixture(): ReturnType<typeof resolveE2eFixture> {
  try {
    return resolveE2eFixture(
      process.env.SHARKER_E2E_FIXTURE,
      app.isPackaged,
      process.env.SHARKER_E2E_FIXTURE_REDUCED_MOTION,
      process.env.SHARKER_E2E_FIXTURE_THEME,
      process.env.SHARKER_E2E_FIXTURE_LOCALE,
      process.env.SHARKER_E2E_FIXTURE_TIMEZONE,
      process.env.SHARKER_E2E_FIXTURE_PLATFORM,
      process.env.SHARKER_E2E_FIXTURE_SCROLL_MOTION,
    );
  } catch (error) {
    if (!process.env.SHARKER_E2E_FIXTURE) throw error;
    console.error(
...
async function confirmDesktopStorageRootRepair(
  ...
  const { response } = await showStartupDiagnosticDialog(
    {
      type: "warning",
      title: isChinese ? "Sharker 工作区需要修复" : "Sharker workspace needs repair",
      message: isChinese
        ? "Sharker 无法验证这个工作区。"
        : "Sharker cannot verify this workspace.",
      detail: isChinese
        ? `系统中的磁盘标识可能发生了变化。仅当这是本机原来的 Sharker 工作区、而不是复制出的工作区时，才选择修复。\n\n${workspaceRoot}`
        : `The disk identity may have changed. Repair only if this is the original Sharker workspace on this computer, not a copied workspace.\n\n${workspaceRoot}`,
      buttons: isChinese
        ? ["修复工作区", "退出"]
