...
import type { AppSettings, UpdateAppSettingsInput } from '@sharker/core/settings';
import type { BotChannelSettings } from '@sharker/core/bot-chat-settings';
import type {
  BotOnboardingBrand,
...
import { generalizedErrorMessageChinese, redactSecrets } from '@sharker/core/redaction';
import { isBotOnboardingBrand, isBotOnboardingProvider } from '@sharker/core/bot-onboarding';
import type { BotRegistry } from '@sharker/runtime/bots';
import { proxiedFetch } from '@sharker/runtime/bots';
import type { SettingsStore } from '@sharker/storage/settings-store';
  ...
  return {
    dingtalk: {
      async start(_input, signal) {
        const init = await postJson('https://oapi.dingtalk.com/app/registration/init', { source: 'SHARKER' }, signal);
    ...
    feishu: {
      async start(input, signal) {
        ...
        verificationUrl.searchParams.set('from', 'sharker');
    ...
    wecom: {
      async start(_input, signal) {
        const response = await proxiedFetch(
          `https://work.weixin.qq.com/ai/qc/generate?source=sharker&plat=${platformCode()}`,
