...
test('development uses the native peer addon only for the peer-enabled launch', async (t) => {
  const root = await mkdtemp(join(tmpdir(), 'sharker-desktop-peer-client-'));
  ...
  const nativePath = join(
    ...
    'target',
    'release',
    'sharker_runtime_host_peer.node',
  );
  ...
  assert.equal(ordinaryEnvironment.SHARKER_RUNTIME_HOST_PEER_NATIVE_PATH, undefined);
  ...
  assert.equal(peerEnvironment.SHARKER_RUNTIME_HOST_PEER_NATIVE_PATH, nativePath);
});
