...
import type { SharkerTool, SharkerToolContext } from '@sharker/runtime/tool-runtime';
import {
...
function ctx(): SharkerToolContext {
  return {
    sessionId: 's1',
    turnId: 't1',
    cwd: '/tmp',
...
function run<P>(tool: SharkerTool<P, string>, args: P, extra: Partial<SharkerToolContext> = {}): Promise<string> {
  return Promise.resolve(tool.impl(args, { ...ctx(), ...extra })) as Promise<string>;
}
...
describe('browser tool execution', () => {
  ...
  it('navigate opens a workspace HTML file in the embedded browser', async () => {
    const parent = await mkdtemp(join(tmpdir(), 'sharker-browser-html-'));
    const file = join(parent, 'intro.html');
    await writeFile(file, '<h1>sharker</h1>', 'utf8');
    const real = await realpath(file);
    const expected = pathToFileURL(real).href;
    ...
    try {
      ...
      const out = await run(buildBrowserNavigateTool(), { url: 'intro.html' }, { cwd: parent });
      assert.equal(got, expected);
      assert.match(out, /Loaded /);
      assert.match(out, /Title: sharker/);
    } finally {
      await rm(parent, { recursive: true, force: true });
    }
  });

  it('navigate rejects a missing workspace HTML file without turning it into a Google search', async () => {
    install({});
    await assert.rejects(run(buildBrowserNavigateTool(), { url: 'missing.html' }), /HTML file not found/);
  });
