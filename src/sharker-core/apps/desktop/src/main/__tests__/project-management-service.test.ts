...
import { createProjectCatalog, type ProjectCatalog } from '@sharker/storage/project-catalog';
import {
...
test('owns Project selection and reversible lifecycle actions in Desktop', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-project-service-'));
...
test('adding a nested folder selects that folder instead of the parent project', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-project-nested-add-'));
