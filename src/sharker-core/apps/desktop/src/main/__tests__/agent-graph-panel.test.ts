...
import type { AgentGraphClientSnapshot } from '@sharker/runtime/stream-graph-read-model';
...
function installGraphRenderer(
  ...
  (window as unknown as { sharker: unknown }).sharker = {
    graphs: {
...
describe('AgentGraphPanel dismiss', () => {
  ...
  it('switches to a historical epoch without exposing current-graph controls', async () => {
    ...
    assert.equal(harness.container.querySelector('.sharker-agent-graph-dismiss'), null);
  ...
  it('shows dismiss only after the graph has settled', async () => {
    ...
    assert.ok(active.container.querySelector('.sharker-agent-graph-panel'));
    assert.equal(active.container.querySelector('.sharker-agent-graph-dismiss'), null);
    await act(async () => active.root.unmount());

    const completed = await renderPanel(snapshot({ graphId: 'graph-1', status: 'completed' }));
    assert.ok(completed.container.querySelector('.sharker-agent-graph-dismiss'));
  ...
  it('hides the panel after dismiss and brings it back for a new graph', async () => {
    ...
    const dismiss = harness.container.querySelector('.sharker-agent-graph-dismiss');
    assert.ok(dismiss);
    await act(async () => {
      (dismiss as HTMLElement).click();
    });
    assert.equal(harness.container.querySelector('.sharker-agent-graph-panel'), null);

    await harness.setSnapshot(snapshot({ graphId: 'graph-2', status: 'active' }));
    assert.ok(harness.container.querySelector('.sharker-agent-graph-panel'));
    assert.equal(harness.container.querySelector('.sharker-agent-graph-dismiss'), null);
    await act(async () => harness.root.unmount());
  });
  ...
  it('lets the user dismiss a stopped or failed graph', async () => {
    for (const status of ['stopped', 'failed'] as const) {
      ...
      assert.equal(harness.container.querySelector('.sharker-agent-graph-panel'), null, status);
      await act(async () => harness.root.unmount());
    }
  });
