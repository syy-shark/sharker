...
import { type AttachmentRef } from '@sharker/core/events';
import {
...
test("uploads a selected workspace file as a Host-owned Session Artifact", async (t) => {
  const cwd = await mkdtemp(join(tmpdir(), "sharker-host-attachment-"));
...
function observerWithSnapshot(
  overrides: Partial<import('@sharker/runtime-host/protocol').SessionContinuitySnapshot> = {},
): RuntimeHostSessionObserver {
  return observerWithTranscript([], overrides);
}

function observerWithTranscript(
  transcript: readonly import('@sharker/core/session').StoredMessage[],
  overrides: Partial<import('@sharker/runtime-host/protocol').SessionContinuitySnapshot> = {},
): RuntimeHostSessionObserver {
