...
import { SENSITIVE_PLACEHOLDER } from "@sharker/core/settings/network-settings";
import {
...
describe("settings IPC helpers", () => {
  ...
  test("maps runtime bot test results as credential checks, not operational readiness", () => {
    const result = toSettingsTestResult("telegram", {
      ok: true,
      identity: { id: "42", username: "sharker_bot", displayName: "Sharker" },
      hint: "ready",
    });
    ...
    assert.equal(
      result.message,
      "Telegram credentials are valid for sharker_bot.",
    );
    assert.deepEqual(result.details?.identity, {
      id: "42",
      username: "sharker_bot",
      displayName: "Sharker",
    });
