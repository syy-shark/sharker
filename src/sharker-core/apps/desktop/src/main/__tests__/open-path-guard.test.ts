...
describe('open path guard', () => {
  ...
  test('allows an allowed root that is itself a symlink after realpath normalization', async () => {
    const realRoot = await mkdtemp(join(tmpdir(), 'sharker-open-path-real-'));
    const linkRoot = await mkdtemp(join(tmpdir(), 'sharker-open-path-link-parent-'));
...
async function withWorkspace(
  ...
  const workspaceRoot = await mkdtemp(join(tmpdir(), 'sharker-open-path-workspace-'));
  const outsideRoot = await mkdtemp(join(tmpdir(), 'sharker-open-path-outside-'));
  try {
