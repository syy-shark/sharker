...
describe('main window Chromium permission policy', () => {
  ...
  it('trusts the dev origin and exact packaged entry without trusting arbitrary files', () => {
    ...
    assert.equal(
      matchesTrustedRendererUrl(
        'file:///Applications/Sharker.app/Contents/Resources/app.asar/apps/desktop/dist-renderer/index.html#settings',
        'file:///Applications/Sharker.app/Contents/Resources/app.asar/apps/desktop/dist-renderer/index.html',
      ),
      true,
    );
    assert.equal(
  ...
  it('installs both Electron handlers on the renderer session', () => {
    ...
    installMainWindowPermissionPolicy(owner, 'file:///Applications/Sharker.app/index.html');
    assert.ok(checkHandler);
    assert.ok(requestHandler);
    assert.equal(checkHandler(owner, 'media', 'file://', {
      isMainFrame: true,
      mediaType: 'audio',
      requestingUrl: 'file:///Applications/Sharker.app/index.html',
    }), false);
