...
test('local setup installs one managed service for the Desktop root with Direct peer enabled', () => {
  assert.deepEqual(
    runtimeHostLocalSetupCommand({
      packageSpecifier: 'sharker-agent@0.2.0',
      clientDataRoot: '/Users/ada/Library/Application Support/Sharker',
      rootPath: '/Users/ada/Library/Application Support/Sharker/workspaces/default',
    ...
    }),
    {
      executable: 'npm',
      args: [
        'exec', '--yes', '--package', 'sharker-agent@0.2.0', '--',
        'sharker', 'runtime-host', 'setup',
        '--client-data-root', '/Users/ada/Library/Application Support/Sharker',
        '--root', '/Users/ada/Library/Application Support/Sharker/workspaces/default',
        ...
        '--expected-root-path', '/Users/ada/Library/Application Support/Sharker/workspaces/default',
...
test('local setup forwards the exact development archive evidence', async (t) => {
  const archive = '/tmp/sharker-agent-development.tgz';
  ...
  const spawnProcess = ((_command, _args, options) => {
    ...
    process.nextTick(() => {
      stdout.end(encodeRuntimeHostSetupFrame({
        ...
        operatorPath: '/tmp/sharker/operator',
        rootPath: '/tmp/sharker/root',
        rootId: 'a'.repeat(64),
  ...
  await operator.runSetup({
    ...
    clientDataRoot: '/tmp/sharker/client',
    rootPath: '/tmp/sharker/root',
    principalId: 'desktop-owner:pairing',
    expectedTarget: {
      serviceId: 'b'.repeat(64),
...
test('local update runs the selected package against the exact managed deployment', async (t) => {
  ...
  await operator.runUpdate(
    {
      setupPackage: { kind: 'npm', specifier: 'sharker-agent@0.3.0' },
      target: {
