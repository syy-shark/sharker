...
import type { EnvironmentRuntimeHostProfile } from '@sharker/runtime-host/client';
...
test('onboards WSL as a credential-free environment profile', async () => {
  ...
  const harness = createHarness({
    ...
    resolveSetupPackage: (peerTarget) => {
      ...
      return { kind: 'npm', specifier: 'sharker-agent@0.2.0' };
    },
  });
...
function createHarness(overrides: HarnessOverrides = {}) {
  ...
  const onboarding = createDesktopRuntimeHostOnboarding({
    ...
    resolveSetupPackage: () => ({ kind: 'npm', specifier: 'sharker-agent@0.2.0' }),
