...
describe('single live-turn handoff', () => {
  ...
  it('does not flash the empty-chat Sharker hero before a first transient message', () => {
    const markup = renderWithLocale(createElement(ChatView, {
      activeSession: {
    ...
    assert.doesNotMatch(markup, /sharker-hero-empty-chat/);
  });
  ...
  it('shows a loading transient before its real live Turn answer', () => {
    ...
    assert.doesNotMatch(markup, /sharker-chat-message-loading/);
  ...
  it('renders one ordered timeline: thinking before its tool and answer', () => {
    ...
    assert.equal((markup.match(/sharker-processing-block/g) ?? []).length, 0);
  ...
  it('keeps a completed live answer as the only visible owner until settle', () => {
    ...
    assert.match(markup, /sharker-bubble-streaming/);
  ...
  it('keeps an incomplete live answer as the only owner after early persistence', () => {
    ...
    assert.equal((markup.match(/sharker-bubble-streaming/g) ?? []).length, 1);
