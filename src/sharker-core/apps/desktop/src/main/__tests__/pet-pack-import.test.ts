...
import { PET_PACK_SCHEMA_V1 } from '@sharker/core/pet';
import { createSettingsStore } from '@sharker/storage/settings-store';
import { createPetPackStore } from '@sharker/storage/pet-pack-store';
import {
...
async function withFixture(
  ...
  const root = await mkdtemp(join(tmpdir(), 'sharker-pet-import-'));
