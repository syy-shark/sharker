 ...
 *
 * Imported through the `@sharker/ui` barrel.
 */

import { strict as assert } from 'node:assert';
import { describe, it } from 'node:test';
import { TOOL_OUTPUT_DELTA_MAX_CHARS } from '@sharker/core/events';
import {
...
describe('applyToolOutputChunk — secondary redaction (defense in depth)', () => {
  it('masks Authorization: Bearer header in raw chunk before storing', () => {
    ...
    // The actual mask token comes from @sharker/ui redactSecrets — we
