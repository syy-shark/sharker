...
async function createFakeShell(body: string): Promise<{
  ...
  const directory = await mkdtemp(join(tmpdir(), 'sharker-shell-env-'));
...
export SHARKER_E2E_FIXTURE='first-run'
...
`);
      ...
      try {
        ...
        delete process.env.SHARKER_SKIP_SHELL_ENV;
        delete process.env.SHARKER_E2E_FIXTURE;
        ...
        assert.equal(process.env.SHARKER_E2E_FIXTURE, undefined);
...
`);
  ...
  it(
    'does not execute the shell when SHARKER_SKIP_SHELL_ENV is set',
    async () => {
      const restoreEnv = snapshotEnv();
      const shell = await createFakeShell(`
...
`);
      ...
      try {
        ...
        process.env.PATH = '/usr/bin:/bin:/usr/sbin:/sbin';
        process.env.SHARKER_SKIP_SHELL_ENV = '1';
        delete process.env.TERM;
        delete process.env.COLORTERM;
