...
import { type CuDispatchBackend } from '@sharker/runtime/computer-use-types';
import { type SharkerTool, type SharkerToolContext } from '@sharker/runtime/tool-runtime';
import type { ClientCapabilityProvider } from '@sharker/runtime-host/client';
import {
...
test('publishes and admits additional Desktop native-effect services', async () => {
  ...
  const provider = createDesktopNativeCapabilityProvider(
    {
      ...
      additionalServices: (scope) => [
        {
          serviceId: 'sharker_scheduled_task_native_effect',
          version: '1',
  ...
  assert.deepEqual(provider.services?.(), [
    { serviceId: 'sharker_scheduled_task_native_effect', version: '1' },
  ]);
...
test('validates before admission and invokes the exact offered tool with Host context', async () => {
  ...
  let received:
    | {
        args: unknown;
        context: Pick<SharkerToolContext, 'sessionId' | 'turnId' | 'cwd' | 'toolCallId'>;
      }
    | undefined;
  const provider = createDesktopNativeCapabilityProvider(
    {
...
function tool<P, R>(
  ...
  impl: (args: P, context: SharkerToolContext) => Promise<R>,
): SharkerTool<P, R> {
  return {
    name,
    displayName: name,
...
function computerTools(
  impl?: (args: { wait?: boolean }, context: SharkerToolContext) => Promise<unknown>,
  clearSession: (sessionId: string) => void = () => undefined,
): ComputerUseToolSet {
  const tools = (impl
    ? [
        {
          ...tool('sharker_computer', z.object({ wait: z.boolean().optional() }), impl),
