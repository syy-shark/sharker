...
describe('browser logic', () => {
  it('parseNavigable accepts http(s), hostnames, and Google omnibox search', () => {
    ...
    assert.equal(parseNavigable('file:///etc/passwd', { localHtmlRoots: ['/tmp'] }), null);
    ...
    assert.equal(parseNavigable('intro.html'), 'https://intro.html/');
  });

  it('parseNavigable opens a workspace HTML file instead of rewriting it to https', async () => {
    const parent = await mkdtemp(join(tmpdir(), 'sharker-parse-html-'));
    const file = join(parent, 'intro.html');
    await writeFile(file, '<p>ok</p>', 'utf8');
    try {
      const real = await realpath(file);
      assert.equal(parseNavigable('intro.html', { localHtmlRoots: [parent] }), pathToFileURL(real).href);
    } finally {
      await rm(parent, { recursive: true, force: true });
    }
  });

  it('parseNavigable allows workspace HTML only when localHtmlRoots are supplied', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-nav-html-'));
    try {
      const file = join(await realpath(root), 'intro.html');
      await writeFile(file, '<html></html>', 'utf8');
      const fileUrl = pathToFileURL(file).href;
      assert.equal(parseNavigable(fileUrl), null);
      assert.equal(parseNavigable('intro.html'), 'https://intro.html/');
      assert.equal(parseNavigable(fileUrl, { localHtmlRoots: [root] }), fileUrl);
      assert.equal(parseNavigable('intro.html', { localHtmlRoots: [root] }), fileUrl);
      assert.equal(parseNavigable('file:///etc/passwd.html', { localHtmlRoots: [root] }), null);
    } finally {
