...
import type { ComputerUseToolSet } from '@sharker/runtime/computer-use-tools';
import type { SharkerTool } from '@sharker/runtime/tool-runtime';
import { connectRuntimeHost } from '@sharker/runtime-host/client';
import {
...
test('drives Desktop Session operations through a real Runtime Host connection', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-desktop-host-client-'));
...
test('drives the renderer Session catalog facade through real UDS framing', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-desktop-host-ipc-'));
  let host: RuntimeHostKernel | undefined;
...
test('drives the renderer Session execution facade through real UDS framing', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-desktop-host-execution-ipc-'));
  let host: RuntimeHostKernel | undefined;
  try {
    const capability = await resolveStorageRoot({ path: base, kind: 'interactive' });
    const owner = await tryAcquireInteractiveRootOwner(capability);
...
test('drives bounded Session domain projections through real UDS framing', async () => {
  const base = await mkdtemp(join(tmpdir(), 'sharker-desktop-host-domains-ipc-'));
  let host: RuntimeHostKernel | undefined;
  try {
    const capability = await resolveStorageRoot({ path: base, kind: 'interactive' });
    const owner = await tryAcquireInteractiveRootOwner(capability);
...
function nativeTool(): SharkerTool {
  return {
