...
test('development setup lazily caches CLI archives by peer target unless overridden', async (t) => {
  ...
  const directory = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-setup-package-'));
  t.after(() => rm(directory, { recursive: true, force: true }));
  const archive = join(directory, 'sharker-agent-dev.tgz');
  ...
  const resolveOverride = createRuntimeHostSetupPackageResolver({
    isPackaged: false,
    appPath: join(repoRoot, 'apps', 'desktop'),
    environment: { SHARKER_RUNTIME_HOST_SETUP_ARCHIVE: override },
  ...
  const resolveInvalidOverride = createRuntimeHostSetupPackageResolver({
    isPackaged: false,
    appPath: join(repoRoot, 'apps', 'desktop'),
    environment: { SHARKER_RUNTIME_HOST_SETUP_ARCHIVE: invalidOverride },
  });
