...
describe('app bundle resolution for the drag', () => {
  it('walks three levels up from the executable to the .app', () => {
    assert.deepEqual(
      resolveAppBundle({
        executablePath: '/Applications/Sharker.app/Contents/MacOS/Sharker',
        platform: 'darwin',
        exists: () => true,
      }),
      { ok: true, bundlePath: '/Applications/Sharker.app' },
    );
  });
  ...
  it('resolves the containing app bundle independent of its development name', () => {
    assert.deepEqual(
      resolveAppBundle({
        executablePath: '/repo/apps/desktop/.sharker-dev/Sharker Dev.app/Contents/MacOS/Electron',
        platform: 'darwin',
        exists: () => true,
      }),
      { ok: true, bundlePath: '/repo/apps/desktop/.sharker-dev/Sharker Dev.app' },
    );
  });

  it('fails explicitly when the walk does not land on a bundle', () => {
    const result = resolveAppBundle({
      executablePath: '/usr/local/bin/sharker',
      platform: 'darwin',
      exists: () => true,
    });
    assert.equal(result.ok, false);
  ...
  it('fails on non-darwin', () => {
    const result = resolveAppBundle({
      executablePath: 'C:/Program Files/Sharker/Sharker.exe',
      platform: 'win32',
      exists: () => true,
    });
    assert.equal(result.ok, false);
