...
test('maps each request-composition category to the same colour in the chart and legend', () => {
  ...
  for (const [kind, tokens] of [
    ...
    assert.match(
      markup,
      new RegExp(`class="sharker-inspector-composition-band"[^>]*data-segment="${kind}"[^>]*flex-grow:${tokens}`),
    );
    assert.match(
      markup,
      new RegExp(`class="sharker-inspector-composition-swatch"[^>]*data-segment="${kind}"`),
    );
  }
});
