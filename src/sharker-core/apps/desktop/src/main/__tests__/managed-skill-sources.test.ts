...
describe('managed skill sources', () => {
  it('resolves the global source cache under .sharker without making it a runtime path', () => {
    assert.equal(resolveManagedSkillSourcesRoot('C:\\Users\\Ada'), join('C:\\Users\\Ada', '.sharker', 'skill-sources'));
  });
...
async function withTempRoot(fn: (root: string) => Promise<void>): Promise<void> {
  const root = await mkdtemp(join(tmpdir(), 'sharker-managed-sources-'));
  try {
    await fn(root);
  } finally {
