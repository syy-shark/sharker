...
import { createClientRuntimeHostProfileCatalog } from "@sharker/runtime-host/client";
import {
...
test("keeps Desktop service bindings outside the shared profile catalog", async () => {
  const root = await mkdtemp(join(tmpdir(), "sharker-managed-host-services-"));
  roots.push(root);
...
test("persists a WSL deployment through its environment control route", async () => {
  const root = await mkdtemp(join(tmpdir(), "sharker-managed-wsl-deployment-"));
  roots.push(root);
