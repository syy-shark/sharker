...
async function repository(): Promise<string> {
  ...
  await git(root, 'config', 'user.name', 'Sharker Test');
  await git(root, 'config', 'user.email', 'sharker@example.invalid');
  await writeFile(join(root, 'base.txt'), 'base\n', 'utf8');
...
async function temporaryRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'sharker-git-review-'));
  roots.add(root);
  return root;
}
