...
describe('Computer Use host health', () => {
  ...
  it('does not report a binary-only executor as healthy before first use', () => {
    assert.deepEqual(computerUseServiceHealth('sharker-cu', snapshot('idle')), {
      state: 'not_run',
      reason: 'sharker-cu 已可用，将在首次调用时启动。',
    });
  });

  it('reports ready, recovery, and unavailable states', () => {
    assert.equal(computerUseServiceHealth('sharker-cu', snapshot('ready')).state, 'healthy');
    assert.equal(
      computerUseServiceHealth('sharker-cu', snapshot('backing_off')).reason,
      'sharker-cu executor 正在启动或恢复。',
    );
    assert.equal(
      computerUseServiceHealth('sharker-cu', snapshot('starting')).state,
      'degraded',
    );
    assert.deepEqual(computerUseServiceHealth('sharker-cu', snapshot('unavailable')), {
      state: 'not_available',
      reason: 'sharker-cu executor 启动失败或已退出。',
    });
    assert.deepEqual(computerUseServiceHealth('sharker-cu', snapshot('disposed')), {
      state: 'not_available',
      reason: 'sharker-cu executor 已停止。',
    });
  });
  ...
  it('constructs a backend only when the local artifact matches the manifest hash', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'sharker-cu-host-'));
    try {
      const binaryPath = join(directory, 'sharker-cu');
      ...
      await writeFile(manifestPath, JSON.stringify({
        sharkerCu: { binarySha256: hash, distributionReady: false },
      }));
      ...
      await writeFile(manifestPath, JSON.stringify({
        sharkerCu: { binarySha256: hash, distributionReady: true },
      }));
      ...
      const linkedBinaryPath = join(directory, 'linked-sharker-cu');
