...
test('download verification accepts only a trusted exact artifact subject', async (t) => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-update-attestation-'));
  t.after(() => rm(directory, { recursive: true, force: true }));
  const artifact = join(directory, 'cached-update.zip');
  const bytes = Buffer.from('attested update bytes');
  await writeFile(artifact, bytes);
  const digest = createHash('sha256').update(bytes).digest('hex');
  const bundle = provenanceBundle('Sharker-1.2.3-mac-arm64.zip', digest);
...
test('nightly verification fetches provenance from the versioned GitHub Release asset', async (t) => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-nightly-attestation-'));
  t.after(() => rm(directory, { recursive: true, force: true }));
  const artifact = join(directory, 'cached-update.zip');
  const bytes = Buffer.from('nightly update bytes');
  await writeFile(artifact, bytes);
  const version = '0.2.0-dev.20260829.42';
  const name = `Sharker-${version}-mac-arm64.zip`;
  const digest = createHash('sha256').update(bytes).digest('hex');
  ...
  assert.equal(
    fetchedUrl,
    `https://github.com/apache/sharker/releases/download/v${version}/Sharker-${version}-attestation.sigstore.json`,
  );
});

test('packaged update trust accepts only an explicit release or nightly channel', () => {
  assert.equal(desktopUpdateChannelFromManifest({ sharkerUpdateChannel: 'release' }), 'release');
  assert.equal(desktopUpdateChannelFromManifest({ sharkerUpdateChannel: 'nightly' }), 'nightly');
  assert.throws(
    () => desktopUpdateChannelFromManifest({ sharkerUpdateChannel: 'preview' }),
