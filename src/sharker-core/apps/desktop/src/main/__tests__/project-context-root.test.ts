...
describe('resolveProjectContextRoot', () => {
  it('uses the app project root when there is no active session', async () => {
    const root = await mkdtemp(join(tmpdir(), 'sharker-new-task-project-'));
    try {
  ...
  it('uses the persisted session cwd instead of the app project root', async () => {
    const appRoot = await mkdtemp(join(tmpdir(), 'sharker-app-project-'));
    const sessionRoot = await mkdtemp(join(tmpdir(), 'sharker-session-project-'));
    let appRootReads = 0;
    try {
  ...
  it('rejects a deleted session cwd without falling back to the app project', async () => {
    const appRoot = await mkdtemp(join(tmpdir(), 'sharker-fallback-project-'));
    const deletedRoot = await mkdtemp(join(tmpdir(), 'sharker-deleted-session-project-'));
  ...
  it('rejects a session cwd without read and traversal access', {
    ...
    const sessionRoot = await mkdtemp(join(tmpdir(), 'sharker-inaccessible-session-project-'));
