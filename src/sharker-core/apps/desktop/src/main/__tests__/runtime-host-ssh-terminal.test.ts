...
import { type RuntimeHostSshProcessFactory } from '@sharker/runtime-host/client';
import {
...
test('detects the development peer target through the bounded SSH preflight', async () => {
  ...
  const marker = command.match(/__SHARKER_RUNTIME_HOST_TARGET_[0-9a-f]+__/u)?.[0];
  assert.ok(marker);
  harness.pty.emitData(`${marker}Linux:x86_64\r\n`);
  harness.pty.exit(0);

  assert.equal(await detection, 'linux-x64');
  assert.doesNotMatch(JSON.stringify(harness.events), /SHARKER_RUNTIME_HOST_TARGET/u);
...
test('keeps setup credentials out of the interactive terminal projection', async () => {
  ...
  const setup = harness.terminal.runSetup(
    {
      destination: 'operator@example.com',
      setupPackage: { kind: 'npm', specifier: 'sharker-agent@1.2.3+desktop.1' },
  ...
  const completeFrame = encodeRuntimeHostSetupFrame({
    ...
    operatorPath: '/home/operator/.local/share/sharker/operator',
    rootPath: '/home/operator/.config/Sharker/workspaces/default',
    rootId: 'a'.repeat(64),
  ...
  assert.doesNotMatch(JSON.stringify(harness.events), /secret-access-token|SHARKER_RUNTIME/u);
...
test('discards an oversized reserved setup line instead of projecting its tail', async () => {
  ...
  const setup = harness.terminal.runSetup(
    {
      destination: 'operator@example.com',
      setupPackage: { kind: 'npm', specifier: 'sharker-agent@1.2.3' },
      principalId: 'desktop:stable-client',
    },
    () => undefined,
  );
...
test('reads a framed service result without projecting it into the SSH terminal', async () => {
  ...
  await waitFor(() => harness.pty.hasDataListener());
  const remoteCommand = harness.launchArgs.at(-1)?.at(-1) ?? '';
  assert.match(remoteCommand, /\.local\/share\/sharker\/operator/u);
  assert.match(remoteCommand, /SHARKER_RUNTIME_HOST_OPERATOR_CAPABILITY_REQUEST/u);
  assert.match(remoteCommand, /peer-management-v1/u);
  assert.doesNotMatch(remoteCommand, /npx|sharker-agent@/u);
  harness.pty.emitData('Password: ');
  harness.pty.emitData(
    encodeRuntimeHostServiceManagementFrame({
      ...
      service: {
        ...
        stateRoot: '/home/operator/.config/Sharker/workspaces/default',
  ...
  assert.doesNotMatch(JSON.stringify(harness.events), /SHARKER_RUNTIME_HOST_SERVICE/u);
  assert.match(JSON.stringify(harness.events), /Password/u);
  await harness.terminal.close();
});
...
test('applies the complete remote Project root policy through the managed operator', async () => {
  ...
  assert.match(
    remoteCommand,
    /SHARKER_RUNTIME_HOST_OPERATOR_PROJECT_DIRECTORY_CONFIGURATION_REQUEST=1/u,
  );
  harness.pty.emitData(
    encodeRuntimeHostServiceManagementFrame({
      schemaVersion: 1,
...
test('runs an exact update package and reports progress before an active-work result', async () => {
  ...
  const update = harness.terminal.runUpdate(
    {
      destination: 'operator@example.com',
      setupPackage: { kind: 'npm', specifier: 'sharker-agent@1.3.0' },
      expectedTarget: {
        serviceId: 'b'.repeat(64),
        rootPath: '/srv/sharker',
        rootId: 'a'.repeat(64),
  ...
  await waitFor(() => harness.pty.hasDataListener());
  const remoteCommand = harness.launchArgs.at(-1)?.at(-1) ?? '';
  assert.match(remoteCommand, /--package.*sharker-agent@1\.3\.0/u);
...
test('keeps a prepared access credential out of the SSH terminal projection', async () => {
  const harness = createHarness('pending');
  const credential = 'sharker_rh_secret-replacement';
  ...
  assert.doesNotMatch(JSON.stringify(harness.events), /secret-replacement|SHARKER_RUNTIME/u);
...
test('requires an absent operator deployment root to be absent', async () => {
  ...
  assert.match(remoteCommand, /home\/operator\/\.local\/share\/sharker/u);
...
test('uploads a development release archive before running the same remote setup', async (t) => {
  const directory = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-development-package-'));
  t.after(() => rm(directory, { recursive: true, force: true }));
  const archive = join(directory, 'sharker-agent-development.tgz');
  ...
  assert.match(launches[0]?.args.at(-2) ?? '', /sharker-agent-development\.tgz$/u);
  assert.match(
    launches[0]?.args.at(-1) ?? '',
    /^operator@example\.com:\.\/\.sharker-runtime-host-setup-.+\.tgz$/u,
  );
  ...
  assert.match(remoteCommand, /--package.*sharker-runtime-host-setup-.+\.tgz/u);
  assert.match(remoteCommand, /SHARKER_RUNTIME_HOST_SETUP_SOURCE_PACKAGE_INTEGRITY=/u);
