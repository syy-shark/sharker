...
import { APP_ICONS } from '@sharker/core/settings';
import {
...
import { isAppIcon, toAppIconChoice, type AppIconChoice } from '@sharker/core/settings';
...
test('a packaged build reads artwork from the copy beside the app', () => {
  ...
  assert.equal(
    resolveAppIconPath(
      desktopAssetRoot({ isPackaged: true, resourcesPath: join('/Apps', 'Sharker.app', 'Contents', 'Resources') }),
      'sky',
    ),
    join('/Apps', 'Sharker.app', 'Contents', 'Resources', 'assets', 'app-icons', 'sky.png'),
  );
});
