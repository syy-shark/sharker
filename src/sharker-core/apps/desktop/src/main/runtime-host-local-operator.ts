...
export function createDesktopRuntimeHostLocalOperator(input: {
  ...
  return {
    async runSetup(setup, onProgress) {
      ...
      const workingDirectory = await mkdtemp(join(tmpdir(), 'sharker-runtime-host-local-setup-'));
      try {
        if (closed) throw new Error('Local Runtime Host operator is closed');
...
function abortError(signal: AbortSignal | undefined): Error {
  return signal?.reason instanceof Error ? signal.reason : new Error('Local Sharker setup was cancelled');
}
