...
/**
 * 助手对本机记忆文件夹的增删改查工具（SharkerMemoryVaultList/Read/Write/Delete）。
 */
import type { SharkerTool } from '@sharker/runtime/tool-runtime';
import { z } from 'zod';
import type { MemoryVaultListing } from '@sharker/core/local-memory-vault';
import { parseMemoryVaultRelativePath } from '@sharker/core/local-memory-vault';
import {
...
/** Agent 对本机记忆文件夹的增删改查。 */
export function buildMemoryVaultTools(authority: MemoryVaultToolAuthority): readonly SharkerTool[] {
  const listTool: SharkerTool<Record<string, never>, MemoryVaultListing> = {
    name: 'SharkerMemoryVaultList',
  ...
  const readTool: SharkerTool<{ path: string }, { path: string; content: string }> = {
    name: 'SharkerMemoryVaultRead',
  ...
  const writeTool: SharkerTool<{ path: string; content: string }, { path: string }> = {
    name: 'SharkerMemoryVaultWrite',
  ...
  const deleteTool: SharkerTool<{ path: string }, { path: string }> = {
    name: 'SharkerMemoryVaultDelete',
