...
export function applyComputerUseRealModelPolicy(
  ...
  const wrapped = tools.map((tool) => {
    if (tool.name !== 'sharker_computer') return tool;
    return {
      ...tool,
      impl: async (args, context) => {
        ...
        if (totalActions > policy.maxTotalActions) {
          return {
            text: 'sharker_computer failed: total_action_budget_exceeded',
            error: 'total_action_budget_exceeded',
          };
        }
        if (!allowed.has(action)) {
          return {
            text: `sharker_computer.${action} failed: unsupported_action_policy`,
        ...
        if (
          ...
          return {
            text: `sharker_computer.${action} failed: target_policy_mismatch`,
        ...
        if (actionCount > (policy.maxActionCounts[action] ?? 0)) {
          return {
            text: `sharker_computer.${action} failed: action_budget_exceeded`,
