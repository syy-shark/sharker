...
// the real user data: a stray SHARKER_E2E on a build/dev machine would otherwise
// swap in the fake backend or hide the window. app.isPackaged is true for
// asar-packaged builds; SHARKER_E2E_USER_DATA_DIR must also be set, so the fake
// backend can't write test sessions into a real profile if someone sets only
// SHARKER_E2E.
export const hasIsolatedE2eProfile =
  !app.isPackaged &&
  !!process.env.SHARKER_E2E_USER_DATA_DIR;
export const isE2e = hasIsolatedE2eProfile && process.env.SHARKER_E2E === '1';
export const isComputerUseRealModelE2e =
  hasIsolatedE2eProfile &&
  process.env.SHARKER_CU_REAL_MODEL_E2E === '1';
