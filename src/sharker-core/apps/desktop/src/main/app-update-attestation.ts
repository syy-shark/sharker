...
const PRODUCT_REPOSITORY = 'apache/sharker';
...
export function desktopUpdateChannelFromManifest(manifest: unknown): DesktopUpdateChannel {
  const channel =
    manifest && typeof manifest === 'object'
      ? (manifest as { sharkerUpdateChannel?: unknown }).sharkerUpdateChannel
      : undefined;
...
function exactDesktopUpdateArtifactName(
  ...
  if (platform === 'darwin' && arch === 'arm64') return `Sharker-${version}-mac-arm64.zip`;
  if (platform === 'win32' && arch === 'x64') return `Sharker-${version}-win-x64.exe`;
...
function productReleaseAttestationName(version: string): string {
  ...
  return `Sharker-${version}-attestation.sigstore.json`;
}
