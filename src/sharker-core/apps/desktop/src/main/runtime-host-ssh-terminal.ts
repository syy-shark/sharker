...
import { terminateProcessTree } from '@sharker/runtime/process-tree-terminator';
import {
...
export function createDesktopRuntimeHostSshTerminal(input: {
  ...
  return {
    ...
    resolveDevelopmentPeerTarget: async (targetInput) => {
      ...
      const marker = `__SHARKER_RUNTIME_HOST_TARGET_${randomUUID().replaceAll('-', '')}__`;
    ...
    runSetup: async (setupInput, onProgress, onComplete) => {
      ...
      try {
        ...
        if (!complete) {
          throw new Error(
            wait.timedOut
              ? 'Remote Sharker setup timed out'
              : wait.exit.code === 0
              ? 'Remote Sharker setup ended without a completion result'
              : wait.exit.code === 2
                ? 'The released Sharker CLI on this channel does not support automated Runtime Host setup'
                : `Remote Sharker setup exited with code ${String(wait.exit.code)}`,
          );
        }
        completePresentation(terminal);
        return complete;
...
function remoteDevelopmentArchivePath(principalId: string): string {
  ...
  return `./.sharker-runtime-host-setup-${owner}.tgz`;
}
...
function runtimeHostPackageRemoteCommand(
  ...
  const commandArgs = ['sharker', ...args].map(quotePosix).join(' ');
  ...
  const commandInvocation = setupPackage.kind === 'development_archive'
    ...
    : `${invocationPrefix}npx --yes --prefix "$sharker_command_prefix" --package ${quotePosix(setupPackage.specifier)} ${commandArgs}`;
  const command = setupPackage.kind === 'development_archive'
    ? `cd "$HOME" || exit 1; sharker_command_exit=0; ${commandInvocation} || sharker_command_exit=$?; rm -f -- ${quotePosix(setupPackage.removeAfterSetup)}; exit "$sharker_command_exit"`
    : `sharker_command_prefix=$(mktemp -d) || exit 1; trap 'rm -rf -- "$sharker_command_prefix"' EXIT; trap 'exit 129' HUP; trap 'exit 130' INT; trap 'exit 143' TERM; cd "$sharker_command_prefix" || exit 1; ${commandInvocation}`;
