...
// `persist:sharker-browser`, so the handlers + will-download guard belong to that
...
export class BrowserViewController {
  ...
  /** Project directories whose `.html` / `.htm` files may load as `file://`. */
  private readonly localHtmlRoots = new Set<string>();
  ...
  private wireEvents(): void {
    ...
    wc.setWindowOpenHandler(({ url }) => {
      const navigable = this.parseAllowed(url);
      if (navigable) void this.loadInternal(navigable);
      else this.openExternal(url);
      return { action: 'deny' };
    });

    // Block link navigations to non-web schemes; workspace HTML `file://` is
    // allowed only after allowLocalHtmlRoot registered the conversation cwd.
    wc.on('will-navigate', (event, url) => {
      if (this.parseAllowed(url)) return;
  ...
  /**
   * Permit this conversation's view to open `.html` / `.htm` files under `root`
   * (typically the session cwd). Must run before a `file://` goto so
   * `will-navigate` does not cancel it.
   */
  allowLocalHtmlRoot(root: string): void {
    const trimmed = root.trim();
    if (trimmed) this.localHtmlRoots.add(trimmed);
  }
  ...
  async navigate(input: string): Promise<void> {
    const url = this.parseAllowed(input);
    if (!url) return;
    await this.loadInternal(url);
  }

  private parseAllowed(input: string): string | null {
    return parseNavigable(input, { localHtmlRoots: [...this.localHtmlRoots] });
  }
