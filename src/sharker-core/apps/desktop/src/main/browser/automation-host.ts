...
export function createBrowserViewHost(
  ...
  return {
    canDrive(sessionId, kind, opts) {
      const currentShown = shownSessionId();
      // First navigation may create the view before the renderer has marked a
      // session shown (panel still on the start page / not yet opened). Later
      // actions still require this conversation to be the one on screen.
      const shown = sessionId === currentShown || (kind === 'navigate' && currentShown === null);
    ...
    allowLocalHtmlRoot(sessionId, root) {
      manager.getOrCreate(sessionId).allowLocalHtmlRoot(root);
    },
  };
}
