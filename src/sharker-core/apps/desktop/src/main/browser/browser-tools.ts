 ...
 *
 * Permission: every tool is the dedicated `browser` category — Sharker's mode
...
async function runBrowserAction<T>(input: {
  ...
  timeoutMs?: number;
  /** Session cwd — registered so the view may open workspace HTML. */
  cwd?: string;
  ...
  return withBrowserPage(
    ...
    input.label,
    async (page, info) => {
      if (input.cwd) browserViewHost().allowLocalHtmlRoot?.(input.sessionId, input.cwd);
      return input.run(page, info);
    },
    {
...
export function buildBrowserNavigateTool(): SharkerTool<{ url: string }, string> {
  return {
    name: 'browser_navigate',
    displayName: '浏览器导航',
    description:
      'Open a URL or a workspace HTML page in the conversation\'s embedded browser. ' +
      'Pass a full http:// or https:// URL, a project-relative or absolute .html / .htm path, or a file:// URL to that file. ' +
      'Other schemes and files outside the project are rejected. ' +
      'Use this after writing a local webpage so the user can see it in the right-hand browser. ' +
    ...
    parameters: z.object({
      url: z
        ...
        .max(4000)
        .describe(
          'http(s) URL, workspace .html / .htm path, or file:// URL to that file. Other schemes are rejected.',
        ),
    }),
    categoryHint: BROWSER_TOOL_CATEGORY,
    impl: async ({ url: rawUrl }, { sessionId, cwd, abortSignal }) => {
      const local = resolveWorkspaceHtmlPage(rawUrl, [cwd]);
      let url: string | null = null;
      if (local.ok) {
        url = local.url;
      } else if (looksLikeLocalHtmlAttempt(rawUrl)) {
        throw new Error(workspaceHtmlErrorMessage(local.reason, rawUrl));
      } else {
        url = parseNavigable(rawUrl);
      }
      if (!url) {
        throw new Error(
          `Not a navigable URL: ${JSON.stringify(rawUrl)}. Pass a full http:// or https:// URL, or a workspace .html / .htm path.`,
        );
      }
...
export function buildBrowserSnapshotTool(): SharkerTool<Record<string, never>, string> {
  return {
    ...
    parameters: z.object({}),
    categoryHint: BROWSER_TOOL_CATEGORY,
    impl: async (_args, { sessionId, cwd, abortSignal }) => {
      const result = await runBrowserAction({
        sessionId,
        label: 'snapshot',
        abortSignal,
...
export function buildBrowserClickTool(): SharkerTool<{ ref: string }, string> {
  return {
    name: 'browser_click',
    displayName: '浏览器点击',
    description:
    ...
    categoryHint: BROWSER_TOOL_CATEGORY,
    impl: async ({ ref }, { sessionId, cwd, abortSignal }) => {
      const result = await runBrowserAction({
        sessionId,
        label: 'click',
        abortSignal,
...
export function buildBrowserTypeTool(): SharkerTool<{ ref: string; text: string; submit?: boolean }, string> {
  return {
    name: 'browser_type',
    displayName: '浏览器输入',
    description:
    ...
    categoryHint: BROWSER_TOOL_CATEGORY,
    impl: async ({ ref, text, submit }, { sessionId, cwd, abortSignal }) => {
      const result = await runBrowserAction({
        sessionId,
        label: 'type',
        abortSignal,
...
export function buildBrowserWaitTool(): SharkerTool<
...
> {
  return {
    ...
    categoryHint: BROWSER_TOOL_CATEGORY,
    impl: async ({ text, selector, time, timeout }, { sessionId, cwd, abortSignal }) => {
...
export function buildBrowserExtractTool(): SharkerTool<{ selector?: string; start?: number }, string> {
  return {
    ...
    categoryHint: BROWSER_TOOL_CATEGORY,
    impl: async ({ selector, start: rawStart }, { sessionId, cwd, abortSignal }) => {
...
export function buildBrowserTools(): SharkerTool[] {
  return [
