...
export interface BrowserViewHost {
  ...
  /**
   * Allow this session's view to open workspace `.html` / `.htm` under `root`.
   * Optional on test fakes that never load `file://`.
   */
  allowLocalHtmlRoot?(sessionId: string, root: string): void;
}
