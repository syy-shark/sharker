...
import { normalizeBrowserAddressInput, type BrowserState, type BrowserViewRect } from '@sharker/core/browser';
import { resolveWorkspaceHtmlPage } from './local-page.js';

export type { BrowserState, BrowserViewRect };

export interface ParseNavigableOptions {
  /** Workspace roots that may serve a local `.html` / `.htm` `file://` page. */
  localHtmlRoots?: readonly string[];
}
 ...
 * Arbitrary `file://` is rejected; a workspace `.html` / `.htm` is allowed only
 * when `localHtmlRoots` is supplied and the path stays inside one of them.
 * Local HTML is tried first so `intro.html` is not rewritten to `https://intro.html/`.
 */
export function parseNavigable(input: string, options?: ParseNavigableOptions): string | null {
  const roots = options?.localHtmlRoots;
  if (roots?.length) {
    const local = resolveWorkspaceHtmlPage(input, roots);
