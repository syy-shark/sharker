...
const NATIVE_FILE = 'sharker_runtime_host_peer.node';
...
export async function configureDesktopRuntimeHostPeerClient(input: {
  ...
  environment.SHARKER_RUNTIME_HOST_PEER_NATIVE_PATH = nativePath;
  const keyPath = join(input.clientDataRoot, 'runtime-host-client.peer.key');
  environment.SHARKER_RUNTIME_HOST_PEER_KEY_PATH = keyPath;
