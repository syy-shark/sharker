...
import { appIconForTheme, type AppSettings } from '@sharker/core/settings';
...
const titleBarOverlayOptions = (
  ...
  // (sharker-tokens.css :root, oklch(1 0 0) == #ffffff) so the top-right
...
export function createMainWindowController(deps: MainWindowControllerDeps): MainWindowController {
  ...
  async function createWindow(signal: AbortSignal): Promise<void> {
    ...
    mainWindow = new BrowserWindow({
      ...
      // flashes the index.html `.sharker-preload` skeleton before React paints.
      ...
      // never flashes the `.sharker-preload` skeleton past it.
      show: false,
      ...
      // Skip vibrancy under SHARKER_E2E_FIXTURE — fixture / E2E
      // environments can't paint native window material reliably.
      ...(process.platform === 'darwin' && !process.env.SHARKER_E2E_FIXTURE
      ...
      webPreferences: {
        ...
        contextIsolation: true,    // window.sharker via contextBridge only
      ...
      mainWindow?.webContents.executeJavaScript(`
        (() => {
          const block = (e) => {
            ...
            if (target?.closest('[data-sharker-file-drop-target="true"]')) return;
            e.preventDefault();
            e.stopPropagation();
          };
    ...
    if (process.env.SHARKER_REAL_WINDOW_SMOKE === '1') {
  ...
  return {
    ...
    focus() {
      ...
      // flash the `.sharker-preload` skeleton past the reveal gate. The gate
...
function e2eFixtureWindowBounds(
  ...
  const width = Number(process.env.SHARKER_E2E_FIXTURE_WIDTH);
  const height = Number(process.env.SHARKER_E2E_FIXTURE_HEIGHT);
  if (
  ...
  target.webContents
    .executeJavaScript(
      `(() => ({
        ...
        searchModalPresent: Boolean(document.querySelector('[data-sharker-contract="search-modal"]')),
        searchModalOpen: Boolean(document.querySelector('dialog[data-sharker-contract="search-modal"][open]')),
        errorBoundaryPresent: Boolean(document.querySelector('.sharker-error-surface')),
        bodyTextLength: document.body?.innerText?.trim().length ?? 0,
        bodyTextSample: document.body?.innerText?.trim().slice(0, 240) ?? '',
        stylesheetCount: document.styleSheets.length,
        rootChildren: document.getElementById('root')?.children.length ?? 0,
        elements: ['body', '#root', '.appFrame', '.app', '.sharker-panel-detail', '.mainColumn', '.sharker-onboarding-loading'].map((selector) => {
        ...
        activeElementInSearchModal: Boolean(document.activeElement && document.activeElement.closest && document.activeElement.closest('[data-sharker-contract="search-modal"]')),
