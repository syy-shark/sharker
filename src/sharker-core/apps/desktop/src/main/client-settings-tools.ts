...
import { isThemePalette, type AppSettings, type ThemePalette, type UpdateAppSettingsInput } from '@sharker/core/settings';
import { UI_LOCALE_PREFERENCES } from '@sharker/core/ui-locale';
...
export function buildClientSettingsTools(
  ...
  const readTool: SharkerTool<Record<string, never>, ClientSettingsSnapshot> = {
    name: 'SharkerClientSettingsGet',
  ...
  const updateTool: SharkerTool<ClientSettingsPatch, ClientSettingsUpdateResult> = {
    name: 'SharkerClientSettingsUpdate',
    ...
    impl: async (input) => {
      ...
      if (!(await authority.confirm(proposedChanges))) {
        return {
          kind: 'sharker_client_settings_update',
          applied: false,
          reason: 'cancelled',
...
type ClientSettingsUpdateResult = {
  readonly kind: 'sharker_client_settings_update';
