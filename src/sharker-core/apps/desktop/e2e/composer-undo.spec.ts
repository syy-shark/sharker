...
test('pasted absolute path sends on the first Enter without opening the slash menu', async ({
  ...
  await expect(page.locator('button.sharker-workspace-picker')).toBeEnabled();
  await composer.click();
  await pastePlainText(composer, pasted);
...
test('plain-text paste synchronizes the controlled draft once', async ({ window: page }) => {
  ...
  await expect(page.locator('button.sharker-workspace-picker')).toBeEnabled();
  await page.evaluate(() => {
    const storageKey = 'sharker-new-task-reload-intent-v1';
    ...
    Object.defineProperty(globalThis, '__sharkerComposerDraftWrites', {
      configurable: true,
      get: () => writes,
    });
  });
  ...
  const writes = await page.evaluate(
    () =>
      (globalThis as typeof globalThis & { __sharkerComposerDraftWrites?: number })
        .__sharkerComposerDraftWrites ?? 0,
  );
