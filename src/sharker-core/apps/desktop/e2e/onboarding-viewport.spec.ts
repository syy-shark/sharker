...
test('first-run onboarding stays within the chat viewport', async ({ onboardingWindow }) => {
  const geometry = await onboardingWindow.evaluate(() => {
    ...
    const surface = document.querySelector<HTMLElement>('[data-sharker-contract="onboarding-surface"]');
    const card = document.querySelector<HTMLElement>('[data-sharker-contract="onboarding-card"]');
    ...
    return {
      ...
      onboardingLayout: scrollContainer.dataset.sharkerOnboarding,
...
test('first-run onboarding remains reachable at the minimum window size', async ({
  ...
  const surface = onboardingWindow.locator('[data-sharker-contract="onboarding-surface"]');
  ...
  const geometry = await onboardingWindow.evaluate(() => {
    ...
    const scrollContainer = document.querySelector<HTMLElement>('[data-chat-scroll-container="true"]');
    const surface = document.querySelector<HTMLElement>('[data-sharker-contract="onboarding-surface"]');
    const content = surface?.querySelector<HTMLElement>('.sharker-onboarding-center');
