...
test('switching sessions does not rewrite the whole Session rail', async ({
  ...
  const target = page.locator('.sharker-session-row button.astryx-side-nav-item', {
    hasText: 'Rail row 3',
  });
  const selected = page.locator('.sharker-session-row button.astryx-side-nav-item.selected');
  await expect(target).toBeVisible();

  await page.evaluate(() => {
    const selectedRowId = (): string | null =>
      document
        .querySelector('.sharker-session-row button.astryx-side-nav-item.selected')
        ?.closest('.sharker-session-row')
      ...
      for (const record of records) {
        if (record.type === 'childList') {
          for (const node of record.addedNodes) {
            ...
            if (element.classList?.contains('sharker-session-row')) counters.rowRemounts += 1;
          }
        ...
        const row = (record.target as Element).closest?.('.sharker-session-row');
        if (!row) continue;
  ...
  const targetId = await page
    .locator('.sharker-session-row', { hasText: 'Rail row 3' })
