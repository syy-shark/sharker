...
const PERF_ENABLED = process.env.SHARKER_TRANSCRIPT_PERF === '1';
const STRESS_ENABLED = process.env.SHARKER_TRANSCRIPT_STRESS === '1';
...
async function prepareFrameRecorder(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    Object.assign(window, { __sharkerTranscriptPerf: state });
...
async function scrollGesture(page: Page, delta: number, frames = 240): Promise<FrameSample> {
  return page.evaluate(async ({ selector, delta, frames }) => {
    ...
    const recorder = (window as Window & { __sharkerTranscriptPerf?: Recorder })
      .__sharkerTranscriptPerf;
...
async function returnToLatest(page: Page): Promise<void> {
  ...
  else await page.locator('.sharker-prompt-rail-tick').last().click({ force: true });
}
...
async function measureSessionSwitch(page: Page): Promise<number> {
  ...
  const originalId = await selected.evaluate((button) =>
    button.closest('.sharker-session-row')?.getAttribute('data-session-id'),
  );
