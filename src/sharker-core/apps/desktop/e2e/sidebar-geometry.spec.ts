 ...
 *
 *   1. Geometry — `.sharker-sidenav-motion { height: 100% }` (shell-layout.css).
...
async function revealPopulatedSidebar(page: Page): Promise<Locator> {
  ...
  await expect(page.locator('[data-sharker-contract="search-modal"]')).not.toBeVisible();
...
async function wrapperWidth(wrapper: Locator): Promise<number> {
  ...
  if (!box) throw new Error('sidenav wrapper (.sharker-sidenav-motion) has no visible bounds');
  return box.width;
}
...
test('sidenav footer stays inside the window under an overflowing list', async ({
  ...
  const listOverflows = await page.evaluate(() => {
    const nav = document.querySelector('nav.sharker-session-panel');
    if (!nav) return false;
  ...
  const wrapper = page.locator('.sharker-sidenav-motion');
  const footer = page.locator('.sharker-session-panel-footer');
