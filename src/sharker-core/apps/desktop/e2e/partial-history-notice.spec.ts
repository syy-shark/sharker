...
const NOTICE = '.sharker-transcript-history-controls';
...
async function noticePresentation(page: Page) {
  return page.locator(NOTICE).evaluate((notice) => {
    ...
    const composer = document.querySelector('.sharker-composer-astryx');
...
test('partial history is a quiet reading-column control with neutral rail ticks', async ({
  ...
  const firstPrompt = page.locator(
    '.sharker-prompt-rail-tick[data-prompt-turn-id="turn-partial-history-1"]',
  );
  ...
  const railPresentation = await page.evaluate(() => {
    const ticks = [...document.querySelectorAll<HTMLElement>('.sharker-prompt-rail-tick')];
    const presentation = (tick: HTMLElement) => {
      const bar = tick.querySelector<HTMLElement>('.sharker-prompt-rail-tick-bar');
