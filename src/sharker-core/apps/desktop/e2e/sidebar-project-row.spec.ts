...
test('task row action menu accepts pointer selection', async ({
  ...
  const actionMenu = taskRow.locator(':scope > .sharker-session-row-action');
  const timestamp = taskRow.locator('.sharker-session-row-time');
...
test('rail grouping survives a renderer reload', async ({ projectSidebarWindow: page }) => {
  ...
  await expect
    .poll(() => page.evaluate(() => localStorage.getItem('sharker-chat-list-view-mode-v1')))
    .toBe('project');

  await page.reload();
  await expect(page.locator('[data-sharker-contract="search-modal"][open]')).toBeVisible();
  await page.keyboard.press('Escape');
  await expect(page.locator('[data-sharker-contract="search-modal"]')).not.toBeVisible();
