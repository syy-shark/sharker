 ...
 * `.sharker-chat-shell` while Astryx's ChatLayout owned the scroll container, so
 ...
 *  - e2e-fixture renders carry `data-sharker-e2e-fixture`, and `base.css` gives
...
const RAIL_PROBE = `(() => {
  ...
  const rail = document.querySelector('.sharker-prompt-rail');
...
test('every tick paints a bar with a real box', async ({ promptRailWindow: page }) => {
  ...
  const bars = await page.evaluate(() =>
    [...document.querySelectorAll('.sharker-prompt-rail-tick-bar')].map((bar) => {
...
test('the pointer is always on a tick while it travels down the rail', async ({
  ...
  const travel = await page.evaluate(() => {
    const bars = [...document.querySelectorAll('.sharker-prompt-rail-tick-bar')];
    ...
    for (let y = Math.round(first.top + first.height / 2); y <= Math.round(last.top + last.height / 2); y += 1) {
      ...
      if (!found?.closest('.sharker-prompt-rail-tick')) misses.push(y);
    }
...
test('the first click of a session lands on its prompt and holds', async ({
  ...
  // collapse is keyed on `data-sharker-e2e-fixture`, not on the media query.
  expect(await page.evaluate(() => document.documentElement.dataset.sharkerScrollMotion)).toBe(
    'smooth',
  );
  ...
  await page.locator('.sharker-prompt-rail-tick').first().click({ force: true });
  ...
  const landing = async () =>
    page.evaluate((turnId) => {
      ...
      if (!turn) return null;
      const tick = document.querySelector('.sharker-prompt-rail-tick');
      return {
        offset: Math.round(
...
test('active transcript Turns keep stable DOM identities while scrolling', async ({
  ...
  const sourceCount = Number(
    await page.locator('.sharker-chat-message-list').getAttribute('data-turn-source-count'),
  );
...
test('switching sessions reconstructs only the Host active range', async ({
  ...
  const rows = page.locator('.sharker-session-row');
  const selected = rows.locator('button.astryx-side-nav-item.selected');
  const originalId = await selected
    .evaluate((button) => button.closest('.sharker-session-row')?.getAttribute('data-session-id'));
  ...
  await page.locator(`.sharker-session-row[data-session-id=${JSON.stringify(otherId)}] button`)
    .first()
    .click();
  await expect(page.locator(
    `.sharker-session-row[data-session-id=${JSON.stringify(otherId)}] button.selected`,
  ))
    .toHaveCount(1);

  await page.locator(`.sharker-session-row[data-session-id=${JSON.stringify(originalId)}] button`)
    .first()
    .click();
  await expect(page.locator('[data-turn-id="turn-prompt-rail-120"]')).toHaveCount(1);
...
test('a tick is what the pointer lands on, not the scrollbar', async ({
  ...
  const hit = await page.evaluate(() => {
    ...
    return { insideRail: found?.closest('.sharker-prompt-rail') !== null };
  });
