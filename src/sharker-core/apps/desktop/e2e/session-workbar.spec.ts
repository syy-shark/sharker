...
async function ensureRightWorkbarOpen(page: Page) {
  ...
  await expect(
    page.locator('.sharker-session-workbar[data-placement="right"]'),
  ).toBeVisible();
}
...
test('a collapsed workbar never flashes during the first send', async ({
  ...
  await page.evaluate(() => {
    ...
    const inspect = () => {
      const panel = document.querySelector<HTMLElement>(
        '.sharker-session-workbar[data-placement="right"]',
      );
      if (
        panel &&
    ...
    (
      window as typeof window & {
        __sharkerFirstSendWorkbarWatch?: typeof watch;
        __sharkerFirstSendWorkbarWatchStop?: () => void;
      }
    ).__sharkerFirstSendWorkbarWatch = watch;
    (
      window as typeof window & {
        __sharkerFirstSendWorkbarWatchStop?: () => void;
      }
    ).__sharkerFirstSendWorkbarWatchStop = () => {
      inspect();
      observer.disconnect();
    };
  });
  ...
  const watch = await page.evaluate(() => {
    const target = window as typeof window & {
      __sharkerFirstSendWorkbarWatch?: { visibleRightWorkbar: boolean };
      __sharkerFirstSendWorkbarWatchStop?: () => void;
    };
    target.__sharkerFirstSendWorkbarWatchStop?.();
    return target.__sharkerFirstSendWorkbarWatch;
  });
...
async function waitForCompanionForkId(page: Page, sourceSessionId: string) {
  ...
  await expect
    .poll(async () => {
      forkId = (await page.evaluate(() => window.sharker.sessions.list())).find(
...
async function setRightWorkbarWidth(page: Page, width: number) {
  const layoutOwner = page.locator('.sharker-workbar-layout-vars');
...
test('narrow right workbar keeps launcher shortcuts and side-chat send button inside', async ({
  ...
  await page
    ...
    .click();
  const companion = page.locator('.sharker-quote-companion');
  await expect(companion).toBeVisible();
  await setRightWorkbarWidth(page, 320);
  const sideChatPanel = page
    .locator('.sharker-session-workbar-panel[data-overlay][data-placement="right"]')
  ...
  const composerCard = companion.locator('.sharker-composer-astryx');
  ...
  const composerRadii = await page.evaluate(() => {
    const wrappers = [...document.querySelectorAll('.sharker-composer-astryx')];
    const inCompanion = (element: Element) => element.closest('.sharker-quote-companion') !== null;
  ...
  await companion.locator('.sharker-composer-model-chip-text').evaluate((element) => {
...
test('browser address field tracks the workbar column width', async ({ window: page }) => {
  ...
  // so the width override must land there, not on `.sharker-session-workbar`.
  const setPanelWidth = async (width: number) => {
    const panel = page.locator('.sharker-session-workbar-panel[data-overlay][data-placement="right"]');
    await panel.evaluate((element, nextWidth) => {
      (element as HTMLElement).style.setProperty('--sharker-session-workbar-width', `${nextWidth}px`);
    }, width);
...
test('titlebar workbar action restores an existing tool instead of the picker', async ({
  ...
  await page.evaluate((width) => {
    document.documentElement.style.setProperty(
      '--sharker-titlebar-overlay-right-width',
      `${width}px`,
    );
...
test('Terminal ownership follows the active Session and stops the old resource', async ({
  ...
  await expect
    .poll(async () =>
      (await page.evaluate((id) => window.sharker.shellRuns.list(id), sessionId))
...
test('Side Chat survives collapse, confirms close, and cleans up on source switch', async ({
  ...
  await expect
    .poll(async () =>
      (await page.evaluate(() => window.sharker.sessions.list()))
