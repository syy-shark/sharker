...
test('a failed first observation seed reconnects to the live Turn', async ({ window: page }) => {
  const latchInstalled = await page.evaluate(() => {
    const latch = (window as SessionObservationLatchWindow).sharkerE2eLatch;
  ...
  await expect(page.locator('.sharker-bubble-streaming')).toContainText(
...
test('remounting a live surface leaves accumulated output settled', async ({
  ...
  const liveBubble = page.locator('.sharker-bubble-streaming');
  ...
  const currentTaskRow = sidebar.locator(
    '[data-sharker-contract="session-row"] [aria-current="page"]',
  );
  ...
  await liveBubble.evaluate((element) => {
    ...
    (window as typeof window & { __sharkerStreamingRemountObserved?: typeof observed })
      .__sharkerStreamingRemountObserved = observed;
  ...
  const observed = await page.evaluate(() => (
    window as typeof window & {
      __sharkerStreamingRemountObserved?: {
        texts: string[];
      };
    }
  ).__sharkerStreamingRemountObserved);
...
test('keeps a completed reply after an interrupted turn and conversation remount', async ({
  ...
  await expect.poll(
    () => page.evaluate(async (sessionId) => (
      (await window.sharker.sessions.list()).find((session) => session.id === sessionId)
  ...
  await composer.press('Enter');
  await expect(page.locator('.sharker-user-message', {
  ...
  await expect(page.locator('.sharker-bubble-streaming')).toHaveCount(0, {
    timeout: 20_000,
  });
...
test('returning to a live conversation settles output accumulated while away', async ({
  ...
  await page.evaluate(
    ({ sessionId, steering }) => new Promise<void>((resolve, reject) => {
      ...
      const unsubscribe = window.sharker.sessions.subscribeEvents(sessionId, (event) => {
  ...
  await page.evaluate(() => {
    ...
    const sample = () => {
      if (stopped) return;
      const bubble = document.querySelector<HTMLElement>('.sharker-bubble-streaming');
      if (bubble) {
    ...
    (
      window as typeof window & {
        __sharkerBackgroundRestoreObserved?: typeof observed;
      }
    ).__sharkerBackgroundRestoreObserved = observed;
    window.requestAnimationFrame(sample);
  });
    ...
    () =>
      new Promise<{
        ...
        window.requestAnimationFrame(() => {
          const observed = (
            window as typeof window & {
              __sharkerBackgroundRestoreObserved?: {
                texts: string[];
                maxActiveAnimations: number;
                stop(): void;
              };
