...
test('shows only slash commands executable in the current session state', async ({
  ...
  await expect(freshMenu.getByRole('group', { name: '命令' })).toHaveCount(0);
  await expect(freshMenu.getByRole('option', { name: /\/graph/ })).toHaveCount(0);
  await expect(freshMenu.getByRole('option', { name: /\/swarm/ })).toHaveCount(0);
  ...
  await expect(groups.nth(0).getByRole('option')).toHaveCount(2);
  await expect(groups.nth(0).getByRole('option', { name: /压缩上下文.*\/compact/ })).toBeVisible();
  await expect(groups.nth(0).getByRole('option', { name: /打开侧聊.*\/side/ })).toBeVisible();
  await expect(groups.nth(0).getByRole('option', { name: /\/graph/ })).toHaveCount(0);
  await expect(groups.nth(0).getByRole('option', { name: /\/swarm/ })).toHaveCount(0);
});
...
test('compacts the active session', async ({
  ...
  const sessionId = await page.evaluate(async () => (await window.sharker.sessions.list())[0]?.id);
  expect(sessionId).toBeTruthy();
  await page.evaluate((activeSessionId) => {
    const testWindow = window as typeof window & {
      __sharkerObservedCompactCompletion?: boolean;
    };
    testWindow.__sharkerObservedCompactCompletion = false;
    window.sharker.sessions.subscribeChanges((event) => {
      if (event.reason === 'status-change' && event.sessionId === activeSessionId) {
        testWindow.__sharkerObservedCompactCompletion = true;
      }
    });
  }, sessionId!);
  ...
  await expect
    .poll(() =>
      page.evaluate(
        () =>
          (window as typeof window & { __sharkerObservedCompactCompletion?: boolean })
            .__sharkerObservedCompactCompletion,
      ),
    )
    .toBe(true);
...
test('dispatches /side instead of steering it into a running turn', async ({
  ...
  await composer.press('Enter');
  await expect(page.locator('.sharker-user-message', { hasText: runningPrompt })).toBeVisible();
  ...
  await expect(page.locator('.sharker-quote-workbar-panel')).toHaveCount(0);

  await composer.fill('/side discuss separately');
  await composer.press('Enter');

  await expect(page.locator('.sharker-quote-workbar-panel')).toHaveCount(1);
  ...
  const observation = await menu.evaluate(async (menuElement) => {
    const sessions = await (
      window as unknown as {
        sharker: { sessions: { list(): Promise<Array<{ id: string }>> } };
      }
    ).sharker.sessions.list();
    ...
    try {
      for (let round = 0; round < 3; round += 1) {
        ...
        await sharker.sessions.setThinkingLevel(sessionId, null);
