...
import { createProjectCatalog } from '@sharker/storage/project-catalog';
import { createSessionStore } from '@sharker/storage/session-store';
 ...
 */
export const COMPOSER_INPUT = '.sharker-composer-editor [contenteditable="true"]';
...
export async function waitForWorkHubReady(page: Page, workCount: number): Promise<void> {
  await expect
    .poll(async () => {
      const snapshot = await page.evaluate(() => window.sharker.runtimeHostProfiles.getSnapshot());
...
export async function waitForInvocableSkills(
  ...
  await expect
    .poll(async () =>
      page.evaluate(async () =>
        (await window.sharker.skills.listInvocable(undefined)).map((skill) => skill.id),
      ),
    )
...
async function seedE2eInvocableSkills(userDataDir: string): Promise<void> {
  ...
  const projectSkillRoot = path.join(projectRoot, '.sharker', 'skills');
...
async function seedE2eGitReviewProject(
  ...
  await git('config', 'user.email', 'sharker-e2e@example.invalid');
 ...
 * Electron and a leaked `sharker-e2e-*` directory.
 */
async function withE2eWindow(
  ...
  const userDataDir = await mkdtemp(path.join(tmpdir(), 'sharker-e2e-'));
...
export const test = base.extend<{
  ...
  onboardingWindow: async ({}, use) => {
    await withE2eWindow({
      seed: false,
      readinessSelector: '[data-sharker-contract="onboarding-card"]',
      locale: 'zh',
      showWindow: true,
    }, use);
  },
  ...
  projectSidebarWindow: async ({}, use) => {
    await withE2eWindow({
      seed: false,
      readinessSelector: '[data-sharker-contract="search-modal"][open]',
