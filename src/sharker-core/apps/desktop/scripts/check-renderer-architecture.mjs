...
const DESKTOP_SELF_PREFIX = '@sharker/desktop/';
...
function isWindowSharker(node, windowAliases) {
  const expression = unwrapExpression(node);
  return (
    isMemberExpression(expression) &&
    isWindowExpression(expression.object, windowAliases) &&
    memberPropertyName(expression) === 'sharker'
  );
}
...
function bridgePath(node, windowAliases, bridgeAliases = new Map()) {
  ...
  if (isWindowSharker(expression, windowAliases)) return 'window.sharker';
  if (!isMemberExpression(expression)) return undefined;
  ...
  while (changed) {
    ...
    function visit(node) {
      ...
      if (node.type === 'VariableDeclarator' && node.id?.type === 'Identifier') {
        ...
        if (environment && !environment.startsWith('window.sharker')) {
      ...
      if (node.type === 'VariableDeclarator' && node.id?.type === 'ObjectPattern') {
        ...
        for (const property of node.id.properties) {
          ...
          if (isWindowExpression(node.init, windowAliases) && propertyName === 'sharker') {
            addAlias(bridgeAliases, property.value.name, 'window.sharker');
          }
          if (environmentBase && !environmentBase.startsWith('window.sharker')) {
...
export function analyzeRendererSource(source, file = 'fixture.ts') {
  ...
  function visit(node, parent) {
    ...
    if (
      ...
      !directEnvironmentPath.startsWith('window.sharker') &&
    ...
    if (isMemberExpression(node) && memberPropertyName(node) === undefined) {
      ...
      if (dynamicEnvironmentBase && !dynamicEnvironmentBase.startsWith('window.sharker')) {
    ...
    if (
      ...
      for (const property of node.id.properties) {
        if (property.type === 'ObjectProperty' && !property.computed && memberName(property.key) === 'sharker') {
          recordBridgePath('window.sharker');
        }
      }
    }
