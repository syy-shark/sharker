...
async function withDesktopFixture(files, run) {
  const desktopRoot = await mkdtemp(join(tmpdir(), 'sharker-renderer-architecture-'));
  try {
...
describe('renderer architecture checker fixtures', () => {
  ...
  it('resolves @sharker/desktop self-imports before enforcing zone boundaries', async () => {
    await withDesktopFixture(
      {
        'src/renderer/features/alpha/controller.ts': `
          import { desktopOnly } from '@sharker/desktop/src/renderer/platform/desktop/private.js';
          export const alpha = desktopOnly;
        `,
        'src/renderer/shell/shell-frame.ts': `
          import { BetaHost } from '@sharker/desktop/src/renderer/features/beta/index.js';
    ...
    await withDesktopFixture(
      {
        'src/renderer/application/computed-bridge.ts': `
          void window['sharker'].sessions.list();
        `,
        'src/renderer/application/optional-bridge.ts': `
          void window?.sharker?.sessions?.subscribeEvents?.(() => undefined);
        `,
        'src/renderer/application/dynamic-global.ts': `
          const bridgeName = 'sharker';
    ...
    await withDesktopFixture(
      {
        ...
        'src/renderer/platform/desktop/allowed-capabilities.ts': `
          export function createAllowedCapabilities() {
            ...
            return window.sharker.sessions.list();
          }
        `,
      },
      (desktopRoot) => {
  ...
  it('ratchets legacy helpers reachable through a feature public ownership boundary', async () => {
    await withDesktopFixture(
      ...
      (desktopRoot) => {
        ...
        assertHasViolation(
          violations,
          /^src\/renderer\/legacy-session-helper\.ts: new or increased bridgePaths debt window\.sharker\.sessions\.list$/u,
        );
        assertHasViolation(
          violations,
  ...
  it('ratchets capability debt owned directly by a shared Desktop intermediary', async () => {
    ...
    await withDesktopFixture(
      ...
      (desktopRoot) => {
        ...
        assertHasViolation(
          violations,
          /^src\/shared\/root-bridge\.ts: new or increased bridgePaths debt window\.sharker\.sessions\.list$/u,
        );
        assertHasViolation(
          violations,
    ...
    await withDesktopFixture(
      {
        ...
        [sharedPath]: `export const rootBridge = window.sharker.sessions.list();`,
      },
      async (desktopRoot) => {
  ...
  it('rejects bridge debt that grows relative to the base ledger', async () => {
    ...
    const currentConfig = architectureConfig({
      rootDebt: {
        [debtPath]: emptyDebt({ bridgePaths: { 'window.sharker.sessions.list': 1 } }),
      },
    });
    ...
    await withDesktopFixture(
      {
        [debtPath]: 'void window.sharker.sessions.list();',
      },
      (desktopRoot) => {
        const violations = violationsFor(desktopRoot, currentConfig, baseConfig);
        assertHasViolation(
        ...
        assertHasViolation(
          violations,
          /^src\/renderer\/legacy-root\.ts: new or increased bridgePaths debt window\.sharker\.sessions\.list$/u,
        );
      },
    );
  });
      ...
      () =>
        assertRendererEntryHtml(
          canonicalRendererEntryHtml(
            '<script>window.sharker.sessions.list()</script>',
  ...
  it('rejects deleting root debt while the existing root entry is still dirty', async () => {
    ...
    const baseConfig = architectureConfig({
      ...
      rootDebt: {
        [debtPath]: emptyDebt({
          bridgePaths: { 'window.sharker.sessions.list': 1 },
  ...
  it('rejects removing the permanent root guard while a clean thin entry still exists', async () => {
    ...
    const baseConfig = architectureConfig({
      ...
      rootDebt: {
        [debtPath]: emptyDebt({
          bridgePaths: { 'window.sharker.onboarding.getSnapshot': 1 },
          hookCalls: { useEffect: 1 },
        }),
      },
    });
  ...
  it('rejects bridge debt returning to a retained clean root entry', async () => {
    ...
    await withDesktopFixture(
      ...
      (desktopRoot) => {
        ...
        assertHasViolation(
          violations,
          /^src\/renderer\/main\.tsx: new or increased bridgePaths debt window\.sharker\.sessions\.list$/u,
        );
      },
    );
  });
    ...
    await withDesktopFixture(
      {
        'src/renderer/application/bridge-aliases.ts': `
          ...
          void desktopWindow.sharker.sessions.list();
          void globalThis.sharker.sessions.list();
          void self['sharker'].sessions.list();
          void (window as Window).sharker.sessions.list();
          void window!.sharker.sessions.list();
          const { sharker } = window;
          void sharker.sessions.list();
        `,
  ...
  it('retains full bridge method paths after the Desktop bridge is aliased', () => {
    const analysis = analyzeRendererSource(
      `
        const bridge = window.sharker;
        void bridge.sessions.list();
        const { sharker: desktop } = window;
        void desktop.transcripts.open('session-a');
      `,
      'src/renderer/application/bridge-method-aliases.ts',
    );

    assert.equal(analysis.bridgePaths['window.sharker.sessions.list'], 1);
    assert.equal(analysis.bridgePaths['window.sharker.transcripts.open'], 1);
  });
