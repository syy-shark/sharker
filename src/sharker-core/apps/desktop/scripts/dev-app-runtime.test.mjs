...
test('a launcher signal cancels preparation before Electron can spawn', async () => {
  ...
  signals.emit('SIGINT');
  finishPreparation({ appPath: '/tmp/Sharker Dev.app', resultFileArgPrefix: '--result=' });
...
test('launcher and core agree on the loser contract', async () => {
  ...
  const core = await import('@sharker/core/dev-single-instance');
...
test('plainLoserExitCode matches only the contract value', async () => {
  ...
  const { DEV_LOSER_EXIT_CODE } = await import('@sharker/core/dev-single-instance');
...
async function runVerdictCase({ result, stopped = () => false, startupAttempts = 10 }) {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'sharker-abs-'));
...
test('development-app liveness probes only the TCC bundle', async () => {
  ...
  assert.equal(
    isDevelopmentAppRunning({
      executable: '/wt/Sharker Dev.app/Contents/MacOS/Electron',
      probe: (executable) => {
  ...
  assert.deepEqual(calls, ['/wt/Sharker Dev.app/Contents/MacOS/Electron']);
});
...
test('TCC launcher cleanup removes only its own launch artifacts', async () => {
  ...
  const { tmpdir } = await import('node:os');
  const { join } = await import('node:path');
  const directory = mkdtempSync(join(tmpdir(), 'sharker-cleanup-'));
...
test('each TCC launch gets private and separate result and log files', async () => {
  ...
  const first = createDevelopmentLaunchFiles({
    directory: '/tmp/sharker-dev-launches',
    id: 'first',
    mkdir: () => {},
...
test('shared-profile launches warn about legacy TCC data before choosing plain or bundle mode', async () => {
  ...
  warnAboutLegacyTccDataRoot({
    ...
    legacyUserDataDir: '/tmp/Sharker Dev-legacy',
    exists: () => true,
    warn: (message) => warnings.push(message),
  });
  assert.equal(warnings.length, 1);
  assert.match(warnings[0], /Sharker Dev-legacy/);
});
...
test('createMacosDevelopmentLaunch passes its private result file after --args', async () => {
  ...
  const { DEV_LAUNCH_RESULT_FILE_ARG_PREFIX } = await import('@sharker/core/dev-single-instance');
  ...
  assert.ok(
    !createMacosDevelopmentLaunch('/tmp/Sharker Dev.app', '/tmp/app.log').args.includes('--args'),
  );
});
