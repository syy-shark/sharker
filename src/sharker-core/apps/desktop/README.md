...
Set `SHARKER_DEV_TCC=1` to launch through a generated, ad-hoc-signed
`apps/desktop/.sharker-dev/Sharker Dev.app` instead.

```sh
SHARKER_DEV_TCC=1 npm run dev
```

The bundle gives TCC a stable identity (`com.sharker.dev.<worktree-id>`), a
...
`npm --workspace @sharker/desktop run prepare:dev-app` to prepare it explicitly.
...
Application-control variables (API keys, `SHARKER_*`, the Vite URL)
are published to an ignored `0600` file at `.sharker-dev/dev-env.json` rather than
...
The default profile is `~/Library/Application Support/Sharker Dev`, which keeps
development isolated from the packaged Sharker profile and is shared by the plain
...
Grant permissions to **Sharker Dev**, not a generic Electron entry. Screen
Recording changes require restarting the development app. Without
`SHARKER_DEV_TCC`, the permission overlay still runs, but its drag target is the
...
| preload | `src/preload/preload.ts` (single file) | `contextBridge.exposeInMainWorld('sharker', …)` — the only surface the renderer may call to reach Node/Electron. No Node API is directly exposed. |
...
Three patterns, all rooted in preload's `sharker` namespace. Channel names are `<domain>:<action>`.
...
Adding a new IPC surface: if extracting, write the `*-ipc-main.ts` exporting a `register*Ipc(...)`, import it in `main.ts`, and call it inside `registerIpc()`; add the matching method to the `sharker` namespace in `preload.ts`; add the method to the `window.sharker` type in `src/global.d.ts` (the renderer's typed bridge — without it, renderer calls get a TS error); keep the `<domain>:<action>` channel naming. A handler file that isn't registered in `registerIpc()` compiles but never mounts.

## Data flow

```
renderer (React)
  └─ window.sharker.<ns>.<method>(…)        // typed surface, see preload.ts
      └─ ipcRenderer.invoke / send / on
          └─ main: safeSendToRenderer / ipcMain.handle / ipcMain.on
              └─ Runtime Host protocol → @sharker/runtime + @sharker/storage
```

The renderer never imports `@sharker/runtime` or `@sharker/storage` at runtime — all Node-side access goes through the preload `sharker` bridge. The renderer only pulls `import type` from them for a few shared types. Types shared across the IPC boundary mostly come from `@sharker/core`, with some from `@sharker/runtime`, `@sharker/storage`, and `@sharker/ui` (see `preload.ts` imports).
