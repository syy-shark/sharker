...
async function stageReleaseManifests({ packager }) {
  const stage = await packager.info.tempDirManager.createTempDir({
    prefix: 'sharker-release-manifests',
  });
  ...
  packager.config.files.push({ from: stage, to: 'node_modules/@sharker' });
}
...
const baseDesktopBuilderConfig = {
  appId: 'com.sharker.desktop',
  productName: 'Sharker',
  artifactName: 'Sharker-${version}-mac-${arch}.${ext}',
  asar: true,
  beforePack: stageReleaseManifests,
  extraMetadata: { runtimeHostSetupPackage, sharkerUpdateChannel: 'release' },
  directories: {
    output: 'release',
  },
  ...
  //
  // `@xterm/headless` stays a dependency on purpose — `@sharker/runtime` imports
  // it for the PTY stack, so only the renderer-side xterm packages moved.
  files: [
    'dist/**/*',
    'dist-renderer/**/*',
    'package.json',
    '!node_modules/@sharker/{mcp,runtime,runtime-host}/package.json',
    '!**/__tests__/**',
    ...
    // `@sharker/ui` and `@astryxdesign/core`, which the renderer now bundles
  ...
  extraResources: [
    ...
    {
      from: '../../native/runtime-host-peer/target/release/sharker_runtime_host_peer.node',
      to: 'runtime-host-peer/sharker_runtime_host_peer.node',
    },
    {
    ...
    ...(process.platform === 'win32'
      ? [
          {
            from: 'resources/windows-sandbox/sharker-windows-sandbox.exe',
            to: 'windows-sandbox/sharker-windows-sandbox.exe',
          },
          {
    ...
    {
      from: '../../LICENSE',
      to: 'licenses/sharker/LICENSE',
    },
    {
      from: '../../NOTICE',
      to: 'licenses/sharker/NOTICE',
    },
    {
      ...
      from: '../../DISCLAIMER-WIP',
      to: 'licenses/sharker/DISCLAIMER-WIP',
    },
    {
  ...
  mac: {
    ...
    extendInfo: {
      ...
      'Sharker may automate other applications when you explicitly run an agent task.',
    },
  },
  dmg: {
    title: 'Sharker Installer',
  ...
  win: {
    ...
    artifactName: 'Sharker-${version}-win-${arch}.${ext}',
...
export function resolveDesktopBuilderConfig(environment = process.env) {
  const nightlyVersion = environment.SHARKER_DESKTOP_NIGHTLY_VERSION?.trim();
  ...
  return {
    ...
    extraMetadata: {
      ...
      sharkerUpdateChannel: 'nightly',
    },
    publish: [{ provider: 'github', owner: 'apache', repo: 'sharker', channel: 'dev' }],
  };
}
