...
// its snapshot from `window.sharker.graphs` itself (not props or context), so each
...
// A scoped `window.sharker.graphs` bridge that serves one pinned snapshot. `fail`
...
// `.sharker-detail-with-artifacts`). Reuse those wrapper classes so the panel
...
export const EmptyGraph: Story = {
  decorators: [
    withScopedSharkerBridge(graphBridge(snapshot({ status: 'empty', scheduleRevision: 0 }))),
  ],
  render: panel,
...
export const FailedGraph: Story = {
  ...
  play: async ({ canvasElement }) => {
    await waitFor(() =>
      expect(
        canvasElement.querySelector('.sharker-agent-graph-operators li[data-status="failed"]'),
      ).not.toBeNull(),
    );
  },
};
...
export const BlockedOnUpstream: Story = {
  ...
  play: async ({ canvasElement }) => {
    await waitFor(() => {
      expect(
        canvasElement.querySelector('.sharker-agent-graph-operators li[data-status="blocked"]'),
      ).not.toBeNull();
      expect(canvasElement.querySelector('.sharker-agent-graph-wait')).not.toBeNull();
    });
  },
};
...
export const LoadError: Story = {
  decorators: [withScopedSharkerBridge(graphBridge(snapshot({ status: 'active' }), true))],
  render: panel,
  play: async ({ canvasElement }) => {
