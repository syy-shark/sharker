...
const registerDirectory = fn(async () => ({
  ...
  name: 'sharker-agent',
  locations: [{ path: '/workspace/sharker-agent', isWorktree: false }],
  available: true,
}));
...
export const RemoteProjectDirectory: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(await within(dialog).findByRole('button', { name: 'sharker-agent' }));
    ...
    await waitFor(() =>
      expect(registerDirectory).toHaveBeenCalledWith(
        { rootId: 'workspace', segments: ['sharker-agent'] },
