...
import { ToastProvider, useToast } from '@sharker/ui';
import type {
  AppSettings,
  SettingsSection,
  ThemePalette,
...
import { EMPTY_USAGE_PROVENANCE } from '@sharker/core/usage-ledger-merge';
import type { SessionSummary } from '@sharker/core/session';
import { revisionFamilySessionIds } from '@sharker/core/session-revisions';
import type { IdentifiedLlmConnection, LlmConnection, ProviderType } from '@sharker/core/llm-connections';
import { buildChatModelChoices } from '@sharker/core/chat-model-choice';
import type { LocalMemoryBackupInfo, LocalMemoryEntryPreview, LocalMemoryState } from '@sharker/core/local-memory';
import { createDefaultSettings, mergeSettings } from '@sharker/core/settings';
...
import { withScopedSharkerBridge } from '../sharker-bridge';
...
function makeMemoryBackup(kind: LocalMemoryBackupInfo['kind'], minutesAgo: number): LocalMemoryBackupInfo {
  return {
    path: `/Users/storybook/Library/Application Support/Sharker/workspaces/default/memory/MEMORY.md.${kind}.bak`,
    kind,
...
function makeMemoryState(input: {
  ...
  const content = input.entries
    ...
    .join('\n');
  return {
    path: '/Users/storybook/Library/Application Support/Sharker/workspaces/default/memory/MEMORY.md',
    enabled: true,
  ...
  return {
    memory: {
      ...
      listVault: async () => ({
        ok: true as const,
        value: {
          root: '/Users/storybook/Library/Application Support/Sharker/workspaces/default/memory',
          nodes: [
...
const sharkerBridge = {
  ...
  runtimeHostManagement: {
    run: async (
      _profileId: string,
      action: Parameters<typeof window.sharker.runtimeHostManagement.run>[1],
    ) => ({
  ...
  settings: {
    ...
    updateClient: async (
      patch: Parameters<typeof window.sharker.settings.updateClient>[0],
    ): Promise<UpdateAppSettingsResult> => {
      storyClientSettings = mergeSettings(storyClientSettings, patch);
      return { settings: storyClientSettings };
    },
    update: async (patch: Parameters<typeof window.sharker.settings.update>[0]): Promise<UpdateAppSettingsResult> => {
  ...
  // The OAuth cards on 模型 read their live state off window.sharker rather than
  ...
  app: {
    ...
    selectIcon: async (icon: Parameters<typeof window.sharker.app.selectIcon>[0]) => ({
      ok: true as const,
      selection: icon,
    }),
  ...
  // window.sharker.pets. Without this fixture the catalog story throws on mount
...
const withSettingsBridge = withScopedSharkerBridge(sharkerBridge);

function pendingForever<T>(): Promise<T> {
  return new Promise(() => undefined);
}

const withGeneralHostSettingsLoadingBridge = withScopedSharkerBridge({
  ...sharkerBridge,
  settings: {
    ...sharkerBridge.settings,
    get: () => pendingForever(),
  },
} satisfies Record<string, unknown>);

const withGeneralConnectionsLoadingBridge = withScopedSharkerBridge({
  ...sharkerBridge,
  connections: {
    ...connectionsBridge,
...
const withGeneralCachedRevalidationBridge = withScopedSharkerBridge({
  ...sharkerBridge,
  runtimeHostProfiles: {
    ...sharkerBridge.runtimeHostProfiles,
    getSnapshot: () => pendingForever(),
  },
  settings: {
    ...sharkerBridge.settings,
...
const withGeneralHostGenerationRevalidationBridge = withScopedSharkerBridge({
  ...sharkerBridge,
  runtimeHostProfiles: generationStoryRuntimeHostProfilesBridge,
} satisfies Record<string, unknown>);

const withModelsOAuthGenerationRevalidationBridge = withScopedSharkerBridge({
  ...sharkerBridge,
  runtimeHostProfiles: generationStoryRuntimeHostProfilesBridge,
  openAiCodex: {
    ...sharkerBridge.openAiCodex,
    getAccountState: async () => {
...
const withModelsConnectionsGenerationRevalidationBridge = withScopedSharkerBridge({
  ...sharkerBridge,
  runtimeHostProfiles: generationStoryRuntimeHostProfilesBridge,
  connections: {
    ...connectionsBridge,
...
const withModelsOAuthAuthorizationGenerationBridge = withScopedSharkerBridge({
  ...sharkerBridge,
  runtimeHostProfiles: generationStoryRuntimeHostProfilesBridge,
  openAiCodex: {
    ...sharkerBridge.openAiCodex,
...
const withModelsCopilotReimportGenerationBridge = withScopedSharkerBridge({
  ...sharkerBridge,
  runtimeHostProfiles: generationStoryRuntimeHostProfilesBridge,
  connections: {
    ...connectionsBridge,
  ...
  githubCopilotSubscription: {
    ...sharkerBridge.githubCopilotSubscription,
...
const withProviderCatalogIntentRevalidationBridge = withScopedSharkerBridge({
  ...sharkerBridge,
  runtimeHostProfiles: {
    ...sharkerBridge.runtimeHostProfiles,
    getSnapshot: () => pendingForever(),
...
const withGeneralHostSettingsErrorBridge = withScopedSharkerBridge({
  ...sharkerBridge,
  settings: {
    ...sharkerBridge.settings,
    get: async () => {
...
const withGeneralUnavailableBridge = withScopedSharkerBridge({
  ...sharkerBridge,
  runtimeHostProfiles: {
    ...sharkerBridge.runtimeHostProfiles,
...
const archivedTaskProjects: ProjectRecord[] = [
  { id: 'proj-sharker', name: 'sharker-agent', locations: [], available: true },
...
const withGitBashSettingsBridge = withScopedSharkerBridge({
  ...sharkerBridge,
  settings: {
    ...sharkerBridge.settings,
    get: async () => gitBashSettings,
    update: async (
      patch: Parameters<typeof window.sharker.settings.update>[0],
    ): Promise<UpdateAppSettingsResult> => ({
...
const withMemoryPopulatedBridge = withScopedSharkerBridge({
  ...sharkerBridge,
...
function withUsageStoryBridge(
  ...
  return withScopedSharkerBridge({
    ...sharkerBridge,
    settings: {
      ...sharkerBridge.settings,
      get: async () => settings,
...
export const ModelsOAuthHostGenerationRevalidation: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvasElement.querySelector('[data-sharker-contract="provider-catalog"]'),
    ).toBeInTheDocument();
  },
};
...
export const ModelsOAuthAuthorizationHostGenerationRevalidation: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvasElement.querySelector('[data-sharker-contract="provider-setup"]'),
    ).toBeInTheDocument();
...
export const ModelsCopilotReimportHostGenerationRevalidation: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvasElement.querySelector('[data-sharker-contract="connection-detail"]'),
    ).toBeInTheDocument();
  },
};
...
export const ModelsCatalogIntentDuringWarmRevalidation: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await waitForStoryCondition(
      () => canvasElement.querySelector('[data-sharker-contract="provider-catalog"]') !== null,
    ...
    await waitForStoryCondition(
      () => canvasElement.querySelector(
        '[data-sharker-contract="providers-panel"]:not([aria-busy="true"])',
      ) !== null,
    ...
    await expect(
      canvasElement.querySelector('button[data-sharker-contract="add-connection"]'),
    ).toBeInTheDocument();
  },
};
