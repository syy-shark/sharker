...
function catalogRoot(root: HTMLElement): HTMLElement | null {
  return root.querySelector<HTMLElement>('[data-sharker-contract="provider-catalog"]');
}
...
function reachCatalog(root: HTMLElement): HTMLElement | null {
  ...
  root.querySelector<HTMLButtonElement>('button[data-sharker-contract="add-connection"]')?.click();
  return null;
}
...
function clickAutoOpenTarget(root: HTMLElement, target: AutoOpenTarget): boolean {
  ...
  if (target === 'xai-device') {
    const setup = root.querySelector<HTMLElement>('[data-sharker-contract="provider-setup"]');
    if (!setup) {
...
// lists the models Sharker's bundled metadata cannot describe.
