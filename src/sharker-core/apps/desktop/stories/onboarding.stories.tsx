...
import type { LlmConnection, ProviderType } from '@sharker/core/llm-connections';
import type { OnboardingState } from '@sharker/core/onboarding';
import type { SettingsSection } from '@sharker/core/settings';
import { ChatSurfaceLayout, ChatView } from '@sharker/ui';
...
const SHARKER_WINDOW_FLOOR_VIEWPORT = {
  sharkerWindowFloor: {
    name: 'Sharker window floor',
...
function DetailPane(props: { children?: ReactNode }) {
  const emptyOverride = props.children === undefined
    ? undefined
    : <div className="sharker-onboarding-surface">{props.children}</div>;
  return (
    <div
    ...
    >
      <div
        className="sharker-panel sharker-panel-detail agents-parchment-paper-surface"
        ...
        <div className="sharker-detail-with-artifacts">
          <div className="mainColumn" data-home-surface="true">
            <ChatSurfaceLayout
              scrollOwner="host"
              composer={null}
              data-sharker-onboarding={props.children === undefined ? undefined : 'true'}
            >
...
export const NarrowWindow: Story = {
  parameters: { viewport: { options: SHARKER_WINDOW_FLOOR_VIEWPORT } },
  globals: { viewport: { value: 'sharkerWindowFloor', isRotated: false } },
  render: () => (
    <DetailPane>
      <OnboardingHero {...heroProps({ kind: 'needs_connection' })} />
    </DetailPane>
