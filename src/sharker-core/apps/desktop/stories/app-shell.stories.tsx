...
import type { ChatModelChoice, SessionViewMode, TurnViewModel } from '@sharker/ui';
...
const sidebarSessions: SessionSummary[] = [
  makeSession({ id: 'session-running', name: '生成本周 benchmark 对比表', status: 'running', lastMessageAt: NOW - 2 * 60_000, projectId: 'project-sharker', cwd: '/workspace/sharker-agent' }),
  makeSession({ id: 'session-active', name: '整理 Storybook 表面覆盖', lastMessageAt: NOW - 14 * 60_000, hasUnread: true, projectId: 'project-sharker', cwd: '/workspace/sharker-agent/.worktree/storybook' }),
  ...
  makeSession({ id: 'session-pinned', name: 'PR #435 发布风险清单', lastMessageAt: NOW - 76 * 60_000, isFlagged: true, projectId: 'project-sharker', cwd: '/workspace/sharker-agent' }),
...
const catalogProjects: ProjectRecord[] = [
  project({
    id: 'project-sharker',
    name: 'sharker-agent',
    preferredPath: '/workspace/sharker-agent',
    locations: [
      { path: '/workspace/sharker-agent', isWorktree: false },
      { path: '/workspace/sharker-agent/.worktree/storybook', isWorktree: true },
    ],
  }),
    ...
    groups: [
      {
        ...
        selectedProjectId: 'project-sharker',
        onAdd: noop,
...
function ShellFrame(props: {
  ...
  return (
    <div
      ...
      data-sharker-e2e-fixture={props.motionEnabled ? undefined : 'true'}
      ...
      style={
        {
          ...
          ...(props.sidebarCollapsed ? null : { '--sharker-sidenav-width': '260px' }),
        } as CSSProperties
      }
    >
      {props.children}
...
// production `.sharker-turn` frame. The session is `aborted` too, so the sidebar row
      ...
      chat={{
        messages: [
          ...
          },
          {
            ...
            content: {
              kind: 'terminal',
              cwd: '/workspace/sharker-agent/.worktree/storybook',
              cmd: 'npm test',
              status: 'cancelled',
              exitCode: 130,
...
// errored tool row renders inside `.sharker-turn`, and the turn wears its failed
...
export const FailedTurnWithToolError: Story = {
  ...
  play: async ({ canvasElement }) => {
    await waitFor(() => {
      const banner = canvasElement.querySelector('.sharker-turn-failed-banner');
...
export const ProviderRateLimited: Story = {
  ...
  play: async ({ canvasElement }) => {
    await waitFor(() =>
      expect(canvasElement.querySelector('.sharker-turn-failed-banner')?.textContent).toContain(
        '模型请求太频繁被限流了',
      ),
    );
  },
...
export const ProviderRetrying: Story = {
  ...
  play: async ({ canvasElement }) => {
    await waitFor(() =>
      expect(canvasElement.querySelector('.sharker-turn-provider-retry')).not.toBeNull(),
    );
  },
};
...
const multiStepConversation: StoredMessage[] = [
  ...
  },
  {
    ...
    content: {
      kind: 'terminal',
      cwd: '/workspace/sharker-agent/apps/desktop',
