...
import type { BrowserState } from '@sharker/core/browser';
import type { GitReviewReadResult, GitReviewSnapshot } from '@sharker/core/git-review';
import type { SessionSummary } from '@sharker/core/session';
import type { Task } from '@sharker/core/task-ledger';
...
import { ToastProvider } from '@sharker/ui';
...
const STACKED_WINDOW_VIEWPORT = {
  sharkerStackedWindow: {
    name: 'Sharker window below the 990px stack point',
...
const LOADED_BROWSER_STATE: BrowserState = {
  ...EMPTY_BROWSER_STATE,
  url: 'https://sharker.apache.org/docs/getting-started',
  title: 'Getting started — Apache Sharker',
  canGoBack: true,
  secure: true,
  hasPage: true,
};
...
const LONG_BROWSER_URL =
  'https://sharker.apache.org/docs/getting-started/configuration/advanced/runtime-host/agent-graph/scheduling/readiness-and-activation/edge-cases?highlight=very-long-query-string-that-keeps-going#a-deep-anchor-that-also-runs-long';
const LONG_BROWSER_TITLE =
  'Getting started · Configuration · Advanced · Runtime Host · Agent Graph scheduling, readiness, and activation edge cases — Apache Sharker documentation';
...
const artifactText: Record<string, string> = {
  'artifact-patch': [
    ...
    '-    <aside className="sharker-session-workbar">',
...
const gitReviewSnapshot: GitReviewSnapshot = {
  source: 'branch',
  repositoryRoot: '/Users/reviewer/sharker-agent',
...
function Workbar(props: {
  ...
  return (
    <ToastProvider>
      <div
        className="sharker-detail-with-artifacts"
        style={{
          ...
          height: '100dvh',
          ...(props.width ? { '--sharker-session-workbar-width': `${props.width}px` } : {}),
        } as CSSProperties}
      >
    ...
    const expectDiffBody = async (path: string, marker: string) => {
      ...
      await waitFor(() => {
        const row = Array.from(
          canvasElement.querySelectorAll<HTMLElement>('.sharker-session-review-file'),
        ).find((el) => el.textContent?.includes(path));
        expect(row?.querySelector('.sharker-session-review-diff')?.textContent ?? '').toContain(marker);
      });
    };
...
export const BrowserStacked: Story = {
  ...
  globals: { viewport: { value: 'sharkerStackedWindow', isRotated: false } },
  decorators: [bridge({ browserState: LOADED_BROWSER_STATE })],
  render: () => <Workbar tab="browser" />,
};
...
export const BrowserInvalidAddress: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.type(address, 'mailto:team@sharker.apache.org{Enter}');
...
// supplied by WorkbarServices, so the real Composer mounts without window.sharker.
...
export const TraceMinimumWidth: Story = {
  ...
  play: async ({ canvasElement }) => {
    await waitFor(() => {
      ...
      const label = turn?.querySelector<HTMLElement>('.sharker-inspector-turn-label');
      const meta = turn?.querySelector<HTMLElement>('.sharker-inspector-turn-meta');
