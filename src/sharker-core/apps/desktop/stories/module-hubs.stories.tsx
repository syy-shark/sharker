...
import type { DailyReviewArchive, DailyReviewSummary } from '@sharker/core/daily-review';
import type { ScheduledTask, ScheduledTaskRun } from '@sharker/core/scheduled-task';
import type { McpConfigFile, McpServerStatus } from '@sharker/core/mcp';
import { MCP_CONFIG_VERSION } from '@sharker/core/mcp';
import {
  ScheduledTasksPage,
  DailyReviewPage,
  getSharedUiCopy,
...
const INSTALLED_SKILLS: SkillEntry[] = [
  {
    ref: 'workspace:sharker:skill-git-flow',
    id: 'skill-git-flow',
    name: 'git-flow',
    description: '封装分支创建、合并与发布打 tag 的常用 git 操作。',
    path: '~/.sharker/skills/git-flow',
  ...
  },
  {
    ...
    path: '~/.sharker/skills/docs-screenshot',
  ...
  {
    ref: 'project:sharker:skill-release-notes',
    id: 'skill-release-notes',
    name: 'release-notes',
    description: '从最近的 commit 历史生成发布说明草稿。',
    path: '~/.sharker/skills/release-notes',
    declaredTools: ['Bash'],
    sourceType: 'bundled',
    scope: 'project',
    contextStatus: 'advertised',
...
const UPDATE_AVAILABLE_SKILLS: SkillEntry[] = [
  {
    ref: 'workspace:sharker:release-checklist',
    id: 'release-checklist',
    name: 'release-checklist',
    description: '发布前检查版本、测试证据和变更说明。',
    path: '~/.sharker/skills/release-checklist',
    declaredTools: ['Bash', 'Read'],
    sourceType: 'managed',
...
const UPDATE_AVAILABLE_PREVIEW: ManagedSkillUpdatePreview = {
  skill: {
    ...
    name: 'release-checklist',
    description: '发布前检查版本、测试证据和变更说明。',
    path: '~/.sharker/skills/release-checklist/SKILL.md',
    declaredTools: ['Bash', 'Read'],
    sourceType: 'managed',
    userModified: false,
    validationStatus: 'ok',
...
const DISABLED_SKILLS: SkillEntry[] = [
  {
    ref: 'workspace:sharker:spreadsheet-audit',
    id: 'spreadsheet-audit',
    name: 'spreadsheet-audit',
    description: '检查工作簿中的公式、格式和异常值。',
    path: '~/.sharker/skills/spreadsheet-audit',
    declaredTools: ['Read'],
    sourceType: 'bundled',
    scope: 'workspace',
    contextStatus: 'disabled',
...
const LONG_LIST_SKILLS: SkillEntry[] = Array.from({ length: 40 }, (_, index) => ({
  ref: `workspace:sharker:skill-long-${index}`,
  id: `skill-long-${index}`,
  name: `long-list-skill-${index}`,
  description: '长列表占位技能，用于滚动契约。',
  path: `~/.sharker/skills/skill-long-${index}`,
  declaredTools: ['Bash'],
  sourceType: 'workspace',
  scope: 'workspace',
  contextStatus: 'advertised',
...
const withConfiguredMcpBridge = withScopedSharkerBridge({
...
const withEditorMcpBridge = withScopedSharkerBridge({
  runtimeHostProfiles: storyRuntimeHostProfilesBridge,
  mcp: {
...
const withEmptyMcpBridge = withScopedSharkerBridge({
  runtimeHostProfiles: storyRuntimeHostProfilesBridge,
  mcp: {
...
const withFailedMcpBridge = withScopedSharkerBridge({
  runtimeHostProfiles: storyRuntimeHostProfilesBridge,
  mcp: {
...
export const ScheduledDailyReviewRefreshing: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    button.click();
    await waitForStorySelector<HTMLElement>(canvasElement, '.sharker-daily-review-content');
  },
};
