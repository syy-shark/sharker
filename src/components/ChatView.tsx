 ...
 * pin 列 / 历史源 / pin 后缺口 / pinned 槽 / 无 pin / 活动 / handoff 槽 `useMemo`；persist 入列复用同一 gap 与 pin id，`nextPinnedLiveSlots` 同一 map 收 active→frozen，冻结位不跟 loading，活动位只让当前预留 id 接审批与 loading，handoff 未 adopt 前 hold 也不跟、跟进冻结只换 props；挤出冻结行只重画该历史 id，pin 后缺口同样只换该 id 以免重挂冻结 Fragment；直播槽不订 `historyHasReserved`，贴底 setState / persist 入列不重建历史行与直播槽（对标 Codex #22860 / #38220 / #37849）。
...
import {
  ...
  type PinnedLiveSlotIdentity,
  type HistoricalRowIdentity,
  type PinnedAfterRowHold,
...
const EMPTY_PINNED_LIVE_SLOTS: ReadonlyMap<string, ReactNode> = new Map()
const EMPTY_PINNED_LIVE_IDENTITIES: ReadonlyMap<string, PinnedLiveSlotIdentity> = new Map()
const EMPTY_PINNED_LIVE_ROW_HOLD: PinnedLiveRowHold<ReactNode> = {
  ids: [],
  rows: [],
  after: [],
  slots: EMPTY_PINNED_LIVE_SLOTS
}
...
const EMPTY_PINNED_AFTER_ROW_HOLD: PinnedAfterRowHold<ReactNode> = {
  ids: [],
  rows: [],
  gaps: [],
  identities: new Map()
...
interface Props {
  ...
  /** 掉出 ejected 环后仍按冻结 part 画；当前对话不截断，切对话清掉 */
  archivedLiveArticles?: readonly RetiredLiveArticle[]
...
export const ChatView = memo(function ChatView({
  ...
  useEffect(() => {
    ...
    pinnedLiveSlotsHeldRef.current = {
      slots: EMPTY_PINNED_LIVE_SLOTS,
      identities: EMPTY_PINNED_LIVE_IDENTITIES
    }
    ...
    pinnedAfterRowsSessionRef.current = sessionKey
  ...
  const pinnedLiveSlotsHeldRef = useRef<{
    slots: ReadonlyMap<string, ReactNode>
    identities: ReadonlyMap<string, PinnedLiveSlotIdentity>
  }>({ slots: EMPTY_PINNED_LIVE_SLOTS, identities: EMPTY_PINNED_LIVE_IDENTITIES })
  ...
  const pinnedAfterRowsSessionRef = useRef(sessionKey)
  ...
  useEffect(() => {
    ...
    const follow = () => {
      ...
      if (!stickToBottomRef.current) {
        if (shouldRecordLiveStickHeightWhenUnstuck()) {
          lastHeight = h
          lastClient = client
        }
        return
    ...
    const flush = () => {
      ...
      if (scroller) {
        for (const row of snapshots) {
          ...
          const previousSize = resolvePreviousRowIntrinsicSize({
            stored: prev.get(row.id),
            seeded: measuredRowHeightsRef.current.get(row.id)
          })
  ...
  const retiredArticles: readonly RetiredLiveArticle[] = useMemo(() => {
    ...
    return [
      {
        id: retiredLiveId,
        parts: retiredLiveParts ?? [],
        meta: retiredLiveMeta,
        startedAt: retiredLiveStartedAt,
        copyable: retiredLiveCopyable
      }
    ]
  }, [
    retiredLiveArticles,
  ...
  const pinnedGaps = useMemo(() => {
    ...
    pinnedGapsSessionRef.current = sessionKey
    const next = nextPinnedTranscriptGaps(
      windowedMessages,
      pinnedLiveIds,
      [...pinnedLiveIds, liveAssistantId, retiredLiveId],
      prev
    )
    pinnedGapsHeldRef.current = next
    return next
  }, [sessionKey, windowedMessages, pinnedLiveIds, liveAssistantId, retiredLiveId])
  const historicalSource = useMemo(() => {
    if (pinnedGaps) return pinnedGaps[0] ?? EMPTY_HISTORICAL_MESSAGES
    return historicalMessagesDuringLive(
      windowedMessages,
      hideReservedLive ? liveAssistantId : null,
      hideReservedLive
    )
  }, [pinnedGaps, windowedMessages, hideReservedLive, liveAssistantId])
  ...
  const pinnedAfterRows = useMemo(() => {
    if (pinnedAfterRowsSessionRef.current !== sessionKey) {
      pinnedAfterRowsSessionRef.current = sessionKey
      pinnedAfterRowsHeldRef.current = EMPTY_PINNED_AFTER_ROW_HOLD
    }
    ...
    if (afterGaps.length === 0) {
      pinnedAfterRowsHeldRef.current = EMPTY_PINNED_AFTER_ROW_HOLD
      return EMPTY_PINNED_AFTER_ROWS
    }
    const afterMessages = afterGaps.flat()
    const identities = new Map<string, HistoricalRowIdentity>()
    afterMessages.forEach((m, index) => {
      identities.set(m.id, {
        message: m,
        article: frozenHistoricalArticle(ejectedLiveArticles, archivedLiveArticles, m.id),
        findHit: historicalFindIds.has(m.id),
        ...
        selectionSource: selectionSourceId === m.id,
        preserveLiveDiffs: m.id === preserveLiveDiffsId,
        isLast: index === afterMessages.length - 1
      })
    })
    const next = nextPinnedAfterRowNodes(
      pinnedAfterRowsHeldRef.current,
      afterGaps,
      identities,
      (id, index) => {
        const m = afterMessages[index]
        return m.role === 'user' ? (
          <UserMessageRow
            key={m.id}
            id={m.id}
    ...
    pinnedAfterRowsHeldRef.current = next
    return next.gaps.length === 0 ? EMPTY_PINNED_AFTER_ROWS : (next.gaps as ReactNode[][])
  }, [
    archivedLiveArticles,
    currentFindMessageId,
    editUserMessageId,
  ...
  const pinnedLiveSlots = useMemo(() => {
    if (pinnedLiveSlotsSessionRef.current !== sessionKey) {
      pinnedLiveSlotsSessionRef.current = sessionKey
      pinnedLiveSlotsHeldRef.current = {
        slots: EMPTY_PINNED_LIVE_SLOTS,
        identities: EMPTY_PINNED_LIVE_IDENTITIES
    ...
    const identities = new Map<string, PinnedLiveSlotIdentity>()
    ...
    const next = nextPinnedLiveSlots(
      pinnedLiveSlotsHeldRef.current.slots,
      pinnedLiveSlotsHeldRef.current.identities,
      identities,
      (id) => {
        const identity = identities.get(id)
        const article = identity?.frozen
          ? retiredLiveArticle(retiredArticles, id)
          : null
        const frozen = Boolean(article)
        return (
          <LiveAssistantSlot
            ...
            frozen={frozen}
            frozenParts={frozen ? (article?.parts ?? null) : null}
            frozenMeta={frozen ? (article?.meta ?? null) : null}
            frozenStartedAt={frozen ? (article?.startedAt ?? null) : null}
            frozenCopyable={frozen ? (article?.copyable ?? undefined) : undefined}
            frozenProcess={frozen ? (article?.process ?? null) : null}
    ...
    pinnedLiveSlotsHeldRef.current = next
    return next.slots
  }, [
    approval,
    approvalResponding,
  ...
  const pinnedLiveRows = useMemo(() => {
    if (pinnedLiveRowsSessionRef.current !== sessionKey) {
      pinnedLiveRowsSessionRef.current = sessionKey
      pinnedLiveRowsHeldRef.current = EMPTY_PINNED_LIVE_ROW_HOLD
    }
    if (pinnedLiveIds.length === 0) {
      pinnedLiveRowsHeldRef.current = EMPTY_PINNED_LIVE_ROW_HOLD
      return EMPTY_PINNED_LIVE_ROWS
    }
    const next = nextPinnedLiveRowNodes(
      pinnedLiveRowsHeldRef.current,
      { ids: pinnedLiveIds, after: pinnedAfterRows, slots: pinnedLiveSlots },
      (id, index) => (
        <Fragment key={id}>
          {pinnedLiveSlots.get(id)}
          {pinnedAfterRows[index]}
        </Fragment>
      )
    )
    pinnedLiveRowsHeldRef.current = next
    return next.rows.length === 0 ? EMPTY_PINNED_LIVE_ROWS : next.rows
  }, [pinnedAfterRows, pinnedLiveIds, pinnedLiveSlots, sessionKey])
  const unpinnedLiveRow = useMemo(() => {
    if (
