...
| `ComposerDock.tsx` | 输入区独立树：`/` `@` `$`、历史（标题/正文/分支扩匹配）、项目选择器、附件、听写/语音、Enter 注入 / Tab 排队（⌃Tab / ⌘Tab 不补全也不排队）、本地/隔离线程；`/` 列表并入已安装 Skill（选中写入 `$name`，不盖内置命令）；粘贴优先 text/plain（避开 Office 图片层），超长收成 `Pasted text.txt` 可预览/回插；深链 `prompt=` 经 `composerSeed.nonce` 写入；不接收 `streaming` / `liveSegments`；直播中 `speechHint` 置空以免跟 token 重绘；有目标时在输入框上方挂 `GoalProgressRow` |
