...
| `ChatView.tsx` / `.css` | 聊天主视图：消息列表、输入、排队、滚动；「回到底部」浮在输入框正上方（不进滚动层），仅内容溢出且离开底部时出现；提交时拦截 UI 斜杠命令；`/history` 历史选择弹层（浅色玻璃/深色金属，↑↓/Enter/Esc 与 hover 同步，关闭有退出动画并真正卸载、回焦输入框） |
| `ChatToolbar.tsx` / `.css` | 聊天顶栏：侧栏展开/收起、新对话、右侧面板 |
| `AssistantMessage.tsx` / `.css` | 助手消息：直播过程在上；错误/中止卡有进入动画；完成后过程可展开且无 thinking 原文 |
| `TurnFlow.tsx` / `.css` | 直播过程：闲聊只留一行状态字+耗时（文字扫光，无呼吸灯）；有工具才展开时间线；无阶段轨/空壳步骤 |
...
| `InlineDemo.tsx` / `.css` | 对话内联演示 iframe：高度跟内容、外壳透明并对齐宿主玻璃/金属 token |
