...
| `AppearanceSettings.tsx` / `.css` | 外观：浅色玻璃 / 深色金属；界面字号（`uiFontScale` / `--ui-font-scale`）；代码字号（`codeFontScale` / `--code-font-scale`，档位标题仍本机）；代码字体行用官方 Code font（`codeFont` / `--mono`；说明 leftover Codex uses the same font for the review pane…）；Reduce Motion（关掉直播思考扫光，进度圈仍转，对标 Codex #16857 / #22787）；Keep a chat near your work / Always on top（对标 Codex Settings；说明用官方 pop out an active chat… Always on top 原文） |
