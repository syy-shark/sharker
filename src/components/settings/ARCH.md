...
| `AppearanceSettings.tsx` / `.css` | 外观：浅色 Sharker 灰底白浮板 / 深色表面阶梯；`applyAppearanceDom` 同时写 `html.dark` 给 Astryx；界面字号（`uiFontScale` / `--ui-font-scale`）；代码字号（`codeFontScale` / `--code-font-scale`，档位标题仍本机）；代码字体行用官方 Code font（`codeFont` / `--mono`；说明 leftover Codex uses the same font for the review pane…）；Reduce Motion（关掉直播思考扫光，进度圈仍转，对标 Codex #16857 / #22787）；Keep a chat near your work / Always on top（对标 Codex Settings；说明用官方 pop out an active chat… Always on top 原文） |
...
| `BrowserSettings.tsx` / `.css` | 设置 → 浏览器：历史搜索 / 重新打开 / 删除，按时间清除历史，Cookie / 缓存；页说明与分区用官方 The built-in browser uses a browser profile… / Open Settings > Browser to search… / Use Clear browsing data… / Browser downloads go to your system Downloads folder…；下载开关用官方 Ask where to save downloads，清除分区/按钮用官方 Clear browsing data（对标 Codex Settings → Browser / learn.chatgpt.com/docs/browser）。不发明时间范围英文档名、@Browser / 导入系统配置 |
...
| `ShortcutSettings.tsx` / `.css` | Keyboard Shortcuts：页说明用官方 Open Keyboard Shortcuts to review… / Use the search field… / Appshots use a separate global shortcut under Settings > Appshots.；Search by command name / Keystroke search；未绑行 Not assigned by default；录制时 Press a key combination；一条命令多行绑定（New chat 的 ⌘N 与 ⌘⇧O 各一行）；改绑按钮 aria Change shortcut for {title}，按住 Shift 时 Create new shortcut for {title} 并追加（对标官方桌面打包文案 #27835 隐藏手势，不发明可见 Add another shortcut）；解除单行、重置整条（`keyboardShortcuts`；含 Stop / Open Subagents / `/copy` Copy the last response…；动作标题用官方 Commands / slash 文案；Search chats / Cycle reasoning 默认 Not assigned by default） |
| `AppshotSettings.tsx` | Appshots：分区说明用官方 Appshots let you send the frontmost app window… / Press both Command keys… / An appshot captures the frontmost window only… / By default, ChatGPT starts a new chat… last 60 seconds… / If you resume a chat in the CLI that already contains an appshot… / If appshots don't work… Privacy & Security / Screen & System Audio Recording / Accessibility for Codex Computer Use；热键行 **Take an Appshot**，默认 ⌘+⌘，改绑用 Change shortcut for / Press a key combination（对标 learn.chatgpt.com/docs/appshots / Commands；不进 Keyboard Shortcuts 目录，不发明开关 / plugin） |
...
| `WorktreeSettings.tsx` | 设置 → Worktrees：分区说明用官方 By default, Codex keeps your most recent 15 Codex-managed worktrees… / change Worktree root；另挂官方 leftover If your repository ignores local setup files… add a `.worktreeinclude` file… / Use this for files Git intentionally ignores… / Codex automatically copies an ignored `AGENTS.override.md`… / Setup scripts run automatically when Codex creates a new worktree… / App doesn't pick up a teammate's shared local environment… `.codex` folder… / Worktrees are created in a different directory… local environment or `.worktreeinclude`；Worktree root 与托管保留数（默认 15，0 关闭自动删除）。不发明环境编辑器、`.worktreeinclude` 编辑器或单独未落盘的自动删除开关 |
| `ArchivedSettings.tsx` / `.css` | Archived chats：说明 leftover The Archived chats section lists archived chats… / Archived chats can be found in Settings. When you unarchive a chat, it reappears in its original sidebar location.；Unarchive 或彻底删除（含项目菜单一并归档的对话） |
