...
| `AppshotSettings.tsx` | Appshots：分区说明用官方 Appshots let you send the frontmost app window… / Press both Command keys… / An appshot captures the frontmost window only… / By default, ChatGPT starts a new chat… last 60 seconds…；热键行 **Take an Appshot**，默认 ⌘+⌘，改绑用 Change shortcut for / Press a key combination（对标 learn.chatgpt.com/docs/appshots / Commands；不进 Keyboard Shortcuts 目录，不发明开关 / plugin） |
