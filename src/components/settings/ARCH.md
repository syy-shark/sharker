...
| `WorktreeSettings.tsx` | 设置 → Worktrees：分区说明用官方 By default, Codex keeps your most recent 15 Codex-managed worktrees… / change Worktree root；另挂官方 leftover If your repository ignores local setup files… add a `.worktreeinclude` file… / Use this for files Git intentionally ignores… / Codex automatically copies an ignored `AGENTS.override.md`… / Setup scripts run automatically when Codex creates a new worktree… / Worktrees are created in a different directory… local environment or `.worktreeinclude`；Worktree root 与托管保留数（默认 15，0 关闭自动删除）。不发明环境编辑器、`.worktreeinclude` 编辑器或单独未落盘的自动删除开关 |
