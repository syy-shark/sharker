 ...
 * - 同一工具只改详情、写盘 +/- 或收束时只换该步，写盘收束同时新开工具时 remap 并追加；前缀没变或只收束散文或无新写盘的工具时新开一或多个工具只追加这些步并封回答尾、新思考只换旁白（无新写盘的工具收束后同一帧也走这条）、新散文只开回答尾、新 status 只追加一步、审批挂上或收束只换工具步与 Awaiting 行、Ask User 挂上或单条 status 收口只换工具步与 Question requested 行、Stop 把多条 active 收成 cancelled 只换这些步、错误收口后只开错误回答尾、新演示或正文 ```demo 只开回答槽，不重扫整条时间线（对标 Codex #22860）
