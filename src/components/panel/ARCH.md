...
| 文件 | 说明 |
|------|------|
| `FileTree.tsx` / `.css` | 工作区文件树；附加文件夹与主根并列顶层（对标 Codex Edit project）；打开预览 view-enter 并可跳到引用行（附加根按目录名前缀解析）；图 / PDF 走 data URL，办公二进制不灌进文本；长行换行以免撑开面板；文本预览划选可插入输入框或旁路提问（对标 Codex Project Preview selection actions）；子目录展开 list-item-in |
