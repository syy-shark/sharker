...
| 文件 | 说明 |
|------|------|
| `SettingsPage.tsx` / `.css` | 设置页壳：权限（含 Git、命令输出展示量、worktree） / 模型 / MCP 服务器（STDIO / Streamable HTTP） / 通用（含 `file_opener` 与上下文用量环） / 外观 / 通知 / 个性化（启用记忆默认关） / 建议提示 / 键盘快捷键 / 已归档 / 用量（本机 Profile 洞察） |
