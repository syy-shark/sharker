...
| 文件 | 说明 |
|------|------|
| `index.ts` | 主进程入口：窗口、菜单、IPC、`chat:send` → `executeUserInput`（可选 worktreePath / goal）、`/init` 写 AGENTS.md、记忆列表、worktree 探活、记忆初始化、子 Agent 落盘恢复等；窗口锁定 pinch 缩放，字号走 `--ui-font-scale` |
