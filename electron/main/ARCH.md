...
| `app-menu.ts` | macOS 应用菜单（File / Edit / View / Window / Help）；文件菜单 New chat / Open folder（对标 Codex Commands）以及分享只读快照与 Copy as Markdown（对标 Codex Share / Copy as Markdown）；View 用官方 Toggle sidebar / Toggle bottom panel / Toggle terminal / Toggle review panel / Open review tab / Toggle File Tree（对标 #20552）；Help 用 Open command menu / Open keyboard shortcuts；撤销/重做走渲染进程（输入框原生、其它处应用动作）；自定义项 `registerAccelerator: false` |
