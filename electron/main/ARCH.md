...
| `app-menu.ts` | macOS 应用菜单顶层用官方 File / Edit / View / Window / Help（对标 Codex #14450）；应用菜单 Open settings、File 菜单 New chat / New standalone chat / Open folder / Share / Close（对标 Codex Commands / Share / #26890，不发明 Quick chat）以及 Copy as Markdown（对标 Codex Copy as Markdown）；View 用官方 Toggle sidebar / Toggle bottom panel / Toggle terminal / Toggle review panel / Open review tab / Toggle File Tree / Open Browser Tab / Toggle Full Screen（对标 #20552 / #30659 / #26890）；Help 用 Open command menu / Open keyboard shortcuts；撤销/重做走渲染进程（输入框原生、其它处应用动作）；自定义项 `registerAccelerator: false` |
