...
| `app-menu.ts` | macOS 应用菜单（File / Edit / View / Window / Help）；文件菜单含分享只读快照与 Copy as Markdown（对标 Codex Share / Copy as Markdown）；⌘J 开关工作区面板、Ctrl+` 开关终端、⌘⇧E 开关文件树、⌘⌥B 开关审查面板、⌃⇧G 打开审查（对标 Codex Toggle file tree / Toggle review panel / Open review tab）；撤销/重做走渲染进程（输入框原生、其它处应用动作）；自定义项 `registerAccelerator: false` |
