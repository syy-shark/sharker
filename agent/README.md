...
| 文件 | 说明 |
|------|------|
| `pipeline.ts` | `executeUserInput`、`processUserInput`、`onQuery`、`abortActiveTurn` |
| `query-loop.ts` | `queryLoop` — 流式问模型 ↔ 工具 ↔ 审批 ↔ verify |
...
| `tool-definitions.ts` | 发给模型的 OpenAI tools JSON |
...
executeUserInput
  ...
  → onQuery：校验提供商、压缩上下文、组装 system + skills
  → queryLoop：流式模型 ↔ 工具（最多 12 轮）→ done
```
...
- 新工具：同时改 `tool-definitions.ts` + `tools/executor.ts`
