...
describe('at mention', () => {
  ...
  it('uses official Type @ to search for a file and surfaces files first', () => {
    ...
    expect(composerSrc).toContain('DEFAULT_CONVERSATION_TITLE')
    expect(composerSrc).not.toContain('引用文件、对话或 Skill')
    expect(composerSrc).not.toContain("name: c.title || '对话'")
    expect(composerSrc).not.toContain("detail: '对话'")
    expect(composerSrc).not.toContain("'未命名项目'")
