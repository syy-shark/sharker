...
| `live-stream-ui.test.ts` | 相同片段引用与正文不换对象；增长才换；过程切片与闭合回答在 token 增长时复用；DEV seed 与开轮「准备中」补丁带上片段 / 秒表 / 回答正文；收束正文补丁带 streaming / 无 activeTool / copyable；commit 不先抹秒表；`/compact` / 开轮自动压缩 / 重连 n/5 状态进过程区；`App.tsx` 不再 `setLiveSegments` / `setStreaming`（只写 store + ref）；直播写盘推迟审查「本轮」列表 |
