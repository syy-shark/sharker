...
| `file-citation.ts` | Codex 式文件引用：`path:line` / `#L` / `(line N)`、相对路径接到工作区或附加根（目录名前缀）；百分号路径先解一层，解码后带空格的目录路径仍可点（对标 Codex #13123 / #17548）；右键打开 / 揭示 / 复制路径，不接自定义 Open with；拒绝 `www.`、`</tag>` 与尾斜杠 / `a\\` 假路径，以免直播把自动链接 / HTML / 反斜杠硬换行收束成文件芯片 |
