...
| `streaming-markdown.ts` | 流式 Markdown 拆成稳定块 + 尾部；CRLF 归一；`continueStreamingMarkdown` 复用已闭合块；散文尾廉价块（ATX/Setext 标题/列表含缩进嵌套、续行与松散项/引用/表格/分隔线/缩进代码/脚注区）与行内（闭合链接、引用式链接、`<https>` / 邮箱 / `www.`、http 图、下划线强调、删除线、脚注、硬换行、文件引用）；`[` 对不上 `](` 时不吞后续标记；链接定义行不画；`continueCheapProseBlocks` 复用已闭合列表项/嵌套项/表格行 |
