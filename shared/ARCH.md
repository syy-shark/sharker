...
| `live-stream-slices.ts` | 直播过程/回答切片：`liveProcessIdentity` / `shouldSkipLiveProcessIdentity` / `shouldGrowLiveAnswerTail` 在正文或思考只加长时跳过过程指纹与 `buildAnswerParts`；思考增长只换 `thinkText`、时间线引用不动；已确认无 ```demo 时 `hasStreamingDemoFenceGrowth` 只扫末行与后缀；增长比较就地比 all-but-last、不 `slice` 前缀；单段正文增长不 join 闭合块；片段引用没变则复用回答视图；操作条只订布尔（对标 Codex #22860） |
