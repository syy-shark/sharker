...
| `live-stream-slices.ts` | 直播过程/回答切片：`liveProcessIdentity` / `shouldSkipLiveProcessIdentity` / `shouldGrowLiveAnswerTail` 在正文或思考只加长时跳过过程指纹与 `buildAnswerParts`；`nextLiveProcessTimeline` 不含 `thinkText`，思考 token 不抬 TurnFlow；思考增长只换旁白、时间线引用不动；已确认无 ```demo 时 `hasStreamingDemoFenceGrowth` 只扫末行与后缀；增长比较就地比 all-but-last、不 `slice` 前缀；单段正文增长不 join 闭合块；片段引用没变则复用回答视图；操作条只订布尔（对标 Codex #22860） |
| `live-stream-ui.test.ts` | 相同片段引用与正文不换对象；增长才换；过程切片在正文只加长时跳过过程指纹与 buildAnswerParts、闭合回答在 token 增长时复用；DEV seed 与开轮「准备中」补丁带上片段 / 秒表 / 回答正文；收束正文补丁带 streaming / 无 activeTool / copyable；commit 不先抹秒表；`/compact` / 开轮自动压缩 / 重连 n/5 状态进过程区；`App.tsx` 不再 `setLiveSegments` / `setStreaming`（只写 store + ref）；直播写盘推迟审查「本轮」列表；打开预览不跟每条 `tool_done` 抬 App，改由文件树 `revision` 重读 |
...
| `workbench-shortcuts.ts` | 默认工作台快捷键与 `SHORTCUT_CATALOG`（设置页改绑；标题用官方 Open command menu / Toggle sidebar / Toggle bottom panel / Toggle terminal / Toggle file tree / Run environment action 1 / Toggle Activity view / Open model picker / Open project picker / Start dictation / Approve request / Decline request / New window（⌘⇧N，对标 #12773）等，对标 developers.openai.com/codex/app/commands 与 #20552）；Search chats 默认不绑、⌘G 留给 Find next；Esc 停止当前回合可改绑；帮助表含审批打开时 Enter Approve request / Esc Decline request、输入框 Shift+Tab 切计划、⌘↑⌘↓ / Home / End 跳对话顶底、点对话柱后滚动；浏览器聚焦刷新 / 批注；终端聚焦 ⌘K 清屏判定 |
