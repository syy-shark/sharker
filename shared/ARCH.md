...
| 文件 | 说明 |
|------|------|
| `types.ts` | 跨进程核心类型与默认设置（含 `WorkspaceItem.extraPaths` 附加文件夹、`worktreeKeepCount` / `worktreeRoot`、`uiFontScale` / `reduceMotion`（对标 Codex Reduce Motion）/ `codeFont` / `codeFontScale`、`keyboardShortcuts`、`followUpBehavior` / `composerEnterBehavior`（旧 `requireModEnter`）/ `suggestedPrompts`、`reviewDelivery` / `reviewProviderId`（对标 Codex `review_model`）/ Git 文案与 force-with-lease / 分支前缀 / `toolOutputDisplay` / `fileOpener`（对标 Codex `file_opener`）/ `showContextWindowUsage`（对标 Codex Show context window usage，官方默认关）、`browserDownloadPath` / `browserAskWhereToSave`（对标 Codex Settings → Browser downloads）、`turnNotifyMode` / `approvalNotify` / `preventSleepWhileRunning` / `popoutAlwaysOnTop`、`memoriesEnabled`（官方默认关）与注入/写入开关、`resultOutputDeferred` / `contentDeferred` / `thinkingPreviewDeferred` 启动窗占位、`UserInputRequest` / `user_input_needed`（对标 Codex `request_user_input`）） |
...
| `conversation.ts` | 对话模型、标题推导（空线程回退官方 `New chat`，旧盘 `新对话` 仍当占位再推导）、侧栏排序（置顶优先）、Search chats 扩匹配（标题 / 正文摘要 / git 分支；官方默认不绑 ⌘G）、对话路径、进行中任务拆分、⌘⌥A 先等审批再进行中、`conversationIdAtIndex` / `recentConversationIdAtIndex` 给 ⌘1–9 Go to chat / ⌘⌥1–6 Open recent chat（对标 Codex 桌面）、侧栏 Chronological / 进行中 / 等待回复 / 未读 / 定时 / 置顶筛选、⌘⌥U 开关 Activity、项目菜单 Archive chats 只收该项目未归档 id（可跳过进行中）、`/fork` 分叉标题与拷贝、`messagesThroughInclusive` / `canForkThroughMessage`（对标 Codex `thread/fork` `lastTurnId`，直播未完成行拒分叉）、`/fork [local|worktree]` 目标、`/rename` `/pin` 未读；`historyStartSeq` / `historyTotal` 给长线程尾页 |
| `conversation.test.ts` | 按标题 / 自定义标题 / id / 正文 / 分支过滤、进行中拆分、分叉标题与 `/fork` 目标、按消息截断拷贝、空线程 `New chat` / 旧盘「新对话」占位再推导、置顶排序、`/rename`、⌘1–9 / ⌘⌥1–6 按序取对话 |
...
| `workbench-shortcuts.ts` | 默认工作台快捷键与 `SHORTCUT_CATALOG`（设置页改绑；标题用官方 Open command menu / Toggle sidebar / Toggle bottom panel / Toggle terminal / Toggle file tree / Run environment action 1 / Toggle Activity view / Open model picker / Open project picker / Start dictation / Approve request / Decline request / New window（⌘⇧N，对标 #12773）/ Close current tab or window（⌘W，右侧面板开着先关面板）等，对标 developers.openai.com/codex/app/commands 与 #20552）；Search chats 默认不绑、⌘G 留给 Find next；Esc 停止当前回合可改绑；帮助表含审批打开时 Enter Approve request / Esc Decline request、输入框 Shift+Tab 切计划、⌘↑⌘↓ / Home / End 跳对话顶底、点对话柱后滚动；浏览器聚焦刷新 / 批注；终端聚焦 ⌘K 清屏判定 |
...
| `files-changed-card.test.ts` | 标题 vs 展开、平台揭示文案、路径去重、basename 碰撞标签、种类标签、HTML 文档种类、片段 +/- 合计与复用、正文加长或无 +/- 新工具跳过指纹、有新写盘不跳 |
...
| `reveal-in-folder.ts` | 官方 Open in Finder / Open in Explorer / Open in File Manager / Copy path：平台文案、线程项目目录、审查文件绝对路径、侧栏线程菜单项（Open in Finder / Copy as Markdown / Rename / Pin / Unpin / Archive，对标 Codex #13123 / #25646 / #33217 / #29449）、顶栏 Copy 子菜单（Copy working directory / Copy session ID / Copy deeplink / Copy as Markdown，对标 Codex #25201 / #28233）。另有官方 New chat / Find in chat / Search chats / Skills / Open command menu / Toggle sidebar / Toggle file tree / Open model picker / Start dictation / Approve request / Decline request 等快捷键文案，以及应用菜单顶层 File / Edit / View / Window / Help 与 About / Hide / Hide Others / Show All / Quit（对标 #28543 About Codex）与 Window Minimize / Zoom / Bring All to Front（窗口 Zoom，不是 View Zoom In）与 File Close / Close current tab or window / New window / Edit Undo Redo Cut Copy Paste Select All / View Open Terminal / Open Browser Tab / Focus Browser Address Bar / Reload Browser Page / Find / Previous Chat / Next Chat / Back / Forward / Toggle Full Screen / Help Codex Documentation / Send Feedback（对标 Codex #14450 / #26890 / #30659 / #37104 / #12773 / #28543，不发明 Check for Updates / Log Out）与 Settings 页 General / Appearance / Notifications / Personalization / Suggested prompts / Browser / MCP servers / Open MCP status / Profile / Unarchive / Show context window usage / Prevent sleep while running / Share / Keep a chat near your work / Always on top / Open in Popup Window / Edit project / Archive chats / Add folder / Make primary / Fork / Pause / Resume / Edit / Clear / Hand off / Local / Worktree / Create branch here（对标 developers.openai.com/codex/app/settings / learn.chatgpt.com/docs/projects / learn.chatgpt.com/docs/long-running-work / learn.chatgpt.com/docs/environments/git-worktrees / #40832 / #15162）。不接自定义 Open with |
...
| `thread-goal.ts` | `/goal` 解析（含官方 `edit`）、暂停/清除、4000 字上限、system 注入块、进度行 Active / Paused 与 `startedAt`；`shouldStartGoalTurn` 只对设定文本开首轮 |
...
| `composer-submit.ts` | Composer Enter/Tab：空闲发送；忙时按 `followUpBehavior` 默认 Queue（对标 Codex 桌面 Follow-up behavior）；⌘⇧Enter 反转单条；`composerEnterBehavior`（`enter` / `cmdIfMultiline` / `cmdAlways`，旧 `requireModEnter`）决定是否要修饰键；Tab 仍 Queue；Shift+Tab 不排队（`isPlanModeToggleKey`，对标 Codex Best practices `/plan` 或 Shift+Tab）；审批打开时 Enter 允许一次 / Esc 拒绝；空输入 ↑ 恢复刚提交或上一条（取消运行 / 取消 worktree 创建后即使还没进对话也能恢复）；Ctrl+R 提示历史；空输入 Esc+Esc 就地回编上一条并分叉；`shouldStickAfterComposerSubmit` 只有 `'send'` 贴底（对标 Codex #13698 / #38220，Queue / Steer 不拽阅读位置）；`shouldQueueComposerSlash` 让忙时 `/` 与 `!` 先排队、收束后再解析（对标 Codex Tab queue slash）；用户可见短标签 `Steer` / `Queue n` / `Send` / `Stop` / `Follow-up behavior` / `Wait for the next run` / `Steer the current run` / `Enter always sends` / `Require Cmd+Enter for multiline prompts` / `The modifier is always required` / `Plan mode` / `Toggle plan mode`（不把 TUI 长标题画进芯片） |
| `composer-submit.test.ts` | Enter/Tab 与菜单/换行、默认排队、⌘⇧Enter 反转、⌘Enter 发送、Shift+Tab 切计划不排队、审批热键、恢复上一条 / 刚提交草稿、空输入 Esc+Esc 回编、只有 send 贴底、官方 Queue / Steer 芯片与忙时占位文案 |
...
| `session-runtime.test.ts` | 队列隔离 / 编辑重排取出 / Stop-while-queued / 无对话 id 仍收口 / persist 目标 / 直播预留 id / 收束空窗 / 首枚 token 不藏尚未入列的预留行 |
...
| `automation.ts` | 自动化任务类型；`destination` 新对话或回到指定对话；`runIn` 隔离 worktree / 本地项目；可选 `providerId` / `thinkingLevel`（空则跟随当前）；可选 `rrule` 与 `workspaceIds`（对标 Codex RRULE / 同一任务多个项目）；用户可见 `Scheduled` / `All` / `Active` / `Paused` / `Run now`；`filterAutomationJobs` 对标 Scheduled All / Active / Paused；`scheduledActivityConversationIds` 给 Activity「定时」；`applyScheduledTurnSettings` 只覆盖本轮；`applyScheduledTaskAction` 供对话内创建（对标 Codex Scheduled） |
