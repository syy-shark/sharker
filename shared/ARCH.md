...
| `command-palette.ts` | ⌘K 命令面板目录（含 Find in chat、Search chats、Start dictation、Start voice chat、Open model picker、Open project picker、Open settings、Open keyboard shortcuts、Toggle sidebar / Toggle file tree / Toggle bottom panel / Toggle terminal / Open Terminal / Toggle Activity view / Toggle browser panel、Open review tab、Open browser tab、Focus Browser Address Bar / Reload Browser Page、New window、Run environment action 1、Undo last app action / Redo last app action、Navigate back / Navigate forward、Increase / Decrease / Reset font size、Clear terminal、Copy working directory / Copy session ID / Copy conversation path / Copy chat deep link / Copy as Markdown、Fork、Archive chats、`/task` `/chat` Start a chat without a project、`/compact` `/init` `/status` `/review` `/goal` `/plan` `/memories` `/reasoning` `/personality` `/approve` 用 learn.chatgpt.com slash 原文作标题、Open in Finder、Open MCP status、Local / Worktree / Create branch here、Share、Codex Documentation、Send Feedback） |
