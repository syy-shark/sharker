...
| `live-stream-slices.ts` | 直播过程/回答切片：`liveProcessIdentity` / `shouldSkipLiveProcessIdentity` / `shouldSkipLiveAnswerIdentity` / `shouldSkipLiveStreamDerivation` / `shouldGrowLiveAnswerTail` 在正文或思考只加长时跳过过程指纹、`buildAnswerParts` 与 16ms flush 的 `extractFinalContent` / 思考预览 / active tool；思考 / 状态 token 不重拆回答切片；`nextLiveProcessTimeline` 不含 `thinkText`，思考 token 不抬 TurnFlow；思考增长只换旁白、时间线引用不动；已确认无 ```demo 时 `hasStreamingDemoFenceGrowth` 只扫末行与后缀；增长比较就地比 all-but-last、不 `slice` 前缀；单段正文增长不 join 闭合块；片段引用没变则复用回答视图；操作条只订布尔；心跳同一数组则 16ms flush 不扫不发（对标 Codex #19260 / #22860） |
| `live-stream-ui.test.ts` | 相同片段引用与正文不换对象；增长才换；过程切片在正文只加长时跳过过程指纹与 buildAnswerParts、思考/状态 token 复用回答视图、16ms flush 在思考/状态/散文只加长时跳过 extractFinalContent、心跳同一数组则不扫不发、闭合回答在 token 增长时复用；DEV seed 与开轮「准备中」补丁带上片段 / 秒表 / 回答正文；收束正文补丁带 streaming / 无 activeTool / copyable；commit 不先抹秒表；`/compact` / 开轮自动压缩 / 重连 n/5 状态进过程区；`App.tsx` 不再 `setLiveSegments` / `setStreaming`（只写 store + ref）；直播写盘推迟审查「本轮」列表；打开预览不跟每条 `tool_done` 抬 App，改由文件树 `revision` 重读 |
