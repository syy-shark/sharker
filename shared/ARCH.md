...
| `reveal-in-folder.ts` | 官方 Open in Finder / Open in Explorer / Open in File Manager / Copy path：平台文案、线程项目目录、审查文件绝对路径、侧栏线程菜单项（Open in Finder / Copy as Markdown / Rename / Pin / Unpin / Archive，对标 Codex #13123 / #25646 / #33217 / #29449）、顶栏 Copy 子菜单（Copy working directory / Copy session ID / Copy deeplink / Copy as Markdown，对标 Codex #25201 / #28233）。另有官方 New chat / Find in chat / Search chats / Open command menu / Toggle sidebar / Toggle file tree / Open model picker / Start dictation / Approve request / Decline request 等快捷键文案，以及 Settings 页 General / Appearance / Notifications / Personalization / Suggested prompts / Browser / MCP servers / Share / Keep a chat near your work / Always on top / Open in Popup Window（对标 developers.openai.com/codex/app/settings / #40832 / #15162）。不接自定义 Open with |
...
| `command-palette.ts` | ⌘K 命令面板目录（含 Find in chat、Search chats、Start dictation、Start voice chat、Open model picker、Open project picker、Open settings、Open keyboard shortcuts、Toggle sidebar / Toggle file tree / Toggle bottom panel / Toggle terminal / Toggle Activity view / Toggle browser panel、Open review tab、Open browser tab、Run environment action 1、Undo last app action / Redo last app action、Navigate back / Navigate forward、Increase / Decrease / Reset font size、Clear terminal、Copy working directory / Copy session ID / Copy conversation path / Copy chat deep link / Copy as Markdown、Fork、归档当前项目对话、无项目 `/chat` `/task`、在文件管理器中显示项目、分享只读快照） |
...
| `slash-commands.ts` | 斜杠命令目录（菜单与 /help，含 /fork [local|worktree]、/side [问题]、/project、/chat（不绑定项目，对标 Codex /chat）、/task（/chat 同义）、/model、/archive、/rename、/pin、/unread、/usage、/init、/permissions（输入框下方也可切）、/memories、/copy、/fast、/reasoning、/skills、/stop、/status、/diff、/goal、/plan 切换计划模式、/plan-mode、/mcp（打开 MCP 状态；空配置打开设置 → MCP 服务器）、/feedback、/share、/local、/worktree、/approve、/subagents、/settings Open settings、/keymap Open keyboard shortcuts）；`slashItemsWithSkills` 把已安装 Skill 并进 `/` 列表；`matchUiSlashCommand` / `composerSlashLine` 给忙时排队、收束后再解析（对标 Codex Tab queue slash） |
