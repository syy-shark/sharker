...
| `process-phases.test.ts` | 思考原文不当标题；已完成步骤在后续工具增长或片段浅拷后仍是同一对象；同一工具只改短路径详情或收束只换该步（含非末步）、前缀没变或只收束思考/status 时新工具只追加一步、命令末行退回原数组；`web_search` / `web_fetch` Searching / Searched；`update_plan` 当前步；MCP Calling / Called 且 JSON 不当 detail；命令 Running / Ran；探索 Read / List / Search；写盘 Edited / Deleted / Failed to apply patch；压缩 Automatically compacting / Context automatically compacted；Ask User Scope / N questions requested；ImageView Viewed Image；重连 `Reconnecting... n/5` |
