...
| `live-display.ts` | 直播头标签/合成「规划下一步」纯函数，与 TurnFlow 共用 |
...
| `turn-segments.ts` | 流式 chunk → 有序 `TurnSegment[]` 状态机；`tool_start` 保留 `toolArgs`；`finalizeSegments` 将未完成工具标为 `cancelled` |
...
| `skill-catalog-data.ts` | Skill 目录纯数据 |
| `plugin-catalog.ts` | 汇总 MCP/Skill 目录导出与安装模板 |
