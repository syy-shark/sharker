...
| `workbench-shortcuts.ts` | 默认工作台快捷键与 `SHORTCUT_CATALOG`（设置页改绑；标题用官方 Open command menu / Toggle sidebar / Toggle bottom panel / Toggle terminal / Toggle file tree / Run environment action 1 / Toggle Activity view / Open model picker / Open project picker / Start dictation / Approve request / Decline request 等，对标 developers.openai.com/codex/app/commands 与 #20552）；Search chats 默认不绑、⌘G 留给 Find next；Esc 停止当前回合可改绑；帮助表含审批打开时 Enter Approve request / Esc Decline request、输入框 Shift+Tab 切计划、⌘↑⌘↓ / Home / End 跳对话顶底、点对话柱后滚动；浏览器聚焦刷新 / 批注；终端聚焦 ⌘K 清屏判定 |
...
| `command-palette.ts` | ⌘K 命令面板目录（含 Find in chat、Search chats、Start dictation、Start voice chat、Open model picker、Open project picker、Open settings、Open keyboard shortcuts、Toggle sidebar / Toggle file tree / Toggle bottom panel / Toggle terminal / Toggle Activity view / Toggle browser panel、Open review tab、Open browser tab、Run environment action 1、Undo last app action / Redo last app action、Navigate back / Navigate forward、Increase / Decrease / Reset font size、Clear terminal、Copy working directory / Copy session ID / Copy conversation path / Copy chat deep link / Copy as Markdown、Fork、Archive chats、无项目 `/chat` `/task`、在文件管理器中显示项目、Local / Worktree / Create branch here、Share） |
