...
| `live-display.ts` | 直播头标签/合成「规划下一步」只关思考占位、不把头闪成规划（对标 Codex flashing thinking summaries）；`正在准备…` status 头改成官方 Read / List / Running 等；开轮 status 用 Thinking（仍认旧「连接模型并准备任务」）；思考正文（去尾部 CSS）/演示可绘判断与首帧估高缓存，与 TurnFlow / InlineDemo 共用；`isNearLiveMessageRow` 标贴底窗口（不用 nth-last-child）；`shouldObserveRowIntrinsicHeight` 跳过直播行，避免 token 长高再叠一层 ResizeObserver（对标 Codex #22860 / #39120）；`shouldMeasureInlineDemoInParent` 直播中不让父页全树量 iframe 高；`shouldWalkInlineDemoTree` 直播中也不让 iframe 内扫整棵、不灌 KaTeX CDN；离开窗口后 `nextRowIntrinsicHeights` / `resolveRowIntrinsicHeight` / `rowIntrinsicSizeStyle` 用实测高度当 content-visibility 内在尺寸（第一帧不走 160px 估高）；`shouldForceStickScroll` 收束时只有真贴底才强制滚；`shouldFollowApprovalIntoView` 审批出现时读历史不抢镜头（对标 Codex #38220 / #37849）；`shouldMarkUnseenLive` / `jumpToBottomAffordance` 读历史时直播长高只换 New message 芯片（ChatView 在 composer-stage 流里占位，对标 Codex #38220 new message / #40788，不改 scrollTop、不盖直播尾）；`liveStickScrollTop` / `liveStickNeedsFollow` 在内容变高或输入框挤矮视口时跟贴底（ChatView 同时盯 `composer-stage`，对标 Codex #40788）；`selectLiveHeadStep` 跳过「规划下一步」桥接；读历史时 `clampLockedScrollTop` 只夹最大 scrollTop；`shouldMountMessageActions` / `shouldReserveMessageActions` 在正文槽上屏就占操作条；`LIVE_TAIL_SAFE_PX` 给对话柱尾留空，避免操作条被输入区阴影盖住（对标 Codex #40788 / #41155）；`shouldFollowArtifactTail` 给代码/diff 内层滚动跟尾（外壳 max-height 后新行不再顶对话柱，对标 Codex #32030 / #38695）；`codeArtifactHeadStickyTop` 给围栏复制条相对对话柱贴顶（对标 Codex #20593）；`continueLiveFenceLines` / `nextClosedFenceLines` 已完成围栏行退回同一引用（对标 Codex #39061 / #22860）；`transcriptNavIntent` 认 ⌘↑⌘↓ / Home / End 跳对话顶底（对标 Codex #39181，输入框与右侧预览不抢）；`shouldFocusTranscriptScroller` / `shouldLockStickOnTranscriptKey` 让点对话柱后方向键 / Page / Space 原生滚动且上翻锁贴底（对标 Codex 桌面 #39851）；`formatElapsedClock` / `ELAPSED_CLOCK_RESERVE_CH` 给 Goal / 长回合秒表预留「1h 59m」；`formatWorkedForLabel` 给 Working / Worked for；`formatThoughtLabel` 给 Thinking / Thought；`formatStreamingFallbackLabel` 给空档 Thinking / Working（不发明规划下一步 / 生成回答中）；`formatAwaitingApprovalLabel` 给 Awaiting approval；`formatStoppedAfterLabel` / `stoppedAfterFootnote` 给中止行 You stopped after 47m 28s（仍认旧「已停止」脚注）；`shouldFoldTurnWork` 在正文上屏后收成 Worked for；`shouldCollapseProcessOnAnswerStart` 在回答刚出现时收回用户展开的 Thought / Worked for；`sameRefList` 在回答 token 增长时保住过程数组引用 |
...
| `personality.ts` | 人格与 system 语气段；用户可见 Pragmatic / Friendly / None 与 Custom instructions（对标 Codex Settings → Personalization；旧 `empathetic` 读成 `friendly`） |
