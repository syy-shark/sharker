...
| `conversation.ts` | 对话模型、标题推导（空线程回退官方 `New chat`，旧盘 `新对话` 仍当占位再推导）、侧栏排序（置顶优先）、Search chats 扩匹配（标题 / 正文摘要 / git 分支；官方默认不绑 ⌘G）、对话路径、进行中任务拆分、⌘⌥A 先等审批再进行中、侧栏 Chronological / 进行中 / 等待回复 / 未读 / 定时 / 置顶筛选、⌘⌥U 开关 Activity、项目菜单 Archive chats 只收该项目未归档 id（可跳过进行中）、`/fork` 分叉标题与拷贝、`messagesThroughInclusive` / `canForkThroughMessage`（对标 Codex `thread/fork` `lastTurnId`，直播未完成行拒分叉）、`/fork [local|worktree]` 目标、`/rename` `/pin` 未读；`historyStartSeq` / `historyTotal` 给长线程尾页 |
| `conversation.test.ts` | 按标题 / 自定义标题 / id / 正文 / 分支过滤、进行中拆分、分叉标题与 `/fork` 目标、按消息截断拷贝、空线程 `New chat` / 旧盘「新对话」占位再推导、置顶排序、`/rename` |
...
| `live-stream-slices.ts` | 直播过程/回答切片：`liveProcessIdentity` / `shouldGrowLiveAnswerTail` 在正文只加长时跳过 `buildAnswerParts`；片段引用没变则复用回答视图；操作条只订布尔（对标 Codex #22860） |
| `live-stream-ui.test.ts` | 相同片段引用与正文不换对象；增长才换；过程切片在正文只加长时跳过 buildAnswerParts、闭合回答在 token 增长时复用；DEV seed 与开轮「准备中」补丁带上片段 / 秒表 / 回答正文；收束正文补丁带 streaming / 无 activeTool / copyable；commit 不先抹秒表；`/compact` / 开轮自动压缩 / 重连 n/5 状态进过程区；`App.tsx` 不再 `setLiveSegments` / `setStreaming`（只写 store + ref）；直播写盘推迟审查「本轮」列表；打开预览不跟每条 `tool_done` 抬 App，改由文件树 `revision` 重读 |
...
| `command-palette.ts` | ⌘K 命令面板目录（含 Find in chat、Search chats、Start dictation、Start voice chat、Open model picker、Open project picker、Open settings、Open keyboard shortcuts、Toggle sidebar / Toggle file tree / Toggle bottom panel / Toggle terminal / Open Terminal / Toggle Activity view / Toggle browser panel、Open review tab、Open browser tab、Focus Browser Address Bar / Reload Browser Page、New window、Run environment action 1、Undo last app action / Redo last app action、Navigate back / Navigate forward、Increase / Decrease / Reset font size、Clear terminal、Copy working directory / Copy session ID / Copy conversation path / Copy chat deep link / Copy as Markdown、Fork（`/fork` Copy a local chat into a new local chat or worktree）、Archive chats、`/task` `/chat` Start a chat without a project、`/compact` `/init` `/status` `/review` `/goal` `/plan` `/memories` `/reasoning` `/personality` `/approve` 用 learn.chatgpt.com slash 原文作标题、Open in Finder、Open MCP status、Local / Worktree / Create branch here、Share、Skills、Scheduled、Codex Documentation、Send Feedback） |
