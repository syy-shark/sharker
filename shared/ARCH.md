...
| `live-stream-ui.test.ts` | 相同片段引用与正文不换对象；增长才换；过程切片与闭合回答在 token 增长时复用；DEV seed 与开轮「准备中」补丁带上片段 / 秒表 / 回答正文；收束正文补丁带 streaming / 无 activeTool / copyable；commit 不先抹秒表；`/compact` / 开轮自动压缩 / 重连 n/5 状态进过程区 |
...
| `session-runtime.test.ts` | 队列隔离 / 编辑重排取出 / Stop-while-queued / persist 目标 / 直播预留 id / 收束空窗 / 首枚 token 不藏尚未入列的预留行 |
