...
| `streaming-markdown.ts` | 流式 Markdown 拆成稳定块 + 尾部；CRLF 归一；`continueStreamingMarkdown` 复用已闭合块；散文尾廉价块（标题/列表/引用/表格/分隔线）与行内（链接 / 文件引用）；`continueCheapProseBlocks` 复用已闭合列表项/表格行 |
| `streaming-markdown.test.ts` | 流式拆分：段落收束、未闭合围栏、稳定 id、增量复用、廉价行内与标题列表/表格 |
