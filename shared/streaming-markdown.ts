...
function growStreamingIndentCodeLastLine(
  ...
  const appending = suffix.startsWith('\n') || prevNorm.endsWith('\n')
  if (appending) {
    const stripped = stripIndentCodeLine(lastStreamingLine(nextText))
    const nextBody = prev.text ? `${prev.text}\n${stripped}` : stripped
