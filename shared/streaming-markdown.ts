...
/** 未闭合围栏开标加长时重读 info，避免 ``` 先挂代码尾、```demo / mermaid 闭合才换组件。 */
export function fenceLangFromOpenTail(tail: string): string | undefined {
  const nl = tail.indexOf('\n')
  const openLine = nl === -1 ? tail : tail.slice(0, nl)
  const match = parseFenceLine(openLine)
  return match ? fenceLang(match.info) : undefined
}
  ...
  if (prev.tailKind === 'prose' && rest.startsWith(prev.tail)) {
    ...
    if (shouldGrowOpenStreamingFenceTail(prev.tail, suffix)) {
      ...
      return {
        ...
        tail: rest,
        tailKind: 'fence',
        tailLang: fenceLangFromOpenTail(rest) ?? prev.tailLang,
        closedEnd
      }
    }
  }
...
/** `[^id]:` 还没写完：先不并进当前段，等定义行完整再开脚注块 */
function isPendingFootnoteDefinitionLine(line: string): boolean {
  if (parseFootnoteDefinitionLine(line)) return false
...
function growStreamingTableLastLine(
  ...
  if (!row) return null
  if (!prev.rows.length) {
    if (prevNorm.includes('\n')) return null
    const reused = reuseInlineLists(prev.header, row)
    if (reused === prev.header) return [prev]
    return [{ type: 'table', header: reused, rows: prev.rows, align: prev.align }]
  }
...
/** 段落后可另起的块（不含 Setext / HR / 表分隔；表头模样的末行仍留给整段窗口改成表） */
function lineStartsSiblingAfterParagraph(line: string, prevLastLine = ''): boolean {
  if (!line) return false
  if (isPendingSetextUnderline(line) || SETEXT_RE.test(line)) return false
  if (parseHrLine(line)) return true
  if (isGfmTableSep(line) || isPendingGfmTableSepLine(line)) return false
  if (looksLikeGfmTableCells(line) || isGfmTableRow(line)) return !looksLikeGfmTableCells(prevLastLine)
  if (isPendingListMarkerLine(line)) return false
...
/** `|` / `| -` 还没写成表行：先不并进当前段 */
function isPendingTableStartLine(line: string): boolean {
  if (!line.includes('|')) return false
  if (isGfmTableSep(line)) return false
  if (isPendingGfmTableSepLine(line)) return true
  if (looksLikeGfmTableCells(line) || isGfmTableRow(line)) return false
  return true
}

/** `#` / `##` 还没写空格：先不并进当前段 */
function isPendingHeadingStartLine(line: string): boolean {
  return /^ {0,3}#{1,6}$/.test(line)
}

/** 围栏开标还不到三个 ` / ~ */
function isPendingFenceStartLine(line: string): boolean {
  return /^ {0,3}(`{1,2}|~{1,2})$/.test(line)
}

/** 有序列表还只写出 `1`，还没到 `1.` / `1)` */
function isPendingOrderedListDigitLine(line: string): boolean {
  return /^ {0,3}\d+$/.test(line)
}

/** 段落后未写完的另起标记：先按住段落，写完再开块 */
export function isPendingSiblingStartLine(line: string): boolean {
  return (
    isPendingFootnoteDefinitionLine(line) ||
    isPendingTableStartLine(line) ||
    isPendingListMarkerLine(line) ||
    isPendingOrderedListDigitLine(line) ||
    isPendingHeadingStartLine(line) ||
    isPendingFenceStartLine(line)
  )
}

function paragraphLastLine(text: string): string {
  const trimmed = text.endsWith('\n') ? text.slice(0, -1) : text
  return trimmed.slice(trimmed.lastIndexOf('\n') + 1)
}

/** 段落后只开 `* * *` / `___` 分隔线：`---` / `===` 仍留给 Setext */
export function shouldOpenStreamingHrAfterParagraph(opts: {
  prevNorm: string
  suffix: string
}): boolean {
  const lines = paragraphSuffixNewLines(opts.prevNorm, opts.suffix)
  if (!lines) return false
  const first = lines.find((line) => line) ?? ''
  return parseHrLine(first) && !SETEXT_RE.test(first)
}

/** 段落后只开表：引用段不是表头模样时保持同一对象 */
export function shouldOpenStreamingTableAfterParagraph(opts: {
  prevNorm: string
  suffix: string
}): boolean {
  const lines = paragraphSuffixNewLines(opts.prevNorm, opts.suffix)
  ...
  const first = lines.find((line) => line) ?? ''
  return (
    lineStartsSiblingAfterParagraph(first, paragraphLastLine(opts.prevNorm)) &&
    (looksLikeGfmTableCells(first) || isGfmTableRow(first))
  )
}
...
/** 未写完的另起标记在同一行写完：段落保持同一对象，只解析新块 */
function continuePendingSiblingAfterParagraph(
  prev: Extract<CheapProseBlock, { type: 'p' }>,
  prevNorm: string,
  suffix: string,
  defs?: ReadonlyMap<string, string | CheapLinkDef>,
  ...
  if (nl < 0) return null
  const lastLine = prevNorm.slice(nl + 1)
  if (!isPendingSiblingStartLine(lastLine)) return null
  const completed = lastLine + suffix
  if (isPendingSiblingStartLine(completed)) return [prev]
  if (!lineStartsSiblingAfterParagraph(completed, paragraphLastLine(prevNorm.slice(0, nl)))) {
    return null
  }
  const para = continueParagraphPrefix(prev, prevNorm.slice(0, nl), prevNorm.slice(0, nl), defs)
  if (!para) return null
  const notes = continueAfterParagraphWithCitedFootnotes(para, completed, citedIds, defs)
  if (notes) return notes
  const siblings = parseCheapProseBlocks(completed, defs)
  if (!siblings.length) return null
  return [...para, ...siblings]
}
...
function continueLastParagraphBlock(
  ...
  if (suffix.includes('\n') || prevNorm.endsWith('\n')) {
    ...
    if (newLines && offset != null) {
      for (const line of newLines) {
        if (isPendingSiblingStartLine(line)) return [prev]
        if (lineStartsSiblingAfterParagraph(line, paragraphLastLine(nextText.slice(0, offset)))) {
  ...
  if (!suffix.includes('\n') && !prevNorm.endsWith('\n')) {
    const pending = continuePendingSiblingAfterParagraph(prev, prevNorm, suffix, defs, cites)
...
function continueLastQuoteBlock(
  ...
  if (!last) {
    ...
    const nextQuote = { type: 'quote' as const, blocks: parsed }
