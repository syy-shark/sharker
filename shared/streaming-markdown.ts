...
/** 续 ATX 标题：无换行只改正文；闭合后把后续块接到后面（对标 Codex #34045） */
function continueLastHeadingBlock(
  ...
  if (!prevH || prevH.level !== prev.level) return null
  if (!nextText.startsWith(first)) return null
...
/**
 * 增长列表 / 表格 / 段落 / 引用 / 标题 / 分隔线 / 脚注：只重解析最后一块（对标 Codex #39061 / #34045）。
 * 段落软换行后续写、嵌套项内引用 / 围栏、围栏 / 标题 / HR / 表闭合后的项后缀、引用内围栏 / 标题 / 分隔线闭合后再起的后续段、闭合并栏后再起的后续段、引用内换行后新列表项、脚注缩进续行、闭合段落 / 表 / 列表 / 分隔线后再起的后续块也走增长段。多块尾跳过已收前缀（单行标题 / HR，或最后一段原文起点，或段落后新起的列表 / 标题 / 引用 / 表 / 脚注 / 围栏）。定义行或前缀对不上时退回全量解析。
 */
 ...
 * 最后一块（含段落软换行、嵌套项内引用 / 围栏、围栏 / 标题 / HR / 表闭合后的项后缀、引用内围栏 / 标题 / 分隔线闭合后再起的后续段、闭合段落 / 表 / 列表 / 分隔线后再起的后续块、缩进代码 / 脚注续行 / 引用内换行后的子块、围栏 / 表 / 列表 / 引用 / 段落后的增长段）先走增长段；前面的标题 / 段落等保持同一引用（对标 Codex #39061 / #34045）。
