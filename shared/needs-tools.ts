...
const TOOL_KEYWORDS =
  /文件|目录|文件夹|工作区|网站|页面|网页|html|css|js|代码|项目|程序|脚本|读取|写入|搜索|创建|删除|修改|编辑|执行|运行|命令|终端|git|skill|工具|bash|npm|写一|做个|帮我|实现|开发|生成|介绍|搭建|部署|修复|bug|报错|重构|添加|新建|保存|继续|接着|微信|桌面|打开|点击|发消息|computer/i

const CONTINUATION_ONLY = /^(继续|接着|接着做|继续做|继续处理|继续执行|continue|go on|keep going)[!.?~，,\s]*$/i
...
export function needsToolCalling(userText: string, history: ChatMessage[]): boolean {
  ...
  if (CONTINUATION_ONLY.test(t)) return true
