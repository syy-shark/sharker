...
describe('process phases privacy', () => {
  it('never exposes raw thinking content as step title', () => {
    ...
    const afterResolvedCompress = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [runningCmd, awaitingDone, approvalCompressDone],
      true
    )
    ...
    const afterResolvedThinkToken = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [runningCmd, awaitingDone, nextThink, firstReply],
      true
    )
    expect(afterResolvedThinkToken).not.toBeNull()
    expect(afterResolvedThinkToken!.some((step) => step.segment === nextThink)).toBe(false)
    expect(afterResolvedThinkToken!.some((step) => step.segment === firstReply)).toBe(false)
    expect(afterResolvedThinkToken!.some((step) => step.segment === awaitingDone)).toBe(true)
    const afterAttachAllowThinkToken = appendProcessPhaseStepOnToolStart(
      approvalSteps,
      [runningCmd],
      [runningCmdAfterAllow, awaitingDone, nextThink, firstReply],
      true
    )
    expect(afterAttachAllowThinkToken).not.toBeNull()
    expect(afterAttachAllowThinkToken!.at(-1)?.segment).toBe(awaitingDone)
    expect(afterAttachAllowThinkToken!.some((step) => step.segment === firstReply)).toBe(false)
    const afterAttachAllowNext = appendProcessPhaseStepOnToolStart(
      approvalSteps,
      [runningCmd],
      [runningCmdAfterAllow, awaitingDone, cmdNext],
      true
    )
    expect(afterAttachAllowNext).not.toBeNull()
    expect(afterAttachAllowNext!.at(-1)?.segment).toBe(cmdNext)
    const afterWriteAllowToken = appendProcessPhaseStepOnToolStart(
      cmdSteps,
      [cmdRunning],
      [cmdDoneDiff, runningCmdAfterAllow, awaitingDone, firstReply],
      true
    )
    ...
    const afterWriteAllowThinkTokenCompress = appendProcessPhaseStepOnToolStart(
      cmdSteps,
      [cmdRunning],
      [cmdDoneDiff, runningCmdAfterAllow, awaitingDone, nextThink, firstReply, approvalCompressDone],
      true
    )
    expect(afterWriteAllowThinkTokenCompress).not.toBeNull()
    expect(afterWriteAllowThinkTokenCompress!.some((step) => step.segment === firstReply)).toBe(false)
    expect(afterWriteAllowThinkTokenCompress!.some((step) => step.segment === nextThink)).toBe(false)
    expect(afterWriteAllowThinkTokenCompress!.at(-1)?.segment).toBe(approvalCompressDone)
    const afterWriteAllowThinkNext = appendProcessPhaseStepOnToolStart(
      cmdSteps,
      [cmdRunning],
      [cmdDoneDiff, runningCmdAfterAllow, awaitingDone, nextThink, cmdNext],
      true
    )
    expect(afterWriteAllowThinkNext).not.toBeNull()
    expect(afterWriteAllowThinkNext!.at(-1)?.segment).toBe(cmdNext)
    expect(afterWriteAllowThinkNext!.some((step) => step.segment === nextThink)).toBe(false)
    const firstReplyCancelledWrite: TurnSegment = { ...firstReply, status: 'cancelled' }
    const runningCmdAfterAllowCancelled: TurnSegment = {
      ...runningCmdAfterAllow,
      status: 'cancelled',
      errorMessage: '任务已停止',
      resultSummary: '已停止',
    ...
    const afterWriteAllowThinkTokenCancel = appendProcessPhaseStepOnToolStart(
      cmdSteps,
      [cmdRunning],
      [cmdDoneDiff, runningCmdAfterAllowCancelled, awaitingDone, nextThink, firstReplyCancelledWrite],
      true
    )
    expect(afterWriteAllowThinkTokenCancel).not.toBeNull()
    expect(afterWriteAllowThinkTokenCancel!.some((step) => step.segment === firstReplyCancelledWrite)).toBe(
      false
    )
    expect(afterWriteAllowThinkTokenCancel!.some((step) => step.segment === awaitingDone)).toBe(true)
    const afterWriteAllowTokenCancel = appendProcessPhaseStepOnToolStart(
      cmdSteps,
      [cmdRunning],
      [cmdDoneDiff, runningCmdAfterAllowCancelled, awaitingDone, firstReplyCancelledWrite],
      true
    )
    expect(afterWriteAllowTokenCancel).not.toBeNull()
    expect(afterWriteAllowTokenCancel!.some((step) => step.segment === firstReplyCancelledWrite)).toBe(false)
    expect(afterWriteAllowTokenCancel!.some((step) => step.segment === awaitingDone)).toBe(true)
    const afterWriteAllowStatusThinkTokenCancel = appendProcessPhaseStepOnToolStart(
      cmdSteps,
      [cmdRunning],
      [
        cmdDoneDiff,
        ...
        awaitingDone,
        nextThink,
        firstReplyCancelledWrite
      ],
      true
    )
    expect(afterWriteAllowStatusThinkTokenCancel).not.toBeNull()
    expect(
      afterWriteAllowStatusThinkTokenCancel!.some((step) => step.segment === firstReplyCancelledWrite)
    ).toBe(false)
    expect(afterWriteAllowStatusThinkTokenCancel!.some((step) => step.segment === awaitingDone)).toBe(true)
    const afterWriteAllowStatusNext = appendProcessPhaseStepOnToolStart(
      cmdSteps,
      [cmdRunning],
      [cmdDoneDiff, reconnectApprovalStatus, runningCmdAfterAllow, awaitingDone, cmdNext],
      true
    )
    expect(afterWriteAllowStatusNext).not.toBeNull()
    expect(afterWriteAllowStatusNext!.at(-1)?.segment).toBe(cmdNext)
    ...
    const afterReconnectAllowThinkTokenCompress = appendProcessPhaseStepOnToolStart(
      helloApprovalHangSteps,
      [helloApprovalHang],
      [
        helloApprovalHangDone,
    ...
    expect(afterReconnectAllowThinkTokenCompress).not.toBeNull()
    expect(afterReconnectAllowThinkTokenCompress!.some((step) => step.segment === firstReply)).toBe(false)
    expect(afterReconnectAllowThinkTokenCompress!.at(-1)?.segment).toBe(approvalCompressDone)
    const afterReconnectAllowNext = appendProcessPhaseStepOnToolStart(
      helloApprovalHangSteps,
      [helloApprovalHang],
      [helloApprovalHangDone, reconnectApprovalStatus, runningCmdAfterAllow, awaitingDone, cmdNext],
      true
    )
    expect(afterReconnectAllowNext).not.toBeNull()
    expect(afterReconnectAllowNext!.at(-1)?.segment).toBe(cmdNext)
    const afterReconnectAllowThinkNext = appendProcessPhaseStepOnToolStart(
      helloApprovalHangSteps,
      [helloApprovalHang],
      [helloApprovalHangDone, reconnectApprovalStatus, runningCmdAfterAllow, awaitingDone, nextThink, cmdNext],
      true
    )
    expect(afterReconnectAllowThinkNext).not.toBeNull()
    expect(afterReconnectAllowThinkNext!.at(-1)?.segment).toBe(cmdNext)
    expect(afterReconnectAllowThinkNext!.some((step) => step.segment === nextThink)).toBe(false)
    const afterReconnectAllowTokenCancel = appendProcessPhaseStepOnToolStart(
      helloApprovalHangSteps,
      [helloApprovalHang],
      [
        helloApprovalHangDone,
    ...
    expect(afterReconnectAllowTokenCancel).not.toBeNull()
    expect(afterReconnectAllowTokenCancel!.some((step) => step.segment === firstReplyCancelledWrite)).toBe(
      false
    )
    expect(afterReconnectAllowTokenCancel!.some((step) => step.segment === awaitingDone)).toBe(true)
    const afterReconnectAllowThinkTokenCancel = appendProcessPhaseStepOnToolStart(
      helloApprovalHangSteps,
      [helloApprovalHang],
      [
        helloApprovalHangDone,
    ...
    expect(afterReconnectAllowThinkTokenCancel).not.toBeNull()
    expect(
      afterReconnectAllowThinkTokenCancel!.some((step) => step.segment === firstReplyCancelledWrite)
    ).toBe(false)
    expect(afterReconnectAllowThinkTokenCancel!.some((step) => step.segment === nextThink)).toBe(false)
    const reconnectDemoFenceCancelled: TurnSegment = {
      id: 'reconnect-fence-cancel',
      kind: 'text',
      status: 'cancelled',
      content: '\n```demo\n<div>x',
      startedAt: 18.2
    }
    const reconnectDemoCancelled: TurnSegment = { ...inlineDemo, status: 'cancelled' }
    const afterReconnectAllowDemoCancel = appendProcessPhaseStepOnToolStart(
      helloApprovalHangSteps,
      [helloApprovalHang],
      [
        helloApprovalHangDone,
    ...
    expect(afterReconnectAllowDemoCancel).not.toBeNull()
    expect(afterReconnectAllowDemoCancel!.some((step) => step.segment === reconnectDemoFenceCancelled)).toBe(
      false
    )
    expect(afterReconnectAllowDemoCancel!.some((step) => step.segment === reconnectDemoCancelled)).toBe(false)
    const afterReconnectAllowThinkSettled = appendProcessPhaseStepOnToolStart(
      helloApprovalHangSteps,
      [helloApprovalHang],
      [
        helloApprovalHangDone,
    ...
    expect(afterReconnectAllowThinkSettled).not.toBeNull()
    expect(afterReconnectAllowThinkSettled!.at(-1)?.segment).toBe(cmdNextSettled)
    expect(afterReconnectAllowThinkSettled!.some((step) => step.segment === nextThink)).toBe(false)
    ...
    const afterResolvedCancel = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdAllowedCancelled, awaitingDone],
      true
    )
    expect(afterResolvedCancel).not.toBeNull()
    ...
    const afterAttachAllowCompress = appendProcessPhaseStepOnToolStart(
      approvalSteps,
      [runningCmd],
      [runningCmdAfterAllow, awaitingDone, approvalCompressDone],
      true
    )
    expect(afterAttachAllowCompress).not.toBeNull()
    expect(afterAttachAllowCompress!.at(-1)?.segment).toBe(approvalCompressDone)
    const afterAttachAllowCancel = appendProcessPhaseStepOnToolStart(
      approvalSteps,
      [runningCmd],
      [cmdAllowedCancelled, awaitingDone],
      true
    )
    expect(afterAttachAllowCancel).not.toBeNull()
    expect(afterAttachAllowCancel!.at(-1)?.segment).toBe(awaitingDone)
    const afterWriteAllowCompress = appendProcessPhaseStepOnToolStart(
      cmdSteps,
      [cmdRunning],
      [cmdDoneDiff, cmdAwaiting, awaitingDone, approvalCompressDone],
      true
    )
    expect(afterWriteAllowCompress).not.toBeNull()
    expect(afterWriteAllowCompress![0].segment).toBe(cmdDoneDiff)
    expect(afterWriteAllowCompress!.at(-1)?.segment).toBe(approvalCompressDone)
    const afterWriteAllowCancel = appendProcessPhaseStepOnToolStart(
      cmdSteps,
      [cmdRunning],
      [cmdDoneDiff, cmdAllowedCancelled, awaitingDone],
      true
    )
    expect(afterWriteAllowCancel).not.toBeNull()
    expect(afterWriteAllowCancel![0].segment).toBe(cmdDoneDiff)
    expect(afterWriteAllowCancel!.some((step) => step.segment === cmdAllowedCancelled)).toBe(true)
    const afterReconnectAllowCompress = appendProcessPhaseStepOnToolStart(
      helloApprovalHangSteps,
      [helloApprovalHang],
      [
        helloApprovalHangDone,
    ...
    expect(afterReconnectAllowCompress).not.toBeNull()
    expect(afterReconnectAllowCompress!.at(-1)?.segment).toBe(approvalCompressDone)
    const afterReconnectAllowCancel = appendProcessPhaseStepOnToolStart(
      helloApprovalHangSteps,
      [helloApprovalHang],
      [helloApprovalHangDone, reconnectApprovalStatus, cmdAllowedCancelled, awaitingDone],
      true
    )
    expect(afterReconnectAllowCancel).not.toBeNull()
    expect(afterReconnectAllowCancel!.some((step) => step.segment === cmdAllowedCancelled)).toBe(true)
    ...
    const denyErrorText: TurnSegment = {
      id: 'deny-err-1',
      kind: 'text',
      status: 'done',
      role: 'final',
    ...
    const denyDemoFence: TurnSegment = {
      id: 'deny-fence-1',
      kind: 'text',
      status: 'active',
      content: '\n```demo\n<div>x',
    ...
    const afterResolvedThinkErrorDemo = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [runningCmd, awaitingDone, nextThink, denyErrorText, denyDemoFence, inlineDemo],
      true
    )
    expect(afterResolvedThinkErrorDemo).not.toBeNull()
    expect(afterResolvedThinkErrorDemo!.some((step) => step.segment === denyErrorText)).toBe(false)
    expect(afterResolvedThinkErrorDemo!.some((step) => step.segment === denyDemoFence)).toBe(false)
    const afterResolvedThinkDemoError = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [runningCmd, awaitingDone, nextThink, denyDemoFence, inlineDemo, denyErrorText],
      true
    )
    expect(afterResolvedThinkDemoError).not.toBeNull()
    expect(afterResolvedThinkDemoError!.some((step) => step.segment === denyErrorText)).toBe(false)
    expect(afterResolvedThinkDemoError!.some((step) => step.segment === denyDemoFence)).toBe(false)
    const afterDeniedThinkDemoCompress = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdDenied, awaitingDenied, nextThink, denyDemoFence, inlineDemo, approvalCompressDone],
      true
    )
    expect(afterDeniedThinkDemoCompress).not.toBeNull()
    expect(afterDeniedThinkDemoCompress!.some((step) => step.segment === denyDemoFence)).toBe(false)
    expect(afterDeniedThinkDemoCompress!.at(-1)?.segment).toBe(approvalCompressDone)
    const afterDeniedThinkErrorCompress = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdDenied, awaitingDenied, nextThink, denyErrorText, approvalCompressDone],
      true
    )
    expect(afterDeniedThinkErrorCompress).not.toBeNull()
    expect(afterDeniedThinkErrorCompress!.some((step) => step.segment === denyErrorText)).toBe(false)
    expect(afterDeniedThinkErrorCompress!.at(-1)?.segment).toBe(approvalCompressDone)
    const afterDeniedThinkTokenCancel = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdDenied, awaitingDenied, nextThink, firstReplyCancelledWrite],
      true
    )
    expect(afterDeniedThinkTokenCancel).not.toBeNull()
    expect(afterDeniedThinkTokenCancel!.some((step) => step.segment === firstReplyCancelledWrite)).toBe(false)
    expect(afterDeniedThinkTokenCancel!.some((step) => step.segment === awaitingDenied)).toBe(true)
    const afterDeniedTokenCancel = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdDenied, awaitingDenied, firstReplyCancelledWrite],
      true
    )
    ...
    const afterWriteAllowDemoCancel = appendProcessPhaseStepOnToolStart(
      cmdSteps,
      [cmdRunning],
      [cmdDoneDiff, runningCmdAfterAllowCancelled, awaitingDone, reconnectDemoFenceCancelled, reconnectDemoCancelled],
      true
    )
    expect(afterWriteAllowDemoCancel).not.toBeNull()
    expect(afterWriteAllowDemoCancel!.some((step) => step.segment === reconnectDemoFenceCancelled)).toBe(false)
    const afterWriteAllowStatusThinkErrorCancel = appendProcessPhaseStepOnToolStart(
      cmdSteps,
      [cmdRunning],
      [
        cmdDoneDiff,
    ...
    expect(afterWriteAllowStatusThinkErrorCancel).not.toBeNull()
    expect(afterWriteAllowStatusThinkErrorCancel!.some((step) => step.segment === denyErrorText)).toBe(false)
    const afterWriteAllowStatusThinkDemoCancel = appendProcessPhaseStepOnToolStart(
      cmdSteps,
      [cmdRunning],
      [
        cmdDoneDiff,
    ...
    expect(afterWriteAllowStatusThinkDemoCancel).not.toBeNull()
    expect(afterWriteAllowStatusThinkDemoCancel!.some((step) => step.segment === reconnectDemoFenceCancelled)).toBe(
      false
    )
    const afterWriteAllowStatusThinkErrorCompress = appendProcessPhaseStepOnToolStart(
      cmdSteps,
      [cmdRunning],
      [
        cmdDoneDiff,
    ...
    const afterReconnectDenyNext = appendProcessPhaseStepOnToolStart(
      helloApprovalHangSteps,
      [helloApprovalHang],
      [helloApprovalHangDone, reconnectApprovalStatus, cmdDenied, awaitingDenied, cmdNext],
      true
    )
    expect(afterReconnectDenyNext).not.toBeNull()
    expect(afterReconnectDenyNext!.at(-1)?.segment).toBe(cmdNext)
    ...
    const afterAllowedSettleTokenCancel = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdAllowedSettled, awaitingDone, firstReplyCancelledWrite],
      true
    )
    expect(afterAllowedSettleTokenCancel).not.toBeNull()
    expect(afterAllowedSettleTokenCancel!.some((step) => step.segment === firstReplyCancelledWrite)).toBe(
      false
    )
    expect(afterAllowedSettleTokenCancel!.some((step) => step.segment === awaitingDone)).toBe(true)
    const afterAllowedSettleErrorCancel = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdAllowedSettled, awaitingDone, denyErrorText],
      true
    )
    ...
    const afterAllowedSettleThinkDemoCancel = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdAllowedSettled, awaitingDone, nextThink, reconnectDemoFenceCancelled, reconnectDemoCancelled],
      true
    )
    expect(afterAllowedSettleThinkDemoCancel).not.toBeNull()
    expect(afterAllowedSettleThinkDemoCancel!.some((step) => step.segment === reconnectDemoFenceCancelled)).toBe(
      false
    )
    const afterAllowedSettleThinkErrorDemo = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdAllowedSettled, awaitingDone, nextThink, denyErrorText, denyDemoFence, inlineDemo],
      true
    )
    expect(afterAllowedSettleThinkErrorDemo).not.toBeNull()
    expect(afterAllowedSettleThinkErrorDemo!.some((step) => step.segment === denyErrorText)).toBe(false)
    expect(afterAllowedSettleThinkErrorDemo!.some((step) => step.segment === denyDemoFence)).toBe(false)
    const afterAllowedSettleThinkDemoError = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdAllowedSettled, awaitingDone, nextThink, denyDemoFence, inlineDemo, denyErrorText],
      true
    )
    expect(afterAllowedSettleThinkDemoError).not.toBeNull()
    expect(afterAllowedSettleThinkDemoError!.some((step) => step.segment === denyErrorText)).toBe(false)
    expect(afterAllowedSettleThinkDemoError!.some((step) => step.segment === denyDemoFence)).toBe(false)
    const afterAllowedSettleThinkErrorDemoCancel = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdAllowedSettled, awaitingDone, nextThink, denyErrorText, reconnectDemoFenceCancelled, reconnectDemoCancelled],
      true
    )
    expect(afterAllowedSettleThinkErrorDemoCancel).not.toBeNull()
    expect(afterAllowedSettleThinkErrorDemoCancel!.some((step) => step.segment === denyErrorText)).toBe(false)
    expect(afterAllowedSettleThinkErrorDemoCancel!.some((step) => step.segment === reconnectDemoFenceCancelled)).toBe(
      false
    )
    const afterDeniedThinkDemoCancel = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdDenied, awaitingDenied, nextThink, reconnectDemoFenceCancelled, reconnectDemoCancelled],
      true
    )
    expect(afterDeniedThinkDemoCancel).not.toBeNull()
    expect(afterDeniedThinkDemoCancel!.some((step) => step.segment === reconnectDemoFenceCancelled)).toBe(false)
    const afterDeniedThinkErrorDemo = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdDenied, awaitingDenied, nextThink, denyErrorText, denyDemoFence, inlineDemo],
      true
    )
    expect(afterDeniedThinkErrorDemo).not.toBeNull()
    expect(afterDeniedThinkErrorDemo!.some((step) => step.segment === denyErrorText)).toBe(false)
    expect(afterDeniedThinkErrorDemo!.some((step) => step.segment === denyDemoFence)).toBe(false)
    const afterDeniedThinkDemoError = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdDenied, awaitingDenied, nextThink, denyDemoFence, inlineDemo, denyErrorText],
      true
    )
    expect(afterDeniedThinkDemoError).not.toBeNull()
    expect(afterDeniedThinkDemoError!.some((step) => step.segment === denyErrorText)).toBe(false)
    expect(afterDeniedThinkDemoError!.some((step) => step.segment === denyDemoFence)).toBe(false)
    const afterDeniedThinkErrorDemoCancel = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdDenied, awaitingDenied, nextThink, denyErrorText, reconnectDemoFenceCancelled, reconnectDemoCancelled],
      true
    )
    expect(afterDeniedThinkErrorDemoCancel).not.toBeNull()
    expect(afterDeniedThinkErrorDemoCancel!.some((step) => step.segment === denyErrorText)).toBe(false)
    expect(afterDeniedThinkErrorDemoCancel!.some((step) => step.segment === reconnectDemoFenceCancelled)).toBe(
      false
    )
    const afterDeniedThinkErrorCancel = appendProcessPhaseStepOnToolStart(
      afterApproval!,
      [cmdAwaiting, awaitingStatus],
      [cmdDenied, awaitingDenied, nextThinkCancelledReconnect, denyErrorText],
      true
    )
    expect(afterDeniedThinkErrorCancel).not.toBeNull()
    expect(afterDeniedThinkErrorCancel!.some((step) => step.segment === denyErrorText)).toBe(false)
    ...
    const afterWriteAllowStatusAsk = appendProcessPhaseStepOnToolStart(
      cmdSteps,
      [cmdRunning],
      [cmdDoneDiff, reconnectApprovalStatus, runningCmdAfterAllow, awaitingDone, askTool, askStatus],
      true
    )
    ...
    const afterAskResolveCancel = appendProcessPhaseStepOnToolStart(
      helloAskHangSteps,
      [helloAskHang],
      [helloAskHangDone, askReadyCancelled, askStatusDoneForHang],
      true
    )
    ...
    const demoFenceReply: TurnSegment = {
      ...firstReply,
      content: '\n```demo\n<div>x'
    }
    const afterAskResolveThinkDemoCompress = appendProcessPhaseStepOnToolStart(
      helloAskHangSteps,
      [helloAskHang],
      [
        helloAskHangDone,
    ...
    expect(afterAskResolveThinkDemoCompress).not.toBeNull()
    ...
    const afterAskResolveThinkDemoErrorCancelCompress = appendProcessPhaseStepOnToolStart(
      helloAskHangSteps,
      [helloAskHang],
      [
        helloAskHangDone,
    ...
    expect(afterAskResolveThinkDemoErrorCancelCompress).not.toBeNull()
    expect(afterAskResolveThinkDemoErrorCancelCompress!.some((step) => step.segment === denyErrorText)).toBe(
      false
    )
    expect(
      afterAskResolveThinkDemoErrorCancelCompress!.some((step) => step.segment === reconnectDemoFenceCancelled)
    ).toBe(false)
    expect(afterAskResolveThinkDemoErrorCancelCompress!.at(-1)?.segment).toBe(approvalCompressDone)
    const afterAskResolveDemoErrorCancel = appendProcessPhaseStepOnToolStart(
      helloAskHangSteps,
      [helloAskHang],
      [
        helloAskHangDone,
    ...
    expect(afterAskResolveDemoErrorCancel).not.toBeNull()
    expect(afterAskResolveDemoErrorCancel!.some((step) => step.segment === denyErrorText)).toBe(false)
    expect(afterAskResolveDemoErrorCancel!.some((step) => step.segment === reconnectDemoFenceCancelled)).toBe(
      false
    )
    const afterAskResolveErrorDemoCompress = appendProcessPhaseStepOnToolStart(
      helloAskHangSteps,
      [helloAskHang],
      [helloAskHangDone, askReady, askStatusDoneForHang, denyErrorText, denyDemoFence, inlineDemo, approvalCompressDone],
      true
    )
    expect(afterAskResolveErrorDemoCompress).not.toBeNull()
    expect(afterAskResolveErrorDemoCompress!.some((step) => step.segment === denyErrorText)).toBe(false)
    expect(afterAskResolveErrorDemoCompress!.some((step) => step.segment === denyDemoFence)).toBe(false)
    expect(afterAskResolveErrorDemoCompress!.at(-1)?.segment).toBe(approvalCompressDone)
    const afterAskResolveThinkErrorCancel = appendProcessPhaseStepOnToolStart(
      helloAskHangSteps,
      [helloAskHang],
      [helloAskHangDone, askReadyCancelled, askStatusDoneForHang, nextThinkCancelledAskResolve, denyErrorText],
      true
    )
    expect(afterAskResolveThinkErrorCancel).not.toBeNull()
    expect(afterAskResolveThinkErrorCancel!.some((step) => step.segment === denyErrorText)).toBe(false)
    const afterAskResolveErrorCancel = appendProcessPhaseStepOnToolStart(
      helloAskHangSteps,
      [helloAskHang],
      [helloAskHangDone, askReadyCancelled, askStatusDoneForHang, denyErrorText],
      true
    )
    expect(afterAskResolveErrorCancel).not.toBeNull()
    expect(afterAskResolveErrorCancel!.some((step) => step.segment === denyErrorText)).toBe(false)
    const afterAskResolveDemoCancel = appendProcessPhaseStepOnToolStart(
      helloAskHangSteps,
      [helloAskHang],
      [helloAskHangDone, askReadyCancelled, askStatusDoneForHang, reconnectDemoFenceCancelled, reconnectDemoCancelled],
      true
    )
    expect(afterAskResolveDemoCancel).not.toBeNull()
    expect(afterAskResolveDemoCancel!.some((step) => step.segment === reconnectDemoFenceCancelled)).toBe(false)
    const afterAskResolveNext = appendProcessPhaseStepOnToolStart(
      helloAskHangSteps,
      [helloAskHang],
      [helloAskHangDone, askReady, askStatusDoneForHang, cmdNext],
      true
    )
    expect(afterAskResolveNext).not.toBeNull()
    expect(afterAskResolveNext!.at(-1)?.segment).toBe(cmdNext)
    const afterAskResolveThinkNext = appendProcessPhaseStepOnToolStart(
      helloAskHangSteps,
      [helloAskHang],
      [helloAskHangDone, askReady, askStatusDoneForHang, nextThink, cmdNext],
      true
    )
    expect(afterAskResolveThinkNext).not.toBeNull()
    expect(afterAskResolveThinkNext!.at(-1)?.segment).toBe(cmdNext)
    expect(afterAskResolveThinkNext!.some((step) => step.segment === nextThink)).toBe(false)
    const afterReconnectAllowAsk = appendProcessPhaseStepOnToolStart(
      helloApprovalHangSteps,
      [helloApprovalHang],
      [helloApprovalHangDone, reconnectApprovalStatus, runningCmdAfterAllow, awaitingDone, askReady, askStatus],
      true
    )
    expect(afterReconnectAllowAsk).not.toBeNull()
    expect(afterReconnectAllowAsk!.some((step) => step.segment === askReady)).toBe(true)
    expect(afterReconnectAllowAsk!.some((step) => step.segment === askStatus)).toBe(true)
    const afterReconnectDenyAsk = appendProcessPhaseStepOnToolStart(
      helloApprovalHangSteps,
      [helloApprovalHang],
      [helloApprovalHangDone, reconnectApprovalStatus, cmdDenied, awaitingDenied, askReady, askStatus],
      true
    )
    expect(afterReconnectDenyAsk).not.toBeNull()
    expect(afterReconnectDenyAsk!.some((step) => step.segment === askReady)).toBe(true)
    expect(afterReconnectDenyAsk!.some((step) => step.segment === askStatus)).toBe(true)
    const afterPlanAllowNext = appendProcessPhaseStepOnToolStart(
      helloApprovalHangSteps,
      [helloApprovalHang],
      [helloApprovalHangDone, planStatusDone, runningCmdAfterAllow, awaitingDone, cmdNext],
      true
    )
    expect(afterPlanAllowNext).not.toBeNull()
    expect(afterPlanAllowNext!.at(-1)?.segment).toBe(cmdNext)
    const afterPlanAllowAsk = appendProcessPhaseStepOnToolStart(
      helloApprovalHangSteps,
      [helloApprovalHang],
      [helloApprovalHangDone, planStatusDone, runningCmdAfterAllow, awaitingDone, askReady, askStatus],
      true
    )
    expect(afterPlanAllowAsk).not.toBeNull()
    expect(afterPlanAllowAsk!.some((step) => step.segment === askReady)).toBe(true)
    expect(afterPlanAllowAsk!.some((step) => step.segment === askStatus)).toBe(true)
    const firstReplyCancelled: TurnSegment = { ...firstReply, status: 'cancelled' }
    const afterAskHangTokenCancelCompress = appendProcessPhaseStepOnToolStart(
      helloAskHangSteps,
      [helloAskHang],
      [helloAskHangDone, askReadyCancelled, askStatusDoneForHang, firstReplyCancelled, approvalCompressDone],
      true
    )
    expect(afterAskHangTokenCancelCompress).not.toBeNull()
    expect(afterAskHangTokenCancelCompress!.some((step) => step.segment === firstReplyCancelled)).toBe(false)
    expect(afterAskHangTokenCancelCompress!.at(-1)?.segment).toBe(approvalCompressDone)
    const afterWriteAllowAsk = appendProcessPhaseStepOnToolStart(
      cmdSteps,
      [cmdRunning],
      [cmdDoneDiff, runningCmdAfterAllow, awaitingDone, askReady, askStatus],
      true
    )
    expect(afterWriteAllowAsk).not.toBeNull()
    expect(afterWriteAllowAsk!.some((step) => step.segment === awaitingDone)).toBe(true)
    expect(afterWriteAllowAsk!.some((step) => step.segment === askReady)).toBe(true)
