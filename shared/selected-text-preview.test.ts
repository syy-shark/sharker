...
describe('selected-text-preview', () => {
  it('stages official Selection chips and wraps submit as # Selected text:', () => {
    ...
    const chatViewSrc = readFileSync(new URL('../src/components/ChatView.tsx', import.meta.url), 'utf8')
    expect(chatViewSrc).toContain('title={selectedTextTitle(index)}')
    expect(chatViewSrc).toContain('SELECTED_TEXT_COMMENT_LABEL')
    expect(chatViewSrc).not.toContain('回到原文')
    expect(chatViewSrc).not.toContain('备注：')
