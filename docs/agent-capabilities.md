...
划选历史正文、**直播已出现正文**、文件预览或集成终端输出会出现「加入对话」与「旁路提问」（对标 Codex Add to chat / Ask in side chat / #37560）。加入对话进 composer `Selection N` 芯片，不把长摘录灌进输入框；发送收成官方 `# Selected text:` / `## My request for Codex:`。对话柱只画请求与可再点开的 annotation 芯片，长划选不撑开贴底（对标 Codex selected-text references remain after sending / #22670 / #20294）。芯片可预览、加备注、移除，或「插入正文」回退成引用块（备注对标 Codex response annotation comments / #33763，不发明 #22677 划选跟帖气泡）。`/` 与 `!` 仍清芯片。旁路提问仍把摘录交给新线程。复制为 Markdown 仍带全文划选块。
