...
- 长线程先画最近一段，上滑到顶再揭示更早消息并保住滚动锚（对标 Codex Improved loading for long threads / older history fetched as needed；不预挂全量、没有「加载更早」按钮）。线程查找、⌘↑ 到顶、回编上一条会扩到目标行；查找仍扫全部消息，只是未滚到的旧行先不挂 DOM
