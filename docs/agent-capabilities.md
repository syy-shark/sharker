...
- 设置 → 外观 → **Reduce Motion**（对标 Codex #16857）：关掉直播思考扫光，减轻 GPU；进度圈仍转（对标 #22787）。不跟系统辅助功能绑定
