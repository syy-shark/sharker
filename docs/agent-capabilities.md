...
- 写盘预览一开始对话里就出现 **已改 N 个文件**（对标 Codex 回合内 N files edited / 约 0.5s 逐文件），点开审查切到本轮全部仓库；数字与直播 +/- 统计预留等宽，侧栏进行中/未读占同一槽，文件树写盘重拉不再播进入动画（对标 Codex animated diff stat alignment / sidebar jitter）；收束不再整块冒出跳贴底
