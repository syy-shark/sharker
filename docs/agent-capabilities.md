...
- 长线程打开时只从库里取最近一段（对标 Codex `excludeTurns` + `initialTurnsPage`），上滑到顶再取更早一页并保住滚动锚（对标 Codex `thread/turns/list` / older history fetched as needed；不预拉全量、没有「加载更早」按钮、不在后台抽干）。启动窗再按约 50KiB 人类可读预算瘦身：用户/助手正文走快路径，过长命令输出与思考点开「查看输出」/「已思考」再取（对标 Codex #38653）；落盘跳过仍是占位的消息，以免空壳盖掉库里的全文。⌘F 查找只扫用户/助手正文、不回放整段（对标 Codex #33907）；跳到未加载的命中再揭开该 seq 起的一段。发送 / 压缩 / 分叉从库取未瘦身全文给模型，不灌进当前 DOM（官方分页线程已弃用全量水合）；⌘↑ 到顶揭开已瘦身全文后只挂最早一段滑动窗（约 70 行），下翻再滑向尾页，不把整段灌进 React；直播中不揭开全文以免卡住贴底，收束后再揭；点开输出/思考再取一条原文。落盘带 `historyStartSeq`，不删尚未加载的更早页。查找把窗口滑到命中行，不从命中铺到最新
