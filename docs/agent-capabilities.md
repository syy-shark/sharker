...
- 设置 → 外观 → **代码字体**（对标 Codex Code font）：审查、终端与对话代码共用 `--mono`；`sharker://settings/code-font` 打开该页。只换等宽栈，不改主题色
...
- 直播思考默认折叠成「思考中」（对标 Codex Thought），点开才看旁白，避免增长正文把回答顶下去；散文尾廉价画 GFM 表格、任务列表、http 图与分隔线，并复用已闭合列表项/表格行；廉价尾与收束后共用块边距 / 任务列表 class，正文出现后收起命令输出，贴底在布局后同帧写 scrollTop
