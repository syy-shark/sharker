...
- 长线程打开时只从库里取最近一段（对标 Codex `excludeTurns` + `initialTurnsPage`），上滑到顶再取更早一页并保住滚动锚（对标 Codex `thread/turns/list` / older history fetched as needed；不预拉全量、没有「加载更早」按钮、不在后台抽干）。发送 / 压缩 / 分叉 / 查找 / ⌘↑ 到顶再补全量，避免模型丢上下文；落盘带 `historyStartSeq`，不删尚未加载的更早页。DOM 仍只挂最近一段，查找仍先扩到目标行
