...
| `present_inline_demo` | 把自包含 HTML/CSS/JS **嵌进对话**做演示；工具一开始就占演示槽（直播开槽不重拆过程 / 回答，未可绘先 96px 骨架叠在同一 iframe 上，HTML 增长与收束只换该槽，可绘只换 srcDoc）；正文 ```demo 围栏未写完 `dem` / `viz` 就占同一 `demo-stream` 槽（不先当散文再跳；不认 ```diff / ```html / ```vim；围栏开槽仍整拆），开闭都挂 `InlineDemo`；首帧按声明高度 / 块数估高并缓存实测，避免 48px 猛涨顶跳贴底；教学/可视化请用此工具，不要写文件再开浏览器 |
