...
- 闭合的 ```mermaid / ```mmd 围栏在对话里内联成图（对标 Codex transcript Mermaid）；开闭共用同一 `MermaidBlock` 与 `CodeArtifactShell`，未闭合与成图前都显示代码尾，成图只换体内 SVG；成图前按节点/边/行数估高占位，成图后高度只升不降，避免代码尾换成图时把贴底顶跳；解析失败仍留同一外壳的代码尾（不换一套代码块）；按主题缓存 SVG，收束重挂时首帧仍是图，避免闪回源码
...
| `present_inline_demo` | 把自包含 HTML/CSS/JS **嵌进对话**做演示；工具一开始就占演示槽（未可绘先 96px 骨架叠在同一 iframe 上，可绘只换 srcDoc）；正文 ```demo 围栏开闭都挂 `InlineDemo`（不先画代码围栏再换组件）；首帧按声明高度 / 块数估高并缓存实测，避免 48px 猛涨顶跳贴底；教学/可视化请用此工具，不要写文件再开浏览器 |
