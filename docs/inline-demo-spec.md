...
- 内联演示在对话里 **默认完整展开**，像 Markdown 一样嵌在正文里；**不要**在演示块内部出现滚动条。
- 宿主 iframe 高度 = 内容真实底边（含嵌套 flex/grid 卡片），无 640px 一类硬顶。字体、图片、布局完成后再量；高度 **只升不降**。
- 模型根节点若写了 `overflow:hidden` + 固定高度，宿主会改成 `height:auto; max-height:none; overflow:visible`（canvas 除外）。
- **禁止**用大 `min-height`（如 400px+ 空画布）撑出聊天里「演示下方一大片白」。
- 根 / 主卡片高度应 **由内容决定**；底部 padding 控制在约 8–16px。
- 生成侧不要写 `overflow: auto` + 固定高度的大滚动容器。
- 可选「收起」只是演示 **后面** 的一行小字，不是包住演示的标题栏。

### 2.3 多栏布局（如 Git 四区）

- 使用 `display: flex` + `flex-wrap: wrap` 或 `grid`，子项 `flex: 1 1 Npx` + `min-width: 0`。
- **禁止**中间栏被挤成竖条、文字竖排溢出、栏重叠。
- 每张区卡片的标题和两行说明必须完整可见（宿主按内容撑开 iframe，不裁切）。
- 窄宽时允许换行成 1 列，而不是重叠。

### 2.4 与正文的关系

- 直播时：思考/工具在上，正文与演示在下（对齐 Cursor）。
- 完成后：正文在上；「已思考 · Ns」折叠条贴在正文上方；工具过程摘要在正文下方。
- 演示 **无外框**：不要整页灰卡片、不要边框包一层、不要「收起/展开」标题栏。`body` 透明，贴进聊天背景，和上下文字同一块，不要「单独开了一扇窗」的感觉。
- 四区 / 按钮等 **内层** 控件可以有自己的表面；外层 `.card` 壳必须消失。
- 演示块与上下 Markdown 段落间距紧凑（宿主约 8–10px），不要演示底自己再留 80px+ 空 margin。
- 流式尚未出现真实结构节点（只有 CSS 声明）时 **不要** 把半截样式画成代码卡片。
...
- 浅色：贴近水滴玻璃（半透、细边）；深色：金属感。用宿主 CSS 变量（`--bg` / `--surface` / `--surface-nested` / `--border`），不要写死 `#111` / `#1a1a1a` / `#222` 整页底（假终端窗体除外，由宿主套壳）。
- `body` 背景必须 **transparent**，融入聊天背景；卡片用 `--surface`，不要另起一块灰。
- **禁止**给整段演示套外层假卡片 / 假窗口（大圆角灰底 + 描边）。内层交互控件（如 Git 四区小卡）可以有自己的表面。
- 宿主会把中性灰/黑底 remap 到上述 token，按内容真实底边撑开 iframe（禁止内部裁切、禁止 iframe 滚动条）。
...
| `src/components/InlineDemo.tsx` | iframe 宿主：无外框、透明底、按内容底边撑高 |
