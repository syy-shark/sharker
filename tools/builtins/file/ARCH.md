...
| `view-image.ts` | 官方 `view_image`：校验本地图并写短结果；过程标题 Viewed Image；相对路径接 cwd（#29526）；像素由 query-loop 回灌（对标 Codex #36966） |
| `view-image.test.ts` | 短结果、相对路径、空格路径、`original`、非图/缺失、沙箱拒绝、计划模式白名单 |
