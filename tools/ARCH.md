...
| `thread-worktree.ts` | 为会话创建或复用 `~/.sharker/worktrees` 隔离 worktree；可选起点分支；按 `.worktreeinclude` 拷被忽略文件；默认保留最近 15 个并在删除前快照；归档可移除；项目菜单可建永久 worktree（`worktrees/permanent/`，不自动删） |
