...
| `thread-worktree.ts` | 为会话创建或复用隔离 worktree（默认 `~/.sharker/worktrees`，可设 `worktreeRoot`）；可选起点分支；按 `.worktreeinclude` 拷被忽略文件；新建时跑 `[setup] script`，归档/裁掉托管 worktree 前先快照再跑 `[cleanup] script`（对标 Codex Local environments / #19480）。`[[actions]]` 由顶栏 / ⌘⇧D 在集成终端跑，不在本文件解析 |
